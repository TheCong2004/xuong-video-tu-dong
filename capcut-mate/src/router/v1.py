...
from src.schemas.publish_draft import PublishDraftRequest, PublishDraftResponse
import asyncio
...
# Phase 1: all Mate business goes through engines.mate.facade (re-exports src.service)
from engines.mate import facade as service
...
@router.post(path="/publish_draft", response_model=PublishDraftResponse)
def publish_draft(pdr: PublishDraftRequest) -> PublishDraftResponse:
    try:
        result = service.publish_draft(pdr.staging_path, pdr.draft_id)
        logger.info(
            "PUBLISH DRAFT: draft_id=%s staging_path=%s final_path=%s",
            pdr.draft_id,
            result["staging_path"],
            result["final_path"],
        )
        return PublishDraftResponse(**result)
    except service.DraftPublishError as error:
        status = 404 if error.code == "CAPCUT_DESKTOP_ROOT_NOT_FOUND" else 422
        if error.code == "CAPCUT_DRAFT_PUBLISH_FAILED":
            status = 500
        raise HTTPException(
            status_code=status,
...
def create_draft(cdr: CreateDraftRequest) -> CreateDraftResponse:
    ...
    draft_path = os.path.realpath(os.path.join(config.DRAFT_DIR, draft_id))
    logger.info(f"CREATE DRAFT: draft_id={draft_id}, draft_path={draft_path}")
