...
pub async fn get_dashboard_summary(
  ...
  // 1. Authoritative Job aggregation query
  let job_row: JobAggregateRow = sqlx::query_as(
    r#"
    SELECT
      COUNT(*) AS total,
      COUNT(CASE WHEN status = 'QUEUED' OR status = 'pending' THEN 1 END) AS queued,
      COUNT(CASE WHEN status = 'WAITING_WORKER' THEN 1 END) AS waiting_worker,
      COUNT(CASE WHEN (current_stage = 'GROK_IMAGE_EDIT' OR current_stage = 'generating_image') AND (status = 'RUNNING' OR status = 'started') THEN 1 END) AS generating_image,
      COUNT(CASE WHEN (current_stage = 'GROK_EXPAND_9_16' OR current_stage = 'converting_9_16') AND (status = 'RUNNING' OR status = 'started') THEN 1 END) AS converting_9_16,
      COUNT(CASE WHEN (current_stage = 'GROK_VIDEO_GENERATE' OR current_stage = 'generating_video') AND (status = 'RUNNING' OR status = 'started') THEN 1 END) AS generating_video,
      COUNT(CASE WHEN (current_stage = 'AUTO_DOWNLOAD' OR current_stage = 'downloading') AND (status = 'RUNNING' OR status = 'started') THEN 1 END) AS downloading,
      COUNT(CASE WHEN (current_stage = 'SAVE_LOCAL' OR current_stage = 'saving_local') AND (status = 'RUNNING' OR status = 'started') THEN 1 END) AS saving_local,
      COUNT(CASE WHEN status = 'READY_TO_POST' THEN 1 END) AS ready_to_post,
      COUNT(CASE WHEN status = 'DONE' OR status = 'COMPLETED' OR status = 'complete_success' THEN 1 END) AS done,
      COUNT(CASE WHEN status = 'ERROR' OR status = 'FAILED' OR status = 'complete_failure' OR status = 'dead' THEN 1 END) AS error,
