...
pub async fn update_task_status_with_rich_failure(args: UpdateTaskWithRichFailureArgs<'_>) -> Result<bool, SqliteTasksError> {
  ...
  let maybe_failure_message_temp = args.maybe_failure_message.map(|s| if s.len() > 255 { &s[..255] } else { s });
