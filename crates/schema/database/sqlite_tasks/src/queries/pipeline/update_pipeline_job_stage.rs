...
mod tests {
  ...
  fn stage_only_update_preserves_persisted_artifact_outputs() {
    tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async {
      ...
      let id = create_pipeline_job(CreatePipelineJobArgs { db: &db, status: TaskStatus::Pending, current_stage: PipelineStage::IngestAnalyze, maybe_page_id: None, maybe_input_payload: None, maybe_page_snapshot: None, maybe_business_status: None }).await.unwrap();
      let outputs = r#"{"ingest_analyze":{"artifact_ids":["art_source_video"]}}"#;

      update_pipeline_job_stage(UpdatePipelineJobStageArgs { db: &db, pipeline_job_id: &id, current_stage: PipelineStage::IngestAnalyze, maybe_stage_outputs: Some(outputs), maybe_business_status: Some("ANALYZING") }).await.unwrap();
      update_pipeline_job_stage(UpdatePipelineJobStageArgs { db: &db, pipeline_job_id: &id, current_stage: PipelineStage::PreflightCheck, maybe_stage_outputs: None, maybe_business_status: None }).await.unwrap();
      ...
      assert_eq!(job.maybe_business_status.as_deref(), Some("ANALYZING"));
    });
  }
}
