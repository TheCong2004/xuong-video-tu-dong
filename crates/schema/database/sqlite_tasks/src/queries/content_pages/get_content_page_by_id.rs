...
pub async fn get_content_page_by_id(args: GetContentPageByIdArgs<'_>) -> Result<Option<ContentPage>, SqliteTasksError> {
  let query = sqlx::query_as::<_, RawContentPage>("SELECT id, name, slug, output_root, target_platform, default_model_id, default_workflow_id, default_language, default_tone, default_aspect_ratio, browser_profile_id, worker_pool_id, default_image_prompt, default_expand_9_16_prompt, default_video_prompt, is_archived, created_at, updated_at FROM content_pages WHERE id = ?1 LIMIT 1").bind(args.id);
