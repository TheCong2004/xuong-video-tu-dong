...
pub async fn archive_content_page(args: ArchiveContentPageArgs<'_>) -> Result<bool, SqliteTasksError> {
  ...
  let result = sqlx::query("UPDATE content_pages SET is_archived = ?1, updated_at = unixepoch('now') WHERE id = ?2").bind(archived_val).bind(args.id).execute(args.db.get_pool()).await?;
