...
impl FirehosePublisher {
  pub async fn publish_user_sign_up(&self, user_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::UserSignUp, Some(user_token), Some(user_token), Some(user_token)).await?;
    Ok(())
  }

  pub async fn publish_user_badge_granted(&self, user_token: &str, badge_slug: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::UserBadgeGranted, Some(user_token), Some(badge_slug), None).await?;
    Ok(())
  }

  pub async fn enqueue_tts_model_upload(&self, user_token: &str, job_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::TtsModelUploadStarted, Some(user_token), None, Some(job_token)).await?;
    Ok(())
  }

  pub async fn publish_tts_model_upload_finished(&self, user_token: &UserToken, model_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::TtsModelUploadCompleted, Some(user_token.as_str()), Some(model_token), Some(model_token)).await?;
    Ok(())
  }

  pub async fn enqueue_tts_inference(&self, maybe_user_token: Option<&str>, job_token: &str, model_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::TtsInferenceStarted, maybe_user_token, Some(model_token), Some(job_token)).await?;
    Ok(())
  }

  pub async fn tts_inference_finished(&self, maybe_user_token: Option<&str>, model_token: &str, result_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::TtsInferenceCompleted, maybe_user_token, Some(model_token), Some(result_token)).await?;
    Ok(())
  }

  pub async fn enqueue_w2l_template_upload(&self, user_token: &str, job_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::W2lTemplateUploadStarted, Some(user_token), None, Some(job_token)).await?;
    Ok(())
  }

  pub async fn enqueue_w2l_inference(&self, maybe_user_token: Option<&str>, job_token: &str, template_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::W2lInferenceStarted, maybe_user_token, Some(template_token), Some(job_token)).await?;
    Ok(())
  }

  pub async fn publish_w2l_template_upload_finished(&self, user_token: &str, template_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::W2lTemplateUploadCompleted, Some(user_token), Some(template_token), Some(template_token)).await?;
    Ok(())
  }
  ...
  pub async fn enqueue_vc_inference(&self, maybe_user_token: Option<&UserToken>, inference_job_token: &InferenceJobToken) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::VcInferenceStarted, maybe_user_token.map(|u| u.as_str()), Some(inference_job_token.as_str()), Some(inference_job_token.as_str())).await?;
    Ok(())
  }
  ...
  pub async fn enqueue_lipsync_animation(&self, maybe_user_token: Option<&UserToken>, inference_job_token: &InferenceJobToken) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::LipsyncAnimationStarted, maybe_user_token.map(|u| u.as_str()), Some(inference_job_token.as_str()), Some(inference_job_token.as_str())).await?;
    Ok(())
  }
  ...
  pub async fn enqueue_generic_download(&self, user_token: &str, job_token: &str) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::GenericDownloadStarted, Some(user_token), None, Some(job_token)).await?;
    Ok(())
  }
  ...
  pub async fn publish_generic_download_finished(&self, user_token: &UserToken, entity_token: Option<&str>) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::GenericDownloadCompleted, Some(user_token.as_str()), entity_token, entity_token).await?;
    Ok(())
  }

  pub async fn publish_media_uploaded(&self, maybe_user_token: Option<&UserToken>, upload_token: &MediaUploadToken) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::MediaUploaded, maybe_user_token.map(|u| u.as_str()), Some(upload_token.as_str()), Some(upload_token.as_str())).await?;
    Ok(())
  }
  ...
  pub async fn publish_device_media_recorded(&self, maybe_user_token: Option<&UserToken>, upload_token: &MediaUploadToken) -> AnyhowResult<()> {
    let _record_id = self.insert(FirehoseEvent::DeviceMediaRecorded, maybe_user_token.map(|u| u.as_str()), Some(upload_token.as_str()), Some(upload_token.as_str())).await?;
    Ok(())
  }
  ...
  async fn insert(&self, event_type: FirehoseEvent, user_token: Option<&str>, entity_token: Option<&str>, created_entity_token: Option<&str>) -> AnyhowResult<u64> {
