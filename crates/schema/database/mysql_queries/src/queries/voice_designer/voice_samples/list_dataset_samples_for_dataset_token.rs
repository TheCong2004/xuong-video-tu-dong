...
pub async fn list_dataset_samples_for_dataset_token(dataset_token: &ZsVoiceDatasetToken, can_see_deleted: bool, mysql_pool: &MySqlPool) -> AnyhowResult<Vec<DatasetSampleRecordForList>> {
  let mut connection = mysql_pool.acquire().await?;
  list_samples_by_dataset_token_with_connection(dataset_token, can_see_deleted, &mut connection).await
}

pub async fn list_samples_by_dataset_token_with_connection(dataset_token: &ZsVoiceDatasetToken, can_see_deleted: bool, mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Vec<DatasetSampleRecordForList>> {
  let maybe_samples = list_samples_by_dataset_token(mysql_connection, dataset_token, can_see_deleted).await;
  ...
  Ok(records.into_iter().map(|record| DatasetSampleRecordForList { sample_token: record.token, media_file_token: record.media_file_token, media_type: record.media_type, public_bucket_directory_hash: record.public_bucket_directory_hash, maybe_public_bucket_prefix: record.maybe_public_bucket_prefix, maybe_public_bucket_extension: record.maybe_public_bucket_extension, maybe_creator_user_token: record.maybe_creator_user_token, created_at: record.created_at, updated_at: record.updated_at }).collect())
}

async fn list_samples_by_dataset_token(mysql_connection: &mut PoolConnection<MySql>, dataset_token: &ZsVoiceDatasetToken, can_see_deleted: bool) -> Result<Vec<InternalRawDatasetSampleRecordForList>, Error> {
