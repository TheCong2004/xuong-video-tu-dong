...
impl ListVoicesQueryBuilder {
  pub fn new() -> Self {
    Self { scope_creator_username: None, include_user_hidden: false, include_mod_deleted_results: false, include_user_deleted_results: false, sort_ascending: false, offset: None, limit: DEFAULT_MYSQL_QUERY_RESULT_PAGE_SIZE, cursor_is_reversed: false }
  }
  ...
  async fn perform_internal_query(&self, mysql_pool: &MySqlPool) -> AnyhowResult<Vec<RawInternalVoiceRecordForList>> {
  ...
  pub async fn perform_query_for_page(&self, mysql_pool: &MySqlPool) -> AnyhowResult<ZsVoiceListPage> {
    let internal_voices = self.perform_internal_query(mysql_pool).await?;

    let first_id = internal_voices.first().map(|raw_result| raw_result.voice_id);

    let last_id = internal_voices.last().map(|raw_result| raw_result.voice_id);

    let voices = internal_voices.into_iter().map(|v| ZsVoice { voice_token: v.token, title: v.title, creator_set_visibility: Visibility::from_str(&v.creator_set_visibility).unwrap_or(Visibility::Public), ietf_language_tag: v.ietf_language_tag, ietf_primary_language_subtag: v.ietf_primary_language_subtag, creator_user_token: v.creator_user_token, creator_username: v.creator_username, creator_display_name: v.creator_display_name, creator_email_gravatar_hash: v.creator_email_gravatar_hash, created_at: v.created_at, updated_at: v.updated_at }).collect::<Vec<ZsVoice>>();

    Ok(ZsVoiceListPage { voices, sort_ascending: self.sort_ascending, first_id, last_id })
  }
...
mod tests {
  ...
  fn predicates_scoped_to_user() {
    let query_builder = ListVoicesQueryBuilder::new().scope_creator_username(Some("echelon"));
  ...
  fn predicates_including_user_hidden() {
    let query_builder = ListVoicesQueryBuilder::new().include_user_hidden(true);
  ...
  fn predicates_including_mod_deleted() {
    let query_builder = ListVoicesQueryBuilder::new().include_mod_deleted_results(true);
  ...
  fn predicates_including_user_deleted() {
    let query_builder = ListVoicesQueryBuilder::new().include_user_deleted_results(true);
  ...
  fn predicates_including_mod_deleted_and_user_deleted() {
    let query_builder = ListVoicesQueryBuilder::new().include_mod_deleted_results(true).include_user_deleted_results(true);
