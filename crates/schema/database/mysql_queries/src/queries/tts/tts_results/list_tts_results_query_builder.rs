...
impl ListTtsResultsQueryBuilder {
  pub fn new() -> Self {
    Self { scope_creator_username: None, include_user_hidden: false, include_mod_disabled_results: false, sort_ascending: false, offset: None, limit: DEFAULT_MYSQL_QUERY_RESULT_PAGE_SIZE, cursor_is_reversed: false }
  }
  ...
  pub async fn perform_query_for_page(&self, mysql_pool: &MySqlPool) -> AnyhowResult<TtsInferenceListPage> {
    let internal_results = self.perform_internal_query(mysql_pool).await?;

    let first_id = internal_results.first().map(|raw_result| raw_result.tts_result_id);

    let last_id = internal_results.last().map(|raw_result| raw_result.tts_result_id);

    let inference_results = internal_results.into_iter().map(|r| TtsInferenceRecordForList { tts_result_token: r.tts_result_token, tts_model_token: r.tts_model_token, tts_model_title: r.tts_model_title, raw_inference_text: r.raw_inference_text, public_bucket_wav_audio_path: r.public_bucket_wav_audio_path, maybe_creator_user_token: r.maybe_creator_user_token, maybe_creator_username: r.maybe_creator_username, maybe_creator_display_name: r.maybe_creator_display_name, maybe_creator_result_id: r.maybe_creator_result_id.map(|v| v as u64), file_size_bytes: if r.file_size_bytes > 0 { r.file_size_bytes as u32 } else { 0 }, duration_millis: if r.duration_millis > 0 { r.duration_millis as u32 } else { 0 }, visibility: Visibility::from_str(&r.creator_set_visibility).unwrap_or(Visibility::Public), created_at: r.created_at, updated_at: r.updated_at }).collect::<Vec<TtsInferenceRecordForList>>();

    Ok(TtsInferenceListPage { inference_records: inference_results, sort_ascending: self.sort_ascending, first_id, last_id })
  }

  /// Perform the query based on the set predicates.
  /// The records contain IDs, which we should not expose to the frontend.
  pub async fn perform_internal_query(&self, mysql_pool: &MySqlPool) -> AnyhowResult<Vec<RawInternalTtsRecord>> {
...
mod tests {
  ...
  fn predicates_scoped_to_user() {
    let query_builder = ListTtsResultsQueryBuilder::new().scope_creator_username(Some("echelon"));
  ...
  fn predicates_including_user_hidden() {
    let query_builder = ListTtsResultsQueryBuilder::new().include_user_hidden(true);
  ...
  fn predicates_including_deleted() {
    let query_builder = ListTtsResultsQueryBuilder::new().include_mod_disabled_results(true);
  ...
  fn predicates_sort_ascending() {
    let query_builder = ListTtsResultsQueryBuilder::new().sort_ascending(true);
  ...
  fn predicates_offset() {
    let query_builder = ListTtsResultsQueryBuilder::new().offset(Some(100));
  ...
  fn predicates_offset_and_sort_ascending() {
    let query_builder = ListTtsResultsQueryBuilder::new().sort_ascending(true).offset(Some(100));
  ...
  #[test]
  fn predicates_limit() {
    let query_builder = ListTtsResultsQueryBuilder::new().limit(15);
  ...
  fn predicates_cursor_is_reversed_without_cursor() {
    let query_builder = ListTtsResultsQueryBuilder::new().cursor_is_reversed(true);
  ...
  fn predicates_offset_cursor_is_reversed() {
    let query_builder = ListTtsResultsQueryBuilder::new().offset(Some(100)).cursor_is_reversed(true);
  ...
  fn predicates_offset_cursor_is_reversed_sort_ascending() {
    let query_builder = ListTtsResultsQueryBuilder::new().offset(Some(100)).cursor_is_reversed(true).sort_ascending(true);
  ...
  fn predicates_limit_scope_user_offset_cursor_is_reversed_sort_ascending() {
    let query_builder = ListTtsResultsQueryBuilder::new().limit(1000).scope_creator_username(Some("pikachu")).offset(Some(100)).cursor_is_reversed(true).sort_ascending(true);
  ...
  fn predicates_limit_scope_user_show_hidden_offset_cursor_is_reversed_sort_ascending() {
    let query_builder = ListTtsResultsQueryBuilder::new().limit(1000).include_user_hidden(true).scope_creator_username(Some("pikachu")).offset(Some(100)).cursor_is_reversed(true).sort_ascending(true);
