...
/// actually inserted.
pub async fn bulk_insert_folder_media_files<'e, 'c: 'e, E>(args: BulkInsertFolderMediaFilesArgs<'e, 'c, E>) -> Result<u64, sqlx::Error>
where
  E: 'e + Executor<'c, Database = MySql>,
{
  ...
  let mut builder = QueryBuilder::<MySql>::new("INSERT IGNORE INTO folder_media_files (folder_token, media_file_token) ");
  builder.push_values(args.media_file_tokens, |mut b, media_file_token| {
    b.push_bind(folder_token_str).push_bind(media_file_token.as_str());
  });
