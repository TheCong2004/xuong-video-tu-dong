...
pub(super) async fn internal_select_wallet_balance_for_update(wallet_token: &WalletToken, transaction: &mut sqlx::Transaction<'_, MySql>) -> Result<WalletForUpdate, SelectExactlyOneError> {
...
FOR UPDATE
  ...
  Ok(WalletForUpdate { token: record.token, namespace: record.namespace, owner_user_token: record.owner_user_token, banked_credits: record.banked_credits, monthly_credits: record.monthly_credits })
}
