...
pub async fn try_to_spend_wallet_balance(wallet_token: &WalletToken, amount_to_spend_request: u64, maybe_ledger_ref: Option<&str>, transaction: &mut sqlx::Transaction<'_, MySql>) -> Result<WalletUpdateSummary, WalletSpendError> {
  ...
  if total_available < amount_to_spend_request {
    return Err(WalletSpendError::InsufficientBalance { requested_to_spend_amount: amount_to_spend_request, available_amount: total_available });
  }
