...
pub async fn get_model_weight_info_lite(token: &ModelWeightToken, mysql_pool: &MySqlPool) -> AnyhowResult<Option<ModelWeightInfoLite>> {
  let mut connection = mysql_pool.acquire().await?;
  get_model_weight_info_lite_with_connection(token, &mut connection).await
}

pub async fn get_model_weight_info_lite_with_connection(token: &ModelWeightToken, mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Option<ModelWeightInfoLite>> {
...
WHERE m.token = ?
  ...
  Ok(Some(ModelWeightInfoLite { token: model.token, weights_type: model.weights_type }))
}
