...
pub async fn list_model_weight_info_lite(mysql_pool: &MySqlPool) -> AnyhowResult<Vec<ModelWeightInfoLite>> {
  let mut connection = mysql_pool.acquire().await?;
  list_model_weight_info_lite_with_connection(&mut connection).await
}

pub async fn list_model_weight_info_lite_with_connection(mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Vec<ModelWeightInfoLite>> {
...
FROM model_weights as w
  ...
  Ok(models.into_iter().map(|model| ModelWeightInfoLite { token: model.token, weights_type: model.weights_type }).collect::<Vec<_>>())
}
