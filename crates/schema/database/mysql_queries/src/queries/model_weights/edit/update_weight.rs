...
pub async fn update_weights(args: UpdateWeightArgs<'_, '_>) -> AnyhowResult<()> {
  if args.title.is_none() && args.maybe_description_markdown.is_none() && args.maybe_description_rendered_html.is_none() && args.creator_set_visibility.is_none() && args.cover_image.is_none() {
...
mod tests {
  ...
  async fn test_update_weights() -> AnyhowResult<()> {
    ...
    let pool = MySqlPoolOptions::new().max_connections(3).connect(&db_connection_string).await?;
