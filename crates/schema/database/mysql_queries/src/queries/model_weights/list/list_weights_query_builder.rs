...
impl ListWeightsQueryBuilder {
  pub fn new() -> Self {
    Self { scope_creator_username: None, include_user_hidden: false, include_mod_deleted_results: false, include_user_deleted_results: false, sort_ascending: false, weights_type: None, weights_category: None, offset: None, limit: DEFAULT_MYSQL_QUERY_RESULT_PAGE_SIZE, cursor_is_reversed: false }
  }
  ...
  async fn perform_internal_query(&self, mysql_pool: &MySqlPool) -> AnyhowResult<Vec<RawWeightJoinUser>> {
  ...
  pub async fn perform_query_for_page(&self, mysql_pool: &MySqlPool) -> AnyhowResult<WeightsPage> {
    ...
    let weights = weights
      ...
      .map(|record| {
        ...
        WeightJoinUser {
          ...
          maybe_last_update_user_token: record.maybe_last_update_user_token.map(|token| UserToken::new_from_str(&token)),
    ...
    Ok(WeightsPage { weights, sort_ascending: self.sort_ascending, first_id, last_id })
  }
...
mod tests {
  ...
  fn predicates_scoped_to_weights_category() {
    let query_builder = ListWeightsQueryBuilder::new().weights_category(Some(WeightsCategory::VoiceConversion));
    println!("Query HERE! {}", &query_builder.build_predicates());
    assert_eq!(
      &query_builder.build_predicates(),
  ...
  fn predicates_including_mod_deleted_and_user_deleted() {
    let query_builder = ListWeightsQueryBuilder::new().include_mod_deleted_results(true).include_user_deleted_results(true);
