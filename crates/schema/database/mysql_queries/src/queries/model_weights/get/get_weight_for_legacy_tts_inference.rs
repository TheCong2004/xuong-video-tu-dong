...
pub async fn get_weight_for_legacy_tts_inference(weight_token: &ModelWeightToken, mysql_pool: &MySqlPool) -> Result<Option<ModelWeightForLegacyTtsInference>, TtsModelForInferenceError> {
  let mut connection = mysql_pool.acquire().await.map_err(|err| {
    error!("Error acquiring db connection from pool: {:?}", err);
    TtsModelForInferenceError::DatabaseError { reason: format!("Mysql connection error: {:?}", err) }
  })?;

  get_weight_for_legacy_tts_inference_with_connection(weight_token, &mut connection).await
}

pub async fn get_weight_for_legacy_tts_inference_with_connection(weight_token: &ModelWeightToken, mysql_connection: &mut PoolConnection<MySql>) -> Result<Option<ModelWeightForLegacyTtsInference>, TtsModelForInferenceError> {
  ...
  Ok(Some(ModelWeightForLegacyTtsInference {
    ...
    maybe_custom_vocoder: match record.maybe_custom_vocoder_token {
      ...
      None => None,
      Some(vocoder_token) => Some(CustomVocoderFields { vocoder_token, vocoder_type: record.maybe_custom_vocoder_type.ok_or(TtsModelForInferenceError::DatabaseError { reason: "custom_vocoder_type field error".to_string() })?, vocoder_title: record.maybe_custom_vocoder_title.ok_or(TtsModelForInferenceError::DatabaseError { reason: "custom_vocoder_title field error".to_string() })?, vocoder_private_bucket_hash: record.maybe_custom_vocoder_private_bucket_hash.ok_or(TtsModelForInferenceError::DatabaseError { reason: "vocoder_private_bucket_hash field error".to_string() })? }),
    },
...
async fn select_include_deleted(weight_token: &ModelWeightToken, mysql_connection: &mut PoolConnection<MySql>) -> Result<RawWeight, sqlx::Error> {
  sqlx::query_as!(
    RawWeight,
    r#"
        SELECT
