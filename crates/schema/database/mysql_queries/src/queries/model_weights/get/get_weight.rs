...
pub async fn get_weight_by_token(weight_token: &ModelWeightToken, can_see_deleted: bool, mysql_pool: &MySqlPool) -> AnyhowResult<Option<RetrievedModelWeight>> {
  let mut connection = mysql_pool.acquire().await?;
  get_weights_by_token_with_connection(weight_token, can_see_deleted, &mut connection).await
}

pub async fn get_weights_by_token_with_connection(weight_token: &ModelWeightToken, can_see_deleted: bool, mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Option<RetrievedModelWeight>> {
  get_weight_by_token_with_transactor(weight_token, can_see_deleted, Transactor::for_connection(mysql_connection)).await
}

pub async fn get_weight_by_token_with_transactor(weight_token: &ModelWeightToken, can_see_deleted: bool, transactor: Transactor<'_, '_>) -> AnyhowResult<Option<RetrievedModelWeight>> {
  let maybe_result = if can_see_deleted { select_include_deleted(weight_token, transactor).await } else { select_without_deleted(weight_token, transactor).await };
...
async fn select_include_deleted(weight_token: &ModelWeightToken, mut transactor: Transactor<'_, '_>) -> Result<Option<RawWeight>, sqlx::Error> {
...
async fn select_without_deleted(weight_token: &ModelWeightToken, mut transactor: Transactor<'_, '_>) -> Result<Option<RawWeight>, sqlx::Error> {
  let query = sqlx::query_as!(
    RawWeight,
    r#"
        SELECT
