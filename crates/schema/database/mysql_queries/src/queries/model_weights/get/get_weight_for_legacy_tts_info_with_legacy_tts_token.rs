...
pub async fn get_weight_for_legacy_tts_info_with_legacy_tts_token(legacy_tts_model_token: &TtsModelToken, can_see_deleted: bool, mysql_pool: &MySqlPool) -> AnyhowResult<Option<ModelWeightForLegacyTtsInfo>> {
...
pub async fn get_weight_for_legacy_tts_info_with_legacy_tts_token_with_connection(legacy_tts_model_token: &TtsModelToken, can_see_deleted: bool, mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Option<ModelWeightForLegacyTtsInfo>> {
  let maybe_result = if can_see_deleted { select_include_deleted(legacy_tts_model_token, mysql_connection).await } else { select_without_deleted(legacy_tts_model_token, mysql_connection).await };
  ...
  Ok(Some(ModelWeightForLegacyTtsInfo {
    token: record.token,
    title: record.title,
...
async fn select_include_deleted(legacy_tts_model_token: &TtsModelToken, mysql_connection: &mut PoolConnection<MySql>) -> Result<RawWeight, sqlx::Error> {
  sqlx::query_as!(
    RawWeight,
    r#"
        SELECT
...
async fn select_without_deleted(legacy_tts_model_token: &TtsModelToken, mysql_connection: &mut PoolConnection<MySql>) -> Result<RawWeight, sqlx::Error> {
