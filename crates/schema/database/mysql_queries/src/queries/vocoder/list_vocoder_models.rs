...
pub async fn list_vocoder_models(mysql_pool: &MySqlPool, scope_creator_username: Option<&str>, can_see_deleted_and_hidden: bool) -> AnyhowResult<Vec<VocoderModelListItem>> {
  let maybe_models = match scope_creator_username {
    Some(username) => list_vocoder_models_creator_scoped(mysql_pool, username, can_see_deleted_and_hidden).await,
    None => list_vocoder_models_for_all_creators(mysql_pool, can_see_deleted_and_hidden).await,
  };
  ...
  Ok(models.into_iter().map(|model| VocoderModelListItem { vocoder_token: model.vocoder_token, vocoder_type: model.vocoder_type, title: model.title, is_staff_recommended: i8_to_bool(model.is_staff_recommended), creator_user_token: model.creator_user_token, creator_username: model.creator_username, creator_display_name: model.creator_display_name, creator_gravatar_hash: model.creator_gravatar_hash, created_at: model.created_at, updated_at: model.updated_at, moderator_fields: VocoderModelListItemModFields { is_mod_disabled_from_public_use: i8_to_bool(model.is_mod_disabled_from_public_use), is_mod_disabled_from_author_use: i8_to_bool(model.is_mod_disabled_from_author_use), is_mod_author_editing_locked: i8_to_bool(model.is_mod_author_editing_locked), user_deleted_at: model.user_deleted_at, mod_deleted_at: model.mod_deleted_at } }).collect::<Vec<VocoderModelListItem>>())
}

async fn list_vocoder_models_for_all_creators(mysql_pool: &MySqlPool, can_see_deleted_and_hidden: bool) -> AnyhowResult<Vec<InternalVocoderModelListItemRaw>> {
...
async fn list_vocoder_models_creator_scoped(mysql_pool: &MySqlPool, scope_creator_username: &str, can_see_deleted_and_hidden: bool) -> AnyhowResult<Vec<InternalVocoderModelListItemRaw>> {
  // NB: Sqlx doesn't like non-string literal queries, so we have to branch.
  // Repeating the query is ugly, but that's the best we can do without a
  // full-on query builder pattern.
  let maybe_models = if !can_see_deleted_and_hidden {
