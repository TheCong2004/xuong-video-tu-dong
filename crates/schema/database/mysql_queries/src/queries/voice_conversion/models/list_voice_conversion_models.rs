...
pub async fn list_voice_conversion_models(mysql_pool: &MySqlPool, scope_creator_username: Option<&str>) -> AnyhowResult<Vec<VoiceConversionModelRecordForList>> {
  let mut connection = mysql_pool.acquire().await?;
  list_voice_conversion_models_with_connection(&mut connection, scope_creator_username).await
}

pub async fn list_voice_conversion_models_with_connection(mysql_connection: &mut PoolConnection<MySql>, scope_creator_username: Option<&str>) -> AnyhowResult<Vec<VoiceConversionModelRecordForList>> {
  let maybe_models = match scope_creator_username {
    Some(username) => list_voice_conversion_models_creator_scoped(mysql_connection, username).await,
    None => list_voice_conversion_models_for_all_creators(mysql_connection).await,
  };
  ...
  Ok(models.into_iter().map(|model| VoiceConversionModelRecordForList { token: model.token, model_type: model.model_type, creator_user_token: model.creator_user_token, creator_username: model.creator_username, creator_display_name: model.creator_display_name, creator_gravatar_hash: model.creator_gravatar_hash, title: model.title, ietf_language_tag: model.ietf_language_tag, ietf_primary_language_subtag: model.ietf_primary_language_subtag, is_front_page_featured: i8_to_bool(model.is_front_page_featured), creator_set_visibility: model.creator_set_visibility, created_at: model.created_at, updated_at: model.updated_at }).collect::<Vec<VoiceConversionModelRecordForList>>())
}

async fn list_voice_conversion_models_for_all_creators(mysql_connection: &mut PoolConnection<MySql>) -> Result<Vec<RawVoiceConversionModelRecord>, Error> {
  Ok(
    sqlx::query_as!(
...
async fn list_voice_conversion_models_creator_scoped(mysql_connection: &mut PoolConnection<MySql>, scope_creator_username: &str) -> Result<Vec<RawVoiceConversionModelRecord>, Error> {
  Ok(
    sqlx::query_as!(
      RawVoiceConversionModelRecord,
      r#"
