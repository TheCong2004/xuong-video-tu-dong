...
impl std::fmt::Display for VoiceConversionModelForInferenceError {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      VoiceConversionModelForInferenceError::DatabaseError { reason } => write!(f, "Database error: {:?}", reason),
    }
  }
}
...
pub async fn get_voice_conversion_model_for_inference(pool: &MySqlPool, model_token: &str) -> Result<Option<VoiceConversionModelForInference>, VoiceConversionModelForInferenceError> {
...
WHERE vc.token = ?
  ...
  let model: InternalVoiceConversionModelForInferenceRaw = match maybe_model {
    ...
    Err(err) => match err {
      ...
      _ => {
        ...
        return Err(VoiceConversionModelForInferenceError::DatabaseError { reason: format!("Mysql error: {:?}", err) });
      },
    },
  };
  ...
  Ok(Some(VoiceConversionModelForInference { token: model.token, model_type: model.model_type, title: model.title, has_index_file: i8_to_bool(model.has_index_file), private_bucket_hash: model.private_bucket_hash, created_at: model.created_at, updated_at: model.updated_at, mod_deleted_at: model.mod_deleted_at, user_deleted_at: model.user_deleted_at }))
}
