...
pub async fn get_voice_conversion_model_info_lite(token: &str, mysql_pool: &MySqlPool) -> AnyhowResult<Option<VoiceConversionModelInfoLite>> {
  let mut connection = mysql_pool.acquire().await?;
  get_voice_conversion_model_info_lite_with_connection(token, &mut connection).await
}

pub async fn get_voice_conversion_model_info_lite_with_connection(token: &str, mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Option<VoiceConversionModelInfoLite>> {
...
WHERE vc.token = ?
  ...
  Ok(Some(VoiceConversionModelInfoLite { token: model.token, model_type: model.model_type }))
}
