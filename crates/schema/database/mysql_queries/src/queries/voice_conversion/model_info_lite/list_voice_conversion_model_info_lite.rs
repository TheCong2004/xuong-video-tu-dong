...
pub async fn list_voice_conversion_model_info_lite(mysql_pool: &MySqlPool) -> AnyhowResult<Vec<VoiceConversionModelInfoLite>> {
  let mut connection = mysql_pool.acquire().await?;
  list_voice_conversion_model_info_lite_with_connection(&mut connection).await
}

pub async fn list_voice_conversion_model_info_lite_with_connection(mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Vec<VoiceConversionModelInfoLite>> {
...
FROM voice_conversion_models as vc
  ...
  Ok(models.into_iter().map(|model| VoiceConversionModelInfoLite { token: model.token, model_type: model.model_type }).collect::<Vec<VoiceConversionModelInfoLite>>())
}
