...
pub async fn list_trending_tts_models_with_pool(mysql_pool: &MySqlPool) -> AnyhowResult<TrendingModels> {
  let mut mysql_connection = mysql_pool.acquire().await?;
  list_trending_tts_models(&mut mysql_connection).await
}

pub async fn list_trending_tts_models(mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<TrendingModels> {
  let query_parts = vec![
    // English
  ...
  Ok(TrendingModels { models: results })
}
...
(
  ...
  "#,
    model_type, window_name, ietf_primary_language_subtag, limit
  )
}
