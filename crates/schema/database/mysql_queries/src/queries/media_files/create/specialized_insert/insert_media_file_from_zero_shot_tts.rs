...
pub async fn insert_media_file_from_zero_shot(args: InsertArgs<'_>) -> AnyhowResult<(MediaFileToken, u64)> {
  ...
  if let Some(creator_user_token) = args.job.maybe_creator_user_token.as_deref() {
    ...
    let next_lipsync_id = transactional_increment_generic_synthetic_id(&user_token, IdCategory::ZeroShotTtsResult, &mut transaction).await?;
