...
pub struct InsertF5TTSMediaFileArgs<'a> {
...
pub async fn insert_media_file_from_f5_tts(args: InsertF5TTSMediaFileArgs<'_>) -> AnyhowResult<(MediaFileToken, u64)> {
  ...
  if let Some(creator_user_token) = args.job.maybe_creator_user_token.as_deref() {
    ...
    let next_voice_conversion_id = transactional_increment_generic_synthetic_id(&user_token, IdCategory::TtsResult, &mut transaction).await?;
