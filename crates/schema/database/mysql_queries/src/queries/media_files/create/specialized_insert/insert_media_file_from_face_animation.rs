...
pub async fn insert_media_file_from_face_animation(args: InsertArgs<'_>) -> AnyhowResult<(MediaFileToken, u64)> {
  ...
  if let Some(creator_user_token) = args.job.maybe_creator_user_token.as_deref() {
    ...
    let next_lipsync_id = transactional_increment_generic_synthetic_id(&user_token, IdCategory::LipsyncAnimationResult, &mut transaction).await?;
  ...
  let media_type = args.maybe_mime_type.and_then(MediaFileType::try_from_mime_type).unwrap_or(MediaFileType::Video); // Coarse fallback for unrecognized mimes
