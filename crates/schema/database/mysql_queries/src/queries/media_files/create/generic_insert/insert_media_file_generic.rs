...
/// the actual INSERT.
pub async fn insert_media_file_generic(args: InsertArgs<'_>) -> AnyhowResult<(MediaFileToken, u64)> {
  let extra_file_modification_info = args.maybe_extra_media_info.map(|extra| extra.to_json_string()).transpose()?;
  ...
  if let Some(user_token) = args.maybe_creator_user_token.as_deref() {
    let next_media_file_id = transactional_increment_generic_synthetic_id(user_token, args.maybe_creator_file_synthetic_id_category, &mut transaction).await?;

    let category_id = transactional_increment_generic_synthetic_id(user_token, args.maybe_creator_category_synthetic_id_category, &mut transaction).await?;
