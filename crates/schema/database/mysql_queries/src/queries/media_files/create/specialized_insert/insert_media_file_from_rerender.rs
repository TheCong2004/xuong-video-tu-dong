...
pub async fn insert_media_file_from_rerender(args: InsertArgs<'_>) -> AnyhowResult<(MediaFileToken, u64)> {
  ...
  if let Some(creator_user_token) = args.job.maybe_creator_user_token.as_deref() {
    ...
    let next_rerender_id = transactional_increment_generic_synthetic_id(&user_token, IdCategory::VideoFilterResult, &mut transaction).await?;
