...
pub struct ProjectMediaFileListPage {
  pub records: Vec<ProjectMediaFileListItem>,
...
pub struct ProjectMediaFileListItem {
  pub id: u64,
...
pub struct ListSessionProjectMediaFilesArgs<'a, 'c, E>
where
  E: 'a + Executor<'c, Database = MySql>,
{
  ...
  pub maybe_filter_project_type: Option<MediaFileProjectType>,
...
/// List the session user's project files (`media_class = 'project'`), newest first
/// by default, with keyset pagination on `id`.
///
...
SELECT
    ...
    m.maybe_project_type as `project_type!: enums::by_table::media_files::media_file_project_type::MediaFileProjectType`,
...
WHERE
    ...
    AND m.media_class = 'project'
    AND m.maybe_project_type IS NOT NULL
    AND m.maybe_project_type = ?
