...
pub async fn batch_get_media_files_by_tokens(mysql_pool: &MySqlPool, media_file_tokens: &[MediaFileToken], can_see_deleted: bool) -> AnyhowResult<Vec<MediaFilesByTokensRecord>> {
  batch_get_media_files_by_tokens_with_transactor(Transactor::Pool { pool: mysql_pool }, media_file_tokens, can_see_deleted).await
}

pub async fn batch_get_media_files_by_tokens_with_connection(mysql_connection: &mut PoolConnection<MySql>, media_file_tokens: &[MediaFileToken], can_see_deleted: bool) -> AnyhowResult<Vec<MediaFilesByTokensRecord>> {
  batch_get_media_files_by_tokens_with_transactor(Transactor::Connection { connection: mysql_connection }, media_file_tokens, can_see_deleted).await
}

pub async fn batch_get_media_files_by_tokens_with_transactor(transactor: Transactor<'_, '_>, media_file_tokens: &[MediaFileToken], can_see_deleted: bool) -> AnyhowResult<Vec<MediaFilesByTokensRecord>> {
...
async fn get_raw_media_files_by_tokens(mut transactor: Transactor<'_, '_>, media_file_tokens: &[MediaFileToken], can_see_deleted: bool) -> AnyhowResult<Vec<RawMediaFileJoinUser>> {
...
impl FromRow<'_, MySqlRow> for RawMediaFileJoinUser {
  fn from_row(row: &MySqlRow) -> Result<Self, sqlx::Error> {
    Ok(Self {
      ...
      maybe_creator_user_token: row.try_get::<Option<String>, _>("maybe_creator_user_token")?.and_then(|token| Some(UserToken::new_from_str(&token))),
