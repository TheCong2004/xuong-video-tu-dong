...
pub async fn list_active_user_subscriptions<'e, 'c: 'e, E>(mysql_executor: E, user_token: &str) -> AnyhowResult<Vec<ActiveUserSubscription>>
where
...
WHERE
  ...
  let records = records.into_iter().map(|record: RawActiveUserSubscription| ActiveUserSubscription { user_token: record.user_token, subscription_namespace: record.subscription_namespace, subscription_product_slug: record.subscription_product_slug, subscription_expires_at: record.subscription_expires_at }).collect::<Vec<ActiveUserSubscription>>();
