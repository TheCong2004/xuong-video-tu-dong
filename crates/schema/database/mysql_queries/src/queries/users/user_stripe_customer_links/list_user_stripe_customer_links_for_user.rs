...
pub async fn list_user_stripe_customer_links_for_user(user_token: &UserToken, connection: &mut PoolConnection<MySql>) -> Result<Vec<UserStripeCustomerLink>, sqlx::Error> {
...
ORDER BY id ASC
  ...
  let links = records.into_iter().map(|record| UserStripeCustomerLink { user_token: record.user_token, payments_namespace: record.payments_namespace, stripe_customer_id: record.stripe_customer_id }).collect();
