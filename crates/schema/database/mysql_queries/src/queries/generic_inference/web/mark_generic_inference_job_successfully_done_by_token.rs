...
pub async fn mark_generic_inference_job_successfully_done_by_token(pool: &MySqlPool, token: &InferenceJobToken, maybe_entity_type: Option<InferenceResultType>, maybe_entity_token: Option<&str>, total_job_duration: Option<Duration>, inference_duration: Option<Duration>) -> AnyhowResult<()> {
  ...
  let truncated_total_job_execution_millis = total_job_duration.map(|duration| duration.as_millis() as u32).unwrap_or(0);
  let truncated_inference_execution_millis = inference_duration.map(|duration| duration.as_millis() as u32).unwrap_or(0);
