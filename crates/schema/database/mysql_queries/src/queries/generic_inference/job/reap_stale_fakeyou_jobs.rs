...
pub async fn reap_stale_fakeyou_jobs_using_connection(mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<()> {
...
LIMIT 5000
  ...
  match query_result {
    ...
    Err(err) => Err(anyhow!("error with reaping stale jobs: {:?}", err)),
  }
}
