...
pub async fn insert_generic_inference_job_for_fal_queue_mock_failure<'e, 'c: 'e, E>(args: InsertGenericInferenceForFalMockFailureArgs<'e, 'c, E>) -> Result<InferenceJobToken, DatabaseQueryError>
where
  E: 'e + Executor<'c, Database = MySql>,
{
  ...
  insert_generic_inference_job_for_fal_queue_with_apriori_job_token(InsertGenericInferenceForFalWithAprioriJobTokenArgs { uuid_idempotency_token: args.uuid_idempotency_token, apriori_job_token: &job_token, maybe_external_third_party_id: &synthetic_external_id, fal_category: args.fal_category, maybe_model_type: args.maybe_model_type, maybe_inference_args: args.maybe_inference_args, maybe_prompt_token: args.maybe_prompt_token, maybe_creator_user_token: args.maybe_creator_user_token, maybe_avt_token: args.maybe_avt_token, creator_ip_address: args.creator_ip_address, creator_set_visibility: args.creator_set_visibility, maybe_platform_type: args.maybe_platform_type, maybe_cost_estimates: None, starting_job_status_override: Some(JobStatusPlus::CompleteFailure), maybe_frontend_failure_category: args.maybe_frontend_failure_category, maybe_failure_reason: args.maybe_failure_reason, maybe_debug_log_event_token: None, mysql_executor: args.mysql_executor, phantom: args.phantom }).await
}
