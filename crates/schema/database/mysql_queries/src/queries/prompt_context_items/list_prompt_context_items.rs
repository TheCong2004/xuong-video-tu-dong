...
pub async fn list_prompt_context_items(prompt_token: &PromptToken, mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Vec<PromptContextItem>> {
...
ORDER BY pci.id ASC
  ...
  match result {
    Ok(items) => Ok(items.into_iter().map(|item| PromptContextItem { media_token: item.media_token, context_semantic_type: item.context_semantic_type, public_bucket_directory_hash: item.public_bucket_directory_hash, maybe_public_bucket_prefix: item.maybe_public_bucket_prefix, maybe_public_bucket_extension: item.maybe_public_bucket_extension }).collect::<Vec<PromptContextItem>>()),
    Err(err) => match err {
