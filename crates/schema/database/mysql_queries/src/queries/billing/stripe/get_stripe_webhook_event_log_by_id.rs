...
pub async fn get_stripe_webhook_event_log_by_id(stripe_event_id: &str, mysql_pool: &MySqlPool) -> AnyhowResult<Option<StripeWebhookEventLog>> {
  let mut connection = mysql_pool.acquire().await?;
  get_stripe_webhook_event_log_by_id_with_connection(stripe_event_id, &mut connection).await
}

pub async fn get_stripe_webhook_event_log_by_id_with_connection(stripe_event_id: &str, mysql_connection: &mut PoolConnection<MySql>) -> AnyhowResult<Option<StripeWebhookEventLog>> {
...
WHERE
  ...
  match maybe_record {
    ...
    Ok(r) => Ok(Some(StripeWebhookEventLog { stripe_event_id: r.stripe_event_id, should_ignore_retry: i8_to_bool(r.should_ignore_retry) })),
  }
}
