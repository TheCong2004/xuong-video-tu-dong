...
///
/// The detected model / variant of an uploaded video. The variant set mirrors
...
pub enum UploadedVideoDetectedModelType {
...
mod tests {
  ...
  #[test]
  fn round_trip() {
    for variant in UploadedVideoDetectedModelType::all_variants() {
      assert_eq!(variant, UploadedVideoDetectedModelType::from_str(variant.to_str()).unwrap());
      assert_eq!(variant, UploadedVideoDetectedModelType::from_str(&format!("{}", variant)).unwrap());
      assert_eq!(variant, UploadedVideoDetectedModelType::from_str(&format!("{:?}", variant)).unwrap());
    }
    assert!(UploadedVideoDetectedModelType::from_str("foo").is_err());
  }
  ...
  fn serde_round_trip() {
    for variant in UploadedVideoDetectedModelType::all_variants() {
      ...
      let deserialized: UploadedVideoDetectedModelType = serde_json::from_str(&json).unwrap();
