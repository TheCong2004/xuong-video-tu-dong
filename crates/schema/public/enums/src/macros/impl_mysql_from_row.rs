  ...
  ($t:ident) => {
    ...
    impl crate::traits::mysql_from_row::MySqlFromRow<$t> for $t {
      fn try_from_mysql_row(row: &sqlx_mysql::MySqlRow, field_name: &str) -> Result<$t, sqlx::Error> {
        ...
        let output_type = Self::from_str(&value).map_err(|err| Error::ColumnDecode { index: format!("mysql_from_row failure on field {}: {:?}", field_name, err), source: format!("mysql_from_row failure on field {}: {:?}", field_name, err).into() })?;
