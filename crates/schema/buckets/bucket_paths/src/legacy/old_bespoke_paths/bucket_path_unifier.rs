...
impl BucketPathUnifier {
  ...
  pub fn tts_synthesizer_path(&self, tts_synthesizer_file_hash: &str) -> PathBuf {
    ...
    self.tts_synthesizer_model_root.join(hashed_path).join(model_filename)
  }
  ...
  pub fn vocoder_path(&self, vocoder_file_hash: &str) -> PathBuf {
    ...
    self.vocoder_model_root.join(hashed_path).join(model_filename)
  }
  ...
  pub fn tts_inference_wav_audio_output_path(&self, tts_inference_output_uuid: &str) -> PathBuf {
    ...
    self.tts_inference_output_root.join(hashed_path).join(audio_filename)
  }
  ...
  pub fn tts_inference_spectrogram_output_path(&self, tts_inference_output_uuid: &str) -> PathBuf {
    ...
    self.tts_inference_output_root.join(hashed_path).join(json_filename)
  }
  ...
  pub fn media_templates_for_w2l_path(&self, template_file_hash: &str) -> PathBuf {
    ...
    self.user_uploaded_w2l_templates_root.join(hashed_path).join(template_file_hash)
  }
  ...
  pub fn precomputed_faces_for_w2l_path(&self, template_file_hash: &str) -> PathBuf {
    ...
    self.user_uploaded_w2l_templates_root.join(hashed_path).join(faces_filename)
  }
  ...
  pub fn user_audio_for_w2l_inference_path(&self, audio_uuid: &str) -> PathBuf {
    ...
    self.user_uploaded_audio_for_w2l_root.join(hashed_path).join(audio_uuid)
  }
  ...
  pub fn w2l_inference_video_output_path(&self, w2l_inference_job_token: &str) -> PathBuf {
    ...
    if w2l_inference_job_token.contains(":") {
      if let Some((_token_type, token)) = w2l_inference_job_token.split_once(":") {
        ...
        return self.w2l_inference_output_root.join(hashed_path).join(video_filename);
      }
    }
    ...
    self.w2l_inference_output_root.join(hashed_path).join(video_filename)
  }
  ...
  pub fn zero_shot_tts_speaker_encoding(&self, entropic_hash: &str, version: u32) -> PathBuf {
    ...
    self.zs_tts_speaker_embedding_root.join(hashed_path).join(speaker_encoding_filename)
  }
  ...
  pub fn rvc_v2_model_index_path(&self, entropic_hash: &str) -> PathBuf {
    ...
    self.rvc_v2_model_root.join(hashed_path).join(model_index_filename)
  }
  ...
  pub fn rvc_v2_model_path(&self, entropic_hash: &str) -> PathBuf {
    ...
    self.rvc_v2_model_root.join(hashed_path).join(model_filename)
  }
  ...
  pub fn softvc_model_path(&self, softvc_model_file_hash: &str) -> PathBuf {
    ...
    self.softvc_model_root.join(hashed_path).join(model_filename)
  }
  ...
  pub fn so_vits_svc_model_path(&self, so_vits_svc_model_file_hash: &str) -> PathBuf {
    ...
    self.so_vits_svc_model_root.join(hashed_path).join(model_filename)
  }
  ...
  pub fn voice_conversion_inference_wav_audio_output_path(&self, vc_inference_output_uuid: &str) -> PathBuf {
    ...
    self.vc_inference_output_root.join(hashed_path).join(audio_filename)
  }
...
mod tests {
  ...
  fn test_tts_pretrained_vocoders_path() {
    ...
    assert_eq!(paths.tts_pretrained_vocoders_path("melgan.pth").to_str().unwrap(), "/test_path_tts_vocoders/melgan.pth");
  }
  ...
  fn test_vocoders_path() {
    let paths = get_instance();
    assert_eq!(paths.vocoder_path("foobar").to_str().unwrap(), "/test_path_user_vocoders/f/o/o/foobar.pt");
  }

  #[test]
  fn test_tts_synthesizer_path() {
    let paths = get_instance();
    assert_eq!(paths.tts_synthesizer_path("foobar").to_str().unwrap(), "/test_path_synthesizers/f/o/o/foobar.pt");
  }

  #[test]
  fn test_tts_zipped_synthesizer_path() {
    let paths = get_instance();
    assert_eq!(paths.tts_zipped_synthesizer_path("foobar").to_str().unwrap(), "/test_path_synthesizers/f/o/o/foobar.zip");
  }

  #[test]
  fn test_tts_inference_wav_audio_output_path() {
    let paths = get_instance();
    assert_eq!(paths.tts_inference_wav_audio_output_path("foobar").to_str().unwrap(), "/test_path_tts_output/f/o/o/vocodes_foobar.wav");
  }

  #[test]
  fn test_tts_inference_spectrogram_output_path() {
    let paths = get_instance();
    assert_eq!(paths.tts_inference_spectrogram_output_path("foobar").to_str().unwrap(), "/test_path_tts_output/f/o/o/foobar.json");
  }

  #[test]
  fn test_w2l_pretrained_models_path() {
    let paths = get_instance();
    assert_eq!(paths.w2l_pretrained_models_path("model.pth").to_str().unwrap(), "/test_path_w2l_pretrained_models/model.pth");
  }

  #[test]
  fn test_end_bump_video_for_w2l_path() {
    let paths = get_instance();
    assert_eq!(paths.end_bump_video_for_w2l_path("logo.mp4").to_str().unwrap(), "/test_path_w2l_end_bumps/logo.mp4");
  }

  #[test]
  fn test_user_audio_for_w2l_inference_path() {
    let paths = get_instance();
    assert_eq!(paths.user_audio_for_w2l_inference_path("foobar").to_str().unwrap(), "/test_path_w2l_audio/f/o/o/foobar");
  }

  #[test]
  fn test_media_templates_for_w2l_path() {
    let paths = get_instance();
    assert_eq!(paths.media_templates_for_w2l_path("foobar").to_str().unwrap(), "/test_path_w2l_templates/f/o/o/foobar");
  }

  #[test]
  fn test_precomputed_faces_for_w2l_path() {
    let paths = get_instance();
    assert_eq!(paths.precomputed_faces_for_w2l_path("foobar").to_str().unwrap(), "/test_path_w2l_templates/f/o/o/foobar_detected_faces.pickle");
  }
  ...
  fn test_w2l_inference_video_output_path() {
    ...
    assert_eq!(paths.w2l_inference_video_output_path("foobar").to_str().unwrap(), "/test_path_w2l_output/f/o/o/vocodes_video_foobar.mp4");

    // Case 2: Tokens with a "token type"
    // Note: that it removes the token type from dir path and handles the colon:
    assert_eq!(paths.w2l_inference_video_output_path("type:abcdef").to_str().unwrap(), "/test_path_w2l_output/a/b/c/vocodes_video_typeabcdef.mp4");
  ...
  fn test_zero_shot_tts_speaker_encoding_paths() {
    ...
    let version = 0;
    assert_eq!(paths.zero_shot_tts_speaker_encoding(entropic_hash, version).to_str().unwrap(), "/test/zs_tts_speaker_embedding_root/e/n/t/entropic_0.zs");

    let version = 23;
    assert_eq!(paths.zero_shot_tts_speaker_encoding(entropic_hash, version).to_str().unwrap(), "/test/zs_tts_speaker_embedding_root/e/n/t/entropic_23.zs");
  }
  ...
  fn test_rvc_model_and_index_paths() {
    let paths = get_instance();
    let entropic_hash = "entropic";
    assert_eq!(paths.rvc_v2_model_path(entropic_hash).to_str().unwrap(), "/test_path_rvc_v2_models/e/n/t/entropic.pt");

    assert_eq!(paths.rvc_v2_model_index_path(entropic_hash).to_str().unwrap(), "/test_path_rvc_v2_models/e/n/t/entropic.index");
  }

  #[test]
  fn test_softvc_model_path() {
    let paths = get_instance();
    assert_eq!(paths.softvc_model_path("foobar").to_str().unwrap(), "/test_path_softvc_models/f/o/o/foobar.pt");
  }

  #[test]
  fn test_so_vits_svc_model_path() {
    let paths = get_instance();
    assert_eq!(paths.so_vits_svc_model_path("foobar").to_str().unwrap(), "/test_path_so_vits_svc_models/f/o/o/foobar.pt");
  }

  #[test]
  fn test_vc_inference_wav_audio_output_path() {
    let paths = get_instance();
    assert_eq!(paths.voice_conversion_inference_wav_audio_output_path("foobar").to_str().unwrap(), "/test_path_vc_output/f/o/o/fakeyou_foobar.wav");
  }
