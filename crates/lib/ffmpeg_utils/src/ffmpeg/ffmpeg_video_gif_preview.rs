...
pub fn ffmpeg_video_gif_preview<I: AsRef<Path>, O: AsRef<Path>>(args: FfmpegVideoGifPreviewArgs<I, O>) -> AnyhowResult<()> {
  let mut command = Command::new("ffmpeg");

  command.arg("-nostdin").arg("-y").arg("-ss").arg("0").arg("-to").arg("5").arg("-i").arg(args.input_video_path.as_ref()).arg("-filter_complex").arg("fps=10,scale=360:-1[s]; [s]split[a][b]; [a]palettegen[palette]; [b][palette]paletteuse").arg(args.output_gif_path.as_ref());
...
mod tests {
  ...
  fn test_extract_gif_preview_from_mp4() {
    let input_path = test_file_path("test_data/video/mp4/golden_sun_garoh.mp4").expect("test video should exist");

    let temp_dir = TempDir::new_in("/tmp", "ffmpeg_gif_preview_test").expect("should create temp dir");

    let output_path = temp_dir.path().join("preview.gif");

    ffmpeg_video_gif_preview(FfmpegVideoGifPreviewArgs { input_video_path: &input_path, output_gif_path: &output_path }).expect("ffmpeg should succeed");
    ...
    let result = ffprobe::ffprobe(&output_path).expect("ffprobe should succeed");

    let video_stream = result.streams.iter().find(|s| s.codec_type.as_deref() == Some("video")).expect("should have a video stream");
