...
pub fn ffmpeg_video_first_frame_to_jpg_thumbnail<I: AsRef<Path>, O: AsRef<Path>>(args: FfmpegVideoFirstFrameToJpgThumbnailArgs<I, O>) -> AnyhowResult<()> {
  let mut command = Command::new("ffmpeg");

  command.arg("-nostdin").arg("-y").arg("-sseof").arg("-1").arg("-i").arg(args.input_video_path.as_ref()).arg("-vf").arg("scale=320:320:force_original_aspect_ratio=decrease").arg("-vframes").arg("1").arg(args.output_jpg_path.as_ref());
...
mod tests {
  ...
  fn test_extract_jpg_thumbnail_from_mp4() {
    ...
    let temp_dir = TempDir::new_in("/tmp", "ffmpeg_jpg_thumb_test").expect("should create temp dir");

    let output_path = temp_dir.path().join("thumbnail.jpg");

    ffmpeg_video_first_frame_to_jpg_thumbnail(FfmpegVideoFirstFrameToJpgThumbnailArgs { input_video_path: &input_path, output_jpg_path: &output_path }).expect("ffmpeg should succeed");
    ...
    let dimensions = ffprobe_get_dimensions(&output_path).expect("ffprobe should succeed").expect("dimensions should not be empty");
