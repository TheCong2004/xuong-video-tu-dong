...
mod tests {
  ...
  fn test_transcode_mov_to_mp4() {
    let input_path = test_file_path("test_data/video/mov/majoras_mask_intro.mov").expect("test video should exist");

    let temp_dir = TempDir::new_in("/tmp", "ffmpeg_transcode_to_mp4_test").expect("should create temp dir");

    let output_path = temp_dir.path().join("transcoded.mp4");

    ffmpeg_transcode_to_mp4(FfmpegTranscodeToMp4Args { video_input_path: &input_path, video_output_path: &output_path }).expect("ffmpeg should succeed");
    ...
    let probe = ffprobe::ffprobe(&output_path).expect("ffprobe should succeed on the transcoded file");
    ...
    assert!(format_name.contains("mp4"), "expected mp4 container, got format_name = {:?}", format_name,);

    // Video stream is H.264.
    let video_stream = probe.streams.iter().find(|s| s.codec_type.as_deref() == Some("video")).expect("should have a video stream");
    assert_eq!(video_stream.codec_name.as_deref(), Some("h264"), "expected H.264 video, got {:?}", video_stream.codec_name,);
    assert_eq!(video_stream.pix_fmt.as_deref(), Some("yuv420p"), "expected yuv420p pixel format, got {:?}", video_stream.pix_fmt,);

    // Audio stream is AAC (if the source has audio — Majora's Mask intro does).
    let audio_stream = probe.streams.iter().find(|s| s.codec_type.as_deref() == Some("audio")).expect("should have an audio stream");
    assert_eq!(audio_stream.codec_name.as_deref(), Some("aac"), "expected AAC audio, got {:?}", audio_stream.codec_name,);
