...
pub fn get_mimetype_for_bytes(bytes: &[u8]) -> Option<&'static str> {
  infer::get(bytes).map(|typ| typ.mime_type())
}

pub fn get_mimetype_for_bytes_or_default(bytes: &[u8]) -> &'static str {
  infer::get(bytes).map(|typ| typ.mime_type()).unwrap_or(DEFAULT_BINARY_MIMETYPE)
}
...
mod tests {
  ...
  fn test_mimetype_png() {
    ...
    let bytes: Vec<u8> = vec![137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 20, 193, 0, 0, 15, 239, 8, 6, 0, 0, 0, 163, 131, 119, 122, 0, 0, 0];
