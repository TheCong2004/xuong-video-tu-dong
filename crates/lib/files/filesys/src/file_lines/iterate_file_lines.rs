...
mod tests {
  ...
  fn load_file() {
    ...
    let lines = iterate_file_lines(filename).expect("should be readable file");

    let lines: Vec<String> = lines.map(|line| line.unwrap()).collect();

    assert_eq!(lines, vec!["# this is test data".to_string(), "".to_string(), "# local cidrs".to_string(), "127.0.0.0/24".to_string(), "192.168.0.0/24".to_string(),]);
  }
}
