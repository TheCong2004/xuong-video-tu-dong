...
pub fn iterate_trimmed_file_lines_without_comments<P>(filename: P) -> io::Result<impl Iterator<Item = io::Result<String>>>
where
  P: AsRef<Path>,
{
...
mod tests {
  ...
  fn load_file() {
    ...
    let lines = iterate_trimmed_file_lines_without_comments(filename).expect("should be readable file");

    let lines: Vec<String> = lines.filter_map(|line| line.ok()).collect();

    assert_eq!(lines, vec!["127.0.0.0/24".to_string(), "192.168.0.0/24".to_string(),]);
  }
}
