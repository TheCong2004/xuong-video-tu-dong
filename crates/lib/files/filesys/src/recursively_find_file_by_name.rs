...
pub fn recursively_find_file_by_name<P: AsRef<Path>, Q: AsRef<Path>>(search_filename: P, search_root: Q, max_depth: usize) -> AnyhowResult<Option<PathBuf>> {
  let walker = WalkDir::new(search_root).max_depth(max_depth);
...
mod tests {
  ...
  fn test_path_exists() {
    ...
    let result = recursively_find_file_by_name("super_mario_rpg_beware_the_forests_mushrooms.mp3", test_data_path, 5);
  ...
  fn test_path_does_not_exist() {
    let test_data_path = test_file_path("test_data/").expect("should work");
    let result = recursively_find_file_by_name("fake_name.foo", test_data_path, 5);
    let result = result.expect("should be ok");
