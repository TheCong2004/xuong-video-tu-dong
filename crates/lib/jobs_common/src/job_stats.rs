...
impl JobStats {
  pub fn new() -> Self {
    Self { inner: Arc::new(RwLock::new(JobStatsInner::default())) }
  }
  ...
  pub fn get_status(&self) -> AnyhowResult<SuccessAndFailureStats> {
    ...
    let lock = self.inner.read().map_err(|e| anyhow!("lock read error: {:?}", e))?;

    Ok(SuccessAndFailureStats { total_success_count: lock.total_success_count, total_failure_count: lock.total_failure_count, consecutive_success_count: lock.consecutive_success_count, consecutive_failure_count: lock.consecutive_failure_count, maybe_current_job: lock.maybe_current_job.as_ref().map(|job| JobDetail { job_token: job.job_token.clone(), job_started_at: job.job_started_at }) })
  }

  pub fn increment_failure_count(&self) -> AnyhowResult<SuccessAndFailureStats> {
    // NB: lock errors can't be moved between threads, so we change their type
    let mut lock = self.inner.write().map_err(|e| anyhow!("lock error: {:?}", e))?;
  ...
  pub fn record_job_start(&self, job_token: &str) -> AnyhowResult<SuccessAndFailureStats> {
    ...
    Ok(SuccessAndFailureStats { total_success_count: lock.total_success_count, total_failure_count: lock.total_failure_count, consecutive_success_count: lock.consecutive_success_count, consecutive_failure_count: lock.consecutive_failure_count, maybe_current_job: Some(JobDetail { job_token: job_token.to_string(), job_started_at: Utc::now() }) })
  }
  ...
  pub fn record_job_end(&self) -> AnyhowResult<SuccessAndFailureStats> {
    ...
    Ok(SuccessAndFailureStats { total_success_count: lock.total_success_count, total_failure_count: lock.total_failure_count, consecutive_success_count: lock.consecutive_success_count, consecutive_failure_count: lock.consecutive_failure_count, maybe_current_job: None })
  }
}
