...
use datadog_client::client::{DatadogClient, DistributionPoint, DistributionSeries, MetricPoint, MetricSeries};
...
impl MetricsWorker {
  pub(crate) fn new(queue: SampleQueue, client: DatadogClient, config: MetricsWorkerConfig, shutdown: Arc<AtomicBool>) -> Self {
  ...
  pub async fn run(&self) {
    info!("Metrics worker started (flush every {:?}, service={}, env={}, host={})", self.config.flush_interval, self.config.service_name, self.config.env, self.config.host.as_deref().unwrap_or("<unset>"),);
  ...
  async fn startup_ping(&self) {
    ...
    let ping = MetricSeries { metric: "metrics.startup.ping".to_string(), points: vec![MetricPoint(now, 1.0)], metric_type: "count".to_string(), tags: vec![format!("env:{}", self.config.env), format!("service:{}", self.config.service_name)], host: self.config.host.clone(), interval: Some(60) };
    match self.client.submit_series(vec![ping]).await {
      Ok(()) => info!("metrics: startup ping submitted successfully (service={}, env={})", self.config.service_name, self.config.env,),
      Err(e) => error!(
  ...
  async fn flush_once(&self) {
    ...
    match (&dist_result, &series_result) {
      (Ok(()), Ok(())) => {
        info!("metrics flush ok: {} samples → {} distribution series, {} count series", sample_count, dist_count, count_count,);
      },
      _ => {
        if let Err(e) = dist_result {
          warn!("metrics: distribution submission FAILED ({} series, {} samples): {}", dist_count, sample_count, e,);
        }
        if let Err(e) = series_result {
          warn!("metrics: series submission FAILED ({} series, {} samples): {}", count_count, sample_count, e,);
        }
      },
    }
  }
  ...
  fn build_request_metrics(&self, samples: Vec<RequestSample>, interval: i64, distributions: &mut Vec<DistributionSeries>, counts: &mut Vec<MetricSeries>) {
    ...
    for s in samples {
      ...
      let bucket = by_key.entry(key).or_insert_with(|| RequestBucket { durations_ms: Vec::new(), last_timestamp_secs: s.timestamp_secs });
    ...
    for (key, bucket) in by_key {
      ...
      distributions.push(DistributionSeries { metric: self.config.duration_metric_name.clone(), points: vec![DistributionPoint(bucket.last_timestamp_secs, bucket.durations_ms)], tags: tags.clone(), host: self.config.host.clone() });
      counts.push(MetricSeries { metric: self.config.count_metric_name.clone(), points: vec![MetricPoint(bucket.last_timestamp_secs, count)], metric_type: "count".to_string(), tags, host: self.config.host.clone(), interval: Some(interval) });
    }
  }

  fn build_counter_metrics(&self, samples: Vec<CounterSample>, interval: i64, counts: &mut Vec<MetricSeries>) {
    let mut by_key: HashMap<(String, Vec<String>), CounterBucket> = HashMap::new();
    for s in samples {
      let tags = sort_tags(s.tags);
      let entry = by_key.entry((s.metric, tags)).or_insert_with(|| CounterBucket { value: 0.0, last_timestamp_secs: s.timestamp_secs });
      entry.value += 1.0;
    ...
    for ((metric, tags), bucket) in by_key {
      counts.push(MetricSeries { metric, points: vec![MetricPoint(bucket.last_timestamp_secs, bucket.value)], metric_type: "count".to_string(), tags: with_service_env_tags(tags, &self.config), host: self.config.host.clone(), interval: Some(interval) });
    }
  }

  fn build_observation_metrics(&self, samples: Vec<ObservationSample>, distributions: &mut Vec<DistributionSeries>) {
    let mut by_key: HashMap<(String, Vec<String>), ObservationBucket> = HashMap::new();
    for s in samples {
      let tags = sort_tags(s.tags);
      let entry = by_key.entry((s.metric, tags)).or_insert_with(|| ObservationBucket { values: Vec::new(), last_timestamp_secs: s.timestamp_secs });
    ...
    for ((metric, tags), bucket) in by_key {
      distributions.push(DistributionSeries { metric, points: vec![DistributionPoint(bucket.last_timestamp_secs, bucket.values)], tags: with_service_env_tags(tags, &self.config), host: self.config.host.clone() });
    }
  }

  fn build_request_tags(&self, key: &RequestKey) -> Vec<String> {
    let class = status_class(key.status_code);
    vec![format!("route:{}", key.route), format!("method:{}", key.method), format!("status_code:{}", key.status_code), format!("status_class:{}", class), format!("env:{}", self.config.env), format!("service:{}", self.config.service_name)]
  }
}
...
mod tests {
  ...
  fn make_worker() -> MetricsWorker {
    ...
    let config = MetricsWorkerConfig { flush_interval: Duration::from_secs(10), duration_metric_name: "test.duration_ms".to_string(), count_metric_name: "test.count".to_string(), host: Some("test-host".to_string()), env: "test".to_string(), service_name: "test-service".to_string() };
  ...
  mod request_aggregation {
    ...
    fn sample(route: &str, method: &str, status: u16, dur: f64, ts: i64) -> Sample {
      Sample::Request(RequestSample { route: route.to_string(), method: method.to_string(), status_code: status, duration_ms: dur, timestamp_secs: ts })
    }

    #[test]
    fn groups_by_route_method_status() {
      let w = make_worker();
      let samples = vec![sample("/a", "GET", 200, 1.0, 100), sample("/a", "GET", 200, 2.0, 110), sample("/a", "GET", 500, 9.0, 120), sample("/b", "POST", 201, 5.0, 130)];
      let build = w.build_metrics(samples);
      assert_eq!(build.distributions.len(), 3);
      assert_eq!(build.counts.len(), 3);
      let a_200 = build.distributions.iter().find(|d| d.tags.iter().any(|t| t == "route:/a") && d.tags.iter().any(|t| t == "status_code:200")).expect("missing /a 200 series");
  ...
  mod counter_aggregation {
    ...
    fn counter(metric: &str, tags: Vec<&str>, ts: i64) -> Sample {
      Sample::Counter(CounterSample { metric: metric.to_string(), tags: tags.into_iter().map(|s| s.to_string()).collect(), timestamp_secs: ts })
    }

    #[test]
    fn groups_by_metric_and_tag_set() {
      let w = make_worker();
      let samples = vec![counter("orders.placed", vec!["region:us"], 1), counter("orders.placed", vec!["region:us"], 2), counter("orders.placed", vec!["region:eu"], 3), counter("orders.failed", vec!["region:us"], 4)];
      let build = w.build_metrics(samples);
      assert_eq!(build.distributions.len(), 0);
      assert_eq!(build.counts.len(), 3);
      let us_placed = build.counts.iter().find(|m| m.metric == "orders.placed" && m.tags.iter().any(|t| t == "region:us")).expect("missing orders.placed/us");
    ...
    fn tag_order_does_not_split_groups() {
      let w = make_worker();
      let samples = vec![counter("x", vec!["a:1", "b:2"], 1), counter("x", vec!["b:2", "a:1"], 2)];
      let build = w.build_metrics(samples);
  ...
  mod observation_aggregation {
    ...
    fn obs(metric: &str, value: f64, tags: Vec<&str>, ts: i64) -> Sample {
      Sample::Observation(ObservationSample { metric: metric.to_string(), value, tags: tags.into_iter().map(|s| s.to_string()).collect(), timestamp_secs: ts })
    }

    #[test]
    fn groups_into_distributions() {
      let w = make_worker();
      let samples = vec![obs("queue.depth", 10.0, vec!["queue:a"], 1), obs("queue.depth", 20.0, vec!["queue:a"], 2), obs("queue.depth", 30.0, vec!["queue:b"], 3)];
      let build = w.build_metrics(samples);
      assert_eq!(build.distributions.len(), 2);
      assert_eq!(build.counts.len(), 0);
      let a = build.distributions.iter().find(|d| d.tags.iter().any(|t| t == "queue:a")).expect("missing queue:a series");
