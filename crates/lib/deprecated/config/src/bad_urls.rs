...
fn is_bad_download_url(url: &str) -> anyhow::Result<bool> {
  ...
  match url.host() {
    Some(Host::Domain(domain)) => {
      ...
      let bad_host = domain.contains("fb.watch") || domain.contains("tiktok.com") || domain.contains("vm.tiktok.com") || domain.contains("youtu.be") || domain.contains("youtube.com");
...
mod tests {
  ...
  fn bad_tts_model_url() {
    ...
    assert_eq!(is_bad_tts_model_download_url("https://m.youtube.com/watch?v=HY-vzGBiAZo").unwrap(), true);
