...
impl CommonClaims {
  pub fn from_json(raw_json_claims: &str) -> Result<Self, JwtError> {
    let claims: serde_json::Map<String, Value> = serde_json::from_str(&raw_json_claims).map_err(|err| JwtError::CommonFieldError(format!("failure to parse claims json: {:?}", err)))?;

    // NB: Some JWTs have "iat" and "exp" as integer numbers, some have them as floats
    let iat = {
      let iat = claims.get("iat").map(|val| val.as_number()).flatten().ok_or_else(|| JwtError::CommonFieldError("no iat claim".to_string()))?;

      json_number_to_datetime(iat, "iat")?
    };

    let exp = {
      let exp = claims.get("exp").map(|val| val.as_number()).flatten().ok_or_else(|| JwtError::CommonFieldError("no exp claim".to_string()))?;

      json_number_to_datetime(exp, "exp")?
    };

    Ok(Self { created: iat, expiration: exp })
  }
}
