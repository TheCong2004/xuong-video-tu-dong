...
pub fn init_env_logger(default_if_absent: Option<&str>) {
  if env::var(ENV_RUST_LOG).as_ref().ok().is_none() {
    let default_log_level = default_if_absent.unwrap_or(DEFAULT_LOG_LEVEL);
    println!("Setting default logging level to \"{}\", override with env var {}.", default_log_level, ENV_RUST_LOG);
  ...
  env_logger::Builder::from_env(env_logger::Env::default())
    .format(|buf, record| {
      ...
      match trace_id::current_trace_id() {
        Some(trace) => writeln!(buf, "[{} {level_style}{}{level_style:#} {} {}] {}", timestamp, record.level(), record.target(), trace, record.args(),),
        None => writeln!(buf, "[{} {level_style}{}{level_style:#} {}] {}", timestamp, record.level(), record.target(), record.args(),),
      }
    })
    .init();
}
