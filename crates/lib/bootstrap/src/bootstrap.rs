...
pub fn bootstrap<P: AsRef<Path>>(args: BootstrapArgs<'_, P>) -> AnyhowResult<ContainerEnvironment> {
  ...
  let server_environment = easyenv::get_env_string_optional(SERVER_ENVIRONMENT).map(|environment| ServerEnvironment::from_str(&environment).ok_or(anyhow!("couldn't parse environment: {:?}", &environment))).transpose()?.unwrap_or(ServerEnvironment::Development);
  ...
  let server_hostname = hostname::get().ok().and_then(|h| h.into_string().ok()).unwrap_or_else(|| format!("{}-unknown", args.app_name));

  info!("With hostname: {:?}", &server_hostname);

  let cluster_name = easyenv::get_env_string_optional("K8S_CLUSTER_NAME").unwrap_or("unknown-cluster".to_string());
  ...
  Ok(ContainerEnvironment { server_environment, hostname: server_hostname, cluster_name, is_on_prem })
}
...
fn load_env_config_files<P: AsRef<Path>>(server_environment: ServerEnvironment, args: &BootstrapArgs<'_, P>) -> AnyhowResult<()> {
  let env_config_file_names = match server_environment {
    ...
    ServerEnvironment::Production => vec![format!("{}.common.env", &args.app_name), format!("{}.production.env", &args.app_name)],
  };

  for env_config_file in env_config_file_names.into_iter() {
    info!("Loading environment variable config file: {}", &env_config_file);

    let was_read = easyenv::maybe_read_env_config_from_filename_and_paths(&env_config_file, args.config_search_directories)?;
