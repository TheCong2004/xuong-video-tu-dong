...
impl RedisTtlCache {
  ...
  pub fn new_with_ttl(redis_pool: Pool<Client>, key_ttl: u64) -> Self {
    Self { redis_pool, default_key_ttl: key_ttl }
  }

  pub fn get_connection(&self) -> AnyhowResult<RedisTtlCacheConnection> {
    let redis = self.redis_pool.get()?;
    Ok(RedisTtlCacheConnection { redis_connection: redis, default_key_ttl: self.default_key_ttl })
  }
}
...
impl RedisTtlCacheConnection {
  ...
  pub fn get_from_cache<T>(&mut self, item_key: &str) -> AnyhowResult<Option<T>>
  where
  ...
  pub fn persist_to_cache<T>(&mut self, item_key: &str, item: T) -> AnyhowResult<()>
  where
    T: Serialize,
  {
    let redis_key = Self::cache_key(item_key);
  ...
  pub async fn lazy_load_if_not_cached<T, F, Fut>(&mut self, item_key: &str, loader_func: F) -> AnyhowResult<Option<T>>
  where
