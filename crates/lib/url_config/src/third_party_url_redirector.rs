...
impl ThirdPartyUrlRedirector {
  ...
  pub fn frontend_redirect_url_for_path(&self, http_request: &HttpRequest, path: &str) -> anyhow::Result<String> {
    let request_hostname = get_request_host(http_request).ok_or(anyhow!("request did not have host header"))?;

    fn url_from_hostname_and_path(scheme: &str, hostname: &str, path: &str) -> anyhow::Result<String> {
      Ok(Uri::builder().scheme(scheme).authority(hostname).path_and_query(path).build()?.to_string())
    }
    ...
    let redirect_hostname = match (self.environment, request_hostname) {
      ...
      (ServerEnvironment::Development, hostname) => {
        ...
        let maybe_port = parts.get(1).map(|p| u32::from_str(p)).transpose()?;
...
mod tests {
  ...
  fn request_with_host(hostname: &str) -> HttpRequest {
    TestRequest::default().insert_header((HOST, hostname)).to_http_request()
  }
  ...
  fn test_wrong_environment_development_hostnames() {
    let redirector = ThirdPartyUrlRedirector::new(ServerEnvironment::Development);
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("api.storyteller.io"), "/foo").is_err());
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("api.fakeyou.com"), "/foo").is_err());
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("unrelated.com"), "/foo").is_err());
  }

  #[test]
  fn test_wrong_environment_production_hostnames() {
    let redirector = ThirdPartyUrlRedirector::new(ServerEnvironment::Production);
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("dev.api.storyteller.io"), "/foo").is_err());
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("dev.api.fakeyou.com"), "/foo").is_err());
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("localhost"), "/foo").is_err());
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("localhost:1000"), "/foo").is_err());
    assert!(redirector.frontend_redirect_url_for_path(&request_with_host("unrelated.com"), "/foo").is_err());
  }
}
