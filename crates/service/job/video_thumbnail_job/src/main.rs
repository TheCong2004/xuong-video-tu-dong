...
async fn main() -> AnyhowResult<()> {
  let container_environment = bootstrap(BootstrapArgs { app_name: "video-thumbnail-job", default_logging_override: Some(DEFAULT_RUST_LOG), config_search_directories: &[".", "./config", "crates/service/job/video_thumbnail_job/config"], ignore_legacy_dot_env_file: true })?;
  ...
  let poll_interval_millis: u64 = easyenv::get_env_num("VIDEO_THUMBNAIL_POLL_INTERVAL_MILLIS", 30_000)?;

  let query_delay_millis: u64 = easyenv::get_env_num("VIDEO_THUMBNAIL_QUERY_DELAY_MILLIS", 100)?;

  let query_failure_retry_delay_millis: u64 = easyenv::get_env_num("VIDEO_THUMBNAIL_QUERY_FAILURE_RETRY_DELAY_MILLIS", 10_000)?;

  // Optional overrides for query parameters
  let custom_max_lookback_hours: Option<i32> = easyenv::get_env_string_optional("VIDEO_THUMBNAIL_MAX_LOOKBACK_HOURS").and_then(|s| s.parse().ok());

  let custom_page_size: Option<i64> = easyenv::get_env_string_optional("VIDEO_THUMBNAIL_PAGE_SIZE").and_then(|s| s.parse().ok());

  // Temp directory for video downloads and thumbnail intermediates
  let temp_dir: PathBuf = easyenv::get_env_pathbuf_or_default("VIDEO_THUMBNAIL_TEMP_DIR", "/tmp/video-thumbnails");

  // Ensure the temp directory exists
  tokio::fs::create_dir_all(&temp_dir).await?;

  // Optional sharding configuration
  let maybe_number_of_shards: Option<u8> = easyenv::get_env_string_optional("NUMBER_OF_SHARDS").and_then(|s| s.parse().ok());
  let maybe_shard_index: Option<u8> = easyenv::get_env_string_optional("SHARD_INDEX").and_then(|s| s.parse().ok());
  ...
  let create_server_args = CreateServerArgs { container_environment: container_environment.clone(), job_stats: job_stats.clone(), pager: pager.clone() };

  let job_dependencies = JobDependencies { mysql_pool, public_bucket_client, server_environment, job_stats, poll_interval_millis, query_delay_millis, query_failure_retry_delay_millis, custom_max_lookback_hours, custom_page_size, temp_dir, shard_info, application_shutdown: application_shutdown.clone(), pager };
