...
pub async fn dispatch_job_to_handler<'a, 'b: 'a>(args: DispatchJobToHandlerArgs<'a, 'b>) -> AnyhowResult<Option<CreatedEntityDetails>> {
  ...
  match args.job.download_type {
    GenericDownloadType::HifiGan => {
      let results = process_hifigan_vocoder(args.job_runner_state, args.job, args.temp_dir, args.download_filename, args.redis_logger).await?;
      entity_token = results.entity_token.clone();
      entity_type = results.entity_type.clone();
    },
    GenericDownloadType::HifiGanRocketVc => {
      let results = process_hifigan_softvc_vocoder(args.job_runner_state, args.job, args.temp_dir, args.download_filename, args.redis_logger).await?;
      entity_token = results.entity_token.clone();
      entity_type = results.entity_type.clone();
    },
    ...
    GenericDownloadType::RocketVc => {
      let results = process_softvc_model(args.job_runner_state, args.job, args.temp_dir, args.download_filename, args.redis_logger).await?;
      entity_token = results.entity_token.clone();
      entity_type = results.entity_type.clone();
    },
    GenericDownloadType::RvcV2 => {
      let results = process_rvc_v2_model(args.job_runner_state, args.job, args.temp_dir, args.download_filename, args.redis_logger).await?;
      entity_token = results.entity_token.clone();
      entity_type = results.entity_type.clone();
    },
    GenericDownloadType::SoVitsSvc => {
      let results = process_so_vits_svc_model(args.job_runner_state, args.job, args.temp_dir, args.download_filename, args.redis_logger).await?;
      entity_token = results.entity_token.clone();
      entity_type = results.entity_type.clone();
    },
    GenericDownloadType::Tacotron2 => {
      let results = process_tacotron_model(args.job_runner_state, args.job, args.temp_dir, args.download_filename, args.redis_logger).await?;
      entity_token = results.entity_token.clone();
      entity_type = results.entity_type.clone();
    },
    GenericDownloadType::Vits => {
      let results = process_vits_model(args.job_runner_state, args.job, args.temp_dir, args.download_filename, args.redis_logger).await?;
      entity_token = results.entity_token.clone();
      entity_type = results.entity_type.clone();
    },
  }
  ...
  if let Some(entity_type) = entity_type {
    if let Some(entity_token) = entity_token {
      return Ok(Some(CreatedEntityDetails { entity_token, entity_type }));
    }
  }
