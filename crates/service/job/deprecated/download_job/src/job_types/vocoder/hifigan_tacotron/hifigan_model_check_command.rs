...
impl HifiGanModelCheckCommand {
  pub fn new(hifigan_root_code_directory: &str, maybe_virtual_env_activation_command: Option<&str>, hifigan_model_check_script_name: &str, maybe_docker_options: Option<DockerOptions>) -> Self {
    Self { hifigan_root_code_directory: hifigan_root_code_directory.to_string(), maybe_virtual_env_activation_command: maybe_virtual_env_activation_command.map(|s| s.to_string()), hifigan_model_check_script_name: hifigan_model_check_script_name.to_string(), maybe_docker_options }
  }
