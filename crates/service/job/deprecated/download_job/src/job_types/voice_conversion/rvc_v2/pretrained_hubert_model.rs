...
impl PretrainedHubertModel {
  pub fn from_env() -> Self {
    ...
    // NB: For now rvc-v2 is all that uses this hubert, but other models
    // may (re)use it in the future.
    let filesystem_path = easyenv::get_env_pathbuf_or_default("RVC_V2_PRETRAINED_HUBERT_FILESYSTEM_PATH", "/tmp/hubert/rvc_v2_hubert_base.pt");

    Self { cloud_bucket_path, filesystem_path }
  }

  pub async fn download_if_not_on_filesystem(&self, bucket_client: &LegacyBucketClient, temp_dir: &TempDir) -> AnyhowResult<()> {
    ...
    std::fs::rename(&temp_path, &self.filesystem_path).map_err(|e| {
      safe_delete_directory(&temp_dir);
