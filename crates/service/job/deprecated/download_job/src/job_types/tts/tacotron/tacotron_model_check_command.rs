...
impl TacotronModelCheckCommand {
  pub fn new(tacotron_root_code_directory: &str, maybe_virtual_env_activation_command: Option<&str>, tacotron_model_check_script_name: &str, maybe_docker_options: Option<DockerOptions>) -> Self {
    Self { tacotron_root_code_directory: tacotron_root_code_directory.to_string(), maybe_virtual_env_activation_command: maybe_virtual_env_activation_command.map(|s| s.to_string()), tacotron_model_check_script_name: tacotron_model_check_script_name.to_string(), maybe_docker_options }
  }
