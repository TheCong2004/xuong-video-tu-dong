...
pub async fn password_reset_email_sender(job: &AvailableEmailSenderJob, job_dependencies: &JobDependencies) -> Result<(), ProcessSingleJobError> {
  let maybe_inner_args = job.maybe_email_args.as_ref().and_then(|args| args.args.as_ref());

  let maybe_secret_key = match maybe_inner_args {
    Some(PolymorphicEmailSenderJobArgs::Pr(reset_args)) => reset_args.password_reset_secret_key.clone(),
    _ => {
  ...
  let from_address = "Support <support@storyteller.ai>".parse().map_err(|err| {
    log::error!("Error parsing from address: {err}");
    ProcessSingleJobError::InvalidJob(anyhow!("Error parsing from address: {err}"))
  })?;

  let to_address = job.destination_email_address.parse().map_err(|err| {
  ...
  let email = Message::builder().from(from_address).to(to_address).subject("FakeYou Password Reset").header(letter_exports::ContentType::TEXT_HTML).body(message).map_err(|err| {
