...
pub fn init_otel_metrics_pipeline(job_instrument_labels: JobInstrumentLabels) -> Result<(), opentelemetry::metrics::MetricsError> {
...
impl JobInstruments {
  pub fn new_from_meter(meter: Meter) -> Self {
    ...
    let batch_query_duration = meter.u64_histogram("inference_job_batch_query_duration").with_unit(Unit::new("milliseconds")).with_description("inference job batch query duration").init();

    let batch_size = meter.u64_histogram("inference_job_batch_size").with_unit(Unit::new("jobs")).with_description("inference job batch size").init();

    let inference_command_execution_duration = meter.u64_histogram("inference_job_inference_command_execution_duration").with_unit(Unit::new("milliseconds")).with_description("inference job inference command execution duration").init();

    let batch_processing_duration = meter.u64_histogram("inference_job_batch_processing_duration").with_unit(Unit::new("milliseconds")).with_description("inference job batch processing duration").init();

    let total_job_count = meter.u64_counter("inference_job_total_job_count").with_unit(Unit::new("jobs")).with_description("inference job total job count").init();

    let job_success_count = meter.u64_counter("inference_job_success_count").with_unit(Unit::new("jobs")).with_description("inference job success count").init();

    let job_failure_count = meter.u64_counter("inference_job_failure_count").with_unit(Unit::new("jobs")).with_description("inference job failure count").init();

    Self { job_duration, inference_command_execution_duration, batch_query_duration, batch_size, batch_processing_duration, total_job_count, job_success_count, job_failure_count }
  }
}
