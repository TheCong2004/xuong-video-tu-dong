...
pub async fn process_single_f5_tts_inference_job(job_dependencies: &JobDependencies, job: &AvailableInferenceJob) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let f5_tts_deps = job_dependencies.job.job_specific_dependencies.maybe_f5_tts_dependencies.as_ref().ok_or_else(|| ProcessSingleJobError::JobSystemMisconfiguration(Some("Missing F5-TTS dependencies".to_string())))?;
  ...
  let command_exit_status = f5_tts_deps.inference_command.execute_inference(InferenceArgs { stderr_output_file: &stderr_output_file, stdout_output_file: &stdout_output_file, reference_audio_path: &original_media_upload_fs_path, input_text_file: &text_input_fs_path, reference_transcript_path: None, output_audio_directory: &output_dir });
  ...
  if let Some(truncate_seconds) = job_args.maybe_truncate_seconds {
    if truncate_seconds > 0 {
      ...
      let command_exit_status = f5_tts_deps.ffmpeg_command_runner.run_with_subprocess(RunAsSubprocessArgs { args: Box::new(&FfmpegAudioTruncateArgs { input_audio_file: &output_file_path, output_audio_file: &truncated_audio_output_path, truncate_seconds: truncate_seconds as usize }), stderr: StreamRedirection::Pipe, stdout: StreamRedirection::Pipe });
