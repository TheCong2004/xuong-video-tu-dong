...
pub async fn download_static_dependencies<'a>(job_dependencies: &'a JobDependencies, job: &'a AvailableInferenceJob, model_dependencies: &'a Tacotron2Dependencies, mut job_progress_reporter: &mut Box<dyn JobProgressReporter>) -> Result<StaticDependencies, ProcessSingleJobError> {
  ...
  let waveglow_vocoder_model_fs_path = {
    ...
    maybe_download_file_from_bucket(MaybeDownloadArgs { name_or_description_of_file: "waveglow vocoder model", final_filesystem_file_path: &waveglow_vocoder_model_fs_path, bucket_object_path: &waveglow_vocoder_model_object_path, bucket_client: &job_dependencies.buckets.private_bucket_client, job_progress_reporter: &mut job_progress_reporter, job_progress_update_description: "downloading vocoder (1 of 3)", job_id: job.id.0, scoped_tempdir_creator: &job_dependencies.fs.scoped_temp_dir_creator_for_long_lived_downloads, maybe_existing_file_minimum_size_required: Some(1000) }).await?;
  ...
  let pretrained_hifigan_vocoder_model_fs_path = {
    ...
    maybe_download_file_from_bucket(MaybeDownloadArgs { name_or_description_of_file: "hifigan vocoder model", final_filesystem_file_path: &hifigan_vocoder_model_fs_path, bucket_object_path: &hifigan_vocoder_model_object_path, bucket_client: &job_dependencies.buckets.private_bucket_client, job_progress_reporter: &mut job_progress_reporter, job_progress_update_description: "downloading vocoder (2 of 3)", job_id: job.id.0, scoped_tempdir_creator: &job_dependencies.fs.scoped_temp_dir_creator_for_long_lived_downloads, maybe_existing_file_minimum_size_required: Some(1000) }).await?;
  ...
  let hifigan_superres_vocoder_model_fs_path = {
    ...
    maybe_download_file_from_bucket(MaybeDownloadArgs { name_or_description_of_file: "hifigan superres vocoder model", final_filesystem_file_path: &hifigan_superres_vocoder_model_fs_path, bucket_object_path: &hifigan_superres_vocoder_model_object_path, bucket_client: &job_dependencies.buckets.private_bucket_client, job_progress_reporter: &mut job_progress_reporter, job_progress_update_description: "downloading vocoder (3 of 3)", job_id: job.id.0, scoped_tempdir_creator: &job_dependencies.fs.scoped_temp_dir_creator_for_long_lived_downloads, maybe_existing_file_minimum_size_required: Some(1000) }).await?;

    hifigan_superres_vocoder_model_fs_path
  };

  Ok(StaticDependencies { waveglow_vocoder_model_fs_path, pretrained_hifigan_vocoder_model_fs_path, hifigan_superres_vocoder_model_fs_path })
}
