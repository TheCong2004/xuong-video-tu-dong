...
pub async fn process_rvc_job(args: RvcV2ProcessJobArgs<'_>) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let model_dependencies = args.job_dependencies.job.job_specific_dependencies.maybe_rvc_v2_dependencies.as_ref().ok_or_else(|| ProcessSingleJobError::JobSystemMisconfiguration(Some("missing RVC dependencies".to_string())))?;

  // ==================== DOWNLOAD HUBERT ==================== //

  info!("Download RVC hubert model (if not present)...");

  model_dependencies.pretrained_hubert_model.download_if_not_on_filesystem(&args.job_dependencies.buckets.private_bucket_client, &args.job_dependencies.fs.scoped_temp_dir_creator_for_short_lived_downloads).await.map_err(|err| {
  ...
  let rvc_v2_model_fs_path = {
    ...
    create_dir_all_if_missing(args.job_dependencies.fs.semi_persistent_cache.voice_conversion_model_directory()).map_err(|e| {
    ...
    let bucket_client = if vc_model.get_model_token().starts_with(ModelWeightToken::token_prefix()) {
    ...
    maybe_download_file_from_bucket(MaybeDownloadArgs { name_or_description_of_file: "rvc (v2) model", final_filesystem_file_path: &fs_path, bucket_object_path: &model_object_path, bucket_client, job_progress_reporter: &mut job_progress_reporter, job_progress_update_description: "downloading rvc (v2) model", job_id: job.id.0, scoped_tempdir_creator: &args.job_dependencies.fs.scoped_temp_dir_creator_for_short_lived_downloads, maybe_existing_file_minimum_size_required: None }).await?;
  ...
  if vc_model.has_index_file() {
    ...
    maybe_rvc_v2_model_index_fs_path = {
      ...
      let model_index_object_path = vc_model.get_index_file_cloud_bucket_path(&args.job_dependencies.buckets.bucket_path_unifier).ok_or_else(|| ProcessSingleJobError::from_anyhow_error(anyhow!("Index file path couldn't be determined.")))?;
      ...
      maybe_download_file_from_bucket(MaybeDownloadArgs { name_or_description_of_file: "rvc (v2) model index", final_filesystem_file_path: &fs_path, bucket_object_path: &model_index_object_path, bucket_client, job_progress_reporter: &mut job_progress_reporter, job_progress_update_description: "downloading rvc (v2) model index", job_id: job.id.0, scoped_tempdir_creator: &args.job_dependencies.fs.scoped_temp_dir_creator_for_short_lived_downloads, maybe_existing_file_minimum_size_required: None }).await?;
  ...
  let command_exit_status = model_dependencies.inference_command.execute_inference(InferenceArgs { model_path: &rvc_v2_model_fs_path, maybe_model_index_path: maybe_rvc_v2_model_index_fs_path, hubert_path: &model_dependencies.pretrained_hubert_model.filesystem_path, rmvpe_path: &model_dependencies.downloaders.rmvpe.filesystem_path, input_path: &input_wav_path, output_path: &output_audio_fs_path, stderr_output_file: &stderr_output_file, maybe_override_f0_method, maybe_f0_up_key });
  ...
  let file_size_bytes = file_size(&output_audio_fs_path).map_err(|err| ProcessSingleJobError::Other(err))?;

  let maybe_mimetype = get_mimetype_for_file(&output_audio_fs_path).map_err(|err| ProcessSingleJobError::from_io_error(err))?.map(|mime| mime.to_string());

  let audio_info = decode_basic_audio_file_info(&output_audio_fs_path, maybe_mimetype.as_deref(), None).map_err(|err| ProcessSingleJobError::Other(err))?;
  ...
  let file_checksum = sha256_hash_file(&output_audio_fs_path).map_err(|err| ProcessSingleJobError::Other(anyhow!("Error hashing file: {:?}", err)))?;

  // TODO: Make a new python image that generates spectrograms from any audio file.

  let file_metadata = FileMetadata { duration_millis: audio_info.duration_millis, mimetype: maybe_mimetype.clone(), file_size_bytes };
  ...
  args.job_dependencies.buckets.public_bucket_client.upload_filename_with_content_type(&result_bucket_object_pathbuf, &output_audio_fs_path, "audio/wav").await.map_err(|e| ProcessSingleJobError::Other(e))?;
  ...
  args.job_dependencies.clients.firehose_publisher.vc_inference_finished(maybe_user_token.as_ref(), &job.inference_job_token, inference_result_token.as_str()).await.map_err(|e| {
  ...
  Ok(JobSuccessResult { maybe_result_entity: Some(ResultEntity { entity_type: InferenceResultType::MediaFile, entity_token: inference_result_token.to_string() }), inference_duration })
}
