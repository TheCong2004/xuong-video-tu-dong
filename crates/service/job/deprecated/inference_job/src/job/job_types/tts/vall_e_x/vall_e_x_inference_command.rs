...
impl VallEXInferenceCommand {
  ...
  pub fn from_env() -> AnyhowResult<Self> {
    let root_code_directory = easyenv::get_env_pathbuf_required("VALL_E_X_INFERENCE_ROOT_DIRECTORY")?;

    let maybe_inference_command = easyenv::get_env_string_optional("VALL_E_X_INFERENCE_COMMAND");

    // EVENTUALLY should remove the check
    // Optional, eg. `./infer.py`. Typically we'll use the command form instead.
    let maybe_inference_executable = easyenv::get_env_pathbuf_optional("VALL_E_X_INFERENCE_EXECUTABLE");
    ...
    let maybe_virtual_env_activation_command = easyenv::get_env_string_optional("VALL_E_X_INFERENCE_MAYBE_VENV_COMMAND");

    let maybe_default_config_path = easyenv::get_env_pathbuf_optional("VALL_E_X_INFERENCE_MAYBE_DEFAULT_CONFIG_PATH");

    let maybe_execution_timeout = easyenv::get_env_duration_seconds_optional("TIMEOUT_SECONDS");

    // Probably for local
    let maybe_docker_options = easyenv::get_env_string_optional("VALL_E_X_INFERENCE_MAYBE_DOCKER_IMAGE").map(|image_name| DockerOptions { image_name, maybe_bind_mount: Some(DockerFilesystemMount::tmp_to_tmp()), maybe_environment_variables: None, maybe_gpu: Some(DockerGpu::All) });

    // Override for --checkpoint_dir at inference time
    let alternate_checkpoint_dir = easyenv::get_env_pathbuf_optional("VALL_E_X_ALTERNATE_CHECKPOINT_PATH");
...
impl VallEXCreateEmbeddingCommand {
  ...
  pub fn from_env() -> AnyhowResult<Self> {
    ...
    // Override for --checkpoint_dir at inference time
    let alternate_checkpoint_dir = easyenv::get_env_pathbuf_optional("VALL_E_X_CHECKPOINT_PATH");
