...
pub async fn process_video_style_transfer_job(deps: &JobDependencies, job: &AvailableInferenceJob) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let redis_pool_dep = deps.db.maybe_keepalive_redis_pool.clone();

  let redis_pool = redis_pool_dep.ok_or_else(|| ProcessSingleJobError::Other(anyhow!("failed to get redis pool")))?;
  // .map(|redis| redis.get())
  // .transpose()
  ...
  match job_args.workflow_source {
    Some(workflow_token) => {
      let retrieved_workflow_record = get_weight_by_token_with_transactor(workflow_token, false, Transactor::for_connection(&mut mysql_connection)).await?.ok_or_else(|| ProcessSingleJobError::Other(anyhow!("Workflow not found")))?;

      let bucket_details = RemoteCloudBucketDetails { object_hash: retrieved_workflow_record.public_bucket_hash, prefix: retrieved_workflow_record.maybe_public_bucket_prefix.unwrap(), suffix: retrieved_workflow_record.maybe_public_bucket_extension.unwrap() };
  ...
  let maybe_args = job.maybe_inference_args.as_ref().map(|args| args.args.as_ref()).flatten();
  ...
  workflow_path = write_workflow_prompt(WorkflowPromptArgs { workflow_path: &workflow_path, comfy_args: &comfy_args, model_dependencies: &comfy_deps, maybe_positive_prompt: comfy_args.positive_prompt.as_deref(), maybe_negative_prompt: comfy_args.negative_prompt.as_deref() })?;
  ...
  match job_args.maybe_lora_model {
    Some(lora_model_weight_token) => {
      ...
      // make if not exist
      if !lora_dir.exists() {
        std::fs::create_dir_all(&lora_dir).map_err(|err| ProcessSingleJobError::IoError(err))?;
      }

      info!("Querying lora model by token: {:?} ...", &lora_model_weight_token);

      let retrieved_lora_record = get_weight_by_token_with_transactor(lora_model_weight_token, true, Transactor::for_connection(&mut mysql_connection)).await?.ok_or_else(|| {
      ...
      let bucket_details = RemoteCloudBucketDetails { object_hash: retrieved_lora_record.public_bucket_hash, prefix: retrieved_lora_record.maybe_public_bucket_prefix.unwrap(), suffix: retrieved_lora_record.maybe_public_bucket_extension.unwrap() };
  ...
  if !input_dir.exists() {
    std::fs::create_dir_all(&input_dir).map_err(|err| ProcessSingleJobError::IoError(err))?;
  }
  ...
  if let Some(ipa_media_token) = comfy_args.global_ip_adapter_token.as_ref() {
    let results = download_global_ipa_image(DownloadGlobalIpaImageArgs { ipa_media_token, comfy_input_directory: &input_dir, remote_cloud_file_client: &remote_cloud_file_client }, Transactor::for_connection(&mut mysql_connection)).await?;
  ...
  let mut videos = download_input_videos(DownloadInputVideoArgs { job_args: &job_args, comfy_dirs: &comfy_dirs, mysql_connection: &mut mysql_connection, remote_cloud_file_client: &remote_cloud_file_client }).await?;
  ...
  let expected_frame_count = preprocess_trim_and_resample_videos(ProcessTrimAndResampleVideoArgs { comfy_args, comfy_deps, comfy_dirs: &comfy_dirs, videos: &mut videos })?;

  // ========================= PREPROCESS AUDIO ======================== //

  let mut lipsync_enabled = comfy_args.lipsync_enabled.unwrap_or(false);
  if let Err(err) = preprocess_save_audio(ProcessSaveAudioArgs { comfy_deps, videos: &mut videos }) {
  ...
  let maybe_positive_prompt_filename = comfy_args.positive_prompt.as_deref().map(|prompt| std::fs::write(&positive_prompt_file, prompt).map(|_| positive_prompt_file.as_path()).map_err(|e| ProcessSingleJobError::IoError(e))).transpose()?;

  let maybe_negative_prompt_filename = comfy_args.negative_prompt.as_deref().map(|prompt| std::fs::write(&negative_prompt_file, prompt).map(|_| negative_prompt_file.as_path()).map_err(|e| ProcessSingleJobError::IoError(e))).transpose()?;

  let maybe_travel_prompt_filename = comfy_args.travel_prompt.as_deref().map(|prompt| std::fs::write(&travel_prompt_file, prompt).map(|_| travel_prompt_file.as_path()).map_err(|e| ProcessSingleJobError::IoError(e))).transpose()?;

  let inference_details = InferenceDetails::NewPythonArgs { maybe_style: comfy_args.style_name, maybe_positive_prompt_filename, maybe_negative_prompt_filename, maybe_travel_prompt_filename };
  ...
  let mut preview_processor = PreviewProcessor::new(job.inference_job_token.clone(), redis_pool, preview_frames_dir.clone(), previews_media_file_directory, expected_frame_count as u32, rx);
  ...
  let command_exit_status = comfy_deps
    ...
    .execute_inference(
      ...
      InferenceArgs {
        ...
        global_ipa_image_filename: global_ipa_image.as_ref().map(|image| path_to_string(&image.ipa_image_path)),
        global_ipa_strength: None, // TODO: Expose a UI slider
        depth_video_path: videos.maybe_depth.as_ref().map(|v| v.maybe_processed_path.as_deref()).flatten(),
        normal_video_path: videos.maybe_normal.as_ref().map(|v| v.maybe_processed_path.as_deref()).flatten(),
        outline_video_path: videos.maybe_outline.as_ref().map(|v| v.maybe_processed_path.as_deref()).flatten(),
      },
    )
    .await;
  ...
  if let Err(err) = check_file_exists(&videos.primary_video.comfy_output_video_path) {
    ...
    safe_delete_possible_files_and_directories::<&Path>(&[Some(&negative_prompt_file), Some(&output_dir), Some(&positive_prompt_file), Some(&preview_frames_dir), Some(&stderr_output_file), Some(&stdout_output_file), Some(&travel_prompt_file), Some(work_temp_dir.path()), Some(workflow_path.as_ref()), global_ipa_image.as_ref().map(|ipa| ipa.ipa_image_path.as_path()), maybe_lora_path.as_deref()]);

    return Err(ProcessSingleJobError::Other(anyhow!("Output file did not exist: {:?}", &videos.primary_video.comfy_output_video_path)));
  }

  // ==================== COPY BACK AUDIO ==================== //

  post_process_restore_audio(PostProcessRestoreVideoArgs { comfy_deps, videos: &mut videos });

  // ==================== OPTIONAL WATERMARK ==================== //

  post_process_add_watermark(PostProcessAddWatermarkArgs { comfy_deps, videos: &mut videos });
  ...
  let result = validate_and_save_results(SaveResultsArgs { job, deps: &deps, job_args: &job_args, comfy_deps, comfy_args, videos: &videos, job_progress_reporter: &mut job_progress_reporter, inference_duration }).await;
  ...
  let media_file_token = match result {
    ...
    Err(err) => {
      ...
      // NB: Forcing generic type to `&Path` with turbofish
      safe_delete_possible_files_and_directories::<&Path>(&[Some(&comfy_dirs.comfy_output_dir), Some(&negative_prompt_file), Some(&output_dir), Some(&preview_frames_dir), Some(&stderr_output_file), Some(&stdout_output_file), Some(&travel_prompt_file), Some(&positive_prompt_file), Some(work_temp_dir.path()), Some(workflow_path.as_ref()), global_ipa_image.as_ref().map(|ipa| ipa.ipa_image_path.as_path()), maybe_lora_path.as_deref()]);
...
fn safe_delete_primary_videos(video: &PrimaryInputVideoAndPaths) {
  safe_delete_possible_files_and_directories::<&Path>(&[Some(&video.comfy_input_video_path), Some(&video.comfy_output_video_path), Some(&video.original_download_path), Some(&video.trimmed_wav_audio_path), Some(video.get_final_video_to_upload()), Some(video.get_non_watermarked_video_to_upload()), Some(video.video_to_watermark()), video.audio_restored_video_path.as_deref(), video.maybe_trimmed_resampled_path.as_deref(), video.watermarked_video_path.as_deref()]);
}

fn safe_delete_secondary_videos(video: &SecondaryInputVideoAndPaths) {
  safe_delete_possible_files_and_directories::<&Path>(&[Some(&video.original_download_path), video.maybe_processed_path.as_deref()]);
}
