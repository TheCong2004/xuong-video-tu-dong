...
impl StableDiffusionInferenceCommand {
  pub fn new<P: AsRef<Path>>(stable_diffusion_root_code_directory: P, executable_or_command: ExecutableOrCommand, maybe_virtual_env_activation_command: Option<&str>, maybe_default_config_path: Option<P>, maybe_docker_options: Option<DockerOptions>, maybe_execution_timeout: Option<Duration>, alternate_checkpoint_dir: Option<PathBuf>) -> Self {
    Self { stable_diffusion_root_code_directory: stable_diffusion_root_code_directory.as_ref().to_path_buf(), executable_or_command, maybe_virtual_env_activation_command: maybe_virtual_env_activation_command.map(|s| s.to_string()), maybe_default_config_path: maybe_default_config_path.map(|p| p.as_ref().to_path_buf()), maybe_docker_options, maybe_execution_timeout, alternate_checkpoint_dir }
  }

  pub fn from_env() -> AnyhowResult<Self> {
    let stable_diffusion_root_code_directory = easyenv::get_env_pathbuf_required("STABLE_DIFFUSION_INFERENCE_ROOT_DIRECTORY")?;

    let maybe_inference_command = easyenv::get_env_string_optional("STABLE_DIFFUSION_INFERENCE_COMMAND");
    ...
    let maybe_virtual_env_activation_command = easyenv::get_env_string_optional("STABLE_DIFFUSION_INFERENCE_MAYBE_VENV_COMMAND");

    let maybe_default_config_path = easyenv::get_env_pathbuf_optional("STABLE_DIFFUSION_INFERENCE_MAYBE_DEFAULT_CONFIG_PATH");

    let maybe_execution_timeout: Option<Duration> = easyenv::get_env_duration_seconds_optional("STABLE_DIFFUSION_TIMEOUT_SECONDS");

    let maybe_docker_options = easyenv::get_env_string_optional("STABLE_DIFFUSION_INFERENCE_MAYBE_DOCKER_IMAGE").map(|image_name| DockerOptions { image_name, maybe_bind_mount: Some(DockerFilesystemMount::tmp_to_tmp()), maybe_environment_variables: None, maybe_gpu: Some(DockerGpu::All) });

    // Override for --checkpoint_dir at inference time
    let alternate_checkpoint_dir = easyenv::get_env_pathbuf_optional("STABLE_DIFFUSION_ALTERNATE_CHECKPOINT_PATH");

    Ok(Self { stable_diffusion_root_code_directory, executable_or_command, maybe_virtual_env_activation_command, maybe_default_config_path, maybe_docker_options, maybe_execution_timeout, alternate_checkpoint_dir })
  }
  ...
  fn do_execute_inference(&self, args: InferenceArgs) -> AnyhowResult<CommandExitStatus> {
