...
pub async fn process_job(args: RerenderProcessJobArgs<'_>) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let model_dependencies = args.job_dependencies.job.job_specific_dependencies.maybe_rerender_dependencies.as_ref().ok_or_else(|| ProcessSingleJobError::JobSystemMisconfiguration(Some("missing Rerender dependencies".to_string())))?;
  ...
  let video_path = download_video_file(&job_args.video_source, &args.job_dependencies.buckets.public_bucket_client, &mut job_progress_reporter, job, &args.job_dependencies.fs.scoped_temp_dir_creator_for_work, &work_temp_dir, &deps.db.mysql_pool).await?;
  ...
  let retrieved_sd_record = get_weight_by_token(sd_model_token, false, &deps.db.mysql_pool).await?;

  let model_path = download_model_file(&retrieved_sd_record, &args.job_dependencies.buckets.public_bucket_client, &mut job_progress_reporter, job, &args.job_dependencies.fs.scoped_temp_dir_creator_for_work, &work_temp_dir).await?;
  ...
  let lora_path = match rr_args.maybe_lora_model_token.clone() {
    Some(lora_model_token) => {
      let retrieved_lora_record = get_weight_by_token(&lora_model_token, false, &deps.db.mysql_pool).await?;

      let lora_path = download_model_file(&retrieved_lora_record, &args.job_dependencies.buckets.public_bucket_client, &mut job_progress_reporter, job, &args.job_dependencies.fs.scoped_temp_dir_creator_for_work, &work_temp_dir).await?;
  ...
  let config = RerenderConfig { input: Some(usable_video_path.to_str().unwrap().to_string()), output: Some(output_video_fs_path.to_str().unwrap().to_string()), work_dir: Some(work_temp_dir.path().to_str().unwrap().to_string()), sd_model: Some(sd_model.to_string()), lora_path: sd_lora, prompt: rr_args.maybe_prompt.clone(), a_prompt: rr_args.maybe_a_prompt.clone(), n_prompt: rr_args.maybe_n_prompt.clone(), seed: Some(seed as i64), ..RerenderConfig::new() };
  ...
  let command_exit_status = model_dependencies.inference_command.execute_inference(InferenceArgs { config_file: &config_path, stderr_output_file: &stderr_output_file });
...
impl RerenderConfig {
  pub fn new() -> Self {
    Self { input: None, output: None, work_dir: None, key_subdir: Some("keys".to_string()), sd_model: None, lora_path: None, interval: Some(8), prompt: None, a_prompt: None, n_prompt: None, x0_strength: Some(0.95), control_type: Some("canny".to_string()), canny_low: Some(50), canny_high: Some(100), control_strength: Some(0.7), seed: None, warp_period: Some(vec![0., 0.1]), ada_period: Some(vec![0.8, 1.]) }
  }

  pub fn validate(&self) -> Result<(), Error> {
    let fields = &[(self.input.is_some(), "input"), (self.output.is_some(), "output"), (self.work_dir.is_some(), "work_dir"), (self.key_subdir.is_some(), "key_subdir"), (self.sd_model.is_some(), "sd_model"), (self.interval.is_some(), "interval"), (self.prompt.is_some(), "prompt"), (self.a_prompt.is_some(), "a_prompt"), (self.n_prompt.is_some(), "n_prompt"), (self.x0_strength.is_some(), "x0_strength"), (self.control_type.is_some(), "control_type"), (self.canny_low.is_some(), "canny_low"), (self.canny_high.is_some(), "canny_high"), (self.control_strength.is_some(), "control_strength"), (self.seed.is_some(), "seed"), (self.warp_period.is_some(), "warp_period"), (self.ada_period.is_some(), "ada_period")];

    let missing_fields: Vec<&str> = fields.iter().filter_map(|(is_some, field_name)| if *is_some { None } else { Some(*field_name) }).collect();
