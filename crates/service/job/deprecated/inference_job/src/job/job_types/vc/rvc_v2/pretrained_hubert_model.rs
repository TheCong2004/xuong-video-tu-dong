...
impl PretrainedHubertModel {
  pub fn from_env() -> Self {
    ...
    let cloud_bucket_path = easyenv::get_env_string_or_default("RVC_V2_PRETRAINED_HUBERT_BUCKET_PATH", "/hubert_pretrained/rvc_v2_hubert_base.pt");
    ...
    Self { cloud_bucket_path, filesystem_path }
  }

  pub async fn download_if_not_on_filesystem(&self, bucket_client: &LegacyBucketClient, scoped_tempdir_creator: &ScopedTempDirCreator) -> Result<(), ProcessSingleJobError> {
    ...
    let temp_dir = scoped_tempdir_creator.new_tempdir("hubert_download").map_err(|e| anyhow!("problem creating tempdir: {:?}", e))?;

    let temp_path = temp_dir.path().join("download.part");

    info!("Downloading hubert from bucket path: {:?}", &self.cloud_bucket_path);

    bucket_client.download_file_to_disk(&self.cloud_bucket_path, &temp_path).await.map_err(|e| {
    ...
    rename_across_devices(&temp_path, &self.filesystem_path).map_err(|e| {
