...
pub async fn process_job_lora_upload(args: &StableDiffusionProcessArgs<'_>) -> Result<JobSuccessResult, ProcessSingleJobError> {
  let job = args.job;
  ...
  if file_exists(download_file_path.as_path()) == false {
    return Err(ProcessSingleJobError::from_anyhow_error(anyhow!("Failed to Download loRA Model from Google Drive")));
  }

  info!("Downloading predefined SD weight from: {:?} to {:?}", &sd_deps.predefined_sd_weight_bucket_path, &sd_checkpoint_path);

  args.job_dependencies.buckets.public_bucket_client.download_file_to_disk(&sd_deps.predefined_sd_weight_bucket_path, &sd_checkpoint_path).await.map_err(|err| {
  ...
  info!("Downloading predefined VAE weight from: {:?} to {:?}", &sd_deps.vae_bucket_path, &vae_path);
  ...
  if file_exists(file_path.path()) == false {
    return Err(ProcessSingleJobError::from_anyhow_error(anyhow!("Failed to Upload Not a LoRA")));
  }
  ...
  let model_weight_token_result = create_weight(CreateModelWeightsArgs { token: &model_weight_token, weights_type: WeightsType::LoRA, weights_category: WeightsCategory::ImageGeneration, title: sd_args.maybe_name.unwrap_or(String::from("")), maybe_cover_image_media_file_token: job.maybe_cover_image_media_file_token.clone(), maybe_description_markdown: sd_args.maybe_description, maybe_description_rendered_html: None, creator_user_token: Some(&creator_user_token), creator_ip_address, creator_set_visibility: Default::default(), maybe_last_update_user_token: None, original_download_url: Some(download_url), original_filename: None, file_size_bytes: metadata.file_size_bytes, file_checksum_sha2: metadata.sha256_checksum, public_bucket_hash: bucket_details.object_hash, maybe_public_bucket_prefix: Some(bucket_details.prefix), maybe_public_bucket_extension: Some(bucket_details.suffix), version: sd_args.maybe_version.unwrap_or(0), mysql_pool }).await?;
