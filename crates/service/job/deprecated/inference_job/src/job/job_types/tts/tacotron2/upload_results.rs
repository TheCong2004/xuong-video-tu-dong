...
async fn upload_as_media_file(args: UploadResultArgs<'_>) -> Result<ResultDetails, ProcessSingleJobError> {
  ...
  args.job_dependencies.buckets.public_bucket_client.upload_filename_with_content_type(&result_bucket_object_pathbuf, &args.output_audio_fs_path, "audio/wav").await.map_err(|e| ProcessSingleJobError::Other(e))?;

  let file_checksum = sha256_hash_file(&args.output_audio_fs_path).map_err(|err| ProcessSingleJobError::Other(anyhow!("Error hashing file: {:?}", err)))?;
  ...
  let (inference_result_token, _id) = insert_media_file_from_tacotron2(InsertMediaFileArgs { pool: &args.job_dependencies.db.mysql_pool, job: &args.job, maybe_mime_type: Some(&MIME_TYPE), file_size_bytes: args.file_metadata.file_size_bytes, maybe_duration_millis: args.maybe_audio_duration_millis, maybe_audio_codec_name: args.maybe_audio_codec_name, sha256_checksum: &file_checksum, public_bucket_directory_hash: result_bucket_location.get_object_hash(), maybe_public_bucket_prefix: Some(BUCKET_FILE_PREFIX), maybe_public_bucket_extension: Some(BUCKET_FILE_EXTENSION), is_on_prem: args.job_dependencies.job.info.container.is_on_prem, worker_hostname: &args.job_dependencies.job.info.container.hostname, worker_cluster: &args.job_dependencies.job.info.container.cluster_name }).await.map_err(|e| ProcessSingleJobError::Other(e))?;

  Ok(ResultDetails { entity_type: InferenceResultType::MediaFile, entity_token: inference_result_token.to_string() })
}
...
async fn upload_as_legacy_tts_result(args: UploadResultArgs<'_>) -> Result<ResultDetails, ProcessSingleJobError> {
  ...
  let audio_result_object_path = args.job_dependencies.buckets.bucket_path_unifier.tts_inference_wav_audio_output_path(&args.job.uuid_idempotency_token); // TODO: Don't use this!

  info!("Audio destination bucket path: {:?}", &audio_result_object_path);

  info!("Uploading audio...");

  args.job_dependencies.buckets.public_bucket_client.upload_filename_with_content_type(&audio_result_object_path, &args.output_audio_fs_path, "audio/wav").await.map_err(|e| ProcessSingleJobError::Other(e))?;

  safe_delete_file(&args.output_audio_fs_path);

  // ==================== UPLOAD SPECTROGRAM TO BUCKETS ==================== //

  let spectrogram_result_object_path = args.job_dependencies.buckets.bucket_path_unifier.tts_inference_spectrogram_output_path(&args.job.uuid_idempotency_token); // TODO: Don't use this!

  info!("Spectrogram destination bucket path: {:?}", &spectrogram_result_object_path);

  info!("Uploading spectrogram...");

  args.job_dependencies.buckets.public_bucket_client.upload_filename_with_content_type(&spectrogram_result_object_path, &args.output_spectrogram_fs_path, "application/json").await.map_err(|e| ProcessSingleJobError::Other(e))?;
  ...
  let text_hash = sha256_hash_string(&args.cleaned_inference_text).map_err(|e| ProcessSingleJobError::Other(e))?;

  info!("Saving tts inference record...");

  let (_id, inference_result_token) = insert_tts_result(&args.job_dependencies.db.mysql_pool, JobType::GenericInferenceJob(&args.job), &text_hash, Some(args.pretrained_vocoder), &audio_result_object_path, &spectrogram_result_object_path, args.file_metadata.file_size_bytes, args.file_metadata.duration_millis.unwrap_or(0), args.job_dependencies.job.info.container.is_on_prem, &args.job_dependencies.job.info.container.hostname, args.job_dependencies.job.system.is_debug_worker).await.map_err(|e| ProcessSingleJobError::Other(e))?;

  Ok(ResultDetails { entity_type: InferenceResultType::TextToSpeech, entity_token: inference_result_token })
}
