...
pub async fn dispatch_rvc_v2_job(job_dependencies: &JobDependencies, job: &AvailableInferenceJob) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  return match job.job_type {
    InferenceJobType::RvcV2 => {
      match job.maybe_download_url {
        ...
        None => {
          let job_success_result = process_rvc_job_old_entrypoint_adapter(job_dependencies, job).await?;
          Ok(job_success_result)
        },
      }
    },
    _ => Err(ProcessSingleJobError::Other(anyhow!("job type not set (new rvc code path)"))),
  };
}
