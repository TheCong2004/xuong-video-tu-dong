...
impl FbxToGltfDependencies {
  pub fn setup() -> AnyhowResult<Self> {
    ...
    let maybe_docker_options = easyenv::get_env_string_optional("FBX2GLTF_DOCKER_IMAGE").map(|image_name| DockerOptions { image_name, maybe_bind_mount: Some(DockerFilesystemMount::tmp_to_tmp()), maybe_environment_variables: None, maybe_gpu: None });
