...
#[derive(Clone)]
pub struct SeedVcInferenceCommand {
  /// Where the code lives
  pub(crate) seed_vc_code_directory: PathBuf,
...
pub struct InferenceArgs<'s> {
  ...
  pub input_audio_path: &'s Path,
  pub reference_audio_path: &'s Path, // SEED-VC calls this target
...
impl SeedVcInferenceCommand {
  ...
  pub fn from_env() -> anyhow::Result<Self> {
    let seed_vc_code_directory = easyenv::get_env_pathbuf_required("SEED_VC_CODE_DIRECTORY")?;

    let maybe_inference_command = easyenv::get_env_string_optional("SEED_VC_INFERENCE_COMMAND");

    let maybe_inference_executable = easyenv::get_env_pathbuf_optional("SEED_VC_INFERENCE_EXECUTABLE");
  ...
  pub fn do_execute_inference(&self, args: InferenceArgs) -> anyhow::Result<CommandExitStatus> {
    ...
    command.push_str(&format!(" --target {}", path_to_string(args.reference_audio_path)));

    command.push_str(&format!(" --output {}", path_to_string(args.output_audio_directory)));
