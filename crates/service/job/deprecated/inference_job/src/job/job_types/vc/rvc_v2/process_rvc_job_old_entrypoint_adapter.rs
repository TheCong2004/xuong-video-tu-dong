...
pub async fn process_rvc_job_old_entrypoint_adapter(job_dependencies: &JobDependencies, job: &AvailableInferenceJob) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let job_success_result = match model.get_model_type() {
    VcModelType::RvcV2 => rvc_v2::process_rvc_job::process_rvc_job(RvcV2ProcessJobArgs { job_dependencies, job, vc_model: &model, inference_media: &inference_media }).await?,
