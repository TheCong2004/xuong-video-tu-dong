...
impl AnimateXProcessFramesCommand {
  pub fn new_from_env() -> AnyhowResult<Self> {
    Ok(Self { root_code_directory: easyenv::get_env_pathbuf_or_default("ANIMATE_X_ROOT_CODE_DIRECTORY", PathBuf::from("/model_code")), executable_or_command: ExecutableOrCommand::Command("python process_data.py".to_string()), maybe_virtual_env_activation_command: easyenv::get_env_string_optional("ANIMATE_X_VENV_ACTIVATION_COMMAND"), maybe_docker_options: None, maybe_execution_timeout: None })
  }

  pub async fn execute_inference<'a, 'b>(&'a self, args: ProcessFramesArgs<'b>) -> AnyhowResult<CommandExitStatus> {
