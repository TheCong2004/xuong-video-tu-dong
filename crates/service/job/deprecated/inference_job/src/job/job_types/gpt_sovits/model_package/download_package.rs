...
pub async fn download_package(model_weight_token: ModelWeightToken, weights_file_bucket_directory: &WeightFileBucketDirectory, download_directory: &Path, bucket_client: &LegacyBucketClient, scoped_tempdir_creator: &ScopedTempDirCreator, force_download: bool) -> Result<(), ProcessSingleJobError> {
  if !download_directory.exists() {
    std::fs::create_dir_all(&download_directory).map_err(|err| ProcessSingleJobError::IoError(err))?;
  }

  let temp_dir_name = format!("model_download_{}", model_weight_token.entropy_suffix());

  let temp_dir = scoped_tempdir_creator.new_tempdir(&temp_dir_name).map_err(|e| anyhow!("problem creating tempdir: {:?}", e))?;

  for entry in GptSovitsPackageFileType::all_variants() {
    let result = do_download_package(model_weight_token.clone(), weights_file_bucket_directory, download_directory, bucket_client, &temp_dir, force_download, entry).await;
...
pub async fn do_download_package(model_weight_token: ModelWeightToken, weights_file_bucket_directory: &WeightFileBucketDirectory, download_directory: &Path, bucket_client: &LegacyBucketClient, temp_dir: &TempDir, force_download: bool, entry: GptSovitsPackageFileType) -> Result<(), ProcessSingleJobError> {
  ...
  if final_download_path.exists() {
    ...
    std::fs::remove_file(&final_download_path).map_err(|err| ProcessSingleJobError::IoError(err))?;
  }

  let bucket_public_download_path = WeightFileBucketPath::from_object_hash(weights_file_bucket_directory.get_object_hash(), Some("weight_"), Some(&suffix_with_package_identifier));

  debug!("Downloading to {} from bucket path: {:?}", temp_path.display().to_string(), &bucket_public_download_path.get_full_object_path_str());
  bucket_client.download_file_to_disk(bucket_public_download_path.get_full_object_path_str(), &temp_path).await.map_err(|e| anyhow!("couldn't download {} cloud object to disk: {:?}", &bucket_public_download_path.get_full_object_path_str(), e))?;

  debug!("Renaming file from {:?} to {:?}!", &temp_path, final_download_path.display().to_string());

  rename_across_devices(&temp_path, final_download_path).map_err(|err| {
    safe_delete_directory(&temp_dir);
    match err {
