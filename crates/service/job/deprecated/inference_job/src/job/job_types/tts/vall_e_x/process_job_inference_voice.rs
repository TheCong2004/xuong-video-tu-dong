...
pub async fn process_inference_voice(args: VALLEXProcessJobArgs<'_>, voice_token: String) -> Result<JobSuccessResult, ProcessSingleJobError> {
...
pub async fn download_voice_embedding(voice: &ZsVoice, private_bucket_client: &LegacyBucketClient, download_path: &PathBuf) -> Result<VoiceFile, ProcessSingleJobError> {
  let embedding_bucket_location = ZeroShotVoiceEmbeddingBucketPath::from_object_hash(&voice.bucket_hash, ModelCategory::Tts, ModelType::VallEx, voice.model_version);
