...
pub async fn download_video_file(video_source: &VideofilterVideoSource, public_bucket_client: &LegacyBucketClient, job_progress_reporter: &mut Box<dyn JobProgressReporter>, job: &AvailableInferenceJob, temp_dir_creator: &ScopedTempDirCreator, work_temp_dir: &TempDir, mysql_pool: &MySqlPool) -> Result<VideoFile, ProcessSingleJobError> {
  ...
  match video_source {
    VideofilterVideoSource::F(media_file_token) => {
      ...
      match media_file_result {
        Ok(Some(result)) => {
          let media_file_bucket_path = MediaFileBucketPath::from_object_hash(&result.public_bucket_directory_hash, result.maybe_public_bucket_prefix.as_deref(), result.maybe_public_bucket_extension.as_deref());
          bucket_object_path = media_file_bucket_path.to_full_object_pathbuf();
        },
        Ok(None) => return Err(ProcessSingleJobError::from_anyhow_error(anyhow!("could not find media file: {:?}", media_file_token))),
        Err(e) => {
    ...
    VideofilterVideoSource::U(media_upload_token) => {
      ...
      let media_upload_result = get_media_upload_for_inference(&media_upload_token, mysql_pool).await;
