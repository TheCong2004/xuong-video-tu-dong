...
pub async fn process_create_voice(args: StyleTTS2ProcessJobArgs<'_>, dataset_token: String) -> Result<JobSuccessResult, ProcessSingleJobError> {
  let deps = args.job_dependencies;
  let job = args.job;
  let mysql_pool = &deps.db.mysql_pool;

  let model_dependencies = deps.job.job_specific_dependencies.maybe_styletts2_dependencies.as_ref().ok_or_else(|| ProcessSingleJobError::JobSystemMisconfiguration(Some("missing StyleTTS2 dependencies".to_string())))?;
  ...
  for (index, record) in dataset.iter().enumerate() {
    ...
    info!("Record=> hash:{} prefix:{:?} extension:{:?}", record.public_bucket_directory_hash, prefix, extension);

    let audio_media_file = MediaFileBucketPath::from_object_hash(&record.public_bucket_directory_hash, prefix, extension);
    ...
    let result = deps.buckets.public_bucket_client.download_file_to_disk(audio_media_file.to_full_object_pathbuf(), &file_path).await;
  ...
  // Run Inference
  let command_exit_status = model_dependencies.create_embedding_command.execute_inference(job::job_types::tts::styletts2::styletts2_inference_command::CreateVoiceInferenceArgs { output_embedding_path: &output_embedding_path, audio_file: audio_files, stderr_output_file: &stderr_output_file });
  ...
  let embedding_bucket_location = ZeroShotVoiceEmbeddingBucketPath::generate_new(ModelCategory::Tts, ModelType::StyleTTS2, CURRENT_STYLE_TTS2_MODEL_VERSION);
  ...
  // insert record
  let voice_token = create_voice(CreateVoiceArgs { dataset_token: &voice_dataset_token, model_category: ZsVoiceModelCategory::Tts, model_type: ZsVoiceModelType::StyleTTS2, model_version: CURRENT_STYLE_TTS2_MODEL_VERSION, model_encoding_type: ZsVoiceEncodingType::Encodec, voice_title: &voice_name, bucket_hash: embedding_bucket_location.get_object_hash(), maybe_creator_user_token: Some(&creator_user_token), creator_ip_address: &creator_ip_address, creator_set_visibility: Visibility::Public, mysql_pool }).await.map_err(|e| ProcessSingleJobError::Other(e))?;

  Ok(JobSuccessResult { maybe_result_entity: Some(ResultEntity { entity_type: InferenceResultType::ZeroShotVoiceEmbedding, entity_token: voice_token.to_string() }), inference_duration })
}
...
pub async fn download_audio_from_hash(bucket_hash: &str, name: &str, private_bucket_client: &LegacyBucketClient, path: &PathBuf) -> Result<AudioFile, ProcessSingleJobError> {
  ...
  private_bucket_client.download_file_to_disk(object_path, &path).await.map_err(|err| {
    error!("Could not download audio file: {err}");
    ProcessSingleJobError::from_anyhow_error(anyhow!("Could not download audio file: {err}"))
  })?;

  let audio_file = AudioFile { filesystem_path: PathBuf::from(&path.clone()) };

  Ok(audio_file)
}

fn join_paths(paths: Vec<PathBuf>) -> String {
  paths.into_iter().map(|p| format!("\"{}\"", p.display())).collect::<Vec<String>>().join(" ")
}
...
mod tests {
  ...
  #[test]
  fn test_path_build() {
    let paths = vec![PathBuf::from("/home/tensor/code/TTSDockerContainer/Vall-E-mount/input/20.wav"), PathBuf::from("/home/tensor/code/TTSDockerContainer/Vall-E-mount/input/21.wav")];
