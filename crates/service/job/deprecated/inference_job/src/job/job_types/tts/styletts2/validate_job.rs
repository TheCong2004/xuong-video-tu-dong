...
pub fn validate_job(job: &AvailableInferenceJob) -> Result<JobArgs, ProcessSingleJobError> {
  ...
  if let Some(voice_token) = &ttsArgs.voice_token {
    return Ok::<JobArgs, ProcessSingleJobError>(JobArgs { voice_token: Some(voice_token.clone()), voice_dataset_token: None, job_type: JobType::Inference });
  }

  if let Some(dataset_token) = &ttsArgs.dataset_token {
    return Ok::<JobArgs, ProcessSingleJobError>(JobArgs { voice_token: None, voice_dataset_token: Some(dataset_token.clone()), job_type: JobType::Create });
  }
