...
pub async fn process_job(args: MocapNetProcessJobArgs<'_>) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let model_dependencies = args.job_dependencies.job.job_specific_dependencies.maybe_mocapnet_dependencies.as_ref().ok_or_else(|| ProcessSingleJobError::JobSystemMisconfiguration(Some("missing Mocap dependencies".to_string())))?;
  ...
  let output_bvh_path = model_dependencies.inference_command.mocapnet_root_code_directory.clone().join("out.bvh");
  ...
  let command_exit_status = model_dependencies.inference_command.execute_inference(InferenceArgs { video_file: &usable_video_path, maybe_ik1: &mc_args.maybe_ik1, maybe_ik2: &mc_args.maybe_ik2, maybe_ik3: &mc_args.maybe_ik3, maybe_smoothing1: &mc_args.maybe_smoothing1, maybe_smoothing2: &mc_args.maybe_smoothing2, maybe_size1: &maybe_width, maybe_size2: &maybe_height, stderr_output_file: &stderr_output_file });
