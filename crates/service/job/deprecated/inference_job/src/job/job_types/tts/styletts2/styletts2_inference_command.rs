...
impl StyleTTS2InferenceCommand {
  pub fn new<P: AsRef<Path>>(root_code_directory: P, executable_or_command: ExecutableOrCommand, maybe_virtual_env_activation_command: Option<&str>, maybe_default_config_path: Option<P>, maybe_docker_options: Option<DockerOptions>, maybe_execution_timeout: Option<Duration>, alternate_checkpoint_dir: Option<PathBuf>) -> Self {
    Self { root_code_directory: root_code_directory.as_ref().to_path_buf(), executable_or_command, maybe_virtual_env_activation_command: maybe_virtual_env_activation_command.map(|s| s.to_string()), maybe_default_config_path: maybe_default_config_path.map(|p| p.as_ref().to_path_buf()), maybe_docker_options, maybe_execution_timeout, alternate_checkpoint_dir }
  }

  pub fn from_env() -> AnyhowResult<Self> {
    let root_code_directory = easyenv::get_env_pathbuf_required("STYLE_TTS2_INFERENCE_ROOT_DIRECTORY")?;

    let maybe_inference_command = easyenv::get_env_string_optional("STYLE_TTS2_INFERENCE_COMMAND");

    // EVENTUALLY should remove the check
    // Optional, eg. `./infer.py`. Typically we'll use the command form instead.
    let maybe_inference_executable = easyenv::get_env_pathbuf_optional("STYLE_TTS2_INFERENCE_EXECUTABLE");
    ...
    let maybe_virtual_env_activation_command = easyenv::get_env_string_optional("STYLE_TTS2_INFERENCE_MAYBE_VENV_COMMAND");

    let maybe_default_config_path = easyenv::get_env_pathbuf_optional("STYLE_TTS2_INFERENCE_MAYBE_DEFAULT_CONFIG_PATH");

    let maybe_execution_timeout = easyenv::get_env_duration_seconds_optional("TIMEOUT_SECONDS");

    // Probably for local
    let maybe_docker_options = easyenv::get_env_string_optional("STYLE_TTS2_INFERENCE_MAYBE_DOCKER_IMAGE").map(|image_name| DockerOptions { image_name, maybe_bind_mount: Some(DockerFilesystemMount::tmp_to_tmp()), maybe_environment_variables: None, maybe_gpu: Some(DockerGpu::All) });

    // Override for --checkpoint_dir at inference time
    let alternate_checkpoint_dir = easyenv::get_env_pathbuf_optional("STYLE_TTS2_ALTERNATE_CHECKPOINT_PATH");

    Ok(Self { root_code_directory, executable_or_command, maybe_virtual_env_activation_command, maybe_default_config_path, maybe_docker_options, maybe_execution_timeout, alternate_checkpoint_dir })
  }
...
impl StyleTTS2CreateEmbeddingCommand {
  ...
  pub fn from_env() -> AnyhowResult<Self> {
    ...
    // Override for --checkpoint_dir at inference time
    let alternate_checkpoint_dir = easyenv::get_env_pathbuf_optional("STYLE_TTS2_CHECKPOINT_PATH");
  ...
  fn do_execute_inference<P: AsRef<Path>>(&self, args: CreateVoiceInferenceArgs<P>) -> AnyhowResult<CommandExitStatus> {
    let mut command = String::new();
    command.push_str(&format!("cd {}", path_to_string(&self.root_code_directory)));
