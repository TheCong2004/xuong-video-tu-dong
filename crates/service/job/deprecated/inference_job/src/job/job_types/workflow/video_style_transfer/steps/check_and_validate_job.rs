...
pub fn check_and_validate_job(job: &AvailableInferenceJob) -> Result<JobArgs, ProcessSingleJobError> {
  ...
  let output_path = match inference_args.maybe_output_path.as_deref() {
    ...
    None => "vid2vid/SparseUpscaleInterp_00001.mp4",
  };

  Ok(JobArgs { workflow_source: &inference_args.maybe_workflow_config, output_path, maybe_lora_model: &inference_args.maybe_lora_model, maybe_json_modifications: &inference_args.maybe_json_modifications, maybe_input_file: &inference_args.maybe_input_file, maybe_depth_input_file: inference_args.maybe_input_depth_file.as_ref(), maybe_normal_input_file: inference_args.maybe_input_normal_file.as_ref(), maybe_outline_input_file: inference_args.maybe_input_outline_file.as_ref() })
}
