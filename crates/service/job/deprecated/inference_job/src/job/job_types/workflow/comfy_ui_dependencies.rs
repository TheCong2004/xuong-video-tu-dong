...
impl ComfyDependencies {
  pub async fn setup() -> AnyhowResult<Self> {
    ...
    let output = Command::new("python3").arg(&comfy_setup_script).output().await.expect("failed to run comfyui setup script");
    println!("stdout: {}", String::from_utf8_lossy(&output.stdout));

    // Start the comfy start script as a daemon process
    let launch_command = format!("cd {} && {}", inference_command.clone().comfy_root_code_directory.display(), inference_command.clone().comfy_launch_command.display());
    ...
    Ok(Self { inference_command, ffmpeg_watermark_command: FfmpegLogoWatermarkCommand::from_env()?, ffmpeg_command_runner: FfmpegCommandRunner::from_env()?, configs: ComfyConfigs { main_workflow: get_env_string_optional("MAIN_IPA_WORKFLOW"), face_detailer_workflow: get_env_string_optional("FACE_DETAILER_WORKFLOW"), upscaler_workflow: get_env_string_optional("UPSCALER_WORKFLOW") }, watermarks: WatermarkConfigs::from_env()? })
  }
}
