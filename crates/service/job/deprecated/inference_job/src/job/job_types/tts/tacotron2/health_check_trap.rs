...
pub async fn maybe_block_on_sidecar_health_check(tts_sidecar_health_check_client: &Tacotron2SidecarHealthCheckClient) {
  let mut error_timeout_millis = HEALTH_CHECK_START_TIMEOUT_MILLIS;

  loop {
    info!("Performing sidecar health check...");
    let maybe_health = tts_sidecar_health_check_client.request_health_check().await;
