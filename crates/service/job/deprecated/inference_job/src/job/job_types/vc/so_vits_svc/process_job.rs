...
pub async fn process_job(args: SoVitsSvcProcessJobArgs<'_>) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let model_dependencies = args.job_dependencies.job.job_specific_dependencies.maybe_svc_dependencies.as_ref().ok_or_else(|| ProcessSingleJobError::JobSystemMisconfiguration(Some("missing SVC dependencies".to_string())))?;
  ...
  let so_vits_svc_fs_path = {
    ...
    maybe_download_file_from_bucket(MaybeDownloadArgs { name_or_description_of_file: "so-vits-svc model", final_filesystem_file_path: &so_vits_svc_fs_path, bucket_object_path: &so_vits_svc_model_object_path, bucket_client, job_progress_reporter: &mut job_progress_reporter, job_progress_update_description: "downloading so-vits-svc model", job_id: job.id.0, scoped_tempdir_creator: &args.job_dependencies.fs.scoped_temp_dir_creator_for_short_lived_downloads, maybe_existing_file_minimum_size_required: None }).await?;
  ...
  let command_exit_status = model_dependencies.inference_command.execute_inference(InferenceArgs { model_path: &so_vits_svc_fs_path, input_path: &input_wav_path, output_path: &output_audio_fs_path, stderr_output_file: &stderr_output_file, maybe_config_path: None, device: Device::Cuda, auto_predict_f0, maybe_transpose });
