...
pub async fn process_inference_voice(args: StyleTTS2ProcessJobArgs<'_>, voice_token: String) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  let text = match job.maybe_raw_inference_text.clone() {
    Some(value) => clean_symbols(value.as_str()),
    None => {
  ...
  let voice_file = download_voice_embedding(&voice, &deps.buckets.private_bucket_client, &downloaded_weights_path).await?;
  ...
  std::fs::write(&text_input_fs_path, &text).map_err(|e| ProcessSingleJobError::from_io_error(e))?;
  ...
  // Run Inference
  let command_exit_status = model_dependencies.inference_command.execute_inference(InferenceArgs {
  ...
  args.job_dependencies.buckets.public_bucket_client.upload_filename_with_content_type(&result_bucket_object_pathbuf, &finished_file, &MIME_TYPE).await.map_err(|e| ProcessSingleJobError::Other(e))?;
...
pub async fn download_voice_embedding_from_hash(bucket_hash: &str, name: &str, private_bucket_client: &LegacyBucketClient, path: &PathBuf) -> Result<VoiceFile, ProcessSingleJobError> {
  ...
  let voice_file = VoiceFile { filesystem_path: PathBuf::from(&path.clone()) };

  Ok(voice_file)
}

pub async fn download_voice_embedding(voice: &ZsVoice, private_bucket_client: &LegacyBucketClient, download_path: &PathBuf) -> Result<VoiceFile, ProcessSingleJobError> {
  let embedding_bucket_location = ZeroShotVoiceEmbeddingBucketPath::from_object_hash(&voice.bucket_hash, ModelCategory::Tts, ModelType::StyleTTS2, voice.model_version);
  ...
  let result = private_bucket_client.download_file_to_disk(embedding_bucket_location.to_full_object_pathbuf(), &download_path).await;
  ...
  let voice_file = VoiceFile { filesystem_path: PathBuf::from(&download_path.clone()) };
