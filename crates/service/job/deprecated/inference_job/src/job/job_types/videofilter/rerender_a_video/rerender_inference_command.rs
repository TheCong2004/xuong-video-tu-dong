...
impl RerenderInferenceCommand {
  pub fn from_env() -> AnyhowResult<Self> {
    let rerender_root_code_directory = easyenv::get_env_pathbuf_required("RERENDER_INFERENCE_ROOT_DIRECTORY")?;

    let config_path = easyenv::get_env_pathbuf_optional("RERENDER_INFERENCE_CONFIG_PATH");

    let executable_or_command = match easyenv::get_env_string_optional("RERENDER_INFERENCE_EXECUTABLE_OR_COMMAND") {
      None => {
    ...
    let maybe_virtual_env_activation_command = easyenv::get_env_string_optional("RERENDER_INFERENCE_MAYBE_VENV_COMMAND");

    let maybe_execution_timeout = easyenv::get_env_duration_seconds_optional("RERENDER_TIMEOUT_SECONDS");

    let maybe_docker_options = easyenv::get_env_string_optional("RERENDER_INFERENCE_MAYBE_DOCKER_IMAGE").map(|image_name| DockerOptions { image_name, maybe_bind_mount: Some(DockerFilesystemMount::tmp_to_tmp()), maybe_environment_variables: None, maybe_gpu: Some(DockerGpu::All) });

    Ok(Self { rerender_root_code_directory, executable_or_command, config_path, maybe_virtual_env_activation_command, maybe_docker_options, maybe_execution_timeout })
  }
