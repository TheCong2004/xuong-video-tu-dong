...
pub fn live_portrait_title(maybe_existing_portrait_title: Option<&str>, maybe_existing_driver_title: Option<&str>) -> String {
  const FIELD_LENGTH: usize = 255;
  const TITLE_PREFIX: &str = "Live Portrait of ";

  let maybe_title_basis = maybe_existing_portrait_title.or(maybe_existing_driver_title).map(|title| trim_or_empty(title)).flatten();
