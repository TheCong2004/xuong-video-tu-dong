...
impl ComfyInferenceCommand {
  pub fn from_env() -> AnyhowResult<Self> {
    let comfy_root_code_directory = easyenv::get_env_pathbuf_required("COMFY_INFERENCE_ROOT_DIRECTORY")?;

    let config_path = easyenv::get_env_pathbuf_optional("COMFY_INFERENCE_CONFIG_PATH");

    let executable_or_command = match easyenv::get_env_string_optional("COMFY_INFERENCE_EXECUTABLE_OR_COMMAND") {
      None => {
    ...
    let comfy_setup_script = easyenv::get_env_pathbuf_required("COMFY_SETUP_SCRIPT")?;

    let comfy_launch_command = easyenv::get_env_pathbuf_required("COMFY_LAUNCH_COMMAND")?;

    let comfy_startup_healthcheck_enabled = easyenv::get_env_bool_or_default("COMFY_STARTUP_HEALTHCHECK_ENABLED", true);

    let maybe_virtual_env_activation_command = easyenv::get_env_string_optional("COMFY_INFERENCE_MAYBE_VENV_COMMAND");

    let maybe_execution_timeout = easyenv::get_env_duration_seconds_optional("COMFY_TIMEOUT_SECONDS");

    let maybe_docker_options = easyenv::get_env_string_optional("COMFY_INFERENCE_MAYBE_DOCKER_IMAGE").map(|image_name| DockerOptions { image_name, maybe_bind_mount: Some(DockerFilesystemMount::tmp_to_tmp()), maybe_environment_variables: None, maybe_gpu: Some(DockerGpu::All) });

    let mounts_directory = easyenv::get_env_pathbuf_required("COMFY_MOUNTS_DIRECTORY")?;

    let processing_script = easyenv::get_env_pathbuf_required("COMFY_VIDEO_PROCESSING_SCRIPT")?;

    let styles_directory = easyenv::get_env_pathbuf_required("COMFY_STYLES_DIRECTORY")?;

    let workflows_directory = easyenv::get_env_pathbuf_required("COMFY_WORKFLOWS_DIRECTORY")?;

    let mappings_directory = easyenv::get_env_pathbuf_required("COMFY_MAPPINGS_DIRECTORY")?;

    Ok(Self { comfy_root_code_directory, executable_or_command, config_path, mounts_directory, processing_script, comfy_setup_script, comfy_launch_command, comfy_startup_healthcheck_enabled, maybe_virtual_env_activation_command, maybe_docker_options, maybe_execution_timeout, styles_directory, workflows_directory, mappings_directory })
  }

  pub async fn execute_inference<'a, 'b>(&'a self, cancellation_receiver: &mut tokio::sync::oneshot::Receiver<()>, args: InferenceArgs<'b>) -> CommandExitStatus {
  ...
  async fn do_execute_inference<'a, 'b>(&'a self, cancellation_receiver: &mut tokio::sync::oneshot::Receiver<()>, args: InferenceArgs<'b>) -> AnyhowResult<CommandExitStatus> {
    ...
    match args.inference_details {
      ...
      InferenceDetails::NewPythonArgs { maybe_positive_prompt_filename, maybe_negative_prompt_filename, maybe_travel_prompt_filename, maybe_style } => {
  ...
  pub async fn execute_live_portrait_inference<'a, 'b>(&'a self, args: LivePortraitCommandArgs<'b>) -> CommandExitStatus {
  ...
  async fn do_execute_live_portrait_inference<'a, 'b>(&'a self, args: LivePortraitCommandArgs<'b>) -> AnyhowResult<CommandExitStatus> {
    info!("InferenceArgs: {:?}", &args);
  ...
  // TODO(bt,2024-07-16): This belongs in another module / runner, and all of these need to be consolidated
  pub async fn execute_face_fusion_inference<'a, 'b>(&'a self, args: FaceFusionCommandArgs<'b>) -> CommandExitStatus {
  ...
  async fn do_execute_face_fusion_inference<'a, 'b>(&'a self, args: FaceFusionCommandArgs<'b>) -> AnyhowResult<CommandExitStatus> {
    info!("InferenceArgs: {:?}", &args);
