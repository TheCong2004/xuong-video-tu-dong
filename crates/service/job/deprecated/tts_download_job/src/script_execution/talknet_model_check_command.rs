...
impl TalknetModelCheckCommand {
  pub fn new(talknet_directory: &str, check_model_script_name: &str) -> Self {
    Self { talknet_directory: talknet_directory.to_string(), check_model_script_name: check_model_script_name.to_string() }
  }
  ...
  pub fn execute<P: AsRef<Path>>(&self, synthesizer_checkpoint_path: P, output_metadata_filename: P, spawn_process: bool) -> AnyhowResult<()> {
    ...
    if spawn_process {
      ...
      let stdout_file = OpenOptions::new().read(true).write(true).create(true).truncate(true).open("/tmp/talknet_upload_stdout.txt")?;

      let stderr_file = OpenOptions::new().read(true).write(true).create(true).truncate(true).open("/tmp/talknet_upload_stderr.txt")?;
