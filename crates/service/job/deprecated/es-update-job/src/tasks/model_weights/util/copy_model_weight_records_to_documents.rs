...
pub async fn copy_model_weight_records_to_documents(mut tokens: Vec<ModelWeightToken>, mysql_connection: &mut MySqlConnection, elasticsearch: &Elasticsearch, sleep_config: &SleepConfigs) -> AnyhowResult<Option<Cursor>> {
  ...
  while !tokens.is_empty() {
    ...
    let drained_tokens = tokens.drain(0..last).into_iter().collect::<Vec<_>>();

    let records = batch_get_model_weights_for_elastic_search_backfill(&mut *mysql_connection, &drained_tokens).await?;

    for record in records {
      let new_cursor = Cursor { token: record.token.clone(), updated_at: record.updated_at.clone() };

      create_model_weight_document_from_record(elasticsearch, record).await?;

      let last_timestamp = last_updated_record.as_ref().map(|cursor: &Cursor| cursor.updated_at).unwrap_or_else(|| DateTime::UNIX_EPOCH);
