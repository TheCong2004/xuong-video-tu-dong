...
async fn calculate_with_connection(connection: &mut PoolConnection<MySql>, job_state: &JobState) -> AnyhowResult<()> {
  let backfill_tokens = list_all_model_weight_tokens_for_cached_usage_backfill(&mut **connection).await?;
  ...
  for backfill_token in backfill_tokens {
    let usage_count = sum_model_weight_usage_count_for_models(&backfill_token.token, &mut **connection).await?;

    let usage_count = usage_count.map(|usage| usage.total_usage_count);
    ...
    update_model_weight_cached_usage_count(Args { model_weight_token: &backfill_token.token, usage_count, mysql_executor: &mut **connection, phantom: Default::default() }).await?;
