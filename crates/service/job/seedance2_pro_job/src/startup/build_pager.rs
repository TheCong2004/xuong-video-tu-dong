...
pub fn build_pager(server_environment: server_environment::ServerEnvironment, hostname: &str) -> (Pager, PagerWorker) {
  ...
  let builder = PagerBuilder::new().application_name("seedance2-pro-job".to_string()).environment(environment.to_string()).hostname(hostname.to_string()).service_id(ROOTLY_SERVICE_ID_SEEDANCE2_PRO_JOB.to_string());
