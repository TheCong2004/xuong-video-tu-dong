...
async fn main() -> AnyhowResult<()> {
  let container_environment = bootstrap(BootstrapArgs { app_name: "seedance2-pro-job", default_logging_override: Some(DEFAULT_RUST_LOG), config_search_directories: &[".", "./config", "crates/service/job/seedance2_pro_job/config"], ignore_legacy_dot_env_file: true })?;
  ...
  let poll_interval_millis: u64 = easyenv::get_env_num("SEEDANCE_POLL_INTERVAL_MILLIS", 5_000)?;

  let maybe_max_job_age = easyenv::try_get_env_num_optional::<i64>(ENV_MAX_JOB_AGE_THRESHOLD_HOURS)?.and_then(chrono::Duration::try_hours);

  if let Some(ref duration) = maybe_max_job_age {
    info!("Max job age threshold: {} hours", duration.num_hours());
  }

  let poll_max_retries: u32 = easyenv::get_env_num("POLL_MAX_RETRIES", 3)?;

  let poll_retry_max_delay_millis: u64 = easyenv::get_env_num("POLL_RETRY_MAX_DELAY_MILLIS", 10_000)?;

  info!("Poll max retries: {}, max retry delay: {}ms", poll_max_retries, poll_retry_max_delay_millis);

  let credits_alert_threshold: u64 = easyenv::get_env_num("CREDITS_ALERT_THRESHOLD", 10_000)?;
  ...
  let job_dependencies = JobDependencies { mysql_pool, public_bucket_client, seedance2pro_session: kinovi_session, kinovi_version, server_environment, job_stats, poll_interval_millis, maybe_max_job_age, poll_max_retries, poll_retry_max_delay_millis, credits_alert_threshold, application_shutdown: application_shutdown.clone(), shutdown_notify: shutdown_notify.clone(), pager, order_reconciler };
