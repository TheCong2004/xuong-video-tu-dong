...
pub async fn process_successful_video_job(deps: &JobDependencies, job: &PendingSeedance2ProJob, order: &OrderStatus) -> AnyhowResult<()> {
  // Get the video URL.
  let video_url = match &order.result_url {
    ...
    None => {
      ...
      match order.results.first() {
        ...
        None => {
          return Err(anyhow!("Completed order {} has no result_url and no results", order.order_id));
        },
      }
    },
  };

  info!("Downloading video for order {} from: {}", order.order_id, video_url);
  ...
  let video_bytes: Vec<u8> = match reqwest::get(video_url).await {
    Ok(resp) => match resp.bytes().await {
      ...
      Err(err) => {
        ...
        return alert_pager_and_return_err(&deps.pager, "Kinovi video download failed", anyhow!("error reading video bytes: {:?}", err), Some(job));
      },
    },
    Err(err) => {
      error!("Error downloading video for order {}: {:?}", order.order_id, err);
      return alert_pager_and_return_err(&deps.pager, "Kinovi video download failed", anyhow!("reqwest error downloading video: {:?}", err), Some(job));
    },
  };

  info!("Downloaded {} bytes for order {}", video_bytes.len(), order.order_id);
  ...
  if let Err(err) = upload_result {
    ...
    return alert_pager_and_return_err(&deps.pager, "Kinovi bucket upload failed", anyhow!("error uploading video to bucket: {:?}", err), Some(job));
  }

  info!("Uploaded video for order {}. Creating media file record.", order.order_id);
  ...
  let media_file_result = MediaFileInsertBuilder::new().checksum_sha2(&checksum).creator_ip_address(&job.creator_ip_address).creator_set_visibility(job.creator_set_visibility).file_size_bytes(video_bytes.len() as u64).maybe_creator_anonymous_visitor(job.maybe_creator_anonymous_visitor_token.as_ref()).maybe_creator_user(job.maybe_creator_user_token.as_ref()).maybe_frame_height(maybe_frame_height).maybe_frame_width(maybe_frame_width).maybe_generation_provider(Some(GenerationProvider::Artcraft)).maybe_prompt_token(job.maybe_prompt_token.as_ref()).maybe_platform_type(job.maybe_platform_type).media_file_class(MediaFileClass::Video).media_file_origin_category(MediaFileOriginCategory::Inference).media_file_origin_product_category(MediaFileOriginProductCategory::VideoGeneration).media_file_type(MediaFileType::Mp4).mime_type("video/mp4").public_bucket_directory_hash(&bucket_path).insert_pool(&deps.mysql_pool).await;

  let media_file_token = match media_file_result {
    Ok(token) => token,
    Err(err) => {
      error!("Error inserting media file record for order {}: {:?}", order.order_id, err);
      return alert_pager_and_return_err(&deps.pager, "Kinovi media file insert failed", anyhow!("error inserting media file record: {:?}", err), Some(job));
    },
  };

  info!("Created media file {} for order {}. Finalizing job {}.", media_file_token.as_str(), order.order_id, job.job_token.as_str());
...
async fn finalize_success(deps: &JobDependencies, job: &PendingSeedance2ProJob, order: &OrderStatus, media_file_token: &str) -> AnyhowResult<()> {
