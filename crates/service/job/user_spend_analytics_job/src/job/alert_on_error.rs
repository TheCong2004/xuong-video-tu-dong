...
pub fn alert_pager(pager: &Pager, title: &str, detail: &str) {
  let notification = NotificationDetailsBuilder::from_title(title.to_string()).set_description(Some(detail.to_string())).set_urgency(Some(NotificationUrgency::Medium)).build();
