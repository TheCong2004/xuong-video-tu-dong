...
use rootly_config::services::ROOTLY_SERVICE_ID_USER_SPEND_ANALYTICS_JOB;
...
pub fn build_pager(server_environment: server_environment::ServerEnvironment, hostname: &str) -> (Pager, PagerWorker) {
  let is_paging_enabled = env_enable_paging_default_false();

  info!("Paging enabled: {}", is_paging_enabled);

  let environment = if server_environment.is_deployed_in_production() { "production" } else { "development" };

  let builder = PagerBuilder::new().application_name("user-spend-analytics-job".to_string()).environment(environment.to_string()).hostname(hostname.to_string()).service_id(ROOTLY_SERVICE_ID_USER_SPEND_ANALYTICS_JOB.to_string());
...
fn build_rootly_pager(builder: PagerBuilder, api_key: String) -> (Pager, PagerWorker) {
  let mut rootly_builder = builder.rootly(RootlyApiKey::new(api_key)).urgency_id_high(ROOTLY_URGENCY_ID_HIGH.to_string()).urgency_id_medium(ROOTLY_URGENCY_ID_MEDIUM.to_string()).urgency_id_low(ROOTLY_URGENCY_ID_LOW.to_string());
