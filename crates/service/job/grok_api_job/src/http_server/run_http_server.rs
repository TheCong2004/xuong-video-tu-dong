...
pub fn run_http_server(args: CreateServerArgs) -> AnyhowResult<Server> {
  ...
  let _ = args.container_environment;
  ...
  let handle = HttpServer::new(move || App::new().app_data(server_state_arc.clone()).wrap(Logger::new(&log_format).exclude("/_status")).service(web::resource("/").route(web::get().to(|| HttpResponse::Ok())).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/_status").route(web::get().to(get_health_check_handler)).route(web::head().to(|| HttpResponse::Ok())))).bind(bind_address)?.workers(num_workers).run();
