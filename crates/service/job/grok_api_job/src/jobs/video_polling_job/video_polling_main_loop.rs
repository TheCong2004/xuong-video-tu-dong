...
use grok_api_client::api::requests::videos::video_status::video_status::{video_status, VideoStatus, VideoStatusArgs, VideoStatusRequest};
use mysql_queries::queries::generic_inference::api_providers::grok_api::list_pending_grok_api_jobs::{list_pending_grok_api_jobs, PendingGrokApiJob};

use crate::job_dependencies::JobDependencies;
use crate::jobs::video_polling_job::process_job::process_failed_job::{process_failed_status, process_video_status_error};
...
async fn poll_one_job(deps: &JobDependencies, job: &PendingGrokApiJob) {
  let poll_result = video_status(VideoStatusArgs { api_key: &deps.grok_api_key, request: VideoStatusRequest { request_id: job.request_id.clone() } }).await;
  ...
  match response.status {
    VideoStatus::Pending { progress } => {
      ...
      info!("Grok request {} for job {} still pending ({} progress).", job.request_id, job.job_token.as_str(), progress_str,);
    },
