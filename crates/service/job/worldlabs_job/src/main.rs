...
async fn main() -> AnyhowResult<()> {
  let container_environment = bootstrap(BootstrapArgs { app_name: "worldlabs-job", default_logging_override: Some(DEFAULT_RUST_LOG), config_search_directories: &[".", "./config", "crates/service/job/worldlabs_job/config"], ignore_legacy_dot_env_file: true })?;
  ...
  let poll_interval_millis: u64 = easyenv::get_env_num("WORLDLABS_POLL_INTERVAL_MILLIS", 15_000)?;
  ...
  let job_dependencies = JobDependencies { mysql_pool, public_bucket_client, worldlabs_creds, server_environment, job_stats, poll_interval_millis, application_shutdown: application_shutdown.clone() };
