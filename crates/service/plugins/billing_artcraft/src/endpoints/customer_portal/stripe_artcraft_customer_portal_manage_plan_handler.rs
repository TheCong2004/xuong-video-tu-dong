...
pub async fn stripe_artcraft_customer_portal_manage_plan_handler(http_request: HttpRequest, request: Json<StripeArtcraftCustomerPortalManagePlanRequest>, stripe_config: Data<ArtcraftStripeConfigWithClient>, server_environment: Data<ServerEnvironment>, internal_user_lookup: Data<dyn InternalUserLookup>, mysql_pool: Data<MySqlPool>) -> Result<Json<StripeArtcraftCustomerPortalManagePlanResponse>, CommonWebError> {
  ...
  let mut portal_builder = CreateBillingPortalSession::new(subscription.stripe_customer_id.clone()).return_url(stripe_config.portal_return_url.clone()); // TODO: This can be a different URL.
  ...
  Ok(Json(StripeArtcraftCustomerPortalManagePlanResponse { success: true, stripe_portal_url: portal_session.url }))
}
