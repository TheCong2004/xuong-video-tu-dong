...
pub fn ignore_known_unwanted_events(webhook_payload: &Event) -> Option<WebhookEventLogSummary> {
  match webhook_payload.data.object {
    ...
    EventObject::CustomerCreated(_) | EventObject::CustomerDeleted(_) | EventObject::CustomerUpdated(_) => {
  ...
  Some(WebhookEventLogSummary { maybe_user_token: None, maybe_event_entity_id: None, maybe_stripe_customer_id: None, action_was_taken: false, should_ignore_retry: true })
}
