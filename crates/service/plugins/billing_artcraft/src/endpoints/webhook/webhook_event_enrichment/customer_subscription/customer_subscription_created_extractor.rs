...
pub async fn customer_subscription_created_extractor(subscription: &Subscription, server_environment: ServerEnvironment) -> Result<EnrichedWebhookEvent, StripeArtcraftWebhookError> {
  let summary = extract_common_subscription_details(subscription, server_environment)?;

  Ok(EnrichedWebhookEvent { maybe_billing_action: Some(ArtcraftBillingAction::SubscriptionCreated(summary.subscription_details)), webhook_event_log_summary: summary.event_log_summary })
}
