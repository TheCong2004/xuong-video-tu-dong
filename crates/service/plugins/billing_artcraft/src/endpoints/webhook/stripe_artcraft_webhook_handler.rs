...
//   params()
// )]
pub async fn stripe_artcraft_webhook_handler(http_request: HttpRequest, server_environment: Data<ServerEnvironment>, stripe_config: Data<ArtcraftStripeConfigWithClient>, request_body_bytes: Bytes, mysql_pool: Data<MySqlPool>) -> Result<Json<StripeArtcraftWebhookSuccessResponse>, StripeArtcraftWebhookError> {
  verify_stripe_webhook_ip_address(&http_request, **server_environment).map_err(|e| {
  ...
  let stripe_signature = get_request_header_optional(&http_request, "Stripe-Signature").unwrap_or_default();

  // NB: Treat the request payload as unstructured and defer to Stripe libraries.
  let webhook_payload = String::from_utf8(request_body_bytes.to_vec()).map_err(|err| {
    let reason = format!("Could not decode request body to UTF-8: {:?}", err);
    error!("{}", &reason);
    StripeArtcraftWebhookError::BadRequest(reason)
  })?;

  let webhook_payload = Webhook::construct_event(&webhook_payload, &stripe_signature, secret_signing_key).map_err(|e| {
  ...
  let stripe_event_descriptor = StripeEventDescriptor { stripe_event_id: stripe_event_id.clone(), stripe_event_type: stripe_event_type.clone() };

  info!("Stripe webhook event type: {} ; is production: {}, created at: {}, pending events to be handled: {}", &stripe_event_descriptor, stripe_is_production, &stripe_event_created_at, webhook_payload.pending_webhooks);
  ...
  let maybe_previously_played_event = get_stripe_webhook_event_log_by_id_with_connection(&stripe_event_id, &mut mysql_connection).await.map_err(|err| {
  ...
  if let Some(event) = maybe_previously_played_event {
    ...
    if event.should_ignore_retry {
      warn!("Stripe is replaying event ID={} ; ignoring it since we have already previously handled it.", &stripe_event_descriptor);

      return Ok(Json(StripeArtcraftWebhookSuccessResponse { success: true }));
    }
  }

  let enriched_event = handle_webhook_event_enrichment(&stripe_event_descriptor, &stripe_config.client, **server_environment, webhook_payload).await?;

  let mut transaction = mysql_connection.begin().await?;

  let result = process_enriched_event(**server_environment, &stripe_config, &enriched_event, stripe_event_descriptor.clone(), stripe_event_id.clone(), stripe_event_created_at, stripe_event_type.clone(), stripe_is_production, &mut transaction).await;
  ...
  match result {
    ...
    Err(err) => {
      error!("Error handling Stripe webhook event {} : {:?}", stripe_event_descriptor, err);
  ...
  Ok(Json(StripeArtcraftWebhookSuccessResponse { success: true }))
}

async fn process_enriched_event(server_environment: ServerEnvironment, stripe_config: &ArtcraftStripeConfigWithClient, artcraft_event: &EnrichedWebhookEvent, stripe_event_descriptor: StripeEventDescriptor, stripe_event_id: String, stripe_event_created_at: NaiveDateTime, stripe_event_type: String, stripe_is_production: bool, transaction: &mut Transaction<'_, MySql>) -> Result<(), StripeArtcraftWebhookError> {
  if let Some(billing_action) = &artcraft_event.maybe_billing_action {
    ...
    transactionally_fulfill_artcraft_billing_action(billing_action, transaction).await?;
  } else {
  ...
  let query = InsertStripeWebhookEventLog { stripe_event_id, stripe_event_type, maybe_stripe_event_entity_id: artcraft_event.webhook_event_log_summary.maybe_event_entity_id.clone(), maybe_stripe_customer_id: artcraft_event.webhook_event_log_summary.maybe_stripe_customer_id.clone(), stripe_event_created_at, stripe_is_production, maybe_user_token: artcraft_event.webhook_event_log_summary.maybe_user_token.as_ref().map(|t| t.to_string()), action_was_taken: artcraft_event.webhook_event_log_summary.action_was_taken, should_ignore_retry: artcraft_event.webhook_event_log_summary.should_ignore_retry };

  query.insert_transactional(transaction).await.map_err(|err| {
...
async fn maybe_update_user_email(artcraft_event: &EnrichedWebhookEvent, mysql_connection: &mut PoolConnection<MySql>) -> Result<(), StripeArtcraftWebhookError> {
  ...
  maybe_update_email_from_synthetic_value(MaybeUpdateEmailFromSyntheticValueArgs { user_token: &user_token, email_address: &user_email, email_gravatar_hash: &gravatar_hash, mysql_connection }).await?;
