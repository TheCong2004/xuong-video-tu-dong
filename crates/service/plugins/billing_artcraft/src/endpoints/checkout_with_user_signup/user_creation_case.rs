...
pub(super) async fn user_creation_case(http_request: &HttpRequest, price_id: &PriceId, mysql_connection: &mut PoolConnection<MySql>, stripe_config: &ArtcraftStripeConfigWithClient, maybe_referral_url: Option<String>, maybe_landing_url: Option<String>, maybe_referral_partner: Option<String>, maybe_referral_user_token: Option<UserToken>) -> Result<CreationPayload, CommonWebError> {
  ...
  let user_feature_flags = vec![UserFeatureFlag::Studio, UserFeatureFlag::VideoStyleTransfer].into_iter().map(|flag| flag.to_string()).collect::<Vec<String>>().join(",");
  ...
  for _ in 0..3 {
    ...
    let result = create_account_from_stripe_checkout(CreateAccountFromStripeCheckoutArgs { username: &username, display_name: &display_name, email_address: &user_email_address, email_gravatar_hash: &user_email_gravatar_hash, maybe_feature_flags: Some(&user_feature_flags), ip_address: &ip_address, maybe_source, maybe_referral_url: maybe_referral_url.clone(), maybe_landing_url: maybe_landing_url.clone(), maybe_referral_partner: maybe_referral_partner.clone(), maybe_referral_user_token: maybe_referral_user_token.as_ref() }, &mut *transaction).await;
  ...
  let session_token = create_user_session_with_executor(&final_user_token, &ip_address, &mut **mysql_connection).await.map_err(|e| {
    warn!("error creating user session: {:?}", e);
    CommonWebError::ServerError
  })?;

  let checkout_session = create_subscription_checkout_session(CreateSubscriptionCheckoutSessionArgs { subscription_price_id: price_id, maybe_existing_stripe_customer_id: None, user_token: &final_user_token, username: None, user_email: None, stripe_client: &stripe_config.client, success_url: &stripe_config.checkout_success_url, cancel_url: &stripe_config.checkout_cancel_url }).await?;

  Ok(CreationPayload { checkout_session, maybe_new_user_metadata: Some(UserMetadata { user_token: final_user_token, session_token, username: final_username, display_name: final_display_name, email_address: final_email_address }) })
}
