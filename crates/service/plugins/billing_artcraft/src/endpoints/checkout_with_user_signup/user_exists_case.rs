...
pub(super) async fn user_exists_case(price_id: &PriceId, user_metadata: &UserMetadata, mysql_connection: &mut PoolConnection<MySql>, stripe_config: &ArtcraftStripeConfigWithClient) -> Result<CreationPayload, CommonWebError> {
  ...
  let maybe_active_subscription = find_subscription_for_owner_user_using_connection(&user_metadata.user_token_typed, PaymentsNamespace::Artcraft, mysql_connection).await.map_err(|err| {
  ...
  let result = find_user_stripe_customer_link_using_connection(&user_metadata.user_token_typed, PaymentsNamespace::Artcraft, mysql_connection).await;
  ...
  let checkout_session = create_subscription_checkout_session(CreateSubscriptionCheckoutSessionArgs { subscription_price_id: price_id, maybe_existing_stripe_customer_id: maybe_existing_stripe_customer_id.as_ref(), user_token: &user_metadata.user_token_typed, username: user_metadata.username.as_deref(), user_email: user_metadata.user_email.as_deref(), stripe_client: &stripe_config.client, success_url: &stripe_config.checkout_success_url, cancel_url: &stripe_config.checkout_cancel_url }).await?;

  Ok(CreationPayload { checkout_session, maybe_new_user_metadata: None })
}
