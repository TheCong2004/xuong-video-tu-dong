...
pub async fn link_user_to_customer(details: &UserCustomerLink, transaction: &mut sqlx::Transaction<'_, sqlx::MySql>) -> anyhow::Result<()> {
  let maybe_record = find_user_stripe_customer_link_using_transaction(&details.user_token, PaymentsNamespace::Artcraft, transaction).await?;
  ...
  let insert = UpsertUserStripeCustomerLink { user_token: &details.user_token, stripe_customer_id: &details.stripe_customer_id, payments_namespace: PaymentsNamespace::Artcraft };
