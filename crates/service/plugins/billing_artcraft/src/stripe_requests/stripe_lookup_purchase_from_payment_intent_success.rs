...
pub async fn stripe_lookup_purchase_from_payment_intent_success(payment_intent_id: &str, stripe_client: &Client) -> anyhow::Result<PurchaseSummary> {
  let result = ListCheckoutSession::new().payment_intent(payment_intent_id).limit(1).build().customize::<List<CheckoutSession>>().request_strategy(STRIPE_CLIENT_RETRY_STRATEGY).send(stripe_client).await.map_err(|err| {
  ...
  let checkout_session = result.data.get(0).ok_or_else(|| anyhow::anyhow!("No items in ListCheckoutSession request."))?;

  let has_invoice = checkout_session.invoice.is_some();

  let checkout_session_id = result.data.get(0).map(|session| &session.id).ok_or_else(|| anyhow::anyhow!("No items in ListCheckoutSession request."))?;

  let line_items = ListLineItemsCheckoutSession::new(checkout_session_id).build().customize::<List<CheckoutSessionItem>>().request_strategy(STRIPE_CLIENT_RETRY_STRATEGY).send(stripe_client).await.map_err(|err| {
    error!("Stripe Error looking up checkout session for payment intent {}: {:?}", &payment_intent_id, err);
    err
  })?;
  ...
  let line_item = line_items.data.get(0).ok_or_else(|| anyhow::anyhow!("No line items found in ListCheckoutSession request."))?;

  let price = line_item.price.as_ref().ok_or_else(|| anyhow::anyhow!("No items in ListCheckoutSession request."))?;
  ...
  Ok(PurchaseSummary { price_id, product_id, quantity, has_invoice })
}
