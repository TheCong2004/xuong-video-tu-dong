  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  ...
  app.service(web::scope("/v1").service(web::scope("/stripe").service(web::resource("/webhook").route(web::post().to(stripe_webhook_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::scope("/checkout").service(web::resource("/create_redirect").route(web::post().to(stripe_create_checkout_session_json_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/redirect").route(web::get().to(stripe_create_checkout_session_redirect_handler)).route(web::head().to(|| HttpResponse::Ok())))).service(web::scope("/portal").service(web::resource("/create_redirect").route(web::post().to(stripe_create_customer_portal_session_json_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/redirect").route(web::get().to(stripe_create_customer_portal_session_redirect_handler)).route(web::head().to(|| HttpResponse::Ok()))))))
}
