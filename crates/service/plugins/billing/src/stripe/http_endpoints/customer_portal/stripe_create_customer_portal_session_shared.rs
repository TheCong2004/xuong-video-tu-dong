...
pub async fn stripe_create_customer_portal_session_shared(http_request: HttpRequest, stripe_config: web::Data<StripeConfig>, stripe_client: web::Data<stripe::Client>, url_redirector: web::Data<ThirdPartyUrlRedirector>, internal_product_to_stripe_lookup: web::Data<dyn InternalProductToStripeLookup>, internal_user_lookup: web::Data<dyn InternalUserLookup>, portal_config_id: &str) -> Result<String, CreateCustomerPortalSessionError> {
  ...
  let stripe_customer_id = match user_metadata.maybe_existing_stripe_customer_id {
    Some(stripe_customer_id) => {
      CustomerId::from_str(&stripe_customer_id).map_err(|err| {
  ...
  let return_url = match &stripe_config.portal.return_url {
    ...
    FullUrlOrPath::Path(path) => url_redirector.frontend_redirect_url_for_path(&http_request, path).map_err(|_| CreateCustomerPortalSessionError::ServerError)?,
  };

  params.return_url = Some(&return_url);
  params.configuration = Some(portal_config_id);

  let response = BillingPortalSession::create(stripe_client.as_ref(), params).await.map_err(|e| {
