...
pub async fn customer_subscription_created_handler(subscription: &Subscription, internal_subscription_product_lookup: &dyn InternalSubscriptionProductLookup, mysql_pool: &MySqlPool) -> Result<StripeWebhookSummary, StripeWebhookError> {
  ...
  let mut result = StripeWebhookSummary { maybe_user_token: summary.user_token.clone(), maybe_event_entity_id: Some(summary.stripe_subscription_id.clone()), maybe_stripe_customer_id: Some(summary.stripe_customer_id.clone()), action_was_taken: false, should_ignore_retry: false };

  // NB: It's possible to receive events out of order.
  // We won't want to play a `create` event on top.
  let maybe_existing_subscription = get_user_subscription_by_stripe_subscription_id(&summary.stripe_subscription_id, &mysql_pool).await.map_err(|err| {
  ...
  let maybe_internal_subscription_product = internal_subscription_product_lookup.lookup_internal_product_from_stripe_product_id(&summary.stripe_product_id).map_err(|err| {
  ...
  if let Some(user_token) = summary.user_token.as_deref() {
    ...
    let upsert = UpsertUserSubscription { stripe_subscription_id: &summary.stripe_subscription_id, user_token, subscription_namespace, subscription_product_slug, maybe_stripe_customer_id: Some(&summary.stripe_customer_id), maybe_stripe_product_id: Some(&summary.stripe_product_id), maybe_stripe_price_id: Some(&summary.stripe_price_id), maybe_stripe_recurring_interval: Some(summary.subscription_interval), maybe_stripe_subscription_status: Some(summary.stripe_subscription_status), maybe_stripe_is_production: Some(summary.stripe_is_production), subscription_start_at: summary.subscription_start_date, maybe_stripe_billing_cycle_anchor: None, current_billing_period_start_at: summary.current_billing_period_start, current_billing_period_end_at: summary.current_billing_period_end, subscription_expires_at: calculate_subscription_end_date(&summary), maybe_cancel_at: summary.maybe_cancel_at, maybe_canceled_at: summary.maybe_canceled_at };

    let _r = upsert.upsert(mysql_pool).await.map_err(|err| {
      let reason = format!("Mysql error: {:?}", err);
      error!("{}", reason);
      StripeWebhookError::ServerError(reason)
    })?;
    ...
    update_user_record_with_new_stripe_customer_id(mysql_pool, user_token, Some(&summary.stripe_customer_id)).await.map_err(|err| {
      let reason = format!("Mysql error: {:?}", err);
      error!("{}", reason);
      StripeWebhookError::ServerError(reason)
    })?;
