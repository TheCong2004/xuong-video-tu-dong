...
pub async fn customer_subscription_updated_handler(subscription: &Subscription, internal_subscription_product_lookup: &dyn InternalSubscriptionProductLookup, mysql_pool: &MySqlPool) -> Result<StripeWebhookSummary, StripeWebhookError> {
  ...
  Ok(StripeWebhookSummary { maybe_user_token: summary.user_token, maybe_event_entity_id: Some(summary.stripe_subscription_id), maybe_stripe_customer_id: Some(summary.stripe_customer_id), action_was_taken, should_ignore_retry })
}
