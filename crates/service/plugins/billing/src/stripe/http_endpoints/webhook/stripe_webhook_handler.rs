...
pub async fn stripe_webhook_handler(http_request: HttpRequest, request_body_bytes: Bytes, mysql_pool: web::Data<MySqlPool>, stripe_config: web::Data<StripeConfig>, internal_subscription_product_lookup: web::Data<dyn InternalSubscriptionProductLookup>) -> Result<HttpResponse, StripeWebhookError> {
  verify_stripe_webhook_ip_address(&http_request).map_err(|e| {
  ...
  let stripe_event_type = serde_json::to_string(&webhook_payload.type_).map(|s| s.replace("\"", "")).map_err(|err| {
  ...
  info!("Stripe webhook event type: {} ({:?}); is production: {}, created at: {}, pending events to be handled: {}", &stripe_event_type, &webhook_payload.type_, stripe_is_production, &stripe_event_created_at, webhook_payload.pending_webhooks);

  let maybe_previously_played_event = get_stripe_webhook_event_log_by_id(&stripe_event_id, &mysql_pool).await.map_err(|err| {
  ...
  if let Some(event) = maybe_previously_played_event {
    ...
    if event.should_ignore_retry {
      warn!("Stripe is replying event with ID={}. Ignoring it since we have previously handled it.", &stripe_event_id);
  ...
  let mut webhook_summary = StripeWebhookSummary { maybe_user_token: None, maybe_event_entity_id: None, maybe_stripe_customer_id: None, action_was_taken: false, should_ignore_retry: false };
  ...
  match webhook_payload.type_ {
    ...
    EventType::CustomerSubscriptionCreated => {
      if let EventObject::Subscription(subscription) = webhook_payload.data.object {
        webhook_summary = customer_subscription_created_handler(&subscription, internal_subscription_product_lookup.get_ref(), &mysql_pool).await?;
      }
    },
    EventType::CustomerSubscriptionUpdated => {
      if let EventObject::Subscription(subscription) = webhook_payload.data.object {
        webhook_summary = customer_subscription_updated_handler(&subscription, internal_subscription_product_lookup.get_ref(), &mysql_pool).await?;
      }
    },
    EventType::CustomerSubscriptionDeleted => {
      if let EventObject::Subscription(subscription) = webhook_payload.data.object {
        webhook_summary = customer_subscription_deleted_handler(&subscription, internal_subscription_product_lookup.get_ref(), &mysql_pool).await?;
      }
    },
  ...
  if unhandled_event_type {
    warn!("Unhandled Stripe webhook event type: {} ({:?})", &stripe_event_type, &webhook_payload.type_);
  }

  let query = InsertStripeWebhookEventLog { stripe_event_id, stripe_event_type, maybe_stripe_event_entity_id: webhook_summary.maybe_event_entity_id, maybe_stripe_customer_id: webhook_summary.maybe_stripe_customer_id, stripe_event_created_at, stripe_is_production, maybe_user_token: webhook_summary.maybe_user_token, action_was_taken: webhook_summary.action_was_taken, should_ignore_retry: webhook_summary.should_ignore_retry };

  query.insert(&mysql_pool).await.map_err(|err| {
...
fn report_success() -> Result<HttpResponse, StripeWebhookError> {
  let response = StripeWebhookSuccessResponse { success: true };

  let body = serde_json::to_string(&response).map_err(|_e| StripeWebhookError::ServerError(_e.to_string()))?;
