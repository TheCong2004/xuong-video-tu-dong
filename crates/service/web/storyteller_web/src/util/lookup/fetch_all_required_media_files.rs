...
pub async fn fetch_all_required_media_files(mysql_connection: &mut PoolConnection<MySql>, media_file_tokens: &[MediaFileToken]) -> Result<Vec<MediaFilesByTokensRecord>, CommonWebError> {
  let result = batch_get_media_files_by_tokens_with_connection(mysql_connection, &media_file_tokens, CAN_SEE_DELETED).await;
  ...
  if media_files.len() != media_file_tokens.len() {
    ...
    return Err(CommonWebError::BadInputWithSimpleMessage(format!("Not all media files could be found. Media files found: {}, tokens provided: {}", media_files.len(), media_file_tokens.len())));
  }
