...
pub async fn poll_ip_bans(ip_ban_list: IpBanList, mysql_pool: MySqlPool) {
  loop {
    ...
    let ip_addresses = bans.iter().map(|record| record.ip_address.clone()).collect::<HashSet<String>>();
    ...
    match ip_ban_list.add_set(DYNAMIC_BAN_LIST_NAME.to_string(), ip_set) {
      Ok(_) => {
        ...
        info!("Internal IP ban list updated! Total bans: {} ({} from database)", total_count, database_count);
      },
      Err(e) => {
