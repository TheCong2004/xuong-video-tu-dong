...
pub async fn require_user_session_extended(http_request: &HttpRequest, session_checker: &SessionChecker, mysql_executor: &mut MySqlConnection) -> Result<UserSessionExtended, CommonWebError> {
  let maybe_user_session = session_checker.maybe_get_user_session_extended_from_executor(http_request, mysql_executor).await.map_err(|e| {
