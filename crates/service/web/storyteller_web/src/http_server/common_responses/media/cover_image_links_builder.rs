...
impl CoverImageLinksBuilder {
  pub fn from_media_path(domain: MediaDomain, server_environment: ServerEnvironment, bucket_path: &MediaFileBucketPath) -> Option<CoverImageLinks> {
    let rooted_path = bucket_path.get_full_object_path_str();
    Self::from_rooted_path(domain, server_environment, rooted_path)
  }

  pub fn from_maybe_media_path(domain: MediaDomain, server_environment: ServerEnvironment, bucket_path: Option<&MediaFileBucketPath>) -> Option<CoverImageLinks> {
    match bucket_path {
      None => None,
  ...
  pub fn from_parts(domain: MediaDomain, server_environment: ServerEnvironment, maybe_cover_image_public_bucket_path: Option<&str>, maybe_cover_image_public_bucket_prefix: Option<&str>, maybe_cover_image_public_bucket_extension: Option<&str>) -> Option<CoverImageLinks> {
    let bucket_path = match maybe_cover_image_public_bucket_path {
      None => return None,
      Some(path) => path,
    };
    let bucket_path = MediaFileBucketPath::from_object_hash(bucket_path, maybe_cover_image_public_bucket_prefix, maybe_cover_image_public_bucket_extension);
    Self::from_media_path(domain, server_environment, &bucket_path)
  }

  pub fn from_rooted_path(domain: MediaDomain, server_environment: ServerEnvironment, rooted_path: &str) -> Option<CoverImageLinks> {
    ...
    Some(CoverImageLinks { cdn_url, thumbnail_template: thumbnail_template(domain, server_environment, rooted_path) })
  }
}
