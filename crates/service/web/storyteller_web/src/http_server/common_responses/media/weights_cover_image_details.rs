...
impl WeightsCoverImageDetails {
  pub fn from_optional_db_fields(domain: MediaDomain, server_environment: ServerEnvironment, model_weight_token: &ModelWeightToken, maybe_cover_image_public_bucket_path: Option<&str>, maybe_cover_image_public_bucket_prefix: Option<&str>, maybe_cover_image_public_bucket_extension: Option<&str>) -> Self {
    ...
    Self { maybe_cover_image_public_bucket_path, maybe_cover_image_public_bucket_url, maybe_links, default_cover: WeightsDefaultCoverInfo::from_token(model_weight_token) }
  }
}

impl WeightsDefaultCoverInfo {
  pub fn from_token(model_weight_token: &ModelWeightToken) -> Self {
    Self { image_index: default_cover_image_from_token(model_weight_token), color_index: default_cover_image_color_from_token(model_weight_token) }
  }
}
...
mod tests {
  ...
  fn test_from_optional_db_fields() {
    ...
    let cover_image = WeightsCoverImageDetails::from_optional_db_fields(domain, ServerEnvironment::Production, &token, maybe_public_bucket_hash, maybe_prefix, maybe_extension);
    ...
    assert_eq!(cover_image.maybe_cover_image_public_bucket_url, Some(Url::parse("https://cdn-2.fakeyou.com/media/b/u/c/k/e/bucket_hash/image_bucket_hash.png").unwrap()));
    assert_eq!(cover_image.default_cover.image_index, 18);

    let links = cover_image.maybe_links.unwrap();

    assert_eq!(links.cdn_url, Url::parse("https://cdn-2.fakeyou.com/media/b/u/c/k/e/bucket_hash/image_bucket_hash.png").unwrap());
    assert_eq!(links.thumbnail_template, "https://cdn-2.fakeyou.com/cdn-cgi/image/width={WIDTH},quality=95/media/b/u/c/k/e/bucket_hash/image_bucket_hash.png");
  }
}
