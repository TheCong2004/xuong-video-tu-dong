...
pub fn hydrate_to_router_request(request: &OmniGenMeshCostAndGenerateRequest) -> Result<GenerateMeshRequestBuilder, CommonWebError> {
...
fn convert_model(model: &CommonMeshModelEnum) -> Result<RouterMeshModel, CommonWebError> {
  let json = serde_json::to_string(model)?;
  serde_json::from_str(&json).map_err(|e| CommonWebError::BadInputWithSimpleMessage(format!("Unsupported mesh model: {}", e)))
}
