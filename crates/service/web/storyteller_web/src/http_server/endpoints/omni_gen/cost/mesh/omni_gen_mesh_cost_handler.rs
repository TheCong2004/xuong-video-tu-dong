...
use artcraft_api_defs::omni_gen::cost_response::omni_gen_mesh_cost_response::OmniGenMeshCostResponse;
...
#[utoipa::path(
  post,
  tag = "Omni Gen",
  path = "/v1/omni_gen/cost/mesh",
  request_body = OmniGenMeshCostAndGenerateRequest,
  responses(
    (status = 200, description = "Success", body = OmniGenMeshCostResponse),
    (status = 400, description = "Bad input"),
    (status = 500, description = "Server error"),
  ),
)]
pub async fn omni_gen_mesh_cost_handler(_http_request: HttpRequest, request: Json<OmniGenMeshCostAndGenerateRequest>, _server_state: web::Data<Arc<ServerState>>) -> Result<Json<OmniGenMeshCostResponse>, CommonWebError> {
