...
async fn upload_and_generate(draft_or_request: SplatGenerationDraftOrRequest, server_state: &ServerState, media_file_urls_by_token: Option<&HashMap<MediaFileToken, String>>) -> Result<GenerateSplatResponse, CommonWebError> {
  ...
  let splat_request = match draft_or_request {
    ...
    SplatGenerationDraftOrRequest::Draft(draft) => {
      let draft_context = SplatGenerationDraftContext { client: Some(&client), media_file_to_artcraft_url_map: media_file_urls_by_token };
  ...
  splat_request.send_request(&client).await.map_err(|err| {
