...
use artcraft_router::generate::generate_audio::audio_generation_draft_context::AudioGenerationDraftContext;
use artcraft_router::generate::generate_audio::audio_generation_draft_or_request::AudioGenerationDraftOrRequest;
...
pub struct RunPipelineV2Args<'a> {
  pub router_builder: &'a GenerateAudioRequestBuilder,
...
pub async fn run_pipeline_v2(args: RunPipelineV2Args<'_>) -> Result<PipelineResult, CommonWebError> {
  ...
  let system_cost_estimate = {
    ...
    cost_builder
      ...
      .estimate_cost()
      .map_err(|e| {
        warn!("Failed to estimate audio cost for v2: {}", e);
        CommonWebError::from_error(e)
      })?
  };
  ...
  info!("v2 audio generation response: {:?}", response);
