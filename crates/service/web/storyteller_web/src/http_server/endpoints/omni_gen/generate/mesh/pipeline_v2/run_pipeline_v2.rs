...
use artcraft_router::generate::generate_mesh::mesh_generation_draft_context::MeshGenerationDraftContext;
use artcraft_router::generate::generate_mesh::mesh_generation_draft_or_request::MeshGenerationDraftOrRequest;
...
pub struct RunPipelineV2Args<'a> {
  pub router_builder: &'a GenerateMeshRequestBuilder,
...
pub async fn run_pipeline_v2(args: RunPipelineV2Args<'_>) -> Result<PipelineResult, CommonWebError> {
  ...
  // All mesh models are fulfilled through Fal.
  let provider = RouterProvider::Fal;

  // 1. Build execution request
  let mut exec_builder = router_builder.clone();
  exec_builder.provider = provider;

  // Fal takes media URLs directly, not media file tokens.
  ...
  let system_cost_estimate = {
    ...
    cost_builder
      ...
      .estimate_cost()
      .map_err(|e| {
        warn!("Failed to estimate mesh cost for v2: {}", e);
        CommonWebError::from_error(e)
      })?
  };
  ...
  if let Err(ref err) = result {
    if let Some(ledger_entry_token) = billing.maybe_wallet_ledger_entry_token.as_ref() {
      warn!("v2 mesh generation failed, issuing refund for {}: {:?}", ledger_entry_token.as_str(), err);
  ...
  info!("v2 mesh generation response: {:?}", response);
