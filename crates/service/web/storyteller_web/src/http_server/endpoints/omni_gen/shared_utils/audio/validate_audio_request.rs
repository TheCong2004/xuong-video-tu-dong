...
pub fn validate_audio_request(request: &OmniGenAudioCostAndGenerateRequest) -> Result<(), CommonWebError> {
  ...
  match model {
    CommonAudioModel::SunoMusic | CommonAudioModel::SunoSounds => {
      if audio_reference_count > 0 {
        return Err(bad_input(format!("model {:?} does not accept audio references", model,)));
      }
      if image_reference_count > 0 {
        return Err(bad_input(format!("model {:?} does not accept image references", model,)));
      }
    },
    CommonAudioModel::SunoRemix | CommonAudioModel::SunoSample => {
      if audio_reference_count != 1 {
        return Err(bad_input(format!("model {:?} requires exactly one audio reference", model,)));
      }
      if image_reference_count > 0 {
        return Err(bad_input(format!("model {:?} does not accept image references", model,)));
      }
    ...
    CommonAudioModel::SeedAudio1p0 => {
      if audio_reference_count > SEED_AUDIO_MAX_AUDIO_REFERENCES {
        return Err(bad_input(format!("model seed_audio_1p0 accepts at most {} audio references", SEED_AUDIO_MAX_AUDIO_REFERENCES,)));
      }
      if image_reference_count > SEED_AUDIO_MAX_IMAGE_REFERENCES {
        return Err(bad_input(format!("model seed_audio_1p0 accepts at most {} image reference", SEED_AUDIO_MAX_IMAGE_REFERENCES,)));
      }
      if audio_reference_count > 0 && image_reference_count > 0 {
        return Err(bad_input("model seed_audio_1p0 cannot combine audio and image references".to_string()));
      }
...
fn validate_seed_audio_output_controls(request: &OmniGenAudioCostAndGenerateRequest) -> Result<(), CommonWebError> {
  if let Some(sample_rate_hz) = request.sample_rate_hz {
    if !SEED_AUDIO_SAMPLE_RATES_HZ.contains(&sample_rate_hz) {
      return Err(bad_input(format!("sample_rate_hz must be one of {:?}", SEED_AUDIO_SAMPLE_RATES_HZ,)));
    }
  }
...
mod tests {
  ...
  mod reference_count_tests {
    ...
    fn suno_music_rejects_audio_references() {
      let request = OmniGenAudioCostAndGenerateRequest { audio_media_tokens: Some(vec![media_token()]), ..base_request(CommonAudioModel::SunoMusic) };
    ...
    fn suno_remix_accepts_exactly_one_audio_reference() {
      let request = OmniGenAudioCostAndGenerateRequest { audio_media_tokens: Some(vec![media_token()]), ..base_request(CommonAudioModel::SunoRemix) };
      assert!(validate_audio_request(&request).is_ok());
    }

    #[test]
    fn seed_audio_rejects_too_many_audio_references() {
      let request = OmniGenAudioCostAndGenerateRequest { audio_media_tokens: Some(vec![media_token(), media_token(), media_token(), media_token()]), ..base_request(CommonAudioModel::SeedAudio1p0) };
      assert!(validate_audio_request(&request).is_err());
    }

    #[test]
    fn seed_audio_rejects_combined_audio_and_image_references() {
      let request = OmniGenAudioCostAndGenerateRequest { audio_media_tokens: Some(vec![media_token()]), image_media_tokens: Some(vec![media_token()]), ..base_request(CommonAudioModel::SeedAudio1p0) };
      assert!(validate_audio_request(&request).is_err());
    }
  }
  ...
  mod output_control_tests {
    ...
    fn seed_audio_rejects_unsupported_sample_rate() {
      let request = OmniGenAudioCostAndGenerateRequest { sample_rate_hz: Some(11025), ..base_request(CommonAudioModel::SeedAudio1p0) };
      assert!(validate_audio_request(&request).is_err());
    }

    #[test]
    fn seed_audio_accepts_supported_controls() {
      let request = OmniGenAudioCostAndGenerateRequest { sample_rate_hz: Some(24000), speed: Some(1.5), volume: Some(0.75), pitch: Some(-3.0), ..base_request(CommonAudioModel::SeedAudio1p0) };
      assert!(validate_audio_request(&request).is_ok());
    }

    #[test]
    fn seed_audio_rejects_out_of_range_speed() {
      let request = OmniGenAudioCostAndGenerateRequest { speed: Some(3.0), ..base_request(CommonAudioModel::SeedAudio1p0) };
      assert!(validate_audio_request(&request).is_err());
    }

    #[test]
    fn seed_audio_rejects_out_of_range_pitch() {
      let request = OmniGenAudioCostAndGenerateRequest { pitch: Some(13.0), ..base_request(CommonAudioModel::SeedAudio1p0) };
      assert!(validate_audio_request(&request).is_err());
    }
  }

  fn base_request(model: CommonAudioModel) -> OmniGenAudioCostAndGenerateRequest {
    OmniGenAudioCostAndGenerateRequest { idempotency_token: None, model: Some(model), prompt: Some("a corgi barking in the rain".to_string()), style_prompt: None, audio_media_tokens: None, image_media_tokens: None, keep_lyrics: None, is_instrumental: None, is_loopable: None, bpm: None, musical_key: None, sample_rate_hz: None, speed: None, volume: None, pitch: None }
  }
