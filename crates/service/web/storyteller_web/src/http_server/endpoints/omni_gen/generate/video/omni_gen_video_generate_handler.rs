...
pub async fn omni_gen_video_generate_handler(http_request: HttpRequest, request: Json<OmniGenVideoCostAndGenerateRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<OmniGenVideoGenerateResponse>, CommonWebError> {
  ...
  let (primary_job_token, all_job_tokens) = match &pipeline_result.response {
    GenerateVideoResponse::Seedance2Pro(payload) => {
      ...
      let result = insert_seedance2pro_jobs(InsertSeedance2proJobsArgs { primary_order_id: &payload.order_id, maybe_additional_order_ids: payload.maybe_order_ids.as_deref(), maybe_wallet_ledger_entry_token: pipeline_result.billing.maybe_wallet_ledger_entry_token.as_ref(), kinovi_version, shared: SharedJobArgs { apriori_job_token: &pipeline_result.billing.apriori_job_token, idempotency_token: &idempotency_token, user_token, maybe_avt_token: maybe_avt_token.as_ref(), maybe_model_type: request.model.map(|v| v.to_common_model_type()), maybe_prompt_token: prompt_token.as_ref(), maybe_debug_log_event_token: Some(&debug_log_event_token), maybe_platform_type, maybe_cost_estimates: Some(pipeline_result.cost_estimates), ip_address: &ip_address, transaction: &mut transaction } }).await?;
    ...
    GenerateVideoResponse::GmiCloud(payload) => {
      ...
      let token = insert_gmicloud_job(InsertGmiCloudJobArgs { external_request_id: &payload.request_id, shared: SharedJobArgs { apriori_job_token: &pipeline_result.billing.apriori_job_token, idempotency_token: &idempotency_token, user_token, maybe_avt_token: maybe_avt_token.as_ref(), maybe_model_type: request.model.map(|v| v.to_common_model_type()), maybe_prompt_token: prompt_token.as_ref(), maybe_debug_log_event_token: Some(&debug_log_event_token), maybe_platform_type, maybe_cost_estimates: Some(pipeline_result.cost_estimates), ip_address: &ip_address, transaction: &mut transaction } }).await?;

      (token.clone(), vec![token])
    },
    GenerateVideoResponse::Grok(payload) => {
      info!("Inserting Grok (xAI) API job with token: {:?}", pipeline_result.billing.apriori_job_token);
      let token = insert_grok_api_job(InsertGrokApiJobArgs { external_request_id: &payload.request_id, shared: SharedJobArgs { apriori_job_token: &pipeline_result.billing.apriori_job_token, idempotency_token: &idempotency_token, user_token, maybe_avt_token: maybe_avt_token.as_ref(), maybe_model_type: request.model.map(|v| v.to_common_model_type()), maybe_prompt_token: prompt_token.as_ref(), maybe_debug_log_event_token: Some(&debug_log_event_token), maybe_platform_type, maybe_cost_estimates: Some(pipeline_result.cost_estimates), ip_address: &ip_address, transaction: &mut transaction } }).await?;
