...
pub fn hydrate_to_router_request(request: &OmniGenImageCostAndGenerateRequest) -> Result<GenerateImageRequestBuilder, CommonWebError> {
  ...
  let quality = request.quality.as_ref().map(convert_quality).transpose()?;

  Ok(GenerateImageRequestBuilder { model, provider: RouterProvider::Artcraft, prompt: request.prompt.clone(), image_inputs: request.image_media_tokens.clone().map(ImageListRef::MediaFileTokens), resolution, aspect_ratio, quality, image_batch_count: request.image_batch_count, horizontal_angle: request.adjust_horizontal_angle, vertical_angle: request.adjust_vertical_angle, zoom: request.adjust_zoom, request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::PayMoreUpgrade, generation_mode_mismatch_strategy: None, idempotency_token: request.idempotency_token.clone() })
}

fn convert_model(model: &CommonImageModelEnum) -> Result<RouterImageModel, CommonWebError> {
  let json = serde_json::to_string(model)?;
  serde_json::from_str(&json).map_err(|e| CommonWebError::BadInputWithSimpleMessage(format!("Unsupported image model: {}", e)))
}
...
fn convert_quality(quality: &CommonQualityEnum) -> Result<RouterQuality, CommonWebError> {
  let json = serde_json::to_string(quality)?;
  serde_json::from_str(&json).map_err(|e| CommonWebError::BadInputWithSimpleMessage(format!("Unsupported quality: {}", e)))
}
