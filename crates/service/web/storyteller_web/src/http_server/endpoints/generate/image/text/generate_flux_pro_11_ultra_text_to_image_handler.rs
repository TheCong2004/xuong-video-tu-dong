...
pub async fn generate_flux_pro_11_ultra_text_to_image_handler(http_request: HttpRequest, request: Json<GenerateFluxPro11UltraTextToImageRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GenerateFluxPro11UltraTextToImageResponse>, CommonWebError> {
  ...
  let ultra_request = FluxPro11UltraRequest { prompt: request.prompt.clone().unwrap_or_default(), aspect_ratio, num_images };
  ...
  let args = FluxPro11UltraArgs { request: ultra_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

  let fal_result = enqueue_flux_pro_11_ultra_text_to_image_webhook(args).await.map_err(|err| {
  ...
  Ok(Json(GenerateFluxPro11UltraTextToImageResponse { success: true, inference_job_token: job_token }))
}
