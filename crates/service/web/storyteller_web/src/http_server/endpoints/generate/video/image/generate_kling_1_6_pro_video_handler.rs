...
use fal_client::requests_old::webhook::video::image::enqueue_kling_v1p6_pro_image_to_video_webhook::{enqueue_kling_v1p6_pro_image_to_video_webhook, Kling1p6ProArgs, Kling1p6ProRequest, Kling1p6ProAspectRatio, Kling1p6ProDuration};
...
pub async fn generate_kling_1_6_pro_video_handler(http_request: HttpRequest, request: Json<GenerateKling16ProImageToVideoRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GenerateKling16ProImageToVideoResponse>, CommonWebError> {
  ...
  let fal_request = Kling1p6ProRequest { image_url: start_frame_url.to_string(), end_frame_image_url: maybe_end_frame_url.map(|u| u.to_string()), prompt: prompt.to_string(), duration, aspect_ratio };

  let cost = fal_request.calculate_cost_in_cents();

  let args = Kling1p6ProArgs { request: fal_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };
  ...
  let fal_result = enqueue_kling_v1p6_pro_image_to_video_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_aspect_ratio: request.aspect_ratio.as_ref().map(|ar| match ar {
      GenerateKling16ProAspectRatio::Square => CommonAspectRatio::Square,
      GenerateKling16ProAspectRatio::WideSixteenNine => CommonAspectRatio::WideSixteenByNine,
      GenerateKling16ProAspectRatio::TallNineSixteen => CommonAspectRatio::TallNineBySixteen,
    }),
    maybe_resolution: None,
    maybe_batch_count: None,
    maybe_generate_audio: None,
    maybe_duration_seconds: request.duration.as_ref().map(|d| match d {
      GenerateKling16ProDuration::FiveSeconds => 5,
      GenerateKling16ProDuration::TenSeconds => 10,
    }),
  ...
  Ok(Json(GenerateKling16ProImageToVideoResponse { success: true, inference_job_token: job_token }))
}
