...
use artcraft_api_defs::generate::object::multi_function::hunyuan3d_v3_multi_function_object_gen::{Hunyuan3dV3GenerateType, Hunyuan3dV3MultiFunctionObjectGenRequest, Hunyuan3dV3MultiFunctionObjectGenResponse, Hunyuan3dV3PolygonType};
use enums::by_table::prompt_context_items::prompt_context_semantic_type::PromptContextSemanticType;
use enums::by_table::prompts::prompt_type::PromptType;
use enums::common::generation_provider::GenerationProvider;
use enums::common::generation::common_model_type::CommonModelType;
use enums::common::visibility::Visibility;
use fal_client::requests_old::webhook::object::enqueue_hunyuan3d_v3_image_to_3d_webhook::{enqueue_hunyuan3d_v3_image_to_3d_webhook, EnqueueHunyuan3dV3ImageTo3dArgs, EnqueueHunyuan3dV3ImageTo3dGenerateType, EnqueueHunyuan3dV3ImageTo3dPolygonType, EnqueueHunyuan3dV3ImageTo3dRequest};
use fal_client::requests_old::webhook::object::enqueue_hunyuan3d_v3_sketch_to_3d_webhook::{enqueue_hunyuan3d_v3_sketch_to_3d_webhook, EnqueueHunyuan3dV3SketchTo3dArgs, EnqueueHunyuan3dV3SketchTo3dGenerateType, EnqueueHunyuan3dV3SketchTo3dPolygonType, EnqueueHunyuan3dV3SketchTo3dRequest};
use fal_client::requests_old::webhook::object::enqueue_hunyuan3d_v3_text_to_3d_webhook::{enqueue_hunyuan3d_v3_text_to_3d_webhook, EnqueueHunyuan3dV3TextTo3dArgs, EnqueueHunyuan3dV3TextTo3dGenerateType, EnqueueHunyuan3dV3TextTo3dPolygonType, EnqueueHunyuan3dV3TextTo3dRequest};
use http_server_common::request::get_request_ip::get_request_ip;
use log::{error, info, warn};
use mysql_queries::queries::generic_inference::api_providers::fal::insert_generic_inference_job_for_fal_queue::{insert_generic_inference_job_for_fal_queue, FalCategory, InsertGenericInferenceForFalArgs};
...
pub async fn generate_hunyuan3d_v3_multi_function_object_handler(http_request: HttpRequest, request: Json<Hunyuan3dV3MultiFunctionObjectGenRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<Hunyuan3dV3MultiFunctionObjectGenResponse>, CommonWebError> {
  ...
  let image_url = request.image_media_token.as_ref().and_then(|token| image_urls_by_token.get(token)).map(|url| url.to_string());

  let back_image_url = request.back_image_media_token.as_ref().and_then(|token| image_urls_by_token.get(token)).map(|url| url.to_string());

  let left_image_url = request.left_image_media_token.as_ref().and_then(|token| image_urls_by_token.get(token)).map(|url| url.to_string());

  let right_image_url = request.right_image_media_token.as_ref().and_then(|token| image_urls_by_token.get(token)).map(|url| url.to_string());
  ...
  let has_prompt = request.prompt.as_ref().map(|p| !p.trim().is_empty()).unwrap_or(false);
  ...
  let fal_result = match (has_prompt, has_image) {
    ...
    (true, false) => {
      ...
      let polygon_type = request.polygon_type.map(|t| match t {
        ...
        Hunyuan3dV3PolygonType::Quadrilateral => EnqueueHunyuan3dV3TextTo3dPolygonType::Quadrilateral,
      });

      let args = EnqueueHunyuan3dV3TextTo3dArgs { request: EnqueueHunyuan3dV3TextTo3dRequest { prompt: request.prompt.clone().unwrap_or_default(), face_count: request.face_count, generate_type, polygon_type, enable_pbr: Some(enable_pbr) }, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

      enqueue_hunyuan3d_v3_text_to_3d_webhook(args).await.map_err(|err| {
        warn!("Error calling enqueue_hunyuan3d_v3_text_to_3d_webhook: {:?}", err);
    ...
    (true, true) => {
      ...
      let polygon_type = request.polygon_type.map(|t| match t {
        ...
        Hunyuan3dV3PolygonType::Quadrilateral => EnqueueHunyuan3dV3SketchTo3dPolygonType::Quadrilateral,
      });

      let args = EnqueueHunyuan3dV3SketchTo3dArgs { request: EnqueueHunyuan3dV3SketchTo3dRequest { prompt: request.prompt.clone().unwrap_or_default(), image_url: image_url.unwrap(), face_count: request.face_count, generate_type, polygon_type, enable_pbr: Some(enable_pbr) }, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

      enqueue_hunyuan3d_v3_sketch_to_3d_webhook(args).await.map_err(|err| {
        warn!("Error calling enqueue_hunyuan3d_v3_sketch_to_3d_webhook: {:?}", err);
        CommonWebError::from_error(err)
      })?
    },
    ...
    (false, true) => {
      ...
      let polygon_type = request.polygon_type.map(|t| match t {
        ...
        Hunyuan3dV3PolygonType::Quadrilateral => EnqueueHunyuan3dV3ImageTo3dPolygonType::Quadrilateral,
      });

      let args = EnqueueHunyuan3dV3ImageTo3dArgs { request: EnqueueHunyuan3dV3ImageTo3dRequest { image_url: image_url.unwrap(), back_image_url, left_image_url, right_image_url, face_count: request.face_count, generate_type, polygon_type, enable_pbr: Some(enable_pbr) }, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

      enqueue_hunyuan3d_v3_image_to_3d_webhook(args).await.map_err(|err| {
        warn!("Error calling enqueue_hunyuan3d_v3_image_to_3d_webhook: {:?}", err);
        CommonWebError::from_error(err)
      })?
    },
    ...
    (false, false) => {
      return Err(CommonWebError::BadInputWithSimpleMessage("Either prompt or image_media_token must be provided".to_string()));
    },
  };
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_model_type: Some(CommonModelType::Hunyuan3d3),
  ...
  let db_result = insert_generic_inference_job_for_fal_queue(InsertGenericInferenceForFalArgs { uuid_idempotency_token: &request.uuid_idempotency_token, maybe_external_third_party_id: &external_job_id, fal_category: FalCategory::ObjectGeneration, maybe_model_type: Some(CommonModelType::Hunyuan3d3), maybe_inference_args: None, maybe_prompt_token: prompt_token.as_ref(), maybe_creator_user_token: maybe_user_session.as_ref().map(|s| &s.user_token), maybe_avt_token: maybe_avt_token.as_ref(), creator_ip_address: &ip_address, creator_set_visibility: Visibility::Public, maybe_platform_type: get_request_platform_type(&http_request), mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(Hunyuan3dV3MultiFunctionObjectGenResponse { success: true, inference_job_token: job_token }))
}
