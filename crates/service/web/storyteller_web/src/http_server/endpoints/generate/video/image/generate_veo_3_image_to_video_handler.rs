...
pub async fn generate_veo_3_image_to_video_handler(http_request: HttpRequest, request: Json<GenerateVeo3ImageToVideoRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GenerateVeo3ImageToVideoResponse>, CommonWebError> {
  ...
  let veo3_request = Veo3Request { image_url: media_links.cdn_url.to_string(), prompt: prompt.to_string(), duration, resolution, aspect_ratio, generate_audio };

  let cost = veo3_request.calculate_cost_in_cents();

  let args = Veo3Args { request: veo3_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };
  ...
  let fal_result = enqueue_veo_3_image_to_video_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_aspect_ratio: request.aspect_ratio.as_ref().map(|ar| match ar {
      GenerateVeo3AspectRatio::WideSixteenNine => CommonAspectRatio::WideSixteenByNine,
      GenerateVeo3AspectRatio::TallNineSixteen => CommonAspectRatio::TallNineBySixteen,
      GenerateVeo3AspectRatio::Square => CommonAspectRatio::Square,
    }),
    maybe_resolution: None,
    maybe_batch_count: None,
    maybe_generate_audio: Some(generate_audio),
    maybe_duration_seconds: request.duration.as_ref().map(|d| match d {
      GenerateVeo3Duration::EightSeconds => 8,
    }),
  ...
  Ok(Json(GenerateVeo3ImageToVideoResponse { success: true, inference_job_token: job_token }))
}
