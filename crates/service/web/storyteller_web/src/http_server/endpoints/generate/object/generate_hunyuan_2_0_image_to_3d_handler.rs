...
pub async fn generate_hunyuan_2_0_image_to_3d_handler(http_request: HttpRequest, request: Json<GenerateHunyuan20ImageTo3dRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GenerateHunyuan20ImageTo3dResponse>, CommonWebError> {
  ...
  let args = Hunyuan3d2Args { request: Hunyuan3d2Request { image_url: media_links.cdn_url.to_string() }, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

  let fal_result = enqueue_hunyuan_3d_2_image_to_3d_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs { maybe_bitrate: None, maybe_apriori_prompt_token: None, prompt_type: PromptType::ArtcraftApp, maybe_creator_user_token: maybe_user_session.as_ref().map(|s| &s.user_token), maybe_model_type: Some(CommonModelType::Hunyuan3d2_0), maybe_generation_provider: Some(GenerationProvider::Artcraft), maybe_positive_prompt: None, maybe_negative_prompt: None, maybe_other_args: None, maybe_generation_mode: None, maybe_aspect_ratio: None, maybe_resolution: None, maybe_batch_count: None, maybe_generate_audio: None, maybe_duration_seconds: None, creator_ip_address: &ip_address, mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(GenerateHunyuan20ImageTo3dResponse { success: true, inference_job_token: job_token }))
}
