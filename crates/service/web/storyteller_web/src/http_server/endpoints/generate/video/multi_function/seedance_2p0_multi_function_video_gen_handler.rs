...
use artcraft_api_defs::generate::video::multi_function::seedance_2p0_multi_function_video_gen::{Seedance2p0AspectRatio, Seedance2p0BatchCount, Seedance2p0MultiFunctionVideoGenRequest, Seedance2p0MultiFunctionVideoGenResponse, Seedance2p0OutputResolution};
...
use seedance2pro_client::requests::generate_video::generate_video::{generate_video, KinoviBatchCount, GenerateVideoArgs, KinoviGenerateVideoRequest, GenerateVideoResponse, KinoviModelType, KinoviOutputResolution, KinoviAspectRatio};
...
pub async fn seedance_2p0_multi_function_video_gen_handler(http_request: HttpRequest, request: Json<Seedance2p0MultiFunctionVideoGenRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<Seedance2p0MultiFunctionVideoGenResponse>, CommonWebError> {
  ...
  let debug_log_context = DebugLogContext { event_token: &debug_log_event_token, user_token, ip_address: &ip_address, request_url: &request_url };
  ...
  let file_urls_by_token = if all_media_tokens.is_empty() {
    ...
    lookup_media_file_urls_as_map(&http_request, &mut mysql_connection, server_state.server_environment, &all_media_tokens).await?
  };

  // --- Look up character tokens (if any) ---

  let kinovi_character_ids = resolve_kinovi_character_ids(request.reference_character_tokens.as_deref(), &mut mysql_connection).await?;
  ...
  let deduction_result = attempt_wallet_deduction_else_common_web_error(user_token, Some(apriori_job_token.as_str()), cost_in_cents, &mut mysql_connection).await?;

  // --- Determine session and generate ---

  let user_feature_flags = UserSessionFeatureFlags::new(user_session.maybe_feature_flags.as_deref());
  ...
  let gen_result = if is_whitelisted {
    ...
    let whitelist_session = Seedance2ProSession::from_cookies_string(server_state.inference_providers.seedance2pro.cookies_byteplus.clone());

    let result = upload_and_generate(&whitelist_session, &request, &file_urls_by_token, aspect_ratio, output_resolution, batch_count, duration_seconds, kinovi_character_ids.clone(), &debug_log_context, &mut mysql_connection).await;
    ...
    match result {
      ...
      Err(err) => {
        ...
        let regular_session = Seedance2ProSession::from_cookies_string(server_state.inference_providers.seedance2pro.cookies_volcengine.clone());

        let result = upload_and_generate(&regular_session, &request, &file_urls_by_token, aspect_ratio, output_resolution, batch_count, duration_seconds, kinovi_character_ids.clone(), &debug_log_context, &mut mysql_connection).await;
        ...
        match result {
          ...
          Err(err) => {
            ...
            if err.status_code() != StatusCode::BAD_REQUEST {
              let notification = NotificationDetailsBuilder::from_boxed_error(err.clone().into()).set_title("Seedance 2.0 generation failed (whitelist + fallback)".to_string()).set_urgency(Some(NotificationUrgency::High)).build();
              if let Err(page_err) = server_state.pager.enqueue_page(notification) {
                warn!("Failed to enqueue pager alert: {:?}", page_err);
              }
            }

            insert_generation_failure_debug_log(&debug_log_event_token, user_token, &ip_address, &request_url, &format!("Seedance 2.0 generation failed: {:?}", err), &mut *mysql_connection).await;
    ...
    let result = upload_and_generate(&regular_session, &request, &file_urls_by_token, aspect_ratio, output_resolution, batch_count, duration_seconds, kinovi_character_ids, &debug_log_context, &mut mysql_connection).await;
    ...
    match result {
      ...
      Err(err) => {
        ...
        // surface to the user as 400s and shouldn't page.
        if err.status_code() != StatusCode::BAD_REQUEST {
          let notification = NotificationDetailsBuilder::from_boxed_error(err.clone().into()).set_title("Seedance 2.0 generation failed".to_string()).set_urgency(Some(NotificationUrgency::High)).build();
          if let Err(page_err) = server_state.pager.enqueue_page(notification) {
            warn!("Failed to enqueue pager alert: {:?}", page_err);
          }
        }
  ...
  info!("Seedance2pro task_id={}, order_id={}", gen_response.task_id, gen_response.order_id);
  ...
  if let Some(token) = prompt_token.as_ref() {
    ...
    let ref_token_groups: [(Option<&[MediaFileToken]>, PromptContextSemanticType); 3] = [(request.reference_image_media_tokens.as_deref(), PromptContextSemanticType::Imgref), (request.reference_video_media_tokens.as_deref(), PromptContextSemanticType::VidRef), (request.reference_audio_media_tokens.as_deref(), PromptContextSemanticType::Audioref)];

    for (maybe_tokens, semantic_type) in ref_token_groups {
      if let Some(tokens) = maybe_tokens {
        for media_token in tokens {
          context_items.push(PromptContextItem { media_token: media_token.clone(), context_semantic_type: semantic_type });
        }
      }
    }
  ...
  for (i, order_id) in order_ids.iter().enumerate() {
    ...
    let idempotency_str = if i == 0 { request.uuid_idempotency_token.clone() } else { format!("{}-batch-{}", request.uuid_idempotency_token, i) };

    let db_result = insert_generic_inference_job_for_seedance2pro_queue_with_apriori_job_token(InsertGenericInferenceForSeedance2ProWithAprioriJobTokenArgs { kinovi_version: KinoviVersion::Volcengine, apriori_job_token: &job_token, uuid_idempotency_token: &idempotency_str, maybe_external_third_party_id: order_id, maybe_model_type: Some(CommonModelType::Seedance2p0), maybe_inference_args: None, maybe_prompt_token: prompt_token.as_ref(), maybe_wallet_ledger_entry_token: Some(&deduction_result.ledger_entry_token), maybe_creator_user_token: Some(user_token), maybe_avt_token: maybe_avt_token.as_ref(), creator_ip_address: &ip_address, creator_set_visibility: Visibility::Public, maybe_platform_type: get_request_platform_type(&http_request), maybe_cost_estimates: None, maybe_debug_log_event_token: Some(&debug_log_event_token), mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(Seedance2p0MultiFunctionVideoGenResponse { success: true, inference_job_token: first_job_token, all_inference_job_tokens: all_job_tokens }))
}
...
/// we know will fail.
fn validate_seedance_2p0_limits(request: &Seedance2p0MultiFunctionVideoGenRequest) -> Result<(), CommonWebError> {
  if let Some(prompt) = request.prompt.as_deref() {
    let char_count = prompt.chars().count();
    if char_count > MAX_PROMPT_CHARS && !text_contains_cjk(prompt) {
      return Err(CommonWebError::BadInputWithSimpleMessage(format!("Prompt is too long: Seedance 2.0 models accept at most {MAX_PROMPT_CHARS} characters (got {char_count}).",)));
    }
  }

  let image_reference_count = request.reference_image_media_tokens.as_ref().map_or(0, |tokens| tokens.len());

  if image_reference_count > MAX_IMAGE_REFERENCES {
    return Err(CommonWebError::BadInputWithSimpleMessage(format!("Too many image references: Seedance 2.0 models accept at most {MAX_IMAGE_REFERENCES} reference images (got {image_reference_count}).",)));
  }
...
fn estimate_cost_upfront(aspect_ratio: KinoviAspectRatio, output_resolution: Option<KinoviOutputResolution>, batch_count: KinoviBatchCount, duration_seconds: u8) -> u64 {
  let request = KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: String::new(), aspect_ratio, duration_seconds, batch_count, output_resolution, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None };
...
async fn upload_and_generate(session: &Seedance2ProSession, request: &Seedance2p0MultiFunctionVideoGenRequest, file_urls_by_token: &HashMap<MediaFileToken, Url>, aspect_ratio: KinoviAspectRatio, output_resolution: Option<KinoviOutputResolution>, batch_count: KinoviBatchCount, duration_seconds: u8, kinovi_character_ids: Option<Vec<String>>, debug_log_context: &DebugLogContext<'_>, mysql_connection: &mut sqlx::pool::PoolConnection<MySql>) -> Result<SeedanceGenerationResult, CommonWebError> {
  ...
  let reference_image_urls = upload_reference_tokens_to_seedance2pro(session, file_urls_by_token, request.reference_image_media_tokens.as_deref(), "Reference image").await?;

  let reference_video_urls = upload_reference_tokens_to_seedance2pro(session, file_urls_by_token, request.reference_video_media_tokens.as_deref(), "Reference video").await?;

  let reference_audio_urls = upload_reference_tokens_to_seedance2pro(session, file_urls_by_token, request.reference_audio_media_tokens.as_deref(), "Reference audio").await?;

  // --- Determine generation mode ---

  let is_keyframe = request.start_frame_media_token.is_some() || request.end_frame_media_token.is_some();

  let is_reference = request.reference_image_media_tokens.is_some() || request.reference_video_media_tokens.is_some() || request.reference_audio_media_tokens.is_some();
  ...
  let kinovi_request = KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt, aspect_ratio, output_resolution, duration_seconds, batch_count, start_frame_url, end_frame_url, reference_image_urls, reference_video_urls, reference_audio_urls, character_ids: kinovi_character_ids, use_face_blur_hack: None };

  // ==================== DEBUG LOG: KINOVI REQUEST ==================== //
  // Logged BEFORE the send so the outbound payload is captured even when
  // the generation call fails.

  if let Err(err) = insert_debug_log(InsertDebugLogArgs { apriori_debug_log_event_token: Some(debug_log_context.event_token), maybe_creator_user_token: Some(debug_log_context.user_token), debug_log_type: DebugLogType::KinoviRequest, maybe_log_level: Some(DebugLogLevel::Info), maybe_ip_address: Some(debug_log_context.ip_address), maybe_url: Some(debug_log_context.request_url), message: &format!("{:#?}", kinovi_request), mysql_executor: &mut **mysql_connection, phantom: Default::default() }).await {
    warn!("Failed to insert Kinovi request debug log: {:?}", err);
  }

  let video_gen_args = GenerateVideoArgs { session, host_override: None, request: kinovi_request };

  let gen_response = generate_video(video_gen_args).await.map_err(|err| {
    warn!("Error calling seedance2pro generate_video: {:?}", err);
    map_seedance2pro_error_to_web_error(err)
  })?;

  Ok(SeedanceGenerationResult { gen_response, generation_mode })
}
...
async fn resolve_kinovi_character_ids(maybe_tokens: Option<&[CharacterToken]>, connection: &mut sqlx::pool::PoolConnection<MySql>) -> Result<Option<Vec<String>>, CommonWebError> {
  ...
  let ids: Vec<String> = characters.iter().filter(|c| c.is_active).filter_map(|c| c.kinovi_character_id.clone()).collect();
...
async fn upload_reference_tokens_to_seedance2pro(session: &Seedance2ProSession, file_urls_by_token: &HashMap<MediaFileToken, Url>, maybe_tokens: Option<&[MediaFileToken]>, label: &str) -> Result<Option<Vec<String>>, CommonWebError> {
  let tokens = match maybe_tokens {
    None => return Ok(None),
    Some(tokens) if tokens.is_empty() => return Ok(None),
    Some(tokens) => tokens,
  ...
  for token in tokens {
    match file_urls_by_token.get(token) {
      None => return Err(CommonWebError::BadInputWithSimpleMessage(format!("{} media not found: {:?}", label, token))),
      Some(url) => {
...
async fn upload_to_seedance2pro(session: &Seedance2ProSession, our_cdn_url: &Url) -> Result<String, CommonWebError> {
  let extension = extract_extension_from_url(our_cdn_url, &ExtractExtensions::All).map(|ext| ext.without_period().to_string()).unwrap_or_else(|| "png".to_string());
  ...
  let prepare_result = prepare_file_upload(PrepareFileUploadArgs { session, extension, host_override: None }).await.map_err(|err| {
    warn!("Error preparing seedance2pro file upload: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  let upload_result = upload_file(UploadFileArgs { upload_url: prepare_result.upload_url, file_bytes, host_override: None }).await.map_err(|err| {
...
mod tests {
  ...
  mod seedance_limits_tests {
    ...
    fn accepts_prompt_at_limit() {
      let req = Seedance2p0MultiFunctionVideoGenRequest { prompt: Some("a".repeat(MAX_PROMPT_CHARS)), ..base_request() };
      assert!(validate_seedance_2p0_limits(&req).is_ok());
    }

    #[test]
    fn rejects_prompt_over_limit() {
      let req = Seedance2p0MultiFunctionVideoGenRequest { prompt: Some("a".repeat(MAX_PROMPT_CHARS + 1)), ..base_request() };
      assert!(matches!(validate_seedance_2p0_limits(&req), Err(CommonWebError::BadInputWithSimpleMessage(_))));
    }
    ...
    fn accepts_over_limit_prompt_containing_cjk() {
      ...
      prompt.push('猫');
      let req = Seedance2p0MultiFunctionVideoGenRequest { prompt: Some(prompt), ..base_request() };
      assert!(validate_seedance_2p0_limits(&req).is_ok());
    }

    #[test]
    fn accepts_nine_image_references() {
      let req = Seedance2p0MultiFunctionVideoGenRequest { reference_image_media_tokens: Some(reference_tokens(MAX_IMAGE_REFERENCES)), ..base_request() };
      assert!(validate_seedance_2p0_limits(&req).is_ok());
    }

    #[test]
    fn rejects_ten_image_references() {
      let req = Seedance2p0MultiFunctionVideoGenRequest { reference_image_media_tokens: Some(reference_tokens(MAX_IMAGE_REFERENCES + 1)), ..base_request() };
      assert!(matches!(validate_seedance_2p0_limits(&req), Err(CommonWebError::BadInputWithSimpleMessage(_))));
    }
  }

  fn base_request() -> Seedance2p0MultiFunctionVideoGenRequest {
    Seedance2p0MultiFunctionVideoGenRequest { uuid_idempotency_token: "a1b2c3d4-e5f6-7890-abcd-ef1234567890".to_string(), prompt: Some("test".to_string()), start_frame_media_token: None, end_frame_media_token: None, reference_image_media_tokens: None, reference_video_media_tokens: None, reference_audio_media_tokens: None, reference_character_tokens: None, aspect_ratio: None, output_resolution: None, duration_seconds: Some(5), batch_count: None }
  }
