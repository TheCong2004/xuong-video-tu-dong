...
pub async fn nano_banana_2_multi_function_image_gen_handler(http_request: HttpRequest, request: Json<NanaBanana2MultiFunctionImageGenRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<NanaBanana2MultiFunctionImageGenResponse>, CommonWebError> {
  ...
  if let Some(input_image_urls) = image_urls.as_deref() {
    ...
    let edit_request = EnqueueNanoBanana2EditImageRequest { prompt: request.prompt.as_deref().unwrap_or("").to_string(), image_urls: input_image_urls.to_owned(), num_images, resolution, aspect_ratio };

    let cost = edit_request.calculate_cost_in_cents();

    let args = EnqueueNanoBanana2EditImageArgs { request: edit_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };
    ...
    fal_result = enqueue_nano_banana_2_edit_image_webhook(args).await.map_err(|err| {
    ...
    let t2i_request = EnqueueNanoBanana2TextToImageRequest { prompt: request.prompt.as_deref().unwrap_or("").to_string(), num_images, resolution, aspect_ratio };

    let cost = t2i_request.calculate_cost_in_cents();

    let args = EnqueueNanoBanana2TextToImageArgs { request: t2i_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

    info!("Charging wallet: {}", cost);

    attempt_wallet_deduction_else_common_web_error(user_token, Some(apriori_job_token.as_str()), cost, &mut mysql_connection).await?;

    fal_result = enqueue_nano_banana_2_text_to_image_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs { maybe_bitrate: None, maybe_apriori_prompt_token: None, prompt_type: PromptType::ArtcraftApp, maybe_creator_user_token: Some(&user_token), maybe_model_type: Some(CommonModelType::NanoBanana2), maybe_generation_provider: Some(GenerationProvider::Artcraft), maybe_positive_prompt: request.prompt.as_deref(), maybe_negative_prompt: None, maybe_other_args: None, maybe_generation_mode: Some(generation_mode), maybe_aspect_ratio, maybe_resolution, maybe_batch_count, maybe_generate_audio: None, maybe_duration_seconds: None, creator_ip_address: &ip_address, mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(NanaBanana2MultiFunctionImageGenResponse { success: true, inference_job_token: job_token }))
}
