...
use fal_client::requests_old::webhook::video::image::enqueue_sora_2_pro_image_to_video_webhook::{enqueue_sora_2_pro_image_to_video_webhook, EnqueueSora2ProImageToVideoArgs, EnqueueSora2ProImageToVideoAspectRatio, EnqueueSora2ProImageToVideoDurationSeconds, EnqueueSora2ProImageToVideoRequest, EnqueueSora2ProImageToVideoResolution};
use fal_client::requests_old::webhook::video::text::enqueue_sora_2_pro_text_to_video_webhook::{enqueue_sora_2_pro_text_to_video_webhook, EnqueueSora2ProTextToVideoArgs, EnqueueSora2ProTextToVideoRequest, EnqueueSora2ProTextToVideoAspectRatio, EnqueueSora2ProTextToVideoDurationSeconds, EnqueueSora2ProTextToVideoResolution};
...
pub async fn sora_2_pro_multi_function_video_gen_handler(http_request: HttpRequest, request: Json<Sora2ProMultiFunctionVideoGenRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<Sora2ProMultiFunctionVideoGenResponse>, CommonWebError> {
  ...
  if let Some(image_url) = maybe_image_url {
    ...
    let i2v_request = EnqueueSora2ProImageToVideoRequest { image_url, prompt: request.prompt.as_deref().unwrap_or("").to_string(), duration: Some(duration), aspect_ratio: Some(aspect_ratio), resolution: Some(resolution) };
    ...
    let args = EnqueueSora2ProImageToVideoArgs { request: i2v_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

    fal_result = enqueue_sora_2_pro_image_to_video_webhook(args).await.map_err(|err| {
    ...
    let t2v_request = EnqueueSora2ProTextToVideoRequest { prompt: request.prompt.as_deref().unwrap_or("").to_string(), duration: Some(duration), aspect_ratio: Some(aspect_ratio), resolution: Some(resolution) };
    ...
    let args = EnqueueSora2ProTextToVideoArgs { request: t2v_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

    fal_result = enqueue_sora_2_pro_text_to_video_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_model_type: Some(CommonModelType::Sora2Pro),
    ...
    maybe_duration_seconds: request.duration.as_ref().map(|d| match d {
      Sora2ProMultiFunctionVideoGenDuration::FourSeconds => 4,
      Sora2ProMultiFunctionVideoGenDuration::EightSeconds => 8,
      Sora2ProMultiFunctionVideoGenDuration::TwelveSeconds => 12,
    }),
  ...
  Ok(Json(Sora2ProMultiFunctionVideoGenResponse { success: true, inference_job_token: job_token }))
}
