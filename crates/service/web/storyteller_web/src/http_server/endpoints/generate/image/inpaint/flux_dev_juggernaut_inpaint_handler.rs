...
use fal_client::requests_old::webhook::image::infill::enqueue_flux_dev_juggernaut_infill_webhook::{enqueue_flux_dev_juggernaut_infill_webhook, FluxDevJuggernautInfillArgs, FluxDevJuggernautInfillRequest, FluxDevJuggernautInfillNumImages};
...
pub async fn flux_dev_juggernaut_inpaint_image_handler(http_request: HttpRequest, request: Json<FluxDevJuggernautInpaintImageRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<FluxDevJuggernautInpaintImageResponse>, CommonWebError> {
  ...
  let args = FluxDevJuggernautInfillArgs { request: FluxDevJuggernautInfillRequest { prompt: request.prompt.as_deref().unwrap_or("").to_string(), image_url, mask_url: mask_image_url, num_images }, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

  let fal_result = enqueue_flux_dev_juggernaut_infill_webhook(args).await.map_err(|err| {
  ...
  let db_result = insert_generic_inference_job_for_fal_queue(InsertGenericInferenceForFalArgs { uuid_idempotency_token: &request.uuid_idempotency_token, maybe_external_third_party_id: &external_job_id, fal_category: FalCategory::ImageGeneration, maybe_model_type: Some(CommonModelType::FluxDevJuggernaut), maybe_inference_args: None, maybe_prompt_token: prompt_token.as_ref(), maybe_creator_user_token: maybe_user_session.as_ref().map(|s| &s.user_token), maybe_avt_token: maybe_avt_token.as_ref(), creator_ip_address: &ip_address, creator_set_visibility: Visibility::Public, maybe_platform_type: get_request_platform_type(&http_request), mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(FluxDevJuggernautInpaintImageResponse { success: true, inference_job_token: job_token }))
}
