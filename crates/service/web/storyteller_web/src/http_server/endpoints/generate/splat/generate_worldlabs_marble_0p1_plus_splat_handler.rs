...
pub async fn generate_worldlabs_marble_0p1_plus_splat_handler(http_request: HttpRequest, request: Json<GenerateWorldlabsMarble0p1PlusSplatRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GenerateWorldlabsMarble0p1PlusSplatResponse>, CommonWebError> {
  ...
  let generate_result = match generate_world(GenerateWorldArgs { creds: &creds, world_prompt, display_name: None, model: WorldLabsModel::Marble0p1Plus, seed: None, tags: None, permission: None, request_timeout: None }).await {
    Ok(result) => result,
    Err(err) => {
  ...
  let db_result = insert_generic_inference_job_for_worldlabs_queue_with_apriori_job_token(InsertGenericInferenceForWorldlabsWithAprioriJobTokenArgs { apriori_job_token: &apriori_job_token, uuid_idempotency_token: &request.uuid_idempotency_token, maybe_external_third_party_id: generate_result.operation_id.as_str(), maybe_model_type: Some(CommonModelType::Marble0p1Plus), maybe_inference_args: None, maybe_prompt_token: prompt_token.as_ref(), maybe_wallet_ledger_entry_token: Some(&wallet_deduction.ledger_entry_token), maybe_creator_user_token: Some(user_token), maybe_avt_token: maybe_avt_token.as_ref(), creator_ip_address: &ip_address, creator_set_visibility: Visibility::Public, maybe_platform_type: get_request_platform_type(&http_request), maybe_cost_estimates: None, maybe_debug_log_event_token: None, mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(GenerateWorldlabsMarble0p1PlusSplatResponse { success: true, inference_job_token: job_token }))
}
