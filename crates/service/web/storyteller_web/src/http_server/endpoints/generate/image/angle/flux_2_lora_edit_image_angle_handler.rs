...
pub async fn flux_2_lora_edit_image_angle_handler(http_request: HttpRequest, request: Json<Flux2LoraEditImageAngleRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<Flux2LoraEditImageAngleResponse>, CommonWebError> {
  ...
  let fal_request = EnqueueFlux2LoraEditImageAngleRequest { image_urls, horizontal_angle: request.horizontal_angle, vertical_angle: request.vertical_angle, zoom: request.zoom, num_images, image_size, lora_scale: None, guidance_scale: None, num_inference_steps: None };

  let cost = fal_request.calculate_cost_in_cents();

  let args = EnqueueFlux2LoraEditImageAngleArgs { request: fal_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };
  ...
  Ok(Json(Flux2LoraEditImageAngleResponse { success: true, inference_job_token: job_token }))
}
