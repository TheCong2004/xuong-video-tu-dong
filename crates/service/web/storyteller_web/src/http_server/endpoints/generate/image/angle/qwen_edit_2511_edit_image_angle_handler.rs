...
pub async fn qwen_edit_2511_edit_image_angle_handler(http_request: HttpRequest, request: Json<QwenEdit2511EditImageAngleRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<QwenEdit2511EditImageAngleResponse>, CommonWebError> {
  ...
  let image_urls = lookup_image_urls_as_optional_list(&http_request, &mut mysql_connection, server_state.server_environment, &media_tokens).await?.ok_or_else(|| {
  ...
  let fal_request = EnqueueQwenEdit2511EditImageAngleRequest { image_urls, horizontal_angle: request.horizontal_angle, vertical_angle: request.vertical_angle, zoom: request.zoom, additional_prompt: request.additional_prompt.clone(), num_images, image_size, lora_scale: None, guidance_scale: None, num_inference_steps: None };

  let cost = fal_request.calculate_cost_in_cents();

  let args = EnqueueQwenEdit2511EditImageAngleArgs { request: fal_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

  info!("Charging wallet: {}", cost);

  let wallet_deduction = attempt_wallet_deduction_else_common_web_error(user_token, Some(apriori_job_token.as_str()), cost, &mut mysql_connection).await?;
  ...
  if let Some(token) = prompt_token.as_ref() {
    let result = insert_batch_prompt_context_items(InsertBatchArgs { prompt_token: token.clone(), items: media_tokens.iter().map(|token| PromptContextItem { media_token: token.clone(), context_semantic_type: PromptContextSemanticType::Imgsrc }).collect(), transaction: &mut transaction }).await;
  ...
  Ok(Json(QwenEdit2511EditImageAngleResponse { success: true, inference_job_token: job_token }))
}
