...
use fal_client::requests_old::webhook::video::text::enqueue_sora_2_text_to_video_webhook::{enqueue_sora_2_text_to_video_webhook, EnqueueSora2TextToVideoArgs, EnqueueSora2TextToVideoRequest, EnqueueSora2TextToVideoAspectRatio, EnqueueSora2TextToVideoDurationSeconds, EnqueueSora2TextToVideoResolution};
...
pub async fn sora_2_multi_function_video_gen_handler(http_request: HttpRequest, request: Json<Sora2MultiFunctionVideoGenRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<Sora2MultiFunctionVideoGenResponse>, CommonWebError> {
  ...
  if let Some(image_url) = maybe_image_url {
    ...
    let i2v_request = EnqueueSora2ImageToVideoRequest { image_url, prompt: request.prompt.as_deref().unwrap_or("").to_string(), duration: Some(duration), aspect_ratio: Some(aspect_ratio), resolution: Some(resolution) };
    ...
    let args = EnqueueSora2ImageToVideoArgs { request: i2v_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

    fal_result = enqueue_sora_2_image_to_video_webhook(args).await.map_err(|err| {
    ...
    let aspect_ratio = match request.aspect_ratio {
      Some(Sora2MultiFunctionVideoGenAspectRatio::Auto) => EnqueueSora2TextToVideoAspectRatio::SixteenByNine,
    ...
    let resolution = match request.resolution {
      Some(Sora2MultiFunctionVideoGenResolution::Auto) => EnqueueSora2TextToVideoResolution::SevenTwentyP,
    ...
    let t2v_request = EnqueueSora2TextToVideoRequest { prompt: request.prompt.as_deref().unwrap_or("").to_string(), duration: Some(duration), aspect_ratio: Some(aspect_ratio), resolution: Some(resolution) };
    ...
    let args = EnqueueSora2TextToVideoArgs { request: t2v_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

    fal_result = enqueue_sora_2_text_to_video_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_model_type: Some(CommonModelType::Sora2),
  ...
  Ok(Json(Sora2MultiFunctionVideoGenResponse { success: true, inference_job_token: job_token }))
}
