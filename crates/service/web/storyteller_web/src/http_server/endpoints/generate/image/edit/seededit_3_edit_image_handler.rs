...
use fal_client::requests_old::webhook::image::edit::enqueue_seededit_v3_edit_webhook::{enqueue_seededit_v3_edit_webhook, SeedEditV3EditArgs, SeedEditV3EditRequest};
...
pub async fn seededit_3_edit_image_handler(http_request: HttpRequest, request: Json<SeedEdit3EditImageRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<SeedEdit3EditImageResponse>, CommonWebError> {
  ...
  let args = SeedEditV3EditArgs { request: SeedEditV3EditRequest { prompt: request.prompt.as_deref().unwrap_or("").to_string(), image_url: image_url.to_string() }, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

  let fal_result = enqueue_seededit_v3_edit_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_model_type: Some(CommonModelType::SeedEdit3),
  ...
  Ok(Json(SeedEdit3EditImageResponse { success: true, inference_job_token: job_token }))
}
