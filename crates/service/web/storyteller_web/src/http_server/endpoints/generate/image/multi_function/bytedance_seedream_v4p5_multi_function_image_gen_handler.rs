...
pub async fn bytedance_seedream_v4p5_multi_function_image_gen_handler(http_request: HttpRequest, request: Json<BytedanceSeedreamV4p5MultiFunctionImageGenRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<BytedanceSeedreamV4p5MultiFunctionImageGenResponse>, CommonWebError> {
  ...
  if let Some(input_image_urls) = image_urls.as_deref() {
    ...
    let edit_request = EnqueueBytedanceSeedreamV4p5EditImageRequest { prompt: request.prompt.clone().unwrap_or_default(), image_urls: input_image_urls.to_owned(), num_images: Some(num_images), image_size: Some(image_size), max_images: None };

    let cost = edit_request.calculate_cost_in_cents();

    let args = EnqueueBytedanceSeedreamV4p5EditImageArgs { request: edit_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };
    ...
    fal_result = enqueue_bytedance_seedream_v4p5_edit_image_webhook(args).await.map_err(|err| {
    ...
    let t2i_request = EnqueueBytedanceSeedreamV4p5TextToImageRequest { prompt: request.prompt.clone().unwrap_or_default(), num_images: Some(num_images), image_size: Some(image_size), max_images: None };
    ...
    let args = EnqueueBytedanceSeedreamV4p5TextToImageArgs { request: t2i_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

    fal_result = enqueue_bytedance_seedream_v4p5_text_to_image_webhook(args).await.map_err(|err| {
  ...
  let prompt_result = insert_prompt(InsertPromptArgs { maybe_bitrate: None, maybe_apriori_prompt_token: None, prompt_type: PromptType::ArtcraftApp, maybe_creator_user_token: Some(&user_token), maybe_model_type: Some(CommonModelType::Seedream4p5), maybe_generation_provider: Some(GenerationProvider::Artcraft), maybe_positive_prompt: request.prompt.as_deref(), maybe_negative_prompt: None, maybe_other_args: None, maybe_generation_mode: Some(generation_mode), maybe_aspect_ratio, maybe_resolution: None, maybe_batch_count, maybe_generate_audio: None, maybe_duration_seconds: None, creator_ip_address: &ip_address, mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(BytedanceSeedreamV4p5MultiFunctionImageGenResponse { success: true, inference_job_token: job_token }))
}
