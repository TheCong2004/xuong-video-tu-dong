...
pub async fn bytedance_seedream_v4_multi_function_image_gen_handler(http_request: HttpRequest, request: Json<BytedanceSeedreamV4MultiFunctionImageGenRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<BytedanceSeedreamV4MultiFunctionImageGenResponse>, CommonWebError> {
  ...
  if let Some(input_image_urls) = image_urls.as_deref() {
    ...
    let edit_request = EnqueueBytedanceSeedreamV4EditImageRequest { prompt: request.prompt.clone().unwrap_or_default(), image_urls: input_image_urls.to_owned(), num_images: Some(num_images), image_size: Some(image_size), max_images: None };

    let cost = edit_request.calculate_cost_in_cents();

    let args = EnqueueBytedanceSeedreamV4EditImageArgs { request: edit_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };
    ...
    fal_result = enqueue_bytedance_seedream_v4_edit_image_webhook(args).await.map_err(|err| {
    ...
    let t2i_request = EnqueueBytedanceSeedreamV4TextToImageRequest { prompt: request.prompt.clone().unwrap_or_default(), num_images: Some(num_images), image_size: Some(image_size), max_images: None };
    ...
    let args = EnqueueBytedanceSeedreamV4TextToImageArgs { request: t2i_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key };

    fal_result = enqueue_bytedance_seedream_v4_text_to_image_webhook(args).await.map_err(|err| {
  ...
  let maybe_aspect_ratio = match request.image_size {
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::Square) => Some(CommonAspectRatio::Square),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::SquareHd) => Some(CommonAspectRatio::SquareHd),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::PortraitFourThree) => Some(CommonAspectRatio::TallThreeByFour),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::PortraitSixteenNine) => Some(CommonAspectRatio::TallNineBySixteen),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::LandscapeFourThree) => Some(CommonAspectRatio::WideFourByThree),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::LandscapeSixteenNine) => Some(CommonAspectRatio::WideSixteenByNine),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::Auto) => Some(CommonAspectRatio::Auto),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::Auto2k) => Some(CommonAspectRatio::Auto2k),
    Some(BytedanceSeedreamV4MultiFunctionImageGenImageSize::Auto4k) => Some(CommonAspectRatio::Auto4k),
    None => None,
  };

  let maybe_batch_count: Option<u8> = match request.num_images {
    Some(BytedanceSeedreamV4MultiFunctionImageGenNumImages::One) => Some(1),
    Some(BytedanceSeedreamV4MultiFunctionImageGenNumImages::Two) => Some(2),
    Some(BytedanceSeedreamV4MultiFunctionImageGenNumImages::Three) => Some(3),
    Some(BytedanceSeedreamV4MultiFunctionImageGenNumImages::Four) => Some(4),
    None => None,
  };

  let prompt_result = insert_prompt(InsertPromptArgs { maybe_bitrate: None, maybe_apriori_prompt_token: None, prompt_type: PromptType::ArtcraftApp, maybe_creator_user_token: Some(&user_token), maybe_model_type: Some(CommonModelType::Seedream4), maybe_generation_provider: Some(GenerationProvider::Artcraft), maybe_positive_prompt: request.prompt.as_deref(), maybe_negative_prompt: None, maybe_other_args: None, maybe_generation_mode: Some(generation_mode), maybe_aspect_ratio, maybe_resolution: None, maybe_batch_count, maybe_generate_audio: None, maybe_duration_seconds: None, creator_ip_address: &ip_address, mysql_executor: &mut *transaction, phantom: Default::default() }).await;
  ...
  Ok(Json(BytedanceSeedreamV4MultiFunctionImageGenResponse { success: true, inference_job_token: job_token }))
}
