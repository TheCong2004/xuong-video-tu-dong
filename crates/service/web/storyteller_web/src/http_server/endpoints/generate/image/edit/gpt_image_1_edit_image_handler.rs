...
pub async fn gpt_image_1_edit_image_handler(http_request: HttpRequest, request: Json<GptImage1EditImageRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GptImage1EditImageResponse>, CommonWebError> {
  ...
  let result = batch_get_media_files_by_tokens_with_connection(&mut mysql_connection, tokens, CAN_SEE_DELETED).await;
  ...
  if media_files.len() != tokens.len() {
    ...
    return Err(CommonWebError::BadInputWithSimpleMessage(format!("Not all media files could be found. Media files found: {}, tokens provided: {}", media_files.len(), tokens.len())));
  }

  let media_domain = get_media_domain(&http_request);

  let image_urls = media_files
    .iter()
    .map(|file| {
  ...
  let args = GptEditImageByokArgs { request: GptEditImageByokRequest { image_urls, prompt: request.prompt.as_deref().unwrap_or("").to_string(), num_images, image_size, quality }, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key, openai_api_key: &openai_api_key };
  ...
  let fal_result = enqueue_gpt_image_1_byok_edit_image_webhook(args).await.map_err(|err| {
  ...
  let maybe_aspect_ratio = match request.image_size {
    Some(GptImage1EditImageImageSize::Square) => Some(CommonAspectRatio::Square),
    Some(GptImage1EditImageImageSize::Horizontal) => Some(CommonAspectRatio::WideSixteenByNine),
    Some(GptImage1EditImageImageSize::Vertical) => Some(CommonAspectRatio::TallNineBySixteen),
    None => None,
  };

  let maybe_batch_count: Option<u8> = match request.num_images {
    Some(GptImage1EditImageNumImages::One) => Some(1),
    Some(GptImage1EditImageNumImages::Two) => Some(2),
    Some(GptImage1EditImageNumImages::Three) => Some(3),
    Some(GptImage1EditImageNumImages::Four) => Some(4),
    None => None,
  };
  ...
  if let Some(token) = prompt_token.as_ref() {
    let result = insert_batch_prompt_context_items(InsertBatchArgs { prompt_token: token.clone(), items: media_files.iter().map(|media_file| PromptContextItem { media_token: media_file.token.clone(), context_semantic_type: PromptContextSemanticType::Imgref }).collect(), transaction: &mut transaction }).await;
  ...
  Ok(Json(GptImage1EditImageResponse { success: true, inference_job_token: job_token }))
}
