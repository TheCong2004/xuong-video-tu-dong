...
pub async fn generate_gpt_image_1_text_to_image_handler(http_request: HttpRequest, request: Json<GenerateGptImage1TextToImageRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GenerateGptImage1TextToImageResponse>, CommonWebError> {
  ...
  let byok_request = GptTextToImageByokRequest { prompt: request.prompt.as_deref().unwrap_or("").to_string(), num_images, image_size, quality };

  let apriori_job_token = InferenceJobToken::generate();

  let cost = byok_request.calculate_cost_in_cents();

  let args = GptTextToImageByokArgs { request: byok_request, webhook_url: &server_state.inference_providers.fal.webhook_url, api_key: &server_state.inference_providers.fal.api_key, openai_api_key: &openai_api_key };
  ...
  let fal_result = enqueue_gpt_image_1_byok_text_to_image_webhook(args).await.map_err(|err| {
  ...
  Ok(Json(GenerateGptImage1TextToImageResponse { success: true, inference_job_token: job_token }))
}
