...
pub async fn dev_input_error_handler() -> Result<HttpResponse, CommonWebError> {
  let body = BadInputError { fields: vec![BadFieldMessage { field_name: "username".to_string(), message: "username is taken".to_string(), max_length: Some(32), min_length: Some(3) }, BadFieldMessage { field_name: "email".to_string(), message: "email is invalid".to_string(), max_length: None, min_length: None }, BadFieldMessage { field_name: "password".to_string(), message: "password is too short".to_string(), max_length: Some(128), min_length: Some(8) }] };
