...
pub async fn log_web_referral_handler(http_request: HttpRequest, request: web::Json<LogWebReferralRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  // 1. Try to get the referral URL from the request body, falling back to the Referer header.
  let referral_url = request.maybe_referral_url.as_deref().map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()).or_else(|| http_request.headers().get("referer").and_then(|v| v.to_str().ok()).map(|s| s.to_string())).filter(|s| !s.is_empty());
  ...
  let maybe_domain = parsed.as_ref().and_then(|u| u.host_str()).map(|s| s.to_string());
  ...
  let existing_avt_token = server_state.avt_cookie_manager.get_avt_token_from_request(&http_request);

  let (avt_token, maybe_new_avt_cookie) = match existing_avt_token {
    Some(token) => (token, None),
    None => {
      let token = AnonymousVisitorTrackingToken::generate();
      let cookie = server_state.avt_cookie_manager.make_new_cookie_with_apriori_token(&token).map_err(|e| {
  ...
  insert_web_referral(&server_state.mysql_pool, InsertWebReferralArgs { url: &referral_url, maybe_domain: maybe_domain.as_deref(), utm_source: utm_source.as_deref(), utm_medium: utm_medium.as_deref(), utm_campaign: utm_campaign.as_deref(), ip_address: &ip_address, maybe_anonymous_visitor_token: Some(&avt_token_str) }).await.map_err(|e| {
  ...
  let response = LogWebReferralResponse { success: true };
...
fn extract_query_param(parsed: Option<&Url>, key: &str) -> Option<String> {
  parsed.and_then(|u| u.query_pairs().find(|(k, _)| k == key).map(|(_, v)| v.to_string()))
}
