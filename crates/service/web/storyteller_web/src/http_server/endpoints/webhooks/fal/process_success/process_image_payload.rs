...
pub async fn process_image_payload(image_data: &ImageData, job: &FalJobDetails, server_state: &ServerState) -> Result<MediaFileToken, CommonWebError> {
  let image_url = image_data.url.as_deref().ok_or_else(|| {
  ...
  let metadata = resolve_file_metadata(&file_bytes, image_data.content_type.as_deref()).ok_or_else(|| {
    warn!("Could not determine file type for image (bytes: {}, fal content_type: {:?})", file_bytes.len(), image_data.content_type,);
    CommonWebError::server_error_with_message(&format!("Could not determine file type for image (bytes: {}, fal content_type: {:?})", file_bytes.len(), image_data.content_type))
  })?;
  ...
  match metadata.file_extension {
    FileExtension::Webp => {
      ...
      upload_single_image_bytes(job, server_state, &converted, &converted_metadata.mime_type, converted_metadata.file_extension).await
    },
    _ => upload_single_image_bytes(job, server_state, &file_bytes, &metadata.mime_type, metadata.file_extension).await,
  }
}

async fn upload_single_image_bytes(job: &FalJobDetails, server_state: &ServerState, file_bytes: &[u8], mime_type: &str, file_extension: FileExtension) -> Result<MediaFileToken, CommonWebError> {
