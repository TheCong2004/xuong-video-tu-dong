...
/// up from any member.
pub async fn process_result_files_payload(result_files: &[ResultFileData], job: &FalJobDetails, server_state: &ServerState, pager: &Pager) -> Result<(Option<MediaFileToken>, Option<BatchGenerationToken>), CommonWebError> {
  ...
  for (i, file) in result_files.iter().enumerate() {
    ...
    let result = upload_mesh_file(UploadMeshFileArgs { mesh_url: url, maybe_content_type: file.content_type.as_deref(), maybe_file_name: file.file_name.as_deref(), maybe_batch_token: maybe_batch_token.as_ref(), job, server_state }).await;
  ...
  if let Some(err) = maybe_error {
    ...
    let notification = NotificationDetailsBuilder::from_boxed_error(err.into())
      .set_title(format!("Failure to download all result files from FAL webhook: {} out of {} succeeded", success_count, result_files.len(),))
