...
pub(crate) async fn attach_cover_image(args: AttachCoverImageArgs<'_>) -> Result<(), CommonWebError> {
  let AttachCoverImageArgs { image_url, maybe_content_type, maybe_origin_product_category, target_media_token, job, server_state } = args;

  // Download with a retry if the first attempt returns suspiciously few bytes.
  let mut file_bytes = http_download_url_to_bytes(image_url).await.map_err(|err| {
    warn!("Failed to download cover image from {}: {:?}", image_url, err);
    CommonWebError::server_error_with_message(&format!("Failed to download cover image: {:?}", err))
  })?;

  if file_bytes.len() <= 10 {
    warn!("Downloaded only {} bytes from {} — retrying once", file_bytes.len(), image_url,);
    file_bytes = http_download_url_to_bytes(image_url).await.map_err(|err| {
      warn!("Failed to download cover image on retry from {}: {:?}", image_url, err);
      CommonWebError::server_error_with_message(&format!("Failed to download cover image on retry: {:?}", err))
    })?;
  }

  // Resolve mime type: magic bytes first, fal content_type as fallback.
  let metadata = resolve_file_metadata(&file_bytes, maybe_content_type).ok_or_else(|| {
    warn!("Could not determine file type for cover image (bytes: {}, fal content_type: {:?})", file_bytes.len(), maybe_content_type,);
    CommonWebError::server_error_with_message(&format!("Could not determine file type for cover image (bytes: {}, fal content_type: {:?})", file_bytes.len(), maybe_content_type))
  })?;
  ...
  let cover_media_token = insert_cover_media_file(InsertCoverMediaFileArgs { maybe_creator_user_token: job.maybe_creator_user_token.as_ref(), maybe_creator_anonymous_visitor_token: job.maybe_creator_anonymous_visitor_token.as_ref(), creator_ip_address: &job.creator_ip_address, media_file_type, mime_type, file_size_bytes: file_size_bytes as u64, checksum_sha2: &file_hash, public_bucket_path: &public_upload_path, maybe_origin_product_category, maybe_prompt_token: job.maybe_prompt_token.as_ref(), maybe_platform_type: job.maybe_platform_type, mysql_executor: &server_state.mysql_pool, phantom: Default::default() }).await.map_err(|err| {
    warn!("Failed to insert cover image media file record: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  let query_result = set_media_file_cover_image(UpdateArgs { media_file_token: target_media_token, maybe_cover_image_media_file_token: Some(&cover_media_token), mysql_pool: &server_state.mysql_pool }).await;
