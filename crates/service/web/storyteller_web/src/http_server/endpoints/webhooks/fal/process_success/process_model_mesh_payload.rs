...
pub async fn process_model_mesh_payload(model_mesh_data: &ModelMeshData, maybe_preprocessed_image: Option<&PreprocessedImageData>, job: &FalJobDetails, server_state: &ServerState) -> Result<MediaFileToken, CommonWebError> {
  let mesh_url = model_mesh_data.url.as_deref().ok_or_else(|| {
    warn!("No `url` in model mesh payload");
    CommonWebError::server_error_with_message("no `url` in model mesh payload")
  })?;

  let media_token = upload_mesh_file(UploadMeshFileArgs { mesh_url, maybe_content_type: model_mesh_data.content_type.as_deref(), maybe_file_name: model_mesh_data.file_name.as_deref(), maybe_batch_token: None, job, server_state }).await?;
  ...
  if let Some(preprocessed_image) = maybe_preprocessed_image {
    let result = try_to_attach_preprocessed_image(preprocessed_image, job, server_state, &media_token).await;
...
pub async fn process_model_obj_payload(model_obj_data: &ModelObjData, job: &FalJobDetails, server_state: &ServerState) -> Result<MediaFileToken, CommonWebError> {
  let mesh_url = model_obj_data.url.as_deref().ok_or_else(|| {
    warn!("No `url` in model obj payload");
    CommonWebError::server_error_with_message("no `url` in model obj payload")
  })?;

  let media_token = upload_mesh_file(UploadMeshFileArgs { mesh_url, maybe_content_type: model_obj_data.content_type.as_deref(), maybe_file_name: model_obj_data.file_name.as_deref(), maybe_batch_token: None, job, server_state }).await?;
...
pub(crate) async fn upload_mesh_file(args: UploadMeshFileArgs<'_>) -> Result<MediaFileToken, CommonWebError> {
  let UploadMeshFileArgs { mesh_url, maybe_content_type, maybe_file_name, maybe_batch_token, job, server_state } = args;

  // Download with a retry if the first attempt returns suspiciously few bytes.
  let mut file_bytes = http_download_url_to_bytes(mesh_url).await.map_err(|err| {
    warn!("Failed to download mesh from {}: {:?}", mesh_url, err);
    CommonWebError::from_error(err)
  })?;

  if file_bytes.len() <= 10 {
    warn!("Downloaded only {} bytes from {} — retrying once", file_bytes.len(), mesh_url,);
    file_bytes = http_download_url_to_bytes(mesh_url).await.map_err(|err| {
  ...
  let metadata = resolve_file_metadata_with_file_name(&file_bytes, maybe_content_type, maybe_file_name).ok_or_else(|| {
    warn!("Could not determine file type for mesh (bytes: {}, fal content_type: {:?}, file_name: {:?})", file_bytes.len(), maybe_content_type, maybe_file_name,);
    CommonWebError::server_error_with_message(&format!("Could not determine file type for mesh (bytes: {}, fal content_type: {:?})", file_bytes.len(), maybe_content_type))
  })?;
  ...
  let file_hash = sha256_hash_bytes(&file_bytes).map_err(|err| {
  ...
  server_state.public_bucket_client.upload_file_with_content_type_process(public_upload_path.get_full_object_path_str(), file_bytes.as_ref(), &mime_type).await.map_err(|err| {
    warn!("Failed to upload mesh to bucket: {:?}", err);
    CommonWebError::from_anyhow_error(err)
  })?;

  let mut insert_builder = MediaFileInsertBuilder::new().checksum_sha2(&file_hash).creator_ip_address(&job.creator_ip_address).file_size_bytes(file_size_bytes as u64).maybe_batch_generation_token(maybe_batch_token).maybe_creator_anonymous_visitor(job.maybe_creator_anonymous_visitor_token.as_ref()).maybe_creator_user(job.maybe_creator_user_token.as_ref()).maybe_generation_provider(Some(GenerationProvider::Artcraft)).maybe_prompt_token(job.maybe_prompt_token.as_ref()).maybe_platform_type(job.maybe_platform_type).media_file_origin_category(MediaFileOriginCategory::Inference).media_file_type(media_file_type).mime_type(mime_type).public_bucket_directory_hash(&public_upload_path);

  if is_gaussian_splat {
    insert_builder = insert_builder.media_file_class(MediaFileClass::Splat).media_file_origin_product_category(MediaFileOriginProductCategory::WorldGeneration);
  } else {
    insert_builder = insert_builder.media_file_class(MediaFileClass::Mesh).maybe_engine_category(Some(MediaFileEngineCategory::Object));
  }

  let media_token = insert_builder.insert_pool(&server_state.mysql_pool).await.map_err(|err| {
...
async fn try_to_attach_preprocessed_image(preprocessed_image: &PreprocessedImageData, job: &FalJobDetails, server_state: &ServerState, splat_media_token: &MediaFileToken) -> Result<(), CommonWebError> {
  info!("Fal Preprocessed Image Data: {:?}", preprocessed_image);

  let image_url = preprocessed_image.url.as_deref().ok_or_else(|| {
    warn!("No `url` in preprocessed image payload");
    CommonWebError::server_error_with_message("no `url` in preprocessed image payload")
  })?;

  attach_cover_image(AttachCoverImageArgs { image_url, maybe_content_type: preprocessed_image.content_type.as_deref(), maybe_origin_product_category: Some(MediaFileOriginProductCategory::WorldGeneration), target_media_token: splat_media_token, job, server_state }).await
}
