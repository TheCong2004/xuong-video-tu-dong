...
use beeble_client::webhook_api::beeble_webhook_payload::{parse_beeble_webhook, BeebleWebhookPayload, BeebleWebhookStatus};
...
use mysql_queries::queries::generic_inference::api_providers::beeble::get_inference_job_by_beeble_id::{get_inference_job_by_beeble_id_from_connection, BeebleJobDetails};
...
pub async fn beeble_webhook_handler(http_request: HttpRequest, request_body_bytes: Bytes, server_state: web::Data<Arc<ServerState>>) -> Result<Json<SimpleGenericJsonSuccess>, CommonWebError> {
  // Step 1: Parse bytes into a UTF-8 string and log it.
  let raw_body = String::from_utf8(request_body_bytes.to_vec()).map_err(|err| {
  ...
  let payload = parse_beeble_webhook(&raw_body).map_err(|err| {
  ...
  let result = match payload.status {
    BeebleWebhookStatus::Completed => handle_completed(&server_state, &mut mysql_connection, job_id, &payload, &job).await,
    BeebleWebhookStatus::Failed => handle_failed(&mut mysql_connection, job_id, &payload, &job).await,
    BeebleWebhookStatus::Unknown => {
      warn!("Beeble webhook received unknown status for job_id {}", job_id);
      Err(CommonWebError::server_error_with_message(&format!("Beeble webhook received unknown status for job_id {}", job_id)))
    },
  };
  ...
  if let Err(ref err) = result {
    if err.is_server_error() {
      ...
      let description = format!("Beeble webhook failed for job_id: {}\n\nError: {:?}\n\nWebhook JSON Payload: {}", job_id, err, raw_body,);

      let mut builder = if let Some(cause_arc) = err.clone_cause_arc() { NotificationDetailsBuilder::from_error(cause_arc).set_title(format!("Beeble webhook processing failed (job_id: {})", job_id)) } else { NotificationDetailsBuilder::from_title(format!("Beeble webhook processing failed (job_id: {})", job_id)) };

      builder = builder.set_description(Some(description)).set_third_party_id(Some(job_id.to_string())).set_urgency(Some(NotificationUrgency::High)).set_http_method(Some(http_request.method().to_string())).set_http_path(Some(http_request.path().to_string()));
...
async fn handle_completed(server_state: &ServerState, mysql_connection: &mut PoolConnection<MySql>, job_id: &str, payload: &BeebleWebhookPayload, job: &BeebleJobDetails) -> Result<Json<SimpleGenericJsonSuccess>, CommonWebError> {
  ...
  mark_fal_generic_inference_job_successfully_done(MarkJobArgs { job_token: &job.job_token, media_file_token: &render_media_token, maybe_batch_token: None, mysql_executor: &mut **mysql_connection, phantom: Default::default() }).await.map_err(|err| {
...
async fn handle_failed(mysql_connection: &mut PoolConnection<MySql>, job_id: &str, payload: &BeebleWebhookPayload, job: &BeebleJobDetails) -> Result<Json<SimpleGenericJsonSuccess>, CommonWebError> {
  let public_failure_reason = payload.error.as_deref().unwrap_or("Unknown Beeble error").to_string();

  let internal_failure_reason = format!("Beeble job_id={}, error={:?}", job_id, payload.error,);

  info!("Marking job {} as failed for Beeble job_id {}. Reason: {}", job.job_token.as_str(), job_id, public_failure_reason,);

  if let Err(err) = mark_job_failed_by_token_from_connection(MarkJobFailedByTokenFromConnectionArgs { mysql_connection, job_token: &job.job_token, maybe_public_failure_reason: Some(&public_failure_reason), internal_debugging_failure_reason: &internal_failure_reason, maybe_frontend_failure_category: Some(FrontendFailureCategory::GenerationFailed) }).await {
    error!("Error marking job {} as failed for Beeble job_id {}: {:?}", job.job_token.as_str(), job_id, err,);
    return Err(CommonWebError::from_anyhow_error(err));
  }

  info!("Job {} marked as failed for Beeble job_id {}.", job.job_token.as_str(), job_id,);

  Ok(SimpleGenericJsonSuccess::wrapped(true))
}

async fn download_and_upload_video(video_url: &str, job: &BeebleJobDetails, server_state: &ServerState) -> Result<MediaFileToken, CommonWebError> {
  ...
  let metadata = resolve_file_metadata(&file_bytes, None).ok_or_else(|| {
    warn!("Could not determine file type for Beeble video (bytes: {})", file_bytes.len(),);
    CommonWebError::server_error_with_message(&format!("Could not determine file type for Beeble video (bytes: {})", file_bytes.len()))
  })?;
  ...
  if let Ok(mut file) = NamedTempFile::new() {
    ...
    if let Ok(video_info) = ffprobe_get_info(&file.path()) {
      ...
      maybe_frame_width = video_info.dimensions.as_ref().map(|dims| dims.width as u32);
...
fn enqueue_parse_error_alert<E: std::fmt::Debug>(server_state: &ServerState, http_request: &HttpRequest, context: &str, err: &E, maybe_raw_body: Option<&str>) {
  ...
  let notification = NotificationDetailsBuilder::from_title(format!("Beeble webhook parse failure: {}", context)).set_description(Some(description)).set_urgency(Some(NotificationUrgency::High)).set_http_method(Some(http_request.method().to_string())).set_http_path(Some(http_request.path().to_string())).build();
