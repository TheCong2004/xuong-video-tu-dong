...
/// record.
pub async fn process_audio_payload(audio_data: &AudioData, job: &FalJobDetails, server_state: &ServerState) -> Result<MediaFileToken, CommonWebError> {
  let audio_url = audio_data.url.as_deref().ok_or_else(|| {
  ...
  let mut file_bytes = http_download_url_to_bytes(audio_url).await.map_err(|err| {
    warn!("Failed to download audio from {}: {:?}", audio_url, err);
    CommonWebError::from_error(err)
  })?;

  if file_bytes.len() <= 10 {
    warn!("Downloaded only {} bytes from {} — retrying once", file_bytes.len(), audio_url,);
    file_bytes = http_download_url_to_bytes(audio_url).await.map_err(|err| {
  ...
  let metadata = resolve_file_metadata(&file_bytes, audio_data.content_type.as_deref()).ok_or_else(|| {
    warn!("Could not determine file type for audio (bytes: {}, fal content_type: {:?})", file_bytes.len(), audio_data.content_type,);
    CommonWebError::server_error_with_message(&format!("Could not determine file type for audio (bytes: {}, fal content_type: {:?})", file_bytes.len(), audio_data.content_type))
  })?;
  ...
  let maybe_duration_millis = audio_data.duration.map(|seconds| (seconds * 1000.0) as u64);
  ...
  let media_token = MediaFileInsertBuilder::new().checksum_sha2(&file_hash).creator_ip_address(&job.creator_ip_address).file_size_bytes(file_size_bytes as u64).maybe_creator_anonymous_visitor(job.maybe_creator_anonymous_visitor_token.as_ref()).maybe_creator_user(job.maybe_creator_user_token.as_ref()).maybe_duration_millis(maybe_duration_millis).maybe_generation_provider(Some(GenerationProvider::Artcraft)).maybe_prompt_token(job.maybe_prompt_token.as_ref()).maybe_platform_type(job.maybe_platform_type).media_file_class(MediaFileClass::Audio).media_file_origin_category(MediaFileOriginCategory::Inference).media_file_type(media_file_type).mime_type(mime_type).public_bucket_directory_hash(&public_upload_path).insert_pool(&server_state.mysql_pool).await.map_err(|err| {
