...
pub async fn get_character_handler(http_request: HttpRequest, path: Path<GetCharacterPathInfo>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GetCharacterResponse>, CommonWebError> {
  let mut mysql_connection = server_state.mysql_pool.acquire().await?;

  let character = get_character_by_token(&path.character_token, &mut mysql_connection).await?.ok_or_else(|| {
  ...
  let media_records = if media_tokens_to_lookup.is_empty() {
    ...
    batch_get_media_files_by_tokens_with_connection(&mut mysql_connection, &media_tokens_to_lookup, true).await.unwrap_or_else(|err| {
  ...
  Ok(Json(GetCharacterResponse { success: true, character: GetCharacterDetails { token: character.token, models: vec![CommonModelType::Seedance2p0], name: character.character_name.unwrap_or_default(), maybe_description: character.maybe_description, maybe_avatar, maybe_full_image } }))
}
