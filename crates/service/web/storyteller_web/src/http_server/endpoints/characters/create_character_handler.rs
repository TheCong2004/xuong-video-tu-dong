...
use mysql_queries::queries::generic_inference::api_providers::seedance2pro::insert_generic_inference_job_for_seedance2pro_character_with_apriori_job_token::{insert_generic_inference_job_for_seedance2pro_character_with_apriori_job_token, InsertGenericInferenceForSeedance2ProCharacterWithAprioriJobTokenArgs};
...
pub async fn create_character_handler(http_request: HttpRequest, request: Json<CreateCharacterRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<CreateCharacterResponse>, CommonWebError> {
  // --- Auth ---
  ...
  let character_description = request.character_description.as_deref().map(|desc| desc.trim());

  if let Some(desc) = character_description {
    if desc.len() > CHARACTER_MAX_DESCRIPTION_LENGTH {
      return Err(CommonWebError::BadInputWithSimpleMessage(format!("Description exceeds maximum length of {} characters.", CHARACTER_MAX_DESCRIPTION_LENGTH)));
    }
  }

  // --- Look up the image media file ---

  let file_urls_by_token = lookup_media_file_urls_as_map(&http_request, &mut mysql_connection, server_state.server_environment, &[request.image_media_token.clone()]).await?;

  let image_cdn_url = file_urls_by_token.get(&request.image_media_token).ok_or_else(|| {
  ...
  let seedance_image_url = upload_to_kinovi(&server_state.inference_providers.seedance2pro.cookies_volcengine, image_cdn_url).await?;
  ...
  let gen_result = generate_character(GenerateCharacterArgs { session: &session, name: character_name.clone(), description: character_description.map(|d| d.to_string()).unwrap_or_else(|| "".to_string()), reference_image_url: seedance_image_url, is_public: false, host_override: None }).await.map_err(|err| {
    error!("Error calling Kinovi generate_character: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  info!("Kinovi character created: id={}, character_id={}, name={}", gen_result.id, gen_result.character_id, gen_result.name,);
  ...
  let character_token = create_pending_character(CreatePendingCharacterArgs { character_type: CharacterType::KinoviSeedance, character_name: &character_name, maybe_description: character_description, maybe_original_upload_media_token: Some(&request.image_media_token), maybe_creator_user_token: Some(user_token), creator_ip_address: &ip_address, kinovi_character_id: &gen_result.character_id, kinovi_character_name: &gen_result.name, maybe_generic_inference_job_token: Some(&apriori_job_token), mysql_executor: &mut *transaction, phantom: PhantomData }).await?;

  info!("Created pending character: {}", character_token);

  insert_generic_inference_job_for_seedance2pro_character_with_apriori_job_token(InsertGenericInferenceForSeedance2ProCharacterWithAprioriJobTokenArgs { uuid_idempotency_token: &request.uuid_idempotency_token, apriori_job_token: &apriori_job_token, kinovi_character_id: &gen_result.character_id, maybe_model_type: None, maybe_creator_user_token: Some(user_token), maybe_avt_token: user_session.maybe_avt_token.as_ref(), creator_ip_address: &ip_address, creator_set_visibility: Visibility::Hidden, mysql_executor: &mut *transaction, phantom: PhantomData }).await.map_err(|err| {
  ...
  Ok(Json(CreateCharacterResponse { success: true, inference_job_token: apriori_job_token }))
}

// =============== Private helpers ===============

/// Upload an image from our CDN to Kinovi's CDN via prepare + upload.
async fn upload_to_kinovi(cookies: &str, our_cdn_url: &url::Url) -> Result<String, CommonWebError> {
  ...
  let prepare_result = prepare_file_upload(PrepareFileUploadArgs { session: &session, extension, host_override: None }).await.map_err(|err| {
