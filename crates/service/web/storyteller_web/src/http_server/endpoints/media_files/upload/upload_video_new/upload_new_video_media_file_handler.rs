...
pub async fn upload_new_video_media_file_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>, MultipartForm(mut form): MultipartForm<UploadNewVideoMediaFileForm>) -> Result<Json<UploadNewVideoMediaFileSuccessResponse>, MediaFileUploadError> {
  ...
  Ok(Json(UploadNewVideoMediaFileSuccessResponse { success: true, media_file_token: token }))
}
