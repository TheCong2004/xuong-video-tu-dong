...
static ALLOWED_FRAME_EXTENSIONS: Lazy<HashSet<&'static str>> = Lazy::new(|| HashSet::from([".png", ".jpg"]));
...
pub fn extract_frames_from_zip<P: AsRef<Path>>(zip_container_file_bytes: &[u8], frame_temp_dir: P) -> Result<FrameType, ExtractFramesError> {
  ...
  for entry in entries.iter() {
    ...
    let mut file = archive.by_name(&safe_enclosed_name).map_err(|err| {
      error!("Problem reading file from archive: {:?}", err);
      ExtractFramesError::InvalidArchive
    })?;

    let maybe_filesystem_name = entry.safe_enclosed_name.file_name().map(|s| s.to_str()).flatten();
    ...
    std::io::copy(&mut file, &mut output_file).map_err(|err| {
...
fn get_relevant_zip_entries(archive: &mut ZipArchive<BufReader<Cursor<&[u8]>>>) -> Result<Vec<FrameEntryDetail>, ExtractFramesError> {
  ...
  for i in 0..(archive.len()) {
    ...
    let file_is_frame = ALLOWED_FRAME_EXTENSIONS.iter().any(|ext| filename_lowercase.ends_with(ext));
