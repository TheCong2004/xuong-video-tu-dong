...
pub async fn get_media_file_handler(http_request: HttpRequest, path: Path<GetMediaFilePathInfo>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GetMediaFileSuccessResponse>, CommonWebError> {
  ...
  let response = if media_file_token.0.starts_with("TR:") {
    ...
    emulate_media_file_with_legacy_tts_result_lookup(&media_file_token, show_deleted_results, &server_state, media_domain).await?
  } else {
    modern_media_file_lookup(&media_file_token, show_deleted_results, &server_state, media_domain).await?
  };

  Ok(Json(response))
}

async fn modern_media_file_lookup(media_file_token: &MediaFileToken, show_deleted_results: bool, server_state: &ServerState, media_domain: MediaDomain) -> Result<GetMediaFileSuccessResponse, CommonWebError> {
  let is_mod = show_deleted_results;

  let result = get_media_file(media_file_token, show_deleted_results, &server_state.mysql_pool).await;
  ...
  Ok(GetMediaFileSuccessResponse {
    ...
    media_file: MediaFileInfo {
      ...
      public_bucket_url: bucket_url_from_media_path(&public_bucket_path, media_domain, server_state.server_environment).map_err(|err| {
      ...
      used_face_detailer: result.maybe_prompt_args.as_ref().and_then(|args| args.used_face_detailer).unwrap_or(false),
      used_upscaler: result.maybe_prompt_args.as_ref().and_then(|args| args.used_upscaler).unwrap_or(false),
      ...
      maybe_moderator_fields: if is_mod { Some(GetMediaFileModeratorFields { maybe_style_transfer_source_media_file_token: result.maybe_style_transfer_source_media_file_token }) } else { None },
    },
  })
}

async fn emulate_media_file_with_legacy_tts_result_lookup(media_file_token: &MediaFileToken, show_deleted_results: bool, server_state: &ServerState, media_domain: MediaDomain) -> Result<GetMediaFileSuccessResponse, CommonWebError> {
  let result = select_tts_result_by_token(&media_file_token.as_str(), show_deleted_results, &server_state.mysql_pool).await;
  ...
  Ok(GetMediaFileSuccessResponse {
    ...
    media_file: MediaFileInfo {
      ...
      public_bucket_url: bucket_url_from_str_path(&public_bucket_path, media_domain, server_state.server_environment).map_err(|err| {
        warn!("error creating URL: {:?}", err);
        CommonWebError::from_anyhow_error(err)
      })?,
      public_bucket_path,
      ...
      is_featured: false,
      stats: SimpleEntityStats { positive_rating_count: 0, bookmark_count: 0 },
      created_at: result.created_at,
      updated_at: result.updated_at,
