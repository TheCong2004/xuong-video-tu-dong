...
pub async fn upload_audio_media_file_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>, MultipartForm(mut form): MultipartForm<UploadAudioMediaFileForm>) -> Result<Json<UploadAudioMediaFileSuccessResponse>, MediaFileUploadError> {
  ...
  Ok(Json(UploadAudioMediaFileSuccessResponse { success: true, media_file_token: token }))
}
