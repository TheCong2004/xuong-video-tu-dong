...
pub async fn upload_scene_snapshot_media_file_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>, MultipartForm(mut form): MultipartForm<UploadSceneSnapshotMediaFileForm>) -> Result<Json<UploadSceneSnapshotMediaFileSuccessResponse>, MediaFileUploadError> {
  ...
  let maybe_scene_source_media_file_token = form.maybe_scene_source_media_file_token.map(|token| token.clone());
  ...
  Ok(Json(UploadSceneSnapshotMediaFileSuccessResponse { success: true, media_file_token: token }))
}
