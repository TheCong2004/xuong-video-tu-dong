...
pub async fn list_pinned_media_files_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>) -> Result<Json<ListPinnedMediaFilesSuccessResponse>, CommonWebError> {
  ...
  let media_file_tokens = token_list.unwrap_or_else(|| "".to_string()).split(",").into_iter().map(|item| item.trim()).filter(|item| !item.is_empty()).map(|item| MediaFileToken::new_from_str(item)).collect::<Vec<MediaFileToken>>();
  ...
  if !media_file_tokens.is_empty() {
    let query_results = batch_get_media_files_by_tokens(&server_state.mysql_pool, &media_file_tokens, false).await;
    ...
    results: media_files
      ...
      .map(|m| {
        ...
        PinnedMediaFile {
          ...
          maybe_creator: UserDetailsLight::from_optional_db_fields_owned(m.maybe_creator_user_token, m.maybe_creator_username, m.maybe_creator_display_name, m.maybe_creator_email_gravatar_hash),
