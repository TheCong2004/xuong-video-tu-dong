...
pub async fn batch_get_media_files_handler(http_request: HttpRequest, query: Query<BatchGetMediaFilesQueryParams>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<BatchGetMediaFilesSuccessResponse>, CommonWebError> {
  ...
  let tokens = query.tokens.iter().map(|token| MediaFileToken::new_from_str(&token)).collect::<Vec<_>>();
  ...
  let result = batch_get_media_files(&tokens, show_deleted_results, &server_state.mysql_pool).await;
  ...
  let media_files = media_files
    ...
    .map(|result| {
      ...
      let maybe_cover_image_public_bucket_path = match result.maybe_model_cover_image_public_bucket_hash.as_deref() {
        None => None,
        Some(hash) => Some(MediaFileBucketPath::from_object_hash(&hash, result.maybe_model_cover_image_public_bucket_prefix.as_deref(), result.maybe_model_cover_image_public_bucket_extension.as_deref()).get_full_object_path_str().to_string()),
      };
      ...
      BatchMediaFileInfo {
        ...
        cover_image: MediaFileCoverImageDetails::from_optional_db_fields(&result.token, media_domain, server_state.server_environment, result.maybe_file_cover_image_public_bucket_hash.as_deref(), result.maybe_file_cover_image_public_bucket_prefix.as_deref(), result.maybe_file_cover_image_public_bucket_extension.as_deref()),
        maybe_title: result.maybe_title,
        maybe_text_transcript: result.maybe_text_transcript,
        maybe_live_portrait_details: result.extra_media_file_info.as_ref().map(|info| MediaFileLivePortraitDetails::maybe_from_extra_info(&info)).flatten(),
        maybe_style_name: result.maybe_prompt_args.as_ref().and_then(|args| args.style_name.as_ref()).and_then(|style| style.to_style_name()),
        maybe_model_weight_info: match result.maybe_model_weights_token {
          ...
          Some(weight_token) => Some(BatchGetMediaFilesModelInfo {
            ...
            maybe_weight_creator: UserDetailsLight::from_optional_db_fields_owned(result.maybe_model_weight_creator_user_token, result.maybe_model_weight_creator_username, result.maybe_model_weight_creator_display_name, result.maybe_model_weight_creator_gravatar_hash),
          }),
        },
        maybe_creator_user: UserDetailsLight::from_optional_db_fields_owned(result.maybe_creator_user_token, result.maybe_creator_username, result.maybe_creator_display_name, result.maybe_creator_gravatar_hash),
        ...
        stats: SimpleEntityStats { positive_rating_count: result.maybe_ratings_positive_count.unwrap_or(0), bookmark_count: result.maybe_bookmark_count.unwrap_or(0) },
  ...
  Ok(Json(BatchGetMediaFilesSuccessResponse { success: true, media_files }))
}
