...
pub async fn upload_new_editor_2d_project_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>, MultipartForm(form): MultipartForm<NewProjectMultipartForm>) -> Result<Json<UploadNewEditor2dProjectSuccessResponse>, CommonWebError> {
  let media_file_token = save_new_project(SaveNewProjectArgs { http_request: &http_request, server_state: &server_state, config: &EDITOR_2D_PROJECT_CONFIG, form }).await?;

  Ok(Json(UploadNewEditor2dProjectSuccessResponse { success: true, media_file_token }))
}
