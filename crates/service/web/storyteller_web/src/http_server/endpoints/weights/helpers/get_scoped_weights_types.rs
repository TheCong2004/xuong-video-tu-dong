...
pub fn get_scoped_weights_types(maybe_query_param: Option<&str>) -> Option<HashSet<WeightsType>> {
  ...
  let weights_types = weights_types.split(",").map(|ty| WeightsType::from_str(ty)).flatten().collect::<HashSet<_>>();
...
mod test {
  ...
  #[test]
  fn valid_scope() {
    assert_eq!(get_scoped_weights_types(Some("tt2,gpt_so_vits,vall_e")), Some(HashSet::from([WeightsType::GptSoVits, WeightsType::Tacotron2, WeightsType::VallE])))
  }
}
