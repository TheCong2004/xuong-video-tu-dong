...
pub async fn get_weight_handler(http_request: HttpRequest, path: Path<GetWeightPathInfo>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GetWeightResponse>, CommonWebError> {
  ...
  let is_mod = maybe_user_session.as_ref().map(|session| session.can_ban_users).unwrap_or(false);

  let weight_lookup_result = get_weight_by_token(&path.weight_token, is_mod, &server_state.mysql_pool).await;
  ...
  let creator = UserDetailsLight::from_db_fields(&weight.creator_user_token, &weight.creator_username, &weight.creator_display_name, &weight.creator_gravatar_hash);
