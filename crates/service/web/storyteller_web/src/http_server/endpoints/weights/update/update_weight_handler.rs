...
pub async fn update_weight_handler(http_request: HttpRequest, path: Path<UpdateWeightPathInfo>, request: Json<UpdateWeightRequest>, mysql_pool: web::Data<MySqlPool>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<SimpleGenericJsonSuccess>, CommonWebError> {
  ...
  let weight_lookup_result = get_weight_by_token_with_transactor(&ModelWeightToken::new(weight_token.clone()), is_mod, Transactor::for_connection(&mut mysql_connection)).await;
  ...
  let query_result = update_weights(UpdateWeightArgs { weight_token: &ModelWeightToken::new(path.weight_token.clone()), title: weight_title.as_deref(), cover_image, maybe_description_markdown: description_markdown.as_deref(), maybe_description_rendered_html: description_rendered_html.as_deref(), creator_set_visibility: request.visibility.as_ref(), ietf_language_tag: ietf_language_tag.as_deref(), ietf_primary_language_subtag: ietf_primary_language_subtag.as_deref(), transactor: Transactor::for_connection(&mut mysql_connection) }).await;
