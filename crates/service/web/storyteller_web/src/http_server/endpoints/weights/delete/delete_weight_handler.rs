...
use mysql_queries::queries::model_weights::delete_weights::{delete_weights_as_mod, delete_weights_as_user, undelete_weights_as_user};
...
pub async fn delete_weight_handler(http_request: HttpRequest, path: Path<DeleteWeightPathInfo>, request: Json<DeleteWeightRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<SimpleGenericJsonSuccess>, CommonWebError> {
  ...
  let weight_lookup_result = get_weight_by_token(&ModelWeightToken::new(weight_token.clone()), is_mod, &server_state.mysql_pool).await;
  ...
  let is_creator = weight.creator_user_token.to_string() == user_session.user_token.as_str().to_string();
  ...
  let query_result = if request.set_delete {
    match delete_role {
      ...
      DeleteRole::AsUser => delete_weights_as_user(&ModelWeightToken::new_from_str(&path.weight_token), &server_state.mysql_pool).await,
      DeleteRole::AsMod => delete_weights_as_mod(&ModelWeightToken::new_from_str(&path.weight_token), &server_state.mysql_pool).await,
    }
  } else {
    match delete_role {
      DeleteRole::ErrorDoNotDelete => {
      ...
      DeleteRole::AsUser => undelete_weights_as_user(&ModelWeightToken::new_from_str(&path.weight_token), &server_state.mysql_pool).await,
      DeleteRole::AsMod => undelete_weights_as_user(&ModelWeightToken::new_from_str(&path.weight_token), &server_state.mysql_pool).await,
    }
  };
