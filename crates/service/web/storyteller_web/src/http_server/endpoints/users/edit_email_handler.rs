...
pub async fn edit_email_handler(http_request: HttpRequest, request: Json<EditEmailRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<EditEmailResponse>, CommonWebError> {
  ...
  let result = update_email(UpdateEmailArgs { token: &user_session.user_token_typed, email_address: &email_address, email_gravatar_hash: &gravatar_hash, ip_address: &ip_address, transactor: Transactor::for_connection(&mut mysql_connection) }).await;
