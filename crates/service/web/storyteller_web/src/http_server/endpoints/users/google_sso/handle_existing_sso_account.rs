...
pub async fn handle_existing_sso_account(args: ExistingAccountArgs<'_>) -> Result<UserToken, CommonWebError> {
  let user_token = match &args.sso_account.maybe_user_token {
    ...
    None => {
      ...
      return Err(CommonWebError::server_error_with_message("no user token for existing google sign in account"));
    },
  };
  ...
  if should_update_sso_claims(args.sso_account, &args.claims) {
    ...
    let result = update_google_sign_in_account(UpdateGoogleSignInArgs { subject: &args.sso_account.subject, email_address: args.claims_email_address, is_email_verified: args.claims.email_verified(), maybe_locale: args.claims.locale(), maybe_name: args.claims.name(), maybe_given_name: args.claims.given_name(), maybe_family_name: args.claims.family_name(), creator_ip_address: &ip_address, transactor: Transactor::for_connection(args.mysql_connection) }).await;
...
fn should_update_sso_claims(sso_account: &GoogleSignInAccount, claims: &Claims) -> bool {
  sso_account.email_address.as_deref() != claims.email() || sso_account.is_email_verified != claims.email_verified() || sso_account.maybe_locale.as_deref() != claims.locale() || sso_account.maybe_name.as_deref() != claims.name() || sso_account.maybe_given_name.as_deref() != claims.given_name() || sso_account.maybe_family_name.as_deref() != claims.family_name()
}
