...
pub async fn check_claims(request: &GoogleCreateAccountRequest, google_sign_in_cert: &GoogleSignInCert) -> Result<Claims, CommonWebError> {
  let keys = google_sign_in_cert.fetch_key_map(false).await.map_err(|e| {
  ...
  let claims = match decode_and_verify_token_claims(&keys, &request.google_credential, verification_options) {
    ...
    Err(err) => {
      ...
      let claims = decode_and_verify_token_claims(&keys, &request.google_credential, verification_options).map_err(|e| {
...
fn build_options() -> VerificationOptions {
  VerificationOptions { allowed_issuers: Some(HashSet::from(["https://accounts.google.com".to_string(), "accounts.google.com".to_string()])), allowed_audiences: Some(HashSet::from(["788843034237-uqcg8tbgofrcf1to37e1bqphd924jaf6.apps.googleusercontent.com".to_string()])), ..Default::default() }
}
