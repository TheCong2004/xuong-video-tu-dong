...
pub async fn handle_new_sso_account_for_existing_user(args: LinkArgs<'_>) -> Result<NewSsoAccountInfo, CommonWebError> {
  ...
  let _token = insert_google_sign_in_account(InsertGoogleSignInArgs { subject: args.claims_subject, maybe_user_token: Some(&args.user_account.token), email_address: args.claims_email_address, is_email_verified: args.claims.email_verified(), maybe_locale: args.claims.locale(), maybe_name: args.claims.name(), maybe_given_name: args.claims.given_name(), maybe_family_name: args.claims.family_name(), creator_ip_address: &ip_address, transaction: &mut transaction }).await.map_err(|err| {
  ...
  Ok(NewSsoAccountInfo { user_token: args.user_account.token, user_display_name: args.user_account.display_name, username_is_not_customized: args.user_account.username_is_not_customized, is_new_account: false })
}
