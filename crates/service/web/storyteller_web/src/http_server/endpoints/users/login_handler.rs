...
use mysql_queries::queries::user_impersonation_requests::mark_impersonation_token_as_redeemed::{mark_impersonation_token_as_redeemed, MarkImpersonationTokenAsRedeemedArgs};
...
use mysql_queries::queries::users::user_sessions::create_impersonated_user_session::{create_impersonated_user_session, CreateImpersonatedUserSessionArgs};
...
impl LoginErrorResponse {
  fn invalid_credentials() -> Self {
    Self { success: false, error_type: LoginErrorType::InvalidCredentials, error_message: "invalid credentials".to_string() }
  }

  fn server_error() -> Self {
    Self { success: false, error_type: LoginErrorType::ServerError, error_message: "server error".to_string() }
  }

  fn account_without_password() -> Self {
    Self { success: false, error_type: LoginErrorType::AccountNeedsPassword, error_message: "account was created without a password; please try password reset".to_string() }
  }
}
...
pub async fn login_handler(http_request: HttpRequest, request: web::Json<LoginRequest>, session_cookie_manager: web::Data<HttpUserSessionManager>, mysql_pool: web::Data<MySqlPool>) -> Result<HttpResponse, LoginErrorResponse> {
  ...
  let maybe_user = if check_username_or_email.contains("@") { lookup_user_for_login_by_email(&check_username_or_email, &mysql_pool).await } else { lookup_user_for_login_by_username(&check_username_or_email, &mysql_pool).await };
  ...
  let password_error = match bcrypt_confirm_password(request.password.clone(), &actual_hash) {
    ...
    Err(PasswordConfirmError::HashIsSentinelValue) => Some(LoginErrorResponse::account_without_password()),
    Err(e) => {
  ...
  let session_token = if password_error.is_none() {
    ...
    let create_session_result = create_user_session(&user.token.0, &ip_address, &mysql_pool).await;
    ...
    let user_feature_flags = UserSessionFeatureFlags::new(user.maybe_feature_flags.as_deref());
    ...
    if let Ok(mut conn) = mysql_pool.acquire().await {
      enroll_in_studio(&user.token, &ip_address, &mut conn, Some(&user_feature_flags))
        .await
        .map_err(|e| {
  ...
  let response = LoginSuccessResponse { success: true, signed_session };

  let body = serde_json::to_string(&response).map_err(|_e| LoginErrorResponse::server_error())?;
...
async fn try_impersonation_login(target_user_token: &UserToken, password: &str, ip_address: &str, mysql_pool: &MySqlPool) -> Result<UserSessionToken, LoginErrorResponse> {
  ...
  if record.impersonated_user_token != *target_user_token {
    warn!("Impersonation token user mismatch: token is for {} but login is for {}", record.impersonated_user_token.as_str(), target_user_token.as_str(),);
    return Err(LoginErrorResponse::invalid_credentials());
  }

  info!("Valid impersonation login: moderator={} impersonating={}", record.impersonator_user_token.as_str(), target_user_token.as_str(),);

  // Begin transaction: audit log + mark redeemed + create session.
  let mut transaction = mysql_pool.begin().await.map_err(|err| {
    warn!("Failed to begin impersonation transaction: {:?}", err);
    LoginErrorResponse::server_error()
  })?;

  // Insert staff audit log for the redemption.
  let _audit_token = insert_staff_audit_log(InsertStaffAuditLogArgs { audit_action: StaffAuditAction::ImpersonateUserRedeem, maybe_entity_type: Some(StaffAuditEntityType::User), maybe_entity_token: Some(target_user_token.as_str()), staff_user_token: &record.impersonator_user_token, actor_ip_address: ip_address, mysql_executor: &mut *transaction, phantom: PhantomData }).await.map_err(|err| {
    warn!("Failed to insert impersonation audit log: {:?}", err);
    LoginErrorResponse::server_error()
  })?;

  // Mark the impersonation token as redeemed.
  mark_impersonation_token_as_redeemed(MarkImpersonationTokenAsRedeemedArgs { user_impersonation_token: password, ip_address_redemption: ip_address, mysql_executor: &mut *transaction, phantom: PhantomData }).await.map_err(|err| {
  ...
  let session_token = create_impersonated_user_session(CreateImpersonatedUserSessionArgs { user_token: target_user_token, impersonator_user_token: &record.impersonator_user_token, ip_address, expires_at, mysql_executor: &mut *transaction, phantom: PhantomData }).await.map_err(|err| {
  ...
  info!("Impersonation session created: moderator={} as user={} session={}", record.impersonator_user_token.as_str(), target_user_token.as_str(), session_token.as_str(),);
