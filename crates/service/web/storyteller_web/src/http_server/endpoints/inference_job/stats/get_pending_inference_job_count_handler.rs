...
pub async fn get_pending_inference_job_count_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, GetPendingInferenceJobCountError> {
  if server_state.flags.disable_inference_queue_length_endpoint {
    ...
    return render_response_busy(GetPendingInferenceJobSuccessResponse { success: true, pending_job_count: 10_000, cache_time: NaiveDateTime::from_timestamp(0, 0), refresh_interval_millis: server_state.flags.frontend_pending_inference_refresh_interval_millis });
  }

  let maybe_cached = server_state.caches.ephemeral.inference_queue_length.grab_copy_without_bump_if_unexpired().map_err(|e| {
  ...
  let count_result = match maybe_cached {
    ...
    None => {
      ...
      let count_query_result = get_pending_inference_job_count(&server_state.mysql_pool).await;
      ...
      match count_query_result {
        ...
        Err(err) => {
          ...
          let maybe_cached = server_state.caches.ephemeral.inference_queue_length.grab_even_expired_and_bump().map_err(|err| {
        ...
        // Happy path...
        Ok(count_result) => {
          server_state.caches.ephemeral.inference_queue_length.store_copy(&count_result).map_err(|e| {
  ...
  render_response_ok(GetPendingInferenceJobSuccessResponse { success: true, pending_job_count: count_result.record_count, cache_time: count_result.present_time, refresh_interval_millis: server_state.flags.frontend_pending_inference_refresh_interval_millis })
}
