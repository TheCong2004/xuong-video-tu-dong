...
use mysql_queries::queries::users::user::update::set_user_ban_status::{set_user_ban_status, SetUserBanStatusArgs};
...
pub async fn moderation_ban_user_handler(http_request: HttpRequest, request: Json<ModerationBanUserRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<ModerationBanUserSuccessResponse>, CommonWebError> {
  ...
  let user_profile = get_user_profile_by_username(&username_lower, &server_state.mysql_pool)
    .await
    .map_err(|err| {
  ...
  let audit_action = if request.is_banned { StaffAuditAction::BanUser } else { StaffAuditAction::UnbanUser };

  info!("Moderator {} {} user {} ({})", user_session.user_token.as_str(), audit_action.to_str(), user_profile.user_token.as_str(), username_lower,);
  ...
  // 4. Set ban status.
  set_user_ban_status(SetUserBanStatusArgs { subject_user_token: &user_profile.user_token, is_banned: request.is_banned, mod_user_token: &user_session.user_token, maybe_mod_comments: Some(&request.mod_notes), mysql_executor: &mut *transaction, phantom: PhantomData }).await.map_err(|err| {
    warn!("Failed to set user ban status: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  // 5. Insert staff audit log.
  let _audit_token = insert_staff_audit_log(InsertStaffAuditLogArgs { audit_action, maybe_entity_type: Some(StaffAuditEntityType::User), maybe_entity_token: Some(user_profile.user_token.as_str()), staff_user_token: &user_session.user_token, actor_ip_address: &ip_address, mysql_executor: &mut *transaction, phantom: PhantomData }).await.map_err(|err| {
  ...
  info!("User {} ban status set to {} by moderator {}", user_profile.user_token.as_str(), request.is_banned, user_session.user_token.as_str(),);

  Ok(Json(ModerationBanUserSuccessResponse { success: true }))
}
