...
use artcraft_api_defs::moderation::wallets::moderator_add_banked_balance_to_wallet::{ModeratorAddBankedBalanceToWalletPathInfo, ModeratorAddBankedBalanceToWalletRequest, ModeratorAddBankedBalanceToWalletResponse};
...
pub async fn moderator_add_banked_balance_to_wallet_handler(http_request: HttpRequest, path: Path<ModeratorAddBankedBalanceToWalletPathInfo>, request: Json<ModeratorAddBankedBalanceToWalletRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<ModeratorAddBankedBalanceToWalletResponse>, CommonWebError> {
  let user_session = require_moderator(&http_request, &server_state.session_checker, &server_state.mysql_pool).await?;

  let ip_address = get_request_ip(&http_request);

  info!("Moderator {} adding {} banked credits to wallet {}", user_session.user_token.as_str(), request.credits, path.wallet_token.as_str(),);
  ...
  let _result = add_durable_banked_balance_to_wallet(&path.wallet_token, request.credits as u64, None, Some(WalletLedgerEntryType::StaffAddBanked), &mut transaction).await.map_err(|err| {
    warn!("moderator_add_banked_balance_to_wallet error: {:?}", err);
    CommonWebError::from_anyhow_error(err)
  })?;

  // Insert staff audit log.
  let _audit_token = insert_staff_audit_log(InsertStaffAuditLogArgs { audit_action: StaffAuditAction::AddWalletBankedBalance, maybe_entity_type: Some(StaffAuditEntityType::Wallet), maybe_entity_token: Some(path.wallet_token.as_str()), staff_user_token: &user_session.user_token, actor_ip_address: &ip_address, mysql_executor: &mut *transaction, phantom: PhantomData }).await.map_err(|err| {
  ...
  Ok(Json(ModeratorAddBankedBalanceToWalletResponse { success: true }))
}
