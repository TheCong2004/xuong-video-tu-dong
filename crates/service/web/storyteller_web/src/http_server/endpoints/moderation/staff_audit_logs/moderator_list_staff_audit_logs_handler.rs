...
use mysql_queries::queries::staff_audit_logs::list_staff_audit_logs::{list_staff_audit_logs, ListStaffAuditLogsArgs};
...
pub async fn moderator_list_staff_audit_logs_handler(http_request: HttpRequest, query: Query<ListStaffAuditLogsQueryParams>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<ListStaffAuditLogsSuccessResponse>, CommonWebError> {
  ...
  let records = list_staff_audit_logs(ListStaffAuditLogsArgs { maybe_cursor_id, limit, mysql_pool: &server_state.mysql_pool }).await.map_err(|err| {
  ...
  let audit_logs = records.into_iter().map(|r| StaffAuditLogResponse { token: r.token, audit_action: r.audit_action, maybe_entity_type: r.maybe_entity_type, maybe_entity_token: r.maybe_entity_token, maybe_target_username: r.maybe_target_username, maybe_target_display_name: r.maybe_target_display_name, staff_user_token: r.staff_user_token, staff_username: r.staff_username, staff_display_name: r.staff_display_name, staff_ip_address: r.staff_ip_address, created_at: r.created_at }).collect();

  Ok(Json(ListStaffAuditLogsSuccessResponse { success: true, audit_logs, maybe_cursor }))
}
