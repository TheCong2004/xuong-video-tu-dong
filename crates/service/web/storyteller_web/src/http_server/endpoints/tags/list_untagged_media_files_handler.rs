...
use mysql_queries::queries::tags::list_untagged_media_files_for_user::{list_untagged_media_files_for_user, ListUntaggedMediaFilesForUserArgs};
...
pub async fn list_untagged_media_files_handler(http_request: HttpRequest, query: Query<ListUntaggedMediaFilesQueryParams>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<ListUntaggedMediaFilesSuccessResponse>, CommonWebError> {
  ...
  let rows = list_untagged_media_files_for_user(ListUntaggedMediaFilesForUserArgs { owner_user_token: &user_session.user_token, maybe_cursor_id, limit, mysql_executor: &mut *conn, phantom: PhantomData }).await.map_err(|err| {
  ...
  Ok(Json(ListUntaggedMediaFilesSuccessResponse { success: true, media_files, maybe_cursor }))
}
