...
pub async fn delete_tag_handler(http_request: HttpRequest, path: Path<DeleteTagPathInfo>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<DeleteTagSuccessResponse>, CommonWebError> {
  ...
  let removed_link_count = perform_atomic_delete(&mut conn, &path.tag_token, &user_session.user_token).await?;

  Ok(Json(DeleteTagSuccessResponse { success: true, removed_link_count }))
}

/// Run the link hard-delete and the tag soft-delete as one unit.
async fn perform_atomic_delete(conn: &mut PoolConnection<MySql>, tag_token: &TagToken, creator_user_token: &UserToken) -> Result<u64, CommonWebError> {
  ...
  match work_result {
    ...
    Err(err) => {
      if let Err(rollback_err) = tx.rollback().await {
        error!("Rollback after delete-tag failure also failed: {:?} (original error: {:?})", rollback_err, err,);
      }
      Err(err)
    },
  }
...
async fn perform_delete_work(tx: &mut Transaction<'_, MySql>, tag_token: &TagToken, creator_user_token: &UserToken) -> Result<u64, CommonWebError> {
  let removed_link_count = delete_media_file_tags_for_tag(DeleteMediaFileTagsForTagArgs { tag_token, mysql_executor: &mut **tx, phantom: PhantomData }).await.map_err(|err| {
    warn!("delete_media_file_tags_for_tag failed: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  let updated = soft_delete_tag(SoftDeleteTagArgs { tag_token, creator_user_token, mysql_executor: &mut **tx, phantom: PhantomData }).await.map_err(|err| {
