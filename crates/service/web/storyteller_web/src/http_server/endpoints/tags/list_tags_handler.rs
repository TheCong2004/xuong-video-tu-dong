...
pub async fn list_tags_handler(http_request: HttpRequest, query: Query<ListTagsQueryParams>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<ListTagsSuccessResponse>, CommonWebError> {
  ...
  let rows = list_tags_for_user(ListTagsForUserArgs { creator_user_token: &user_session.user_token, maybe_cursor_id, limit, mysql_executor: &mut *conn, phantom: PhantomData }).await.map_err(|err| {
  ...
  Ok(Json(ListTagsSuccessResponse { success: true, tags: rows.into_iter().map(tag_row_to_details).collect(), maybe_cursor }))
}
