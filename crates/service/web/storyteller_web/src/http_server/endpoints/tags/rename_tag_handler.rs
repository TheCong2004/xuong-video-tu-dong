...
use artcraft_api_defs::tags::rename_tag::{RenameTagPathInfo, RenameTagRequest, RenameTagSuccessResponse};
use mysql_queries::errors::database_insert_error::DatabaseInsertError;
use mysql_queries::queries::tags::delete_media_file_tags_for_tag::{delete_media_file_tags_for_tag, DeleteMediaFileTagsForTagArgs};
use mysql_queries::queries::tags::get_soft_deleted_tag_by_lowercase::{get_soft_deleted_tag_by_lowercase, GetSoftDeletedTagByLowercaseArgs};
use mysql_queries::queries::tags::get_tag_for_owner::{get_tag_for_owner, GetTagForOwnerArgs};
use mysql_queries::queries::tags::hard_delete_soft_deleted_tag::{hard_delete_soft_deleted_tag, HardDeleteSoftDeletedTagArgs};
...
pub async fn rename_tag_handler(http_request: HttpRequest, path: Path<RenameTagPathInfo>, request: Json<RenameTagRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<RenameTagSuccessResponse>, CommonWebError> {
  let new_tag_value = request.new_tag_value.trim();
  if new_tag_value.is_empty() {
    return Err(CommonWebError::BadInputWithSimpleMessage("new_tag_value must not be empty".to_string()));
  }
  ...
  if new_tag_value.chars().count() > MAX_TAG_LENGTH_CHARS || new_tag_value_lowercase.chars().count() > MAX_TAG_LENGTH_CHARS {
  ...
  perform_atomic_rename(&mut conn, &path.tag_token, &user_session.user_token, new_tag_value, &new_tag_value_lowercase).await?;

  Ok(Json(RenameTagSuccessResponse { success: true, tag: TagDetails { tag_token: tag.token, tag_value: new_tag_value.to_string(), tag_value_lowercase: new_tag_value_lowercase, use_count: tag.use_count } }))
}
...
async fn perform_atomic_rename(conn: &mut PoolConnection<MySql>, tag_token: &TagToken, creator_user_token: &UserToken, new_tag_value: &str, new_tag_value_lowercase: &str) -> Result<(), CommonWebError> {
  let mut tx = conn.begin().await.map_err(|err| {
    warn!("Failed to begin rename-tag transaction: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  let work_result = perform_rename_work(&mut tx, tag_token, creator_user_token, new_tag_value, new_tag_value_lowercase).await;
  ...
  match work_result {
    ...
    Err(err) => {
      if let Err(rollback_err) = tx.rollback().await {
        error!("Rollback after rename-tag failure also failed: {:?} (original error: {:?})", rollback_err, err,);
      }
      Err(err)
    },
  }
...
async fn perform_rename_work(tx: &mut Transaction<'_, MySql>, tag_token: &TagToken, creator_user_token: &UserToken, new_tag_value: &str, new_tag_value_lowercase: &str) -> Result<(), CommonWebError> {
  ...
  let maybe_dead_tag_token = get_soft_deleted_tag_by_lowercase(GetSoftDeletedTagByLowercaseArgs { tag_value_lowercase: new_tag_value_lowercase, creator_user_token, mysql_executor: &mut **tx, phantom: PhantomData }).await.map_err(|err| {
    warn!("get_soft_deleted_tag_by_lowercase failed: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  if let Some(dead_tag_token) = maybe_dead_tag_token {
    delete_media_file_tags_for_tag(DeleteMediaFileTagsForTagArgs { tag_token: &dead_tag_token, mysql_executor: &mut **tx, phantom: PhantomData }).await.map_err(|err| {
      warn!("delete_media_file_tags_for_tag failed: {:?}", err);
      CommonWebError::from_error(err)
    })?;

    hard_delete_soft_deleted_tag(HardDeleteSoftDeletedTagArgs { tag_token: &dead_tag_token, creator_user_token, mysql_executor: &mut **tx, phantom: PhantomData }).await.map_err(|err| {
      warn!("hard_delete_soft_deleted_tag failed: {:?}", err);
      CommonWebError::from_error(err)
    })?;
  }

  let updated = rename_tag(RenameTagArgs { tag_token, creator_user_token, new_tag_value, new_tag_value_lowercase, mysql_executor: &mut **tx, phantom: PhantomData }).await.map_err(|err| {
    ...
    match DatabaseInsertError::from(err) {
      DatabaseInsertError::DuplicateKeyError => CommonWebError::BadInputWithSimpleMessage("you already have a tag with that name".to_string()),
      other => {
