...
use mysql_queries::queries::uploaded_video_notes::insert_uploaded_video_note::{insert_uploaded_video_note, InsertUploadedVideoNoteArgs};
use mysql_queries::queries::uploaded_video_notes::update_uploaded_video_note::{update_uploaded_video_note, UpdateUploadedVideoNoteArgs};
...
pub async fn video_info_notes_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>, Json(request): Json<VideoInfoNoteRequest>) -> Result<Json<VideoInfoNoteResponse>, CommonWebError> {
  ...
  let uploaded_video_note_token = match &request.maybe_uploaded_video_note_token {
    ...
    Some(note_token) => {
      update_uploaded_video_note(UpdateUploadedVideoNoteArgs { token: note_token, maybe_filename, maybe_reported_model_type: request.maybe_reported_model_type, maybe_reported_model_name: maybe_reported_model_name.as_deref(), maybe_website: request.maybe_website, maybe_other_website: maybe_other_website.as_deref(), maybe_comments: maybe_comments.as_deref(), maybe_email_address: maybe_email_address.as_deref(), can_share_report, was_scammed, maybe_comment_update_ip_address: Some(ip_address.as_str()), mysql_executor: &mut *mysql_connection, phantom: PhantomData }).await?;
      note_token.clone()
    },
    // Create a new note.
    None => insert_uploaded_video_note(InsertUploadedVideoNoteArgs { uploaded_video_token: &request.uploaded_video_token, maybe_filename, maybe_reported_model_type: request.maybe_reported_model_type, maybe_reported_model_name: maybe_reported_model_name.as_deref(), maybe_website: request.maybe_website, maybe_other_website: maybe_other_website.as_deref(), maybe_comments: maybe_comments.as_deref(), maybe_email_address: maybe_email_address.as_deref(), can_share_report, was_scammed, comment_create_ip_address: &ip_address, mysql_executor: &mut *mysql_connection, phantom: PhantomData }).await?,
  };

  Ok(Json(VideoInfoNoteResponse { success: true, uploaded_video_note_token }))
}

/// Trim a free-form text field; a missing or whitespace-only value becomes `None`.
fn normalize_optional(value: &Option<String>) -> Option<String> {
  value.as_deref().map(str::trim).filter(|trimmed| !trimmed.is_empty()).map(str::to_string)
}
