...
use artcraft_api_defs::video_info::read_only::{DreaminaVideoInfo, KlingVideoInfo, SeedanceVideoInfo, SoraVideoInfo, VeoVideoInfo, VideoProvenanceKind};
...
pub fn to_provenance(result: &Result<VideoInfo, VideoInfoError>, maybe_encoder: Option<String>) -> Provenance {
  let mut provenance = Provenance { kind: VideoProvenanceKind::Unrecognized, maybe_encoder, maybe_seedance: None, maybe_veo: None, maybe_sora: None, maybe_dreamina: None, maybe_kling: None };
...
pub fn detect_model_type(result: &Result<VideoInfo, VideoInfoError>) -> Option<UploadedVideoDetectedModelType> {
...
pub fn model_family_for_type(model_type: UploadedVideoDetectedModelType) -> UploadedVideoDetectedModelFamily {
  ...
  match model_type {
    ...
    Type::Kling16Pro | Type::Kling21Pro | Type::Kling21Master | Type::Kling2p5TurboPro | Type::Kling2p6Pro | Type::Kling3p0Standard | Type::Kling3p0Pro => Family::Kling,
    Type::HappyHorse1p0 => Family::HappyHorse,
    Type::Seedance10Lite | Type::Seedance1p5Pro | Type::Seedance2p0 | Type::Seedance2p0Fast | Type::Seedance2p0BytePlus | Type::Seedance2p0BytePlusFast | Type::Seedance2p0Ultra | Type::Seedance2p0UltraFast | Type::Seedance2p0BytePlusUltra | Type::Seedance2p0BytePlusUltraFast => Family::Seedance,
...
pub fn detect_model_family(result: &Result<VideoInfo, VideoInfoError>) -> Option<UploadedVideoDetectedModelFamily> {
  use UploadedVideoDetectedModelFamily as Family;
...
fn to_veo(info: &VeoInfo) -> VeoVideoInfo {
  VeoVideoInfo { producer: info.producer.clone(), has_c2pa_manifest: info.has_c2pa_manifest, maybe_encoder: info.encoder.clone(), maybe_created_description: info.created_description.clone(), has_synthid_watermark: info.has_synthid_watermark, maybe_synthid_description: info.synthid_description.clone(), maybe_digital_source_type: info.digital_source_type.clone(), maybe_claim_generator: info.claim_generator.clone(), maybe_claim_generator_version: info.claim_generator_version.clone(), maybe_manifest_id: info.manifest_id.clone(), maybe_instance_id: info.instance_id.clone(), maybe_cert_serial: info.cert_serial.clone(), maybe_signer_ca: info.signer_ca.clone(), is_timestamped: info.is_timestamped, maybe_timestamp_authority: info.timestamp_authority.clone() }
}

fn to_sora(info: &SoraInfo) -> SoraVideoInfo {
  SoraVideoInfo { producer: info.producer.clone(), maybe_model_name: info.model_name.clone(), maybe_created_description: info.created_description.clone(), maybe_digital_source_type: info.digital_source_type.clone(), maybe_claim_generator: info.claim_generator.clone(), maybe_manifest_id: info.manifest_id.clone(), maybe_instance_id: info.instance_id.clone(), maybe_cert_serial: info.cert_serial.clone() }
}

fn to_dreamina(info: &DreaminaInfo) -> DreaminaVideoInfo {
  DreaminaVideoInfo { product: info.product.clone(), maybe_export_type: info.export_type.clone(), maybe_os: info.os.clone(), maybe_source_info: info.source_info.clone(), maybe_aigc_label_type: info.aigc_label_type, maybe_video_id: info.video_id.clone(), has_c2pa: info.has_c2pa, maybe_signer_org_id: info.signer_org_id.clone(), maybe_signer_country: info.signer_country.clone(), maybe_cert_serial: info.cert_serial.clone() }
}

fn to_kling(info: &KlingInfo) -> KlingVideoInfo {
  KlingVideoInfo { maybe_model_version: info.model_version.clone(), is_ai_generated: info.is_ai_generated, maybe_label: info.label.clone(), maybe_content_producer: info.content_producer.clone(), maybe_produce_id: info.produce_id.clone(), maybe_content_propagator: info.content_propagator.clone(), maybe_propagate_id: info.propagate_id.clone(), maybe_reserved_code_1: info.reserved_code_1.clone(), maybe_reserved_code_2: info.reserved_code_2.clone(), has_stream_watermark: info.has_stream_watermark, maybe_watermark_uuid: info.watermark_uuid.clone() }
}
