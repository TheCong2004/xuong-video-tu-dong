...
pub(super) fn validate_when_image_required(request: &OmniApiVideoGenerateRequest) -> Result<(), CommonWebError> {
  match request.model {
  ...
  let has_start_frame = request.start_frame_image_media_token.is_some() || request.start_frame_image_url.is_some();

  let has_reference_images = request.reference_image_media_tokens.as_ref().is_some_and(|v| !v.is_empty()) || request.reference_image_urls.as_ref().is_some_and(|v| !v.is_empty());

  if has_start_frame || has_reference_images {
    return Ok(());
  }

  Err(CommonWebError::BadInputWithSimpleMessage("Image is required".to_string()))
}
...
mod tests {
  ...
  fn passes_for_other_models_without_image() {
    ...
    let req = OmniApiVideoGenerateRequest { model: Some(CommonVideoModel::Seedance2p0), ..base_request() };
    assert!(validate_when_image_required(&req).is_ok());
  }

  #[test]
  fn rejects_v1p5_with_no_image_inputs() {
    let req = OmniApiVideoGenerateRequest { model: Some(CommonVideoModel::GrokImagineVideo1p5), ..base_request() };
  ...
  fn rejects_v1p5_with_empty_reference_list_and_no_start_frame() {
    let req = OmniApiVideoGenerateRequest { model: Some(CommonVideoModel::GrokImagineVideo1p5), reference_image_media_tokens: Some(vec![]), ..base_request() };
    assert!(matches!(validate_when_image_required(&req), Err(CommonWebError::BadInputWithSimpleMessage(_))));
  }

  #[test]
  fn accepts_v1p5_with_start_frame_only() {
    let req = OmniApiVideoGenerateRequest { model: Some(CommonVideoModel::GrokImagineVideo1p5), start_frame_image_media_token: Some(token("mf_start")), ..base_request() };
    assert!(validate_when_image_required(&req).is_ok());
  }

  #[test]
  fn accepts_v1p5_with_reference_images_only() {
    let req = OmniApiVideoGenerateRequest { model: Some(CommonVideoModel::GrokImagineVideo1p5), reference_image_media_tokens: Some(vec![token("mf_a"), token("mf_b")]), ..base_request() };
    assert!(validate_when_image_required(&req).is_ok());
  }

  #[test]
  fn accepts_v1p5_with_both_start_frame_and_reference_images() {
    let req = OmniApiVideoGenerateRequest { model: Some(CommonVideoModel::GrokImagineVideo1p5), start_frame_image_media_token: Some(token("mf_start")), reference_image_media_tokens: Some(vec![token("mf_a")]), ..base_request() };
    assert!(validate_when_image_required(&req).is_ok());
  }
  ...
  fn end_frame_alone_does_not_count_as_image_input() {
    ...
    let req = OmniApiVideoGenerateRequest { model: Some(CommonVideoModel::GrokImagineVideo1p5), end_frame_image_media_token: Some(token("mf_end")), ..base_request() };
