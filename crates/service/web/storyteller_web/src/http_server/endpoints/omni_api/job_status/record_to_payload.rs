use artcraft_api_defs::omni_api::job_status::omni_api_job_status_payload::{OmniApiJobRequestDetails, OmniApiJobResultDetails, OmniApiJobStatusDetails, OmniApiJobStatusPayload};
...
pub fn record_to_payload(record: GenericInferenceJobStatus, server_environment: ServerEnvironment, media_domain: MediaDomain) -> OmniApiJobStatusPayload {
  ...
  OmniApiJobStatusPayload {
    ...
    request: OmniApiJobRequestDetails { inference_category: record.request_details.inference_category, maybe_prompt_token: record.request_details.maybe_prompt_token, maybe_model_type: maybe_filter_model_name(record.request_details.maybe_model_type.as_deref()), maybe_model_token: record.request_details.maybe_model_token },
    status: OmniApiJobStatusDetails { status: record.status, maybe_first_started_at: record.maybe_first_started_at, maybe_failure_category: record.maybe_frontend_failure_category, progress_percentage },
    maybe_result: record.maybe_result_details.map(|result_details| {
      ...
      OmniApiJobResultDetails { entity_type: result_details.entity_type, entity_token: result_details.entity_token, media_links: MediaLinksBuilder::from_rooted_path_and_env(media_domain, server_environment, &public_bucket_media_path), maybe_successfully_completed_at: result_details.maybe_successfully_completed_at }
    }),
