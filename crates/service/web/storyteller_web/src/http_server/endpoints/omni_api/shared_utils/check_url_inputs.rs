...
pub fn check_mutually_exclusive(media_token_field_name: &str, media_token_is_set: bool, url_field_name: &str, url_is_set: bool) -> Result<(), CommonWebError> {
  if media_token_is_set && url_is_set {
    return Err(CommonWebError::BadInputWithSimpleMessage(format!("Either {} or {} must be set, not both", media_token_field_name, url_field_name,)));
  }
  Ok(())
}
...
pub fn check_single_url_format(url: &str) -> Result<(), CommonWebError> {
  if !url.starts_with("http://") && !url.starts_with("https://") {
    return Err(CommonWebError::BadInputWithSimpleMessage(format!("URL must start with http:// or https://, bad URL: {}", url,)));
  }
  Ok(())
}
