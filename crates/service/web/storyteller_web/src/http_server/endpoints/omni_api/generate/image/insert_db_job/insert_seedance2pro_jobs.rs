...
pub async fn insert_seedance2pro_jobs(args: InsertSeedance2proJobsArgs<'_, '_>) -> Result<InsertSeedance2proJobsResult, CommonWebError> {
  let InsertSeedance2proJobsArgs { primary_order_id, maybe_additional_order_ids, maybe_wallet_ledger_entry_token, kinovi_version, mut shared } = args;
  ...
  for (i, order_id) in all_order_ids.iter().enumerate() {
    ...
    let db_result = insert_generic_inference_job_for_seedance2pro_queue_with_apriori_job_token(InsertGenericInferenceForSeedance2ProWithAprioriJobTokenArgs { kinovi_version, apriori_job_token: &job_token, uuid_idempotency_token: &idempotency_str, maybe_external_third_party_id: order_id, maybe_model_type: shared.maybe_model_type, maybe_prompt_token: shared.maybe_prompt_token, maybe_wallet_ledger_entry_token, maybe_creator_user_token: Some(shared.user_token), maybe_avt_token: shared.maybe_avt_token, creator_ip_address: shared.ip_address, creator_set_visibility: Visibility::Public, maybe_platform_type: shared.maybe_platform_type, maybe_cost_estimates: None, maybe_debug_log_event_token: shared.maybe_debug_log_event_token, mysql_executor: &mut **shared.transaction, maybe_inference_args: None, phantom: Default::default() }).await;
  ...
  Ok(InsertSeedance2proJobsResult { primary_job_token, all_job_tokens })
}
