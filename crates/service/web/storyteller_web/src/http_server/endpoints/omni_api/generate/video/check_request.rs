...
pub fn check_request(request: &OmniApiVideoGenerateRequest) -> Result<(), CommonWebError> {
  check_mutually_exclusive("start_frame_image_media_token", request.start_frame_image_media_token.is_some(), "start_frame_image_url", request.start_frame_image_url.is_some())?;
  check_mutually_exclusive("end_frame_image_media_token", request.end_frame_image_media_token.is_some(), "end_frame_image_url", request.end_frame_image_url.is_some())?;
  check_mutually_exclusive("reference_image_media_tokens", request.reference_image_media_tokens.is_some(), "reference_image_urls", request.reference_image_urls.is_some())?;
  check_mutually_exclusive("reference_video_media_tokens", request.reference_video_media_tokens.is_some(), "reference_video_urls", request.reference_video_urls.is_some())?;
  check_mutually_exclusive("reference_audio_media_tokens", request.reference_audio_media_tokens.is_some(), "reference_audio_urls", request.reference_audio_urls.is_some())?;
