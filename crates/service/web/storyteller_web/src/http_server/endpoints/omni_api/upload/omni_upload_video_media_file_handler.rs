...
use artcraft_api_defs::omni_api::omni_upload_video::OmniUploadVideoMediaFileSuccessResponse;
...
///
/// IF VIEWING DOCS, PLEASE SEE BOTTOM OF PAGE `OmniUploadVideoMediaFileForm` (Under "Schema") FOR DETAILS ON FIELDS AND NULLABILITY.
#[derive(MultipartForm, ToSchema)]
#[multipart(duplicate_field = "deny")]
pub struct OmniUploadVideoMediaFileForm {
...
/// API-key authenticated video upload (Omni API). Identity is read from the `Authorization` header
/// API key rather than a session cookie.
#[utoipa::path(
  post,
  tag = "Omni API",
  path = "/v1/omni_api/upload/video",
  responses(
    (status = 200, description = "Success Update", body = OmniUploadVideoMediaFileSuccessResponse),
  ...
  params(
    (
      "request" = OmniUploadVideoMediaFileForm,
      description = "IF VIEWING DOCS, PLEASE SEE BOTTOM OF PAGE `OmniUploadVideoMediaFileForm` (Under 'Schema') FOR DETAILS ON FIELDS AND NULLABILITY."
    ),
  )
)]
