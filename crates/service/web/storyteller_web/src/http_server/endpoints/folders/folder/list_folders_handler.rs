...
use artcraft_api_defs::folders::folder::{ListFoldersQueryParams, ListFoldersSuccessResponse};
use mysql_queries::queries::folders::folder::list_folders_for_user::{list_folders_for_user, ListFoldersForUserArgs};
...
pub async fn list_folders_handler(http_request: HttpRequest, query: Query<ListFoldersQueryParams>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<ListFoldersSuccessResponse>, CommonWebError> {
  ...
  let rows = list_folders_for_user(ListFoldersForUserArgs { owner_user_token: &user_session.user_token, maybe_cursor_id, limit, mysql_executor: &mut *conn, phantom: PhantomData }).await.map_err(|err| {
  ...
  let folders = rows.into_iter().map(|row| folder_row_to_info(row, &thumbnails)).collect();

  Ok(Json(ListFoldersSuccessResponse { success: true, folders, maybe_cursor }))
}
