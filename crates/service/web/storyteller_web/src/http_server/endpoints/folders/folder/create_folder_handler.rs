...
pub async fn create_folder_handler(http_request: HttpRequest, request: Json<CreateFolderRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<CreateFolderSuccessResponse>, CommonWebError> {
  ...
  if let Some(parent_token) = &request.maybe_parent_folder_token {
    let parent = get_folder_for_owner(GetFolderForOwnerArgs { folder_token: parent_token, owner_user_token: &user_session.user_token, mysql_executor: &mut *conn, phantom: PhantomData }).await.map_err(|err| {
      warn!("Parent folder lookup failed: {:?}", err);
      CommonWebError::from_error(err)
    })?;

    if parent.is_none() {
      return Err(CommonWebError::BadInputWithSimpleMessage("parent folder does not exist".to_string()));
    }
  }

  let token = insert_folder(InsertFolderArgs { name: &name, owner_user_token: &user_session.user_token, maybe_parent_folder_token: request.maybe_parent_folder_token.as_ref(), maybe_color_code: request.maybe_color_code.as_deref(), mysql_executor: &mut *conn, phantom: PhantomData }).await.map_err(|err| {
    warn!("insert_folder failed: {:?}", err);
    CommonWebError::from_error(err)
  })?;

  // Re-fetch so we return the canonical FolderInfo including is_orphaned.
  let row = get_folder_for_owner(GetFolderForOwnerArgs { folder_token: &token, owner_user_token: &user_session.user_token, mysql_executor: &mut *conn, phantom: PhantomData })
    .await
    .map_err(|err| {
  ...
  Ok(Json(CreateFolderSuccessResponse { success: true, folder }))
}
