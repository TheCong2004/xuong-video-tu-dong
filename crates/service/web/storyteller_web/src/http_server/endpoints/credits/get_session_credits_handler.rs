...
pub async fn get_session_credits_handler(http_request: HttpRequest, path: Path<GetSessionCreditsPathInfo>, server_state: web::Data<Arc<ServerState>>) -> Result<Json<GetSessionCreditsResponse>, CommonWebError> {
  ...
  let maybe_wallet = find_primary_wallet_for_owner_using_connection(&user_token, path.namespace, &mut mysql_connection).await.map_err(|err| {
  ...
  let monthly_credits = maybe_wallet.as_ref().map(|w| w.monthly_credits).unwrap_or(0);

  let banked_credits = maybe_wallet.as_ref().map(|w| w.banked_credits).unwrap_or(0);

  let sum_total_credits = free_credits.saturating_add(monthly_credits).saturating_add(banked_credits);

  Ok(Json(GetSessionCreditsResponse { success: true, free_credits, monthly_credits, banked_credits, sum_total_credits }))
}
