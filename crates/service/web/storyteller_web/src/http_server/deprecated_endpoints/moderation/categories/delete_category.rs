...
pub async fn delete_category_handler(http_request: HttpRequest, path: Path<DeleteCategoryPathInfo>, request: web::Json<DeleteCategoryRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  ...
  let soft_delete_state = if request.set_delete { ToggleSoftDeleteState::Delete } else { ToggleSoftDeleteState::Undelete };

  let query_result = toggle_model_category_soft_delete(&path.token, soft_delete_state, &server_state.mysql_pool).await;
  ...
  let response = DeleteCategoryResponse { success: true };
