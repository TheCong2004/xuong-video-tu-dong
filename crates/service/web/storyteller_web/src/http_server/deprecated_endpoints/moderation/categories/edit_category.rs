...
pub async fn edit_category_handler(http_request: HttpRequest, path: Path<EditCategoryPathInfo>, request: web::Json<EditCategoryRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  ...
  if let Some(parent_category_token) = request.maybe_super_category_token.as_deref() {
    if parent_category_token == &path.token {
      return Err(CommonWebError::BadInputWithSimpleMessage("category cannot have itself as a parent".to_string()));
    }

    let parent_category_lookup = get_category_by_token(parent_category_token, &server_state.mysql_pool).await;

    match parent_category_lookup {
      Ok(Some(parent_category)) => {
        if !parent_category.can_have_subcategories {
          return Err(CommonWebError::BadInputWithSimpleMessage("parent category cannot have children".to_string()));
        }
      },
  ...
  let query_result = update_model_category(UpdateModelCategoryArgs { name: &request.name, maybe_dropdown_name: request.maybe_dropdown_name.as_deref(), can_directly_have_models: request.can_directly_have_models, can_have_subcategories: request.can_have_subcategories, can_only_mods_apply: request.can_only_mods_apply, maybe_super_category_token: request.maybe_super_category_token.as_deref(), is_mod_approved: request.is_mod_approved, mod_user_token: user_session.user_token.as_str(), maybe_mod_comments: request.maybe_mod_comments.as_deref(), model_category_token: &path.token, mysql_pool: &server_state.mysql_pool }).await;
  ...
  let response = EditCategoryResponse { success: true };
