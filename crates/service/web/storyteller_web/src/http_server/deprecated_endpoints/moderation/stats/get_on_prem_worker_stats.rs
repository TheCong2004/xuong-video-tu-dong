...
pub async fn get_on_prem_worker_stats_handler(http_request: HttpRequest, query: Query<QueryFields>, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  ...
  let sample_size = query.sample_size.unwrap_or(DEFAULT_SAMPLE_SIZE).max(MIN_SAMPLE_SIZE).min(MAX_SAMPLE_SIZE);

  let result = get_on_prem_worker_stats(&server_state.mysql_pool, sample_size).await.map_err(|e| CommonWebError::from_anyhow_error(e))?;

  let response = GetOnPremWorkerStatsResponse { success: true, worker_mix_stats: WorkerMixStats { total_records_sampled: result.total_records_sampled, on_prem_count: result.on_prem_count, cloud_count: result.cloud_count } };
