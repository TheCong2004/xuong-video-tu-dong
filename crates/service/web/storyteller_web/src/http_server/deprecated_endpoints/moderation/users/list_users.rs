...
impl ListUsersErrorResponse {
  fn unauthorized() -> Self {
    Self { success: false, error_type: ListUsersErrorType::Unauthorized, error_message: "unauthorized".to_string() }
  }
  fn server_error() -> Self {
    Self { success: false, error_type: ListUsersErrorType::ServerError, error_message: "server error".to_string() }
  }
  fn bad_request(error_message: &str) -> Self {
    Self { success: false, error_type: ListUsersErrorType::BadInput, error_message: error_message.to_string() }
  }
}
...
pub async fn list_users_handler(http_request: HttpRequest, request: web::Query<ListUsersRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, ListUsersErrorResponse> {
  ...
  let query_results = ListUsersQueryBuilder::new().sort_ascending(true).query_for_page(&server_state.mysql_pool).await;
  ...
  let response = ListUsersResponse { success: true, users: results_page.users };

  let body = serde_json::to_string(&response).map_err(|e| ListUsersErrorResponse::server_error())?;
