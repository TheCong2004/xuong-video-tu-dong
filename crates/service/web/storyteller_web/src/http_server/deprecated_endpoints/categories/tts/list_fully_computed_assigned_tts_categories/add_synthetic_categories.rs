...
pub fn add_recent_models(recursive_category_to_model_map: &mut CategoryTokenToModelTokensMap, models: &Vec<TtsModelInfoLite>) {
  let mut model_refs: Vec<&TtsModelInfoLite> = models.iter().collect::<Vec<&TtsModelInfoLite>>();
...
pub fn add_trending_models(recursive_category_to_model_map: &mut CategoryTokenToModelTokensMap, trending_models: TrendingModels) {
  ...
  let model_tokens = trending_models.models.iter().filter(|trending_model| trending_model.window_name == WindowName::Last3Hours).take(30).map(|trending_model| trending_model.model_token.clone()).collect::<BTreeSet<TtsModelToken>>();
