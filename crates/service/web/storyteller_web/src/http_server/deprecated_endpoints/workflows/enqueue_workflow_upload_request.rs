...
pub async fn enqueue_workflow_upload_request(http_request: HttpRequest, request: Json<EnqueueWorkFlowRequest>, server_state: Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  if request.google_drive_link.is_empty() {
    return Err(CommonWebError::BadInputWithSimpleMessage("Missing Google Drive Link".to_string()));
  }

  if request.title.is_empty() {
    return Err(CommonWebError::BadInputWithSimpleMessage("Missing Title".to_string()));
  }

  if request.description.is_empty() {
    return Err(CommonWebError::BadInputWithSimpleMessage("Missing Description".to_string()));
  }

  if request.commit_hash.is_empty() {
    return Err(CommonWebError::BadInputWithSimpleMessage("Missing commit hash".to_string()));
  }

  let mut maybe_user_token: Option<UserToken> = None;

  let visibility = request.creator_set_visibility.unwrap_or(Visibility::Private);
  ...
  let inference_args = WorkflowArgs {
    ...
    enable_lipsync: None,
    rollout_python_workflow_args: get_request_header_optional(&http_request, "PYTHON-WORKFLOW-ARGS").map(|value| str_to_bool(&value)),
  ...
  let response: EnqueueWorkFlowRequestSuccessResponse = EnqueueWorkFlowRequestSuccessResponse { success: true, inference_job_token: job_token };
