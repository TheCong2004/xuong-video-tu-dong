...
pub async fn enqueue_video_style_transfer_handler(http_request: HttpRequest, request: web::Json<EnqueueVideoStyleTransferRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  // ==================== VALIDATION ==================== //
  ...
  let maybe_strength = request.use_strength.map(|strength| if strength < MINIMUM_STRENGTH || strength > MAXIMUM_STRENGTH { Err(CommonWebError::BadInputWithSimpleMessage("Strength must be between 0.0 and 1.0".to_string())) } else { Ok(strength) }).transpose()?;

  let coordinated_args = CoordinatedWorkflowArgs { prompt: request.prompt.new_string_trim_or_empty(), travel_prompt: request.travel_prompt.new_string_trim_or_empty(), use_lipsync: lazy_any_option_true(&[Box::new(|| request.enable_lipsync), Box::new(|| request.use_lipsync), Box::new(|| get_request_header_optional(&http_request, "LIPSYNC-ENABLED").map(|value| str_to_bool(&value)))]), disable_lcm: lazy_any_option_true(&[Box::new(|| request.disable_lcm), Box::new(|| get_request_header_optional(&http_request, "DISABLE-LCM").map(|value| str_to_bool(&value)))]), use_cinematic: lazy_any_option_true(&[Box::new(|| request.use_cinematic), Box::new(|| get_request_header_optional(&http_request, "USE-CINEMATIC").map(|value| str_to_bool(&value)))]), use_face_detailer: request.use_face_detailer, use_upscaler: request.use_upscaler, use_cogvideo: None, remove_watermark: request.remove_watermark };
  ...
  let response: EnqueueVideoStyleTransferSuccessResponse = EnqueueVideoStyleTransferSuccessResponse { success: true, inference_job_token: job_token };
