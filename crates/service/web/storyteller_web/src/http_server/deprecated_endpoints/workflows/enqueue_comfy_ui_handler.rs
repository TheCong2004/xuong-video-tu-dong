...
pub async fn enqueue_comfy_ui_handler(http_request: HttpRequest, request: web::Json<EnqueueComfyRequest>, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  ...
  let response: EnqueueComfySuccessResponse = EnqueueComfySuccessResponse { success: true, inference_job_token: job_token };
