...
pub async fn disable_design_refresh_flag_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>) -> impl Responder {
  ...
  HttpResponse::build(StatusCode::OK).content_type("text/plain").cookie(cookie).body("cookie disabled")
}
