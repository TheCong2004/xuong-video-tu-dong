...
pub async fn list_vocoders_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>) -> Result<HttpResponse, CommonWebError> {
  ...
  let query_results = list_vocoder_models(&server_state.mysql_pool, NO_CREATOR_SCOPING_HERE, false).await;
  ...
  let vocoders = vocoders
    ...
    .map(|v: VocoderModelListItem| {
      let mut vocoder = VocoderListItem { vocoder_token: v.vocoder_token, vocoder_type: v.vocoder_type, title: v.title, is_staff_recommended: v.is_staff_recommended, creator_user_token: v.creator_user_token, creator_username: v.creator_username, creator_display_name: v.creator_display_name, creator_gravatar_hash: v.creator_gravatar_hash, created_at: v.created_at, updated_at: v.updated_at, moderator_fields: Some(VocoderListItemModFields { is_mod_disabled_from_public_use: v.moderator_fields.is_mod_disabled_from_public_use, is_mod_disabled_from_author_use: v.moderator_fields.is_mod_disabled_from_author_use, is_mod_author_editing_locked: v.moderator_fields.is_mod_author_editing_locked, user_deleted_at: v.moderator_fields.user_deleted_at, mod_deleted_at: v.moderator_fields.mod_deleted_at }) };
  ...
  let response = ListVocodersSuccessResponse { success: true, vocoders };
