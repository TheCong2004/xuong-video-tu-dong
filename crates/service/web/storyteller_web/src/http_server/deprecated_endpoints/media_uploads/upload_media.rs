...
#[deprecated(note = "Use `media_files` instead of `media_uploads`.")]
pub async fn upload_media_handler(http_request: HttpRequest, server_state: web::Data<Arc<ServerState>>, mut multipart_payload: Multipart) -> Result<HttpResponse, UploadError> {
  ...
  let response = UploadMediaSuccessResponse { success: true, upload_token: media_upload_token };
