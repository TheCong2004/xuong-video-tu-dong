...
{
  app.service(
    web::scope("/v1/omni_api")
      .service(web::scope("/generate").service(web::resource("/video").route(web::post().to(omni_api_video_generate_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/image").route(web::post().to(omni_api_image_generate_handler)).route(web::head().to(|| HttpResponse::Ok()))))
      .service(web::scope("/upload").service(web::resource("/audio").route(web::post().to(omni_upload_audio_media_file_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/image").route(web::post().to(omni_upload_image_media_file_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/video").route(web::post().to(omni_upload_video_media_file_handler)).route(web::head().to(|| HttpResponse::Ok()))))
      .service(web::scope("/job_status").service(web::resource("/job/{token}").route(web::get().to(omni_api_get_job_status_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/batch").route(web::get().to(omni_api_batch_get_job_status_handler)).route(web::head().to(|| HttpResponse::Ok())))),
  )
}
