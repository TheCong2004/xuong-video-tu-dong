...
{
  app.service(web::scope("/v1/video_info").service(web::resource("/read_only").route(web::post().to(video_info_read_info_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/upload").route(web::post().to(video_info_upload_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/notes").route(web::post().to(video_info_notes_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
