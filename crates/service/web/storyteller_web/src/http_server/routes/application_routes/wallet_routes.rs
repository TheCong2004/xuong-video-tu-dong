...
{
  app.service(web::scope("/v1/wallets").service(web::resource("/session_artcraft_wallet").route(web::get().to(get_or_create_session_artcraft_wallet_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
