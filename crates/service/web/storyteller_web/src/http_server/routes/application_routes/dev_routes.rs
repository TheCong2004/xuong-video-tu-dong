...
{
  app.service(web::scope("/v1/dev").service(web::resource("/input_error").route(web::get().to(dev_input_error_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/basic_input_error").route(web::get().to(dev_basic_input_error_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
