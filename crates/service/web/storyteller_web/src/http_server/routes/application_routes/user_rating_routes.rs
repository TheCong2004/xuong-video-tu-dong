...
{
  app.service(web::scope("/v1/user_rating").service(web::resource("/batch").route(web::get().to(batch_get_user_rating_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/rate").route(web::post().to(set_user_rating_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/view/{entity_type}/{entity_token}").route(web::get().to(get_user_rating_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
