...
{
  app.service(web::scope("/v1/weights").service(web::resource("/weight/{weight_token}").route(web::get().to(get_weight_handler)).route(web::post().to(update_weight_handler)).route(web::delete().to(delete_weight_handler))).service(web::resource("/search").route(web::get().to(search_model_weights_http_get_handler)).route(web::post().to(search_model_weights_http_post_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/weight/{token}/cover_image").route(web::post().to(set_model_weight_cover_image_handler))).route("/by_user/{username}", web::get().to(list_weights_by_user_handler)).route("/list", web::get().to(list_available_weights_handler)).route("/list_featured", web::get().to(list_featured_weights_handler)).route("/list_pinned", web::get().to(list_pinned_weights_handler)))
}
