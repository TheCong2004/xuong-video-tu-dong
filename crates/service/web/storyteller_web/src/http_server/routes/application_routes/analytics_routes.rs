...
{
  app.service(web::scope("/v1/analytics").service(web::resource("/active_user").route(web::post().to(log_app_active_user_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/active_user_v2").route(web::post().to(log_app_active_user_json_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
