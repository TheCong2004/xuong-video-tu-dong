...
{
  app.service(web::scope("/v1/beta_keys").service(web::resource("/list").route(web::get().to(list_beta_keys_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/create").route(web::post().to(create_beta_keys_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/redeem").route(web::post().to(redeem_beta_key_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/{token}/note").route(web::post().to(edit_beta_key_note_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/{token}/distributed").route(web::post().to(edit_beta_key_distributed_flag_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
