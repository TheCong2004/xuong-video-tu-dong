...
{
  app.service(web::scope("/v1/model_download").service(web::resource("/gsv").route(web::head().to(|| HttpResponse::Ok())).route(web::post().to(enqueue_gptsovits_model_download_handler))))
}
