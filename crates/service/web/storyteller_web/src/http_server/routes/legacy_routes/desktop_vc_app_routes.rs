...
{
  app.service(web::scope("/v1/vc").service(web::resource("/report_analytics").route(web::post().to(post_app_analytics_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/news").route(web::get().to(get_app_news_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/features").route(web::get().to(get_app_plans_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/downloads").route(web::get().to(get_app_model_downloads_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
