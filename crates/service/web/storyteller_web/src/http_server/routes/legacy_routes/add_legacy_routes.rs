  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  ...
  let mut app = RouteBuilder::from_app(app).add_post("/v1/animation/face_animation/create", enqueue_face_animation_handler).add_post("/v1/animation/rerender/create", enqueue_rerender_animation_handler).into_app();

  // ==================== Format Conversion ====================

  let mut app = RouteBuilder::from_app(app).add_post("/v1/conversion/enqueue_fbx_to_gltf", enqueue_fbx_to_gltf_handler).into_app();

  // =================== BVH from Workflow ====================

  // TODO(bt,2024-03-15): Migrate from "bvh_to_workflow" to "render_engine_scene"
  let mut app = RouteBuilder::from_app(app).add_post("/v1/conversion/enqueue_bvh_to_workflow", enqueue_render_engine_scene_to_video_handler).add_post("/v1/conversion/enqueue_render_engine_scene", enqueue_render_engine_scene_to_video_handler).into_app();

  // ==================== FakeYou Frontend Application State ====================

  let mut app = RouteBuilder::from_app(app).add_get("/v1/app_state", get_app_state_handler).into_app();

  // ==================== Stats ====================

  // NB: Deprecated; serves a hardcoded snapshot for legacy clients that still poll it.
  #[allow(deprecated)]
  let mut app = RouteBuilder::from_app(app).add_get("/v1/stats/queues", get_unified_queue_stats_handler).into_app();

  let app = app.service(web::resource("/events").route(web::get().to(list_events_handler)).route(web::head().to(|| HttpResponse::Ok())));

  let app = app.service(enable_alpha_handler).service(enable_alpha_easy_handler);
...
where
  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  app.service(web::scope("/vocoder").service(web::resource("/list").route(web::get().to(list_vocoders_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/model/{token}").route(web::get().to(get_vocoder_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
...
where
  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  app.service(web::scope("/v1/category").service(web::scope("/list").service(web::resource("/tts").route(web::get().to(list_tts_categories_handler)).route(web::head().to(|| HttpResponse::Ok())))).service(web::scope("/computed_assignments").service(web::resource("/tts").route(web::get().to(list_fully_computed_assigned_tts_categories_handler)).route(web::head().to(|| HttpResponse::Ok()))))).service(
    web::scope("/category")
      .service(web::resource("/create").route(web::post().to(create_category_handler)).route(web::head().to(|| HttpResponse::Ok())))
      .service(web::resource("/view/{token}").route(web::get().to(get_category_handler)).route(web::head().to(|| HttpResponse::Ok())))
      .service(web::scope("/list").service(web::resource("/tts")
                        .route(web::get().to(list_tts_categories_handler)) // TODO: Deprecate with use of /v1* copy
                        .route(web::head().to(|| HttpResponse::Ok()))))
      .service(web::scope("/assign").service(web::resource("/tts").route(web::post().to(assign_tts_category_handler)).route(web::head().to(|| HttpResponse::Ok())))),
  )
}
...
where
  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  app.service(web::scope("/api_tokens").service(web::resource("/create").route(web::post().to(create_api_token_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/edit").route(web::post().to(edit_api_token_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/{api_token}/delete").route(web::post().to(delete_api_token_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/list").route(web::get().to(list_api_tokens_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
...
where
  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  app.service(web::scope("/flags").service(web::scope("/design_refresh").service(web::resource("/enable").route(web::get().to(enable_design_refresh_flag_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/disable").route(web::get().to(disable_design_refresh_flag_handler)).route(web::head().to(|| HttpResponse::Ok())))))
}
...
where
  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  app.service(web::scope("/v1/media_uploads").service(web::resource("/upload").route(web::post().to(upload_media_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/upload_audio").route(web::post().to(upload_audio_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/upload_image").route(web::post().to(upload_image_handler)).route(web::head().to(|| HttpResponse::Ok()))).service(web::resource("/by_session/{media_type}").route(web::get().to(list_user_media_uploads_of_type_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
...
where
  ...
  T: ServiceFactory<ServiceRequest, Config = (), Response = ServiceResponse<B>, Error = Error, InitError = ()>,
{
  app.service(web::scope("/v1/trending").service(web::resource("/tts_models").route(web::get().to(list_trending_tts_models_handler)).route(web::head().to(|| HttpResponse::Ok()))))
}
