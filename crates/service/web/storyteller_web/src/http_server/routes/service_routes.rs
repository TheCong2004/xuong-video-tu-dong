...
{
  app
    .service(web::resource("/_status").route(web::get().to(get_health_check_handler)).route(web::head().to(|| HttpResponse::Ok())))
    .service(
      // TODO(bt,2023-01-21): Couldn't scope to /v1/, actix routing table might not like collision
      web::resource("/server_info").route(web::get().to(get_public_info_handler)).route(web::head().to(|| HttpResponse::Ok())),
    )
    .service(web::resource("/detect_locale").route(web::get().to(detect_locale_handler)).route(web::head().to(|| HttpResponse::Ok())))
    .service(web::resource("/v1/status_alert_check").route(web::get().to(status_alert_handler)).route(web::head().to(|| HttpResponse::Ok())))
    .service(web::resource("/").route(web::get().to(get_root_handler)).route(web::head().to(|| HttpResponse::Ok())))
