...
impl RequestDebuggingMetadata {
  ...
  pub(crate) fn from_service_request(req: &ServiceRequest) -> Self {
    ...
    let trace_id = http_request.extensions().get::<TraceId>().map(|t| t.to_string());

    let server_state = req.app_data::<web::Data<Arc<ServerState>>>();

    let avt_cookie_token = server_state
      .and_then(|state| {
        state.avt_cookie_manager.decode_cookie_payload_from_request(http_request).unwrap_or_else(|e| {
    ...
    let (session_token, session_user_token) = server_state
      .and_then(|state| {
        state.session_cookie_manager.decode_session_payload_from_request(http_request).unwrap_or_else(|e| {
    ...
    let http_host = Some(http_request.connection_info().host().to_string()).filter(|host| !host.is_empty());
    ...
    let artcraft_version = get_header_value(http_request, HeaderName::from_static(ARTCRAFT_VERSION_HEADER));

    Self { request_ip_address, avt_cookie_token, session_token, session_user_token, trace_id, http_host, http_origin, http_referer, http_user_agent, artcraft_version }
  }
}
...
fn get_header_value(request: &HttpRequest, header_name: HeaderName) -> Option<String> {
  request.headers().get(header_name).and_then(|value| value.to_str().ok()).map(|value| value.to_string())
}
