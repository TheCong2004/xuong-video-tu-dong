...
pub fn build_pager(server_environment: server_environment::ServerEnvironment, hostname: &str) -> (Pager, PagerWorker, PagingFlags) {
  ...
  let builder = PagerBuilder::new().application_name("storyteller-web".to_string()).environment(environment.to_string()).hostname(hostname.to_string()).service_id(ROOTLY_SERVICE_ID_STORYTELLER_WEB.to_string());
