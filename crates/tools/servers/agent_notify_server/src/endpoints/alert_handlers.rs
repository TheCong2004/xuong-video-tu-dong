...
pub async fn alert_await_handler(state: web::Data<ServerState>) -> impl Responder {
  play_once_or_404(&state, state.config.alert_await_user_input_sound.clone(), "alert_await_user_input_sound")
}

fn play_once_or_404(state: &ServerState, maybe_path: Option<PathBuf>, config_key: &str) -> HttpResponse {
  match maybe_path {
    Some(path) => {
