...
use crate::endpoints::alert_handlers::{alert_await_handler, alert_beep_handler, alert_done_handler};
...
async fn main() -> anyhow::Result<()> {
  ...
  let bind_address = env::var("HTTP_BIND_ADDRESS").unwrap_or_else(|_| DEFAULT_BIND_ADDRESS.to_string());

  let config_path: PathBuf = env::var("NOTIFY_CONFIG_PATH").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from(DEFAULT_CONFIG_PATH));

  let config = Arc::new(NotifyConfig::read_from_file_or_default(&config_path));

  let (audio_handle, audio_thread) = spawn_audio_player();

  let state = ServerState { config: config.clone(), audio: audio_handle.clone() };

  log::info!("agent-notify-server listening on http://{}", bind_address);

  let server = HttpServer::new(move || App::new().app_data(Data::new(state.clone())).wrap(Logger::default()).route("/", web::get().to(root_handler)).route("/alert_beep", web::get().to(alert_beep_handler)).route("/alert_done", web::get().to(alert_done_handler)).route("/alert_await", web::get().to(alert_await_handler)).route("/loop_beep", web::get().to(loop_beep_handler)).route("/loop_done", web::get().to(loop_done_handler)).route("/loop_await", web::get().to(loop_await_handler)).route("/stop", web::get().to(stop_handler)).route("/state", web::get().to(state_handler))).bind(&bind_address)?.workers(1).shutdown_timeout(0).run().await?;
