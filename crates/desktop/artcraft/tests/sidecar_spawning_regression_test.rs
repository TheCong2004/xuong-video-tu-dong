...
fn test_sidecar_startup_files_use_background_command_for_all_spawns() {
  ...
  for file_name in &sidecar_files {
    ...
    assert!(
      file_path.exists(),
      "Target sidecar file does not exist: {}",
      file_path.display()
    );
    ...
    assert!(
      content.contains("background_command"),
      "File {} does not import or use background_command",
      file_name
    );
    ...
    for (line_idx, line) in content.lines().enumerate() {
      ...
      if trimmed.contains("Command::new(") {
        assert!(
          trimmed.contains("background_command(Command::new("),
          "Unshielded Command::new found at {}:{} -> {}",
          file_name,
          line_idx + 1,
          trimmed
        );
