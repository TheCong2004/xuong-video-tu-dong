...
impl AppDataRoot {
  ...
  pub fn pipeline_artifacts_dir(&self) -> PathBuf {
...
mod tests {
  ...
  #[test]
  fn pipeline_artifacts_live_under_the_resolved_app_data_root() {
    let temp = tempfile::tempdir().unwrap();
    let root = AppDataRoot::create_existing(temp.path()).unwrap();

    assert_eq!(root.pipeline_artifacts_dir(), root.path().join("artifacts"));
    assert!(!root.pipeline_artifacts_dir().starts_with(env!("CARGO_MANIFEST_DIR")));
  }
}
