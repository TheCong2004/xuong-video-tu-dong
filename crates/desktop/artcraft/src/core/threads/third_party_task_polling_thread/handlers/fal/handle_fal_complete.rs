...
use crate::core::threads::third_party_task_polling_thread::events::notify_frontend_of_completion::{notify_frontend_of_completion, CompletionData};
...
pub async fn handle_fal_complete(app_handle: &AppHandle, app_env_configs: &AppEnvConfigs, app_data_root: &AppDataRoot, task_database: &TaskDatabase, storyteller_creds_manager: &StorytellerCredentialManager, task: &Task, job_response: PollJobResponse) {
  info!("[FalComplete] Handling completed task {}", task.id.as_str());

  let result = handle_fal_complete_inner(app_handle, app_env_configs, app_data_root, task_database, storyteller_creds_manager, task, job_response).await;
...
async fn handle_fal_complete_inner(app_handle: &AppHandle, app_env_configs: &AppEnvConfigs, app_data_root: &AppDataRoot, task_database: &TaskDatabase, storyteller_creds_manager: &StorytellerCredentialManager, task: &Task, job_response: PollJobResponse) -> Result<(), Box<dyn std::error::Error>> {
  let creds = storyteller_creds_manager.get_credentials()?.ok_or("No Storyteller credentials available")?;
  ...
  for (i, url) in urls.iter().enumerate() {
    ...
    let media_token = upload_to_backend(&creds, app_env_configs, &download_path, maybe_prompt_token.as_ref(), maybe_batch_token.as_ref(), media_class).await?;
  ...
  let updated = update_successful_task_status_with_metadata(UpdateSuccessfulTaskArgs { db: task_database.get_connection(), task_id: &task.id, maybe_batch_token: maybe_batch_token.as_ref(), maybe_primary_media_file_token: maybe_primary_media_file_token.as_ref(), maybe_primary_media_file_class: Some(media_class), maybe_primary_media_file_cdn_url: maybe_cdn_url_str.as_deref(), maybe_primary_media_file_thumbnail_url_template: maybe_thumbnail_url_template.as_deref() }).await?;

  if updated {
    if let Some(primary_token) = maybe_primary_media_file_token {
      let completion = CompletionData { primary_media_file_token: primary_token, maybe_cdn_url, maybe_thumbnail_url_template, maybe_batch_token, media_class };

      notify_frontend_of_completion(app_handle, &app_env_configs.storyteller_host, Some(&creds), task, &completion).await;
    }
  }
...
fn collect_media_urls(task: &Task, extracted: &Option<PollResponseExtractedContents>) -> Result<(Vec<String>, TaskMediaFileClass), Box<dyn std::error::Error>> {
  ...
  if let Some(images) = &extracted.images {
    let urls: Vec<String> = images.iter().filter_map(|img| img.url.clone()).collect();
    if !urls.is_empty() {
...
async fn download_file(url: &str, app_data_root: &AppDataRoot, index: usize) -> Result<PathBuf, Box<dyn std::error::Error>> {
  let response = reqwest::get(url).await?;
  let bytes = response.bytes().await?;

  let extension = url_utils::download_extension::extract_download_extension_from_url::extract_download_extension_from_url_str(url).map(|ext| ext.as_extension_without_period()).unwrap_or("bin");
...
async fn upload_to_backend(creds: &StorytellerCredentialSet, app_env_configs: &AppEnvConfigs, download_path: &PathBuf, maybe_prompt_token: Option<&PromptToken>, maybe_batch_token: Option<&BatchGenerationToken>, media_class: TaskMediaFileClass) -> Result<MediaFileToken, Box<dyn std::error::Error>> {
    ...
    match result {
      ...
      Err(StorytellerError::Api(ApiError::TooManyRequests(_))) => {
        if attempt + 1 < MAX_UPLOAD_RETRIES {
          warn!("[FalComplete] Upload rate-limited (429), retrying in {}s (attempt {}/{})", retry_delay_secs, attempt + 1, MAX_UPLOAD_RETRIES,);
          ...
          return Err(Box::new(StorytellerError::Api(ApiError::TooManyRequests("Max retries exceeded".to_string()))));
        }
      },
...
async fn try_upload(creds: &StorytellerCredentialSet, app_env_configs: &AppEnvConfigs, download_path: &PathBuf, maybe_prompt_token: Option<&PromptToken>, maybe_batch_token: Option<&BatchGenerationToken>, media_class: TaskMediaFileClass) -> Result<MediaFileToken, StorytellerError> {
  let media_token = match media_class {
    TaskMediaFileClass::Video => {
      let result = upload_video_media_file_from_file(UploadVideoFromFileArgs { api_host: &app_env_configs.storyteller_host, maybe_creds: Some(creds), path: download_path, maybe_prompt_token, maybe_generation_provider: Some(GenerationProvider::Fal) }).await?;
      result.media_file_token
    },
    TaskMediaFileClass::Dimensional => {
      let result = legacy_upload_media_file_from_file(LegacyUploadMediaFileFromFileArgs { api_host: &app_env_configs.storyteller_host, maybe_creds: Some(creds), path: download_path, maybe_generation_provider: Some(GenerationProvider::Fal) }).await?;
      result.media_file_token
    },
    _ => {
      let result = upload_image_media_file_from_file(UploadImageFromFileArgs { api_host: &app_env_configs.storyteller_host, maybe_creds: Some(creds), path: download_path, is_intermediate_system_file: false, maybe_prompt_token, maybe_batch_token, maybe_generation_provider: Some(GenerationProvider::Fal) }).await?;
      result.media_file_token
    },
  };
