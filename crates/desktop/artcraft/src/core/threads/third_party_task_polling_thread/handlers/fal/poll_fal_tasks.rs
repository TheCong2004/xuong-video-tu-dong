...
pub async fn poll_fal_tasks(app_handle: &AppHandle, app_env_configs: &AppEnvConfigs, app_data_root: &AppDataRoot, task_database: &TaskDatabase, storyteller_creds_manager: &StorytellerCredentialManager, credential_cache: &ProviderCredentialLoadingCache, fal_tasks: &[&Task]) {
  ...
  for task in fal_tasks {
    let result = poll_single_fal_task(app_handle, app_env_configs, app_data_root, task_database, storyteller_creds_manager, &api_key, task).await;

    if let Err(err) = result {
      error!("[FalPolling] Error processing task {}: {:?}", task.id.as_str(), err,);
    }
  }
}

async fn poll_single_fal_task(app_handle: &AppHandle, app_env_configs: &AppEnvConfigs, app_data_root: &AppDataRoot, task_database: &TaskDatabase, storyteller_creds_manager: &StorytellerCredentialManager, api_key: &FalApiKey, task: &Task) -> Result<(), Box<dyn std::error::Error>> {
  let status_url = match &task.queue_status_url {
    Some(url) => url.as_str(),
    None => {
      warn!("[FalPolling] Task {} has no queue_status_url, marking as failed", task.id.as_str());
      handle_fal_failure(app_handle, task_database, task, "No queue status URL to check").await;
      return Ok(());
    },
  };
  ...
  let status_response = poll_job_status(PollJobStatusArgs { status_url, api_key }).await?;

  match status_response.status {
    FalJobStatus::InQueue | FalJobStatus::InProgress => {
      info!("[FalPolling] Task {} still {:?} (queue_position={:?})", task.id.as_str(), status_response.status, status_response.maybe_queue_position,);
      return Ok(());
    },
  ...
  let response_url = match response_url {
    ...
    None => {
      ...
      handle_fal_failure(app_handle, task_database, task, "Job completed but no response URL available").await;
      return Ok(());
    },
  };

  let job_response = match poll_job_response(PollJobResponseArgs { response_url: &response_url, api_key }).await {
    Ok(response) => response,
    Err(err) => {
      error!("[FalPolling] Failed to fetch response for task {}: {}. Skipping for now.", task.id.as_str(), err,);
      return Ok(());
    },
  };

  // Step 3: Handle completed job
  handle_fal_complete(app_handle, app_env_configs, app_data_root, task_database, storyteller_creds_manager, task, job_response).await;
...
fn load_fal_api_key(credential_cache: &ProviderCredentialLoadingCache) -> Option<FalApiKey> {
  match credential_cache.get_credentials(ProviderCredentialKey::FalApiKey) {
    Ok(Some(ProviderCredentialPayload::ApiKey(data))) => Some(FalApiKey::from_str(data.as_str())),
    _ => None,
  }
}
  ...
  match (&task.queue_response_url, polled_response_url) {
    (Some(db_url), Some(polled_url)) => {
      if db_url != polled_url {
        warn!("[FalPolling] Task {} response URLs differ: db={}, polled={}. Using db URL.", task.id.as_str(), db_url, polled_url,);
      }
      Some(db_url.clone())
    },
