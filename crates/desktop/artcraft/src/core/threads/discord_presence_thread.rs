...
async fn discord_main_loop(mut client: DiscordIpcClient) -> AnyhowResult<()> {
  ...
  client.connect().map_err(|err| anyhow!("Error connecting to Discord IPC: {:?}", err))?;
  ...
  loop {
    ...
    let assets = discord_rich_presence::activity::Assets::new().large_image(discord_logo_image).large_text("https://GetArtCraft.com");
    //.small_image("https://storyteller.ai/android-chrome-192x192.png")
    //.small_text("small text");

    client
      .set_activity(discord_rich_presence::activity::Activity::new()
