...
pub async fn main_window_thread(app: AppHandle, app_data_root: AppDataRoot, storyteller_creds_manager: StorytellerCredentialManager) -> ! {
  ...
  info!("git commit id: {:?}", build_metadata::git_commit_id().unwrap_or_else(|| "unknown"));
  info!("git commit timestamp: {:?}", build_metadata::git_commit_timestamp().map(|t| t.to_string()).unwrap_or_else(|| "unknown".to_string()));
  info!("build timestamp: {:?}", build_metadata::build_timestamp().to_string());
  ...
  loop {
    for (window_name, window) in app.windows() {
      if window_name == MAIN_WINDOW_NAME {
        let result = handle_main_window(&app, &window, &app_data_root, &storyteller_creds_manager, &window_size_slot, &window_pos_slot, &app_startup_time).await;
...
pub async fn handle_main_window(app: &AppHandle, window: &Window, app_data_root: &AppDataRoot, storyteller_creds_manager: &StorytellerCredentialManager, window_size_slot: &CloneSlot<MainWindowSize>, window_pos_slot: &CloneSlot<MainWindowPosition>, app_startup_time: &AppStartupTime) -> AnyhowResult<()> {
  loop {
