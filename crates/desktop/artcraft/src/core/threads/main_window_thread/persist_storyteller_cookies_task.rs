...
pub async fn persist_storyteller_cookies_task(app: &AppHandle, storyteller_credential_manager: &StorytellerCredentialManager, app_startup_time: &AppStartupTime) -> AnyhowResult<()> {
  ...
  sync_storyteller_credentials_from_http_plugin(app, storyteller_credential_manager)
}

/// Synchronize the canonical ArtCraft account cookies persisted by
/// `tauri-plugin-http` into the in-memory credential manager. Floword calls
/// this before status/test operations so it reuses the existing login flow and
/// does not depend on the one-second main-window polling interval.
pub(crate) fn sync_storyteller_credentials_from_http_plugin(app: &AppHandle, storyteller_credential_manager: &StorytellerCredentialManager) -> AnyhowResult<()> {
