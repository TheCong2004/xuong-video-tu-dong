...
pub async fn upload_image_bytes_as_media_file(creds: &StorytellerCredentialSet, api_host: &ApiHost, image_bytes: Vec<u8>) -> Result<MediaFileToken, GenerateError> {
  info!("Uploading image bytes ({} bytes) as media file...", image_bytes.len());

  let result = upload_image_media_file_from_bytes(UploadImageBytesArgs { api_host, maybe_creds: Some(creds), image_bytes, image_type: ImageType::Png, is_intermediate_system_file: false, maybe_generation_provider: None }).await.map_err(|err| {
