...
pub async fn upload_mask_image_bytes_as_media_file(creds: &StorytellerCredentialSet, api_host: &ApiHost, mask_image_bytes: &[u8]) -> Result<MediaFileToken, GenerateError> {
  info!("Normalizing mask image bytes ({} bytes)...", mask_image_bytes.len());

  let normalized = normalize_image_bytes_to_flux_mask(mask_image_bytes).map_err(|err| {
  ...
  let result = upload_image_media_file_from_bytes(UploadImageBytesArgs { api_host, maybe_creds: Some(creds), image_bytes: normalized.0, image_type: ImageType::Png, is_intermediate_system_file: true, maybe_generation_provider: None }).await.map_err(|err| {
