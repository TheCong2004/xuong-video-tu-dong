...
fn multipart_upload(file_path: String, parts_urls: Vec<String>, chunk_size: u64, max_files: usize, parallel_failures: usize, max_retries: usize) -> Result<Vec<HashMap<String, String>>, Error> {
  if parallel_failures > max_files {
    return Err(Error::new_err("Error parallel_failures cannot be > max_files".to_string()));
  }
  if (parallel_failures == 0) != (max_retries == 0) {
    return Err(Error::new_err("For retry mechanism you need to set both `parallel_failures` and `max_retries`".to_string()));
  }

  tokio::runtime::Builder::new_multi_thread().enable_all().build()?.block_on(async { upload_async(file_path, parts_urls, chunk_size, max_files, parallel_failures, max_retries).await })
}

#[allow(clippy::too_many_arguments)]
async fn upload_async(file_path: String, parts_urls: Vec<String>, chunk_size: u64, max_files: usize, parallel_failures: usize, max_retries: usize) -> Result<Vec<HashMap<String, String>>, Error> {
  ...
  for (part_number, part_url) in parts_urls.iter().enumerate() {
    ...
    handles.push(tokio::spawn(async move {
      let permit = semaphore.clone().acquire_owned().await.map_err(|err| Error::new_err(format!("Error acquiring semaphore: {err}")))?;
      let mut chunk = upload_chunk(&client, &url, &path, start, chunk_size).await;
      let mut i = 0;
      if parallel_failures > 0 {
        while let Err(ul_err) = chunk {
          if i >= max_retries {
            return Err(Error::new_err(format!("Failed after too many retries ({max_retries}): {ul_err}")));
          }

          let parallel_failure_permit = parallel_failures_semaphore.clone().try_acquire_owned().map_err(|err| Error::new_err(format!("Failed too many failures in parallel ({parallel_failures}): {ul_err} ({err})")))?;
  ...
  while let Some(result) = handles.next().await {
    match result {
      ...
      Err(err) => {
        return Err(Error::new_err(format!("Error occurred while uploading: {err}")));
      },
    }
  }
...
async fn upload_chunk(client: &reqwest::Client, url: &str, path: &str, start: u64, chunk_size: u64) -> Result<HashMap<String, String>, Error> {
  ...
  let response = client.put(url).header(CONTENT_LENGTH, bytes_transferred).body(reqwest::Body::wrap_stream(FramedRead::new(chunk, BytesCodec::new()))).send().await?;
