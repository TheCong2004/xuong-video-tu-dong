...
pub fn resize_main_window(app: &AppHandle, size: &MainWindowSize) -> errors::AnyhowResult<()> {
  ...
  if resize_was_corrected {
    println!("Resizing window to: {}x{} (corrected from {}x{})", resize_width, resize_height, size.width, size.height);
  } else {
