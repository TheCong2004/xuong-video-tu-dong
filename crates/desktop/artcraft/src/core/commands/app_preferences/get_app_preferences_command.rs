...
#[tauri::command]
pub async fn get_app_preferences_command(app_prefs: State<'_, AppPreferencesManager>) -> Result<GetAppPreferencesResponse, String> {
  info!("get_app_preferences_command called");

  let result = get_prefs(&app_prefs).await.map_err(|err| {
    error!("Error getting app preferences: {:?}", err);
    format!("Error getting app preferences: {:?}", err)
  })?;

  Ok(GetAppPreferencesResponse { preferences: result })
}

async fn get_prefs(app_prefs: &AppPreferencesManager) -> AnyhowResult<AppPreferencesPayload> {
  let prefs = app_prefs.get_clone()?;
  Ok(AppPreferencesPayload { preferred_download_directory: prefs.preferred_download_directory, play_sounds: prefs.play_sounds, delete_file_sound: prefs.delete_file_sound, enqueue_success_sound: prefs.enqueue_success_sound, enqueue_failure_sound: prefs.enqueue_failure_sound, generation_success_sound: prefs.generation_success_sound, generation_failure_sound: prefs.generation_failure_sound, generation_enqueue_sound: prefs.generation_enqueue_sound })
}
