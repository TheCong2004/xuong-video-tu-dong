...
#[tauri::command]
pub async fn estimate_image_cost_command(request: EstimateImageCostRequest, app_env_configs: State<'_, AppEnvConfigs>) -> ResponseOrError<EstimateImageCostResponse, EstimateImageCostError> {
  ...
  match result {
    ...
    Err(err) => Err(CommandErrorResponseWrapper { status: CommandErrorStatus::BadRequest, error_message: None, error_type: None, error_details: Some(EstimateImageCostError { success: false, error_type: EstimateImageCostErrorType::InvalidInput, error_message: err.to_string() }) }),
  }
}
