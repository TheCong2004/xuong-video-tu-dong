...
#[tauri::command]
pub async fn estimate_splat_cost_command(request: EstimateSplatCostRequest, app_env_configs: State<'_, AppEnvConfigs>) -> ResponseOrError<EstimateSplatCostResponse, EstimateSplatCostError> {
  ...
  match result {
    ...
    Err(err) => Err(CommandErrorResponseWrapper { status: CommandErrorStatus::BadRequest, error_message: None, error_type: None, error_details: Some(EstimateSplatCostError { success: false, error_type: EstimateSplatCostErrorType::InvalidInput, error_message: err.to_string() }) }),
  }
}
