...
#[tauri::command]
pub async fn estimate_video_cost_command(request: EstimateVideoCostRequest, app_env_configs: State<'_, AppEnvConfigs>) -> ResponseOrError<EstimateVideoCostResponse, EstimateVideoCostError> {
  ...
  match result {
    ...
    Err(err) => Err(CommandErrorResponseWrapper { status: CommandErrorStatus::BadRequest, error_message: None, error_type: None, error_details: Some(EstimateVideoCostError { success: false, error_type: EstimateVideoCostErrorType::InvalidInput, error_message: err.to_string() }) }),
  }
}
