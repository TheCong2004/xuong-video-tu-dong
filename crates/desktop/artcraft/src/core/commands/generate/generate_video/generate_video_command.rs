...
use crate::core::commands::generate::generate_video::request::{TauriGenerateVideoErrorType, TauriGenerateVideoRequest, TauriGenerateVideoResponse, TauriVideoModel};
...
#[tauri::command]
pub async fn generate_video_command(mut request: TauriGenerateVideoRequest, app: AppHandle, app_env_configs: State<'_, AppEnvConfigs>, app_data_root: State<'_, AppDataRoot>, artcraft_usage_tracker: State<'_, ArtcraftUsageTracker>, provider_priority_store: State<'_, ProviderPriorityStore>, task_database: State<'_, TaskDatabase>, grok_creds_manager: State<'_, GrokCredentialManager>, storyteller_creds_manager: State<'_, StorytellerCredentialManager>, sora_task_queue: State<'_, SoraTaskQueue>, sora_creds_manager: State<'_, SoraCredentialManager>) -> Response<TauriGenerateVideoResponse, TauriGenerateVideoErrorType, ()> {
  ...
  let result = handle_request(request, &app, &app_env_configs, &app_data_root, &artcraft_usage_tracker, &provider_priority_store, &task_database, &grok_creds_manager, &sora_creds_manager, &storyteller_creds_manager).await;
...
async fn handle_request(request: TauriGenerateVideoRequest, app: &AppHandle, app_env_configs: &AppEnvConfigs, app_data_root: &AppDataRoot, artcraft_usage_tracker: &ArtcraftUsageTracker, provider_priority_store: &ProviderPriorityStore, task_database: &TaskDatabase, grok_creds_manager: &GrokCredentialManager, sora_creds_manager: &SoraCredentialManager, storyteller_creds_manager: &StorytellerCredentialManager) -> Result<TaskEnqueueSuccess, GenerateError> {
  ...
  let result = match provider {
    GenerationProvider::Grok => handle_grok_video(&request, app, app_data_root, app_env_configs, grok_creds_manager).await,
    GenerationProvider::Sora => handle_sora_sora2(&request, app, app_data_root, app_env_configs, sora_creds_manager).await,
    _ => handle_video_artcraft(&request, app, app_env_configs, storyteller_creds_manager).await,
  };
