...
pub async fn handle_flux_2_lora_angles(request: &TauriGenerateImageRequest, semantic_media_files: &SemanticMediaFiles, creds: &StorytellerCredentialSet, app_env_configs: &AppEnvConfigs) -> Result<TaskEnqueueSuccess, GenerateError> {
  ...
  let router_request = GenerateImageRequestBuilder { model: RouterImageModel::Flux2LoraAngles, provider: RouterProvider::Artcraft, prompt: request.prompt.clone(), image_inputs, resolution: None, aspect_ratio: None, quality: None, image_batch_count: request.batch_size.map(|n| n as u16), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::PayMoreUpgrade, generation_mode_mismatch_strategy: None, idempotency_token: None, horizontal_angle: request.adjust_horizontal_angle, vertical_angle: request.adjust_vertical_angle, zoom: request.adjust_zoom };
  ...
  let request = match dor {
    ...
    ImageGenerationDraftOrRequest::Draft(_) => unreachable!("Artcraft Flux2LoraAngles should never produce a draft"),
  };
  ...
  Ok(TaskEnqueueSuccess { task_type: TaskType::ImageGeneration, model: Some(GenerationModel::Flux2LoraAngles), provider: GenerationProvider::Artcraft, provider_job_id: Some(job_id), maybe_queue_status_url: None, maybe_prompt_token: None, maybe_queue_response_url: None })
}
