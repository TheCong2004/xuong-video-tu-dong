...
use crate::core::commands::generate::generate_image::tauri_generate_image_request::{TauriGenerateImageErrorType, TauriGenerateImageRequest, TauriGenerateImageResponse};
...
#[tauri::command]
pub async fn generate_image_command(request: TauriGenerateImageRequest, app: AppHandle, app_env_configs: State<'_, AppEnvConfigs>, task_database: State<'_, TaskDatabase>, storyteller_creds_manager: State<'_, StorytellerCredentialManager>, credential_cache: State<'_, ProviderCredentialLoadingCache>) -> Response<TauriGenerateImageResponse, TauriGenerateImageErrorType, ()> {
  ...
  let result = match provider {
    GenerationProvider::Artcraft => handle_artcraft(&request, &app_env_configs, &storyteller_creds_manager).await,
    // Midjourney uses its own legacy command path, not this one.
    GenerationProvider::Midjourney => Err(GenerateError::NotYetImplemented("Midjourney should use its dedicated command".to_string())),
    // All other providers go through the router.
    other => handle_router(&request, other, &app_env_configs, &credential_cache, &storyteller_creds_manager).await,
  };
...
async fn handle_success_behavior(app: &AppHandle, task_database: &TaskDatabase, request: &TauriGenerateImageRequest, success: TaskEnqueueSuccess) -> Response<TauriGenerateImageResponse, TauriGenerateImageErrorType, ()> {
  // Insert into task database
  let db_result = success.insert_into_task_database_with_frontend_payload(&task_database, request.frontend_caller, request.frontend_subscriber_id.as_deref(), request.frontend_subscriber_payload.as_deref()).await;
  ...
  let event = GenerationEnqueueSuccessEvent { action: success.to_frontend_event_action(), service: success.to_frontend_event_service(), model: success.model };
...
async fn handle_error_behavior(app: &AppHandle, err: GenerateError) -> Response<TauriGenerateImageResponse, TauriGenerateImageErrorType, ()> {
  ...
  */
  ...
  Err(CommandErrorResponseWrapper { status: CommandErrorStatus::ServerError, error_message: Some(format!("{:?}", err)), error_type: Some(error_type), error_details: None })
}
