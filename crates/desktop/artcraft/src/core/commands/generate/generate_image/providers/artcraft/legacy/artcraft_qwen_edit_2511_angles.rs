...
pub async fn handle_qwen_edit_2511_angles(request: &TauriGenerateImageRequest, semantic_media_files: &SemanticMediaFiles, creds: &StorytellerCredentialSet, app_env_configs: &AppEnvConfigs) -> Result<TaskEnqueueSuccess, GenerateError> {
  let client = RouterClient::Artcraft(RouterArtcraftClient::new(app_env_configs.storyteller_host.clone(), creds.clone()));

  let image_inputs = build_image_inputs(request, semantic_media_files);

  let router_request = GenerateImageRequestBuilder { model: RouterImageModel::QwenEdit2511Angles, provider: RouterProvider::Artcraft, prompt: request.prompt.clone(), image_inputs, resolution: None, aspect_ratio: None, quality: None, image_batch_count: request.batch_size.map(|n| n as u16), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::PayMoreUpgrade, generation_mode_mismatch_strategy: None, idempotency_token: None, horizontal_angle: request.adjust_horizontal_angle, vertical_angle: request.adjust_vertical_angle, zoom: request.adjust_zoom };

  let dor = router_request.build2()?;

  info!("QwenEdit2511Angles request: {:?}", dor);
  ...
  let request = match dor {
    ...
    ImageGenerationDraftOrRequest::Draft(_) => unreachable!("Artcraft QwenEdit2511Angles should never produce a draft"),
  };
  ...
  Ok(TaskEnqueueSuccess { task_type: TaskType::ImageGeneration, model: Some(GenerationModel::QwenEdit2511Angles), provider: GenerationProvider::Artcraft, provider_job_id: Some(job_id), maybe_queue_status_url: None, maybe_prompt_token: None, maybe_queue_response_url: None })
}

fn build_image_inputs(request: &TauriGenerateImageRequest, semantic_media_files: &SemanticMediaFiles) -> Option<ImageListRef> {
