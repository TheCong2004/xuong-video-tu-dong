...
pub async fn handle_flux_pro_kontext_edit(request: &TauriGenerateImageRequest, semantic_media_files: &SemanticMediaFiles, creds: &StorytellerCredentialSet, app_env_configs: &AppEnvConfigs) -> Result<TaskEnqueueSuccess, GenerateError> {
  let image_media_token = get_first_image_token(request, semantic_media_files).ok_or(GenerateError::BadInput(BadInputReason::WrongImageArguments("No input image specified for Flux Kontext Max edit".to_string())))?;
  ...
  let kontext_request = FluxProKontextMaxEditImageRequest { uuid_idempotency_token, prompt: request.prompt.clone(), image_media_token, num_images };

  info!("Calling FluxProKontextMax edit...");

  let response = flux_pro_kontext_max_edit_image(&app_env_configs.storyteller_host, Some(creds), kontext_request).await.map_err(|err| {
  ...
  Ok(TaskEnqueueSuccess { task_type: TaskType::ImageGeneration, model: Some(GenerationModel::FluxProKontextMax), provider: GenerationProvider::Artcraft, provider_job_id: Some(response.inference_job_token.to_string()), maybe_queue_status_url: None, maybe_prompt_token: None, maybe_queue_response_url: None })
}
