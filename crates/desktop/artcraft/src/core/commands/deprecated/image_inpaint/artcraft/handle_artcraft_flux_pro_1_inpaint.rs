...
pub async fn handle_artcraft_flux_pro_1_inpaint(request: &EnqueueInpaintImageCommand, app: &AppHandle, app_data_root: &AppDataRoot, app_env_configs: &AppEnvConfigs, storyteller_creds_manager: &StorytellerCredentialManager) -> Result<TaskEnqueueSuccess, GenerateError> {
  ...
  let request = FluxPro1InpaintImageRequest { uuid_idempotency_token, prompt: Some(request.prompt.clone()), image_media_token, mask_media_token, num_images };

  let result = flux_pro_1_inpaint_image(&app_env_configs.storyteller_host, Some(&creds), request).await;

  let job_id = match result {
    Ok(enqueued) => {
      // TODO(bt,2025-07-05): Enqueue job token?
      info!("Successfully enqueued Artcraft flux pro 1 inpaint. Job token: {}", enqueued.inference_job_token);
  ...
  Ok(TaskEnqueueSuccess { provider: GenerationProvider::Artcraft, model: Some(GenerationModel::FluxPro1), provider_job_id: Some(job_id.to_string()), task_type: TaskType::ImageInpaintEdit, maybe_queue_status_url: None, maybe_prompt_token: None, maybe_queue_response_url: None })
}
