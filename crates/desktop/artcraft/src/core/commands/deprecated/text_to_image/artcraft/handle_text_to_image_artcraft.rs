...
pub async fn handle_text_to_image_artcraft(model: TextToImageModel, request: &EnqueueTextToImageRequest, app: &AppHandle, app_data_root: &AppDataRoot, app_env_configs: &AppEnvConfigs, storyteller_creds_manager: &StorytellerCredentialManager) -> Result<TaskEnqueueSuccess, GenerateError> {
  match model {
    TextToImageModel::Midjourney | TextToImageModel::GrokImage => Err(GenerateError::BadProviderForModel { provider: GenerationProvider::Artcraft, model: text_to_image_model_to_model_type(model) }),
    TextToImageModel::Recraft3 => Err(GenerateError::NotYetImplemented(format!("not yet implemented in Artcraft"))),
    ...
    TextToImageModel::Gemini25Flash | TextToImageModel::NanoBanana => handle_text_to_image_artcraft_via_router(request, app_env_configs, storyteller_creds_manager, RouterImageModel::NanoBanana, GenerationModel::NanoBanana).await,
