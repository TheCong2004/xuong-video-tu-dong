...
#[tauri::command]
pub async fn download_media_file_command(request: DownloadMediaFileRequest, app: AppHandle, app_prefs: State<'_, AppPreferencesManager>, app_data_root: State<'_, AppDataRoot>, app_env_configs: State<'_, AppEnvConfigs>) -> ResponseOrErrorType<DownloadMediaFileSuccessResponse, DownloadMediaFileErrorType> {
  ...
  if let Err(err) = result {
    ...
    return Err(CommandErrorResponseWrapper { status: CommandErrorStatus::ServerError, error_message: Some("error downloading file".to_string()), error_type: Some(DownloadMediaFileErrorType::UnknownError), error_details: None });
  }

  Ok(DownloadMediaFileSuccessResponse {}.into())
}

pub async fn handle_request(request: DownloadMediaFileRequest, app: &AppHandle, app_prefs: &AppPreferencesManager, app_data_root: &AppDataRoot, app_env_configs: &AppEnvConfigs) -> Result<(), ArtcraftError> {
  ...
  let media_file = get_media_file(&app_env_configs.storyteller_host, &request.media_token).await?;

  let asset_url = media_file.media_file.media_links.cdn_url;

  let download_path = download_url_to_user_download_dir(&asset_url, app_data_root, &app_prefs).await?;
