...
#[tauri::command]
pub async fn download_directory_reveal_command(app: AppHandle, app_prefs: State<'_, AppPreferencesManager>, app_data_root: State<'_, AppDataRoot>) -> ResponseOrErrorMessage<DownloadDirectoryRevealSuccessResponse> {
  info!("download_directory_reveal_command called");

  let result = handle_request(&app, &app_prefs, &app_data_root).await;
...
pub async fn handle_request(app: &AppHandle, app_prefs: &AppPreferencesManager, app_data_root: &AppDataRoot) -> Result<(), ArtcraftError> {
  ...
  app.opener().reveal_item_in_dir(download_directory).map_err(|err| anyhow!("Failed to open directory: {:?}", err))?;
