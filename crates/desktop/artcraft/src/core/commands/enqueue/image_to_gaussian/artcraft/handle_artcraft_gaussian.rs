...
pub async fn handle_gaussian_artcraft(request: &EnqueueImageToGaussianRequest, app: &AppHandle, app_env_configs: &AppEnvConfigs, storyteller_creds_manager: &StorytellerCredentialManager) -> Result<TaskEnqueueSuccess, GenerateError> {
  let creds = get_storyteller_creds_or_error(app, storyteller_creds_manager)?;

  match request.model {
    None => Err(GenerateError::no_model_specified()),
    Some(GaussianModel::Marble0p1Mini) => handle_artcraft_splat_via_router(request, app_env_configs, &creds, RouterSplatModel::Marble0p1Mini, GenerationModel::WorldlabsMarble0p1Mini).await,
    Some(
      GaussianModel::Marble0p1Plus | GaussianModel::WorldLabsMarble, // Legacy variant — route to Plus
