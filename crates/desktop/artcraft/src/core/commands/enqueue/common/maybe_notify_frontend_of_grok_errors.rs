...
pub async fn maybe_notify_frontend_of_grok_errors(app: &AppHandle, error: &GenerateError) {
  match error {
...
fn grok_error(app: &AppHandle, error: &GrokError) {
  match error {
    GrokError::ApiSpecific(GrokSpecificApiError::TooManyVideos) => {
      let event = GenerationEnqueueFailureEvent { action: GenerationAction::GenerateVideo, service: GenerationServiceProvider::Grok, model: None, reason: Some("You've generated too many Grok videos. Please wait for more quota from Grok.".to_string()) };
