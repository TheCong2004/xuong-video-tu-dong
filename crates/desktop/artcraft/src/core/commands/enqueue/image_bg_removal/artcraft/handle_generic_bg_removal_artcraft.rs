...
pub async fn handle_generic_bg_removal_artcraft(request: &EnqueueImageBgRemovalCommand, app: &AppHandle, app_data_root: &AppDataRoot, app_env_configs: &AppEnvConfigs, storyteller_creds_manager: &StorytellerCredentialManager) -> Result<TaskEnqueueSuccess, GenerateError> {
  ...
  let image_media_token = match &request.image_media_token {
    ...
    None => upload_image_from_base64_bytes(&request, &app_data_root, &app_env_configs, &creds).await?,
  };

  let request = RemoveImageBackgroundRequest { uuid_idempotency_token, media_file_token: Some(image_media_token) };

  let result = remove_image_background(&app_env_configs.storyteller_host, Some(&creds), request).await;

  let success_result = match result {
    Ok(enqueued) => {
      // TODO(bt,2025-07-05): Enqueue job token?
      info!("Successfully enqueued Artcraft background removal. Job token: {}", enqueued.inference_job_token);
      enqueued
    },
    Err(err) => {
  ...
  Ok(TaskEnqueueSuccess { provider: GenerationProvider::Artcraft, task_type: TaskType::BackgroundRemoval, provider_job_id: Some(success_result.inference_job_token.to_string()), model: None, maybe_queue_status_url: None, maybe_prompt_token: None, maybe_queue_response_url: None })
}

async fn upload_image_from_base64_bytes(request: &EnqueueImageBgRemovalCommand, app_data_root: &AppDataRoot, app_env_configs: &AppEnvConfigs, creds: &StorytellerCredentialSet) -> Result<MediaFileToken, GenerateError> {
  let temp_file;

  if let Some(base64_bytes) = &request.base64_image {
    info!("Saving base64 image to temp dir ...");
    temp_file = save_base64_image_to_temp_dir(&app_data_root, base64_bytes).await.map_err(|err| {
