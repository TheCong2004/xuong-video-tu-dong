...
#[tauri::command]
pub async fn worldlabs_receive_bearer_command(root: State<'_, AppDataRoot>, request: WorldlabsReceiveBearerRequest, bearer_bridge: State<'_, WorldlabsBearerBridge>) -> SimpleResponse {
  info!("worldlabs_receive_bearer_command called");

  info!("Request: {:?}", request);

  set_bearer(request, &bearer_bridge).map_err(|err| {
...
fn set_bearer(request: WorldlabsReceiveBearerRequest, bearer_bridge: &WorldlabsBearerBridge) -> AnyhowResult<()> {
  bearer_bridge.set(WorldlabsBearerBridgeInner { bearer_token: request.bearer_token, refresh_token: request.refresh_token })?;
