...
#[tauri::command]
pub async fn grok_get_credential_info_command(creds_manager: State<'_, GrokCredentialManager>) -> ResponseOrErrorMessage<GrokGetCredentialInfoResponse> {
...
fn get_info(creds: &GrokCredentialManager) -> AnyhowResult<GrokGetCredentialInfoResponse> {
  ...
  let maybe_email = maybe_full_credentials.map(|full_creds| full_creds.client_secrets.user_email).map(|maybe_email| maybe_email.map(|email| email.to_string())).flatten();

  Ok(GrokGetCredentialInfoResponse { maybe_email, can_clear_state })
}
