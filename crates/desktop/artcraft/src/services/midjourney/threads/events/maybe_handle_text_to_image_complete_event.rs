...
pub async fn maybe_handle_text_to_image_complete_event(app: &AppHandle, app_env_configs: &AppEnvConfigs, maybe_creds: Option<&StorytellerCredentialSet>, task: &Task, batch_token: &BatchGenerationToken) -> AnyhowResult<()> {
  ...
  let event = handle_batch(app, app_env_configs, maybe_creds, task, batch_token).await?;
...
async fn handle_batch(app: &AppHandle, app_env_configs: &AppEnvConfigs, maybe_creds: Option<&StorytellerCredentialSet>, task: &Task, batch_token: &BatchGenerationToken) -> AnyhowResult<TextToImageGenerationCompleteEvent> {
