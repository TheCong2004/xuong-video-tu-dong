...
impl SerializableMidjourneyUserInfo {
  pub fn to_user_info(&self) -> MidjourneyUserInfo {
    MidjourneyUserInfo { user_id: self.user_id.as_deref().map(|id| MidjourneyUserId::from_str(id)), email: self.email.clone(), websocket_token: self.websocket_token.clone() }
  }
}
