...
impl MidjourneyCredentialManager {
  pub fn initialize_empty(app_data_root: &AppDataRoot) -> Self {
    Self { cookies: Arc::new(RwLock::new(None)), user_info: Arc::new(RwLock::new(None)), app_data_root: app_data_root.clone() }
  }
  ...
  pub fn initialize_from_disk_infallible(app_data_root: &AppDataRoot) -> Self {
    ...
    match read_midjourney_state_from_disk(app_data_root) {
      ...
      Ok(Some(state)) => {
        ...
        let maybe_user_info = state.user_info.map(|info| info.to_user_info());
        cookies = Arc::new(RwLock::new(maybe_cookies));
        user_info = Arc::new(RwLock::new(maybe_user_info));
      },
    };

    Self { cookies, user_info, app_data_root: app_data_root.clone() }
  }
  ...
  pub fn persist_to_disk(&self) -> anyhow::Result<()> {
    ...
    let maybe_cookies = maybe_cookies.map(|cookies| cookies.to_serializable());

    let maybe_user_info = maybe_user_info.map(|info| info.to_serializable());
    ...
    let state = SerializableMidjourneyState { version: SERIALIZABLE_MIDJOURNEY_STATE_VERSION, user_cookies: maybe_cookies, user_info: maybe_user_info };
...
fn read_midjourney_state_from_disk(root: &AppDataRoot) -> AnyhowResult<Option<SerializableMidjourneyState>> {
  ...
  let contents = read_to_string(&midjourney_state_path)?.trim().to_string();
