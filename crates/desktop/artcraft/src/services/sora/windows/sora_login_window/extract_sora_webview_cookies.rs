...
static CHAT_GPT_ROOT_COOKIE_URL: Lazy<Url> = Lazy::new(|| Url::parse(CHAT_GPT_ROOT_COOKIE_URL_STR).expect("URL should parse"));
...
pub fn extract_sora_webview_cookies(webview: &WebviewWindow) -> AnyhowResult<String> {
  ...
  let cookie_string = cookies.iter().map(|cookie| format!("{}={}", cookie.name(), cookie.value())).collect::<Vec<String>>().join("; ");
  Ok(cookie_string)
}
