...
pub async fn poll_classic_sora_tasks(app_handle: &AppHandle, app_env_configs: &AppEnvConfigs, task_database: &TaskDatabase, sora_creds_manager: &SoraCredentialManager, sora_creds: &SoraCredentialSet, storyteller_creds_manager: &StorytellerCredentialManager, sora_task_queue: &SoraTaskQueue, app_data_root: &AppDataRoot, local_sqlite_tasks_by_sora_task_id: &HashMap<String, Task>) -> AnyhowResult<()> {
  ...
  for task in sora_items.iter() {
    match &task.status {
      TaskStatus::Succeeded => {
        sora_succeeded_tasks_by_id.insert(task.id.clone(), SuccessfulGeneration { prompt: task.prompt.clone(), model_type: CommonModelType::GptImage1, items: task.generations.iter().map(|gen| GenerationItem { item_id: gen.id.clone(), url: gen.url.clone() }).collect() });
      },
      TaskStatus::Failed => {
  ...
  // Clear dead tasks.
  handle_classic_failed_generations(&app_handle, &task_database, &local_sqlite_tasks_by_sora_task_id, &sora_failed_tasks_by_id, &sora_task_queue).await?;

  // Process succeeded tasks.
  handle_classic_successful_generations(&app_handle, &app_data_root, &app_env_configs, &task_database, &storyteller_creds, &sora_succeeded_tasks_by_id, &local_sqlite_tasks_by_sora_task_id, DownloadExtension::Png).await?;
