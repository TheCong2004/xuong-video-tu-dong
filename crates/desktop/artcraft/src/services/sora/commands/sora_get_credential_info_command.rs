...
#[tauri::command]
pub async fn sora_get_credential_info_command(sora_creds_manager: State<'_, SoraCredentialManager>) -> ResponseOrErrorMessage<SoraGetCredentialInfoResponse> {
  info!("sora_get_credential_info_command called");

  let info = get_info(&sora_creds_manager).map_err(|err| {
...
fn get_info(sora_creds_manager: &SoraCredentialManager) -> AnyhowResult<SoraGetCredentialInfoResponse> {
  ...
  Ok(SoraGetCredentialInfoResponse { maybe_email, can_clear_state })
}
