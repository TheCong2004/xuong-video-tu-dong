...
pub async fn handle_failed_job(app_handle: &AppHandle, job: &ListSessionJobsItem, task: &Task, task_database: &TaskDatabase) -> AnyhowResult<()> {
  info!("Marking storyteller job as failed: {:?}", task.id);

  let maybe_failure_type = job.status.maybe_failure_category_updated.as_ref().map(|val| TaskFailureType::from_frontend_failure_category_for_api(val));

  let maybe_failure_message = job.status.maybe_failure_message.as_deref();

  update_task_status_with_rich_failure(UpdateTaskWithRichFailureArgs { db: task_database.get_connection(), task_id: &task.id, status: TaskStatus::CompleteFailure, maybe_failure_type, maybe_failure_message }).await?;

  let service = to_generation_service_provider(task.provider);
  let action = to_generation_action(task.task_type);

  let event = GenerationFailedEvent { action, service, model: None, reason: None };
