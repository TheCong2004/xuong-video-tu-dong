...
pub async fn storyteller_billing_window_thread(app: AppHandle, app_data_root: AppDataRoot) {
  loop {
    ...
    let result = check_billing_window(&app, &billing_webview_window).await;
...
async fn check_billing_window(app_handle: &AppHandle, webview_window: &WebviewWindow) -> AnyhowResult<bool> {
  let url = webview_window.url()?;

  let hostname = url.host().ok_or(anyhow!("no host in url"))?.to_string();

  match hostname.as_str() {
    "getartcraft.com" | "storyteller.ai" => {
