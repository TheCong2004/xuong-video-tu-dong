...
#[tauri::command]
pub async fn storyteller_open_customer_portal_update_payment_method_command(app: AppHandle, app_data_root: State<'_, AppDataRoot>, app_env_configs: State<'_, AppEnvConfigs>, storyteller_creds_manager: State<'_, StorytellerCredentialManager>) -> Result<String, String> {
  info!("storyteller_open_customer_portal_update_payment_method_command called");

  do_open_portal(&app, &app_data_root, &app_env_configs, &storyteller_creds_manager).await.map_err(|err| {
...
async fn do_open_portal(app: &AppHandle, app_data_root: &AppDataRoot, app_env_configs: &AppEnvConfigs, storyteller_creds_manager: &StorytellerCredentialManager) -> AnyhowResult<()> {
  info!("Building billing window...");

  open_storyteller_billing_window(OpenStorytellerBillingWindowArgs { app, app_data_root, app_env_configs, storyteller_creds_manager, billing_window_case: BillingWindowCase::CustomerPortalUpdatePaymentMethod }).await?;
