...
#[tauri::command]
pub async fn storyteller_open_subscription_purchase_command(app: AppHandle, request: StorytellerOpenSubscriptionPurchaseCommand, app_data_root: State<'_, AppDataRoot>, app_env_configs: State<'_, AppEnvConfigs>, storyteller_creds_manager: State<'_, StorytellerCredentialManager>) -> Result<String, String> {
  ...
  do_open_billing(&app, &app_data_root, &app_env_configs, &storyteller_creds_manager, plan, cadence).await.map_err(|err| {
...
async fn do_open_billing(app: &AppHandle, app_data_root: &AppDataRoot, app_env_configs: &AppEnvConfigs, storyteller_creds_manager: &StorytellerCredentialManager, plan: ArtcraftSubscriptionSlug, cadence: PlanBillingCadence) -> AnyhowResult<()> {
  info!("Building billing window...");

  open_storyteller_billing_window(OpenStorytellerBillingWindowArgs { app, app_env_configs, app_data_root, storyteller_creds_manager, billing_window_case: BillingWindowCase::Subscription { plan, cadence } }).await?;
