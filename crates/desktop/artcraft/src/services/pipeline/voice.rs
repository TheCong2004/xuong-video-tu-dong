...
async fn request_piper_speech(input: &VoiceInput, text: &str, cancel_flag: &Arc<AtomicBool>) -> Result<SpeechAudio, VoiceError> {
  ...
  let cmd = TokioCommand::new(executable);
  let mut cmd = crate::core::lifecycle::startup::tasks::background_command::background_tokio_command(cmd);
  let mut child = cmd.current_dir(executable.parent().unwrap_or_else(|| Path::new("."))).arg("--model").arg(model).arg("--output_file").arg(&output_path).stdin(Stdio::piped()).stdout(Stdio::null()).stderr(Stdio::piped()).spawn().map_err(|error| VoiceError::new("VOICE_PIPER_START_FAILED", error.to_string(), false))?;
