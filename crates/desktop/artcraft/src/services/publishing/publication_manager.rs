...
pub async fn create_publications_for_completed_job(
  db: &TaskDbConnection,
  job_id: &str,
  page_id: &str,
  video_path: &str,
...
) -> Result<Vec<JobPublication>, String> {
  info!("[PublicationManager] Creating publication records for job_id={} page_id={}", job_id, page_id);

  let targets = list_publish_targets_for_page(db, page_id)
    .await
    .map_err(|e| format!("Failed to list publish targets: {e}"))?;
  ...
  for target in enabled_targets {
    ...
    let (status, scheduled_at, approved_at) = if target.post_mode.to_lowercase() == "auto" {
      ("READY_TO_POST".to_string(), None, Some(chrono::Utc::now().timestamp()))
    } else {
      ("WAITING_APPROVAL".to_string(), None, None)
    };
    ...
    let args = CreateJobPublicationArgs {
      ...
      target_config_id: Some(target.id.clone()),
      browser_profile_id: target.browser_profile_id.clone(),
      status: status.clone(),
      scheduled_at,
      approved_at,
      ...
      hashtags_json: hashtags_json.clone(),
    ...
    match create_job_publication(db, args).await {
      Ok(pub_record) => {
        info!(
          "[PublicationManager] Created publication record: id={} platform={} status={} key={}",
          pub_record.id, pub_record.platform, pub_record.status, pub_record.idempotency_key
        );

        let stage_str = format!("PUBLISH_{}", pub_record.platform.to_uppercase());
        let msg_str = format!(
          "Khởi tạo bản ghi xuất bản {} [{}]: chế độ {}",
          pub_record.platform, pub_record.id, target.post_mode
        );
        ...
        let _ = insert_pipeline_job_event(
          InsertPipelineJobEventArgs {
            ...
            business_status: Some(&pub_record.status),
