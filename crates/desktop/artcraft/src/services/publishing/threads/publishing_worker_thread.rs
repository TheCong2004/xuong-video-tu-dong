    ...
    tauri::async_runtime::spawn(async move {
      ...
      loop {
        ...
        for pub_item in pending_items {
          ...
          info!("[PublishingWorker] Claimed publication id={} job_id={} platform={} profile={}", claimed.id, claimed.job_id, claimed.platform, claimed.browser_profile_id);
          ...
          let _ = insert_pipeline_job_event(InsertPipelineJobEventArgs { db: &db, id: None, job_id: &claimed.job_id, sequence: 20, stage_id: Some(&stage_str), business_status: Some("POSTING"), event_type: "POST_STARTED", level: "INFO", message: &msg_started, error_code: None, metadata_json: Some(&meta_started) }).await;
          ...
          tokio::spawn(async move {
            ...
            let ctx = PublicationExecutionContext { publication_id: claimed.id.clone(), job_id: claimed.job_id.clone(), page_id: claimed.page_id.clone(), platform: claimed.platform.clone(), browser_profile_id: claimed.browser_profile_id.clone(), video_path: claimed.video_path.clone().unwrap_or_default(), title: claimed.title.clone(), caption: claimed.caption.clone(), hashtags, description: claimed.description.clone(), target_destination_id: claimed.target_config_id.clone().unwrap_or_default(), target_destination_handle: None, idempotency_key: claimed.idempotency_key.clone(), attempt_number: claimed.attempt_count as i32 };
            ...
            let adapter: Arc<dyn PublisherAdapter> = match claimed.platform.to_lowercase().as_str() {
              ...
              unknown => {
                ...
                let _ = update_job_publication(&db, UpdateJobPublicationArgs { id: claimed.id.clone(), status: Some("POST_ERROR".to_string()), scheduled_at: None, approved_at: None, posted_at: None, platform_post_id: None, post_url: None, title: None, caption: None, last_error_code: Some("UNKNOWN_PLATFORM".to_string()), last_error_message: Some(format!("Unsupported publishing platform '{unknown}'")) }).await;
                return;
              },
            };
            ...
            match adapter.publish(&ctx).await {
              Ok(res) => {
                info!("[PublishingWorker] Publication SUCCESS: id={} platform={} post_id={:?}", claimed.id, claimed.platform, res.platform_post_id);

                let _ = update_job_publication(&db, UpdateJobPublicationArgs { id: claimed.id.clone(), status: Some("POSTED".to_string()), scheduled_at: None, approved_at: None, posted_at: Some(res.posted_at), platform_post_id: res.platform_post_id.clone(), post_url: res.post_url.clone(), title: None, caption: None, last_error_code: None, last_error_message: None }).await;
                ...
                let _ = insert_pipeline_job_event(InsertPipelineJobEventArgs { db: &db, id: None, job_id: &claimed.job_id, sequence: 21, stage_id: Some(&stage_str), business_status: Some("POSTED"), event_type: "POST_CONFIRMED", level: "INFO", message: &msg_ok, error_code: None, metadata_json: Some(&meta_ok) }).await;
              },
              Err(err) => {
                error!("[PublishingWorker] Publication ERROR: id={} platform={} err={:?}", claimed.id, claimed.platform, err);
                ...
                let _ = update_job_publication(&db, UpdateJobPublicationArgs { id: claimed.id.clone(), status: Some(new_status.to_string()), scheduled_at: None, approved_at: None, posted_at: None, platform_post_id: None, post_url: None, title: None, caption: None, last_error_code: Some(err.code.as_str().to_string()), last_error_message: Some(err.message.clone()) }).await;
