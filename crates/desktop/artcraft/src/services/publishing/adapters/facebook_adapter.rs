...
impl PublisherAdapter for FacebookPublisherAdapter {
  ...
  async fn validate_session(&self, ctx: &PublicationExecutionContext) -> Result<bool, PublisherError> {
    ...
    let req = AcquireWorkerRequest { job_id: ctx.job_id.clone(), step_id: format!("publish_facebook_{}_session_check", ctx.publication_id), attempt_id: format!("val_session_{}", ctx.attempt_number), capability: "social.facebook.publish".to_string(), pool_id: None, profile_id: Some(ctx.browser_profile_id.clone()), ttl_seconds: Some(30) };
  ...
  async fn publish(&self, ctx: &PublicationExecutionContext) -> Result<PublicationResult, PublisherError> {
    ...
    info!("[FacebookPublisher] Initiating Facebook publishing: pub_id={} profile={} page_id={} attempt={}", ctx.publication_id, ctx.browser_profile_id, ctx.target_destination_id, ctx.attempt_number);
    ...
    let acquire_req = AcquireWorkerRequest { job_id: ctx.job_id.clone(), step_id: step_id.clone(), attempt_id: attempt_id.clone(), capability: "social.facebook.publish".to_string(), pool_id: None, profile_id: Some(ctx.browser_profile_id.clone()), ttl_seconds: Some(300) };
    ...
    match resp {
      Ok(res) if res.status().is_success() => {
        ...
        let raw: serde_json::Value = res.json().await.map_err(|e| PublisherError::new(PublisherErrorCode::PlatformRejected, format!("[Facebook] Failed to parse dispatch response (protocol violation): {e}"), false))?;
        ...
        let post_id = result_obj.and_then(|r| r.get("postId").or_else(|| r.get("platform_post_id"))).and_then(|v| v.as_str()).map(|s| s.to_string());
        let post_url = result_obj.and_then(|r| r.get("postUrl").or_else(|| r.get("post_url"))).and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        if post_id.is_none() && post_url.is_none() {
          return Err(PublisherError::new(PublisherErrorCode::VerificationRequired, "Facebook posting completed without authoritative post_id or post_url evidence from runtime extension", false));
        }
