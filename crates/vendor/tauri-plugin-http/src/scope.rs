...
impl<'de> Deserialize<'de> for Entry {
    ...
    D: Deserializer<'de>,
  {
    EntryRaw::deserialize(deserializer).and_then(|raw| {
      ...
      Ok(Entry { url: parse_url_pattern(&url).map_err(|e| serde::de::Error::custom(format!("`{}` is not a valid URL pattern: {e}", url)))? })
    })
  }
}
...
impl<'a> Scope<'a> {
  ...
  pub fn is_allowed(&self, url: &Url) -> bool {
    let denied = self.denied.iter().any(|entry| entry.url.test(UrlPatternMatchInput::Url(url.clone())).unwrap_or_default());
    if denied {
      false
    } else {
      self.allowed.iter().any(|entry| entry.url.test(UrlPatternMatchInput::Url(url.clone())).unwrap_or_default())
    }
  }
}
