...
type CancelableResponseFuture = Pin<Box<dyn Future<Output = CancelableResponseResult> + Send + Sync>>;
...
impl AddRequest for ResourceTable {
  fn add_request(&mut self, fut: CancelableResponseFuture) -> ResourceId {
    ...
    let req = FetchRequest { fut: Mutex::new(fut), abort_tx_rid: self.add(tx), abort_rx_rid: self.add(rx) };
    self.add(req)
  }
}
...
#[inline]
fn proxy_creator(url_or_config: UrlOrConfig, proxy_fn: fn(String) -> reqwest::Result<reqwest::Proxy>) -> reqwest::Result<reqwest::Proxy> {
  match url_or_config {
...
fn attach_proxy(proxy: Proxy, mut builder: reqwest::ClientBuilder) -> crate::Result<reqwest::ClientBuilder> {
...
#[command]
pub async fn fetch<R: Runtime>(webview: Webview<R>, state: State<'_, Http>, client_config: ClientConfig, command_scope: CommandScope<Entry>, global_scope: GlobalScope<Entry>) -> crate::Result<ResourceId> {
  let ClientConfig { method, url, headers: headers_raw, data, connect_timeout, max_redirections, proxy, danger } = client_config;
  ...
  match scheme {
    "http" | "https" => {
      if Scope::new(command_scope.allows().iter().chain(global_scope.allows()).collect(), command_scope.denies().iter().chain(global_scope.denies()).collect()).is_allowed(&url) {
        ...
        if let Some(danger_config) = danger {
          ...
          {
            builder = builder.danger_accept_invalid_certs(danger_config.accept_invalid_certs).danger_accept_invalid_hostnames(danger_config.accept_invalid_hostnames)
          }
        }
        ...
        if let Some(max_redirections) = max_redirections {
          builder = builder.redirect(if max_redirections == 0 { Policy::none() } else { Policy::limited(max_redirections) });
        }
        ...
        if cfg!(not(feature = "unsafe-headers")) || !headers.contains_key(header::ORIGIN) {
          if let Ok(url) = webview.url() {
            headers.append(header::ORIGIN, HeaderValue::from_str(&url.origin().ascii_serialization())?);
          }
        }
        ...
        if cfg!(feature = "unsafe-headers") && headers.get(header::ORIGIN) == Some(&HeaderValue::from_static("")) {
    ...
    "data" => {
      ...
      let response = http::Response::builder().status(StatusCode::OK).header(header::CONTENT_TYPE, data_url.mime_type().to_string()).body(reqwest::Body::from(body))?;
...
#[command]
pub async fn fetch_send<R: Runtime>(webview: Webview<R>, rid: ResourceId) -> crate::Result<FetchResponse> {
  let (req, abort_rx) = {
    let mut resources_table = webview.resources_table();
    let req = resources_table.get::<FetchRequest>(rid)?;
  ...
  Ok(FetchResponse { status: status.as_u16(), status_text: status.canonical_reason().unwrap_or_default().to_string(), headers, url, rid })
}

#[command]
pub async fn fetch_read_body<R: Runtime>(webview: Webview<R>, rid: ResourceId, stream_channel: Channel<tauri::ipc::InvokeResponseBody>) -> crate::Result<()> {
  let res = {
    let mut resources_table = webview.resources_table();
