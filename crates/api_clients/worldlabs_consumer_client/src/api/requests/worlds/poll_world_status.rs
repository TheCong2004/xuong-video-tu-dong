...
pub async fn poll_world_status(args: PollWorldStatusArgs<'_>) -> Result<PollWorldStatusResponse, WorldLabsError> {
  ...
  let maybe_spz_url = response.generation_output.map(|out| out.spz_urls.full_res);

  Ok(PollWorldStatusResponse { is_complete: maybe_spz_url.is_some(), spz_splat_url: maybe_spz_url })
}
...
mod tests {
  use crate::api::api_types::world_id::WorldObjectId;
  use crate::api::requests::worlds::poll_world_status::{get_url, poll_world_status, PollWorldStatusArgs, BASE_URL};
  ...
  fn test_get_url() {
    ...
    let expected = format!("{}/d60cfa21-5506-43de-9a9c-0707fc17a5ec", BASE_URL);
  ...
  async fn test_requests() {
    ...
    let response = poll_world_status(PollWorldStatusArgs { cookies: &cookies, bearer_token: &bearer_token, world_id: &world_id, request_timeout: None }).await.unwrap();
