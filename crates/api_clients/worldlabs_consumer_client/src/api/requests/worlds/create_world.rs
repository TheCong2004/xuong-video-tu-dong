...
pub async fn create_world(args: CreateWorldArgs<'_>) -> Result<CreateWorldResponse, WorldLabsError> {
  ...
  Ok(CreateWorldResponse { world_id: WorldObjectId(response.id) })
}
...
impl Default for RawRequest {
  fn default() -> Self {
    Self { permission: Permission { public: true }, generation_input: GenerationInput { model: "Marble 0.1-plus".to_string(), prompt: Prompt { r#type: "image".to_string(), image_prompt: ImagePrompt { uri: None }, text_prompt: None } } }
  }
}
