...
pub async fn create_run_object(args: CreateRunObjectArgs<'_>) -> Result<CreateRunObjectResponse, WorldLabsError> {
  ...
  let http_request = request_builder.json(&new_run_object).build().map_err(|err| {
  ...
  let response: RunObject = serde_json::from_str(&response_body).map_err(|err| WorldLabsGenericApiError::SerdeResponseParseErrorWithBody(err, response_body.to_string()))?;

  let run_id = match &response.id {
    Some(id) => RunObjectId(id.to_string()),
    None => return Err(WorldLabsSpecificApiError::NoRunIdInResponse { response_body }.into()),
  };

  Ok(CreateRunObjectResponse { id: run_id, run_object: response })
}
