...
pub async fn finalize_object_image_upload(args: FinalizeObjectImageUploadArgs<'_>) -> Result<FinalizeObjectImageUploadResponse, WorldLabsError> {
  ...
  Ok(FinalizeObjectImageUploadResponse { object_url: response.object_uri, updated_at: response.updated_at })
}
...
mod tests {
  ...
  async fn test_requests() {
    ...
    let response = finalize_object_image_upload(FinalizeObjectImageUploadArgs { cookies: &cookies, bearer_token: &bearer_token, upload_id: &upload_id, request_timeout: None }).await.unwrap();
