...
pub async fn upload_image_and_create_world(args: UploadImageAndCreateWorldArgs<'_>) -> Result<UploadImageAndCreateWorldResponse, WorldLabsError> {
  ...
  let prepare_response = prepare_upload(PrepareUploadArgs { creds: args.creds, file_name: "upload.jpg", kind: MediaAssetKind::Image, request_timeout: args.individual_request_timeout }).await?;
  ...
  upload_to_signed_url(UploadToSignedUrlArgs { upload_url: &upload_url, file_bytes, required_headers: &required_headers, content_type: "image/jpeg", request_timeout: args.individual_request_timeout }).await?;
  ...
  let world_prompt = WorldPrompt::Image { image_prompt: ContentReference::MediaAsset { media_asset_id: media_asset_id.as_str().to_string() }, text_prompt: args.text_prompt, is_pano: None, disable_recaption: None };

  let generate_response = generate_world(GenerateWorldArgs { creds: args.creds, world_prompt, display_name: None, model: args.model, seed: None, tags: None, permission: None, request_timeout: args.individual_request_timeout }).await?;

  info!("Operation ID: {}", generate_response.operation_id.as_str());
  info!("Done: {}", generate_response.done);

  Ok(UploadImageAndCreateWorldResponse { operation_id: generate_response.operation_id, media_asset_id })
}
...
mod tests {
  ...
  async fn test_upload_and_generate() {
    ...
    let results = upload_image_and_create_world(UploadImageAndCreateWorldArgs { creds: &creds, individual_request_timeout: None, file: FileBytesOrPath::Bytes(file_bytes), text_prompt: None, model: WorldLabsModel::Marble0p1Plus }).await.unwrap();
