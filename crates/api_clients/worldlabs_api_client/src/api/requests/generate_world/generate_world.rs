...
pub async fn generate_world(args: GenerateWorldArgs<'_>) -> Result<GenerateWorldResponse, WorldLabsError> {
  ...
  let model_name = args.model.to_new_value().get_model_api_name_str().to_string();

  let payload = RawRequest { world_prompt: args.world_prompt, display_name: args.display_name, model: Some(model_name), seed: args.seed, tags: args.tags, permission: args.permission };
  ...
  Ok(GenerateWorldResponse { operation_id: OperationId(raw.operation_id), done: raw.done.unwrap_or(false) })
}
...
mod tests {
  ...
  async fn test_generate_world_text_prompt() {
    ...
    let response = generate_world(GenerateWorldArgs { creds: &creds, world_prompt: WorldPrompt::Text { text_prompt: Some("A cozy cabin in the snowy mountains".to_string()), disable_recaption: None }, display_name: Some("Test World".to_string()), model: WorldLabsModel::Marble0p1Mini, seed: None, tags: None, permission: None, request_timeout: None }).await.unwrap();
  ...
  async fn test_generate_world_image_uri() {
    ...
    let response = generate_world(GenerateWorldArgs { creds: &creds, world_prompt: WorldPrompt::Image { image_prompt: ContentReference::Uri { uri: "https://upload.wikimedia.org/wikipedia/commons/thumb/4/47/PNG_transparency_demonstration_1.png/280px-PNG_transparency_demonstration_1.png".to_string() }, text_prompt: None, is_pano: None, disable_recaption: None }, display_name: None, model: WorldLabsModel::Marble0p1Mini, seed: None, tags: None, permission: None, request_timeout: None }).await.unwrap();
  ...
  async fn test_generate_world_for_safe_image() {
    ...
    // --- Read file ---
    let file_bytes = file_read_bytes("/Users/bt/dev/storyteller/artcraft/test_data/image/mochi.jpg").expect("failed to read safe test image");
    println!("Read {} bytes from safe test image", file_bytes.len());

    // --- Step 1: prepare_upload ---
    println!("\n=== Step 1: prepare_upload ===");
    let prepare_result = prepare_upload(PrepareUploadArgs { creds: &creds, file_name: "mochi.jpg", kind: MediaAssetKind::Image, request_timeout: None }).await;
    ...
    println!("\n=== Step 2: upload_to_signed_url ===");
    let upload_result = upload_to_signed_url(UploadToSignedUrlArgs { upload_url: &prepare_response.upload_url, file_bytes, required_headers: &prepare_response.required_headers, content_type: "image/jpeg", request_timeout: None }).await;
    ...
    println!("\n=== Step 3: generate_world ===");
    let generate_result = generate_world(GenerateWorldArgs { creds: &creds, world_prompt: WorldPrompt::Image { image_prompt: ContentReference::MediaAsset { media_asset_id: prepare_response.media_asset_id.as_str().to_string() }, text_prompt: Some("A colorful gradient landscape".to_string()), is_pano: None, disable_recaption: None }, display_name: None, model: WorldLabsModel::Marble0p1Mini, seed: None, tags: None, permission: None, request_timeout: None }).await;
  ...
  async fn test_generate_world_for_nsfw_image() {
    ...
    // --- Read file ---
    let file_bytes = file_read_bytes("/Users/bt/Pictures/nsfw_test/nsfw_test_1.jpg").expect("failed to read NSFW test image");
    println!("Read {} bytes from NSFW test image", file_bytes.len());

    // --- Step 1: prepare_upload ---
    println!("\n=== Step 1: prepare_upload ===");
    let prepare_result = prepare_upload(PrepareUploadArgs { creds: &creds, file_name: "nsfw_test_1.jpg", kind: MediaAssetKind::Image, request_timeout: None }).await;
    ...
    // --- Step 3: generate_world ---
    println!("\n=== Step 3: generate_world ===");
    let generate_result = generate_world(GenerateWorldArgs { creds: &creds, world_prompt: WorldPrompt::Image { image_prompt: ContentReference::MediaAsset { media_asset_id: prepare_response.media_asset_id.as_str().to_string() }, text_prompt: None, is_pano: None, disable_recaption: None }, display_name: None, model: WorldLabsModel::Marble0p1Mini, seed: None, tags: None, permission: None, request_timeout: None }).await;
  ...
  async fn test_generate_world_marble_1p1_image_uri() {
    ...
    let result = generate_world(GenerateWorldArgs { creds: &creds, world_prompt: WorldPrompt::Image { image_prompt: ContentReference::Uri { uri: image_url.to_string() }, text_prompt: Some("Mountain landscape with a tree".to_string()), is_pano: None, disable_recaption: None }, display_name: None, model: WorldLabsModel::Marble1p1, seed: None, tags: None, permission: None, request_timeout: None }).await;
  ...
  async fn test_generate_world_marble_1p1_plus_image_uri() {
    ...
    let result = generate_world(GenerateWorldArgs { creds: &creds, world_prompt: WorldPrompt::Image { image_prompt: ContentReference::Uri { uri: image_url.to_string() }, text_prompt: Some("Autumn mountain panorama".to_string()), is_pano: None, disable_recaption: None }, display_name: None, model: WorldLabsModel::Marble1p1Plus, seed: None, tags: None, permission: None, request_timeout: None }).await;
