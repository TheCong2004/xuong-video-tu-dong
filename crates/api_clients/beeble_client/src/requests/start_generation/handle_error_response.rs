...
pub fn handle_error_response<T>(status: StatusCode, response_body: &str, context: &ErrorContext) -> Result<T, BeebleError> {
  match status.as_u16() {
    ...
    400 if response_body.contains("INVALID_CALLBACK_URL") => {
      let message = extract_error_message(response_body).unwrap_or_else(|| "callback_url must be a valid, publicly-reachable HTTPS URL".to_string());
      Err(BeebleSpecificApiError::BadWebhookUrl { message, webhook_url: context.maybe_callback_url.unwrap_or_default().to_string() }.into())
    },
    400 if response_body.contains("VIDEO_TOO_MANY_FRAMES") => {
      let message = extract_error_message(response_body).unwrap_or_else(|| "Video has too many frames".to_string());
      let (detected, max) = extract_frame_counts(&message);
      Err(BeebleSpecificApiError::VideoHasTooManyFrames { max_frames: max, detected_frames: detected, message }.into())
    },
    409 => Err(BeebleSpecificApiError::IdempotencyConflict.into()),
    429 => Err(BeebleSpecificApiError::RateLimited.into()),
    _ => Err(BeebleGenericApiError::UncategorizedBadResponseWithStatusAndBody { status_code: status, body: response_body.to_string() }.into()),
  }
}
...
fn extract_frame_counts(message: &str) -> (u64, u64) {
  let numbers: Vec<u64> = message.split(|c: char| !c.is_ascii_digit()).filter(|s| !s.is_empty()).filter_map(|s| s.parse().ok()).collect();
