...
pub async fn start_generation(args: StartGenerationArgs) -> Result<StartGenerationSuccess, BeebleError> {
  ...
  if !status.is_success() {
    return handle_error_response(status, &response_body, &ErrorContext { maybe_callback_url: req.callback_url.as_deref() });
  }
...
pub(crate) fn map_job_response(parsed: GenerationJobResponseBody) -> StartGenerationSuccess {
  StartGenerationSuccess { id: parsed.id, status: parsed.status, progress: parsed.progress, generation_type: parsed.generation_type, alpha_mode: parsed.alpha_mode, output: parsed.output.map(|o| GenerationOutputResult { render: o.render, source: o.source, alpha: o.alpha }), error: parsed.error, created_at: parsed.created_at }
}
...
mod tests {
  ...
  fn request_body_serializes_minimal() {
    let body = StartGenerationRequestBody { generation_type: "image".to_string(), source_uri: "beeble://upload_abc/photo.jpg".to_string(), alpha_mode: "auto".to_string(), prompt: None, reference_image_uri: None, alpha_uri: None, max_resolution: None, callback_url: None, idempotency_key: None };
  ...
  fn request_body_serializes_full() {
    let body = StartGenerationRequestBody { generation_type: "video".to_string(), source_uri: "https://example.com/video.mp4".to_string(), alpha_mode: "custom".to_string(), prompt: Some("A sunny beach".to_string()), reference_image_uri: Some("beeble://upload_ref/ref.jpg".to_string()), alpha_uri: Some("beeble://upload_alpha/mask.png".to_string()), max_resolution: Some(1080), callback_url: Some("https://example.com/webhook".to_string()), idempotency_key: Some("my-key-123".to_string()) };
    let json = serde_json::to_string(&body).unwrap();
  ...
  async fn test_start_image_generation() -> errors::AnyhowResult<()> {
    ...
    let result = start_generation(StartGenerationArgs { api_key, request: StartGenerationRequest { generation_type: BeebleGenerationType::Image, source_uri: "https://images.unsplash.com/photo-1506748686214-e9df14d4d9d0?w=800".to_string(), alpha_mode: BeebleAlphaMode::Auto, prompt: Some("A tropical beach at sunset".to_string()), reference_image_uri: None, alpha_uri: None, max_resolution: Some(720), callback_url: None, idempotency_key: None } }).await?;
  ...
  async fn upload_asset_bytes(api_key: &crate::creds::beeble_api_key::BeebleApiKey, filename: &str, bytes: Vec<u8>) -> errors::AnyhowResult<crate::requests::create_upload_url::create_upload_url::CreateUploadUrlSuccess> {
    use crate::requests::create_upload_url::create_upload_url::{create_upload_url, CreateUploadUrlArgs};

    // 1. Get presigned upload URL.
    let upload = create_upload_url(CreateUploadUrlArgs { api_key: api_key.clone(), filename: filename.to_string() }).await.map_err(|e| anyhow::anyhow!("create_upload_url failed: {:?}", e))?;
    ...
    let put_response = client.put(&upload.upload_url).header("Content-Type", content_type).body(bytes).send().await?;
  ...
  async fn test_end_to_end_video_generation() -> errors::AnyhowResult<()> {
    ...
    let result = start_generation(StartGenerationArgs { api_key, request: StartGenerationRequest { generation_type: BeebleGenerationType::Video, source_uri: video_upload.beeble_uri, alpha_mode: BeebleAlphaMode::Auto, prompt: Some("Place the dog in a forest setting".to_string()), reference_image_uri: Some(image_upload.beeble_uri), alpha_uri: None, max_resolution: Some(720), callback_url: None, idempotency_key: None } }).await.map_err(|e| anyhow::anyhow!("start_generation failed: {:?}", e))?;
