...
impl GoogleDriveDownloadCommand {
  pub fn new(download_script: &str, maybe_change_to_directory: Option<&str>, maybe_venv_activation_script: Option<&str>, maybe_docker_options: Option<DockerOptions>) -> Self {
    Self { download_script: download_script.to_string(), maybe_change_to_directory: maybe_change_to_directory.map(|s| s.to_string()), maybe_venv_activation_script: maybe_venv_activation_script.map(|s| s.to_string()), maybe_docker_options }
  }

  pub fn new_production(download_script: &str) -> Self {
    Self { download_script: download_script.to_string(), maybe_change_to_directory: None, maybe_venv_activation_script: None, maybe_docker_options: None }
  }

  pub async fn download_file_with_file_name(&self, download_url: &str, temp_dir: &TempDir, file_name: &str) -> AnyhowResult<String> {
    let temp_dir_path = temp_dir.path().to_str().unwrap_or("/tmp").to_string();

    let temp_filename = format!("{}/{}", temp_dir_path, file_name);

    info!("Downloading {} to: {}", download_url, temp_filename);

    let mut command = format!("{} --url \"{}\" --output_filename {}", &self.download_script, download_url, &temp_filename);

    if let Some(venv_activation_script) = self.maybe_venv_activation_script.as_deref() {
      // NB: "." is source for non-bash shells
      command = format!(". {} && {}", venv_activation_script, &command);
    }

    if let Some(change_to_directory) = self.maybe_change_to_directory.as_deref() {
      command = format!("cd {} && {}", change_to_directory, &command);
    }
  ...
  pub async fn download_file(&self, download_url: &str, temp_dir: &TempDir) -> AnyhowResult<String> {
    let temp_dir_path = temp_dir.path().to_str().unwrap_or("/tmp").to_string();
