...
pub trait BucketOrchestrationCore {
  #[deprecated(note = "this abstraction is too complicated, use another bucket client")]
  async fn download_file_to_disk(&self, object_path: String, filesystem_path: String, is_public: bool) -> AnyhowResult<()>;

  #[deprecated(note = "this abstraction is too complicated, use another bucket client")]
  async fn upload_file_with_content_type_process(&self, object_name: &str, bytes: &[u8], content_type: &str, is_public: bool) -> AnyhowResult<()>;
}
...
impl BucketOrchestrationCore for BucketOrchestration {
  ...
  async fn upload_file_with_content_type_process(&self, object_name: &str, bytes: &[u8], content_type: &str, is_public: bool) -> AnyhowResult<()> {
    let bucket_client = self.get_bucket_with_visibility(is_public).await?;
    bucket_client.upload_file_with_content_type_process(object_name, bytes, content_type).await
  }
}
...
impl BucketOrchestration {
  ...
  pub fn new_bucket_client_from_existing_env() -> AnyhowResult<Self> {
    ...
    let bucket_orchestration = BucketOrchestration::new(access_key, secret_key, region_name, s3_compatible_endpoint_url, public_bucket_name, private_bucket_name);

    Ok(bucket_orchestration)
  }

  #[deprecated(note = "this abstraction is too complicated, use another bucket client")]
  pub fn new(access_key: String, secret_key: String, region_name: String, s3_compatible_endpoint_url: String, public_bucket_name: String, private_bucket_name: String) -> Self {
    Self { access_key, secret_key, region_name, public_bucket_name, private_bucket_name, s3_compatible_endpoint_url }
  }
  ...
  async fn get_bucket_with_visibility(&self, public: bool) -> AnyhowResult<LegacyBucketClient> {
    ...
    let bucket_client = if public {
      ...
      LegacyBucketClient::create(&self.access_key, &self.secret_key, &self.region_name, &self.public_bucket_name, &self.s3_compatible_endpoint_url, None, Some(bucket_timeout))?
    } else {
      info!("Configuring private GCS bucket...");
      LegacyBucketClient::create(&self.access_key, &self.secret_key, &self.region_name, &self.private_bucket_name, &self.s3_compatible_endpoint_url, None, Some(bucket_timeout))?
    };
    Ok(bucket_client)
  }
}
