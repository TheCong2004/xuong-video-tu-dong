...
impl RemoteCloudFileClient {
  ...
  pub async fn get_remote_cloud_file_client() -> AnyhowResult<Self> {
    ...
    Ok(Self { bucket_orchestration_client: Box::new(bucket_orchestration) })
  }

  #[deprecated(note = "DO NOT USE THIS AWFUL CLOUD BUCKET CLIENT. These abstractions are complicated. See `LegacyBucketClient`")]
  pub fn new(bucket_orchestration_client: Box<dyn BucketOrchestrationCore>) -> Self {
    Self { bucket_orchestration_client }
  }
  ...
  pub async fn upload_file(&self, file_descriptor: Box<dyn FileDescriptor>, from_system_file_path: &str) -> AnyhowResult<FileMetaData> {
    ...
    let directory = FileBucketDirectory::generate_new(file_descriptor);

    result.bucket_details = Some(RemoteCloudBucketDetails { object_hash: directory.get_file_object_hash().to_string(), suffix, prefix });

    println!("Uploading media file to bucket path: {:?}", directory.get_full_remote_cloud_file_path());

    self.bucket_orchestration_client.upload_file_with_content_type_process(&directory.get_full_remote_cloud_file_path(), bytes.as_ref(), result.mimetype.as_ref(), is_public).await?;
  ...
  pub fn get_file_meta_data(system_file_path: &str) -> AnyhowResult<FileMetaData> {
    ...
    Ok(FileMetaData { file_size_bytes, sha256_checksum, mimetype: mimetype.to_string(), bucket_details: None })
  }
}
...
mod tests {
  ...
  async fn remote_file_manager_download_existing_file() {
    ...
    let remote_cloud_file_manager = RemoteCloudFileClient { bucket_orchestration_client: Box::new(BucketOrchestrationMock {}) };

    println!("begin upload from file_path: {:?}", file_path);
    let details = RemoteCloudBucketDetails { object_hash: String::from("123"), prefix: String::from("loRA_"), suffix: String::from(".safetensors") };
