...
impl KinoviGenerateVideoRequest {
  ...
  pub fn estimate_credits(&self) -> u32 {
    let credits_per_second: u32 = match (self.model_type, self.output_resolution) {
      ...
      (KinoviModelType::Seedance2Pro, None) | (KinoviModelType::Seedance2Pro, Some(KinoviOutputResolution::SevenTwentyP)) => 40,
      ...
      (KinoviModelType::Seedance2Fast, None) | (KinoviModelType::Seedance2Fast, Some(KinoviOutputResolution::SevenTwentyP)) => 28,
  ...
  fn credits_per_dollar(&self) -> f64 {
    match (self.model_type, self.output_resolution) {
      ...
      (KinoviModelType::Seedance2Pro, None) | (KinoviModelType::Seedance2Pro, Some(KinoviOutputResolution::SevenTwentyP)) => 250.0,

      // Legacy: Seedance 2.0 Fast @ 720p — 22,000 credits for $99.99
      (KinoviModelType::Seedance2Fast, None) | (KinoviModelType::Seedance2Fast, Some(KinoviOutputResolution::SevenTwentyP)) => 220.0,
...
pub async fn generate_video(args: GenerateVideoArgs<'_>) -> Result<GenerateVideoResponse, Seedance2ProError> {
  ...
  info!("Generating video: mode={}, resolution={}, duration={}, batch={}", video_input_mode, req.aspect_ratio.as_str(), duration, batch_count_value);

  let request_body = BatchRequest { zero: BatchRequestInner { json: BatchRequestJson { business_type: "wan22-video-generation", api_params: ApiParams { prompt: req.prompt, resolution: req.aspect_ratio.as_str().to_string(), content_mode: "normal", model: req.model_type.as_api_str(), duration, mode: video_input_mode, output_resolution: req.output_resolution.and_then(|r| r.as_api_str()), face_blur_mode, character_ids: req.character_ids, uploaded_urls, audio_urls, batch_count } } } };
  ...
  Ok(GenerateVideoResponse { task_id: task_data.task_id, order_id: task_data.order_id, task_ids: task_data.task_ids, order_ids: task_data.order_ids })
}
...
mod tests {
  ...
  mod pricing_tests {
    ...
    fn make_args(model_type: KinoviModelType, duration_seconds: u8, batch_count: KinoviBatchCount, output_resolution: Option<KinoviOutputResolution>) -> KinoviGenerateVideoRequest {
      KinoviGenerateVideoRequest { model_type, prompt: String::new(), aspect_ratio: KinoviAspectRatio::Square1x1, duration_seconds, batch_count, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, output_resolution, use_face_blur_hack: None }
    }
    ...
    fn table_driven_credits() {
      ...
      for (i, (model, res, dur, batch, expected)) in cases.iter().enumerate() {
        ...
        assert_eq!(actual, *expected, "Case {}: {:?} {:?} {}s batch {:?} — expected {} credits, got {}", i, model, res, dur, batch, expected, actual,);
      }
    }
    ...
    fn aspect_ratio_does_not_affect_credits() {
      let resolutions = [KinoviAspectRatio::Landscape16x9, KinoviAspectRatio::Portrait9x16, KinoviAspectRatio::Square1x1, KinoviAspectRatio::Standard4x3, KinoviAspectRatio::Portrait3x4];

      let baseline = pro(5, KinoviBatchCount::One).estimate_credits();

      for res in &resolutions {
        let req = KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: String::new(), aspect_ratio: *res, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, output_resolution: None, use_face_blur_hack: None };
        assert_eq!(req.estimate_credits(), baseline, "Aspect ratio {:?} should not change credits from baseline {}", res, baseline,);
      }
    }
  ...
  mod real_requests {
    ...
    async fn test_generate_text_to_video() -> AnyhowResult<()> {
      ...
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "A corgi eating a cake in a fancy kitchen.".to_string(), aspect_ratio: KinoviAspectRatio::Square1x1, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
    ...
    async fn test_generate_keyframe_video() -> AnyhowResult<()> {
      setup_test_logging(LevelFilter::Trace);
      let session = test_session()?;
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "A dog shakes the glasses off its head. The camera pans out as the shiba shakes. The shiba barks.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: Some("https://static.seedance2-pro.com/materials/20260219/1771496300184-fb32e08c.jpg".to_string()), end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_generate_reference_image_video() -> AnyhowResult<()> {
      setup_test_logging(LevelFilter::Trace);
      let session = test_session()?;
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "The dog in @2 is in the office at @1 without the man. The office is dark and moonlight streams in through the windows. Particles of dust gleam in the moon beams. Suddenly, the dog jumps walks in front of the desk and barks.".to_string(), aspect_ratio: KinoviAspectRatio::Standard4x3, duration_seconds: 10, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: Some(vec!["https://static.seedance2-pro.com/materials/20260219/1771463564512-b14bfe90.png".to_string(), "https://static.seedance2-pro.com/materials/20260219/1771496300184-fb32e08c.jpg".to_string()]), reference_video_urls: None, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_generate_reference_video_only() -> AnyhowResult<()> {
      setup_test_logging(LevelFilter::Trace);
      let session = test_session()?;
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "Change the Video @video1 to night time.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: Some(vec!["https://static.seedance2-pro.com/materials/20260315/1773594284659-3a46d231.mp4".to_string()]), reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_generate_reference_video_and_image() -> AnyhowResult<()> {
      setup_test_logging(LevelFilter::Trace);
      let session = test_session()?;
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "Put the robot in @video1 next to the house in @image1".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: Some(vec!["https://static.seedance2-pro.com/materials/20260315/1773595053724-07a1d500.png".to_string()]), reference_video_urls: Some(vec!["https://static.seedance2-pro.com/materials/20260315/1773594284659-3a46d231.mp4".to_string()]), reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_video_ref_file_that_is_too_long() -> AnyhowResult<()> {
      ...
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "Change @video1 to night time".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: Some(vec![result.public_url]), reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_pro_keyframe_with_start_frame() -> AnyhowResult<()> {
      ...
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "The corgi dog watches the lake.".to_string(), aspect_ratio: KinoviAspectRatio::Portrait9x16, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: Some(upload_result.public_url), end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_fast_keyframe_with_start_frame() -> AnyhowResult<()> {
      ...
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Fast, prompt: "A corgi dog runs along the lake shore, splashing water. Camera follows.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: Some(upload_result.public_url), end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_fast_three_image_references() -> AnyhowResult<()> {
      ...
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Fast, prompt: "The dog in @1 is running through the scenery in @3 towards the building in @2. Golden hour lighting.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: Some(uploaded_urls), reference_video_urls: None, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    async fn test_fast_audio_reference_with_text() -> AnyhowResult<()> {
      ...
      let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Fast, prompt: "A fantasy forest with mushrooms glowing in the dark. Fireflies dance between the trees. A small character walks along a winding path.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: Some(vec![upload_result.public_url]), character_ids: None, use_face_blur_hack: None, output_resolution: None } };
      let result = generate_video(args).await?;
      println!("Task ID: {}", result.task_id);
      println!("Order ID: {}", result.order_id);
      assert!(!result.task_id.is_empty());
    ...
    mod character_tests {
      ...
      async fn test_text_prompt_with_character_pro() -> AnyhowResult<()> {
        setup_test_logging(LevelFilter::Trace);
        let session = test_session()?;
        let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "@Steampunk Clown is juggling flaming torches in a circus tent.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: Some(vec![STEAMPUNK_CLOWN_ID.to_string()]), use_face_blur_hack: None, output_resolution: None } };
        let result = generate_video(args).await?;
        println!("Task ID: {}", result.task_id);
        println!("Order ID: {}", result.order_id);
        assert!(!result.task_id.is_empty());
      ...
      async fn test_text_prompt_with_character_fast() -> AnyhowResult<()> {
        setup_test_logging(LevelFilter::Trace);
        let session = test_session()?;
        let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Fast, prompt: "@Mochi the female shiba inu is eating a cheese pizza while standing on the table".to_string(), aspect_ratio: KinoviAspectRatio::Portrait9x16, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: Some(vec![MOCHI_ID.to_string()]), use_face_blur_hack: None, output_resolution: None } };
        let result = generate_video(args).await?;
        println!("Task ID: {}", result.task_id);
        println!("Order ID: {}", result.order_id);
        assert!(!result.task_id.is_empty());
      ...
      async fn test_character_with_image_ref_pro() -> AnyhowResult<()> {
        setup_test_logging(LevelFilter::Trace);
        let session = test_session()?;
        let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Pro, prompt: "@Steampunk Clown is walking up to pet a dog on the couch.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: Some(vec!["https://static.seedance2-pro.com/materials/20260329/1774752385699-1ff44886.jpeg".to_string()]), reference_video_urls: None, reference_audio_urls: None, character_ids: Some(vec![STEAMPUNK_CLOWN_ID.to_string()]), use_face_blur_hack: None, output_resolution: None } };
        let result = generate_video(args).await?;
        println!("Task ID: {}", result.task_id);
        println!("Order ID: {}", result.order_id);
        assert!(!result.task_id.is_empty());
      ...
      async fn test_two_characters_fast() -> AnyhowResult<()> {
        setup_test_logging(LevelFilter::Trace);
        let session = test_session()?;
        let args = GenerateVideoArgs { session: &session, host_override: None, request: KinoviGenerateVideoRequest { model_type: KinoviModelType::Seedance2Fast, prompt: "@Steampunk Clown and @Mochi are playing fetch in a sunny park.".to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 5, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: Some(vec![STEAMPUNK_CLOWN_ID.to_string(), MOCHI_ID.to_string()]), use_face_blur_hack: None, output_resolution: None } };
        let result = generate_video(args).await?;
        println!("Task ID: {}", result.task_id);
        println!("Order ID: {}", result.order_id);
        assert!(!result.task_id.is_empty());
  ...
  mod output_resolution_tests {
    ...
    fn make_args_with_prompt<'a>(prompt: &'a str, session: &'a Seedance2ProSession, model_type: KinoviModelType, output_resolution: Option<KinoviOutputResolution>) -> GenerateVideoArgs<'a> {
      GenerateVideoArgs { session, host_override: None, request: KinoviGenerateVideoRequest { model_type, prompt: prompt.to_string(), aspect_ratio: KinoviAspectRatio::Landscape16x9, duration_seconds: 4, batch_count: KinoviBatchCount::One, start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls: None, reference_audio_urls: None, character_ids: None, output_resolution, use_face_blur_hack: None } }
    }

    fn make_args<'a>(session: &'a Seedance2ProSession, model_type: KinoviModelType, output_resolution: Option<KinoviOutputResolution>) -> GenerateVideoArgs<'a> {
