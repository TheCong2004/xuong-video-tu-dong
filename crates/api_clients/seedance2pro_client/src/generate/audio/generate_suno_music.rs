...
pub async fn generate_suno_music(args: GenerateSunoMusicArgs<'_>) -> Result<GenerateSunoMusicResponse, Seedance2ProError> {
  ...
  Ok(GenerateSunoMusicResponse { task_id: raw_response.task_id, order_id: raw_response.order_id })
}
...
fn build_api_params(request: &GenerateSunoMusicRequest) -> SunoMusicApiParams {
  SunoMusicApiParams { model: MODEL, prompt: request.prompt.clone(), custom: false, instrumental: request.instrumental, mv: MODEL_VERSION, tags: request.style_tags.clone(), generate_video: false, is_storage: true }
}
...
mod tests {
  ...
  mod pricing_tests {
    ...
    fn base_request() -> GenerateSunoMusicRequest {
      GenerateSunoMusicRequest { prompt: "A song".to_string(), style_tags: None, instrumental: false }
    }
    ...
    fn options_do_not_affect_cost() {
      ...
      let styled = GenerateSunoMusicRequest { prompt: "Another song".to_string(), style_tags: Some("EDM".to_string()), instrumental: true }.calculate_costs();
  ...
  mod request_shape_tests {
    ...
    #[test]
    fn basic_prompt() {
      let params = build_api_params(&GenerateSunoMusicRequest { prompt: "A song about corgi super heroes fighting shiba super villains.".to_string(), style_tags: None, instrumental: false });
      assert_eq!(serde_json::to_string(&params).unwrap(), r#"{"model":"suno-music","prompt":"A song about corgi super heroes fighting shiba super villains.","custom":false,"instrumental":false,"mv":"chirp-v5-5","generateVideo":false,"isStorage":true}"#,);
    }

    /// Mirrors capture 2_suno_music_style_female.txt (style tags present).
    #[test]
    fn with_style_tags() {
      let params = build_api_params(&GenerateSunoMusicRequest { prompt: "A song about Mario and Luigi fighting Bowser".to_string(), style_tags: Some("EDM style meets dance".to_string()), instrumental: false });
      assert_eq!(serde_json::to_string(&params).unwrap(), r#"{"model":"suno-music","prompt":"A song about Mario and Luigi fighting Bowser","custom":false,"instrumental":false,"mv":"chirp-v5-5","tags":"EDM style meets dance","generateVideo":false,"isStorage":true}"#,);
    }

    /// Mirrors capture 3_suno_music_instrumental.txt.
    #[test]
    fn instrumental() {
      let params = build_api_params(&GenerateSunoMusicRequest { prompt: "An epic journey for a JRPG".to_string(), style_tags: Some("Japanese JRPG adventure music".to_string()), instrumental: true });
  ...
  mod live_tests {
    ...
    async fn test_music_with_vocals() -> AnyhowResult<()> {
      ...
      let result = generate_suno_music(GenerateSunoMusicArgs { session: &session, host_override: None, request: GenerateSunoMusicRequest { prompt: "A song about a corgi who learns to sail the open sea".to_string(), style_tags: Some("Sea shanty, folk".to_string()), instrumental: false } }).await?;
    ...
    async fn test_music_instrumental() -> AnyhowResult<()> {
      setup_test_logging(LevelFilter::Trace);
      let session = test_session()?;
      let result = generate_suno_music(GenerateSunoMusicArgs { session: &session, host_override: None, request: GenerateSunoMusicRequest { prompt: "An epic journey across a frozen mountain range".to_string(), style_tags: Some("Cinematic orchestral score".to_string()), instrumental: true } }).await?;
