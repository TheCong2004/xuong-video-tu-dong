...
pub async fn user_connections(req: GetUserConnectionsRequest) -> Result<Vec<Provider>, MidjourneyError> {
  ...
  */
  ...
  let response: Vec<RawProvider> = serde_json::from_str(response_body).map_err(|err| MidjourneyApiError::DeserializationError(err))?;

  let providers = response.into_iter().map(|r| Provider { provider_id: r.providerId, user_data: r.userData.map(|u| UserData { provider_id: u.providerId, display_name: u.displayName, email: u.email, user_id: u.userId }), is_linked: r.isLinked }).collect::<Vec<_>>();
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let result = user_connections(GetUserConnectionsRequest { cookie_header, hostname: MidjourneyHostname::Standard }).await?;
