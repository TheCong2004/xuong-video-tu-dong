...
pub async fn get_index_page_html(req: GetIndexPageRequest) -> Result<String, MidjourneyError> {
  ...
  let mut http_request = client.get(url).header("cookie", cookie_header).header("Referrer-Policy", "origin-when-cross-origin").header("accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7").header("accept-language", "en-US,en;q=0.8").header("content-type", "application/json").header("priority", "u=0, i").header("sec-ch-ua-mobile", "?0").header("sec-fetch-dest", "document").header("sec-fetch-mode", "navigate").header("sec-fetch-site", "none").header("sec-fetch-user", "?1").header("upgrade-insecure-requests", "1");
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let result = get_index_page_html(GetIndexPageRequest { cookie_header, hostname: MidjourneyHostname::Standard }).await?;
