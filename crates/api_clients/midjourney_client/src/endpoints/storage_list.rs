...
pub async fn storage_list(req: GetStorageListRequest) -> Result<Vec<StorageItem>, MidjourneyError> {
  ...
  */
  ...
  let response: Vec<RawStorageItem> = serde_json::from_str(response_body).map_err(|err| MidjourneyApiError::DeserializationError(err))?;

  let items = response.into_iter().map(|r| StorageItem { bucket_pathname: r.bucketPathname }).collect::<Vec<_>>();
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let result = storage_list(GetStorageListRequest { cookie_header, hostname: MidjourneyHostname::Standard }).await?;
