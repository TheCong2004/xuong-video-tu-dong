...
impl ThumbnailTaskInputMimeType {
  ...
  fn to_output_mimetypes(&self) -> Vec<ThumbnailTaskOutputMimeType> {
    match self {
      ThumbnailTaskInputMimeType::MP4 => vec![ThumbnailTaskOutputMimeType::JPEG, ThumbnailTaskOutputMimeType::GIF],
    }
  }
}
...
impl ThumbnailTaskBuilder {
  pub fn new_for_source_mimetype(source_mimetype: ThumbnailTaskInputMimeType) -> Self {
    Self { source_mimetype, bucket: None, path: None, output_extension: None, output_suffix: None, event_id: None }
  }
  ...
  fn build(&self, thumbnail_task_output_mime_type: ThumbnailTaskOutputMimeType) -> Result<ThumbnailTask, String> {
    Ok(ThumbnailTask { source_mimetype: self.source_mimetype.as_string(), output_mimetype: thumbnail_task_output_mime_type.as_string(), output_extension: thumbnail_task_output_mime_type.to_extension().to_string(), bucket: self.bucket.clone().ok_or("bucket is required")?, path: self.path.clone().ok_or("path is required")?, output_suffix: self.output_suffix.clone().unwrap_or("".to_string()), event_id: self.event_id.clone().ok_or("event_id is required")? })
  }
...
impl ThumbnailTask {
  fn request_json(&self) -> Value {
    let output_path = format!("{}-{}.{}", self.path, self.output_suffix, self.output_extension);
  ...
  async fn send(&self) -> Result<(), reqwest::Error> {
    // TODO: retries?
    // TODO: move to config
    let url = easyenv::get_env_string_or_default("THUMBNAIL_GENERATOR_API_URL", "http://media-server.thumbnail-generator/tasks");

    let basic_auth = easyenv::get_env_string_or_default("THUMBNAIL_GENERATOR_API_BASIC_AUTH", "");

    let client = reqwest::Client::new();
    let task_json = self.request_json();
    client.post(url).header("Authorization", "Basic ".to_owned() + &basic_auth).json(&task_json).send().await?.error_for_status()?;
    Ok(())
  }
}
