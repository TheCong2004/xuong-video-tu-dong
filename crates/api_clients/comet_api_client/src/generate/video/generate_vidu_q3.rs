...
pub async fn generate_vidu_q3(args: GenerateViduQ3Args<'_>) -> Result<GenerateViduQ3Response, CometError> {
  ...
  Ok(GenerateViduQ3Response { task_id: result.task_id, status: result.status })
}
  ...
  pub fn to_create_video_request(&self) -> Result<CreateVideoRequest, CometClientError> {
    if let Some(seconds) = self.duration_seconds {
      if !(MIN_DURATION_SECONDS..=MAX_DURATION_SECONDS).contains(&seconds) {
        return Err(CometClientError::InvalidRequestField { field: "duration_seconds", raw_value: seconds.to_string(), reason: format!("Vidu Q3 supports {MIN_DURATION_SECONDS}-{MAX_DURATION_SECONDS} second durations") });
      }
    }

    Ok(CreateVideoRequest { model: CometVideoModelRaw::ViduQ3, prompt: self.prompt.clone(), maybe_seconds: self.duration_seconds, maybe_size: self.size.map(map_size), input_reference_images: self.input_reference_images.clone() })
  }
}
...
mod tests {
  ...
  fn maps_to_wire_request() {
    let request = GenerateViduQ3Request { prompt: "a lighthouse in fog".to_string(), duration_seconds: Some(16), size: Some(ViduQ3Size::Exact { width: 960, height: 528 }), input_reference_images: vec![] };

    let raw = request.to_create_video_request().expect("should validate");
    assert_eq!(raw.text_form_fields(), vec![("model", "viduq3".to_string()), ("prompt", "a lighthouse in fog".to_string()), ("seconds", "16".to_string()), ("size", "960x528".to_string()),]);
  }
  ...
  fn request_with_seconds(duration_seconds: Option<u8>) -> GenerateViduQ3Request {
    GenerateViduQ3Request { prompt: "ok".to_string(), duration_seconds, size: None, input_reference_images: vec![] }
  }
}
