...
pub async fn generate_doubao_seedance_1p0_pro(args: GenerateDoubaoSeedance1p0ProArgs<'_>) -> Result<GenerateDoubaoSeedance1p0ProResponse, CometError> {
  ...
  Ok(GenerateDoubaoSeedance1p0ProResponse { task_id: result.task_id, status: result.status })
}
  ...
  pub fn to_create_video_request(&self) -> Result<CreateVideoRequest, CometClientError> {
    if let Some(seconds) = self.duration_seconds {
      if !(MIN_DURATION_SECONDS..=MAX_DURATION_SECONDS).contains(&seconds) {
        return Err(CometClientError::InvalidRequestField { field: "duration_seconds", raw_value: seconds.to_string(), reason: format!("Seedance 1.0 Pro supports {MIN_DURATION_SECONDS}-{MAX_DURATION_SECONDS} second durations") });
      }
    }

    Ok(CreateVideoRequest { model: CometVideoModelRaw::DoubaoSeedance1p0Pro, prompt: self.prompt.clone(), maybe_seconds: self.duration_seconds, maybe_size: self.size.map(map_size), input_reference_images: vec![] })
  }
}
...
mod tests {
  ...
  fn maps_to_wire_request() {
    let request = GenerateDoubaoSeedance1p0ProRequest { prompt: "time lapse of a city at dusk".to_string(), duration_seconds: Some(2), size: Some(DoubaoSeedance1p0ProSize::Square1x1) };

    let raw = request.to_create_video_request().expect("should validate");
    assert_eq!(raw.text_form_fields(), vec![("model", "doubao-seedance-1-0-pro".to_string()), ("prompt", "time lapse of a city at dusk".to_string()), ("seconds", "2".to_string()), ("size", "1:1".to_string()),]);
  ...
  fn request_with_seconds(duration_seconds: Option<u8>) -> GenerateDoubaoSeedance1p0ProRequest {
    GenerateDoubaoSeedance1p0ProRequest { prompt: "ok".to_string(), duration_seconds, size: None }
  }
}
