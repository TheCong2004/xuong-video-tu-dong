...
pub async fn log_active_user(api_host: &ApiHost, maybe_creds: Option<&StorytellerCredentialSet>, request: LogAppActiveUserRequest) -> Result<LogAppActiveUserResponse, StorytellerError> {
  ...
  Ok(basic_query_string_post_request(api_host, LOG_ACTIVE_USER_V1_PATH, maybe_creds, &query).await?)
}
