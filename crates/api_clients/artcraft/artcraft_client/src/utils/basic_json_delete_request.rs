...
pub async fn basic_json_delete_request<Req: Serialize, Res: DeserializeOwned>(api_host: &ApiHost, route_path: &str, maybe_creds: Option<&StorytellerCredentialSet>, request: Req) -> Result<Res, StorytellerError> {
  ...
  let mut request_builder = client.delete(url).header("User-Agent", USER_AGENT).header("Accept", APPLICATION_JSON).header("Content-Type", APPLICATION_JSON);
