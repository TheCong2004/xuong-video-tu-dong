...
mod tests {
  ...
  fn flat_cost_is_eight_cents() {
    let builder = GenerateAudioRequestBuilder { model: RouterAudioModel::SunoRemix, provider: RouterProvider::Artcraft, prompt: Some("make this electronic".to_string()), audio_references: Some(AudioListRef::MediaFileTokens(vec![MediaFileToken::new("mf_test123".to_string())])), ..Default::default() };
