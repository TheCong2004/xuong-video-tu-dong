use seedance2pro_client::generate::audio::generate_suno_remix::{generate_suno_remix, GenerateSunoRemixArgs, GenerateSunoRemixRequest};
...
impl KinoviSunoRemixRequestState {
  pub async fn send(&self, client: &RouterSeedance2ProClient) -> Result<GenerateAudioResponse, ArtcraftRouterError> {
    ...
    let args = GenerateSunoRemixArgs { session, host_override: None, request: self.request.clone() };

    let response = generate_suno_remix(args).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::Seedance2Pro(err)))?;
...
mod tests {
  ...
  async fn remix_uploaded_audio() {
    ...
    let builder = GenerateAudioRequestBuilder { model: RouterAudioModel::SunoRemix, provider: RouterProvider::Seedance2Pro, prompt: Some("Make this electronic".to_string()), style_prompt: Some("EDM style".to_string()), audio_references: Some(AudioListRef::Urls(vec![audio_url])), ..Default::default() };
  ...
  async fn upload_test_audio(client: &RouterClient) -> String {
    ...
    let prepare_result = prepare_file_upload(PrepareFileUploadArgs { session, extension: "mp3".to_string(), host_override: None }).await.expect("prepare upload");
