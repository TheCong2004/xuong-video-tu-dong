...
impl KinoviSunoSampleCostState {
  ...
  pub fn estimate_cost(&self) -> AudioGenerationCostEstimate {
    ...
    let pricing_request = GenerateSunoSampleRequest { prompt: String::new(), audio_url: "pricing-placeholder".to_string(), chop_sample_start_seconds: 0, chop_sample_end_seconds: 30, style_tags: None, instrumental: false };
...
mod tests {
  ...
  fn cost_from_draft_is_seven_cents() {
    let builder = GenerateAudioRequestBuilder { model: RouterAudioModel::SunoSample, provider: RouterProvider::Seedance2Pro, prompt: Some("mystical RPG adventure".to_string()), audio_references: Some(AudioListRef::Urls(vec!["https://example.com/a.aac".to_string()])), ..Default::default() };
