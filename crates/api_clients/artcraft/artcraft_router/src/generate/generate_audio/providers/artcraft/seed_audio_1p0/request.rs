...
#[derive(Clone, Debug)]
pub struct ArtcraftSeedAudio1p0RequestState {
  pub request: OmniGenAudioCostAndGenerateRequest,
}

impl ArtcraftSeedAudio1p0RequestState {
...
mod tests {
  ...
  async fn text_to_speech_drama() {
    let response = run_pipeline(GenerateAudioRequestBuilder { model: RouterAudioModel::SeedAudio1p0, provider: RouterProvider::Artcraft, prompt: Some("A calm narrator describes ocean waves rolling onto a moonlit beach.".to_string()), ..Default::default() }).await;
