...
impl KinoviSunoSoundsCostState {
  ...
  pub fn estimate_cost(&self) -> AudioGenerationCostEstimate {
    ...
    let pricing_request = GenerateSunoSoundRequest { prompt: String::new(), sound_type: KinoviSunoSoundType::SingleHit, bpm: None, key: KinoviSunoSoundKey::Auto };
...
mod tests {
  ...
  fn cost_from_built_request_is_two_cents() {
    let builder = GenerateAudioRequestBuilder { model: RouterAudioModel::SunoSounds, provider: RouterProvider::Seedance2Pro, prompt: Some("thunder".to_string()), is_loopable: Some(true), bpm: Some(60), ..Default::default() };
