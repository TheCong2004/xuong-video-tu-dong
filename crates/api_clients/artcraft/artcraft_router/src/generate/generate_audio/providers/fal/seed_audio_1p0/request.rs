...
use crate::generate::generate_audio::generate_audio_response::{FalAudioResponsePayload, GenerateAudioResponse};
...
impl FalSeedAudio1p0RequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateAudioResponse, ArtcraftRouterError> {
    ...
    let payload = if let Some(webhook_url) = &client.webhook_url {
      ...
      FalAudioResponsePayload { request_id: response.request_id, gateway_request_id: response.gateway_request_id, maybe_status_url: None, maybe_response_url: None, maybe_outbound_request: Some(outbound) }
    } else {
      let response = self.request.send_queue_request(&client.api_key).await?;
      FalAudioResponsePayload { request_id: Some(response.request_id), gateway_request_id: None, maybe_status_url: Some(response.status_url), maybe_response_url: Some(response.response_url), maybe_outbound_request: Some(outbound) }
    };
...
mod tests {
  ...
  async fn send_seed_audio_webhook() {
    let response = run_pipeline(GenerateAudioRequestBuilder { prompt: Some("A calm narrator describes ocean waves rolling onto a moonlit beach.".to_string()), ..seed_audio_builder() }).await;
  ...
  async fn send_seed_audio_tuned() {
    let response = run_pipeline(GenerateAudioRequestBuilder { prompt: Some("Welcome aboard the midnight express. Please keep your arms inside the train.".to_string()), sample_rate_hz: Some(44_100), speed: Some(0.9), volume: Some(1.2), pitch: Some(-2.0), ..seed_audio_builder() }).await;
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
    assert_eq!(1, 2, "Inspect output above");
  }
  ...
  fn seed_audio_builder() -> GenerateAudioRequestBuilder {
    GenerateAudioRequestBuilder { model: RouterAudioModel::SeedAudio1p0, provider: RouterProvider::Fal, ..Default::default() }
  }
