...
impl KinoviSunoRemixDraftState {
  pub async fn to_request(&mut self, draft_context: &AudioGenerationDraftContext<'_>) -> Result<KinoviSunoRemixRequestState, ArtcraftRouterError> {
    let client = draft_context.get_seedance2pro_client_ref()?;
    let session = &client.session;

    let audio_source = self.audio_source.take().ok_or_else(|| ArtcraftRouterError::InvalidInput("Suno Remix draft has no audio source (already finalized?)".to_string()))?;

    let kinovi_audio_url = resolve_and_upload_audio_ref(session, &audio_source, draft_context).await?;

    let request = GenerateSunoRemixRequest { prompt: self.prompt.clone(), source: KinoviSunoRemixSource::AudioUrl(kinovi_audio_url), style_tags: self.style_tags.clone(), keep_lyrics: self.keep_lyrics };
