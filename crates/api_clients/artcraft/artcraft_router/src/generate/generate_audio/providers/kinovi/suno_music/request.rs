use seedance2pro_client::generate::audio::generate_suno_music::{generate_suno_music, GenerateSunoMusicArgs, GenerateSunoMusicRequest};
...
impl KinoviSunoMusicRequestState {
  pub async fn send(&self, client: &RouterSeedance2ProClient) -> Result<GenerateAudioResponse, ArtcraftRouterError> {
    ...
    let args = GenerateSunoMusicArgs { session, host_override: None, request: self.request.clone() };

    let response = generate_suno_music(args).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::Seedance2Pro(err)))?;
...
mod tests {
  ...
  async fn music_with_vocals() {
    let response = run_pipeline(GenerateAudioRequestBuilder { prompt: Some("A song about a corgi who learns to sail the open sea".to_string()), style_prompt: Some("Sea shanty, folk".to_string()), ..suno_music_builder() }).await;
  ...
  async fn music_instrumental() {
    let response = run_pipeline(GenerateAudioRequestBuilder { prompt: Some("An epic journey across a frozen mountain range".to_string()), style_prompt: Some("Cinematic orchestral score".to_string()), is_instrumental: Some(true), ..suno_music_builder() }).await;
    assert!(matches!(response, GenerateAudioResponse::Seedance2Pro(_)));
    assert_eq!(1, 2, "Inspect output above");
  }
  ...
  fn suno_music_builder() -> GenerateAudioRequestBuilder {
    GenerateAudioRequestBuilder { model: RouterAudioModel::SunoMusic, provider: RouterProvider::Seedance2Pro, ..Default::default() }
  }
