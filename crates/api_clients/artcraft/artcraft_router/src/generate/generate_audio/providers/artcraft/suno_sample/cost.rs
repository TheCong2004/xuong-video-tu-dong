...
/// Suno Sample via Artcraft is flat priced, so the cost state carries no
/// request fields.
#[derive(Clone, Debug)]
pub struct ArtcraftSunoSampleCostState;

impl ArtcraftSunoSampleCostState {
  pub fn from_request(_request: &ArtcraftSunoSampleRequestState) -> Self {
    Self
  }
...
mod tests {
  ...
  fn flat_cost_is_eight_cents() {
    let builder = GenerateAudioRequestBuilder { model: RouterAudioModel::SunoSample, provider: RouterProvider::Artcraft, prompt: Some("mystical RPG adventure".to_string()), audio_references: Some(AudioListRef::MediaFileTokens(vec![MediaFileToken::new("mf_test123".to_string())])), ..Default::default() };
    ...
    assert_eq!(ArtcraftSunoSampleCostState.estimate_cost().cost_in_usd_cents, Some(8));
  }
}
