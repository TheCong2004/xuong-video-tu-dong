...
use crate::generate::generate_audio::providers::artcraft::resolve::{resolve_audio_list_ref, resolve_image_list_ref};
...
pub fn build_artcraft_omni_audio_request(mut builder: GenerateAudioRequestBuilder, model: CommonAudioModelEnum) -> Result<OmniGenAudioCostAndGenerateRequest, ArtcraftRouterError> {
  ...
  Ok(OmniGenAudioCostAndGenerateRequest { idempotency_token: Some(idempotency_token), model: Some(model), prompt: builder.prompt.take(), style_prompt: builder.style_prompt.take(), audio_media_tokens, image_media_tokens, keep_lyrics: builder.keep_lyrics, is_instrumental: builder.is_instrumental, is_loopable: builder.is_loopable, bpm: builder.bpm, musical_key: builder.musical_key, sample_rate_hz: builder.sample_rate_hz, speed: builder.speed, volume: builder.volume, pitch: builder.pitch })
}
...
mod tests {
  ...
  fn all_fields_are_forwarded() {
    ...
    let builder = GenerateAudioRequestBuilder { prompt: Some("a song".to_string()), style_prompt: Some("EDM".to_string()), audio_references: Some(AudioListRef::MediaFileTokens(vec![MediaFileToken::new("mf_audio".to_string())])), image_references: Some(ImageListRef::MediaFileTokens(vec![MediaFileToken::new("mf_image".to_string())])), keep_lyrics: Some(true), is_instrumental: Some(false), is_loopable: Some(true), bpm: Some(120), musical_key: Some(CommonMusicalKey::AMinor), sample_rate_hz: Some(48_000), speed: Some(1.5), volume: Some(0.8), pitch: Some(-2.0), idempotency_token: Some("test-token".to_string()), ..base_builder() };

    let request = build_artcraft_omni_audio_request(builder, CommonAudioModelEnum::SunoMusic).expect("build");
  ...
  fn idempotency_token_is_generated_when_missing() {
    let request = build_artcraft_omni_audio_request(base_builder(), CommonAudioModelEnum::SunoMusic).expect("build");
  ...
  fn audio_url_references_are_rejected() {
    let builder = GenerateAudioRequestBuilder { audio_references: Some(AudioListRef::Urls(vec!["https://example.com/a.mp3".to_string()])), ..base_builder() };
  ...
  fn image_url_references_are_rejected() {
    let builder = GenerateAudioRequestBuilder { image_references: Some(ImageListRef::Urls(vec!["https://example.com/a.png".to_string()])), ..base_builder() };
  ...
  fn base_builder() -> GenerateAudioRequestBuilder {
    GenerateAudioRequestBuilder { model: RouterAudioModel::SunoMusic, provider: RouterProvider::Artcraft, prompt: Some("a song".to_string()), ..Default::default() }
  }
}
