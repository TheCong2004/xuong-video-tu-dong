...
#[derive(Clone, Debug)]
pub struct ArtcraftSunoSampleRequestState {
  pub request: OmniGenAudioCostAndGenerateRequest,
}

impl ArtcraftSunoSampleRequestState {
...
mod tests {
  ...
  async fn sample_uploaded_audio() {
    let response = run_pipeline(GenerateAudioRequestBuilder { model: RouterAudioModel::SunoSample, provider: RouterProvider::Artcraft, prompt: Some("Mystical RPG adventure, make it have a grand climax".to_string()), style_prompt: Some("Fantasy video game score".to_string()), is_instrumental: Some(true), audio_references: Some(AudioListRef::MediaFileTokens(vec![MediaFileToken::new(AUDIO_MEDIA_TOKEN.to_string())])), ..Default::default() }).await;
