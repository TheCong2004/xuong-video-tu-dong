...
pub(crate) fn estimate_splat_cost_artcraft_marble_0p1_mini(plan: &PlanArtcraftMarble0p1Mini) -> SplatGenerationCostEstimate {
...
mod tests {
  ...
  fn estimate_usd_cents(prompt: Option<&str>, image_tokens: Option<Vec<MediaFileToken>>) -> u64 {
    let request = GenerateSplatRequest { model: RouterSplatModel::Marble0p1Mini, provider: RouterProvider::Artcraft, prompt: prompt.map(|s| s.to_string()), reference_images: image_tokens.map(crate::api::image_list_ref::ImageListRef::MediaFileTokens), idempotency_token: None };
