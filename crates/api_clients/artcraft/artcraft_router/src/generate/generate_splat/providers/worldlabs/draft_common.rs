...
use worldlabs_api_client::api::requests::generate_world::http_request::{ContentReference, SphericallyLocatedContent, WorldPrompt};
use worldlabs_api_client::api::requests::prepare_upload::prepare_upload::{prepare_upload, MediaAssetKind, PrepareUploadArgs};
use worldlabs_api_client::api::requests::upload_to_signed_url::upload_to_signed_url::{upload_to_signed_url, UploadToSignedUrlArgs};
...
impl WorldLabsSplatDraft {
  pub async fn to_request(&self, draft_context: &SplatGenerationDraftContext<'_>) -> Result<WorldLabsSplatRequest, ArtcraftRouterError> {
    ...
    let world_prompt = match &self.input {
      ...
      SplatInput::Text => WorldPrompt::Text { text_prompt: self.text_prompt.clone(), disable_recaption: self.disable_recaption },
      SplatInput::Image { image, is_panoramic } => {
        ...
        WorldPrompt::Image { image_prompt: content, text_prompt: self.text_prompt.clone(), is_pano: Some(*is_panoramic), disable_recaption: self.disable_recaption }
      },
      SplatInput::MultiImage { images } => {
        ...
        WorldPrompt::MultiImage { multi_image_prompt, text_prompt: self.text_prompt.clone(), reconstruct_images: None, disable_recaption: self.disable_recaption }
      },
      SplatInput::Video { video } => {
        ...
        WorldPrompt::Video { video_prompt: content, text_prompt: self.text_prompt.clone(), disable_recaption: self.disable_recaption }
      },
    };

    Ok(WorldLabsSplatRequest { model: self.model, world_prompt })
  }
...
async fn upload_media_asset(client: &RouterWorldLabsClient, source_url: &str, kind: MediaAssetKind) -> Result<ContentReference, ArtcraftRouterError> {
  ...
  let prepared = prepare_upload(PrepareUploadArgs { creds: &client.creds, file_name: &file_name, kind, request_timeout: Some(UPLOAD_REQUEST_TIMEOUT) }).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::WorldLabs(err)))?;

  upload_to_signed_url(UploadToSignedUrlArgs { upload_url: &prepared.upload_url, file_bytes, required_headers: &prepared.required_headers, content_type, request_timeout: Some(UPLOAD_REQUEST_TIMEOUT) }).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::WorldLabs(err)))?;

  Ok(ContentReference::MediaAsset { media_asset_id: prepared.media_asset_id.as_str().to_string() })
}

fn resolve_image_source_url(image: &ImageRef, draft_context: &SplatGenerationDraftContext<'_>) -> Result<String, ArtcraftRouterError> {
  match image {
...
fn resolve_video_source_url(video: &VideoRef, draft_context: &SplatGenerationDraftContext<'_>) -> Result<String, ArtcraftRouterError> {
  match video {
...
fn resolve_token_source_url(token: &MediaFileToken, draft_context: &SplatGenerationDraftContext<'_>) -> Result<String, ArtcraftRouterError> {
  let map = draft_context.media_file_to_artcraft_url_map.ok_or(ArtcraftRouterError::Client(ClientError::MediaFileToUrlMapNotProvided))?;
...
fn file_name_for_upload(source_url: &str, kind: MediaAssetKind) -> String {
  ...
  let extension = extension_from_url(source_url).unwrap_or_else(|| default_extension.to_string());
