...
mod tests {
  ...
  async fn text_to_world() {
    let builder = GenerateSplatRequestBuilder { model: RouterSplatModel::Marble1p0Draft, provider: RouterProvider::Artcraft, prompt: Some("A cozy cabin in the snowy mountains".to_string()), ..Default::default() };
