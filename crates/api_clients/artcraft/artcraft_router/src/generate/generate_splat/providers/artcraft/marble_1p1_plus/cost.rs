...
const PRICES: ArtcraftSplatPriceTable = ArtcraftSplatPriceTable { image_panorama: 312, text: 321, image_non_panorama: 321, multi_image: 323, video: 323 };

/// Marble 1.1 Plus via Artcraft is flat priced by input type.
#[derive(Clone, Debug)]
pub struct ArtcraftMarble1p1PlusCostState {
  pub input_type: InputType,
}

impl ArtcraftMarble1p1PlusCostState {
  pub fn from_request(request: &ArtcraftMarble1p1PlusRequestState) -> Self {
...
mod tests {
  ...
  #[test]
  fn text_is_321_cents() {
    assert_eq!(estimate_usd_cents(text_builder()), 321);
  }

  #[test]
  fn single_image_is_321_cents() {
    assert_eq!(estimate_usd_cents(single_image_builder(false)), 321);
  }

  #[test]
  fn panoramic_image_is_312_cents() {
    assert_eq!(estimate_usd_cents(single_image_builder(true)), 312);
  }

  #[test]
  fn multi_image_is_323_cents() {
    assert_eq!(estimate_usd_cents(multi_image_builder()), 323);
  }

  #[test]
  fn video_is_323_cents() {
    assert_eq!(estimate_usd_cents(video_builder()), 323);
  }

  // ── Helpers ──

  fn base_builder() -> GenerateSplatRequestBuilder {
    GenerateSplatRequestBuilder { model: RouterSplatModel::Marble1p1Plus, provider: RouterProvider::Artcraft, ..Default::default() }
  }
