...
pub(super) fn resolve_video_ref(video_ref: Option<VideoRef>) -> Result<Option<MediaFileToken>, ArtcraftRouterError> {
  match video_ref {
    None => Ok(None),
    Some(VideoRef::MediaFileToken(token)) => Ok(Some(token)),
    Some(VideoRef::Url(_)) => Err(ArtcraftRouterError::Client(ClientError::ArtcraftOnlySupportsMediaTokens)),
  }
}
