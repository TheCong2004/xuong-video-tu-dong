...
use crate::generate::generate_splat::providers::artcraft::resolve::{resolve_image_list_ref, resolve_video_ref};
...
pub fn build_artcraft_omni_splat_request(mut builder: GenerateSplatRequestBuilder, model: CommonSplatModelEnum) -> Result<OmniGenSplatCostAndGenerateRequest, ArtcraftRouterError> {
  ...
  Ok(OmniGenSplatCostAndGenerateRequest { idempotency_token: Some(idempotency_token), model: Some(model), prompt: builder.prompt.take(), reference_image_media_tokens, reference_video_media_token, is_panoramic: builder.is_panoramic, disable_recaption: builder.disable_recaption })
}
...
mod tests {
  ...
  fn all_fields_are_forwarded() {
    let builder = GenerateSplatRequestBuilder { prompt: Some("a cozy cabin".to_string()), reference_images: Some(ImageListRef::MediaFileTokens(vec![MediaFileToken::new("mf_image".to_string())])), reference_video: Some(VideoRef::MediaFileToken(MediaFileToken::new("mf_video".to_string()))), is_panoramic: Some(true), disable_recaption: Some(true), idempotency_token: Some("test-token".to_string()), ..base_builder() };

    let request = build_artcraft_omni_splat_request(builder, CommonSplatModelEnum::Marble1p0).expect("build");
  ...
  fn idempotency_token_is_generated_when_missing() {
    let request = build_artcraft_omni_splat_request(base_builder(), CommonSplatModelEnum::Marble1p0Draft).expect("build");
  ...
  fn image_url_references_are_rejected() {
    let builder = GenerateSplatRequestBuilder { reference_images: Some(ImageListRef::Urls(vec!["https://example.com/a.png".to_string()])), ..base_builder() };
  ...
  fn video_url_references_are_rejected() {
    let builder = GenerateSplatRequestBuilder { reference_video: Some(VideoRef::Url("https://example.com/a.mp4".to_string())), ..base_builder() };
    let result = build_artcraft_omni_splat_request(builder, CommonSplatModelEnum::Marble1p0);
    assert!(matches!(result, Err(ArtcraftRouterError::Client(ClientError::ArtcraftOnlySupportsMediaTokens))));
  }
  ...
  fn base_builder() -> GenerateSplatRequestBuilder {
    GenerateSplatRequestBuilder { model: RouterSplatModel::Marble1p0, provider: RouterProvider::Artcraft, prompt: Some("a cozy cabin".to_string()), ..Default::default() }
  }
}
