...
const MODEL: WorldLabsModel = WorldLabsModel::Marble1p1;
...
#[derive(Clone, Debug)]
pub struct WorldLabsMarble1p1CostState {
  pub input_type: InputType,
}

impl WorldLabsMarble1p1CostState {
  pub fn from_request(request: &WorldLabsMarble1p1RequestState) -> Self {
    Self { input_type: request.request.input_type() }
  }

  pub fn from_draft(draft: &WorldLabsMarble1p1DraftState) -> Self {
...
mod tests {
  ...
  fn base_builder() -> GenerateSplatRequestBuilder {
    GenerateSplatRequestBuilder { model: RouterSplatModel::Marble1p1, provider: RouterProvider::WorldLabs, ..Default::default() }
  }
