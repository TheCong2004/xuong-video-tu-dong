...
const MODEL: WorldLabsModel = WorldLabsModel::Marble1p0Draft;
...
#[derive(Clone, Debug)]
pub struct WorldLabsMarble1p0DraftModelCostState {
  pub input_type: InputType,
}

impl WorldLabsMarble1p0DraftModelCostState {
  pub fn from_request(request: &WorldLabsMarble1p0DraftModelRequestState) -> Self {
    Self { input_type: request.request.input_type() }
  }

  pub fn from_draft(draft: &WorldLabsMarble1p0DraftModelDraftState) -> Self {
...
mod tests {
  ...
  #[test]
  fn text_is_18_cents() {
    assert_eq!(estimate_usd_cents(text_builder()), 18);
  }

  #[test]
  fn single_image_is_18_cents() {
    assert_eq!(estimate_usd_cents(single_image_builder(false)), 18);
  }

  #[test]
  fn panoramic_image_is_12_cents() {
    assert_eq!(estimate_usd_cents(single_image_builder(true)), 12);
  }

  #[test]
  fn multi_image_is_20_cents() {
    assert_eq!(estimate_usd_cents(multi_image_builder()), 20);
  }

  #[test]
  fn video_is_20_cents() {
    assert_eq!(estimate_usd_cents(video_builder()), 20);
  }
  ...
  fn credits_come_from_the_provider_pricing_table() {
    ...
    assert_eq!(estimate.cost_in_credits, Some(230));
  }
