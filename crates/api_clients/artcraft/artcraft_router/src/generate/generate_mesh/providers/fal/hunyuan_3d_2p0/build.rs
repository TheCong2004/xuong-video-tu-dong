...
use crate::generate::generate_mesh::providers::fal::resolve::{plan_primary_image_url, plan_textured_mesh};
...
pub(crate) fn build_fal_hunyuan_3d_2p0_state(mut builder: GenerateMeshRequestBuilder) -> Result<FalHunyuan3d2p0RequestState, ArtcraftRouterError> {
  ...
  let image_url = plan_primary_image_url(builder.reference_images.take(), builder.front_image.take(), strategy)?.ok_or_else(|| ArtcraftRouterError::InvalidInput("An input image is required for Hunyuan 3D 2.0".to_string()))?;
...
mod tests {
  ...
  mod image_requirements {
    ...
    fn missing_image_is_rejected() {
      ...
      assert!(matches!(build_fal_hunyuan_3d_2p0_state(builder), Err(ArtcraftRouterError::InvalidInput(_))));
    }

    #[test]
    fn two_images_error_out() {
      let builder = GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(vec![IMAGE_URL.to_string(), "https://example.com/other.png".to_string()])), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..base_builder() };
      assert!(build_fal_hunyuan_3d_2p0_state(builder).is_err());
    }

    #[test]
    fn two_images_keep_the_first_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(vec![IMAGE_URL.to_string(), "https://example.com/other.png".to_string()])), ..base_builder() };
  ...
  mod output_type_mapping {
    ...
    fn low_poly_errors_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::LowPoly), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..base_builder() };
      assert!(build_fal_hunyuan_3d_2p0_state(builder).is_err());
    }
  ...
  mod unsupported_options {
    ...
    fn unsupported_options_error_out() {
      let cases: Vec<GenerateMeshRequestBuilder> = vec![GenerateMeshRequestBuilder { prompt: Some("a teapot".to_string()), ..base_builder() }, GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Quad), ..base_builder() }, GenerateMeshRequestBuilder { face_count: Some(100_000), ..base_builder() }, GenerateMeshRequestBuilder { enable_pbr: Some(true), ..base_builder() }, GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url("https://example.com/back.png".to_string())), ..base_builder() }];
    ...
    fn unsupported_options_are_dropped_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { prompt: Some("a teapot".to_string()), face_count: Some(100_000), enable_pbr: Some(true), ..base_builder() };
  ...
  fn base_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d2p0, provider: RouterProvider::Fal, reference_images: Some(ImageListRef::Urls(vec![IMAGE_URL.to_string()])), ..Default::default() }
  }
}
