...
#[derive(Clone, Debug)]
pub struct FalHunyuan3d2p0RequestState {
  /// Final materialized request; ready to fire.
  pub request: Hunyuan3d2p0ImageToMeshRequest,
}

impl FalHunyuan3d2p0RequestState {
...
mod tests {
  ...
  async fn send_hunyuan_3d_2p0_image_to_mesh() {
    let builder = GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d2p0, provider: RouterProvider::Fal, reference_images: Some(ImageListRef::Urls(vec![ERNEST_SCARED_STUPID_IMAGE_URL.to_string()])), ..Default::default() };
