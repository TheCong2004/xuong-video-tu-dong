...
use fal_client::requests::api::mesh::text::rodin_2p5_fast_text_to_mesh::api::Rodin2p5FastTextToMeshRequest;
...
#[derive(Clone, Debug)]
pub struct FalRodin2p5FastImageRequestState {
  /// Final materialized request; ready to fire.
  pub request: Rodin2p5FastImageToMeshRequest,
}

impl FalRodin2p5FastImageRequestState {
...
#[derive(Clone, Debug)]
pub struct FalRodin2p5FastTextRequestState {
  /// Final materialized request; ready to fire.
  pub request: Rodin2p5FastTextToMeshRequest,
}

impl FalRodin2p5FastTextRequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateMeshResponse, ArtcraftRouterError> {
    send_fal_mesh_request(&self.request, client).await
  }
}
