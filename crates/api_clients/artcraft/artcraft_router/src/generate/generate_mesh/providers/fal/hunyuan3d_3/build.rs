...
use fal_client::requests::api::mesh::image::hunyuan3d_3_image_to_mesh::api::{Hunyuan3d3ImageToMeshGenerateType, Hunyuan3d3ImageToMeshPolygonType, Hunyuan3d3ImageToMeshRequest};
use fal_client::requests::api::mesh::text::hunyuan3d_3_text_to_mesh::api::{Hunyuan3d3TextToMeshGenerateType, Hunyuan3d3TextToMeshPolygonType, Hunyuan3d3TextToMeshRequest};
...
pub(crate) fn build_fal_hunyuan3d_3_state(mut builder: GenerateMeshRequestBuilder) -> Result<FalHunyuan3d3State, ArtcraftRouterError> {
  ...
  match maybe_front_url {
    Some(image_url) => {
      ...
      let request = Hunyuan3d3ImageToMeshRequest { image_url, back_image_url: plan_side_image_url(builder.back_image.take())?, left_image_url: plan_side_image_url(builder.left_image.take())?, right_image_url: plan_side_image_url(builder.right_image.take())?, face_count, generate_type: builder.mesh_output_type.map(to_image_generate_type), polygon_type: builder.polygon_type.map(to_image_polygon_type), enable_pbr: builder.enable_pbr };
    ...
    None => {
      ...
      let prompt = builder.prompt.take().ok_or_else(|| ArtcraftRouterError::InvalidInput("Hunyuan 3D v3 requires an input image or a prompt".to_string()))?;

      let request = Hunyuan3d3TextToMeshRequest { prompt, face_count, generate_type: builder.mesh_output_type.map(to_text_generate_type), polygon_type: builder.polygon_type.map(to_text_polygon_type), enable_pbr: builder.enable_pbr };
...
mod tests {
  ...
  mod image_mode {
    ...
    fn prompt_with_image_is_dropped_under_lenient_strategies() {
      ...
      assert!(matches!(build_fal_hunyuan3d_3_state(builder).expect("build"), FalHunyuan3d3State::Image(_)));
    }
    ...
    fn media_tokens_are_rejected() {
      ...
      assert!(matches!(build_fal_hunyuan3d_3_state(builder), Err(ArtcraftRouterError::Client(ClientError::FalOnlySupportsUrls))));
    }
  }
  ...
  mod text_mode {
    ...
    #[test]
    fn options_map_through() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::Geometry), polygon_type: Some(CommonPolygonType::Triangle), face_count: Some(40_000), enable_pbr: Some(false), ..text_builder() };
    ...
    fn side_images_without_front_are_dropped_under_lenient_strategies() {
      ...
      assert!(matches!(build_fal_hunyuan3d_3_state(builder).expect("build"), FalHunyuan3d3State::Text(_)));
    }
  }
  ...
  fn base_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3, provider: RouterProvider::Fal, ..Default::default() }
  }
