...
pub(crate) fn build_fal_hunyuan_3d_2p1_state(mut builder: GenerateMeshRequestBuilder) -> Result<FalHunyuan3d2p1RequestState, ArtcraftRouterError> {
  ...
  let image_url = plan_primary_image_url(builder.reference_images.take(), builder.front_image.take(), strategy)?.ok_or_else(|| ArtcraftRouterError::InvalidInput("An input image is required for Hunyuan 3D 2.1".to_string()))?;
...
mod tests {
  ...
  fn missing_image_is_rejected() {
    ...
    assert!(matches!(build_fal_hunyuan_3d_2p1_state(builder), Err(ArtcraftRouterError::InvalidInput(_))));
  }
  ...
  fn unsupported_options_error_out() {
    let builder = GenerateMeshRequestBuilder { prompt: Some("a teapot".to_string()), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..base_builder() };
    assert!(build_fal_hunyuan_3d_2p1_state(builder).is_err());
  }

  #[test]
  fn unsupported_options_are_dropped_under_lenient_strategies() {
    let builder = GenerateMeshRequestBuilder { prompt: Some("a teapot".to_string()), face_count: Some(100_000), ..base_builder() };
  ...
  fn base_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d2p1, provider: RouterProvider::Fal, reference_images: Some(ImageListRef::Urls(vec![IMAGE_URL.to_string()])), ..Default::default() }
  }
}
