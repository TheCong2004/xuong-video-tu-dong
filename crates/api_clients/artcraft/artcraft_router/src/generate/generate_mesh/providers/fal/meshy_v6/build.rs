...
use fal_client::requests::api::mesh::text::meshy_v6_text_to_mesh::api::{MeshyV6ModelType, MeshyV6TextToMeshRequest, MeshyV6Topology};
...
pub(crate) fn build_fal_meshy_v6_state(mut builder: GenerateMeshRequestBuilder) -> Result<FalMeshyV6State, ArtcraftRouterError> {
  ...
  let maybe_image_url = plan_primary_image_url(builder.reference_images.take(), builder.front_image.take(), strategy)?;
  ...
  match maybe_image_url {
    Some(image_url) => {
      ...
      let request = MeshyV6ImageToMeshRequest { image_url, model_type: plan_image_model_type(builder.mesh_output_type), topology, target_polycount, symmetry_mode: None, should_remesh: None, should_texture: plan_should_texture(builder.mesh_output_type, builder.enable_texture), enable_pbr: builder.enable_pbr, pose_mode: None, texture_prompt: None, texture_image_url: None, enable_rigging: None, rigging_height_meters: None, enable_animation: None, animation_action_id: None, enable_safety_checker: None };
    ...
    None => {
      ...
      let prompt = builder.prompt.take().ok_or_else(|| ArtcraftRouterError::InvalidInput("Meshy 6 requires an input image or a prompt".to_string()))?;

      let request = MeshyV6TextToMeshRequest { prompt, mode: None, seed: None, model_type: plan_text_model_type(builder.mesh_output_type, strategy)?, topology, target_polycount, should_remesh: None, symmetry_mode: None, enable_pbr: builder.enable_pbr, pose_mode: None, enable_prompt_expansion: None, texture_prompt: None, texture_image_url: None, enable_rigging: None, rigging_height_meters: None, enable_animation: None, animation_action_id: None, enable_safety_checker: None };
...
fn plan_image_model_type(mesh_output_type: Option<CommonMeshOutputType>) -> Option<MeshyV6ModelType> {
...
fn plan_text_model_type(mesh_output_type: Option<CommonMeshOutputType>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<MeshyV6ModelType>, ArtcraftRouterError> {
  match mesh_output_type {
    ...
    Some(CommonMeshOutputType::Geometry) => {
      reject_unsupported_option("mesh_output_type", Some(&CommonMeshOutputType::Geometry), strategy)?;
      Ok(None)
    },
  }
}
...
fn plan_should_texture(mesh_output_type: Option<CommonMeshOutputType>, enable_texture: Option<bool>) -> Option<bool> {
  let skip_texturing = matches!(mesh_output_type, Some(CommonMeshOutputType::Geometry)) || enable_texture == Some(false);
  if skip_texturing { Some(false) } else { None }
}
...
mod tests {
  ...
  mod shape_dispatch {
    ...
    fn media_tokens_are_rejected() {
      ...
      assert!(matches!(build_fal_meshy_v6_state(builder), Err(ArtcraftRouterError::Client(ClientError::FalOnlySupportsUrls))));
    }
  }
  ...
  mod image_mode {
    ...
    fn should_texture_is_unset_by_default() {
      let builder = GenerateMeshRequestBuilder { enable_texture: Some(true), ..image_builder() };
      let image = expect_image(build_fal_meshy_v6_state(builder).expect("build"));
    ...
    fn options_map_through() {
      let builder = GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Quad), face_count: Some(100_000), enable_pbr: Some(true), ..image_builder() };
      let image = expect_image(build_fal_meshy_v6_state(builder).expect("build"));
    ...
    fn prompt_with_image_is_dropped_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { prompt: Some(PROMPT.to_string()), ..image_builder() };
      assert!(matches!(build_fal_meshy_v6_state(builder).expect("build"), FalMeshyV6State::Image(_)));
    }
  }
  ...
  mod text_mode {
    ...
    #[test]
    fn options_map_through() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::LowPoly), polygon_type: Some(CommonPolygonType::Triangle), face_count: Some(40_000), enable_pbr: Some(false), ..text_builder() };
    ...
    fn geometry_errors_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::Geometry), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..text_builder() };
      assert!(build_fal_meshy_v6_state(builder).is_err());
    }
    ...
    fn enable_texture_errors_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { enable_texture: Some(false), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..text_builder() };
      assert!(build_fal_meshy_v6_state(builder).is_err());
    }
    ...
    fn enable_texture_is_dropped_under_lenient_strategies() {
      ...
      assert!(matches!(build_fal_meshy_v6_state(builder).expect("build"), FalMeshyV6State::Text(_)));
    }
  }
  ...
  mod unsupported_options {
    ...
    fn texture_quality_errors_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { texture_quality: Some(CommonMeshQuality::Detailed), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..image_builder() };
      assert!(build_fal_meshy_v6_state(builder).is_err());
    }

    #[test]
    fn geometry_quality_errors_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { geometry_quality: Some(CommonMeshQuality::Detailed), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..image_builder() };
      assert!(build_fal_meshy_v6_state(builder).is_err());
    }
    ...
    fn side_images_error_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url("https://example.com/back.png".to_string())), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..image_builder() };
      assert!(build_fal_meshy_v6_state(builder).is_err());
    }

    #[test]
    fn unsupported_options_are_dropped_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { texture_quality: Some(CommonMeshQuality::Detailed), geometry_quality: Some(CommonMeshQuality::Standard), input_mesh: Some(MeshRef::Url("https://example.com/mesh.glb".to_string())), back_image: Some(ImageRef::Url("https://example.com/back.png".to_string())), left_image: Some(ImageRef::Url("https://example.com/left.png".to_string())), right_image: Some(ImageRef::Url("https://example.com/right.png".to_string())), ..image_builder() };
      assert!(matches!(build_fal_meshy_v6_state(builder).expect("build"), FalMeshyV6State::Image(_)));
    }
  }
  ...
  fn base_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::MeshyV6, provider: RouterProvider::Fal, ..Default::default() }
  }
