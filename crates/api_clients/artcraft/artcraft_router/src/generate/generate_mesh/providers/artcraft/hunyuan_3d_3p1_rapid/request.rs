...
mod tests {
  ...
  async fn text_to_mesh_teapot() {
    let response = run_pipeline(GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3p1Rapid, provider: RouterProvider::Artcraft, prompt: Some("A red ceramic teapot with a curved spout.".to_string()), ..Default::default() }).await;
