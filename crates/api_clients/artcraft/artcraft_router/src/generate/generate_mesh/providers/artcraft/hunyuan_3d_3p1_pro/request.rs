...
#[derive(Clone, Debug)]
pub struct ArtcraftHunyuan3d3p1ProRequestState {
  pub request: OmniGenMeshCostAndGenerateRequest,
}

impl ArtcraftHunyuan3d3p1ProRequestState {
...
mod tests {
  ...
  async fn text_to_mesh_teapot() {
    let response = run_pipeline(GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3p1Pro, provider: RouterProvider::Artcraft, prompt: Some("A red ceramic teapot with a curved spout.".to_string()), ..Default::default() }).await;
