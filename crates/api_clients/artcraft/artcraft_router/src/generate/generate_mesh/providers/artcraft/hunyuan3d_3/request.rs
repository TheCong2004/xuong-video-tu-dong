...
#[derive(Clone, Debug)]
pub struct ArtcraftHunyuan3d3RequestState {
  pub request: OmniGenMeshCostAndGenerateRequest,
}

impl ArtcraftHunyuan3d3RequestState {
...
mod tests {
  ...
  async fn text_to_mesh_teapot() {
    let response = run_pipeline(GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3, provider: RouterProvider::Artcraft, prompt: Some("A red ceramic teapot with a curved spout.".to_string()), ..Default::default() }).await;
