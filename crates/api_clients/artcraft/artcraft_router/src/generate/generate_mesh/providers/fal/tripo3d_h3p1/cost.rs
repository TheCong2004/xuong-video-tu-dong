...
mod tests {
  ...
  mod text_mode_costs {
    ...
    #[test]
    fn add_ons_stack() {
      let builder = GenerateMeshRequestBuilder { texture_quality: Some(CommonMeshQuality::Detailed), geometry_quality: Some(CommonMeshQuality::Detailed), polygon_type: Some(CommonPolygonType::Quad), ..text_builder() };
  ...
  mod multiview_mode_costs {
    ...
    #[test]
    fn untextured_via_geometry_output() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::Geometry), ..multiview_builder() };
      assert_eq!(estimate_usd_cents(builder), 10);
    }
    ...
    fn triangle_polygon_adds_nothing() {
      let builder = GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Triangle), ..multiview_builder() };
      assert_eq!(estimate_usd_cents(builder), 20);
    }

    #[test]
    fn add_ons_stack() {
      let builder = GenerateMeshRequestBuilder { texture_quality: Some(CommonMeshQuality::Detailed), geometry_quality: Some(CommonMeshQuality::Detailed), polygon_type: Some(CommonPolygonType::Quad), ..multiview_builder() };
      // Detailed texture(30) + detailed geometry(20) + quad(5)
      assert_eq!(estimate_usd_cents(builder), 55);
    }
  }
  ...
  fn multiview_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url(BACK_URL.to_string())), ..image_builder() }
  }
