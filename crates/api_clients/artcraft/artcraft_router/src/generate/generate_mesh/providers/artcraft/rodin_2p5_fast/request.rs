...
#[derive(Clone, Debug)]
pub struct ArtcraftRodin2p5FastRequestState {
  pub request: OmniGenMeshCostAndGenerateRequest,
}

impl ArtcraftRodin2p5FastRequestState {
...
mod tests {
  ...
  async fn text_to_mesh_teapot() {
    let response = run_pipeline(GenerateMeshRequestBuilder { model: RouterMeshModel::Rodin2p5Fast, provider: RouterProvider::Artcraft, prompt: Some("A red ceramic teapot with a curved spout.".to_string()), ..Default::default() }).await;
