...
#[derive(Clone, Debug)]
pub struct ArtcraftTripo3dH3p1RequestState {
  pub request: OmniGenMeshCostAndGenerateRequest,
}

impl ArtcraftTripo3dH3p1RequestState {
...
mod tests {
  ...
  async fn text_to_mesh_teapot() {
    let response = run_pipeline(GenerateMeshRequestBuilder { model: RouterMeshModel::Tripo3dH3p1, provider: RouterProvider::Artcraft, prompt: Some("A red ceramic teapot with a curved spout.".to_string()), ..Default::default() }).await;
