...
use fal_client::requests::api::mesh::text::meshy_v6_text_to_mesh::api::MeshyV6TextToMeshRequest;
...
#[derive(Clone, Debug)]
pub struct FalMeshyV6ImageRequestState {
  /// Final materialized request; ready to fire.
  pub request: MeshyV6ImageToMeshRequest,
}

impl FalMeshyV6ImageRequestState {
...
#[derive(Clone, Debug)]
pub struct FalMeshyV6TextRequestState {
  /// Final materialized request; ready to fire.
  pub request: MeshyV6TextToMeshRequest,
}

impl FalMeshyV6TextRequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateMeshResponse, ArtcraftRouterError> {
    send_fal_mesh_request(&self.request, client).await
  }
}
...
mod tests {
  ...
  async fn send_meshy_v6_image_to_mesh() {
    let response = run_pipeline(GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(vec![ERNEST_SCARED_STUPID_IMAGE_URL.to_string()])), ..meshy_v6_builder() }).await;
  ...
  #[tokio::test]
  #[ignore] // requires real API key, incurs cost
  async fn send_meshy_v6_text_to_mesh() {
    let response = run_pipeline(GenerateMeshRequestBuilder { prompt: Some("A velociraptor with an open mouth full of sharp teeth.".to_string()), ..meshy_v6_builder() }).await;
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
    assert_eq!(1, 2, "Inspect output above");
  }
  ...
  fn meshy_v6_builder() -> GenerateMeshRequestBuilder {
