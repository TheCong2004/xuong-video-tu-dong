...
use fal_client::requests::api::mesh::image::hunyuan_3d_3p1_pro_image_to_mesh::api::{Hunyuan3d3p1ProImageToMeshGenerateType, Hunyuan3d3p1ProImageToMeshRequest};
use fal_client::requests::api::mesh::text::hunyuan_3d_3p1_pro_text_to_mesh::api::{Hunyuan3d3p1ProTextToMeshGenerateType, Hunyuan3d3p1ProTextToMeshRequest};
...
pub(crate) fn build_fal_hunyuan_3d_3p1_pro_state(mut builder: GenerateMeshRequestBuilder) -> Result<FalHunyuan3d3p1ProState, ArtcraftRouterError> {
  ...
  match maybe_front_url {
    ...
    None => {
      ...
      let prompt = builder.prompt.take().ok_or_else(|| ArtcraftRouterError::InvalidInput("Hunyuan 3D v3.1 Pro requires an input image or a prompt".to_string()))?;

      let request = Hunyuan3d3p1ProTextToMeshRequest { prompt, generate_type: plan_text_generate_type(builder.mesh_output_type, strategy)?, face_count, enable_pbr: builder.enable_pbr };
...
fn plan_image_generate_type(mesh_output_type: Option<CommonMeshOutputType>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<Hunyuan3d3p1ProImageToMeshGenerateType>, ArtcraftRouterError> {
...
fn plan_text_generate_type(mesh_output_type: Option<CommonMeshOutputType>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<Hunyuan3d3p1ProTextToMeshGenerateType>, ArtcraftRouterError> {
  match mesh_output_type {
    None => Ok(None),
...
mod tests {
  ...
  mod image_mode {
    ...
    fn side_images_map_to_multi_view_params() {
      let builder = GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url(BACK_URL.to_string())), left_image: Some(ImageRef::Url("https://example.com/left.png".to_string())), right_image: Some(ImageRef::Url("https://example.com/right.png".to_string())), ..image_builder() };
    ...
    fn options_map_through() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::Geometry), face_count: Some(100_000), enable_pbr: Some(true), ..image_builder() };
      let image = expect_image(build_fal_hunyuan_3d_3p1_pro_state(builder).expect("build"));
    ...
    fn prompt_with_image_is_dropped_under_lenient_strategies() {
      ...
      assert!(matches!(build_fal_hunyuan_3d_3p1_pro_state(builder).expect("build"), FalHunyuan3d3p1ProState::Image(_)));
    }
  }
  ...
  mod text_mode {
    ...
    #[test]
    fn options_map_through() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::Normal), face_count: Some(40_000), enable_pbr: Some(false), ..text_builder() };
    ...
    fn side_images_without_front_error_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url(BACK_URL.to_string())), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..text_builder() };
      assert!(build_fal_hunyuan_3d_3p1_pro_state(builder).is_err());
    }

    #[test]
    fn side_images_without_front_are_dropped_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url(BACK_URL.to_string())), ..text_builder() };
      assert!(matches!(build_fal_hunyuan_3d_3p1_pro_state(builder).expect("build"), FalHunyuan3d3p1ProState::Text(_)));
    }
  }
  ...
  mod unsupported_options {
    ...
    fn polygon_type_errors_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Quad), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..image_builder() };
      assert!(build_fal_hunyuan_3d_3p1_pro_state(builder).is_err());
    }

    #[test]
    fn texture_options_error_out_under_error_out() {
      let cases: Vec<fn(&mut GenerateMeshRequestBuilder)> = vec![|b| b.enable_texture = Some(false), |b| b.texture_quality = Some(CommonMeshQuality::Detailed), |b| b.geometry_quality = Some(CommonMeshQuality::Detailed)];
    ...
    fn input_mesh_errors_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { input_mesh: Some(MeshRef::Url("https://example.com/model.glb".to_string())), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..text_builder() };
      assert!(build_fal_hunyuan_3d_3p1_pro_state(builder).is_err());
    }

    #[test]
    fn unsupported_options_are_dropped_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Quad), enable_texture: Some(false), texture_quality: Some(CommonMeshQuality::Detailed), geometry_quality: Some(CommonMeshQuality::Detailed), input_mesh: Some(MeshRef::Url("https://example.com/model.glb".to_string())), ..image_builder() };
      assert!(matches!(build_fal_hunyuan_3d_3p1_pro_state(builder).expect("build"), FalHunyuan3d3p1ProState::Image(_)));
    }
  }
