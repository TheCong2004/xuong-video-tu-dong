...
pub(crate) fn fal_mesh_cost_estimate(cost_in_usd_cents: u64) -> MeshGenerationCostEstimate {
  MeshGenerationCostEstimate { cost_in_credits: Some(cost_in_usd_cents), cost_in_usd_cents: Some(cost_in_usd_cents), is_free: false, is_unlimited: false, is_rate_limited: false, has_watermark: false, failures_are_refunded: None }
}
...
mod tests {
  ...
  mod image_mode_costs {
    ...
    #[test]
    fn base_cost_table() {
      let cases = [(None, 38), (Some(CommonMeshOutputType::Normal), 38), (Some(CommonMeshOutputType::LowPoly), 45), (Some(CommonMeshOutputType::Geometry), 23)];
    ...
    #[test]
    fn add_ons_stack() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::LowPoly), enable_pbr: Some(true), face_count: Some(100_000), back_image: Some(ImageRef::Url("https://example.com/back.png".to_string())), ..image_builder() };
  ...
  mod text_mode_costs {
    ...
    #[test]
    fn add_ons_stack() {
      let builder = GenerateMeshRequestBuilder { mesh_output_type: Some(CommonMeshOutputType::Geometry), enable_pbr: Some(true), face_count: Some(40_000), ..text_builder() };
  ...
  fn image_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3, provider: RouterProvider::Fal, reference_images: Some(ImageListRef::Urls(vec![FRONT_URL.to_string()])), ..Default::default() }
  }

  fn text_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3, provider: RouterProvider::Fal, prompt: Some("a red ceramic teapot".to_string()), ..Default::default() }
  }
