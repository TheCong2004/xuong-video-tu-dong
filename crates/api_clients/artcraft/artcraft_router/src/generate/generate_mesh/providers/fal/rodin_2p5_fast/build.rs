...
use fal_client::requests::api::mesh::text::rodin_2p5_fast_text_to_mesh::api::{Rodin2p5FastMaterial, Rodin2p5FastTextToMeshRequest};
...
pub(crate) fn build_fal_rodin_2p5_fast_state(mut builder: GenerateMeshRequestBuilder) -> Result<FalRodin2p5FastState, ArtcraftRouterError> {
  ...
  let material = plan_material(builder.mesh_output_type, builder.enable_pbr, builder.enable_texture, strategy)?;
  let image_urls = plan_image_urls(builder.reference_images.take(), builder.front_image.take(), strategy)?;

  if image_urls.is_empty() {
    // Text mode.
    let prompt = builder.prompt.take().ok_or_else(|| ArtcraftRouterError::InvalidInput("Rodin v2.5 Fast requires an input image or a prompt".to_string()))?;

    let request = Rodin2p5FastTextToMeshRequest { prompt, tier: None, seed: None, geometry_file_format: None, material, quality_mesh_option: None, texture_mode: None, enable_creative_mode: None, hd_texture: None, texture_delight: None, is_micro: None, ta_pose: None, bbox_condition: None };
    Ok(FalRodin2p5FastState::Text(FalRodin2p5FastTextRequestState { request }))
  } else {
    // Image mode. The prompt is optional guidance here.
    let request = Rodin2p5FastImageToMeshRequest { image_urls, prompt: builder.prompt.take(), use_original_alpha: None, tier: None, seed: None, geometry_file_format: None, material, quality_mesh_option: None, texture_mode: None, enable_creative_mode: None, hd_texture: None, texture_delight: None, is_micro: None, ta_pose: None, bbox_condition: None, preview_render: None };
...
fn plan_image_urls(reference_images: Option<ImageListRef>, front_image: Option<ImageRef>, strategy: RequestMismatchMitigationStrategy) -> Result<Vec<String>, ArtcraftRouterError> {
  ...
  if urls.len() > MAX_INPUT_IMAGES {
    match strategy {
      RequestMismatchMitigationStrategy::ErrorOut => {
        return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "reference_images", value: format!("Expected at most {MAX_INPUT_IMAGES} images, got {}", urls.len()) }));
      },
...
/// otherwise.
fn plan_material(mesh_output_type: Option<CommonMeshOutputType>, enable_pbr: Option<bool>, enable_texture: Option<bool>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<Rodin2p5FastMaterial>, ArtcraftRouterError> {
  ...
  let skip_texturing = matches!(mesh_output_type, Some(CommonMeshOutputType::Geometry)) || enable_texture == Some(false);
  if skip_texturing {
...
mod tests {
  ...
  mod image_mode {
    ...
    fn multiple_reference_urls_collect_into_image_urls() {
      let builder = GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(vec![FRONT_URL.to_string(), SECOND_URL.to_string()])), front_image: Some(ImageRef::Url("https://example.com/third.png".to_string())), ..base_builder() };
      let image = expect_image(build_fal_rodin_2p5_fast_state(builder).expect("build"));
      assert_eq!(image.request.image_urls, vec![FRONT_URL.to_string(), SECOND_URL.to_string(), "https://example.com/third.png".to_string(),]);
    }

    #[test]
    fn more_than_five_images_error_out_under_error_out() {
      let builder = GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(six_urls())), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..base_builder() };
      assert!(matches!(build_fal_rodin_2p5_fast_state(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { .. }))));
    }

    #[test]
    fn more_than_five_images_truncate_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(six_urls())), ..base_builder() };
      let image = expect_image(build_fal_rodin_2p5_fast_state(builder).expect("build"));
    ...
    fn prompt_is_forwarded_as_guidance() {
      let builder = GenerateMeshRequestBuilder { prompt: Some(PROMPT.to_string()), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..image_builder() };
      let image = expect_image(build_fal_rodin_2p5_fast_state(builder).expect("build"));
    ...
    fn reference_media_tokens_are_rejected() {
      ...
      assert!(matches!(build_fal_rodin_2p5_fast_state(builder), Err(ArtcraftRouterError::Client(ClientError::FalOnlySupportsUrls))));
    }
  ...
  mod unsupported_options {
    ...
    fn unsupported_options_error_out_under_error_out() {
      let cases: Vec<(&str, GenerateMeshRequestBuilder)> = vec![
        ("polygon_type", GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Quad), ..error_out_image_builder() }),
        ("face_count", GenerateMeshRequestBuilder { face_count: Some(100_000), ..error_out_image_builder() }),
        ("texture_quality", GenerateMeshRequestBuilder { texture_quality: Some(CommonMeshQuality::Detailed), ..error_out_image_builder() }),
        ("geometry_quality", GenerateMeshRequestBuilder { geometry_quality: Some(CommonMeshQuality::Detailed), ..error_out_image_builder() }),
        ("input_mesh", GenerateMeshRequestBuilder { input_mesh: Some(MeshRef::Url("https://example.com/mesh.glb".to_string())), ..error_out_image_builder() }),
        ("back_image", GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url("https://example.com/back.png".to_string())), ..error_out_image_builder() }),
        ("left_image", GenerateMeshRequestBuilder { left_image: Some(ImageRef::Url("https://example.com/left.png".to_string())), ..error_out_image_builder() }),
        ("right_image", GenerateMeshRequestBuilder { right_image: Some(ImageRef::Url("https://example.com/right.png".to_string())), ..error_out_image_builder() }),
      ];
      for (field, builder) in cases {
        assert!(build_fal_rodin_2p5_fast_state(builder).is_err(), "expected error for {field}",);
      }
    }

    #[test]
    fn unsupported_options_are_dropped_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Quad), face_count: Some(100_000), texture_quality: Some(CommonMeshQuality::Detailed), geometry_quality: Some(CommonMeshQuality::Standard), input_mesh: Some(MeshRef::Url("https://example.com/mesh.glb".to_string())), back_image: Some(ImageRef::Url("https://example.com/back.png".to_string())), ..image_builder() };
      assert!(matches!(build_fal_rodin_2p5_fast_state(builder).expect("build"), FalRodin2p5FastState::Image(_)));
    }
  }
  ...
  fn error_out_image_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..image_builder() }
  }

  fn text_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { prompt: Some(PROMPT.to_string()), ..base_builder() }
  }
