...
use crate::generate::generate_mesh::generate_mesh_response::{FalMeshResponsePayload, GenerateMeshResponse};
...
pub(crate) async fn send_fal_mesh_request<R>(request: &R, client: &RouterFalClient) -> Result<GenerateMeshResponse, ArtcraftRouterError>
where
  R: FalEndpoint + Clone + Debug + Send + Sync + 'static,
{
  ...
  let payload = if let Some(webhook_url) = &client.webhook_url {
    ...
    FalMeshResponsePayload { request_id: response.request_id, gateway_request_id: response.gateway_request_id, maybe_status_url: None, maybe_response_url: None, maybe_outbound_request: Some(outbound) }
  } else {
    let response = request.send_queue_request(&client.api_key).await?;
    FalMeshResponsePayload { request_id: Some(response.request_id), gateway_request_id: None, maybe_status_url: Some(response.status_url), maybe_response_url: Some(response.response_url), maybe_outbound_request: Some(outbound) }
  };
...
mod tests {
  ...
  async fn send_hunyuan3d_3_image_to_mesh() {
    let response = run_pipeline(GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(vec![ERNEST_SCARED_STUPID_IMAGE_URL.to_string()])), ..hunyuan3d_3_builder() }).await;
  ...
  async fn send_hunyuan3d_3_text_to_mesh() {
    let response = run_pipeline(GenerateMeshRequestBuilder { prompt: Some("A velociraptor with an open mouth full of sharp teeth.".to_string()), ..hunyuan3d_3_builder() }).await;
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
    assert_eq!(1, 2, "Inspect output above");
  }
