...
use crate::generate::generate_mesh::providers::artcraft::resolve::{resolve_image_list_ref, resolve_image_ref, resolve_mesh_ref};
...
pub fn build_artcraft_omni_mesh_request(mut builder: GenerateMeshRequestBuilder, model: CommonMeshModelEnum) -> Result<OmniGenMeshCostAndGenerateRequest, ArtcraftRouterError> {
  ...
  Ok(OmniGenMeshCostAndGenerateRequest { idempotency_token: Some(idempotency_token), model: Some(model), prompt: builder.prompt.take(), reference_image_media_tokens, front_image_media_token, back_image_media_token, left_image_media_token, right_image_media_token, input_mesh_media_token, mesh_output_type: builder.mesh_output_type, polygon_type: builder.polygon_type, face_count: builder.face_count, enable_pbr: builder.enable_pbr, enable_texture: builder.enable_texture, texture_quality: builder.texture_quality, geometry_quality: builder.geometry_quality })
}
...
mod tests {
  ...
  fn all_fields_are_forwarded() {
    let builder = GenerateMeshRequestBuilder { prompt: Some("a red ceramic teapot".to_string()), reference_images: Some(ImageListRef::MediaFileTokens(vec![MediaFileToken::new("mf_ref".to_string())])), front_image: Some(ImageRef::MediaFileToken(MediaFileToken::new("mf_front".to_string()))), back_image: Some(ImageRef::MediaFileToken(MediaFileToken::new("mf_back".to_string()))), left_image: Some(ImageRef::MediaFileToken(MediaFileToken::new("mf_left".to_string()))), right_image: Some(ImageRef::MediaFileToken(MediaFileToken::new("mf_right".to_string()))), mesh_output_type: Some(CommonMeshOutputType::LowPoly), polygon_type: Some(CommonPolygonType::Quad), face_count: Some(100_000), enable_pbr: Some(true), idempotency_token: Some("test-token".to_string()), ..base_builder() };

    let request = build_artcraft_omni_mesh_request(builder, CommonMeshModelEnum::Hunyuan3d3).expect("build");
  ...
  fn idempotency_token_is_generated_when_missing() {
    let request = build_artcraft_omni_mesh_request(base_builder(), CommonMeshModelEnum::Hunyuan3d3).expect("build");
  ...
  fn reference_image_url_references_are_rejected() {
    let builder = GenerateMeshRequestBuilder { reference_images: Some(ImageListRef::Urls(vec!["https://example.com/a.png".to_string()])), ..base_builder() };
    let result = build_artcraft_omni_mesh_request(builder, CommonMeshModelEnum::Hunyuan3d3);
    assert!(matches!(result, Err(ArtcraftRouterError::Client(ClientError::ArtcraftOnlySupportsMediaTokens))));
  }

  #[test]
  fn side_image_url_references_are_rejected() {
    let builder = GenerateMeshRequestBuilder { back_image: Some(ImageRef::Url("https://example.com/back.png".to_string())), ..base_builder() };
    let result = build_artcraft_omni_mesh_request(builder, CommonMeshModelEnum::Hunyuan3d3);
    assert!(matches!(result, Err(ArtcraftRouterError::Client(ClientError::ArtcraftOnlySupportsMediaTokens))));
  }
  ...
  fn base_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3, provider: RouterProvider::Artcraft, prompt: Some("a red ceramic teapot".to_string()), ..Default::default() }
  }
}
