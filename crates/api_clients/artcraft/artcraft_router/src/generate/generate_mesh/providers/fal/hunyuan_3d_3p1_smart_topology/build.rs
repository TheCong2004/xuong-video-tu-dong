...
use fal_client::requests::api::mesh::topology::hunyuan_3d_3p1_smart_topology::api::{Hunyuan3d3p1SmartTopologyInputFileType, Hunyuan3d3p1SmartTopologyPolygonType, Hunyuan3d3p1SmartTopologyRequest};
...
pub(crate) fn build_fal_hunyuan_3d_3p1_smart_topology_state(mut builder: GenerateMeshRequestBuilder) -> Result<FalHunyuan3d3p1SmartTopologyRequestState, ArtcraftRouterError> {
  let strategy = builder.request_mismatch_mitigation_strategy;

  let input_file_url = plan_input_mesh_url(builder.input_mesh.take())?.ok_or_else(|| ArtcraftRouterError::InvalidInput("Hunyuan 3D v3.1 Smart Topology requires an input mesh file (GLB or OBJ)".to_string()))?;
  ...
  let request = Hunyuan3d3p1SmartTopologyRequest { input_file_url, input_file_type, polygon_type: builder.polygon_type.map(to_polygon_type), face_level: None };
...
fn infer_input_file_type(url: &str) -> Option<Hunyuan3d3p1SmartTopologyInputFileType> {
  ...
  if path.to_ascii_lowercase().ends_with(".obj") { Some(Hunyuan3d3p1SmartTopologyInputFileType::Obj) } else { None }
}
...
mod tests {
  ...
  mod shape_dispatch {
    ...
    fn input_mesh_url_builds_the_request() {
      let state = build_fal_hunyuan_3d_3p1_smart_topology_state(mesh_builder(GLB_URL)).expect("build");
    ...
    fn media_token_input_mesh_is_rejected() {
      ...
      assert!(matches!(build_fal_hunyuan_3d_3p1_smart_topology_state(builder), Err(ArtcraftRouterError::Client(ClientError::FalOnlySupportsUrls))));
    }
  }
  ...
  mod option_mapping {
    ...
    fn obj_url_infers_obj_input_file_type() {
      let state = build_fal_hunyuan_3d_3p1_smart_topology_state(mesh_builder(OBJ_URL)).expect("build");
      assert!(matches!(state.request.input_file_type, Some(Hunyuan3d3p1SmartTopologyInputFileType::Obj)));
    }

    #[test]
    fn obj_inference_is_case_insensitive_and_ignores_the_query_string() {
      let cases = ["https://example.com/model.OBJ", "https://example.com/model.obj?token=abc123", "https://example.com/model.Obj#fragment"];
      for url in cases {
        assert!(matches!(infer_input_file_type(url), Some(Hunyuan3d3p1SmartTopologyInputFileType::Obj)), "for {url}",);
      }
    }

    #[test]
    fn non_obj_urls_leave_input_file_type_unset() {
      let cases = [GLB_URL, "https://example.com/model.fbx", "https://example.com/model.glb?format=obj"];
      for url in cases {
    ...
    fn polygon_type_maps_through() {
      let builder = GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Quad), ..mesh_builder(GLB_URL) };
      let state = build_fal_hunyuan_3d_3p1_smart_topology_state(builder).expect("build");
      assert!(matches!(state.request.polygon_type, Some(Hunyuan3d3p1SmartTopologyPolygonType::Quadrilateral)));

      let builder = GenerateMeshRequestBuilder { polygon_type: Some(CommonPolygonType::Triangle), ..mesh_builder(GLB_URL) };
      let state = build_fal_hunyuan_3d_3p1_smart_topology_state(builder).expect("build");
      assert!(matches!(state.request.polygon_type, Some(Hunyuan3d3p1SmartTopologyPolygonType::Triangle)));
    }
  }
  ...
  mod unsupported_options {
    ...
    fn each_unsupported_option_errors_out_under_error_out() {
      let cases: Vec<fn(&mut GenerateMeshRequestBuilder)> = vec![|b| b.prompt = Some("a red ceramic teapot".to_string()), |b| b.front_image = Some(ImageRef::Url("https://example.com/front.png".to_string())), |b| b.back_image = Some(ImageRef::Url("https://example.com/back.png".to_string())), |b| b.mesh_output_type = Some(CommonMeshOutputType::Normal), |b| b.face_count = Some(100_000), |b| b.enable_pbr = Some(true), |b| b.enable_texture = Some(false)];
      for (index, set) in cases.into_iter().enumerate() {
        let mut builder = GenerateMeshRequestBuilder { request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..mesh_builder(GLB_URL) };
        set(&mut builder);
    ...
    fn unsupported_options_are_dropped_under_lenient_strategies() {
      let builder = GenerateMeshRequestBuilder { prompt: Some("a red ceramic teapot".to_string()), mesh_output_type: Some(CommonMeshOutputType::Normal), face_count: Some(100_000), enable_pbr: Some(true), ..mesh_builder(GLB_URL) };
      let state = build_fal_hunyuan_3d_3p1_smart_topology_state(builder).expect("build");
      assert_eq!(state.request.input_file_url, GLB_URL);
    }
  }
  ...
  fn base_builder() -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { model: RouterMeshModel::Hunyuan3d3p1SmartTopology, provider: RouterProvider::Fal, ..Default::default() }
  }

  fn mesh_builder(url: &str) -> GenerateMeshRequestBuilder {
    GenerateMeshRequestBuilder { input_mesh: Some(MeshRef::Url(url.to_string())), ..base_builder() }
  }
}
