...
mod tests {
  ...
  fn cost_cents(image_batch_count: u16) -> u64 {
    let builder = GenerateImageRequestBuilder { model: RouterImageModel::FluxPro11Ultra, provider: RouterProvider::Artcraft, prompt: None, image_inputs: None, resolution: None, aspect_ratio: None, quality: None, image_batch_count: Some(image_batch_count), horizontal_angle: None, vertical_angle: None, zoom: None, request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, generation_mode_mismatch_strategy: None, idempotency_token: None };
    builder.build2().unwrap().estimate_cost().unwrap().cost_in_usd_cents.unwrap()
  }

  #[test]
  fn one_image_costs_6_cents() {
    assert_eq!(cost_cents(1), 6);
  }

  #[test]
  fn two_images_costs_12_cents() {
    assert_eq!(cost_cents(2), 12);
  }

  #[test]
  fn three_images_costs_18_cents() {
    assert_eq!(cost_cents(3), 18);
  }

  #[test]
  fn four_images_costs_24_cents() {
    assert_eq!(cost_cents(4), 24);
  }
}
