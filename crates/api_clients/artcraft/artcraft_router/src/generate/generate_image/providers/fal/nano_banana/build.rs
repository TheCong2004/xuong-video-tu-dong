use fal_client::requests_old::webhook::image::edit::enqueue_gemini_25_flash_edit_webhook::{Gemini25FlashEditAspectRatio, Gemini25FlashEditNumImages, Gemini25FlashEditRequest};
use fal_client::requests_old::webhook::image::text::enqueue_gemini_25_flash_text_to_image_webhook::{Gemini25FlashTextToImageAspectRatio, Gemini25FlashTextToImageNumImages, Gemini25FlashTextToImageRequest};
...
pub fn build_fal_nano_banana(builder: GenerateImageRequestBuilder) -> Result<ImageGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  let state = if image_urls.is_empty() {
    ...
    FalNanoBananaRequestState::TextToImage(Gemini25FlashTextToImageRequest { prompt, num_images: to_t2i_num_images(num_images), aspect_ratio })
  } else {
    let aspect_ratio = plan_edit_aspect_ratio(builder.aspect_ratio, strategy)?;
    FalNanoBananaRequestState::EditImage(Gemini25FlashEditRequest { prompt, image_urls, num_images: to_edit_num_images(num_images), aspect_ratio })
  };

  Ok(ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalNanoBanana(state)))
}
...
fn plan_t2i_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<Gemini25FlashTextToImageAspectRatio>, ArtcraftRouterError> {
  use Gemini25FlashTextToImageAspectRatio as T;
  match aspect_ratio {
    None => Ok(None),

    Some(RouterAspectRatio::Auto) | Some(RouterAspectRatio::Auto2k) | Some(RouterAspectRatio::Auto4k) => Ok(Some(T::OneByOne)),
...
fn plan_edit_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<Gemini25FlashEditAspectRatio>, ArtcraftRouterError> {
  use Gemini25FlashEditAspectRatio as E;
  match aspect_ratio {
    None => Ok(None),

    Some(RouterAspectRatio::Auto) | Some(RouterAspectRatio::Auto2k) | Some(RouterAspectRatio::Auto4k) => Ok(Some(E::Auto)),
...
mod tests {
  ...
  fn unwrap_edit(result: Result<ImageGenerationDraftOrRequest, ArtcraftRouterError>) -> Gemini25FlashEditRequest {
    let ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalNanoBanana(FalNanoBananaRequestState::EditImage(req))) = result.expect("build should succeed") else {
  ...
  mod mode_detection {
    ...
    fn media_tokens_return_error() {
      ...
      assert!(matches!(build_fal_nano_banana(builder), Err(ArtcraftRouterError::Client(ClientError::FalOnlySupportsUrls))));
    }
  }
  ...
  mod num_images_tests {
    ...
    fn zero_is_error() {
      ...
      assert!(matches!(build_fal_nano_banana(builder), Err(ArtcraftRouterError::Client(ClientError::UserRequestedZeroGenerations))));
    }

    #[test]
    fn over_four_error_out() {
      let builder = GenerateImageRequestBuilder { image_batch_count: Some(9), ..base_builder() };
      assert!(matches!(build_fal_nano_banana(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_batch_count", .. }))));
    }
