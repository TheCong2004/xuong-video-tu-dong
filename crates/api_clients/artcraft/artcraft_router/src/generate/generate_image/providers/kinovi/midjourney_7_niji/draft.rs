...
impl KinoviMidjourney7NijiDraftState {
  pub async fn to_request(&mut self, draft_context: &ImageGenerationDraftContext<'_>) -> Result<KinoviMidjourney7NijiRequestState, ArtcraftRouterError> {
    ...
    let request = GenerateMidjourneyV7NijiRequest { prompt: self.prompt.clone(), aspect_ratio: self.aspect_ratio, negative_prompt: None, stylize: None, weird: None, chaos: None, quality: self.quality, raw_mode: false, batch_count: self.batch_count, reference_image_urls };
