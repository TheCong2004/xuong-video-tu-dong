...
use fal_client::requests_old::webhook::image::text::enqueue_flux_pro_11_ultra_text_to_image_webhook::{enqueue_flux_pro_11_ultra_text_to_image_webhook, FluxPro11UltraArgs, FluxPro11UltraRequest};
...
impl FalFluxPro1p1UltraRequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    ...
    let args = FluxPro11UltraArgs { request: self.request.clone(), webhook_url, api_key: &client.api_key };
    let webhook_response = enqueue_flux_pro_11_ultra_text_to_image_webhook(args).await.map_err(|e| ArtcraftRouterError::Provider(ProviderError::Fal(e)))?;
...
mod tests {
  ...
  use fal_client::requests_old::webhook::image::text::enqueue_flux_pro_11_ultra_text_to_image_webhook::{FluxPro11UltraAspectRatio, FluxPro11UltraNumImages};
  ...
  fn t2i_state() -> FalFluxPro1p1UltraRequestState {
    FalFluxPro1p1UltraRequestState { request: FluxPro11UltraRequest { prompt: "a corgi wearing sunglasses on a surfboard".to_string(), aspect_ratio: FluxPro11UltraAspectRatio::LandscapeSixteenByNine, num_images: FluxPro11UltraNumImages::One } }
  }
