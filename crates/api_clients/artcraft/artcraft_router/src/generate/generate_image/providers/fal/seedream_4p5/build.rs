use fal_client::requests_old::webhook::image::edit::enqueue_bytedance_seedream_v4p5_edit_image_webhook::{EnqueueBytedanceSeedreamV4p5EditImageNumImages, EnqueueBytedanceSeedreamV4p5EditImageRequest, EnqueueBytedanceSeedreamV4p5EditImageSize};
use fal_client::requests_old::webhook::image::text::enqueue_bytedance_seedream_v4p5_text_to_image_webhook::{EnqueueBytedanceSeedreamV4p5TextToImageNumImages, EnqueueBytedanceSeedreamV4p5TextToImageRequest, EnqueueBytedanceSeedreamV4p5TextToImageSize};
...
pub fn build_fal_seedream_4p5(builder: GenerateImageRequestBuilder) -> Result<ImageGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  let state = if image_urls.is_empty() { FalSeedream4p5RequestState::TextToImage(EnqueueBytedanceSeedreamV4p5TextToImageRequest { prompt, num_images: Some(to_t2i_num_images(num_images)), max_images: None, image_size: image_size.map(to_t2i_image_size) }) } else { FalSeedream4p5RequestState::EditImage(EnqueueBytedanceSeedreamV4p5EditImageRequest { prompt, image_urls, num_images: Some(to_edit_num_images(num_images)), max_images: None, image_size: image_size.map(to_edit_image_size) }) };

  Ok(ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalSeedream4p5(state)))
}
...
fn plan_image_size(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<PlannedImageSize>, ArtcraftRouterError> {
...
mod tests {
  ...
  fn unwrap_t2i(result: Result<ImageGenerationDraftOrRequest, ArtcraftRouterError>) -> EnqueueBytedanceSeedreamV4p5TextToImageRequest {
    let ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalSeedream4p5(FalSeedream4p5RequestState::TextToImage(req))) = result.expect("build should succeed") else {
  ...
  fn media_tokens_return_error() {
    ...
    assert!(matches!(build_fal_seedream_4p5(builder), Err(ArtcraftRouterError::Client(ClientError::FalOnlySupportsUrls))));
  }

  #[test]
  fn zero_batch_is_error() {
    let builder = GenerateImageRequestBuilder { image_batch_count: Some(0), ..base_builder() };
    assert!(matches!(build_fal_seedream_4p5(builder), Err(ArtcraftRouterError::Client(ClientError::UserRequestedZeroGenerations))));
  }
}
