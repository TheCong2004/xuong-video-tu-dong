use seedance2pro_client::generate::image::generate_midjourney_v7::{generate_midjourney_v7, GenerateMidjourneyV7Args, GenerateMidjourneyV7Request};
...
#[derive(Debug, Clone)]
pub struct KinoviMidjourney7RequestState {
  /// Final materialized request; ready to fire against Kinovi.
  pub request: GenerateMidjourneyV7Request,
}

impl KinoviMidjourney7RequestState {
  pub async fn send(&self, client: &RouterSeedance2ProClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    let args = GenerateMidjourneyV7Args { session: &client.session, host_override: None, request: self.request.clone() };

    let response = generate_midjourney_v7(args).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::Seedance2Pro(err)))?;
...
mod tests {
  ...
  async fn text_to_image_round_trip() {
    ...
    println!("v7 t2i — task_id={}, order_id={}", payload.task_id, payload.order_id);
  ...
  async fn image_reference_round_trip_via_draft() {
    let client = get_kinovi_client();
    let token = MediaFileToken::new("mf_test_juno".to_string());

    let mut map = HashMap::new();
    map.insert(token.clone(), JUNO_AT_LAKE_IMAGE_URL.to_string());

    let builder = GenerateImageRequestBuilder { prompt: Some("a corgi in the style of @1".to_string()), image_inputs: Some(ImageListRef::MediaFileTokens(vec![token])), ..base_builder() };
    ...
    let request = ImageGenerationDraftRequest::KinoviMidjourney7(draft).finalize(ctx).await.expect("finalize");

    let response = request.send_request(&client).await.expect("send_request");
    let payload = response.get_seedance2pro_payload().expect("expected seedance2pro payload");
    println!("v7 with ref — task_id={}, order_id={}", payload.task_id, payload.order_id);
    assert!(!payload.task_id.is_empty());
  }
}
