...
use fal_client::requests_old::webhook::image::edit::enqueue_bytedance_seedream_v4p5_edit_image_webhook::{enqueue_bytedance_seedream_v4p5_edit_image_webhook, EnqueueBytedanceSeedreamV4p5EditImageArgs, EnqueueBytedanceSeedreamV4p5EditImageRequest};
use fal_client::requests_old::webhook::image::text::enqueue_bytedance_seedream_v4p5_text_to_image_webhook::{enqueue_bytedance_seedream_v4p5_text_to_image_webhook, EnqueueBytedanceSeedreamV4p5TextToImageArgs, EnqueueBytedanceSeedreamV4p5TextToImageRequest};
  ...
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    ...
    let (webhook_response, outbound): (_, Arc<dyn Debug + Send + Sync>) = match self {
      Self::TextToImage(request) => {
        ...
        let args = EnqueueBytedanceSeedreamV4p5TextToImageArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
        (enqueue_bytedance_seedream_v4p5_text_to_image_webhook(args).await, outbound)
      },
      Self::EditImage(request) => {
        let outbound: Arc<dyn Debug + Send + Sync> = Arc::new(request.clone());
        let args = EnqueueBytedanceSeedreamV4p5EditImageArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
...
mod tests {
  ...
  async fn send_t2i_via_webhook() {
    ...
    let state = FalSeedream4p5RequestState::TextToImage(EnqueueBytedanceSeedreamV4p5TextToImageRequest { prompt: "a corgi wearing sunglasses on a surfboard".to_string(), num_images: Some(EnqueueBytedanceSeedreamV4p5TextToImageNumImages::One), max_images: None, image_size: None });
