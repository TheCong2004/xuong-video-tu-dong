...
mod tests {
  ...
  fn t2i_cost(n: EnqueueBytedanceSeedreamV5LiteTextToImageNumImages) -> ImageGenerationCostEstimate {
    FalSeedream5LiteCostState::from_request(&FalSeedream5LiteRequestState::TextToImage(EnqueueBytedanceSeedreamV5LiteTextToImageRequest { prompt: "test".to_string(), num_images: Some(n), max_images: None, image_size: None })).estimate_cost()
  }

  fn edit_cost(n: EnqueueBytedanceSeedreamV5LiteEditImageNumImages) -> ImageGenerationCostEstimate {
    FalSeedream5LiteCostState::from_request(&FalSeedream5LiteRequestState::EditImage(EnqueueBytedanceSeedreamV5LiteEditImageRequest { prompt: "test".to_string(), image_urls: vec!["https://example.com/x.jpg".to_string()], num_images: Some(n), max_images: None, image_size: None })).estimate_cost()
  }
  ...
  fn edit_two_images_matches_t2i_two_images() {
    assert_eq!(edit_cost(EnqueueBytedanceSeedreamV5LiteEditImageNumImages::Two).cost_in_usd_cents, t2i_cost(EnqueueBytedanceSeedreamV5LiteTextToImageNumImages::Two).cost_in_usd_cents,);
  }
