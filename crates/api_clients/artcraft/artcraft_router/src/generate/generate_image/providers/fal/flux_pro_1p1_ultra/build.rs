use fal_client::requests_old::webhook::image::text::enqueue_flux_pro_11_ultra_text_to_image_webhook::{FluxPro11UltraAspectRatio, FluxPro11UltraNumImages, FluxPro11UltraRequest};
...
pub fn build_fal_flux_pro_1p1_ultra(builder: GenerateImageRequestBuilder) -> Result<ImageGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  if builder.image_inputs.is_some() {
    return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_inputs", value: "Flux Pro 1.1 Ultra is text-to-image only".to_string() }));
  }
  ...
  let request = FluxPro11UltraRequest { prompt: builder.prompt.clone().unwrap_or_default(), aspect_ratio, num_images };

  Ok(ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalFluxPro1p1Ultra(FalFluxPro1p1UltraRequestState { request })))
}

fn plan_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<FluxPro11UltraAspectRatio, ArtcraftRouterError> {
  use FluxPro11UltraAspectRatio as Ar;
  match aspect_ratio {
    None => Ok(Ar::Square),

    Some(RouterAspectRatio::Auto) | Some(RouterAspectRatio::Auto2k) | Some(RouterAspectRatio::Auto4k) => Ok(Ar::Square),
...
fn plan_num_images(image_batch_count: Option<u16>, strategy: RequestMismatchMitigationStrategy) -> Result<FluxPro11UltraNumImages, ArtcraftRouterError> {
...
mod tests {
  ...
  fn image_inputs_present_yields_error() {
    ...
    assert!(matches!(build_fal_flux_pro_1p1_ultra(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_inputs", .. }))));
  }
  ...
  mod aspect_ratio_tests {
    ...
    fn wide_three_by_two_has_direct_mapping() {
      let builder = GenerateImageRequestBuilder { aspect_ratio: Some(RouterAspectRatio::WideThreeByTwo), ..base_builder() };
      let req = unwrap_request(build_fal_flux_pro_1p1_ultra(builder));
    ...
    fn tall_two_by_three_has_direct_mapping() {
      let builder = GenerateImageRequestBuilder { aspect_ratio: Some(RouterAspectRatio::TallTwoByThree), ..base_builder() };
      let req = unwrap_request(build_fal_flux_pro_1p1_ultra(builder));
  ...
  mod num_images_tests {
    ...
    fn zero_is_error() {
      ...
      assert!(matches!(build_fal_flux_pro_1p1_ultra(builder), Err(ArtcraftRouterError::Client(ClientError::UserRequestedZeroGenerations))));
    }

    #[test]
    fn over_four_error_out() {
      let builder = GenerateImageRequestBuilder { image_batch_count: Some(5), ..base_builder() };
      assert!(matches!(build_fal_flux_pro_1p1_ultra(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_batch_count", .. }))));
    }
