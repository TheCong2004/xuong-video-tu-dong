...
use fal_client::requests_old::webhook::image::text::enqueue_flux_pro_11_text_to_image_webhook::{enqueue_flux_pro_11_text_to_image_webhook, FluxPro11Args, FluxPro11Request};
...
impl FalFluxPro1p1RequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    ...
    let args = FluxPro11Args { request: self.request.clone(), webhook_url, api_key: &client.api_key };
    let webhook_response = enqueue_flux_pro_11_text_to_image_webhook(args).await.map_err(|e| ArtcraftRouterError::Provider(ProviderError::Fal(e)))?;
...
mod tests {
  ...
  use fal_client::requests_old::webhook::image::text::enqueue_flux_pro_11_text_to_image_webhook::{FluxPro11AspectRatio, FluxPro11NumImages};
  ...
  fn t2i_state() -> FalFluxPro1p1RequestState {
    FalFluxPro1p1RequestState { request: FluxPro11Request { prompt: "a corgi wearing sunglasses on a surfboard".to_string(), aspect_ratio: FluxPro11AspectRatio::LandscapeSixteenByNine, num_images: FluxPro11NumImages::One } }
  }
