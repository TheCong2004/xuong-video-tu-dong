use seedance2pro_client::generate::image::generate_midjourney_v8::{generate_midjourney_v8, GenerateMidjourneyV8Args, GenerateMidjourneyV8Request};
...
impl KinoviMidjourney8RequestState {
  pub async fn send(&self, client: &RouterSeedance2ProClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    let args = GenerateMidjourneyV8Args { session: &client.session, host_override: None, request: self.request.clone() };

    let response = generate_midjourney_v8(args).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::Seedance2Pro(err)))?;
...
mod tests {
  ...
  async fn image_reference_round_trip_via_draft() {
    ...
    let builder = GenerateImageRequestBuilder { prompt: Some("a dramatic painterly portrait inspired by @1".to_string()), image_inputs: Some(ImageListRef::MediaFileTokens(vec![token])), ..base_builder() };
    ...
    let request = ImageGenerationDraftRequest::KinoviMidjourney8(draft).finalize(ctx).await.expect("finalize");
