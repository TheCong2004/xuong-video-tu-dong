...
fn cost_per_image_in_cents(resolution: Option<CommonResolutionEnum>) -> u64 {
  ...
  match resolution {
    Some(R::HalfK) => 6,
    None | Some(R::OneK) | Some(R::FourEightyP) | Some(R::SevenTwentyP) | Some(R::TenEightyP) => 8,
...
mod tests {
  ...
  fn cost_cents(resolution: Option<RouterResolution>, image_batch_count: u16) -> u64 {
    let builder = GenerateImageRequestBuilder { model: RouterImageModel::NanoBanana2, provider: RouterProvider::Artcraft, prompt: None, image_inputs: None, resolution, aspect_ratio: None, quality: None, image_batch_count: Some(image_batch_count), horizontal_angle: None, vertical_angle: None, zoom: None, request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, generation_mode_mismatch_strategy: None, idempotency_token: None };
  ...
  #[test]
  fn default_resolution_one_image_is_8c() {
    assert_eq!(cost_cents(None, 1), 8);
  }

  #[test]
  fn default_resolution_four_images_is_32c() {
    assert_eq!(cost_cents(None, 4), 32);
  }

  #[test]
  fn one_k_one_image_is_8c() {
    assert_eq!(cost_cents(Some(RouterResolution::OneK), 1), 8);
  }

  // ── 0.5K (6¢) ──────────────────────────────────────────────────────────────

  #[test]
  fn half_k_one_image_is_6c() {
    assert_eq!(cost_cents(Some(RouterResolution::HalfK), 1), 6);
  }

  #[test]
  fn half_k_four_images_is_24c() {
    assert_eq!(cost_cents(Some(RouterResolution::HalfK), 4), 24);
  }
  ...
  #[test]
  fn four_k_one_image_is_16c() {
    assert_eq!(cost_cents(Some(RouterResolution::FourK), 1), 16);
  }

  #[test]
  fn four_k_four_images_is_64c() {
    assert_eq!(cost_cents(Some(RouterResolution::FourK), 4), 64);
  }
}
