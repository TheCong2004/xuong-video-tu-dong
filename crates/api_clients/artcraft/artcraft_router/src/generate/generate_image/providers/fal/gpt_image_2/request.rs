...
mod tests {
  ...
  async fn send_text_to_image_webhook() {
    let request = FalGptImage2RequestState::TextToImage(GptImage2TextToImageRequest { prompt: "a polished product photo of a glass compass".to_string(), num_images: GptImage2TextToImageNumImages::One, image_size: Some(GptImage2TextToImageSize::Square), resolution: Some(GptImage2Resolution::OneK), quality: Some(GptImage2TextToImageQuality::Low), output_format: None });
  ...
  async fn send_edit_image_webhook() {
    let request = FalGptImage2RequestState::EditImage(GptImage2EditImageRequest { prompt: "make this look like an elegant magazine cover image".to_string(), image_urls: vec![JUNO_AT_LAKE_IMAGE_URL.to_string()], num_images: GptImage2EditImageNumImages::One, mask_url: None, image_size: Some(GptImage2EditImageSize::Square), resolution: Some(GptImage2Resolution::OneK), quality: Some(GptImage2EditImageQuality::Low), output_format: None });
    let response = request.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
