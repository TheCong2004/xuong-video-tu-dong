use seedance2pro_client::generate::image::generate_seedream_5p0_pro::{generate_seedream_5p0_pro, GenerateSeedream5p0ProArgs, GenerateSeedream5p0ProRequest};
...
impl KinoviSeedream5p0ProRequestState {
  pub async fn send(&self, client: &RouterSeedance2ProClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    let args = GenerateSeedream5p0ProArgs { session: &client.session, host_override: None, request: self.request.clone() };

    let response = generate_seedream_5p0_pro(args).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::Seedance2Pro(err)))?;
...
mod tests {
  ...
  async fn image_reference_round_trip_via_draft() {
    ...
    let builder = GenerateImageRequestBuilder { prompt: Some("The dog in @Image1 wearing a tiny crown in a royal palace".to_string()), image_inputs: Some(ImageListRef::MediaFileTokens(vec![token])), ..base_builder() };
    ...
    let request = ImageGenerationDraftRequest::KinoviSeedream5p0Pro(draft).finalize(ctx).await.expect("finalize");
