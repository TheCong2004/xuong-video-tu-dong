...
mod tests {
  ...
  async fn send_text_to_image_webhook() {
    let request = FalGptImage1p5RequestState::TextToImage(GptImage1p5TextToImageRequest { prompt: "a tiny brass observatory in a snowy field".to_string(), num_images: GptImage1p5TextToImageNumImages::One, image_size: Some(GptImage1p5TextToImageSize::Square), background: None, quality: Some(GptImage1p5TextToImageQuality::Low), output_format: None });
  ...
  async fn send_edit_image_webhook() {
    let request = FalGptImage1p5RequestState::EditImage(GptImage1p5EditImageRequest { prompt: "turn this into a softly lit editorial photograph".to_string(), image_urls: vec![JUNO_AT_LAKE_IMAGE_URL.to_string()], num_images: GptImage1p5EditImageNumImages::One, mask_image_url: None, image_size: Some(GptImage1p5EditImageSize::Square), background: None, quality: Some(GptImage1p5EditImageQuality::Low), input_fidelity: None, output_format: None });
    let response = request.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
