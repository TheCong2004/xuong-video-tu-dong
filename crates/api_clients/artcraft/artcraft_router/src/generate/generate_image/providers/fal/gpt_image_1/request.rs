...
mod tests {
  ...
  fn client_with_webhook() -> RouterFalClient {
    ...
    RouterFalClient::new_with_webhook(FalApiKey::from_str(secret.trim()), "https://example.com/fal-webhook-test".to_string())
  }

  #[tokio::test]
  #[ignore] // requires real API key, incurs cost
  async fn send_text_to_image_webhook() {
    let request = FalGptImage1RequestState::TextToImage(GptImage1TextToImageRequest { prompt: "a small ceramic robot reading by a window".to_string(), num_images: GptImage1TextToImageNumImages::One, image_size: Some(GptImage1TextToImageSize::Square), quality: Some(GptImage1TextToImageQuality::Low), background: None, output_format: None });
  ...
  async fn send_edit_image_webhook() {
    let request = FalGptImage1RequestState::EditImage(GptImage1EditImageRequest { prompt: "make the image feel like a quiet watercolor study".to_string(), image_urls: vec![JUNO_AT_LAKE_IMAGE_URL.to_string()], num_images: GptImage1EditImageNumImages::One, mask_image_url: None, image_size: Some(GptImage1EditImageSize::Square), quality: Some(GptImage1EditImageQuality::Low), input_fidelity: None, background: None, output_format: None });
    let response = request.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
