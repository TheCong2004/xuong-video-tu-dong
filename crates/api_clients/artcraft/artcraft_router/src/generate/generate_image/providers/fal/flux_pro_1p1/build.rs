...
pub fn build_fal_flux_pro_1p1(builder: GenerateImageRequestBuilder) -> Result<ImageGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  if builder.image_inputs.is_some() {
    return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_inputs", value: "Flux Pro 1.1 is text-to-image only".to_string() }));
  }
  ...
  let request = FluxPro11Request { prompt: builder.prompt.clone().unwrap_or_default(), aspect_ratio, num_images };

  Ok(ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalFluxPro1p1(FalFluxPro1p1RequestState { request })))
}

fn plan_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<FluxPro11AspectRatio, ArtcraftRouterError> {
...
fn plan_num_images(image_batch_count: Option<u16>, strategy: RequestMismatchMitigationStrategy) -> Result<FluxPro11NumImages, ArtcraftRouterError> {
...
mod tests {
  ...
  fn image_inputs_present_yields_error() {
    ...
    assert!(matches!(build_fal_flux_pro_1p1(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_inputs", .. }))));
  }
  ...
  mod aspect_ratio_tests {
    ...
    fn tall_four_by_five_collapses_to_three_by_four() {
      let builder = GenerateImageRequestBuilder { aspect_ratio: Some(RouterAspectRatio::TallFourByFive), ..base_builder() };
      let req = unwrap_request(build_fal_flux_pro_1p1(builder));
  ...
  mod num_images_tests {
    ...
    fn zero_is_error() {
      ...
      assert!(matches!(build_fal_flux_pro_1p1(builder), Err(ArtcraftRouterError::Client(ClientError::UserRequestedZeroGenerations))));
    }

    #[test]
    fn over_four_error_out() {
      let builder = GenerateImageRequestBuilder { image_batch_count: Some(7), ..base_builder() };
      assert!(matches!(build_fal_flux_pro_1p1(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_batch_count", .. }))));
    }
