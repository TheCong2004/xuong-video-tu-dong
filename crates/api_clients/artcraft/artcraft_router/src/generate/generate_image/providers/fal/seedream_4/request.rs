...
use fal_client::requests_old::webhook::image::edit::enqueue_bytedance_seedream_v4_edit_image_webhook::{enqueue_bytedance_seedream_v4_edit_image_webhook, EnqueueBytedanceSeedreamV4EditImageArgs, EnqueueBytedanceSeedreamV4EditImageRequest};
use fal_client::requests_old::webhook::image::text::enqueue_bytedance_seedream_v4_text_to_image_webhook::{enqueue_bytedance_seedream_v4_text_to_image_webhook, EnqueueBytedanceSeedreamV4TextToImageArgs, EnqueueBytedanceSeedreamV4TextToImageRequest};
  ...
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    ...
    let (webhook_response, outbound): (_, Arc<dyn Debug + Send + Sync>) = match self {
      Self::TextToImage(request) => {
        ...
        let args = EnqueueBytedanceSeedreamV4TextToImageArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
        (enqueue_bytedance_seedream_v4_text_to_image_webhook(args).await, outbound)
      },
      Self::EditImage(request) => {
        let outbound: Arc<dyn Debug + Send + Sync> = Arc::new(request.clone());
        let args = EnqueueBytedanceSeedreamV4EditImageArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
    ...
    Ok(GenerateImageResponse::Fal(FalImageResponsePayload { request_id: webhook_response.request_id, gateway_request_id: webhook_response.gateway_request_id, maybe_status_url: None, maybe_response_url: None, maybe_outbound_request: Some(outbound) }))
  }
}
...
mod tests {
  ...
  async fn send_t2i_via_webhook() {
    ...
    let state = FalSeedream4RequestState::TextToImage(EnqueueBytedanceSeedreamV4TextToImageRequest { prompt: "a corgi wearing sunglasses on a surfboard".to_string(), num_images: Some(EnqueueBytedanceSeedreamV4TextToImageNumImages::One), max_images: None, image_size: None });
  ...
  async fn send_edit_via_webhook() {
    let client = client_with_webhook();
    let state = FalSeedream4RequestState::EditImage(EnqueueBytedanceSeedreamV4EditImageRequest { prompt: "make it sunset".to_string(), image_urls: vec![JUNO_AT_LAKE_IMAGE_URL.to_string()], num_images: Some(EnqueueBytedanceSeedreamV4EditImageNumImages::One), max_images: None, image_size: None });
    let response = state.send(&client).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
