...
use fal_client::requests_old::webhook::image::edit::enqueue_gemini_25_flash_edit_webhook::{enqueue_gemini_25_flash_edit_webhook, Gemini25FlashEditArgs, Gemini25FlashEditRequest};
use fal_client::requests_old::webhook::image::text::enqueue_gemini_25_flash_text_to_image_webhook::{enqueue_gemini_25_flash_text_to_image_webhook, Gemini25FlashTextToImageArgs, Gemini25FlashTextToImageRequest};
  ...
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateImageResponse, ArtcraftRouterError> {
    ...
    let (webhook_response, outbound): (_, Arc<dyn Debug + Send + Sync>) = match self {
      Self::TextToImage(request) => {
        ...
        let args = Gemini25FlashTextToImageArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
        (enqueue_gemini_25_flash_text_to_image_webhook(args).await, outbound)
      },
      Self::EditImage(request) => {
        let outbound: Arc<dyn Debug + Send + Sync> = Arc::new(request.clone());
        let args = Gemini25FlashEditArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
...
mod tests {
  ...
  use fal_client::requests_old::webhook::image::text::enqueue_gemini_25_flash_text_to_image_webhook::{Gemini25FlashTextToImageAspectRatio, Gemini25FlashTextToImageNumImages};
  ...
  async fn send_t2i_via_webhook() {
    ...
    let state = FalNanoBananaRequestState::TextToImage(Gemini25FlashTextToImageRequest { prompt: "a corgi wearing sunglasses on a surfboard".to_string(), num_images: Gemini25FlashTextToImageNumImages::One, aspect_ratio: Some(Gemini25FlashTextToImageAspectRatio::SixteenByNine) });
  ...
  async fn send_edit_via_webhook() {
    let client = client_with_webhook();
    let state = FalNanoBananaRequestState::EditImage(Gemini25FlashEditRequest { prompt: "add a party hat to the dog".to_string(), image_urls: vec![JUNO_AT_LAKE_IMAGE_URL.to_string()], num_images: Gemini25FlashEditNumImages::One, aspect_ratio: None });
    let response = state.send(&client).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
