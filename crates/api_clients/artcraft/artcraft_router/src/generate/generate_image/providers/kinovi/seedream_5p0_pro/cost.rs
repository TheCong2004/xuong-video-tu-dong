...
impl KinoviSeedream5p0ProCostState {
  pub fn from_request(request: &KinoviSeedream5p0ProRequestState) -> Self {
    Self { resolution: request.request.resolution, batch_count: request.request.batch_count }
  }

  pub fn from_draft(draft: &KinoviSeedream5p0ProDraftState) -> Self {
    Self { resolution: draft.resolution, batch_count: draft.batch_count }
  }

  pub fn estimate_cost(&self) -> ImageGenerationCostEstimate {
    let pricing_request = GenerateSeedream5p0ProRequest { prompt: String::new(), aspect_ratio: KinoviSeedream5p0ProAspectRatio::Auto, resolution: self.resolution, batch_count: self.batch_count, reference_image_urls: None };

    let costs = pricing_request.calculate_costs();

    ImageGenerationCostEstimate { cost_in_credits: Some(costs.kinovi_credits), cost_in_usd_cents: Some(costs.usd_cents_rounded_up), is_free: false, is_unlimited: false, is_rate_limited: false, has_watermark: false, failures_are_refunded: None }
  }
}
...
mod tests {
  ...
  fn one_1k_image_is_fourteen_credits_and_six_cents() {
    let cost = KinoviSeedream5p0ProCostState { resolution: KinoviSeedream5p0ProResolution::OneK, batch_count: KinoviSeedream5p0ProBatchCount::One }.estimate_cost();
  ...
  fn one_2k_image_is_twentyfour_credits_and_ten_cents() {
    let cost = KinoviSeedream5p0ProCostState { resolution: KinoviSeedream5p0ProResolution::TwoK, batch_count: KinoviSeedream5p0ProBatchCount::One }.estimate_cost();
  ...
  fn four_2k_images_is_ninetysix_credits() {
    let cost = KinoviSeedream5p0ProCostState { resolution: KinoviSeedream5p0ProResolution::TwoK, batch_count: KinoviSeedream5p0ProBatchCount::Four }.estimate_cost();
  ...
  fn eight_1k_images_is_one_hundred_twelve_credits() {
    let cost = KinoviSeedream5p0ProCostState { resolution: KinoviSeedream5p0ProResolution::OneK, batch_count: KinoviSeedream5p0ProBatchCount::Eight }.estimate_cost();
