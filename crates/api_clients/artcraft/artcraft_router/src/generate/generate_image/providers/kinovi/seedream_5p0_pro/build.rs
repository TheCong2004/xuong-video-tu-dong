use seedance2pro_client::generate::image::generate_seedream_5p0_pro::{GenerateSeedream5p0ProRequest, KinoviSeedream5p0ProAspectRatio, KinoviSeedream5p0ProBatchCount, KinoviSeedream5p0ProResolution, MAX_REFERENCE_IMAGES};
...
use crate::generate::generate_image::providers::kinovi::seedream_5p0_pro::draft::{KinoviSeedream5p0ProDraftState, KinoviSeedream5p0ProRemainingItems};
use crate::generate::generate_image::providers::kinovi::seedream_5p0_pro::request::KinoviSeedream5p0ProRequestState;

pub fn build_kinovi_seedream_5p0_pro(mut builder: GenerateImageRequestBuilder) -> Result<ImageGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  if !has_image_inputs(image_inputs.as_ref()) {
    let request = GenerateSeedream5p0ProRequest { prompt, aspect_ratio, resolution, batch_count, reference_image_urls: None };
    return Ok(ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::KinoviSeedream5p0Pro(KinoviSeedream5p0ProRequestState { request })));
  }

  let draft = KinoviSeedream5p0ProDraftState { prompt, aspect_ratio, resolution, batch_count, unhandled_request_state: Some(KinoviSeedream5p0ProRemainingItems { reference_images: image_inputs }) };
  Ok(ImageGenerationDraftOrRequest::Draft(ImageGenerationDraftRequest::KinoviSeedream5p0Pro(draft)))
}
...
fn validate_reference_image_count(image_inputs: Option<&ImageListRef>) -> Result<(), ArtcraftRouterError> {
  ...
  if count > MAX_REFERENCE_IMAGES {
    return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_inputs", value: format!("{} reference images (max {})", count, MAX_REFERENCE_IMAGES) }));
  }
  Ok(())
}
...
pub(crate) fn plan_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<KinoviSeedream5p0ProAspectRatio, ArtcraftRouterError> {
  use KinoviSeedream5p0ProAspectRatio as Ar;
  match aspect_ratio {
    None | Some(RouterAspectRatio::Auto) | Some(RouterAspectRatio::Auto2k) | Some(RouterAspectRatio::Auto3k) | Some(RouterAspectRatio::Auto4k) => Ok(Ar::Auto),
...
pub(crate) fn plan_resolution(resolution: Option<RouterResolution>, strategy: RequestMismatchMitigationStrategy) -> Result<KinoviSeedream5p0ProResolution, ArtcraftRouterError> {
...
pub(crate) fn plan_batch_count(image_batch_count: Option<u16>, strategy: RequestMismatchMitigationStrategy) -> Result<KinoviSeedream5p0ProBatchCount, ArtcraftRouterError> {
  ...
  match count {
    ...
    other => match strategy {
      RequestMismatchMitigationStrategy::ErrorOut => Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_batch_count", value: format!("{}", other) })),
      ...
      RequestMismatchMitigationStrategy::PayMoreUpgrade | RequestMismatchMitigationStrategy::PayLessDowngrade => Ok(KinoviSeedream5p0ProBatchCount::Eight),
    },
  }
}
...
mod tests {
  ...
  mod routing_tests {
    ...
    fn no_image_inputs_returns_request_directly() {
      ...
      assert!(matches!(result, ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::KinoviSeedream5p0Pro(_)),));
    }

    #[test]
    fn image_inputs_returns_draft() {
      let builder = GenerateImageRequestBuilder { image_inputs: Some(ImageListRef::Urls(vec!["https://example.com/ref.png".to_string()])), ..base_builder() };
      let result = build_kinovi_seedream_5p0_pro(builder).expect("build");
      assert!(matches!(result, ImageGenerationDraftOrRequest::Draft(ImageGenerationDraftRequest::KinoviSeedream5p0Pro(_)),));
    }

    #[test]
    fn media_file_tokens_route_through_draft() {
      let builder = GenerateImageRequestBuilder { image_inputs: Some(ImageListRef::MediaFileTokens(vec![MediaFileToken::new("mf_x".to_string())])), ..base_builder() };
    ...
    fn fourteen_reference_images_is_accepted() {
      let urls: Vec<String> = (0..MAX_REFERENCE_IMAGES).map(|i| format!("https://example.com/ref{i}.png")).collect();
      let builder = GenerateImageRequestBuilder { image_inputs: Some(ImageListRef::Urls(urls)), ..base_builder() };
      assert!(build_kinovi_seedream_5p0_pro(builder).is_ok());
    }

    #[test]
    fn fifteen_reference_images_is_rejected() {
      let urls: Vec<String> = (0..MAX_REFERENCE_IMAGES + 1).map(|i| format!("https://example.com/ref{i}.png")).collect();
      let builder = GenerateImageRequestBuilder { image_inputs: Some(ImageListRef::Urls(urls)), ..base_builder() };
  ...
  mod plan_tests {
    ...
    fn every_supported_aspect_ratio_maps() {
      let cases = [(RouterAspectRatio::Auto, KinoviSeedream5p0ProAspectRatio::Auto), (RouterAspectRatio::Square, KinoviSeedream5p0ProAspectRatio::Square1x1), (RouterAspectRatio::WideFourByThree, KinoviSeedream5p0ProAspectRatio::Standard4x3), (RouterAspectRatio::TallThreeByFour, KinoviSeedream5p0ProAspectRatio::Portrait3x4), (RouterAspectRatio::WideSixteenByNine, KinoviSeedream5p0ProAspectRatio::Landscape16x9), (RouterAspectRatio::TallNineBySixteen, KinoviSeedream5p0ProAspectRatio::Portrait9x16), (RouterAspectRatio::WideThreeByTwo, KinoviSeedream5p0ProAspectRatio::Wide3x2), (RouterAspectRatio::TallTwoByThree, KinoviSeedream5p0ProAspectRatio::Tall2x3), (RouterAspectRatio::WideTwentyOneByNine, KinoviSeedream5p0ProAspectRatio::UltraWide21x9)];
      for (input, expected) in cases {
        let planned = plan_aspect_ratio(Some(input), RequestMismatchMitigationStrategy::ErrorOut).expect("plan");
    ...
    fn unsupported_aspect_ratio_errors_out() {
      let result = plan_aspect_ratio(Some(RouterAspectRatio::TallNineByTwentyOne), RequestMismatchMitigationStrategy::ErrorOut);
      assert!(result.is_err());
    }

    #[test]
    fn unsupported_aspect_ratio_falls_back_to_auto() {
      let planned = plan_aspect_ratio(Some(RouterAspectRatio::TallNineByTwentyOne), RequestMismatchMitigationStrategy::PayLessDowngrade).expect("plan");
    ...
    fn four_k_resolution_errors_out() {
      let result = plan_resolution(Some(RouterResolution::FourK), RequestMismatchMitigationStrategy::ErrorOut);
      assert!(result.is_err());
    }

    #[test]
    fn four_k_resolution_downgrades_to_2k() {
      let planned = plan_resolution(Some(RouterResolution::FourK), RequestMismatchMitigationStrategy::PayLessDowngrade).expect("plan");
      assert_eq!(planned, KinoviSeedream5p0ProResolution::TwoK);
    }

    #[test]
    fn half_k_resolution_snaps_to_1k() {
      let planned = plan_resolution(Some(RouterResolution::HalfK), RequestMismatchMitigationStrategy::PayMoreUpgrade).expect("plan");
      assert_eq!(planned, KinoviSeedream5p0ProResolution::OneK);
    }

    #[test]
    fn batch_counts_one_through_eight_are_accepted() {
      let cases = [(1, KinoviSeedream5p0ProBatchCount::One), (2, KinoviSeedream5p0ProBatchCount::Two), (3, KinoviSeedream5p0ProBatchCount::Three), (4, KinoviSeedream5p0ProBatchCount::Four), (5, KinoviSeedream5p0ProBatchCount::Five), (6, KinoviSeedream5p0ProBatchCount::Six), (7, KinoviSeedream5p0ProBatchCount::Seven), (8, KinoviSeedream5p0ProBatchCount::Eight)];
      for (input, expected) in cases {
        let planned = plan_batch_count(Some(input), RequestMismatchMitigationStrategy::ErrorOut).expect("plan");
        assert_eq!(planned, expected, "input={}", input);
      }
    }
    ...
    fn batch_count_nine_clamps_to_eight() {
      let planned = plan_batch_count(Some(9), RequestMismatchMitigationStrategy::PayLessDowngrade).expect("plan");
