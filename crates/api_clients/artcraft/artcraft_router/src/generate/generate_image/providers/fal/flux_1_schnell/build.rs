use fal_client::requests::api::image::edit::flux_1_schnell_edit_image::api::{Flux1SchnellEditImageNumImages, Flux1SchnellEditImageRequest, Flux1SchnellEditImageSize};
...
pub fn build_fal_flux_1_schnell(builder: GenerateImageRequestBuilder) -> Result<ImageGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  let state = if let Some(url) = image_url {
    ...
    FalFlux1SchnellRequestState::EditImage(Flux1SchnellEditImageRequest { image_url: url, num_images: to_edit_num_images(num_images), image_size })
  } else {
    // Text-to-image
    let aspect_ratio = plan_aspect_ratio(builder.aspect_ratio);

    FalFlux1SchnellRequestState::TextToImage(Flux1SchnellTextToImageRequest { prompt, num_images: to_t2i_num_images(num_images), aspect_ratio })
  };

  Ok(ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalFlux1Schnell(state)))
}
...
fn plan_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>) -> Flux1SchnellTextToImageAspectRatio {
  use Flux1SchnellTextToImageAspectRatio as T;
  match aspect_ratio {
    None | Some(RouterAspectRatio::Auto) | Some(RouterAspectRatio::Auto2k) | Some(RouterAspectRatio::Auto3k) | Some(RouterAspectRatio::Auto4k) => T::SquareHd,
    Some(RouterAspectRatio::Square) => T::Square,
    Some(RouterAspectRatio::SquareHd) => T::SquareHd,
    Some(RouterAspectRatio::WideFourByThree) | Some(RouterAspectRatio::WideFiveByFour) | Some(RouterAspectRatio::WideThreeByTwo) | Some(RouterAspectRatio::Wide) => T::LandscapeFourByThree,
    Some(RouterAspectRatio::WideSixteenByNine) | Some(RouterAspectRatio::WideTwentyOneByNine) => T::LandscapeSixteenByNine,
    Some(RouterAspectRatio::TallThreeByFour) | Some(RouterAspectRatio::TallFourByFive) | Some(RouterAspectRatio::TallTwoByThree) | Some(RouterAspectRatio::Tall) => T::PortraitThreeByFour,
    Some(RouterAspectRatio::TallNineBySixteen) | Some(RouterAspectRatio::TallNineByTwentyOne) => T::PortraitNineBySixteen,
  }
}

fn plan_edit_image_size(aspect_ratio: Option<RouterAspectRatio>) -> Option<Flux1SchnellEditImageSize> {
  ...
  aspect_ratio.and_then(|ar| match ar {
    ...
    RouterAspectRatio::WideFourByThree | RouterAspectRatio::WideFiveByFour | RouterAspectRatio::WideThreeByTwo | RouterAspectRatio::Wide => Some(S::LandscapeFourByThree),
    RouterAspectRatio::WideSixteenByNine | RouterAspectRatio::WideTwentyOneByNine => Some(S::LandscapeSixteenByNine),
    RouterAspectRatio::TallThreeByFour | RouterAspectRatio::TallFourByFive | RouterAspectRatio::TallTwoByThree | RouterAspectRatio::Tall => Some(S::PortraitThreeByFour),
...
fn resolve_single_image_url(image_inputs: Option<ImageListRef>) -> Result<Option<String>, ArtcraftRouterError> {
  match image_inputs {
    None => Ok(None),
    ...
    Some(ImageListRef::Urls(urls)) => Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_inputs", value: format!("Flux 1 Schnell redux supports exactly 1 image, got {}", urls.len()) })),
...
mod tests {
  ...
  fn unwrap_edit(result: Result<ImageGenerationDraftOrRequest, ArtcraftRouterError>) -> Flux1SchnellEditImageRequest {
    let ImageGenerationDraftOrRequest::Request(ImageGenerationRequest::FalFlux1Schnell(FalFlux1SchnellRequestState::EditImage(req))) = result.expect("build should succeed") else {
  ...
  mod mode_detection {
    ...
    fn multiple_urls_rejected_for_parity_with_v1() {
      ...
      let builder = GenerateImageRequestBuilder { image_inputs: Some(ImageListRef::Urls(vec!["https://example.com/a.jpg".to_string(), "https://example.com/b.jpg".to_string()])), ..base_builder() };
      assert!(matches!(build_fal_flux_1_schnell(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "image_inputs", .. }))));
    }
    ...
    fn media_tokens_return_error() {
      ...
      assert!(matches!(build_fal_flux_1_schnell(builder), Err(ArtcraftRouterError::Client(ClientError::FalOnlySupportsUrls))));
    }
  }
  ...
  mod num_images_tests {
    ...
    fn zero_is_error() {
      ...
      assert!(matches!(build_fal_flux_1_schnell(builder), Err(ArtcraftRouterError::Client(ClientError::UserRequestedZeroGenerations))));
    }

    #[test]
    fn over_four_error_out() {
      let builder = GenerateImageRequestBuilder { image_batch_count: Some(7), ..base_builder() };
      assert!(matches!(build_fal_flux_1_schnell(builder), Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { .. }))));
    }
  ...
  mod aspect_ratio_tests {
    ...
    #[test]
    fn edit_square() {
      let builder = GenerateImageRequestBuilder { aspect_ratio: Some(RouterAspectRatio::Square), image_inputs: Some(ImageListRef::Urls(vec!["https://example.com/img.jpg".to_string()])), ..base_builder() };
      let req = unwrap_edit(build_fal_flux_1_schnell(builder));
    ...
    #[test]
    fn edit_landscape() {
      let builder = GenerateImageRequestBuilder { aspect_ratio: Some(RouterAspectRatio::WideSixteenByNine), image_inputs: Some(ImageListRef::Urls(vec!["https://example.com/img.jpg".to_string()])), ..base_builder() };
      let req = unwrap_edit(build_fal_flux_1_schnell(builder));
    ...
    fn edit_no_aspect_ratio_yields_none() {
      let builder = GenerateImageRequestBuilder { aspect_ratio: None, image_inputs: Some(ImageListRef::Urls(vec!["https://example.com/img.jpg".to_string()])), ..base_builder() };
      let req = unwrap_edit(build_fal_flux_1_schnell(builder));
      assert!(req.image_size.is_none());
    }
  }
