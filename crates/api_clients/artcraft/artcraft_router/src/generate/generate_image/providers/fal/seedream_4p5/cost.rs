...
mod tests {
  ...
  fn t2i_cost(n: EnqueueBytedanceSeedreamV4p5TextToImageNumImages) -> ImageGenerationCostEstimate {
    FalSeedream4p5CostState::from_request(&FalSeedream4p5RequestState::TextToImage(EnqueueBytedanceSeedreamV4p5TextToImageRequest { prompt: "test".to_string(), num_images: Some(n), max_images: None, image_size: None })).estimate_cost()
  }

  fn edit_cost(n: EnqueueBytedanceSeedreamV4p5EditImageNumImages) -> ImageGenerationCostEstimate {
    FalSeedream4p5CostState::from_request(&FalSeedream4p5RequestState::EditImage(EnqueueBytedanceSeedreamV4p5EditImageRequest { prompt: "test".to_string(), image_urls: vec!["https://example.com/x.jpg".to_string()], num_images: Some(n), max_images: None, image_size: None })).estimate_cost()
  }
  ...
  fn edit_two_images_matches_t2i_two_images() {
    assert_eq!(edit_cost(EnqueueBytedanceSeedreamV4p5EditImageNumImages::Two).cost_in_usd_cents, t2i_cost(EnqueueBytedanceSeedreamV4p5TextToImageNumImages::Two).cost_in_usd_cents,);
  }
