...
const RESOLUTIONS: [Option<RouterResolution>; 4] = [None, Some(RouterResolution::FourEightyP), Some(RouterResolution::SevenTwentyP), Some(RouterResolution::TenEightyP)];
    ...
    for &res in &RESOLUTIONS {
      for &dur in &DURATIONS {
        for &batch in &BATCHES {
          ...
          assert_eq!(bpu, bp, "cost mismatch at res={:?} dur={} batch={}: bp={} bpu={}", res, dur, batch, bp, bpu,);
        }
      }
    }
  }
    ...
    for &res in &RESOLUTIONS {
      for &dur in &DURATIONS {
        for &batch in &BATCHES {
          ...
          assert_eq!(bpu.cost_in_credits, bp.cost_in_credits, "credit mismatch at res={:?} dur={} batch={}", res, dur, batch,);
        }
      }
    }
  }
    ...
    for &res in &RESOLUTIONS {
      for &dur in &DURATIONS {
        for &batch in &BATCHES {
          ...
          assert_eq!(bpu, bp, "cost mismatch at res={:?} dur={} batch={}: bp_fast={} bpu_fast={}", res, dur, batch, bp, bpu,);
        }
      }
    }
  }
...
fn build_cost(model: RouterVideoModel, resolution: Option<RouterResolution>, duration_seconds: u16, video_batch_count: u16) -> VideoGenerationCostEstimate {
  GenerateVideoRequestBuilder { model, provider: RouterProvider::Artcraft, resolution, duration_seconds: Some(duration_seconds), video_batch_count: Some(video_batch_count), ..Default::default() }.build2().expect("build2 should succeed").estimate_cost().expect("estimate_cost should succeed")
}

fn cost_cents(model: RouterVideoModel, resolution: Option<RouterResolution>, duration_seconds: u16, video_batch_count: u16) -> u64 {
  build_cost(model, resolution, duration_seconds, video_batch_count).cost_in_usd_cents.expect("cost_in_usd_cents should be Some")
}
