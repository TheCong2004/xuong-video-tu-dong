...
use fal_client::requests::api::video::image::vidu_q3_turbo::api::ViduQ3TurboImageToVideoRequest;
use fal_client::requests::api::video::text::vidu_q3_turbo::api::ViduQ3TurboTextToVideoRequest;
...
#[derive(Clone, Debug)]
pub enum FalViduQ3TurboMode {
  TextToVideo(ViduQ3TurboTextToVideoRequest),
  ImageToVideo(ViduQ3TurboImageToVideoRequest),
}

#[derive(Clone, Debug)]
pub struct FalViduQ3TurboRequestState {
  pub mode: FalViduQ3TurboMode,
}

impl FalViduQ3TurboRequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateVideoResponse, ArtcraftRouterError> {
    match &self.mode {
      FalViduQ3TurboMode::TextToVideo(request) => send_request(request, client).await,
      FalViduQ3TurboMode::ImageToVideo(request) => send_request(request, client).await,
    }
  }
}
...
mod tests {
  ...
  use fal_client::requests::api::video::image::vidu_q3_turbo::api::ViduQ3TurboImageToVideoResolution;
  use fal_client::requests::api::video::text::vidu_q3_turbo::api::{ViduQ3TurboTextToVideoAspectRatio, ViduQ3TurboTextToVideoResolution};
  ...
  async fn send_text_to_video_webhook() {
    let state = FalViduQ3TurboRequestState { mode: FalViduQ3TurboMode::TextToVideo(ViduQ3TurboTextToVideoRequest { prompt: "a calm lake at sunrise".to_string(), duration: Some(5), seed: None, aspect_ratio: Some(ViduQ3TurboTextToVideoAspectRatio::SixteenByNine), resolution: Some(ViduQ3TurboTextToVideoResolution::SevenTwentyP), audio: Some(false) }) };
  ...
  async fn send_image_to_video_webhook() {
    let state = FalViduQ3TurboRequestState { mode: FalViduQ3TurboMode::ImageToVideo(ViduQ3TurboImageToVideoRequest { prompt: "the lake comes alive".to_string(), image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), end_image_url: None, duration: Some(5), seed: None, resolution: Some(ViduQ3TurboImageToVideoResolution::SevenTwentyP), audio: Some(false) }) };
    let response = state.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
