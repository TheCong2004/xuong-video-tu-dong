...
use fal_client::requests::api::video::image::kling_2p5_turbo_pro_image_to_video::api::Kling2p5TurboProImageToVideoRequest;
use fal_client::requests::api::video::text::kling_2p5_turbo_pro_text_to_video::api::Kling2p5TurboProTextToVideoRequest;
...
pub enum FalKling2p5TurboProMode {
  TextToVideo(Kling2p5TurboProTextToVideoRequest),
  ImageToVideo(Kling2p5TurboProImageToVideoRequest),
}
...
impl FalKling2p5TurboProRequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateVideoResponse, ArtcraftRouterError> {
    match &self.mode {
      FalKling2p5TurboProMode::TextToVideo(request) => send_request(request, client).await,
      FalKling2p5TurboProMode::ImageToVideo(request) => send_request(request, client).await,
    }
  }
}
...
mod tests {
  ...
  use fal_client::requests::api::video::image::kling_2p5_turbo_pro_image_to_video::api::Kling2p5TurboProImageToVideoDuration;
  use fal_client::requests::api::video::text::kling_2p5_turbo_pro_text_to_video::api::{Kling2p5TurboProTextToVideoAspectRatio, Kling2p5TurboProTextToVideoDuration};
  ...
  async fn send_text_to_video_webhook() {
    let state = FalKling2p5TurboProRequestState { mode: FalKling2p5TurboProMode::TextToVideo(Kling2p5TurboProTextToVideoRequest { prompt: "a calm lake at sunrise".to_string(), negative_prompt: None, duration: Some(Kling2p5TurboProTextToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling2p5TurboProTextToVideoAspectRatio::SixteenByNine), cfg_scale: None }) };
  ...
  async fn send_image_to_video_webhook() {
    let state = FalKling2p5TurboProRequestState { mode: FalKling2p5TurboProMode::ImageToVideo(Kling2p5TurboProImageToVideoRequest { prompt: "the lake comes alive".to_string(), image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), tail_image_url: None, duration: Some(Kling2p5TurboProImageToVideoDuration::FiveSeconds), negative_prompt: None, cfg_scale: None }) };
    let response = state.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
