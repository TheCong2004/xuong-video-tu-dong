use seedance2pro_client::generate::video::generate_seedance_2p0_fast::{GenerateSeedance2p0FastRequest, KinoviSeedance2p0FastBatchCount, KinoviSeedance2p0FastOutputResolution};
...
mod tests {
  ...
  use seedance2pro_client::generate::video::generate_seedance_2p0_fast::{KinoviSeedance2p0FastOutputResolution as KinoviOutputResolution, KinoviSeedance2p0FastBatchCount as KinoviBatchCount};
  ...
  fn video_reference_adds_surcharge() {
    let base = KinoviSeedance2p0FastCostState { resolution: Some(KinoviOutputResolution::SevenTwentyP), duration_seconds: 5, batch_count: Some(KinoviBatchCount::One), has_video_reference: false };
  ...
  fn usd_cents(resolution: KinoviOutputResolution, duration_seconds: u8, batch_count: KinoviBatchCount) -> u64 {
    KinoviSeedance2p0FastCostState { resolution: Some(resolution), duration_seconds, batch_count: Some(batch_count), has_video_reference: false }.estimate_cost().cost_in_usd_cents.unwrap()
  }

  fn credits(resolution: KinoviOutputResolution, duration_seconds: u8, batch_count: KinoviBatchCount) -> u64 {
    KinoviSeedance2p0FastCostState { resolution: Some(resolution), duration_seconds, batch_count: Some(batch_count), has_video_reference: false }.estimate_cost().cost_in_credits.unwrap()
  }

  /// Build a draft via the builder to test from_draft().
  fn make_draft(duration_seconds: u16, video_batch_count: u16, resolution: Option<RouterResolution>, with_video_ref: bool) -> KinoviSeedance2p0FastDraftState {
    let reference_videos = if with_video_ref { Some(VideoListRef::Urls(vec!["https://example.com/video.mp4".to_string()])) } else { None };

    let builder = GenerateVideoRequestBuilder { model: RouterVideoModel::Seedance2p0Fast, provider: RouterProvider::Seedance2Pro, resolution, reference_videos, duration_seconds: Some(duration_seconds), video_batch_count: Some(video_batch_count), ..Default::default() };
  ...
  fn make_request_state(resolution: Option<KinoviOutputResolution>, duration_seconds: u8, batch_count: KinoviBatchCount, with_video_ref: bool) -> KinoviSeedance2p0FastRequestState {
    let reference_video_urls = if with_video_ref { Some(vec!["https://cdn.seedance2-pro.com/video.mp4".to_string()]) } else { None };

    KinoviSeedance2p0FastRequestState { request: GenerateSeedance2p0FastRequest { prompt: "test".to_string(), aspect_ratio: None, output_resolution: resolution, duration_seconds, batch_count: Some(batch_count), start_frame_url: None, end_frame_url: None, reference_image_urls: None, reference_video_urls, reference_audio_urls: None, character_ids: None, use_face_blur_hack: None, bitrate: None } }
  }
}
