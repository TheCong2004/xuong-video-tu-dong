...
use grok_api_client::api::requests::videos::video_generation::video_generation::{video_generation, VideoGenerationArgs, VideoGenerationRequest as GrokVideoGenerationRequest};

use crate::client::router_grok_api_client::RouterGrokApiClient;
use crate::errors::artcraft_router_error::ArtcraftRouterError;
use crate::errors::client_error::ClientError;
use crate::errors::provider_error::ProviderError;
use crate::generate::generate_video::generate_video_response::{GenerateVideoResponse, GrokVideoResponsePayload};
...
impl GrokApiGrokImagineVideo1p5RequestState {
  pub async fn send(&self, client: &RouterGrokApiClient) -> Result<GenerateVideoResponse, ArtcraftRouterError> {
    ...
    let response = video_generation(VideoGenerationArgs { api_key: &client.api_key, request: self.request.clone() }).await.map_err(|err| ArtcraftRouterError::Provider(ProviderError::Grok(err)))?;

    Ok(GenerateVideoResponse::Grok(GrokVideoResponsePayload { request_id: response.request_id, maybe_outbound_request: Some(outbound_request) }))
  }
}
...
mod tests {
  ...
  async fn test_text_to_video_720p() {
    let response = run_pipeline(GenerateVideoRequestBuilder { prompt: Some("A glowing crystal rocket launching from Mars.".to_string()), aspect_ratio: Some(RouterAspectRatio::WideSixteenByNine), resolution: Some(RouterResolution::SevenTwentyP), duration_seconds: Some(5), ..grok_builder() }).await;
  ...
  async fn test_image_to_video_480p() {
    let response = run_pipeline(GenerateVideoRequestBuilder { prompt: Some("The dog leaps into the lake and splashes around.".to_string()), start_frame: Some(ImageRef::Url(JUNO_AT_LAKE_IMAGE_URL.to_string())), resolution: Some(RouterResolution::FourEightyP), duration_seconds: Some(5), ..grok_builder() }).await;
    assert!(matches!(response, GenerateVideoResponse::Grok(_)));
  }
  ...
  async fn test_image_to_video_720p() {
    let response = run_pipeline(GenerateVideoRequestBuilder { prompt: Some("The dog leaps into the lake and splashes around.".to_string()), start_frame: Some(ImageRef::Url(JUNO_AT_LAKE_IMAGE_URL.to_string())), resolution: Some(RouterResolution::SevenTwentyP), duration_seconds: Some(5), ..grok_builder() }).await;
    assert!(matches!(response, GenerateVideoResponse::Grok(_)));
  }

  fn grok_builder() -> GenerateVideoRequestBuilder {
    GenerateVideoRequestBuilder { model: RouterVideoModel::GrokImagineVideo1p5, provider: RouterProvider::GrokApi, video_batch_count: Some(1), ..Default::default() }
  }

  fn get_grok_client() -> RouterClient {
    let secret = std::fs::read_to_string("/Users/bt/Artcraft/credentials/grok_api_key.txt").expect("Failed to read Grok API key");
  ...
  fn unwrap_request(builder: GenerateVideoRequestBuilder) -> GrokApiGrokImagineVideo1p5RequestState {
    ...
    match result {
      VideoGenerationDraftOrRequest::Request(crate::generate::generate_video::video_generation_request::VideoGenerationRequest::GrokApiGrokImagineVideo1p5(s)) => s,
