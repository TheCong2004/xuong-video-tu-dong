use fal_client::requests_old::webhook::video::image::enqueue_kling_3p0_pro_image_to_video_webhook::{EnqueueKling3p0ProImageToVideoAspectRatio, EnqueueKling3p0ProImageToVideoDuration, EnqueueKling3p0ProImageToVideoRequest};
use fal_client::requests_old::webhook::video::text::enqueue_kling_3p0_pro_text_to_video_webhook::{EnqueueKling3p0ProTextToVideoAspectRatio, EnqueueKling3p0ProTextToVideoDuration, EnqueueKling3p0ProTextToVideoRequest};
...
pub fn build_fal_kling_3p0_pro(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
...
fn build_kling_3p0_pro_state(builder: GenerateVideoRequestBuilder) -> Result<FalKling3p0ProRequestState, ArtcraftRouterError> {
  ...
  let mode = match optional_url(builder.start_frame.clone())? {
    None => {
      if builder.end_frame.is_some() {
        return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "end_frame", value: "Kling 3.0 Pro requires a start_frame when end_frame is provided".to_string() }));
      }
      FalKling3p0ProMode::TextToVideo(EnqueueKling3p0ProTextToVideoRequest { prompt, generate_audio, negative_prompt, duration: duration.map(to_t2v_duration), aspect_ratio: aspect_ratio.map(to_t2v_aspect_ratio), shot_type: None })
    },
    Some(image_url) => FalKling3p0ProMode::ImageToVideo(EnqueueKling3p0ProImageToVideoRequest { prompt, image_url, end_image_url: optional_url(builder.end_frame.clone())?, generate_audio, negative_prompt, duration: duration.map(to_i2v_duration), aspect_ratio: aspect_ratio.map(to_i2v_aspect_ratio), shot_type: None }),
  };

  Ok(FalKling3p0ProRequestState { mode })
}

pub(crate) fn plan_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<PlanAspectRatio>, ArtcraftRouterError> {
...
pub(crate) fn plan_duration(duration_seconds: Option<u16>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<PlanDuration>, ArtcraftRouterError> {
...
mod tests {
  ...
  fn base_builder() -> GenerateVideoRequestBuilder {
    GenerateVideoRequestBuilder { model: RouterVideoModel::Kling3p0Pro, provider: RouterProvider::Fal, prompt: Some("test".to_string()), ..Default::default() }
  }
