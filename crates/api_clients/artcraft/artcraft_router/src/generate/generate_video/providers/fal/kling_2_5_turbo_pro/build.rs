use fal_client::requests::api::video::image::kling_2p5_turbo_pro_image_to_video::api::{Kling2p5TurboProImageToVideoDuration, Kling2p5TurboProImageToVideoRequest};
use fal_client::requests::api::video::text::kling_2p5_turbo_pro_text_to_video::api::{Kling2p5TurboProTextToVideoAspectRatio, Kling2p5TurboProTextToVideoDuration, Kling2p5TurboProTextToVideoRequest};
...
pub fn build_fal_kling_2_5_turbo_pro(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  let mode = match optional_url(builder.start_frame.clone())? {
    None => {
      if builder.end_frame.is_some() {
        return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "end_frame", value: "Kling 2.5 Turbo Pro requires a start_frame when end_frame is provided".to_string() }));
      }
      FalKling2p5TurboProMode::TextToVideo(Kling2p5TurboProTextToVideoRequest { prompt, negative_prompt, duration: duration.map(to_t2v_duration), aspect_ratio: aspect_ratio.map(to_t2v_aspect_ratio), cfg_scale: None })
    },
    Some(image_url) => FalKling2p5TurboProMode::ImageToVideo(Kling2p5TurboProImageToVideoRequest { prompt, image_url, tail_image_url: optional_url(builder.end_frame.clone())?, duration: duration.map(to_i2v_duration), negative_prompt, cfg_scale: None }),
  };

  Ok(VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::FalKling2p5TurboPro(FalKling2p5TurboProRequestState { mode })))
}
...
mod tests {
  ...
  fn base_builder() -> GenerateVideoRequestBuilder {
    GenerateVideoRequestBuilder { model: RouterVideoModel::Kling2p5TurboPro, provider: RouterProvider::Fal, prompt: Some("test".to_string()), ..Default::default() }
  }
