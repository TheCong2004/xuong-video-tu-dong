...
use crate::generate::generate_video::providers::fal::sora_2::request::{FalSora2AspectRatio, FalSora2Duration, FalSora2Mode, FalSora2RequestState, FalSora2Resolution};
use crate::generate::generate_video::video_generation_draft_or_request::VideoGenerationDraftOrRequest;
use crate::generate::generate_video::video_generation_request::VideoGenerationRequest;

pub fn build_fal_sora_2(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  let strategy = builder.request_mismatch_mitigation_strategy;

  if builder.end_frame.is_some() {
    return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "end_frame", value: "Sora 2 does not support an ending frame".to_string() }));
  }
  ...
  Ok(VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::FalSora2(FalSora2RequestState { prompt: builder.prompt.clone().unwrap_or_default(), mode, aspect_ratio, resolution, duration })))
}

fn plan_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<FalSora2AspectRatio>, ArtcraftRouterError> {
...
fn plan_resolution(resolution: Option<RouterResolution>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<FalSora2Resolution>, ArtcraftRouterError> {
...
fn plan_duration(duration_seconds: Option<u16>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<FalSora2Duration>, ArtcraftRouterError> {
...
mod tests {
  ...
  fn base_builder() -> GenerateVideoRequestBuilder {
    GenerateVideoRequestBuilder { model: RouterVideoModel::Sora2, provider: RouterProvider::Fal, prompt: Some("test".to_string()), ..Default::default() }
  }
