...
mod tests {
  ...
  use fal_client::requests::api::video::text::veo_2::api::{Veo2TextToVideoAspectRatio, Veo2TextToVideoDuration};
  use test_data::web::image_urls::JUNO_AT_LAKE_IMAGE_URL;

  #[tokio::test]
  #[ignore] // requires real API key, incurs cost
  async fn live_text_to_video_5s() {
    let state = FalVeo2RequestState { mode: FalVeo2Mode::TextToVideo(Veo2TextToVideoRequest { prompt: "a peaceful lake at dawn".to_string(), aspect_ratio: Some(Veo2TextToVideoAspectRatio::SixteenByNine), duration: Some(Veo2TextToVideoDuration::FiveSeconds), negative_prompt: None, enhance_prompt: None, seed: None, auto_fix: None }) };
  ...
  async fn live_image_to_video_7s() {
    let state = FalVeo2RequestState { mode: FalVeo2Mode::ImageToVideo(Veo2ImageToVideoRequest { prompt: "the dog leaps into the lake and splashes around.".to_string(), image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), duration: Some(Veo2ImageToVideoDuration::SevenSeconds) }) };
    let response = state.send(&client_with_webhook()).await.expect("send should succeed");
    assert!(matches!(response, GenerateVideoResponse::Fal(_)));
  }
