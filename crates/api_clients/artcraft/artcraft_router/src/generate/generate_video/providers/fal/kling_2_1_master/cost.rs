...
impl FalKling21MasterCostState {
  pub fn from_request(request: &FalKling21MasterRequestState) -> Self {
    Self { is_ten_seconds: matches!(request.request.duration, Kling2p1MasterDuration::TenSeconds) }
  }
...
mod tests {
  ...
  fn cost_cents(duration_seconds: Option<u16>) -> u64 {
    let b = GenerateVideoRequestBuilder { model: RouterVideoModel::Kling21Master, provider: RouterProvider::Fal, prompt: Some("test".to_string()), start_frame: Some(ImageRef::Url("https://example.com/a.png".to_string())), duration_seconds, ..Default::default() };
