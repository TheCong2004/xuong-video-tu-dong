...
pub fn build_artcraft_grok_imagine_video(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  let request = build_artcraft_omni_video_request(builder, CommonVideoModelEnum::GrokImagineVideo, SupportedResolutions::Fast, UltraWideSupport::Unsupported)?;
  let state = ArtcraftGrokImagineVideoRequestState { request };
  Ok(VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::ArtcraftGrokImagineVideo(state)))
}
...
mod tests {
  ...
  mod media_token_tests {
    ...
    fn url_start_frame_rejected() {
      ...
      let result = build_artcraft_grok_imagine_video(GenerateVideoRequestBuilder { start_frame: Some(ImageRef::Url("https://example.com".to_string())), ..base_builder() });
  ...
  fn unwrap_request(builder: GenerateVideoRequestBuilder) -> ArtcraftGrokImagineVideoRequestState {
    ...
    match result {
      VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::ArtcraftGrokImagineVideo(state)) => state,
