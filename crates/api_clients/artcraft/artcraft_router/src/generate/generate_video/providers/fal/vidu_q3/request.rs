...
use fal_client::requests::api::video::image::vidu_q3::api::ViduQ3ImageToVideoRequest;
use fal_client::requests::api::video::reference::vidu_q3_reference_to_video::api::ViduQ3ReferenceToVideoRequest;
use fal_client::requests::api::video::reference::vidu_q3_reference_to_video_mix::api::ViduQ3ReferenceToVideoMixRequest;
use fal_client::requests::api::video::text::vidu_q3::api::ViduQ3TextToVideoRequest;
...
#[derive(Clone, Debug)]
pub enum FalViduQ3Mode {
  TextToVideo(ViduQ3TextToVideoRequest),
  ImageToVideo(ViduQ3ImageToVideoRequest),
  ReferenceToVideo(ViduQ3ReferenceToVideoRequest),
  ReferenceToVideoMix(ViduQ3ReferenceToVideoMixRequest),
}

#[derive(Clone, Debug)]
pub struct FalViduQ3RequestState {
  pub mode: FalViduQ3Mode,
}

impl FalViduQ3RequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateVideoResponse, ArtcraftRouterError> {
    match &self.mode {
      FalViduQ3Mode::TextToVideo(request) => send_request(request, client).await,
      FalViduQ3Mode::ImageToVideo(request) => send_request(request, client).await,
      FalViduQ3Mode::ReferenceToVideo(request) => send_request(request, client).await,
      FalViduQ3Mode::ReferenceToVideoMix(request) => send_request(request, client).await,
    }
  }
}
...
mod tests {
  ...
  use fal_client::requests::api::video::image::vidu_q3::api::ViduQ3ImageToVideoResolution;
  use fal_client::requests::api::video::text::vidu_q3::api::{ViduQ3TextToVideoAspectRatio, ViduQ3TextToVideoResolution};
  ...
  async fn send_text_to_video_webhook() {
    let state = FalViduQ3RequestState { mode: FalViduQ3Mode::TextToVideo(ViduQ3TextToVideoRequest { prompt: "a calm lake at sunrise".to_string(), duration: Some(5), seed: None, aspect_ratio: Some(ViduQ3TextToVideoAspectRatio::SixteenByNine), resolution: Some(ViduQ3TextToVideoResolution::SevenTwentyP), audio: Some(false) }) };
  ...
  async fn send_image_to_video_webhook() {
    let state = FalViduQ3RequestState { mode: FalViduQ3Mode::ImageToVideo(ViduQ3ImageToVideoRequest { prompt: "the lake comes alive".to_string(), image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), end_image_url: None, duration: Some(5), seed: None, resolution: Some(ViduQ3ImageToVideoResolution::SevenTwentyP), audio: Some(false) }) };
    let response = state.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
