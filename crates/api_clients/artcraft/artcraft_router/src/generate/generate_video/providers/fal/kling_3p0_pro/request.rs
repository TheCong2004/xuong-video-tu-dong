...
use fal_client::requests_old::webhook::video::image::enqueue_kling_3p0_pro_image_to_video_webhook::{enqueue_kling_3p0_pro_image_to_video_webhook, EnqueueKling3p0ProImageToVideoArgs, EnqueueKling3p0ProImageToVideoRequest};
use fal_client::requests_old::webhook::video::text::enqueue_kling_3p0_pro_text_to_video_webhook::{enqueue_kling_3p0_pro_text_to_video_webhook, EnqueueKling3p0ProTextToVideoArgs, EnqueueKling3p0ProTextToVideoRequest};
  ...
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateVideoResponse, ArtcraftRouterError> {
    ...
    let (webhook_response, outbound_request): (_, Arc<dyn Debug + Send + Sync>) = match &self.mode {
      FalKling3p0ProMode::TextToVideo(request) => {
        ...
        let args = EnqueueKling3p0ProTextToVideoArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
        (enqueue_kling_3p0_pro_text_to_video_webhook(args).await, outbound)
      },
      FalKling3p0ProMode::ImageToVideo(request) => {
        let outbound: Arc<dyn Debug + Send + Sync> = Arc::new(request.clone());
        let args = EnqueueKling3p0ProImageToVideoArgs { request: request.clone(), webhook_url, api_key: &client.api_key };
