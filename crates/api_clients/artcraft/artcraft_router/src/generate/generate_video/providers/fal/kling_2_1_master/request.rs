...
use fal_client::requests_old::webhook::video::image::enqueue_kling_v2p1_master_image_to_video_webhook::{enqueue_kling_v2p1_master_image_to_video_webhook, Kling2p1MasterArgs, Kling2p1MasterRequest};
...
impl FalKling21MasterRequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateVideoResponse, ArtcraftRouterError> {
    ...
    let args = Kling2p1MasterArgs { request: self.request.clone(), webhook_url, api_key: &client.api_key };

    let webhook_response = enqueue_kling_v2p1_master_image_to_video_webhook(args).await.map_err(|e| ArtcraftRouterError::Provider(ProviderError::Fal(e)))?;
