...
pub fn build_artcraft_seedance_2p0_bp_fast(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  let request = build_artcraft_omni_video_request(builder, CommonVideoModelEnum::Seedance2p0BytePlusFast, SupportedResolutions::Fast, UltraWideSupport::Supported)?;
...
mod tests {
  ...
  mod resolution_tests {
    ...
    fn res_1080p_error_out() {
      let result = build_artcraft_seedance_2p0_bp_fast(GenerateVideoRequestBuilder { model: RouterVideoModel::Seedance2p0BytePlusFast, provider: RouterProvider::Artcraft, resolution: Some(RouterResolution::TenEightyP), request_mismatch_mitigation_strategy: RequestMismatchMitigationStrategy::ErrorOut, ..Default::default() });
  ...
  mod media_token_tests {
    ...
    fn url_start_frame_rejected() {
      let result = build_artcraft_seedance_2p0_bp_fast(GenerateVideoRequestBuilder { model: RouterVideoModel::Seedance2p0BytePlusFast, provider: RouterProvider::Artcraft, start_frame: Some(ImageRef::Url("https://example.com".to_string())), ..Default::default() });
      assert!(result.is_err());
    }
  ...
  fn make_builder(f: impl FnOnce(&mut GenerateVideoRequestBuilder)) -> GenerateVideoRequestBuilder {
    let mut builder = GenerateVideoRequestBuilder { model: RouterVideoModel::Seedance2p0BytePlusFast, provider: RouterProvider::Artcraft, duration_seconds: Some(5), video_batch_count: Some(1), ..Default::default() };
    f(&mut builder);
    builder
  }
  ...
  fn unwrap_request(builder: GenerateVideoRequestBuilder) -> ArtcraftSeedance2p0BytePlusFastRequestState {
    ...
    match result {
      VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::ArtcraftSeedance2p0BytePlusFast(state)) => state,
