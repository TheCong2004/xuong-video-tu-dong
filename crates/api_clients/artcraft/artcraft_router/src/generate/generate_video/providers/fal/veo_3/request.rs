...
use fal_client::requests::api::video::image::veo_3::api::Veo3ImageToVideoRequest;
use fal_client::requests::api::video::text::veo_3::api::Veo3TextToVideoRequest;
...
pub enum FalVeo3Mode {
  ...
  ImageToVideo(Veo3ImageToVideoRequest),
}
...
impl FalVeo3RequestState {
  pub async fn send(&self, client: &RouterFalClient) -> Result<GenerateVideoResponse, ArtcraftRouterError> {
    match &self.mode {
      FalVeo3Mode::TextToVideo(request) => send_request(request, client).await,
      FalVeo3Mode::ImageToVideo(request) => send_request(request, client).await,
    }
  }
}
...
mod tests {
  ...
  use fal_client::requests::api::video::image::veo_3::api::{Veo3ImageToVideoDuration, Veo3ImageToVideoResolution};
  use fal_client::requests::api::video::text::veo_3::api::{Veo3TextToVideoDuration, Veo3TextToVideoResolution};
  use test_data::web::image_urls::JUNO_AT_LAKE_IMAGE_URL;

  #[tokio::test]
  #[ignore] // requires real API key, incurs cost
  async fn live_text_to_video_720p_4s_audio_on() {
    let state = FalVeo3RequestState { mode: FalVeo3Mode::TextToVideo(Veo3TextToVideoRequest { prompt: "a serene mountain landscape".to_string(), aspect_ratio: None, duration: Some(Veo3TextToVideoDuration::FourSeconds), resolution: Some(Veo3TextToVideoResolution::SevenTwentyP), negative_prompt: None, generate_audio: Some(true), seed: None, auto_fix: None, safety_tolerance: None }) };
  ...
  async fn live_image_to_video_1080p_8s_no_audio() {
    let state = FalVeo3RequestState { mode: FalVeo3Mode::ImageToVideo(Veo3ImageToVideoRequest { prompt: "the dog leaps into the lake.".to_string(), image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), aspect_ratio: None, duration: Some(Veo3ImageToVideoDuration::EightSeconds), resolution: Some(Veo3ImageToVideoResolution::TenEightyP), generate_audio: Some(false), negative_prompt: None, seed: None, auto_fix: None, safety_tolerance: None }) };
    let response = state.send(&client_with_webhook()).await.expect("send should succeed");
    assert!(matches!(response, GenerateVideoResponse::Fal(_)));
  }
