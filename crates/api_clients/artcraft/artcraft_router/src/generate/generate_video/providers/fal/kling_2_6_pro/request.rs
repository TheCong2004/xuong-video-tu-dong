...
mod tests {
  ...
  use fal_client::requests::api::video::text::kling_2p6_pro_text_to_video::api::{Kling2p6ProTextToVideoAspectRatio, Kling2p6ProTextToVideoDuration};
  ...
  async fn send_image_to_video_webhook() {
    let state = FalKling2p6ProRequestState { mode: FalKling2p6ProMode::ImageToVideo(Kling2p6ProImageToVideoRequest { prompt: "the lake comes alive".to_string(), start_image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), end_image_url: None, duration: Some(Kling2p6ProImageToVideoDuration::FiveSeconds), negative_prompt: None, generate_audio: Some(false), voice_ids: None }) };
    let response = state.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
