...
pub fn build_artcraft_seedance_1p5_pro(mut builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  let mut request = build_artcraft_omni_video_request(builder, CommonVideoModelEnum::Seedance1p5Pro, SupportedResolutions::Full, UltraWideSupport::Supported)?;
...
fn plan_seedance_1p5_pro_duration(duration_seconds: Option<u16>, strategy: RequestMismatchMitigationStrategy) -> Result<Option<u16>, ArtcraftRouterError> {
  const MIN: u16 = 4;
  const MAX: u16 = 12;
