...
impl ArtcraftGrokImagineVideo1p5CostState {
  ...
  pub fn base_cost_in_cents_for_batch(&self) -> u64 {
    self.base_cost_in_cents_for_one_video().saturating_mul(self.batch_count as u64)
  }
  ...
  pub fn estimate_cost(&self) -> Result<VideoGenerationCostEstimate, ArtcraftRouterError> {
    ...
    Ok(VideoGenerationCostEstimate { cost_in_credits: Some(usd_cents), cost_in_usd_cents: Some(usd_cents), is_free: false, is_unlimited: false, is_rate_limited: false, has_watermark: false, failures_are_refunded: None })
  }
  ...
  fn build_equivalent_grok_request(&self) -> GrokVideoGenerationRequest {
    ...
    let image = (self.input_image_count >= 1).then(|| GrokVideoImageSource::Url(String::new()));
    let reference_images = if self.input_image_count >= 2 { Some((1..self.input_image_count).map(|_| GrokVideoImageSource::Url(String::new())).collect()) } else { None };

    GrokVideoGenerationRequest { prompt: String::new(), model: Some(GrokVideoModel::GrokImagineVideo1p5), image, reference_images, aspect_ratio: None, duration: Some(self.duration_seconds as u32), resolution, user: None }
  }
}
...
mod tests {
  ...
  mod markup {
    ...
    fn estimate_cost_equals_ceil_of_base_times_1p05() {
      for &(res, dur, imgs, batch) in MARKUP_CASES {
        ...
        assert_eq!(got, want, "res={res:?} dur={dur} imgs={imgs} batch={batch} base={base}",);
      }
    }
    ...
    fn final_cost_is_always_at_least_base() {
      for &(res, dur, imgs, batch) in MARKUP_CASES {
        ...
        assert!(final_ >= base, "final {final_}¢ < base {base}¢ at res={res:?} dur={dur} imgs={imgs} batch={batch}",);
      }
    }
    ...
    fn estimate_cost_quotes_text_only_request() {
      let state = ArtcraftGrokImagineVideo1p5CostState { resolution: CommonResolution::SevenTwentyP, duration_seconds: 5, batch_count: 1, input_image_count: 0 };
      let base = state.base_cost_in_cents_for_batch();
      let want = base.saturating_mul(MARKUP_NUMERATOR).div_ceil(MARKUP_DENOMINATOR);
      let got = state.estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      assert_eq!(got, want);
    ...
    const MARKUP_CASES: &[(Option<RouterResolution>, u16, u64, u16)] = &[(Some(RouterResolution::FourEightyP), 5, 1, 1), (Some(RouterResolution::FourEightyP), 5, 1, 2), (Some(RouterResolution::FourEightyP), 5, 1, 4), (Some(RouterResolution::FourEightyP), 8, 1, 1), (Some(RouterResolution::FourEightyP), 15, 1, 1), (Some(RouterResolution::FourEightyP), 5, 3, 1), (Some(RouterResolution::SevenTwentyP), 5, 1, 1), (Some(RouterResolution::SevenTwentyP), 5, 1, 2), (Some(RouterResolution::SevenTwentyP), 5, 1, 4), (Some(RouterResolution::SevenTwentyP), 8, 1, 1), (Some(RouterResolution::SevenTwentyP), 15, 1, 1), (Some(RouterResolution::SevenTwentyP), 5, 3, 1), (None, 5, 1, 1)];
  }
  ...
  mod monotonicity {
    ...
    fn higher_resolution_costs_more() {
      let p480 = artcraft_state(Some(RouterResolution::FourEightyP), 5, 1, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      let p720 = artcraft_state(Some(RouterResolution::SevenTwentyP), 5, 1, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      assert!(p480 < p720, "480p ({p480}¢) should be < 720p ({p720}¢)");
    }

    #[test]
    fn longer_duration_costs_more() {
      let c5 = artcraft_state(Some(RouterResolution::SevenTwentyP), 5, 1, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      let c10 = artcraft_state(Some(RouterResolution::SevenTwentyP), 10, 1, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      let c15 = artcraft_state(Some(RouterResolution::SevenTwentyP), 15, 1, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      assert!(c5 < c10);
      assert!(c10 < c15);
    }
    ...
    fn more_images_cost_more_or_equal() {
      ...
      let one = artcraft_state(Some(RouterResolution::SevenTwentyP), 5, 1, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      let three = artcraft_state(Some(RouterResolution::SevenTwentyP), 5, 3, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      assert!(one < three, "one={one}¢ vs three={three}¢");
    }

    #[test]
    fn more_batch_costs_more() {
      let b1 = artcraft_state(Some(RouterResolution::SevenTwentyP), 5, 1, 1).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      let b4 = artcraft_state(Some(RouterResolution::SevenTwentyP), 5, 1, 4).estimate_cost().expect("estimate_cost").cost_in_usd_cents.unwrap();
      assert!(b1 < b4);
    }
  }
  ...
  mod end_to_end {
      ...
      for res in resolutions {
        for dur in [4u16, 5, 10, 15] {
          for batch in [1u16, 2, 4] {
            ...
            assert_eq!(cost.cost_in_credits, cost.cost_in_usd_cents, "credits should equal cents at res={res:?} dur={dur}s batch={batch}",);
          }
        }
      }
    }
    ...
    fn ten_eighty_p_prices_higher_than_720p() {
      ...
      let p1080 = build_cost_via_builder(Some(RouterResolution::TenEightyP), 10, 1).cost_in_usd_cents.unwrap();
      let p720 = build_cost_via_builder(Some(RouterResolution::SevenTwentyP), 10, 1).cost_in_usd_cents.unwrap();
    ...
    fn image_less_request_gets_a_quote_through_the_builder() {
      let builder = GenerateVideoRequestBuilder { model: RouterVideoModel::GrokImagineVideo1p5, provider: RouterProvider::Artcraft, duration_seconds: Some(5), video_batch_count: Some(1), ..Default::default() };
      let cost = builder.build2().expect("build2 should succeed without an image").estimate_cost().expect("estimate_cost should succeed without an image");
  ...
  fn artcraft_state(resolution: Option<RouterResolution>, duration_seconds: u16, input_image_count: u64, batch_count: u16) -> ArtcraftGrokImagineVideo1p5CostState {
  ...
  fn unwrap_request_via_builder(resolution: Option<RouterResolution>, duration_seconds: u16, input_image_count: u64, batch_count: u16) -> ArtcraftGrokImagineVideo1p5RequestState {
    ...
    let start_frame = if input_image_count >= 1 { Some(ImageRef::MediaFileToken(MediaFileToken::new("mf_start".to_string()))) } else { None };
    let reference_images = if input_image_count >= 2 { Some(ImageListRef::MediaFileTokens((1..input_image_count).map(|i| MediaFileToken::new(format!("mf_ref_{i}"))).collect())) } else { None };

    let builder = GenerateVideoRequestBuilder { model: RouterVideoModel::GrokImagineVideo1p5, provider: RouterProvider::Artcraft, resolution, duration_seconds: Some(duration_seconds), video_batch_count: Some(batch_count), start_frame, reference_images, ..Default::default() };
    let result = builder.build2().expect("build2 should succeed");
    match result {
      crate::generate::generate_video::video_generation_draft_or_request::VideoGenerationDraftOrRequest::Request(crate::generate::generate_video::video_generation_request::VideoGenerationRequest::ArtcraftGrokImagineVideo1p5(s)) => s,
      _ => panic!("expected ArtcraftGrokImagineVideo1p5 request"),
    }
  }

  fn build_cost_via_builder(resolution: Option<RouterResolution>, duration_seconds: u16, video_batch_count: u16) -> VideoGenerationCostEstimate {
