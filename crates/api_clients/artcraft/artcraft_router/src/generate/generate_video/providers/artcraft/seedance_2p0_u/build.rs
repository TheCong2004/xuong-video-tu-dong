...
pub fn build_artcraft_seedance_2p0_u(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  let request = build_artcraft_omni_video_request(builder, CommonVideoModelEnum::Seedance2p0Ultra, SupportedResolutions::FullWith4k, UltraWideSupport::Supported)?;
...
mod tests {
  ...
  mod media_token_tests {
    ...
    fn url_start_frame_rejected() {
      let result = build_artcraft_seedance_2p0_u(GenerateVideoRequestBuilder { start_frame: Some(ImageRef::Url("https://example.com".to_string())), ..base_builder() });
  ...
  fn unwrap_request(builder: GenerateVideoRequestBuilder) -> ArtcraftSeedance2p0UltraRequestState {
    ...
    match result {
      VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::ArtcraftSeedance2p0Ultra(state)) => state,
