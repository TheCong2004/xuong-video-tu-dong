...
mod tests {
  ...
  use fal_client::requests::api::video::image::veo_3_fast::api::{Veo3FastImageToVideoDuration, Veo3FastImageToVideoResolution};
  use test_data::web::image_urls::JUNO_AT_LAKE_IMAGE_URL;

  #[tokio::test]
  #[ignore] // requires real API key, incurs cost
  async fn live_image_to_video_720p_4s_audio() {
    let state = FalVeo3FastRequestState { mode: FalVeo3FastMode::ImageToVideo(Veo3FastImageToVideoRequest { prompt: "the dog leaps into the lake.".to_string(), image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), aspect_ratio: None, duration: Some(Veo3FastImageToVideoDuration::FourSeconds), resolution: Some(Veo3FastImageToVideoResolution::SevenTwentyP), generate_audio: Some(true), negative_prompt: None, seed: None, auto_fix: None, safety_tolerance: None }) };
