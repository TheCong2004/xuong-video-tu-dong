...
mod tests {
  ...
  async fn live_text_to_video_4s() {
    let r = run_pipeline(GenerateVideoRequestBuilder { prompt: Some("a candle flickering in the dark".to_string()), duration_seconds: Some(4), ..builder() }).await;
  ...
  async fn live_image_to_video_6s() {
    let r = run_pipeline(GenerateVideoRequestBuilder { prompt: Some("the dog leaps into the lake.".to_string()), start_frame: Some(ImageRef::Url(JUNO_AT_LAKE_IMAGE_URL.to_string())), duration_seconds: Some(6), ..builder() }).await;
    assert!(matches!(r, GenerateVideoResponse::Fal(_)));
  }
  ...
  fn builder() -> GenerateVideoRequestBuilder {
    GenerateVideoRequestBuilder { model: RouterVideoModel::Veo3p1Fast, provider: RouterProvider::Fal, ..Default::default() }
  }
