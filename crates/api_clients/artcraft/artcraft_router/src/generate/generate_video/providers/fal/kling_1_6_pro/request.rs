...
mod tests {
  ...
  use fal_client::requests::api::video::elements::kling_1p6_pro_elements_to_video::api::{Kling1p6ProElementsToVideoAspectRatio, Kling1p6ProElementsToVideoDuration};
  use fal_client::requests::api::video::image::kling_1p6_pro_image_to_video::api::{Kling1p6ProImageToVideoAspectRatio, Kling1p6ProImageToVideoDuration};
  use fal_client::requests::api::video::text::kling_1p6_pro_text_to_video::api::{Kling1p6ProTextToVideoAspectRatio, Kling1p6ProTextToVideoDuration};
  ...
  async fn send_text_to_video_webhook() {
    let state = FalKling16ProRequestState { mode: FalKling16ProMode::TextToVideo(Kling1p6ProTextToVideoRequest { prompt: "a calm lake at sunrise".to_string(), negative_prompt: None, duration: Some(Kling1p6ProTextToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling1p6ProTextToVideoAspectRatio::SixteenByNine), cfg_scale: None }) };
  ...
  async fn send_image_to_video_webhook() {
    let state = FalKling16ProRequestState { mode: FalKling16ProMode::ImageToVideo(Kling1p6ProImageToVideoRequest { prompt: "the lake comes alive with gentle ripples".to_string(), image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(), end_image_url: None, negative_prompt: None, duration: Some(Kling1p6ProImageToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling1p6ProImageToVideoAspectRatio::SixteenByNine), cfg_scale: None }) };
    let response = state.send(&client_with_webhook()).await.expect("send should succeed");
    let payload = response.get_fal_payload().expect("expected Fal payload");
    assert!(payload.request_id.is_some() || payload.gateway_request_id.is_some());
  }
