use fal_client::requests_old::webhook::video::image::enqueue_kling_v2p1_pro_image_to_video_webhook::{Kling2p1ProAspectRatio, Kling2p1ProDuration, Kling2p1ProRequest};
...
pub fn build_fal_kling_2_1_pro(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  let request = Kling2p1ProRequest { image_url, end_frame_image_url, prompt: builder.prompt.clone().unwrap_or_default(), duration, aspect_ratio };

  Ok(VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::FalKling21Pro(FalKling21ProRequestState { request })))
}

fn plan_aspect_ratio(aspect_ratio: Option<RouterAspectRatio>, strategy: RequestMismatchMitigationStrategy) -> Result<Kling2p1ProAspectRatio, ArtcraftRouterError> {
...
fn plan_duration(duration_seconds: Option<u16>, strategy: RequestMismatchMitigationStrategy) -> Result<Kling2p1ProDuration, ArtcraftRouterError> {
