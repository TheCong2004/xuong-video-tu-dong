...
#[derive(Clone, Debug)]
pub struct ArtcraftVeo3p1LiteRequestState {
  pub request: OmniGenVideoCostAndGenerateRequest,
}

impl ArtcraftVeo3p1LiteRequestState {
...
mod tests {
  ...
  async fn text_to_video() {
    let response = run_pipeline(GenerateVideoRequestBuilder { prompt: Some("A lighthouse beam sweeping across a foggy bay at night.".to_string()), aspect_ratio: Some(RouterAspectRatio::WideSixteenByNine), ..artcraft_builder() }).await;
  ...
  async fn keyframe_start_frame() {
    let response = run_pipeline(GenerateVideoRequestBuilder { prompt: Some("The dog turns its head toward the camera.".to_string()), start_frame: Some(ImageRef::MediaFileToken(MediaFileToken::new(JUNO_AT_LAKE_PRODUCTION_MEDIA_TOKEN.to_string()))), ..artcraft_builder() }).await;
    assert!(matches!(response, GenerateVideoResponse::Artcraft(_)));
    assert_eq!(1, 2, "Inspect output above");
  }
  ...
  fn artcraft_builder() -> GenerateVideoRequestBuilder {
    GenerateVideoRequestBuilder { model: RouterVideoModel::Veo3p1Lite, provider: RouterProvider::Artcraft, duration_seconds: Some(8), video_batch_count: Some(1), ..Default::default() }
  }
