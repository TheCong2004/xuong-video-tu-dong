use fal_client::requests::api::video::image::vidu_q3::api::{ViduQ3ImageToVideoRequest, ViduQ3ImageToVideoResolution};
use fal_client::requests::api::video::reference::vidu_q3_reference_to_video::api::{ViduQ3ReferenceToVideoAspectRatio, ViduQ3ReferenceToVideoRequest, ViduQ3ReferenceToVideoResolution};
use fal_client::requests::api::video::reference::vidu_q3_reference_to_video_mix::api::{ViduQ3ReferenceToVideoMixAspectRatio, ViduQ3ReferenceToVideoMixRequest, ViduQ3ReferenceToVideoMixResolution};
use fal_client::requests::api::video::text::vidu_q3::api::{ViduQ3TextToVideoAspectRatio, ViduQ3TextToVideoRequest, ViduQ3TextToVideoResolution};
...
pub fn build_fal_vidu_q3(builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  let state = build_fal_vidu_q3_state(builder)?;
  Ok(VideoGenerationDraftOrRequest::Request(VideoGenerationRequest::FalViduQ3(state)))
}

pub(crate) fn build_fal_vidu_q3_state(builder: GenerateVideoRequestBuilder) -> Result<FalViduQ3RequestState, ArtcraftRouterError> {
  ...
  let mode = if !reference_image_urls.is_empty() {
    if start_image_url.is_some() || end_image_url.is_some() {
      return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "reference_images", value: "Vidu Q3 reference-to-video cannot also accept a start_frame or end_frame".to_string() }));
    }
    if reference_image_urls.len() > MAX_REFERENCE_IMAGES {
      return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "reference_images", value: format!("Vidu Q3 supports at most {} reference images, got {}", MAX_REFERENCE_IMAGES, reference_image_urls.len(),) }));
    }
    ...
    if reference_image_urls.len() == 1 { FalViduQ3Mode::ReferenceToVideo(ViduQ3ReferenceToVideoRequest { prompt, reference_image_urls, duration, seed: None, aspect_ratio: aspect_ratio.map(to_reference_aspect_ratio), resolution: resolution.map(to_reference_resolution), audio }) } else { FalViduQ3Mode::ReferenceToVideoMix(ViduQ3ReferenceToVideoMixRequest { prompt, reference_image_urls, duration, seed: None, aspect_ratio: aspect_ratio.map(to_mix_aspect_ratio), resolution: resolution.map(to_mix_resolution), audio }) }
    ...
    FalViduQ3Mode::ImageToVideo(ViduQ3ImageToVideoRequest { prompt, image_url, end_image_url, duration, seed: None, resolution: i2v_resolution, audio })
  } else {
    if end_image_url.is_some() {
      return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "end_frame", value: "Vidu Q3 requires a start_frame when end_frame is provided".to_string() }));
    }
    FalViduQ3Mode::TextToVideo(ViduQ3TextToVideoRequest { prompt, duration, seed: None, aspect_ratio: aspect_ratio.map(to_t2v_aspect_ratio), resolution: resolution.map(to_t2v_resolution), audio })
  };
...
fn reject_reference_videos(reference_videos: &Option<VideoListRef>) -> Result<(), ArtcraftRouterError> {
  ...
  if has_reference_videos {
    return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "reference_videos", value: "Vidu Q3 does not support video references".to_string() }));
  }
  Ok(())
}
...
fn check_i2v_end_frame_resolution(resolution: Option<ViduQ3ImageToVideoResolution>, has_end_frame: bool) -> Result<(), ArtcraftRouterError> {
  if has_end_frame && matches!(resolution, Some(ViduQ3ImageToVideoResolution::ThreeSixtyP)) {
    return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption { field: "resolution", value: "Vidu Q3 image-to-video does not support 360p when an end_frame is provided".to_string() }));
  }
  Ok(())
}
...
mod tests {
  ...
  mod plan_tests {
    ...
    fn i2v_360p_with_end_frame_is_rejected_by_guard() {
      ...
      let result = check_i2v_end_frame_resolution(Some(ViduQ3ImageToVideoResolution::ThreeSixtyP), true);
  ...
  fn base_builder() -> GenerateVideoRequestBuilder {
    GenerateVideoRequestBuilder { model: RouterVideoModel::ViduQ3, provider: RouterProvider::Fal, prompt: Some("test".to_string()), ..Default::default() }
  }

  fn builder_with_references(count: usize) -> GenerateVideoRequestBuilder {
    let urls = (0..count).map(|i| format!("https://example.com/ref-{}.png", i)).collect();
    let mut b = base_builder();
