...
pub struct ArtcraftSeedance2p0BytePlusMiniCostState {
...
impl ArtcraftSeedance2p0BytePlusMiniCostState {
  pub fn from_request(request: &ArtcraftSeedance2p0BytePlusMiniRequestState) -> Self {
  ...
  pub fn estimate_cost(&self) -> VideoGenerationCostEstimate {
    ...
    let usd_cents = seedance_2p0_byteplus_mini_usd_cents(self.resolution, self.duration_seconds, self.batch_count, self.has_video_reference);
...
mod tests {
  ...
  mod pricing_480p {
    ...
    fn without_video_reference() {
      ...
      assert_eq!(cents(RouterResolution::FourEightyP, 15, 1, false), 50);
    }
    ...
    fn with_video_reference() {
      ...
      assert_eq!(cents(RouterResolution::FourEightyP, 15, 1, true), 63);
    }
  }
  ...
  mod pricing_720p {
    ...
    fn without_video_reference() {
      ...
      assert_eq!(cents(RouterResolution::SevenTwentyP, 10, 1, false), 88);
      assert_eq!(cents(RouterResolution::SevenTwentyP, 15, 1, false), 131);
    }

    #[test]
    fn with_video_reference() {
      assert_eq!(cents(RouterResolution::SevenTwentyP, 4, 1, true), 42);
      assert_eq!(cents(RouterResolution::SevenTwentyP, 5, 1, true), 53);
      assert_eq!(cents(RouterResolution::SevenTwentyP, 10, 1, true), 105);
      assert_eq!(cents(RouterResolution::SevenTwentyP, 15, 1, true), 158);
    }
  ...
  fn build_cost(resolution: Option<RouterResolution>, duration_seconds: u16, video_batch_count: u16, with_video_reference: bool) -> VideoGenerationCostEstimate {
    ...
    let builder = GenerateVideoRequestBuilder { model: RouterVideoModel::Seedance2p0BytePlusMini, provider: RouterProvider::Artcraft, resolution, duration_seconds: Some(duration_seconds), video_batch_count: Some(video_batch_count), reference_videos, ..Default::default() };
