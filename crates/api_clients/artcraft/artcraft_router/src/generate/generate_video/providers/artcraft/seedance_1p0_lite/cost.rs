...
use fal_client::requests_old::webhook::video::image::enqueue_seedance_1_lite_image_to_video_webhook::{Seedance1LiteDuration, Seedance1LiteRequest, Seedance1LiteResolution};
...
impl ArtcraftSeedance10LiteCostState {
  pub fn from_request(request: &ArtcraftSeedance10LiteRequestState) -> Self {
    ...
    let duration = if request.request.duration_seconds == Some(10) { Seedance1LiteDuration::TenSeconds } else { Seedance1LiteDuration::FiveSeconds };
  ...
  pub fn estimate_cost(&self) -> VideoGenerationCostEstimate {
    ...
    let req = Seedance1LiteRequest { image_url: String::new(), end_frame_image_url: None, prompt: String::new(), duration: self.duration, resolution: self.resolution, aspect_ratio: None, camera_fixed: false, seed: None };
...
mod tests {
  ...
  fn cost_cents(duration_seconds: Option<u16>, resolution: Option<RouterResolution>) -> u64 {
    let b = GenerateVideoRequestBuilder { model: RouterVideoModel::Seedance10Lite, provider: RouterProvider::Artcraft, prompt: Some("test".to_string()), duration_seconds, resolution, ..Default::default() };
  ...
  #[test]
  fn p480_5s_is_10() {
    assert_eq!(cost_cents(Some(5), Some(RouterResolution::FourEightyP)), 10);
  }

  #[test]
  fn p480_10s_is_18() {
    assert_eq!(cost_cents(Some(10), Some(RouterResolution::FourEightyP)), 18);
  }

  #[test]
  fn p720_5s_is_19() {
    assert_eq!(cost_cents(Some(5), Some(RouterResolution::SevenTwentyP)), 19);
  }

  #[test]
  fn p720_10s_is_52() {
    assert_eq!(cost_cents(Some(10), Some(RouterResolution::SevenTwentyP)), 52);
  }

  #[test]
  fn p1080_5s_is_58() {
    assert_eq!(cost_cents(Some(5), Some(RouterResolution::TenEightyP)), 58);
  }

  #[test]
  fn p1080_10s_is_116() {
    assert_eq!(cost_cents(Some(10), Some(RouterResolution::TenEightyP)), 116);
  }

  #[test]
  fn p480_5s_cheaper_than_p720_5s() {
    assert!(cost_cents(Some(5), Some(RouterResolution::FourEightyP)) < cost_cents(Some(5), Some(RouterResolution::SevenTwentyP)));
  }
}
