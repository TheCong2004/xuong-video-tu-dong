...
impl DatadogClient {
  ...
  pub async fn submit_distribution_points(&self, series: Vec<DistributionSeries>) -> Result<(), DatadogError> {
    if series.is_empty() {
      return Ok(());
    }
    ...
    let response = self.http.post(&url).header("DD-API-KEY", self.api_key.as_str()).json(&body).send().await?;
    expect_2xx(response).await
  }

  /// POST `/api/v1/series`. For pre-aggregated counts/gauges/rates.
  pub async fn submit_series(&self, series: Vec<MetricSeries>) -> Result<(), DatadogError> {
    if series.is_empty() {
      return Ok(());
    }
...
mod tests {
  ...
  fn metric_series_uses_type_key() {
    let s = MetricSeries { metric: "x".to_string(), points: vec![MetricPoint(1, 1.0)], metric_type: "count".to_string(), tags: vec!["a:b".to_string()], host: Some("h".to_string()), interval: Some(10) };
  ...
  fn empty_tags_omitted() {
    let s = MetricSeries { metric: "x".to_string(), points: vec![MetricPoint(1, 1.0)], metric_type: "gauge".to_string(), tags: vec![], host: None, interval: None };
    let json = serde_json::to_string(&s).unwrap();
  ...
  mod real_api_smoke {
    ...
    fn smoke_tags() -> Vec<String> {
      vec!["source:datadog_client_smoke_test".to_string(), "env:test".to_string()]
    }
    ...
    async fn submit_distribution_points_real_api() {
      ...
      let series = vec![DistributionSeries { metric: "datadog_client.smoke_test.duration".to_string(), points: vec![DistributionPoint(now_secs(), vec![1.0, 2.5, 7.0])], tags: smoke_tags(), host: Some("smoke-test-host".to_string()) }];
      client.submit_distribution_points(series).await.expect("distribution submission should succeed");
    }
    ...
    async fn submit_series_real_api() {
      ...
      let key = get_test_api_key().expect("could not load datadog_key.txt");
      let client = DatadogClient::new(DatadogApiKey::new(key));
      let series = vec![MetricSeries { metric: "datadog_client.smoke_test.count".to_string(), points: vec![MetricPoint(now_secs(), 1.0)], metric_type: "count".to_string(), tags: smoke_tags(), host: Some("smoke-test-host".to_string()), interval: Some(10) }];
      client.submit_series(series).await.expect("series submission should succeed");
    }
  }
}
