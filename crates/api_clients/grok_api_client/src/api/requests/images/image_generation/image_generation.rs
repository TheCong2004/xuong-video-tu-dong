...
pub async fn image_generation(args: ImageGenerationArgs<'_>) -> Result<ImageGenerationSuccess, GrokError> {
  ...
  info!("Grok image_generation: model={}, number_images={:?}, aspect_ratio={:?}, resolution={:?}", model.as_str(), req.number_images, req.aspect_ratio.map(|a| a.as_str()), req.resolution.map(|r| r.as_str()),);

  let request_body = ImageGenerationRequestBody { prompt: req.prompt, model: Some(model.as_str().to_string()), n: req.number_images, aspect_ratio: req.aspect_ratio.map(|a| a.as_str().to_string()), resolution: req.resolution.map(|r| r.as_str().to_string()), response_format: req.response_format.map(|f| f.as_str().to_string()), user: req.user };
  ...
  let parsed: ImageGenerationResponseBody = serde_json::from_str(&response_body).map_err(|err| GrokGenericApiError::SerdeResponseParseErrorWithBody(err, response_body.clone()))?;

  Ok(ImageGenerationSuccess { images: parsed.data.into_iter().map(|d| GeneratedImage { url: d.url, b64_json: d.b64_json, revised_prompt: d.revised_prompt }).collect() })
}
...
mod tests {
  ...
  fn wire_body_serializes_minimal() {
    let body = ImageGenerationRequestBody { prompt: "a cat".to_string(), model: Some("grok-imagine-image-quality".to_string()), n: None, aspect_ratio: None, resolution: None, response_format: None, user: None };
  ...
  fn wire_body_serializes_full() {
    let body = ImageGenerationRequestBody { prompt: "a cat".to_string(), model: Some("grok-imagine-image-quality".to_string()), n: Some(2), aspect_ratio: Some("16:9".to_string()), resolution: Some("2k".to_string()), response_format: Some("b64_json".to_string()), user: Some("user_abc".to_string()) };
    let json = serde_json::to_string(&body).unwrap();
  ...
  fn request_serializes_with_typed_enums() {
    let req = ImageGenerationRequest { prompt: "a serene lake".to_string(), model: Some(ImageModel::GrokImagineImageQuality), number_images: Some(3), aspect_ratio: Some(ImageAspectRatio::Landscape16x9), resolution: Some(ImageResolution::OneK), response_format: Some(ImageResponseFormat::Url), user: None };
  ...
  fn request_does_not_serialize_api_key() {
    ...
    let args = ImageGenerationArgs { api_key: &key, request: ImageGenerationRequest { prompt: "p".to_string(), model: None, number_images: None, aspect_ratio: None, resolution: None, response_format: None, user: None } };
  ...
  async fn live_test_image_generation_simple() -> AnyhowResult<()> {
    ...
    let result = image_generation(ImageGenerationArgs { api_key: &api_key, request: ImageGenerationRequest { prompt: "A serene mountain lake at sunrise, photorealistic".to_string(), model: None, number_images: None, aspect_ratio: Some(ImageAspectRatio::Landscape16x9), resolution: Some(ImageResolution::OneK), response_format: None, user: None } }).await.map_err(|e| anyhow::anyhow!("{}", e))?;
