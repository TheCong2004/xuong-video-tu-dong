...
use crate::requests::api::mesh::image::hunyuan_3d_3p1_pro_image_to_mesh::raw_request::{Hunyuan3d3p1ProImageToMeshInput, Hunyuan3d3p1ProImageToMeshOutput};
...
impl Hunyuan3d3p1ProImageToMeshRequest {
  ...
  pub fn uses_multi_view(&self) -> bool {
    self.back_image_url.is_some() || self.left_image_url.is_some() || self.right_image_url.is_some() || self.top_image_url.is_some() || self.bottom_image_url.is_some() || self.left_front_image_url.is_some() || self.right_front_image_url.is_some()
  }
}
...
impl FalEndpoint for Hunyuan3d3p1ProImageToMeshRequest {
  ...
  fn to_raw_request(&self) -> Result<Self::RawRequest, FalErrorPlus> {
    ...
    Ok(Self::RawRequest { input_image_url: self.image_url.clone(), back_image_url: self.back_image_url.clone(), left_image_url: self.left_image_url.clone(), right_image_url: self.right_image_url.clone(), top_image_url: self.top_image_url.clone(), bottom_image_url: self.bottom_image_url.clone(), left_front_image_url: self.left_front_image_url.clone(), right_front_image_url: self.right_front_image_url.clone(), generate_type, face_count: self.face_count, enable_pbr: self.enable_pbr })
  }
}
...
mod tests {
  ...
  async fn test_image_to_mesh_webhook() -> AnyhowResult<()> {
    ...
    let request = Hunyuan3d3p1ProImageToMeshRequest { image_url: ERNEST_SCARED_STUPID_IMAGE_URL.to_string(), ..base_request() };
  ...
  fn raw_request_maps_all_fields() {
    let request = Hunyuan3d3p1ProImageToMeshRequest { image_url: "https://example.com/front.jpg".to_string(), back_image_url: Some("https://example.com/back.jpg".to_string()), left_image_url: Some("https://example.com/left.jpg".to_string()), right_image_url: Some("https://example.com/right.jpg".to_string()), top_image_url: Some("https://example.com/top.jpg".to_string()), bottom_image_url: Some("https://example.com/bottom.jpg".to_string()), left_front_image_url: Some("https://example.com/lf.jpg".to_string()), right_front_image_url: Some("https://example.com/rf.jpg".to_string()), generate_type: Some(Hunyuan3d3p1ProImageToMeshGenerateType::Geometry), face_count: Some(100_000), enable_pbr: Some(true) };
  ...
  fn uses_multi_view_detects_every_extra_view() {
    ...
    let setters: Vec<fn(&mut Hunyuan3d3p1ProImageToMeshRequest)> = vec![|r| r.back_image_url = Some("u".to_string()), |r| r.left_image_url = Some("u".to_string()), |r| r.right_image_url = Some("u".to_string()), |r| r.top_image_url = Some("u".to_string()), |r| r.bottom_image_url = Some("u".to_string()), |r| r.left_front_image_url = Some("u".to_string()), |r| r.right_front_image_url = Some("u".to_string())];
    for set in setters {
  ...
  fn endpoint_path_is_canonical() {
    assert_eq!(Hunyuan3d3p1ProImageToMeshRequest::ENDPOINT, "fal-ai/hunyuan-3d/v3.1/pro/image-to-3d",);
  }
