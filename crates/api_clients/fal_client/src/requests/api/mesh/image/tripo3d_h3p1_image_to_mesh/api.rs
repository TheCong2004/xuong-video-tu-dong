...
use crate::requests::api::mesh::image::tripo3d_h3p1_image_to_mesh::raw_request::{Tripo3dH3p1ImageToMeshInput, Tripo3dH3p1ImageToMeshOutput};
...
mod tests {
  ...
  async fn test_image_to_mesh_webhook() -> AnyhowResult<()> {
    ...
    let request = Tripo3dH3p1ImageToMeshRequest { image_url: ERNEST_SCARED_STUPID_IMAGE_URL.to_string(), ..base_request() };
  ...
  fn raw_request_maps_all_fields() {
    let request = Tripo3dH3p1ImageToMeshRequest { image_url: "https://example.com/input.jpg".to_string(), face_limit: Some(50_000), texture: Some(true), pbr: Some(false), model_seed: Some(1), texture_seed: Some(3), texture_quality: Some(Tripo3dH3p1ImageTextureQuality::Detailed), geometry_quality: Some(Tripo3dH3p1ImageGeometryQuality::Detailed), texture_alignment: Some(Tripo3dH3p1TextureAlignment::Geometry), auto_size: Some(true), orientation: Some(Tripo3dH3p1Orientation::AlignImage), quad: Some(true) };
