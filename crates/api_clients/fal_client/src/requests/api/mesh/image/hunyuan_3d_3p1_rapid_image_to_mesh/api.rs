...
use crate::requests::api::mesh::image::hunyuan_3d_3p1_rapid_image_to_mesh::raw_request::{Hunyuan3d3p1RapidImageToMeshInput, Hunyuan3d3p1RapidImageToMeshOutput};
...
impl FalEndpoint for Hunyuan3d3p1RapidImageToMeshRequest {
  ...
  fn to_raw_request(&self) -> Result<Self::RawRequest, FalErrorPlus> {
    Ok(Self::RawRequest { input_image_url: self.image_url.clone(), enable_pbr: self.enable_pbr, enable_geometry: self.enable_geometry })
  }
}
...
mod tests {
  ...
  async fn test_image_to_mesh_webhook() -> AnyhowResult<()> {
    ...
    let request = Hunyuan3d3p1RapidImageToMeshRequest { image_url: ERNEST_SCARED_STUPID_IMAGE_URL.to_string(), enable_pbr: None, enable_geometry: None };
  ...
  fn raw_request_maps_all_fields() {
    let request = Hunyuan3d3p1RapidImageToMeshRequest { image_url: "https://example.com/front.jpg".to_string(), enable_pbr: Some(true), enable_geometry: Some(true) };
  ...
  fn raw_request_omits_unset_optionals() {
    let request = Hunyuan3d3p1RapidImageToMeshRequest { image_url: "https://example.com/front.jpg".to_string(), enable_pbr: None, enable_geometry: None };
    let json = serde_json::to_value(request.to_raw_request().unwrap()).unwrap();
  ...
  fn endpoint_path_is_canonical() {
    assert_eq!(Hunyuan3d3p1RapidImageToMeshRequest::ENDPOINT, "fal-ai/hunyuan-3d/v3.1/rapid/image-to-3d",);
  }
