...
use crate::requests::api::mesh::topology::hunyuan_3d_3p1_smart_topology::raw_request::{Hunyuan3d3p1SmartTopologyInput, Hunyuan3d3p1SmartTopologyOutput};
...
impl FalEndpoint for Hunyuan3d3p1SmartTopologyRequest {
  ...
  fn to_raw_request(&self) -> Result<Self::RawRequest, FalErrorPlus> {
    let input_file_type = self.input_file_type.map(|t| {
      match t {
    ...
    let face_level = self.face_level.map(|l| {
      match l {
    ...
    Ok(Self::RawRequest { input_file_url: self.input_file_url.clone(), input_file_type, polygon_type, face_level })
  }
}
...
mod tests {
  ...
  async fn test_smart_topology_webhook() -> AnyhowResult<()> {
    ...
    let request = Hunyuan3d3p1SmartTopologyRequest { input_file_url: "https://example.com/model.glb".to_string(), input_file_type: None, polygon_type: None, face_level: None };
  ...
  fn raw_request_maps_all_fields() {
    let request = Hunyuan3d3p1SmartTopologyRequest { input_file_url: "https://example.com/model.obj".to_string(), input_file_type: Some(Hunyuan3d3p1SmartTopologyInputFileType::Obj), polygon_type: Some(Hunyuan3d3p1SmartTopologyPolygonType::Quadrilateral), face_level: Some(Hunyuan3d3p1SmartTopologyFaceLevel::Low) };
  ...
  fn raw_request_omits_unset_optionals() {
    let request = Hunyuan3d3p1SmartTopologyRequest { input_file_url: "https://example.com/model.glb".to_string(), input_file_type: None, polygon_type: None, face_level: None };
    let json = serde_json::to_value(request.to_raw_request().unwrap()).unwrap();
    assert_eq!(json, serde_json::json!({ "input_file_url": "https://example.com/model.glb" }),);
  }

  #[test]
  fn every_face_level_maps_to_wire_string() {
    for (variant, expected) in [(Hunyuan3d3p1SmartTopologyFaceLevel::High, "high"), (Hunyuan3d3p1SmartTopologyFaceLevel::Medium, "medium"), (Hunyuan3d3p1SmartTopologyFaceLevel::Low, "low")] {
      let request = Hunyuan3d3p1SmartTopologyRequest { input_file_url: "https://example.com/model.glb".to_string(), input_file_type: None, polygon_type: None, face_level: Some(variant) };
  ...
  fn endpoint_path_is_canonical() {
    assert_eq!(Hunyuan3d3p1SmartTopologyRequest::ENDPOINT, "fal-ai/hunyuan-3d/v3.1/smart-topology",);
  }
