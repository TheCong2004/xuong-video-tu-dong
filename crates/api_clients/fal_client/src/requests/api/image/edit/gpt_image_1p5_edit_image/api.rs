...
use crate::requests::api::image::edit::gpt_image_1p5_edit_image::raw_request::{GptImage1p5EditImageInput, GptImage1p5EditImageOutput};
...
impl FalEndpoint for GptImage1p5EditImageRequest {
  ...
  fn to_raw_request(&self) -> Result<Self::RawRequest, FalErrorPlus> {
    ...
    Ok(Self::RawRequest { prompt: self.prompt.clone(), image_urls: self.image_urls.clone(), num_images: Some(num_images), mask_image_url: self.mask_image_url.clone(), image_size, background, quality, input_fidelity, output_format })
  }
}
...
mod tests {
  ...
  async fn test_edit_image_queue() -> AnyhowResult<()> {
    ...
    let request = GptImage1p5EditImageRequest { image_urls: vec![GHOST_IMAGE_URL.to_string(), TREX_SKELETON_IMAGE_URL.to_string(), ERNEST_SCARED_STUPID_IMAGE_URL.to_string()], prompt: "add the ghost and scared man to the image of the t-rex skeleton, make it look spooky but friendly".to_string(), num_images: GptImage1p5EditImageNumImages::Two, mask_image_url: None, image_size: None, background: None, quality: None, input_fidelity: None, output_format: None };
  ...
  async fn test_edit_image_webhook() -> AnyhowResult<()> {
    ...
    let request = GptImage1p5EditImageRequest { image_urls: vec![GHOST_IMAGE_URL.to_string()], prompt: "make the ghost wear a top hat".to_string(), num_images: GptImage1p5EditImageNumImages::One, mask_image_url: None, image_size: Some(GptImage1p5EditImageSize::Square), background: None, quality: Some(GptImage1p5EditImageQuality::High), input_fidelity: None, output_format: Some(GptImage1p5EditImageOutputFormat::Png) };
