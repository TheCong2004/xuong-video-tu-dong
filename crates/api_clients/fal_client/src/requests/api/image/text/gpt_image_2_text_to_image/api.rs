...
use crate::requests::api::image::text::gpt_image_2_text_to_image::raw_request::{GptImage2TextToImageInput, GptImage2TextToImageOutput, ImageSizeParam};
...
impl FalEndpoint for GptImage2TextToImageRequest {
  ...
  fn to_raw_request(&self) -> Result<Self::RawRequest, FalErrorPlus> {
    ...
    Ok(Self::RawRequest { prompt: self.prompt.clone(), num_images: Some(num_images), image_size, quality, output_format })
  }
}
...
mod tests {
  ...
  async fn test_text_to_image_queue() -> AnyhowResult<()> {
    ...
    let request = GptImage2TextToImageRequest { prompt: "an anime girl riding on the back of a t-rex".to_string(), num_images: GptImage2TextToImageNumImages::One, image_size: None, resolution: None, quality: None, output_format: None };
  ...
  async fn test_text_to_image_webhook() -> AnyhowResult<()> {
    ...
    let request = GptImage2TextToImageRequest { prompt: "a corgi wearing sunglasses at the beach".to_string(), num_images: GptImage2TextToImageNumImages::Two, image_size: Some(GptImage2TextToImageSize::Landscape16x9), resolution: None, quality: Some(GptImage2TextToImageQuality::High), output_format: Some(GptImage2TextToImageOutputFormat::Png) };
  ...
  async fn test_combinatorial_size_and_resolution() -> AnyhowResult<()> {
    ...
    let sizes = [("square", GptImage2TextToImageSize::Square), ("square_hd", GptImage2TextToImageSize::SquareHd), ("landscape_4x3", GptImage2TextToImageSize::Landscape4x3), ("landscape_16x9", GptImage2TextToImageSize::Landscape16x9), ("portrait_4x3", GptImage2TextToImageSize::Portrait4x3), ("portrait_16x9", GptImage2TextToImageSize::Portrait16x9)];
    ...
    for (size_name, size) in &sizes {
      for (res_name, resolution) in &resolutions {
        ...
        let prompt = format!("A photorealistic {} with the text \"{} {}\" printed on it, studio lighting, white background", surface, size_name, res_name,);

        let request = GptImage2TextToImageRequest { prompt, num_images: GptImage2TextToImageNumImages::One, image_size: Some(*size), resolution: Some(*resolution), quality: Some(GptImage2TextToImageQuality::Medium), output_format: Some(GptImage2TextToImageOutputFormat::Png) };
  ...
  mod to_raw_request_tests {
    ...
    fn preset_without_resolution() {
      let request = GptImage2TextToImageRequest { prompt: "test".to_string(), num_images: GptImage2TextToImageNumImages::One, image_size: Some(GptImage2TextToImageSize::Square), resolution: None, quality: None, output_format: None };
    ...
    fn custom_with_resolution() {
      let request = GptImage2TextToImageRequest { prompt: "test".to_string(), num_images: GptImage2TextToImageNumImages::One, image_size: Some(GptImage2TextToImageSize::Landscape16x9), resolution: Some(GptImage2Resolution::TwoK), quality: None, output_format: None };
      let raw = request.to_raw_request().unwrap();
      match raw.image_size.unwrap() {
    ...
    fn resolution_without_image_size_defaults_to_square() {
      let request = GptImage2TextToImageRequest { prompt: "test".to_string(), num_images: GptImage2TextToImageNumImages::One, image_size: None, resolution: Some(GptImage2Resolution::TwoK), quality: None, output_format: None };
      let raw = request.to_raw_request().unwrap();
      match raw.image_size.unwrap() {
        ImageSizeParam::Custom(c) => {
          assert_eq!(c.width, 2048);
    ...
    fn neither_size_nor_resolution() {
      let request = GptImage2TextToImageRequest { prompt: "test".to_string(), num_images: GptImage2TextToImageNumImages::One, image_size: None, resolution: None, quality: None, output_format: None };
      let raw = request.to_raw_request().unwrap();
