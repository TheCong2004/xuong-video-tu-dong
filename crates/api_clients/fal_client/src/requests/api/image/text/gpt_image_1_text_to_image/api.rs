...
use crate::requests::api::image::text::gpt_image_1_text_to_image::raw_request::{GptImage1TextToImageInput, GptImage1TextToImageOutput};
...
impl FalEndpoint for GptImage1TextToImageRequest {
  ...
  fn to_raw_request(&self) -> Result<Self::RawRequest, FalErrorPlus> {
    ...
    Ok(Self::RawRequest { prompt: self.prompt.clone(), num_images: Some(num_images), image_size, quality, background, output_format })
  }
}
...
mod tests {
  ...
  async fn test_text_to_image_queue() -> AnyhowResult<()> {
    ...
    let request = GptImage1TextToImageRequest { prompt: "an anime girl riding on the back of a t-rex".to_string(), num_images: GptImage1TextToImageNumImages::One, image_size: Some(GptImage1TextToImageSize::Horizontal), quality: Some(GptImage1TextToImageQuality::Medium), background: None, output_format: None };
  ...
  async fn test_text_to_image_webhook() -> AnyhowResult<()> {
    ...
    let request = GptImage1TextToImageRequest { prompt: "a corgi wearing sunglasses at the beach".to_string(), num_images: GptImage1TextToImageNumImages::Two, image_size: Some(GptImage1TextToImageSize::Square), quality: Some(GptImage1TextToImageQuality::High), background: Some(GptImage1TextToImageBackground::Opaque), output_format: Some(GptImage1TextToImageOutputFormat::Png) };
