use crate::requests::api::video::elements::kling_1p6_pro_elements_to_video::api::{Kling1p6ProElementsToVideoDuration, Kling1p6ProElementsToVideoRequest};
...
impl FalRequestCostCalculator for Kling1p6ProElementsToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Kling1p6ProElementsToVideoDuration::FiveSeconds).to_seconds();
...
mod tests {
  use super::*;
  use crate::requests::api::video::elements::kling_1p6_pro_elements_to_video::api::{Kling1p6ProElementsToVideoAspectRatio, Kling1p6ProElementsToVideoDuration};

  fn make_request(duration: Option<Kling1p6ProElementsToVideoDuration>) -> Kling1p6ProElementsToVideoRequest {
    Kling1p6ProElementsToVideoRequest { prompt: "test".to_string(), input_image_urls: vec!["https://example.com/a.png".to_string()], negative_prompt: None, duration, aspect_ratio: Some(Kling1p6ProElementsToVideoAspectRatio::SixteenByNine) }
  }
  ...
  fn five_seconds() {
    // (98 * 5 + 9) / 10 = 49
    assert_eq!(make_request(Some(Kling1p6ProElementsToVideoDuration::FiveSeconds)).calculate_cost_in_cents(), 49,);
  }

  #[test]
  fn ten_seconds() {
    // (98 * 10 + 9) / 10 = 98
    assert_eq!(make_request(Some(Kling1p6ProElementsToVideoDuration::TenSeconds)).calculate_cost_in_cents(), 98,);
  }
  ...
  fn ten_seconds_costs_more_than_five() {
    let five = make_request(Some(Kling1p6ProElementsToVideoDuration::FiveSeconds)).calculate_cost_in_cents();
    let ten = make_request(Some(Kling1p6ProElementsToVideoDuration::TenSeconds)).calculate_cost_in_cents();
  ...
  fn input_image_count_does_not_affect_cost() {
    ...
    // matches exactly.
    let base = Kling1p6ProElementsToVideoRequest { prompt: "p".to_string(), input_image_urls: vec!["https://example.com/a.png".to_string()], negative_prompt: None, duration: Some(Kling1p6ProElementsToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling1p6ProElementsToVideoAspectRatio::SixteenByNine) };
    ...
    for n in 1..=4usize {
      ...
      assert_eq!(req.calculate_cost_in_cents(), baseline_cost, "image count {n} should not change cost",);
    }
  }
  ...
  mod cost_table {
    ...
    use crate::requests_old::webhook::video::image::enqueue_kling_v1p6_pro_image_to_video_webhook::{Kling1p6ProAspectRatio as LegacyAspectRatio, Kling1p6ProDuration as LegacyDuration, Kling1p6ProRequest as LegacyRequest};

    fn new_request(duration: Option<Kling1p6ProElementsToVideoDuration>, aspect_ratio: Option<Kling1p6ProElementsToVideoAspectRatio>) -> Kling1p6ProElementsToVideoRequest {
      Kling1p6ProElementsToVideoRequest { prompt: "test".to_string(), input_image_urls: vec!["https://example.com/a.png".to_string()], negative_prompt: None, duration, aspect_ratio }
    }
    ...
    const COST_TABLE: &[(Option<Kling1p6ProElementsToVideoDuration>, Option<Kling1p6ProElementsToVideoAspectRatio>, u64)] = &[
    ...
    fn cost_is_independent_of_aspect_ratio() {
      let aspect_ratios = [None, Some(Kling1p6ProElementsToVideoAspectRatio::Square), Some(Kling1p6ProElementsToVideoAspectRatio::SixteenByNine), Some(Kling1p6ProElementsToVideoAspectRatio::NineBySixteen)];
      let durations = [None, Some(Kling1p6ProElementsToVideoDuration::FiveSeconds), Some(Kling1p6ProElementsToVideoDuration::TenSeconds)];
    ...
    fn premium_over_legacy_image_to_video_is_as_expected() {
      fn legacy_image(secs_variant: LegacyDuration) -> u64 {
        LegacyRequest { image_url: "https://example.com/i.png".to_string(), end_frame_image_url: None, prompt: "test".to_string(), duration: secs_variant, aspect_ratio: LegacyAspectRatio::WideSixteenNine }.calculate_cost_in_cents()
      }

      // (elements duration, equivalent legacy image duration, expected premium in cents)
      let cases = [(Kling1p6ProElementsToVideoDuration::FiveSeconds, LegacyDuration::FiveSeconds, 1u64), (Kling1p6ProElementsToVideoDuration::TenSeconds, LegacyDuration::TenSeconds, 3u64)];
      ...
      for (elements_d, legacy_d, expected_premium) in cases {
        ...
        assert_eq!(premium, expected_premium, "{elements_d:?}: elements={elements_cost}¢ legacy_image={image_cost}¢ premium={premium}¢ (want {expected_premium}¢)",);
      }
    }
    ...
    fn matches_text_to_video_cost_at_each_duration() {
      use crate::requests::api::video::text::kling_1p6_pro_text_to_video::api::{Kling1p6ProTextToVideoAspectRatio, Kling1p6ProTextToVideoDuration, Kling1p6ProTextToVideoRequest};
      fn t2v(duration: Kling1p6ProTextToVideoDuration) -> u64 {
        Kling1p6ProTextToVideoRequest { prompt: "test".to_string(), negative_prompt: None, duration: Some(duration), aspect_ratio: Some(Kling1p6ProTextToVideoAspectRatio::SixteenByNine), cfg_scale: None }.calculate_cost_in_cents()
      }
      let pairs = [(Kling1p6ProElementsToVideoDuration::FiveSeconds, Kling1p6ProTextToVideoDuration::FiveSeconds), (Kling1p6ProElementsToVideoDuration::TenSeconds, Kling1p6ProTextToVideoDuration::TenSeconds)];
      for (e_d, t_d) in pairs {
        let elements_cost = new_request(Some(e_d), None).calculate_cost_in_cents();
        let text_cost = t2v(t_d);
        assert_eq!(elements_cost, text_cost, "{e_d:?} vs {t_d:?}: elements={elements_cost}¢ text={text_cost}¢ — rates should match at $0.098/sec",);
      }
    }
  }
}
