use crate::requests::api::video::reference::veo_3p1_fast::api::{Veo3p1FastReferenceToVideoDuration, Veo3p1FastReferenceToVideoRequest, Veo3p1FastReferenceToVideoResolution};
...
impl FalRequestCostCalculator for Veo3p1FastReferenceToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Veo3p1FastReferenceToVideoDuration::EightSeconds).to_seconds();
    let four_k = self.resolution.unwrap_or(Veo3p1FastReferenceToVideoResolution::SevenTwentyP).is_four_k();
...
mod tests {
  ...
  fn make_request(duration: Option<Veo3p1FastReferenceToVideoDuration>, resolution: Option<Veo3p1FastReferenceToVideoResolution>, generate_audio: Option<bool>) -> Veo3p1FastReferenceToVideoRequest {
    Veo3p1FastReferenceToVideoRequest { prompt: "test".to_string(), image_urls: vec!["https://example.com/a.png".to_string()], aspect_ratio: None, duration, resolution, generate_audio, auto_fix: None, safety_tolerance: None }
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3p1FastReferenceToVideoDuration>, Option<Veo3p1FastReferenceToVideoResolution>, Option<bool>, u64)] = &[
    ...
    fn cost_is_independent_of_image_count() {
      let one = Veo3p1FastReferenceToVideoRequest { image_urls: vec!["https://example.com/a.png".to_string()], ..make_request(Some(Veo3p1FastReferenceToVideoDuration::EightSeconds), Some(Veo3p1FastReferenceToVideoResolution::TenEightyP), Some(true)) }.calculate_cost_in_cents();
      let many = Veo3p1FastReferenceToVideoRequest { image_urls: vec!["https://example.com/a.png".to_string(), "https://example.com/b.png".to_string(), "https://example.com/c.png".to_string()], ..make_request(Some(Veo3p1FastReferenceToVideoDuration::EightSeconds), Some(Veo3p1FastReferenceToVideoResolution::TenEightyP), Some(true)) }.calculate_cost_in_cents();
      assert_eq!(one, many);
    }
  }
}
