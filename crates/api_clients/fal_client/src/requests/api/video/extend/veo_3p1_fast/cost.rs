use crate::requests::api::video::extend::veo_3p1_fast::api::{Veo3p1FastExtendVideoDuration, Veo3p1FastExtendVideoRequest};
...
impl FalRequestCostCalculator for Veo3p1FastExtendVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Veo3p1FastExtendVideoDuration::SevenSeconds).to_seconds();
...
mod tests {
  ...
  fn make_request(duration: Option<Veo3p1FastExtendVideoDuration>, generate_audio: Option<bool>) -> Veo3p1FastExtendVideoRequest {
    Veo3p1FastExtendVideoRequest { prompt: "test".to_string(), video_url: "https://example.com/in.mp4".to_string(), aspect_ratio: None, duration, resolution: None, negative_prompt: None, generate_audio, seed: None, auto_fix: None, safety_tolerance: None }
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3p1FastExtendVideoDuration>, Option<bool>, u64)] = &[
    ...
    fn cost_is_independent_of_resolution() {
      for audio in [Some(false), Some(true), None] {
        let base = make_request(Some(Veo3p1FastExtendVideoDuration::SevenSeconds), audio).calculate_cost_in_cents();
        for res in [Veo3p1FastExtendVideoResolution::SevenTwentyP, Veo3p1FastExtendVideoResolution::TenEightyP] {
          let cost = Veo3p1FastExtendVideoRequest { resolution: Some(res), ..make_request(Some(Veo3p1FastExtendVideoDuration::SevenSeconds), audio) }.calculate_cost_in_cents();
