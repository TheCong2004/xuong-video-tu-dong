use crate::requests::api::video::image::veo_3p1_fast::api::{Veo3p1FastImageToVideoDuration, Veo3p1FastImageToVideoRequest, Veo3p1FastImageToVideoResolution};
...
impl FalRequestCostCalculator for Veo3p1FastImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Veo3p1FastImageToVideoDuration::EightSeconds).to_seconds();
    let four_k = self.resolution.unwrap_or(Veo3p1FastImageToVideoResolution::SevenTwentyP).is_four_k();
...
mod tests {
  ...
  fn make_request(duration: Option<Veo3p1FastImageToVideoDuration>, resolution: Option<Veo3p1FastImageToVideoResolution>, generate_audio: Option<bool>) -> Veo3p1FastImageToVideoRequest {
    Veo3p1FastImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/x.png".to_string(), aspect_ratio: Some(Veo3p1FastImageToVideoAspectRatio::SixteenByNine), duration, resolution, generate_audio, negative_prompt: None, seed: None, auto_fix: None, safety_tolerance: None }
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3p1FastImageToVideoDuration>, Option<Veo3p1FastImageToVideoResolution>, Option<bool>, u64)] = &[
    ...
    fn cost_is_independent_of_aspect_ratio() {
      let aspect_ratios = [Veo3p1FastImageToVideoAspectRatio::Auto, Veo3p1FastImageToVideoAspectRatio::SixteenByNine, Veo3p1FastImageToVideoAspectRatio::NineBySixteen];
      let baseline = {
        let mut r = make_request(Some(Veo3p1FastImageToVideoDuration::EightSeconds), Some(Veo3p1FastImageToVideoResolution::TenEightyP), Some(true));
