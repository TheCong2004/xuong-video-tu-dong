...
#[derive(Clone, Debug)]
pub struct Veo3p1LiteImageToVideoRequest {
  ...
  pub aspect_ratio: Option<Veo3p1LiteImageToVideoAspectRatio>,

  /// Video duration. fal's default is `8s` when `None`.
  pub duration: Option<Veo3p1LiteImageToVideoDuration>,

  /// Output resolution. fal's default is `720p` when `None`.
  pub resolution: Option<Veo3p1LiteImageToVideoResolution>,
  ...
  pub safety_tolerance: Option<Veo3p1LiteImageToVideoSafetyTolerance>,
}

#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1LiteImageToVideoAspectRatio {
  Auto,
  SixteenByNine,
  NineBySixteen,
}

impl Veo3p1LiteImageToVideoAspectRatio {
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1LiteImageToVideoDuration {
  FourSeconds,
  SixSeconds,
  EightSeconds,
}

impl Veo3p1LiteImageToVideoDuration {
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1LiteImageToVideoResolution {
  SevenTwentyP,
  TenEightyP,
}

impl Veo3p1LiteImageToVideoResolution {
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1LiteImageToVideoSafetyTolerance {
  /// "1" — strictest.
  Level1,
  Level2,
  Level3,
...
impl Veo3p1LiteImageToVideoSafetyTolerance {
  fn to_str(&self) -> &'static str {
    match self {
      Self::Level1 => "1",
      Self::Level2 => "2",
...
impl FalEndpoint for Veo3p1LiteImageToVideoRequest {
  const ENDPOINT: &str = "fal-ai/veo3.1/lite/image-to-video";

  type RawRequest = Veo3p1LiteImageToVideoInput;
