use crate::requests::api::video::image::kling_2p6_pro_image_to_video::api::{Kling2p6ProImageToVideoDuration, Kling2p6ProImageToVideoRequest};
...
impl FalRequestCostCalculator for Kling2p6ProImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Kling2p6ProImageToVideoDuration::FiveSeconds).to_seconds();
...
mod tests {
  ...
  fn make_request(duration: Option<Kling2p6ProImageToVideoDuration>, generate_audio: Option<bool>, voice_ids: Option<Vec<String>>) -> Kling2p6ProImageToVideoRequest {
    Kling2p6ProImageToVideoRequest { prompt: "test".to_string(), start_image_url: "https://example.com/start.png".to_string(), end_image_url: None, duration, negative_prompt: None, generate_audio, voice_ids }
  }
  ...
  fn five_seconds_audio_off() {
    // (70 * 5 + 9) / 10 = 35
    assert_eq!(make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(false), None).calculate_cost_in_cents(), 35,);
  }

  #[test]
  fn five_seconds_audio_on_no_voice() {
    // (140 * 5 + 9) / 10 = 70 — matches fal's "$0.70 for 5s with audio" example
    assert_eq!(make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), None).calculate_cost_in_cents(), 70,);
  }

  #[test]
  fn five_seconds_audio_on_with_voice() {
    // (168 * 5 + 9) / 10 = 84
    assert_eq!(make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), Some(vec!["voice_a".to_string()]),).calculate_cost_in_cents(), 84,);
  }

  #[test]
  fn ten_seconds_audio_off() {
    // (70 * 10 + 9) / 10 = 70
    assert_eq!(make_request(Some(Kling2p6ProImageToVideoDuration::TenSeconds), Some(false), None).calculate_cost_in_cents(), 70,);
  }

  #[test]
  fn ten_seconds_audio_on_no_voice() {
    // (140 * 10 + 9) / 10 = 140
    assert_eq!(make_request(Some(Kling2p6ProImageToVideoDuration::TenSeconds), Some(true), None).calculate_cost_in_cents(), 140,);
  }

  #[test]
  fn ten_seconds_audio_on_with_voice() {
    // (168 * 10 + 9) / 10 = 168 — matches fal's "$1.68 for 10s with audio + voice" example
    assert_eq!(make_request(Some(Kling2p6ProImageToVideoDuration::TenSeconds), Some(true), Some(vec!["voice_a".to_string()]),).calculate_cost_in_cents(), 168,);
  }
  ...
  fn default_audio_is_on() {
    let none_default = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), None, None).calculate_cost_in_cents();
    let explicit_on = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), None).calculate_cost_in_cents();
  ...
  fn empty_voice_ids_does_not_trigger_voice_tier() {
    ...
    let with_empty = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), Some(vec![])).calculate_cost_in_cents();
    let with_none = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), None).calculate_cost_in_cents();
  ...
  fn voice_ids_with_audio_off_does_not_change_cost() {
    ...
    let with_voice = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(false), Some(vec!["voice_a".to_string()])).calculate_cost_in_cents();
    let without_voice = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(false), None).calculate_cost_in_cents();
  ...
  fn pricing_tiers_are_strictly_ascending() {
    ...
    for d in [Kling2p6ProImageToVideoDuration::FiveSeconds, Kling2p6ProImageToVideoDuration::TenSeconds] {
      ...
      let vc = make_request(Some(d), Some(true), Some(vec!["v".to_string()])).calculate_cost_in_cents();
  ...
  mod cost_table {
    ...
    fn matches_cost_table() {
      for &(duration, generate_audio, voice_present, expected) in COST_TABLE {
        ...
        assert_eq!(got, expected, "duration={duration:?} audio={generate_audio:?} voice={voice_present}",);
      }
    }
    ...
    fn voice_count_does_not_affect_cost() {
      let one = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), Some(vec!["voice_a".to_string()])).calculate_cost_in_cents();
      let two = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), Some(vec!["voice_a".to_string(), "voice_b".to_string()])).calculate_cost_in_cents();
      assert_eq!(one, two);
      assert_eq!(one, 84);
    }
    ...
    fn cost_is_independent_of_end_image_url() {
      let with_end = Kling2p6ProImageToVideoRequest { end_image_url: Some("https://example.com/end.png".to_string()), ..make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), None) };
      let without_end = make_request(Some(Kling2p6ProImageToVideoDuration::FiveSeconds), Some(true), None);
      assert_eq!(with_end.calculate_cost_in_cents(), without_end.calculate_cost_in_cents(),);
    }
