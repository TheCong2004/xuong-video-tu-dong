use crate::requests::api::video::image::veo_3::api::{Veo3ImageToVideoDuration, Veo3ImageToVideoRequest};
...
impl FalRequestCostCalculator for Veo3ImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Veo3ImageToVideoDuration::EightSeconds).to_seconds();
...
mod tests {
  ...
  fn make_request(duration: Option<Veo3ImageToVideoDuration>, generate_audio: Option<bool>) -> Veo3ImageToVideoRequest {
    Veo3ImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/x.png".to_string(), aspect_ratio: None, duration, resolution: None, generate_audio, negative_prompt: None, seed: None, auto_fix: None, safety_tolerance: None }
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3ImageToVideoDuration>, Option<bool>, u64)] = &[
    ...
    fn cost_is_independent_of_aspect_ratio() {
      ...
      for ar in [Veo3ImageToVideoAspectRatio::Auto, Veo3ImageToVideoAspectRatio::SixteenByNine, Veo3ImageToVideoAspectRatio::NineBySixteen] {
        let mut r = make_request(Some(Veo3ImageToVideoDuration::EightSeconds), Some(true));
