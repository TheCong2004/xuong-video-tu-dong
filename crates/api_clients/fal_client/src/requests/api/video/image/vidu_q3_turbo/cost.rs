use crate::requests::api::video::image::vidu_q3_turbo::api::{ViduQ3TurboImageToVideoRequest, ViduQ3TurboImageToVideoResolution};
use crate::requests::api::video::text::vidu_q3_turbo::cost::{vidu_q3_turbo_cost_cents, vidu_q3_turbo_rate_tenth_cents_per_sec};
...
impl FalRequestCostCalculator for ViduQ3TurboImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let is_high_res = self.resolution.unwrap_or(ViduQ3TurboImageToVideoResolution::SevenTwentyP).is_high_res();
...
mod tests {
  ...
  fn make_request(duration: Option<u8>, resolution: Option<ViduQ3TurboImageToVideoResolution>) -> ViduQ3TurboImageToVideoRequest {
    ViduQ3TurboImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/x.png".to_string(), end_image_url: None, duration, seed: None, resolution, audio: Some(true) }
  }
  ...
  mod cost_table {
    ...
    fn cost_is_independent_of_end_frame() {
      ...
      let with = ViduQ3TurboImageToVideoRequest { end_image_url: Some("https://example.com/end.png".to_string()), ..make_request(Some(8), Some(ViduQ3TurboImageToVideoResolution::SevenTwentyP)) }.calculate_cost_in_cents();
