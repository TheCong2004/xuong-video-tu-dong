use crate::requests::api::video::image::kling_1p6_pro_image_to_video::api::{Kling1p6ProImageToVideoDuration, Kling1p6ProImageToVideoRequest};
...
impl FalRequestCostCalculator for Kling1p6ProImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Kling1p6ProImageToVideoDuration::FiveSeconds).to_seconds();
...
mod tests {
  use super::*;
  use crate::requests::api::video::image::kling_1p6_pro_image_to_video::api::{Kling1p6ProImageToVideoAspectRatio, Kling1p6ProImageToVideoDuration};

  fn make_request(duration: Option<Kling1p6ProImageToVideoDuration>) -> Kling1p6ProImageToVideoRequest {
    Kling1p6ProImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/image.jpg".to_string(), end_image_url: None, negative_prompt: None, duration, aspect_ratio: Some(Kling1p6ProImageToVideoAspectRatio::SixteenByNine), cfg_scale: None }
  }
  ...
  fn five_seconds() {
    ...
    assert_eq!(make_request(Some(Kling1p6ProImageToVideoDuration::FiveSeconds)).calculate_cost_in_cents(), 48,);
  }

  #[test]
  fn ten_seconds() {
    // (95 * 10 + 9) / 10 = 95
    assert_eq!(make_request(Some(Kling1p6ProImageToVideoDuration::TenSeconds)).calculate_cost_in_cents(), 95,);
  }
  ...
  fn ten_seconds_costs_more_than_five() {
    let five = make_request(Some(Kling1p6ProImageToVideoDuration::FiveSeconds)).calculate_cost_in_cents();
    let ten = make_request(Some(Kling1p6ProImageToVideoDuration::TenSeconds)).calculate_cost_in_cents();
  ...
  mod legacy_parity {
    ...
    fn new_request(duration: Option<Kling1p6ProImageToVideoDuration>, aspect_ratio: Option<Kling1p6ProImageToVideoAspectRatio>) -> Kling1p6ProImageToVideoRequest {
      Kling1p6ProImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/image.jpg".to_string(), end_image_url: None, negative_prompt: None, duration, aspect_ratio, cfg_scale: None }
    }

    fn legacy_request(duration: Option<Kling1p6ProImageToVideoDuration>, aspect_ratio: Option<Kling1p6ProImageToVideoAspectRatio>) -> LegacyRequest {
      LegacyRequest { image_url: "https://example.com/image.jpg".to_string(), end_frame_image_url: None, prompt: "test".to_string(), duration: legacy_duration_for(duration), aspect_ratio: legacy_aspect_ratio_for(aspect_ratio) }
    }
    ...
    const COST_TABLE: &[(Option<Kling1p6ProImageToVideoDuration>, Option<Kling1p6ProImageToVideoAspectRatio>, u64)] = &[
    ...
    fn cost_is_independent_of_aspect_ratio() {
      let aspect_ratios = [None, Some(Kling1p6ProImageToVideoAspectRatio::Square), Some(Kling1p6ProImageToVideoAspectRatio::SixteenByNine), Some(Kling1p6ProImageToVideoAspectRatio::NineBySixteen)];
      let durations = [None, Some(Kling1p6ProImageToVideoDuration::FiveSeconds), Some(Kling1p6ProImageToVideoDuration::TenSeconds)];
