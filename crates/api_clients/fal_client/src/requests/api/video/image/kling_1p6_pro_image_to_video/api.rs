...
use crate::requests::api::video::image::kling_1p6_pro_image_to_video::raw_request::{Kling1p6ProImageToVideoInput, Kling1p6ProImageToVideoOutput};
...
mod tests {
  ...
  async fn test_image_to_video_webhook() -> AnyhowResult<()> {
    ...
    let request = Kling1p6ProImageToVideoRequest { prompt: "shiba in glasses runs to the lake and stands by the shore".to_string(), image_url: TALL_MOCHI_WITH_GLASSES_IMAGE_URL.to_string(), end_image_url: Some(JUNO_AT_LAKE_IMAGE_URL.to_string()), negative_prompt: None, duration: Some(Kling1p6ProImageToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling1p6ProImageToVideoAspectRatio::SixteenByNine), cfg_scale: None };
  ...
  async fn test_image_to_video_queue() -> AnyhowResult<()> {
    ...
    let request = Kling1p6ProImageToVideoRequest { prompt: "shiba bounds across the meadow chasing a falling leaf".to_string(), image_url: TALL_MOCHI_WITH_GLASSES_IMAGE_URL.to_string(), end_image_url: None, negative_prompt: None, duration: Some(Kling1p6ProImageToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling1p6ProImageToVideoAspectRatio::SixteenByNine), cfg_scale: None };
  ...
  async fn test_all_aspect_ratios() -> AnyhowResult<()> {
    ...
    let aspect_ratios = [Kling1p6ProImageToVideoAspectRatio::Square, Kling1p6ProImageToVideoAspectRatio::SixteenByNine, Kling1p6ProImageToVideoAspectRatio::NineBySixteen];
  ...
  async fn test_all_durations() -> AnyhowResult<()> {
    ...
    let durations = [Kling1p6ProImageToVideoDuration::FiveSeconds, Kling1p6ProImageToVideoDuration::TenSeconds];
  ...
  fn raw_request_aspect_ratio_uses_colon_format() {
    let request = Kling1p6ProImageToVideoRequest { prompt: "p".to_string(), image_url: "https://example.com/i.png".to_string(), end_image_url: None, negative_prompt: None, duration: Some(Kling1p6ProImageToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling1p6ProImageToVideoAspectRatio::SixteenByNine), cfg_scale: None };
  ...
  fn raw_request_translates_end_image_url_to_tail_image_url() {
    let request = Kling1p6ProImageToVideoRequest { prompt: "p".to_string(), image_url: "https://example.com/i.png".to_string(), end_image_url: Some("https://example.com/end.png".to_string()), negative_prompt: None, duration: None, aspect_ratio: None, cfg_scale: None };
    let raw = request.to_raw_request().unwrap();
