use crate::requests::api::video::image::kling_2p5_turbo_pro_image_to_video::api::{Kling2p5TurboProImageToVideoDuration, Kling2p5TurboProImageToVideoRequest};
...
impl FalRequestCostCalculator for Kling2p5TurboProImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Kling2p5TurboProImageToVideoDuration::FiveSeconds).to_seconds();
...
mod tests {
  ...
  fn make_request(duration: Option<Kling2p5TurboProImageToVideoDuration>) -> Kling2p5TurboProImageToVideoRequest {
    Kling2p5TurboProImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/x.png".to_string(), tail_image_url: None, duration, negative_prompt: None, cfg_scale: None }
  }
  ...
  fn five_seconds() {
    ...
    assert_eq!(make_request(Some(Kling2p5TurboProImageToVideoDuration::FiveSeconds)).calculate_cost_in_cents(), 35,);
  }

  #[test]
  fn ten_seconds() {
    // (70 * 10 + 9) / 10 = 70 — matches "$0.35 + 5×$0.07 = $0.70"
    assert_eq!(make_request(Some(Kling2p5TurboProImageToVideoDuration::TenSeconds)).calculate_cost_in_cents(), 70,);
  }
  ...
  fn ten_seconds_costs_more_than_five() {
    let five = make_request(Some(Kling2p5TurboProImageToVideoDuration::FiveSeconds)).calculate_cost_in_cents();
    let ten = make_request(Some(Kling2p5TurboProImageToVideoDuration::TenSeconds)).calculate_cost_in_cents();
  ...
  mod cost_table {
    ...
    fn new_request(duration: Option<Kling2p5TurboProImageToVideoDuration>, tail_image_url: Option<&str>) -> Kling2p5TurboProImageToVideoRequest {
      Kling2p5TurboProImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/x.png".to_string(), tail_image_url: tail_image_url.map(String::from), duration, negative_prompt: None, cfg_scale: None }
    }
    ...
    const COST_TABLE: &[(Option<Kling2p5TurboProImageToVideoDuration>, bool, u64)] = &[
    ...
    fn matches_cost_table() {
      for &(duration, tail_present, expected) in COST_TABLE {
        ...
        assert_eq!(got, expected, "duration={duration:?} tail_present={tail_present}",);
      }
    }
    ...
    fn cost_is_independent_of_tail_image_url() {
      let durations = [None, Some(Kling2p5TurboProImageToVideoDuration::FiveSeconds), Some(Kling2p5TurboProImageToVideoDuration::TenSeconds)];
      for duration in durations {
        let without = new_request(duration, None).calculate_cost_in_cents();
        let with = new_request(duration, Some("https://example.com/tail.png")).calculate_cost_in_cents();
    ...
    fn cost_is_independent_of_cfg_scale() {
      for cfg in [None, Some(0.0_f32), Some(0.5), Some(1.0)] {
        let req = Kling2p5TurboProImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/x.png".to_string(), tail_image_url: None, duration: Some(Kling2p5TurboProImageToVideoDuration::FiveSeconds), negative_prompt: None, cfg_scale: cfg };
    ...
    fn matches_text_to_video_pro_cost_at_each_duration() {
      use crate::requests::api::video::text::kling_2p5_turbo_pro_text_to_video::api::{Kling2p5TurboProTextToVideoAspectRatio, Kling2p5TurboProTextToVideoDuration, Kling2p5TurboProTextToVideoRequest};
      fn t2v(duration: Kling2p5TurboProTextToVideoDuration) -> u64 {
        Kling2p5TurboProTextToVideoRequest { prompt: "test".to_string(), negative_prompt: None, duration: Some(duration), aspect_ratio: Some(Kling2p5TurboProTextToVideoAspectRatio::SixteenByNine), cfg_scale: None }.calculate_cost_in_cents()
      }
      let pairs = [(Kling2p5TurboProImageToVideoDuration::FiveSeconds, Kling2p5TurboProTextToVideoDuration::FiveSeconds), (Kling2p5TurboProImageToVideoDuration::TenSeconds, Kling2p5TurboProTextToVideoDuration::TenSeconds)];
      for (i2v_d, t2v_d) in pairs {
        let image_cost = new_request(Some(i2v_d), None).calculate_cost_in_cents();
        let text_cost = t2v(t2v_d);
        assert_eq!(image_cost, text_cost, "{i2v_d:?} vs {t2v_d:?}: image={image_cost}¢ text={text_cost}¢ — rates should match at $0.07/sec",);
      }
    }
