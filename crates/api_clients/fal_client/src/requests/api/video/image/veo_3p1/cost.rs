use crate::requests::api::video::image::veo_3p1::api::{Veo3p1ImageToVideoDuration, Veo3p1ImageToVideoRequest, Veo3p1ImageToVideoResolution};
...
impl FalRequestCostCalculator for Veo3p1ImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Veo3p1ImageToVideoDuration::EightSeconds).to_seconds();
    let four_k = self.resolution.unwrap_or(Veo3p1ImageToVideoResolution::SevenTwentyP).is_four_k();
...
mod tests {
  ...
  fn make_request(duration: Option<Veo3p1ImageToVideoDuration>, resolution: Option<Veo3p1ImageToVideoResolution>, generate_audio: Option<bool>) -> Veo3p1ImageToVideoRequest {
    Veo3p1ImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/x.png".to_string(), aspect_ratio: Some(Veo3p1ImageToVideoAspectRatio::SixteenByNine), duration, resolution, generate_audio, negative_prompt: None, seed: None, auto_fix: None, safety_tolerance: None }
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3p1ImageToVideoDuration>, Option<Veo3p1ImageToVideoResolution>, Option<bool>, u64)] = &[
    ...
    fn cost_is_independent_of_aspect_ratio() {
      let aspect_ratios = [Veo3p1ImageToVideoAspectRatio::Auto, Veo3p1ImageToVideoAspectRatio::SixteenByNine, Veo3p1ImageToVideoAspectRatio::NineBySixteen];
      let baseline = {
        let mut r = make_request(Some(Veo3p1ImageToVideoDuration::EightSeconds), Some(Veo3p1ImageToVideoResolution::TenEightyP), Some(true));
