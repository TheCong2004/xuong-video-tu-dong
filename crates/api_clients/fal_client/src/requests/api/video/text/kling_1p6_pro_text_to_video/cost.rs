use crate::requests::api::video::text::kling_1p6_pro_text_to_video::api::{Kling1p6ProTextToVideoDuration, Kling1p6ProTextToVideoRequest};
...
impl FalRequestCostCalculator for Kling1p6ProTextToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Kling1p6ProTextToVideoDuration::FiveSeconds).to_seconds();
...
mod tests {
  use super::*;
  use crate::requests::api::video::text::kling_1p6_pro_text_to_video::api::{Kling1p6ProTextToVideoAspectRatio, Kling1p6ProTextToVideoDuration};
  use crate::requests::api::video::image::kling_1p6_pro_image_to_video::api::{Kling1p6ProImageToVideoAspectRatio, Kling1p6ProImageToVideoDuration, Kling1p6ProImageToVideoRequest};

  fn make_request(duration: Option<Kling1p6ProTextToVideoDuration>) -> Kling1p6ProTextToVideoRequest {
    Kling1p6ProTextToVideoRequest { prompt: "test".to_string(), negative_prompt: None, duration, aspect_ratio: Some(Kling1p6ProTextToVideoAspectRatio::SixteenByNine), cfg_scale: None }
  }
  ...
  fn five_seconds() {
    ...
    assert_eq!(make_request(Some(Kling1p6ProTextToVideoDuration::FiveSeconds)).calculate_cost_in_cents(), 49,);
  }

  #[test]
  fn ten_seconds() {
    // (98 * 10 + 9) / 10 = 98
    assert_eq!(make_request(Some(Kling1p6ProTextToVideoDuration::TenSeconds)).calculate_cost_in_cents(), 98,);
  }
  ...
  fn ten_seconds_costs_more_than_five() {
    let five = make_request(Some(Kling1p6ProTextToVideoDuration::FiveSeconds)).calculate_cost_in_cents();
    let ten = make_request(Some(Kling1p6ProTextToVideoDuration::TenSeconds)).calculate_cost_in_cents();
  ...
  fn text_to_video_is_pricier_than_image_to_video_at_each_duration() {
    fn i2v(duration: Kling1p6ProImageToVideoDuration) -> u64 {
      Kling1p6ProImageToVideoRequest { prompt: "test".to_string(), image_url: "https://example.com/i.png".to_string(), end_image_url: None, negative_prompt: None, duration: Some(duration), aspect_ratio: Some(Kling1p6ProImageToVideoAspectRatio::SixteenByNine), cfg_scale: None }.calculate_cost_in_cents()
    }
    let pairs = [(Kling1p6ProTextToVideoDuration::FiveSeconds, Kling1p6ProImageToVideoDuration::FiveSeconds), (Kling1p6ProTextToVideoDuration::TenSeconds, Kling1p6ProImageToVideoDuration::TenSeconds)];
    for (t, i) in pairs {
  ...
  mod cost_table {
    ...
    fn new_request(duration: Option<Kling1p6ProTextToVideoDuration>, aspect_ratio: Option<Kling1p6ProTextToVideoAspectRatio>) -> Kling1p6ProTextToVideoRequest {
      Kling1p6ProTextToVideoRequest { prompt: "test".to_string(), negative_prompt: None, duration, aspect_ratio, cfg_scale: None }
    }
    ...
    const COST_TABLE: &[(Option<Kling1p6ProTextToVideoDuration>, Option<Kling1p6ProTextToVideoAspectRatio>, u64)] = &[
    ...
    fn cost_is_independent_of_aspect_ratio() {
      let aspect_ratios = [None, Some(Kling1p6ProTextToVideoAspectRatio::Square), Some(Kling1p6ProTextToVideoAspectRatio::SixteenByNine), Some(Kling1p6ProTextToVideoAspectRatio::NineBySixteen)];
      let durations = [None, Some(Kling1p6ProTextToVideoDuration::FiveSeconds), Some(Kling1p6ProTextToVideoDuration::TenSeconds)];
    ...
    fn premium_over_legacy_image_to_video_is_as_expected() {
      ...
      let cases = [(Kling1p6ProTextToVideoDuration::FiveSeconds, LegacyDuration::FiveSeconds, 1u64), (Kling1p6ProTextToVideoDuration::TenSeconds, LegacyDuration::TenSeconds, 3u64)];
      ...
      for (text_d, legacy_d, expected_premium) in cases {
        ...
        assert_eq!(premium, expected_premium, "{text_d:?}: text={text_cost}¢ legacy_image={image_cost}¢ premium={premium}¢ (want {expected_premium}¢)",);
      }
    }
  }
}
