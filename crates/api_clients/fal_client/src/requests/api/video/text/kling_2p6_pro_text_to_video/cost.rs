use crate::requests::api::video::text::kling_2p6_pro_text_to_video::api::{Kling2p6ProTextToVideoDuration, Kling2p6ProTextToVideoRequest};
...
impl FalRequestCostCalculator for Kling2p6ProTextToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Kling2p6ProTextToVideoDuration::FiveSeconds).to_seconds();
...
mod tests {
  use super::*;
  use crate::requests::api::video::text::kling_2p6_pro_text_to_video::api::{Kling2p6ProTextToVideoAspectRatio, Kling2p6ProTextToVideoDuration};

  fn make_request(duration: Option<Kling2p6ProTextToVideoDuration>, generate_audio: Option<bool>) -> Kling2p6ProTextToVideoRequest {
    Kling2p6ProTextToVideoRequest { prompt: "test".to_string(), generate_audio, negative_prompt: None, duration, aspect_ratio: Some(Kling2p6ProTextToVideoAspectRatio::SixteenByNine), cfg_scale: None }
  }
  ...
  fn five_seconds_audio_off() {
    // (70 * 5 + 9) / 10 = 35
    assert_eq!(make_request(Some(Kling2p6ProTextToVideoDuration::FiveSeconds), Some(false)).calculate_cost_in_cents(), 35,);
  }

  #[test]
  fn five_seconds_audio_on() {
    // (140 * 5 + 9) / 10 = 70 — matches fal's $0.70 example
    assert_eq!(make_request(Some(Kling2p6ProTextToVideoDuration::FiveSeconds), Some(true)).calculate_cost_in_cents(), 70,);
  }

  #[test]
  fn ten_seconds_audio_off() {
    // (70 * 10 + 9) / 10 = 70
    assert_eq!(make_request(Some(Kling2p6ProTextToVideoDuration::TenSeconds), Some(false)).calculate_cost_in_cents(), 70,);
  }

  #[test]
  fn ten_seconds_audio_on() {
    // (140 * 10 + 9) / 10 = 140
    assert_eq!(make_request(Some(Kling2p6ProTextToVideoDuration::TenSeconds), Some(true)).calculate_cost_in_cents(), 140,);
  }
  ...
  fn default_audio_is_on() {
    ...
    let none_default = make_request(Some(Kling2p6ProTextToVideoDuration::FiveSeconds), None).calculate_cost_in_cents();
    let explicit_on = make_request(Some(Kling2p6ProTextToVideoDuration::FiveSeconds), Some(true)).calculate_cost_in_cents();
  ...
  fn ten_seconds_costs_more_than_five() {
    ...
    for audio in [Some(false), Some(true), None] {
      let five = make_request(Some(Kling2p6ProTextToVideoDuration::FiveSeconds), audio).calculate_cost_in_cents();
      let ten = make_request(Some(Kling2p6ProTextToVideoDuration::TenSeconds), audio).calculate_cost_in_cents();
  ...
  mod cost_table {
    ...
    fn new_request(duration: Option<Kling2p6ProTextToVideoDuration>, generate_audio: Option<bool>, aspect_ratio: Option<Kling2p6ProTextToVideoAspectRatio>) -> Kling2p6ProTextToVideoRequest {
      Kling2p6ProTextToVideoRequest { prompt: "test".to_string(), generate_audio, negative_prompt: None, duration, aspect_ratio, cfg_scale: None }
    }
    ...
    const COST_TABLE: &[(Option<Kling2p6ProTextToVideoDuration>, Option<bool>, u64)] = &[
    ...
    fn matches_cost_table() {
      for &(duration, generate_audio, expected) in COST_TABLE {
        ...
        assert_eq!(got, expected, "duration={duration:?} generate_audio={generate_audio:?}",);
      }
    }
    ...
    fn cost_is_independent_of_aspect_ratio() {
      let aspect_ratios = [None, Some(Kling2p6ProTextToVideoAspectRatio::Square), Some(Kling2p6ProTextToVideoAspectRatio::SixteenByNine), Some(Kling2p6ProTextToVideoAspectRatio::NineBySixteen)];
      let durations = [None, Some(Kling2p6ProTextToVideoDuration::FiveSeconds), Some(Kling2p6ProTextToVideoDuration::TenSeconds)];
      ...
      for duration in durations {
        for audio in audio_options {
          ...
          for &ar in &aspect_ratios[1..] {
            ...
            assert_eq!(cost, baseline, "duration={duration:?} audio={audio:?} ar={ar:?}: cost={cost}¢ != baseline={baseline}¢",);
          }
        }
      }
    }
    ...
    fn cost_is_independent_of_cfg_scale() {
      for cfg in [None, Some(0.0_f32), Some(0.5), Some(1.0)] {
        let req = Kling2p6ProTextToVideoRequest { prompt: "test".to_string(), generate_audio: Some(true), negative_prompt: None, duration: Some(Kling2p6ProTextToVideoDuration::FiveSeconds), aspect_ratio: Some(Kling2p6ProTextToVideoAspectRatio::SixteenByNine), cfg_scale: cfg };
    ...
    fn matches_documented_examples() {
      let cost = make_request(Some(Kling2p6ProTextToVideoDuration::FiveSeconds), Some(true)).calculate_cost_in_cents();
