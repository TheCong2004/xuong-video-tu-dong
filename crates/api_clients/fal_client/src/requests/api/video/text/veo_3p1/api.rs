...
#[derive(Clone, Debug)]
pub struct Veo3p1TextToVideoRequest {
  /// Text prompt describing the video to generate.
  pub prompt: String,

  /// Aspect ratio. fal's default is `16:9` when `None`.
  pub aspect_ratio: Option<Veo3p1TextToVideoAspectRatio>,

  /// Video duration. fal's default is `8s` when `None`.
  pub duration: Option<Veo3p1TextToVideoDuration>,

  /// Output resolution. fal's default is `720p` when `None`.
  pub resolution: Option<Veo3p1TextToVideoResolution>,
  ...
  pub safety_tolerance: Option<Veo3p1TextToVideoSafetyTolerance>,
}

#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1TextToVideoAspectRatio {
  SixteenByNine,
  NineBySixteen,
}

impl Veo3p1TextToVideoAspectRatio {
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1TextToVideoDuration {
  FourSeconds,
  SixSeconds,
  EightSeconds,
}

impl Veo3p1TextToVideoDuration {
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1TextToVideoResolution {
  SevenTwentyP,
  TenEightyP,
  FourK,
}

impl Veo3p1TextToVideoResolution {
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum Veo3p1TextToVideoSafetyTolerance {
  /// "1" — strictest.
  Level1,
  Level2,
  Level3,
...
impl Veo3p1TextToVideoSafetyTolerance {
  fn to_str(&self) -> &'static str {
    match self {
      Self::Level1 => "1",
      Self::Level2 => "2",
...
impl FalEndpoint for Veo3p1TextToVideoRequest {
  const ENDPOINT: &str = "fal-ai/veo3.1";

  type RawRequest = Veo3p1TextToVideoInput;
...
mod tests {
  ...
  fn endpoint_path_is_canonical() {
    assert_eq!(Veo3p1TextToVideoRequest::ENDPOINT, "fal-ai/veo3.1");
  }
