use crate::requests::api::video::text::veo_3_fast::api::{Veo3FastTextToVideoDuration, Veo3FastTextToVideoRequest};
...
impl FalRequestCostCalculator for Veo3FastTextToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    // affect price.
    let duration_secs = self.duration.unwrap_or(Veo3FastTextToVideoDuration::EightSeconds).to_seconds();
...
mod tests {
  use super::*;
  use crate::requests::api::video::text::veo_3_fast::api::{Veo3FastTextToVideoAspectRatio, Veo3FastTextToVideoResolution, Veo3FastTextToVideoSafetyTolerance};

  fn make_request(duration: Option<Veo3FastTextToVideoDuration>, generate_audio: Option<bool>) -> Veo3FastTextToVideoRequest {
    Veo3FastTextToVideoRequest { prompt: "test".to_string(), aspect_ratio: Some(Veo3FastTextToVideoAspectRatio::SixteenByNine), duration, resolution: None, negative_prompt: None, generate_audio, seed: None, auto_fix: None, safety_tolerance: None }
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3FastTextToVideoDuration>, Option<bool>, u64)] = &[
    ...
    fn cost_is_independent_of_resolution() {
      for audio in [Some(false), Some(true), None] {
        ...
        for res in [Veo3FastTextToVideoResolution::SevenTwentyP, Veo3FastTextToVideoResolution::TenEightyP] {
          let cost = Veo3FastTextToVideoRequest { resolution: Some(res), ..make_request(Some(Veo3FastTextToVideoDuration::EightSeconds), audio) }.calculate_cost_in_cents();
