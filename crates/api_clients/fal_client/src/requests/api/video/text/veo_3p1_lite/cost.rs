use crate::requests::api::video::text::veo_3p1_lite::api::{Veo3p1LiteTextToVideoDuration, Veo3p1LiteTextToVideoRequest, Veo3p1LiteTextToVideoResolution};
...
impl FalRequestCostCalculator for Veo3p1LiteTextToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Veo3p1LiteTextToVideoDuration::EightSeconds).to_seconds();
    let is_1080p = self.resolution.unwrap_or(Veo3p1LiteTextToVideoResolution::SevenTwentyP).is_ten_eighty_p();
...
mod tests {
  use super::*;
  use crate::requests::api::video::text::veo_3p1_lite::api::{Veo3p1LiteTextToVideoAspectRatio, Veo3p1LiteTextToVideoSafetyTolerance};

  fn make_request(duration: Option<Veo3p1LiteTextToVideoDuration>, resolution: Option<Veo3p1LiteTextToVideoResolution>, generate_audio: Option<bool>) -> Veo3p1LiteTextToVideoRequest {
    Veo3p1LiteTextToVideoRequest { prompt: "test".to_string(), aspect_ratio: Some(Veo3p1LiteTextToVideoAspectRatio::SixteenByNine), duration, resolution, negative_prompt: None, generate_audio, seed: None, auto_fix: None, safety_tolerance: None }
  }

  /// fal: "a 4 second video at 720p with audio will cost $0.20".
  #[test]
  fn matches_documented_4s_720p_audio_on_example() {
    let cost = make_request(Some(Veo3p1LiteTextToVideoDuration::FourSeconds), Some(Veo3p1LiteTextToVideoResolution::SevenTwentyP), Some(true)).calculate_cost_in_cents();
    assert_eq!(cost, 20);
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3p1LiteTextToVideoDuration>, Option<Veo3p1LiteTextToVideoResolution>, Option<bool>, u64)] = &[
    ...
    fn cost_ignores_non_billing_fields() {
      let baseline = make_request(Some(Veo3p1LiteTextToVideoDuration::EightSeconds), Some(Veo3p1LiteTextToVideoResolution::TenEightyP), Some(true)).calculate_cost_in_cents();
      let embellished = Veo3p1LiteTextToVideoRequest { prompt: "different".to_string(), aspect_ratio: Some(Veo3p1LiteTextToVideoAspectRatio::NineBySixteen), duration: Some(Veo3p1LiteTextToVideoDuration::EightSeconds), resolution: Some(Veo3p1LiteTextToVideoResolution::TenEightyP), negative_prompt: Some("noise".to_string()), generate_audio: Some(true), seed: Some(99), auto_fix: Some(false), safety_tolerance: Some(Veo3p1LiteTextToVideoSafetyTolerance::Level1) }.calculate_cost_in_cents();
