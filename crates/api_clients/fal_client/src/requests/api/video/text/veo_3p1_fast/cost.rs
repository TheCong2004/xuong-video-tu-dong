use crate::requests::api::video::text::veo_3p1_fast::api::{Veo3p1FastTextToVideoDuration, Veo3p1FastTextToVideoRequest, Veo3p1FastTextToVideoResolution};
...
impl FalRequestCostCalculator for Veo3p1FastTextToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration_secs = self.duration.unwrap_or(Veo3p1FastTextToVideoDuration::EightSeconds).to_seconds();
    let four_k = self.resolution.unwrap_or(Veo3p1FastTextToVideoResolution::SevenTwentyP).is_four_k();
...
mod tests {
  use super::*;
  use crate::requests::api::video::text::veo_3p1_fast::api::{Veo3p1FastTextToVideoAspectRatio, Veo3p1FastTextToVideoSafetyTolerance};

  fn make_request(duration: Option<Veo3p1FastTextToVideoDuration>, resolution: Option<Veo3p1FastTextToVideoResolution>, generate_audio: Option<bool>) -> Veo3p1FastTextToVideoRequest {
    Veo3p1FastTextToVideoRequest { prompt: "test".to_string(), aspect_ratio: Some(Veo3p1FastTextToVideoAspectRatio::SixteenByNine), duration, resolution, negative_prompt: None, generate_audio, seed: None, auto_fix: None, safety_tolerance: None }
  }
  ...
  mod cost_table {
    ...
    const COST_TABLE: &[(Option<Veo3p1FastTextToVideoDuration>, Option<Veo3p1FastTextToVideoResolution>, Option<bool>, u64)] = &[
    ...
    fn audio_on_is_more_expensive_than_off() {
      for res in [Veo3p1FastTextToVideoResolution::SevenTwentyP, Veo3p1FastTextToVideoResolution::TenEightyP, Veo3p1FastTextToVideoResolution::FourK] {
        let off = make_request(Some(Veo3p1FastTextToVideoDuration::EightSeconds), Some(res), Some(false)).calculate_cost_in_cents();
        let on = make_request(Some(Veo3p1FastTextToVideoDuration::EightSeconds), Some(res), Some(true)).calculate_cost_in_cents();
    ...
    fn four_k_is_more_expensive_than_sd() {
      for audio in [Some(false), Some(true)] {
        let sd = make_request(Some(Veo3p1FastTextToVideoDuration::EightSeconds), Some(Veo3p1FastTextToVideoResolution::TenEightyP), audio).calculate_cost_in_cents();
        let four_k = make_request(Some(Veo3p1FastTextToVideoDuration::EightSeconds), Some(Veo3p1FastTextToVideoResolution::FourK), audio).calculate_cost_in_cents();
    ...
    fn seven_twenty_and_ten_eighty_cost_the_same() {
      for audio in [Some(false), Some(true), None] {
        let sd = make_request(Some(Veo3p1FastTextToVideoDuration::EightSeconds), Some(Veo3p1FastTextToVideoResolution::SevenTwentyP), audio).calculate_cost_in_cents();
        let hd = make_request(Some(Veo3p1FastTextToVideoDuration::EightSeconds), Some(Veo3p1FastTextToVideoResolution::TenEightyP), audio).calculate_cost_in_cents();
    ...
    fn cost_ignores_non_billing_fields() {
      let baseline = make_request(Some(Veo3p1FastTextToVideoDuration::EightSeconds), Some(Veo3p1FastTextToVideoResolution::TenEightyP), Some(true)).calculate_cost_in_cents();

      let embellished = Veo3p1FastTextToVideoRequest { prompt: "different prompt".to_string(), aspect_ratio: Some(Veo3p1FastTextToVideoAspectRatio::NineBySixteen), duration: Some(Veo3p1FastTextToVideoDuration::EightSeconds), resolution: Some(Veo3p1FastTextToVideoResolution::TenEightyP), negative_prompt: Some("noise".to_string()), generate_audio: Some(true), seed: Some(99), auto_fix: Some(false), safety_tolerance: Some(Veo3p1FastTextToVideoSafetyTolerance::Level1) }.calculate_cost_in_cents();
