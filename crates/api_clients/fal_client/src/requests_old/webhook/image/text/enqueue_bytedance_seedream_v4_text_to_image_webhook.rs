...
pub async fn enqueue_bytedance_seedream_v4_text_to_image_webhook<R: IntoUrl>(args: EnqueueBytedanceSeedreamV4TextToImageArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let result = seedream_4_text_to_image(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let args = EnqueueBytedanceSeedreamV4TextToImageArgs { request: EnqueueBytedanceSeedreamV4TextToImageRequest { prompt: "an anime girl is riding a t-rex in the forest".to_string(), num_images: Some(EnqueueBytedanceSeedreamV4TextToImageNumImages::Two), max_images: Some(EnqueueBytedanceSeedreamV4TextToImageMaxImages::Two), image_size: Some(EnqueueBytedanceSeedreamV4TextToImageSize::LandscapeSixteenNine) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
