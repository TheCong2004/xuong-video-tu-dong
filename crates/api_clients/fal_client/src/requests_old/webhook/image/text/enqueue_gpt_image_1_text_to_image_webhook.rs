...
use crate::requests_old::http::image::text::http_gpt_image_1_text_to_image::{gpt_image_1_text_to_image, GptImage1TextToImageInput};
...
impl FalRequestCostCalculator for EnqueueGptImage1TextToImageRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let use_quality = self.quality.unwrap_or(EnqueueGptImage1TextToImageQuality::Medium);
    let is_square = matches!(self.image_size, None | Some(EnqueueGptImage1TextToImageSize::Square) | Some(EnqueueGptImage1TextToImageSize::Auto));
...
pub async fn enqueue_gpt_image_1_text_to_image_webhook<R: IntoUrl>(args: EnqueueGptImage1TextToImageArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let request = GptImage1TextToImageInput { prompt: req.prompt, num_images: Some(num_images), image_size, quality, background, output_format };

  let result = gpt_image_1_text_to_image(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  use crate::creds::fal_api_key::FalApiKey;
  use crate::requests_old::webhook::image::text::enqueue_gpt_image_1_text_to_image_webhook::{enqueue_gpt_image_1_text_to_image_webhook, EnqueueGptImage1TextToImageArgs, EnqueueGptImage1TextToImageNumImages, EnqueueGptImage1TextToImageQuality, EnqueueGptImage1TextToImageRequest, EnqueueGptImage1TextToImageSize};
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let args = EnqueueGptImage1TextToImageArgs { request: EnqueueGptImage1TextToImageRequest { prompt: "an anime girl riding on the back of a t-rex".to_string(), num_images: EnqueueGptImage1TextToImageNumImages::One, image_size: Some(EnqueueGptImage1TextToImageSize::Horizontal), quality: Some(EnqueueGptImage1TextToImageQuality::Medium), background: None, output_format: None }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
