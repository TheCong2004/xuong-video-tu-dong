...
pub async fn enqueue_bytedance_seedream_v4p5_edit_image_webhook<R: IntoUrl>(args: EnqueueBytedanceSeedreamV4p5EditImageArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let input = SeedreamV4p5EditImageInput { prompt: req.prompt, image_urls: req.image_urls, num_images, max_images, image_size, enable_safety_checker: Some(false), seed: None };

  seedream_4p5_edit_image(input).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await.map_err(|err| classify_fal_error(err))
}
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let args = EnqueueBytedanceSeedreamV4p5EditImageArgs { request: EnqueueBytedanceSeedreamV4p5EditImageRequest { prompt: "add the ghost to the image of the t-rex skeleton, make it look spooky but friendly".to_string(), image_urls: vec![GHOST_IMAGE_URL.to_string(), TREX_SKELETON_IMAGE_URL.to_string()], num_images: Some(EnqueueBytedanceSeedreamV4p5EditImageNumImages::Two), max_images: Some(EnqueueBytedanceSeedreamV4p5EditImageMaxImages::Two), image_size: Some(EnqueueBytedanceSeedreamV4p5EditImageSize::Auto2k) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
