...
pub async fn enqueue_bytedance_seedream_v4_edit_image_webhook<R: IntoUrl>(args: EnqueueBytedanceSeedreamV4EditImageArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let input = SeedreamV4EditImageInput { prompt: req.prompt, image_urls: req.image_urls, num_images, max_images, image_size, enhance_prompt_mode: Some("standard".to_string()), enable_safety_checker: Some(false) };

  seedream_4_edit_image(input).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await.map_err(|err| classify_fal_error(err))
}
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let args = EnqueueBytedanceSeedreamV4EditImageArgs { request: EnqueueBytedanceSeedreamV4EditImageRequest { prompt: "add the ghost to the image of the t-rex skeleton, make it look spooky but friendly".to_string(), image_urls: vec![GHOST_IMAGE_URL.to_string(), TREX_SKELETON_IMAGE_URL.to_string()], num_images: Some(EnqueueBytedanceSeedreamV4EditImageNumImages::Two), max_images: Some(EnqueueBytedanceSeedreamV4EditImageMaxImages::Two), image_size: Some(EnqueueBytedanceSeedreamV4EditImageSize::Auto2k) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
