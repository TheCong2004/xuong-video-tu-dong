...
pub async fn enqueue_flux_1_schnell_edit_image_webhook<U: IntoUrl>(args: Flux1SchnellEditImageArgs<'_, U>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let request = Flux1SchnellEditImageInput { image_url: req.image_url, num_images: Some(num_images), image_size, enable_safety_checker: Some(false), output_format: Some("png".to_string()), ..Default::default() };

  let result = flux_1_schnell_edit_image(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  use crate::creds::fal_api_key::FalApiKey;
  use crate::requests_old::webhook::image::edit::enqueue_flux_1_schnell_edit_image_webhook::{enqueue_flux_1_schnell_edit_image_webhook, Flux1SchnellEditImageArgs, Flux1SchnellEditImageNumImages, Flux1SchnellEditImageRequest, Flux1SchnellEditImageSize};
  ...
  async fn test_single_image_no_size() -> AnyhowResult<()> {
    ...
    let args = Flux1SchnellEditImageArgs { request: Flux1SchnellEditImageRequest { image_url: GHOST_IMAGE_URL.to_string(), num_images: Flux1SchnellEditImageNumImages::One, image_size: None }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
  ...
  async fn test_with_landscape_size() -> AnyhowResult<()> {
    ...
    let args = Flux1SchnellEditImageArgs { request: Flux1SchnellEditImageRequest { image_url: GHOST_IMAGE_URL.to_string(), num_images: Flux1SchnellEditImageNumImages::Two, image_size: Some(Flux1SchnellEditImageSize::LandscapeSixteenByNine) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
