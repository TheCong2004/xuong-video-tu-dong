...
use crate::requests_old::http::video::text::http_veo_2_text_to_video::{veo_2_text_to_video, Veo2TextToVideoInput};
use crate::requests::traits::fal_request_cost_calculator_trait::{FalRequestCostCalculator, UsdCents};
// Re-use the same duration/aspect-ratio enums as image-to-video.
pub use crate::requests_old::webhook::video::image::enqueue_veo_2_image_to_video_webhook::{Veo2AspectRatio, Veo2Duration};
use reqwest::IntoUrl;
...
pub async fn enqueue_veo_2_text_to_video_webhook<V: IntoUrl>(args: Veo2TextToVideoArgs<'_, V>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let request = Veo2TextToVideoInput { prompt: req.prompt, aspect_ratio, duration, negative_prompt: req.negative_prompt };

  let result = veo_2_text_to_video(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  use crate::creds::fal_api_key::FalApiKey;
  use crate::requests_old::webhook::video::text::enqueue_veo_2_text_to_video_webhook::{enqueue_veo_2_text_to_video_webhook, Veo2TextToVideoArgs, Veo2TextToVideoRequest, Veo2AspectRatio, Veo2Duration};
  ...
  async fn test_veo_2_text_to_video() -> AnyhowResult<()> {
    ...
    let args = Veo2TextToVideoArgs { request: Veo2TextToVideoRequest { prompt: "a drone shot of a coastal sunset with waves crashing on rocks".to_string(), negative_prompt: None, duration: Veo2Duration::FiveSeconds, aspect_ratio: Veo2AspectRatio::WideSixteenNine }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
