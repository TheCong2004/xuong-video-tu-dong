...
pub async fn enqueue_sora_2_text_to_video_webhook<R: IntoUrl>(args: EnqueueSora2TextToVideoArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let request = Sora2TextToVideoInput { prompt: req.prompt, duration, resolution, aspect_ratio, delete_video: Some(false) };

  let result = sora_2_text_to_video(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let args = EnqueueSora2TextToVideoArgs { request: EnqueueSora2TextToVideoRequest { prompt: "a dinosaur turns to the camera and asks, 'do you have adequate car insurance?' it then stomps off and attacks a brontosaurus".to_string(), duration: Some(EnqueueSora2TextToVideoDurationSeconds::Eight), aspect_ratio: Some(EnqueueSora2TextToVideoAspectRatio::NineBySixteen), resolution: Some(EnqueueSora2TextToVideoResolution::SevenTwentyP) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
  ...
  async fn test_all_aspect_ratios() -> AnyhowResult<()> {
    ...
    for ar in EnqueueSora2TextToVideoAspectRatio::iter() {
      ...
      let args = EnqueueSora2TextToVideoArgs { request: EnqueueSora2TextToVideoRequest { prompt: "a snowy mountain cabin at sunrise".to_string(), duration: Some(EnqueueSora2TextToVideoDurationSeconds::Four), aspect_ratio: Some(ar), resolution: None }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
      let result = enqueue_sora_2_text_to_video_webhook(args).await?;
      println!("result: {:?}", result);
    }
  ...
  async fn test_all_durations() -> AnyhowResult<()> {
    ...
    for dur in EnqueueSora2TextToVideoDurationSeconds::iter() {
      ...
      let args = EnqueueSora2TextToVideoArgs { request: EnqueueSora2TextToVideoRequest { prompt: "a corgi walks in front of a giant windmill in a beautiful village at sunrise".to_string(), duration: Some(dur), aspect_ratio: Some(EnqueueSora2TextToVideoAspectRatio::SixteenByNine), resolution: None }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
      let result = enqueue_sora_2_text_to_video_webhook(args).await?;
      println!("result: {:?}", result);
    }
