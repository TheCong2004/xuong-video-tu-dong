...
pub async fn enqueue_kling_v2p6_pro_text_to_video_webhook<R: IntoUrl>(args: EnqueueKlingV2p6ProTextToVideoArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let result = kling_v2p6_pro_text_to_video(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  use crate::creds::fal_api_key::FalApiKey;
  use crate::requests_old::webhook::video::text::enqueue_kling_v2p6_pro_text_to_video_webhook::{enqueue_kling_v2p6_pro_text_to_video_webhook, EnqueueKlingV2p6ProTextToVideoArgs, EnqueueKlingV2p6ProTextToVideoRequest, EnqueueKlingV2p6ProTextToVideoAspectRatio, EnqueueKlingV2p6ProTextToVideoDurationSeconds};
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let args = EnqueueKlingV2p6ProTextToVideoArgs { request: EnqueueKlingV2p6ProTextToVideoRequest { prompt: "a humanoid lizard creature walks around on the surface of an alien planet. the sky is purple and there are yellow plants. The lizard says, 'You should consider day trading'. Suddenly a giant t-rex comes in from off screen and eats the lizard creature.".to_string(), negative_prompt: None, aspect_ratio: Some(EnqueueKlingV2p6ProTextToVideoAspectRatio::SixteenByNine), duration: Some(EnqueueKlingV2p6ProTextToVideoDurationSeconds::Ten), generate_audio: Some(true) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
