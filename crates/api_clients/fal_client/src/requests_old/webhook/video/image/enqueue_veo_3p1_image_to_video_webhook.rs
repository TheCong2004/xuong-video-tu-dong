...
use crate::requests_old::http::video::image::http_veo_3p1_image_to_video::{veo_3p1_image_to_video, Veo3p1ImageToVideoInput};
...
impl FalRequestCostCalculator for EnqueueVeo3p1ImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    ...
    let duration = self.duration.unwrap_or(EnqueueVeo3p1ImageToVideoDurationSeconds::Eight);
...
pub async fn enqueue_veo_3p1_image_to_video_webhook<R: IntoUrl>(args: EnqueueVeo3p1ImageToVideoArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let result = veo_3p1_image_to_video(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  use crate::creds::fal_api_key::FalApiKey;
  use crate::requests_old::webhook::video::image::enqueue_veo_3p1_image_to_video_webhook::{enqueue_veo_3p1_image_to_video_webhook, EnqueueVeo3p1ImageToVideoArgs, EnqueueVeo3p1ImageToVideoAspectRatio, EnqueueVeo3p1ImageToVideoDurationSeconds, EnqueueVeo3p1ImageToVideoRequest, EnqueueVeo3p1ImageToVideoResolution};
