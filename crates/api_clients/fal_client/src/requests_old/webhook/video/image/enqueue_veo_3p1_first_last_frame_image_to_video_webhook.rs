...
use crate::requests_old::http::video::image::http_veo_3p1_first_last_frame_image_to_video::{veo_3p1_first_last_frame_image_to_video, Veo3p1FirstLastFrameImageToVideoInput};
use crate::requests::core_api::webhook_response::WebhookResponse;
use reqwest::IntoUrl;

pub struct EnqueueVeo3p1FirstLastFrameImageToVideoArgs<'a, R: IntoUrl> {
  pub request: EnqueueVeo3p1FirstLastFrameImageToVideoRequest,
  pub webhook_url: R,
  pub api_key: &'a FalApiKey,
}

#[derive(Clone, Debug)]
pub struct EnqueueVeo3p1FirstLastFrameImageToVideoRequest {
  // Request required
  pub prompt: String,
...
impl FalRequestCostCalculator for EnqueueVeo3p1FirstLastFrameImageToVideoRequest {
  fn calculate_cost_in_cents(&self) -> UsdCents {
    // "For every second of video you generated, you will be charged
    //  $0.20 (audio off) or
    //  $0.40 (audio on).
    //  For example, a 5s video with audio on will cost $2."
    let duration = self.duration.unwrap_or(EnqueueVeo3p1FirstLastFrameImageToVideoDurationSeconds::Eight);
...
pub async fn enqueue_veo_3p1_first_last_frame_image_to_video_webhook<R: IntoUrl>(args: EnqueueVeo3p1FirstLastFrameImageToVideoArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let result = veo_3p1_first_last_frame_image_to_video(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
