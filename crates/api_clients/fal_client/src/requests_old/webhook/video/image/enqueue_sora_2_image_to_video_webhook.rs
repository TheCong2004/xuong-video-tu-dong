...
pub async fn enqueue_sora_2_image_to_video_webhook<R: IntoUrl>(args: EnqueueSora2ImageToVideoArgs<'_, R>) -> Result<WebhookResponse, FalErrorPlus> {
  ...
  let request = Sora2ImageToVideoInput { prompt: req.prompt, image_url: req.image_url, duration, resolution, aspect_ratio, delete_video: Some(false) };

  let result = sora_2_image_to_video(request).with_api_key(&args.api_key.0).queue_webhook(args.webhook_url).await;
...
mod tests {
  ...
  async fn test() -> AnyhowResult<()> {
    ...
    let args = EnqueueSora2ImageToVideoArgs { request: EnqueueSora2ImageToVideoRequest { image_url: TREX_SKELETON_IMAGE_URL.to_string(), prompt: "the t-rex skeleton gets off the podium and begins walking to the camera. the camera orbits slightly. The t-rex gets close and then bites.".to_string(), duration: Some(EnqueueSora2ImageToVideoDurationSeconds::Twelve), aspect_ratio: Some(EnqueueSora2ImageToVideoAspectRatio::SixteenByNine), resolution: Some(EnqueueSora2ImageToVideoResolution::SevenTwentyP) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
  ...
  async fn test_all_aspect_ratios() -> AnyhowResult<()> {
    ...
    for ar in EnqueueSora2ImageToVideoAspectRatio::iter() {
      ...
      let args = EnqueueSora2ImageToVideoArgs { request: EnqueueSora2ImageToVideoRequest { image_url: TREX_SKELETON_IMAGE_URL.to_string(), prompt: "the skeleton comes alive and roars at the camera".to_string(), duration: Some(EnqueueSora2ImageToVideoDurationSeconds::Four), aspect_ratio: Some(ar), resolution: None }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
      let result = enqueue_sora_2_image_to_video_webhook(args).await?;
      println!("result: {:?}", result);
    }
  ...
  async fn test_all_durations() -> AnyhowResult<()> {
    ...
    for dur in EnqueueSora2ImageToVideoDurationSeconds::iter() {
      ...
      let args = EnqueueSora2ImageToVideoArgs { request: EnqueueSora2ImageToVideoRequest { image_url: TREX_SKELETON_IMAGE_URL.to_string(), prompt: "the skeleton slowly turns its head".to_string(), duration: Some(dur), aspect_ratio: Some(EnqueueSora2ImageToVideoAspectRatio::SixteenByNine), resolution: None }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
      let result = enqueue_sora_2_image_to_video_webhook(args).await?;
      println!("result: {:?}", result);
    }
  ...
  async fn test_all_resolutions() -> AnyhowResult<()> {
    ...
    for res in EnqueueSora2ImageToVideoResolution::iter() {
      ...
      let args = EnqueueSora2ImageToVideoArgs { request: EnqueueSora2ImageToVideoRequest { image_url: TREX_SKELETON_IMAGE_URL.to_string(), prompt: "the skeleton slowly comes alive".to_string(), duration: Some(EnqueueSora2ImageToVideoDurationSeconds::Four), aspect_ratio: Some(EnqueueSora2ImageToVideoAspectRatio::SixteenByNine), resolution: Some(res) }, api_key: &api_key, webhook_url: "https://example.com/webhook" };
      let result = enqueue_sora_2_image_to_video_webhook(args).await?;
      println!("result: {:?}", result);
    }
