...
static SCRIPT_SELECTOR: Lazy<Selector> = Lazy::new(|| Selector::parse("script[src]").expect("HTML selector should parse"));

pub fn parse_index_svg_paths(html: &str) -> Vec<String> {
  let document = Html::parse_document(html);
  let selected = document.select(&SCRIPT_SELECTOR);
  selected.filter_map(|node| node.attr("src")).filter(|src| src.starts_with("/_next/static/chunks/")).map(|s| s.to_string()).collect::<Vec<_>>()
}
