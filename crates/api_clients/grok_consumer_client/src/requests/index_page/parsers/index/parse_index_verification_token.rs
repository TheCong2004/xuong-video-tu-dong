...
static META_SELECTOR: Lazy<Selector> = Lazy::new(|| Selector::parse("meta[name=grok-site-verification]").expect("HTML selector should parse"));
...
static META_TAG_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r#""grok-site-verification"\s*content="([A-Za-z0-9/=+-]{5,})""#).expect("Regex should parse"));

/// Find the verification token embedded in JSON/script payloads using regex
/// eg. {\"name\":\"grok-site-verification\",\"content\":\"iFlTOEJNfQZP1B6YMRf/zuj3eYFrKWi6eNUg5XvvpllTOw5TS82coZkeUBdiHxYr\"}]
static JSON_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r#""grok-site-verification\\?",\s*\\?"content\\?":\\?"([A-Za-z0-9/=+-]{5,})\\?""#).expect("Regex should parse"));
...
pub fn parse_index_verification_token(html: &str) -> Option<VerificationToken> {
  ...
  let maybe_meta_regex = scrape_meta_tag_via_regex(html);

  if let Some(meta) = maybe_meta_regex {
    return Some(VerificationToken(meta));
  }
...
fn scrape_meta_tag_via_regex(html: &str) -> Option<String> {
