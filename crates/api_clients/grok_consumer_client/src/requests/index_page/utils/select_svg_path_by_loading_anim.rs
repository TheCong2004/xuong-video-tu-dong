...
pub fn select_svg_path_by_loading_anim(svg_paths: &[SvgPathData], loading_anim: &LoadingAnim) -> Result<SvgPathData, GrokClientError> {
  svg_paths.get(loading_anim.0).map(|path| path.clone()).ok_or_else(|| {
