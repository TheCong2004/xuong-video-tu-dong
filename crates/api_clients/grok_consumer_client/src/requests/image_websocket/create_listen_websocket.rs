...
pub async fn create_listen_websocket(args: CreateListenWebsocketArgs<'_>) -> Result<WebSocket, GrokError> {
  let client = Client::builder().emulation(Emulation::Firefox143).connection_verbose(true).connect_timeout(Duration::from_secs(30)).build().map_err(|err| GrokClientError::WreqClientError(err))?;
  ...
  let builder = client.websocket(WEBSOCKET_URL).default_headers(false).orig_headers(get_firefox_websocket_http_1_1_headers()).header(COOKIE, args.cookies).header(ORIGIN, "https://grok.com").header(USER_AGENT, FIREFOX_143_MAC_USER_AGENT).header(SEC_WEBSOCKET_EXTENSIONS, "permessage-deflate").header(ACCEPT, "*/*").header(ACCEPT_LANGUAGE, "en-US,en;q=0.5").header("Sec-Fetch-Dest", "empty").header("Sec-Fetch-Mode", "websocket").header("Sec-Fetch-Site", "same-origin").header(PRAGMA, "no-cache").header(CACHE_CONTROL, "no-cache");
  ...
  let websocket = response.into_websocket().await.map_err(|err| GrokGenericApiError::WreqWebsocketUpgradeError(err))?;
...
mod tests {
  ...
  async fn create() -> AnyhowResult<()> {
    ...
    let args = CreateListenWebsocketArgs { cookies: &cookies };
