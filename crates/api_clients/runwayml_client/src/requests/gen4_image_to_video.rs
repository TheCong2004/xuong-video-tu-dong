...
pub async fn gen4_image_to_video(credentials: &Credentials) -> Result<(), RunwayMlError> {
  ...
  let endpoint_request = TaskRequest { task_type: "gen4".to_string(), internal: false, options: TaskRequestOptions { name: "Gen-4 3438043025".to_string(), seed: 3438043025, explore_mode: true, seconds: 5, watermark: false, text_prompt: "".to_string(), route: "i2v".to_string(), init_image: "https://d2jqrm6oza8nb6.cloudfront.net/datasets/586d77ff-b33b-4100-b4d3-c04f8b046b00.jpg?_jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJrZXlIYXNoIjoiNTJhMTY0N2RkZjFjMGU3NCIsImJ1Y2tldCI6InJ1bndheS1kYXRhc2V0cyIsInN0YWdlIjoicHJvZCIsImV4cCI6MTc0Nzc4NTYwMH0.N_TztqlU2pzPegOq7e8tn_6zb16OLmDmerlTtfr_-FI".to_string(), width: 1280, height: 720, asset_group_id: "b5c42f29-d63d-4124-88c0-019ab1299829".to_string() }, as_team_id: 23129405, session_id: "fc66957b-2651-4362-aca0-5468e4ee61b0".to_string() };
  ...
  let http_request = client.post(GENERATE_URL).header("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:137.0) Gecko/20100101 Firefox/137.0").header("Accept", "application/json").header("Accept-Encoding", "gzip, deflate, br").header("Accept-Language", "en-US,en;q=0.5").header("Content-Type", "application/json").header("Authorization", jwt.to_authorization_header_value());

  let http_request = http_request.json(&endpoint_request).build().map_err(|err| RunwayMlError::Client(ClientError::ReqwestError(err)))?;

  let response = client.execute(http_request).await.map_err(|err| RunwayMlError::Api(ApiError::ReqwestError(err)))?;
  ...
  let response_body = &response.text().await.map_err(|err| RunwayMlError::Api(ApiError::ReqwestError(err)))?;
