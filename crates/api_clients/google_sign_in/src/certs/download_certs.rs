...
pub async fn download_certs() -> Result<String, GoogleSignInError> {
  let response = reqwest::get(GOOGLE_CERTS_URL).await.map_err(GoogleSignInError::CertDownloadHttpError)?;

  if !response.status().is_success() {
    return Err(GoogleSignInError::CertDownloadBadStatus(response.status()));
  }

  let body = response.text().await.map_err(GoogleSignInError::CertDownloadHttpError)?;
