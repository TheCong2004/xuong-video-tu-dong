...
pub fn decode_jwt_header(jwt: &str) -> Result<JwtHeader, GoogleSignInError> {
  ...
  let decoded = Base64UrlSafeNoPadding::decode_to_vec(jwt_header_b64, None).map_err(|err| GoogleSignInError::JwtHeaderBase64DecodeFailed(format!("{}", err)))?;

  let jwt_header: JwtHeader = serde_json::from_slice(&decoded).map_err(GoogleSignInError::JwtHeaderJsonParseFailed)?;
