...
pub async fn list_sora2_drafts_with_session_auto_renew(credentials: &SoraCredentialSet) -> Result<(ListSora2DraftsResponse, Option<SoraCredentialSet>), SoraError> {
  let mut maybe_new_creds = maybe_renew_session_jwt(&credentials).await?;

  let use_creds = maybe_new_creds.as_ref().unwrap_or_else(|| &credentials);
...
mod tests {
  ...
  pub async fn manual_test() -> AnyhowResult<()> {
    ...
    let (result, creds) = list_sora2_drafts_with_session_auto_renew(&creds).await?;
