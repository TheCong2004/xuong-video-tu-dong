...
pub async fn generate_sora2_video_with_session_auto_renew(args: GenerateSora2VideoArgs<'_>) -> Result<(GenerateSora2VideoResponse, Option<SoraCredentialSet>), SoraError> {
  let mut maybe_new_creds = maybe_renew_session_jwt(&args.credentials).await?;

  let use_creds = maybe_new_creds.as_ref().unwrap_or_else(|| &args.credentials);
...
mod tests {
  ...
  pub async fn manual_test() -> AnyhowResult<()> {
    ...
    let args = GenerateSora2VideoArgs { prompt: "a Golden Retriever playing basketball at Venice Beach during sunset", credentials: &creds, request_timeout: None, orientation: Orientation::Landscape, image_reference_media_ids: None };

    let (result, creds) = generate_sora2_video_with_session_auto_renew(args).await?;
