...
pub async fn image_remix_with_session_auto_renew(request: ImageRemixAutoRenewRequest<'_>) -> Result<(SoraImageGenResponse, Option<SoraCredentialSet>), SoraError> {
  ...
  if refresh_jwt {
    ...
    match response {
      Err(err) => return Err(SoraGenericApiError::UncategorizedBadResponse(format!("failed to generate new JWT bearer token: {:?}", err)).into()),
  ...
  if refresh_sentinel {
    ...
    match response {
      Err(err) => return Err(SoraGenericApiError::UncategorizedBadResponse(format!("failed to generate new sentinel: {:?}", err)).into()),
  ...
  let result = sora_image_gen_remix(SoraImageGenRemixRequest { prompt: request.prompt, num_images: request.num_images, image_size: request.image_size, sora_media_tokens: request.sora_media_tokens, credentials: &new_creds, request_timeout: request.request_timeout }).await?;
