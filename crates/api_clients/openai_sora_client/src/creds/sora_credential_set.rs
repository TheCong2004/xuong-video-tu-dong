...
impl SoraCredentialSet {
  pub fn initialize(cookies: SoraCookies, bearer: Option<SoraJwtBearerToken>, sentinel: Option<SoraSentinel>, sentinel_token: Option<SoraSentinelToken>) -> Self {
    Self { cookies, jwt_bearer_token: bearer, sora_sentinel: sentinel, sora_sentinel_token: sentinel_token }
  }

  pub fn initialize_with_just_cookies(cookies: SoraCookies) -> Self {
    SoraCredentialSet { cookies, jwt_bearer_token: None, sora_sentinel: None, sora_sentinel_token: None }
  }

  pub fn initialize_with_just_cookies_str(cookies: &str) -> Self {
    Self::initialize_with_just_cookies(SoraCookies::new(cookies.to_string()))
  }

  pub fn to_builder(&self) -> SoraCredentialBuilder {
    let mut builder = SoraCredentialBuilder::new().with_cookies(self.cookies.as_str());
