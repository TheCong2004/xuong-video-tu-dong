...
pub(super) fn extract_expiry(generated_at: DateTime<Utc>, response: &SentinelResponse) -> ExtractedExpiry {
  ...
  let mut maybe_expires_at = response.expire_at.map(|timestamp| DateTime::from_timestamp(timestamp as i64, 0)).flatten();
  ...
  if let Some(expires_at) = maybe_expires_at {
    ...
    if seconds.is_positive() && seconds > LOWER_BOUNDS_SECONDS_I64 && seconds < UPPER_BOUNDS_SECONDS_I64 {
      info!("Using sentinel expire_at timestamp, expires in {} seconds (at {})", seconds, expires_at);

      return ExtractedExpiry { generated_at, expires_in_seconds: seconds as u32 };
    }
  }

  let mut expires_in_seconds = response.expire_after.unwrap_or_else(|| {
  ...
  ExtractedExpiry { generated_at, expires_in_seconds }
}
