...
pub async fn generate_sentinel_token_2() -> Result<SoraSentinelToken, SoraError> {
  ...
  let text_body = &response.text().await.map_err(|err| {
  ...
  let response = serde_json::from_str::<SentinelResponse>(&text_body).map_err(|err| {
...
fn into_sora_sentinel_token(request: SentinelRequest, response: SentinelResponse, raw_body: &str) -> Result<SoraSentinelToken, SoraSpecificApiError> {
  ...
  let maybe_turnstile = response.turnstile.map(|turn| turn.dx).flatten();
  ...
  let (token, turnstile) = {
    let maybe_items = match (maybe_token, maybe_turnstile) {
      (Some(token), Some(turnstile)) => Some((token, turnstile)),
      (None, None) => {
        missing_token = true;
    ...
    match maybe_items {
      ...
      _ => return Err(SoraSpecificApiError::SentinelResponseIsMissingFields { missing_token, missing_turnstile, raw_response: raw_body.to_string() }),
    }
  };

  let sentinel_raw = RawSoraSentinelToken { p: request.p, id: request.id, flow: request.flow, t: turnstile, c: token };

  let sentinel = SoraSentinelToken { token: sentinel_raw, browser_user_agent: CLIENT_USER_AGENT.to_string(), generated_at: expiry.generated_at, expires_in_seconds: expiry.expires_in_seconds };
