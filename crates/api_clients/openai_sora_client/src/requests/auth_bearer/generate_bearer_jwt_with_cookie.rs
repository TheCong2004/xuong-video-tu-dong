...
pub async fn generate_bearer_jwt_with_cookie(cookie: &str) -> Result<String, SoraError> {
  ...
  let response = client.get(SORA_BEARER_GENERATE_URL).header("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:137.0) Gecko/20100101 Firefox/137.0").header("Accept", "*/*").header("Accept-Encoding", "gzip, deflate, br").header("Accept-Language", "en-US,en;q=0.5").header("Cookie", cookie).send().await.map_err(|err| {
  ...
  let auth_response: SoraAuthResponse = serde_json::from_str(&response_body).map_err(|err| {
