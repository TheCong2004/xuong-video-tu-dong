...
pub async fn generate_sentinel_token() -> Result<String, SoraError> {
  ...
  let response = client.post(SORA_IMAGE_GEN_URL).header("User-Agent", CLIENT_USER_AGENT).header("Content-Type", "application/json").json(&request).send().await.map_err(|err| {
