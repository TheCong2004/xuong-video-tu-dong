...
pub async fn list_media(credentials: &SoraCredentialSet) -> Result<ListMediaResponse, SoraError> {
  let auth_header = credentials.jwt_bearer_token.as_ref().map(|bearer| bearer.to_authorization_header_value()).ok_or_else(|| SoraClientError::NoBearerTokenForRequest)?;
  ...
  let response = client.get(SORA_MEDIA_LIST_URL).header("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:137.0) Gecko/20100101 Firefox/137.0").header("Accept", "*/*").header("Accept-Encoding", "gzip, deflate, br").header("Accept-Language", "en-US,en;q=0.5").header("Cookie", &cookie).header("Authorization", &auth_header).send().await.map_err(|err| {
  ...
  let response = serde_json::from_str::<ListMediaResponse>(text_body).map_err(|err| {
