...
pub(crate) async fn upload_media_http_request(file_bytes: Vec<u8>, filename: String, mime_type: &str, credentials: &SoraCredentialSet, maybe_timeout: Option<Duration>) -> Result<SoraMediaUploadResponse, SoraError> {
  ...
  let auth_header = credentials.jwt_bearer_token.as_ref().ok_or(SoraClientError::NoBearerTokenForRequest)?.to_authorization_header_value();

  // Make API request
  let client = Client::new();
  let mut request_builder = client.post(SORA_UPLOAD_MEDIA_URL).multipart(form).header(ACCEPT, "*/*").header(ACCEPT_LANGUAGE, "en-US,en;q=0.9").header(AUTHORIZATION, &auth_header).header(COOKIE, &cookie).header(ORIGIN, "https://sora.chatgpt.com").header("user-agent", CLIENT_USER_AGENT).header(REFERER, "https://sora.chatgpt.com/explore").header("priority", "u=1, i").header("sec-ch-ua", "\"Chromium\";v=\"140\", \"Not=A?Brand\";v=\"24\", \"Google Chrome\";v=\"140\"").header("sec-ch-ua-mobile", "?0").header("sec-ch-ua-platform", "macOS").header("sec-fetch-dest", "empty").header("sec-fetch-mode", "cors").header("sec-fetch-site", "same-origin");

  if let Some(timeout) = maybe_timeout {
    request_builder = request_builder.timeout(timeout);
  }

  let response = request_builder.send().await.map_err(|err| SoraGenericApiError::WreqError(err))?;
  ...
  let upload_response: SoraMediaUploadResponse = serde_json::from_str(&response_body).map_err(|err| {
