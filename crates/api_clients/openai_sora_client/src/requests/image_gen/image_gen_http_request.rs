...
pub(crate) async fn image_gen_http_request(sora_request: RawSoraImageGenRequest, credentials: &SoraCredentialSet, request_timeout: Option<Duration>) -> Result<RawSoraResponse, SoraError> {
  ...
  let sentinel = credentials.sora_sentinel_token.as_ref().map(|sentinel| sentinel.to_request_header_json()).transpose()?.ok_or(SoraClientError::NoSentinelTokenForRequest)?;
  ...
  let mut http_request = client.post(SORA_IMAGE_GEN_URL).header("User-Agent", CLIENT_USER_AGENT).header("Cookie", &cookie).header("Authorization", &authorization_header).header("Content-Type", "application/json").header("OpenAI-Sentinel-Token", &sentinel);

  if let Some(timeout) = request_timeout {
    http_request = http_request.timeout(timeout);
  }

  let http_request = http_request.json(&sora_request).build().map_err(|err| {
  ...
  if status != StatusCode::OK {
    ...
    let error_response: RawSoraErrorResponse = serde_json::from_str(response_body).map_err(|err| SoraGenericApiError::SerdeParseErrorWithBodyOnNon200(err, response_body.to_string()))?;
    ...
    return Err(SoraGenericApiError::UncategorizedBadResponse(format!("Unknown error code: {:?}, message: {:?}", error_response.error.code, error_response.error.message)).into());
  }

  let response = serde_json::from_str(response_body).map_err(|err| SoraGenericApiError::SerdeResponseParseErrorWithBody(err, response_body.to_string()))?;
