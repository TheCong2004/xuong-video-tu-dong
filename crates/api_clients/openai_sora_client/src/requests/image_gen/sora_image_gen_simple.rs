...
pub async fn sora_image_gen_simple(args: SoraImageGenSimpleRequest<'_>) -> Result<SoraImageGenResponse, SoraError> {
  let sora_request = RawSoraImageGenRequest { r#type: VideoGenType::ImageGen, operation: OperationType::SimpleCompose, prompt: args.prompt, n_variants: args.num_images.as_count(), width: args.image_size.as_width(), height: args.image_size.as_height(), n_frames: 1, inpaint_items: vec![] };
...
mod tests {
  ...
  pub async fn manual_test() -> AnyhowResult<()> {
    ...
    let response = sora_image_gen_simple(SoraImageGenSimpleRequest { prompt: "A pirate ship catches on fire as an alien ufo attacks it, photorealistic".to_string(), num_images: NumImages::One, image_size: ImageSize::Square, credentials: &creds, request_timeout: None }).await?;
