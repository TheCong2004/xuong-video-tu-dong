...
pub async fn main() -> AnyhowResult<()> {
  ...
  match mimetype {
    Some("audio/wav") | Some("audio/x-wav") => {
  ...
  let public_upload_path = MediaFileBucketPath::generate_new(maybe_public_bucket_prefix, maybe_public_bucket_extension);

  info!("Uploading media to bucket path: {}", public_upload_path.get_full_object_path_str());

  let _r = public_bucket_client.upload_file_with_content_type(public_upload_path.get_full_object_path_str(), bytes.as_ref(), mimetype).await?;
  ...
  let bucket_url = format!("https://storage.cloud.google.com/{}{}", public_bucket_name, public_upload_path.get_full_object_path_str());

  let gcp_url = format!("https://console.cloud.google.com/storage/browser/_details/{}{}", public_bucket_name, public_upload_path.get_full_object_path_str());
