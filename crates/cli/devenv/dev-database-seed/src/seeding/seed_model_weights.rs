...
pub async fn seed_weights_files(mysql_pool: &Pool<MySql>, maybe_bucket_clients: Option<&BucketClients>) -> AnyhowResult<()> {
  ...
  let seeded_weights = [(HANASHI_USER_TOKEN, "m_21jxt406aattq2cna1xtgf5m4mjyy1", WeightsType::LoRA, WeightsCategory::ImageGeneration, "Niji Mecha", "models/imagegen/loRA/nijiMecha.safetensors"), (HANASHI_USER_TOKEN, "m_cxzabpxf88h10rshqraksxbqmhs412", WeightsType::StableDiffusion15, WeightsCategory::ImageGeneration, "Niji Diffused Mix", "models/imagegen/sd15/nijidiffusedmix_v40.safetensors"), (HANASHI_USER_TOKEN, "m_21jxt406aattq2cna1xtgf5m4mjyy9", WeightsType::LoRA, WeightsCategory::ImageGeneration, "Niji Mecha", "models/imagegen/loRA/nijiMecha.safetensors"), (HANASHI_USER_TOKEN, "m_cxzabpxf88h10rshqraksxbqmhs413", WeightsType::StableDiffusion15, WeightsCategory::ImageGeneration, "Niji Diffused Mix", "models/imagegen/sd15/nijidiffusedmix_v40.safetensors")];

  let seed_tool_data_root = get_seed_tool_data_root();

  for (user_token, _weights_file_token, weights_type, weights_category, model_name, subdirectory_path) in seeded_weights {
    ...
    let bucket_details = seed_model(&mysql_pool, &model_weight_token, &user_token, weights_type, &weight_file_path, maybe_bucket_clients).await;
...
async fn seed_model(_mysql_pool: &Pool<MySql>, _model_weight_token: &ModelWeightToken, _user_token: &UserToken, weight_type: WeightsType, weight_file_path: &Path, maybe_bucket_clients: Option<&BucketClients>) -> AnyhowResult<BucketDetails> {
...
async fn seed_file_to_bucket(weight_file_path: &Path, bucket_clients: &BucketClients, weight_type: WeightsType) -> AnyhowResult<BucketDetails> {
  ...
  let maybe_bucket_extension = weight_file_path.extension().map(|extension| extension.to_str()).flatten();
  ...
  let bucket_location: WeightFileBucketPath = WeightFileBucketPath::generate_new(maybe_bucket_prefix, maybe_bucket_extension);
