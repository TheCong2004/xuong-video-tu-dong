...
pub async fn seed_tts_tacotron2(mysql_pool: &Pool<MySql>, maybe_bucket_clients: Option<&BucketClients>) -> AnyhowResult<()> {
  ...
  let tts_models = [("Mai Valentine from Yugioh (by Vegito)", HANASHI_USER_TOKEN, "TM:cknxtj6mx3t3", "models/tts/tacotron2/MaiYugioh.pth"), ("Nagito (by Vegito)", HANASHI_USER_TOKEN, "TM:tmhxapghwaf3", "models/tts/tacotron2/Nagito.pth"), ("Son Goku V2 (by Vegito, Arpabet version)", HANASHI_USER_TOKEN, "TM:kbvaa4726jdw", "models/tts/tacotron2/SonGokuV2.pth"), ("Vegeta (by Vegito, Arpabet version)", HANASHI_USER_TOKEN, "TM:kvv9xqashp6n", "models/tts/tacotron2/VegetaNew.pth")];
  ...
  for (model_title, user_token, tts_model_token, subdirectory_path) in tts_models {
    ...
    let result = seed_model(&mysql_pool, &tts_model_token, model_title, &user_token, &model_file_path, maybe_bucket_clients, &bucket_path_unifier).await;
...
async fn seed_model(mysql_pool: &Pool<MySql>, tts_model_token: &TtsModelToken, model_title: &str, user_token: &UserToken, model_file_path: &Path, maybe_bucket_clients: Option<&BucketClients>, bucket_path_unifier: &BucketPathUnifier) -> AnyhowResult<()> {
  ...
  let filename = model_file_path.file_name().map(|name| name.to_str()).flatten().unwrap_or("");

  insert_tts_model_from_download_job(InsertTtsModelFromDownloadJobArgs { tts_model_type: TtsModelType::Tacotron2, title: model_title, original_download_url: "https://example.com", original_filename: filename, file_size_bytes: 0, creator_user_token: user_token, creator_ip_address: "127.0.0.1", creator_set_visibility: Visibility::Public, private_bucket_hash: &private_bucket_hash, private_bucket_object_name: &private_bucket_object_name, maybe_model_token: Some(&tts_model_token), mysql_pool }).await?;

  Ok(())
}

async fn tts_model_exists(mysql_pool: &Pool<MySql>, tts_model_token: &TtsModelToken) -> AnyhowResult<bool> {
  const CAN_SEE_DELETED: bool = true;

  let maybe_model = get_tts_model_by_token(tts_model_token.as_str(), CAN_SEE_DELETED, mysql_pool).await?;
...
async fn seed_file_to_bucket(model_file_path: &Path, bucket_clients: &BucketClients, bucket_path_unifier: &BucketPathUnifier) -> AnyhowResult<BucketDetails> {
  ...
  let _r = bucket_clients.private.upload_file_with_content_type(&model_bucket_path, bytes.as_ref(), mimetype).await?;

  Ok(BucketDetails { bucket_hash: private_bucket_hash, bucket_object_name: model_bucket_path })
}
