...
use seedance2pro_client::generate::video::generate_happy_horse_1p0::{generate_happy_horse_1p0, GenerateHappyHorse1p0Args, GenerateHappyHorse1p0Request};
use seedance2pro_client::generate::video::generate_seedance_2p0::{generate_seedance_2p0, GenerateSeedance2p0Args, GenerateSeedance2p0Request};
use seedance2pro_client::generate::video::generate_seedance_2p0_fast::{generate_seedance_2p0_fast, GenerateSeedance2p0FastArgs, GenerateSeedance2p0FastRequest};
...
pub async fn run(state: &Seedance2ProState, args: GenerateVideoArgs) -> anyhow::Result<()> {
  ...
  let media_inputs = ResolvedMediaInputs { start_frame_url, end_frame_url, reference_image_urls: if reference_image_urls.is_empty() { None } else { Some(reference_image_urls) }, reference_video_urls: if reference_video_urls.is_empty() { None } else { Some(reference_video_urls) }, reference_audio_urls: if reference_audio_urls.is_empty() { None } else { Some(reference_audio_urls) } };
...
fn resolve_download_dir(maybe_path: Option<&str>) -> anyhow::Result<PathBuf> {
  ...
  if !dir.exists() {
    ...
    fs::create_dir_all(&dir).map_err(|err| anyhow!("Failed to create download directory {:?}: {}", dir, err))?;
  }
...
async fn download_media_files(tokens: &[MediaFileToken], api_host: &ApiHost, download_dir: &Path) -> anyhow::Result<Vec<DownloadedFile>> {
  ...
  for token in tokens {
    ...
    if let Some(cached) = maybe_cached {
      info!("Cache hit: {} already exists at {:?} ({} bytes)", token.as_str(), cached.path, fs::metadata(&cached.path).map(|m| m.len()).unwrap_or(0),);
      results.push(cached);
      continue;
    }
    ...
    let result = download_media_file(DownloadMediaFileArgs { media_token: token, api_host, download_path: DownloadPath::Directory(download_dir) }).await.map_err(|err| anyhow!("Failed to download {}: {}", token.as_str(), err))?;

    let extension = result.downloaded_file_path.extension().and_then(|e| e.to_str()).unwrap_or("bin").to_string();

    info!("Downloaded: {} -> {:?} ({} bytes, class: {})", token.as_str(), result.downloaded_file_path, result.filesize_bytes, result.media_file_response.media_file.media_class.to_str(),);

    results.push(DownloadedFile { path: result.downloaded_file_path, extension });
  }
...
async fn upload_media_files_to_kinovi(session: &Seedance2ProSession, files: &[DownloadedFile]) -> anyhow::Result<Vec<String>> {
  ...
  for file in files {
    ...
    let prepare_result = prepare_file_upload(PrepareFileUploadArgs { session, extension: file.extension.clone(), host_override: None }).await.map_err(|err| anyhow!("Failed to prepare upload for {:?}: {:?}", file.path, err))?;

    // 2. Read file bytes.
    let file_bytes = fs::read(&file.path).map_err(|err| anyhow!("Failed to read {:?}: {}", file.path, err))?;

    info!("Uploading {} bytes to signed URL...", file_bytes.len());

    // 3. Upload to Cloudflare R2.
    let upload_result = upload_file(UploadFileArgs { upload_url: prepare_result.upload_url, file_bytes, host_override: None }).await.map_err(|err| anyhow!("Failed to upload {:?}: {:?}", file.path, err))?;
...
async fn generate(model: VideoModel, session: &Seedance2ProSession, prompt: &str, media: ResolvedMediaInputs) -> anyhow::Result<()> {
  match model {
    VideoModel::HappyHorse => {
      ...
      let result = generate_happy_horse_1p0(GenerateHappyHorse1p0Args { request: GenerateHappyHorse1p0Request { prompt: prompt.to_string(), aspect_ratio: None, output_resolution: None, batch_count: None, duration_seconds: 5, start_frame_url: media.start_frame_url }, session, host_override: None }).await.map_err(|err| anyhow!("Happy Horse generation failed: {:?}", err))?;
    ...
    VideoModel::Seedance2p0 => {
      ...
      let result = generate_seedance_2p0(GenerateSeedance2p0Args { request: GenerateSeedance2p0Request { prompt: prompt.to_string(), aspect_ratio: None, output_resolution: None, duration_seconds: 5, batch_count: None, start_frame_url: media.start_frame_url, end_frame_url: media.end_frame_url, reference_image_urls: media.reference_image_urls, reference_video_urls: media.reference_video_urls, reference_audio_urls: media.reference_audio_urls, character_ids: None, use_face_blur_hack: None, bitrate: None }, session, host_override: None }).await.map_err(|err| anyhow!("Seedance 2.0 generation failed: {:?}", err))?;
    ...
    VideoModel::Seedance2p0Fast => {
      ...
      let result = generate_seedance_2p0_fast(GenerateSeedance2p0FastArgs { request: GenerateSeedance2p0FastRequest { prompt: prompt.to_string(), aspect_ratio: None, output_resolution: None, duration_seconds: 5, batch_count: None, start_frame_url: media.start_frame_url, end_frame_url: media.end_frame_url, reference_image_urls: media.reference_image_urls, reference_video_urls: media.reference_video_urls, reference_audio_urls: media.reference_audio_urls, character_ids: None, use_face_blur_hack: None, bitrate: None }, session, host_override: None }).await.map_err(|err| anyhow!("Seedance 2.0 Fast generation failed: {:?}", err))?;
