...
pub async fn main() -> AnyhowResult<()> {
  ...
  let deps = Deps { mysql_development: get_mysql("MYSQL_DEVELOPMENT_URL").await?, mysql_production: get_mysql("MYSQL_PRODUCTION_URL").await?, bucket_development_public: get_bucket_client("DEVELOPMENT_PUBLIC")?, bucket_development_private: get_bucket_client("DEVELOPMENT_PRIVATE")?, bucket_production_public: get_bucket_client("PRODUCTION_PUBLIC")?, bucket_production_private: get_bucket_client("PRODUCTION_PRIVATE")? };
...
pub fn get_bucket_client(env_var_suffix: &str) -> AnyhowResult<LegacyBucketClient> {
  ...
  let s3_compatible_endpoint_url = easyenv::get_env_string_or_default(&format!("S3_COMPATIBLE_ENDPOINT_URL_{env_var_suffix}"), "https://storage.googleapis.com");
  ...
  Ok(LegacyBucketClient::create(&access_key, &secret_key, &region_name, &bucket_name, &s3_compatible_endpoint_url, None, Some(bucket_timeout))?)
}
