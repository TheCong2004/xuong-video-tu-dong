...
pub async fn migrate_voice_conversion_to_weights(deps: &Deps) -> AnyhowResult<()> {
  ...
  loop {
    ...
    let results = list_whole_voice_conversion_models_using_cursor(&deps.mysql_production, PAGE_SIZE, cursor).await?;
...
async fn lookup_bucket_details_from_record(model_weight_token: &ModelWeightToken, deps: &Deps) -> AnyhowResult<Option<CopiedFileData>> {
  ...
  Ok(Some(CopiedFileData { bucket_path, file_sha_hash: model_weight_record.file_checksum_sha2 }))
}
