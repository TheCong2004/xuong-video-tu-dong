...
pub struct SubArgs {
  /// Token to search for
  /// This could be a modern model_weights token or a legacy tts_models token
...
pub fn parse_cli_sub_args() -> AnyhowResult<SubArgs> {
  ...
  Ok(SubArgs { model_token: args.model_token, start_date: args.start_date.map(|date| NaiveDate::parse_from_str(&date, "%Y-%m-%d")).transpose()?, end_date: args.end_date.map(|date| NaiveDate::parse_from_str(&date, "%Y-%m-%d")).transpose()? })
}
