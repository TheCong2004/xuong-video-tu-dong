...
def xhs_profile_dir_name() -> str:
    """Keep the proven RedNote profile while isolating it from XHS Mainland."""
    base_name = config.USER_DATA_DIR % config.PLATFORM
    return base_name if config.XHS_INTERNATIONAL else f"cdp_{base_name}"
...
class CDPBrowserManager:
    ...
    def __init__(self):
        ...
        self._using_existing_browser = False
        ...
        try:
            if config.CDP_CONNECT_EXISTING:
                ...
                if await self._test_cdp_connection(self.debug_port):
                    self._using_existing_browser = True
                    return await self._connect_existing_browser(
                        playwright, playwright_proxy, user_agent
                    )
                if config.SESSION_ACTION != "login":
                    # Verify/crawl must not unexpectedly open a visible browser or
                    # spend the whole operation timeout polling a missing port.
                    action = "session verify" if config.SESSION_ACTION == "verify" else "crawl"
                    raise RuntimeError(
                        f"CDP port {self.debug_port} is unavailable for {action}"
                    )
                utils.logger.info(
                    "[CDPBrowserManager] No existing CDP browser; launching visible browser for interactive login"
                )
            ...
            await self._launch_browser(
                browser_path,
                False if config.SESSION_ACTION == "login" else headless,
            )
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        if config.SAVE_LOGIN_STATE:
            profile_name = (
                xhs_profile_dir_name()
                if config.PLATFORM == "xhs"
                else f"cdp_{config.USER_DATA_DIR % config.PLATFORM}"
            )
    ...
    async def _connect_via_cdp(self, playwright: Playwright):
        ...
        """
        try:
            if self._using_existing_browser:
