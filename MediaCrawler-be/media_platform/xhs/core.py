...
from tools.cdp_browser import CDPBrowserManager, xhs_profile_dir_name
...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            session_valid = await self.xhs_client.pong()
            if config.SESSION_ACTION == "verify":
                if not session_valid:
                    raise RuntimeError("MEDIACRAWLER_SESSION_INVALID")
                print("FLOWORD_SESSION_VALID")
                return
            if not session_valid:
            ...
            if config.SESSION_ACTION == "login":
                if not await self.xhs_client.pong():
                    raise RuntimeError("MEDIACRAWLER_LOGIN_FAILED")
                print("FLOWORD_SESSION_VALID")
                return
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            ...
            user_data_dir = os.path.join(
                os.getcwd(), "browser_data", xhs_profile_dir_name()
            )
