...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def login_by_qrcode(self):
        ...
        login_timeout_seconds = max(30, int(os.environ.get("RESEARCH_LOGIN_TIMEOUT_SECONDS", "180")))
        utils.logger.info(f"[XiaoHongShuLogin.login_by_qrcode] waiting for scan code login, remaining time is {login_timeout_seconds}s")
        try:
            await asyncio.wait_for(self.check_login_state(no_logged_in_session), timeout=login_timeout_seconds)
        except asyncio.TimeoutError:
            utils.logger.info("MEDIACRAWLER_LOGIN_TIMEOUT: interactive xiaohongshu login timed out")
            sys.exit()
        except RetryError:
            utils.logger.info("MEDIACRAWLER_LOGIN_FAILED: Login xiaohongshu failed by qrcode login method")
            sys.exit()
