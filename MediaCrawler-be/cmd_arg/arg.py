...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        session_action: Annotated[
            ...
            typer.Option(
                "--session_action",
                help="Floword session lifecycle action (login | verify)",
                rich_help_panel="Account Configuration",
            ),
        ] = config.SESSION_ACTION,
    ) -> SimpleNamespace:
        """MediaCrawler 命令行入口"""
        ...
        if session_action not in ("", "login", "verify"):
            raise typer.BadParameter("session_action must be login or verify")
        config.SESSION_ACTION = session_action
        ...
        return SimpleNamespace(
            ...
            session_action=config.SESSION_ACTION,
        )
