...
export function UserReferrals() {
  ...
  usePageTitle(username ? `@${username} — Referrals` : "Referrals");
  ...
  const loadData = async (cursor?: string | null, append = false) => {
    ...
    try {
      ...
      const resp = await modApi.ListUserReferralsForUser(username, cursor);
  ...
  }, [nextCursor, isLoadingMore, username]);
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 bg-muted/20 border border-dashed rounded-xl">
              ...
              <p className="text-muted-foreground">
                No signups have been attributed to this user yet.
              </p>
            </div>
          ) : (
            <Table
            ...
            >
              ...
              <TableBody>
                {referrals.map((ref, i) => {
                  const urlTitle =
                    ref.maybe_landing_url || ref.maybe_referral_url || undefined;
                  return (
                    <TableRow
                    ...
                    >
                      ...
                      <TableCell className="text-sm">
                        <Link
                          to={`/user/profile/${ref.invited_user.username}`}
                          className="hover:underline text-foreground font-medium"
                        >
                          @{ref.invited_user.username}
                        </Link>
                        {ref.invited_user.display_name && (
                      ...
                      <TableCell className="text-sm text-muted-foreground">
                        {ref.invited_user.email_address || (
                      ...
                      <TableCell className="text-xs">
                        ...
                        ) : ref.maybe_landing_url || ref.maybe_referral_url ? (
                          <Badge
                            variant="outline"
                      ...
                      <TableCell className="text-xs font-mono text-muted-foreground whitespace-nowrap">
                        {ref.maybe_landing_url ? (
                          <a
                            href={ref.maybe_landing_url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="hover:underline hover:text-foreground"
                            title={ref.maybe_landing_url}
                          >
                            {truncateUrl(ref.maybe_landing_url)}
                          </a>
                        ) : (
                          <span className="text-muted-foreground/30">
                            &mdash;
                      ...
                      <TableCell className="text-xs font-mono text-muted-foreground whitespace-nowrap">
                        {ref.maybe_referral_url ? (
                          <a
                            href={ref.maybe_referral_url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="hover:underline hover:text-foreground"
                            title={ref.maybe_referral_url}
                          >
                            {truncateUrl(ref.maybe_referral_url)}
                          </a>
                        ) : (
                          <span className="text-muted-foreground/30">
                            &mdash;
