...
const configuredApiUrl = import.meta.env.VITE_MEDIACRAWLER_API_URL?.trim()
export const MEDIA_CRAWLER_API_BASE =
  (configuredApiUrl || 'http://127.0.0.1:30000/api').replace(/\/+$/, '')
...
const api = axios.create({
  baseURL: MEDIA_CRAWLER_API_BASE,
  timeout: 5000,
  headers: {
...
export const dataApi = {
  ...
  getDownloadUrl: (path: string) =>
    `${MEDIA_CRAWLER_API_BASE}/data/download/${encodeFilePath(path)}`,
}
