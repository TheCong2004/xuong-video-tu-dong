...
const contentSecurityPolicy = [
  ...
  "base-uri 'self'",
  "object-src 'none'",
  // Tauri v2's Windows WebView2 custom-protocol origin is
  // `http://tauri.localhost`. Keep the legacy custom-scheme origin as well so
  // both desktop protocol implementations can embed the local dashboard.
  "frame-ancestors 'self' http://localhost:* http://127.0.0.1:* http://tauri.localhost https://tauri.localhost tauri://localhost",
  "form-action 'self'",
  scriptSrc,
...
const nextConfig = {
  ...
  turbopack: {
    root: process.env.TURBOPACK_ROOT || "D:/capcutpolot/artcraft",
    resolveAlias: {
  ...
  output: process.env.OMNIROUTE_STATIC_EXPORT === "1" ? "export" : "standalone",
  compress: true,
  ...
  async headers() {
    if (process.env.OMNIROUTE_STATIC_EXPORT === "1") {
      return [];
    }
    return [
      {
