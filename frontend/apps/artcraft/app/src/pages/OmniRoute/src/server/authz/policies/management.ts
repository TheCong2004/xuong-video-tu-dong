...
export const managementPolicy: RoutePolicy = {
  ...
  async evaluate(ctx: PolicyContext): Promise<AuthOutcome> {
    ...
    // OmniRoute ships as an embedded loopback app with no login/logout, so
    // there is no dashboard session to obtain. The anonymous allow therefore
    // covers every management path — including the former Tier 2
    // always-protected set, which would otherwise be permanently 403 now that
    // no credential can ever be minted.
