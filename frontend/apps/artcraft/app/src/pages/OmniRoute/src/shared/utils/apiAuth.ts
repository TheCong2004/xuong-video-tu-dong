 ...
 *
 * The dashboard login/logout flow was removed: OmniRoute now runs as a small
 * app embedded in ArtCraft and bound to loopback only, so there is no session
 * to establish and nothing to gate. This always returns false.
 *
 * Kept as a function (instead of deleting its ~30 call sites) so every caller
 * of the `if (await isAuthRequired(req))` shape keeps compiling; each one now
 * simply takes its no-auth branch.
 */
