...
// The password login route was removed (OmniRoute is an embedded local app);
// this OIDC callback is the only remaining place that mints an auth_token.
...
export async function GET(request: Request) {
  ...
  if (!code || !returnedState) {
    return NextResponse.redirect(new URL("/dashboard?oidc_error=missing_code", reqUrlEarly));
  }
  ...
  if (!storedState || storedState !== returnedState) {
    return NextResponse.redirect(new URL("/dashboard?oidc_error=invalid_state", reqUrlEarly));
  }
  ...
  if (!enabled || !issuer || !clientId || !clientSecret) {
    return NextResponse.redirect(new URL("/dashboard?oidc_error=not_configured", reqUrlEarly));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/dashboard?oidc_error=token_exchange", reqUrlEarly));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/dashboard?oidc_error=token_response", reqUrlEarly));
  }
  ...
  if (!idToken) {
    return NextResponse.redirect(new URL("/dashboard?oidc_error=no_id_token", reqUrlEarly));
  }
    ...
    if (allowed.length > 0) {
      ...
      if (!ok) {
        return NextResponse.redirect(
          new URL("/dashboard?oidc_error=subject_not_allowed", reqUrlEarly)
        );
      }
    }
  } catch {
    return NextResponse.redirect(new URL("/dashboard?oidc_error=id_token_invalid", reqUrlEarly));
  }
  ...
  if (!process.env.JWT_SECRET) {
    return NextResponse.redirect(
      new URL("/dashboard?oidc_error=server_misconfigured", reqUrlEarly)
    );
  }
