...
async function authorizeConnection(request: import("http").IncomingMessage): Promise<WsAuthResult> {
  ...
  if (!token) {
    ...
    if (isAnonymousLiveWsAllowed(authRequired, request.socket.remoteAddress)) {
...
/**
 * The embedded ArtCraft dashboard intentionally runs without a login flow.
 * Keep that no-auth mode usable for LiveWS, but only for a peer whose actual
 * socket address is loopback. This prevents a LAN bind or forwarded Origin
 * from turning auth-disabled desktop mode into anonymous remote access.
 */
export function isAnonymousLiveWsAllowed(
