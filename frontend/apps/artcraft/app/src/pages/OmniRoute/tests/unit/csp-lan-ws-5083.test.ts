...
test("#5083 baseline security directives remain intact in the CSP", () => {
  // The dashboard is embedded by the ArtCraft Tauri WebView, whose Windows
  // production origin is http://tauri.localhost. The legacy tauri: origin is
  // retained for other protocol implementations.
  assert.ok(
    nextConfig.includes("http://tauri.localhost") && nextConfig.includes("tauri://localhost"),
    "frame-ancestors must allow ArtCraft's production and legacy Tauri origins"
  );
