...
test("Windows build profile stays inside the project build directory, not the system drive temp", () => {
  const project = path.resolve("D:/workspace/omniroute");
  const profile = getWindowsBuildProfileDir(project);

  assert.equal(path.relative(project, profile).startsWith(".."), false);
  assert.match(profile.replaceAll("\\", "/"), /\/\.build\/tmp\/omniroute-build-winhome-/);
  assert.equal(profile.startsWith(os.tmpdir()), false);
});
