...
import { EmptyStateIllustration } from '@/pages/PageYouwee/components/shared/EmptyStateIllustration';
import { Button } from '@/pages/PageYouwee/components/ui/button';
import { Input } from '@/pages/PageYouwee/components/ui/input';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/pages/PageYouwee/components/ui/select';
import { SimpleMarkdown } from '@/pages/PageYouwee/components/ui/simple-markdown';
import { useAI } from '@/pages/PageYouwee/contexts/AIContext';
import { useSummarySession } from '@/pages/PageYouwee/contexts/summary-session-context';
import { localizeUnknownError } from '@/pages/PageYouwee/lib/backend-error';
import {
...
} from '@/pages/PageYouwee/lib/summary-font-size';
import {
  DEFAULT_LONG_SUMMARY_WORDS,
  MAX_LONG_SUMMARY_WORDS,
  MIN_LONG_SUMMARY_WORDS,
  normalizeLongSummaryWords,
} from '@/pages/PageYouwee/lib/summary-session';
import { LANGUAGE_OPTIONS, type LongSummaryFormat, type SummaryStyle } from '@/pages/PageYouwee/lib/types';
import { cn } from '@/pages/PageYouwee/lib/utils';
...
function SummaryLoadingState({ loadingText }: { loadingText: string }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
      ...
      <div className="w-full max-w-sm rounded-md border border-primary/15 bg-primary/5 p-3 text-left shadow-none">
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-border px-4 sm:px-5">
        <div className="flex min-w-0 items-center gap-3"><div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-primary/30 bg-primary/10 text-primary"><Sparkles className="h-3.5 w-3.5" /></div><h1 className="truncate text-sm font-semibold tracking-tight">{t('summary.title')}</h1></div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-auto p-4 sm:p-5">
        ...
        {/* Settings Panel */}
        {showSettings && (
          <div className="p-4 rounded-md border border-border bg-panel space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {transcriptLanguages.map((code, index) => {
                  ...
                  return (
                    <div
                      key={code}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-background border border-border text-sm"
                    >
        ...
        {result && (
          <div className="flex-1 flex flex-col space-y-3">
            {/* Video Info */}
            <div className="flex items-start gap-3 p-3 rounded-md border border-border bg-panel">
            ...
            {/* Summary */}
            <div className="flex-1 flex flex-col p-4 rounded-md border border-border bg-card">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
            ...
            {missingSummaryConfig && onNavigateToSettings && (
              <div className="mt-6 max-w-sm w-full">
                <div className={cn('relative overflow-hidden rounded-md p-4', 'bg-primary/5')}>
