...
export function TagInput({
  ...
  return (
    <fieldset
      ...
      className={cn(
        'flex min-h-9 w-full flex-wrap items-center gap-1.5 rounded-md border border-input bg-background px-2 py-1.5 text-sm shadow-none',
        'focus-within:ring-1 focus-within:ring-ring focus-within:border-primary/50 transition-colors duration-150',
    ...
    >
      {value.map((tag) => (
        <span
          key={tag}
          className="inline-flex max-w-full items-center gap-1 rounded-sm border border-primary/20 bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary"
        >
          ...
          <button
            ...
            disabled={disabled}
            className="rounded-sm p-0.5 text-primary/70 transition-colors duration-150 hover:bg-primary/15 hover:text-primary disabled:pointer-events-none"
