...
export function ChatPanel({
  ...
  return (
    ...
    <div
      role="region"
      className="relative flex w-[min(420px,40%)] min-w-[280px] flex-col overflow-hidden border-l border-border bg-panel"
    ...
    >
      {/* Drag overlay */}
      {isDragOver && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-primary/10   border-2 border-dashed border-primary/50 rounded-lg m-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                disabled={!hasVideo}
                className={cn(
                  'h-8 max-w-[170px] px-2.5 rounded-lg border border-dashed border-border',
        ...
        <div className="p-4 space-y-3">
            ...
            messages.map((msg) => (
              ...
              >
                {msg.role === 'complete' ? (
                  ...
                  <div className="flex flex-col gap-2 p-3 rounded-md rounded-bl-sm bg-green-500/10 border border-green-500/20 animate-in fade-in slide-in-from-bottom-2 duration-200 max-w-[85%]">
                    <div className="flex items-center gap-2">
                  ...
                  <div
                    className={cn(
                      'rounded-md text-sm animate-in fade-in slide-in-from-bottom-2 duration-200',
                      'max-w-[85%]',
                      msg.role === 'user' && 'p-3 bg-primary text-primary-foreground rounded-br-sm',
                      msg.role === 'assistant' &&
                        'p-3 bg-muted/80 border border-border rounded-bl-sm',
                      msg.role === 'system' &&
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {msg.attachments.map((att) =>
                          att.kind === 'image' && att.previewUrl ? (
                            ...
                            <div
                              key={att.id}
                              className="inline-flex items-center gap-1.5 max-w-[180px] rounded-lg border border-border bg-background/70 px-2 py-1"
                            >
          ...
          {isGenerating && (
            <div className="flex justify-start">
              <div className="inline-flex items-center gap-2 text-muted-foreground text-sm p-3 bg-muted/80 border border-border rounded-md rounded-bl-sm animate-in fade-in slide-in-from-bottom-2 duration-200">
          ...
          {isProcessing && progress && (
            <div className="flex justify-start">
              <div className="flex flex-col gap-3 p-3 bg-muted/80 border border-border rounded-md rounded-bl-sm animate-in fade-in slide-in-from-bottom-2 duration-200 min-w-[200px] max-w-[85%]">
                {/* Header */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Loader2 className="w-4 h-4 animate-spin text-primary" />
        ...
        >
          ...
          <div className="relative flex items-center justify-between mt-1 px-1">
            ...
            <button
              type="button"
              className={cn(
                'relative flex-shrink-0 h-10 w-10 rounded-md flex items-center justify-center',
                'transition-all duration-300 ease-out',
                inputMessage.trim() && hasVideo && !isProcessing && !isGenerating
                  ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/25 hover:shadow-md hover:shadow-primary/30'
