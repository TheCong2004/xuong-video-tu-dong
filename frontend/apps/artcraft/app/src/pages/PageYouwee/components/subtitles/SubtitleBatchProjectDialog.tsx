...
export function SubtitleBatchProjectDialog({ open, onClose }: SubtitleBatchProjectDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50  ">
      <div className="bg-card rounded-md shadow-none border border-border w-[660px] overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <div className="rounded-md border border-border p-3 space-y-2 bg-background/70">
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-medium">{t('batchProject.sourceFiles')}</p>
              <button
                type="button"
                onClick={pickFiles}
                className="h-8 px-2.5 text-xs rounded-md border border-dashed border-border hover:bg-accent transition-colors inline-flex items-center gap-1.5"
              >
            ...
            <div className="rounded-md border border-border p-3 space-y-2 bg-background/70">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="h-9 px-2 rounded-md text-xs bg-background border border-border outline-none"
                >
                ...
                <button
                  ...
                  className="h-9 px-2 rounded-md text-xs border border-dashed border-border hover:bg-accent transition-colors"
                >
              ...
              <button
                ...
                disabled={isRunning}
                className={cn(
                  'h-9 px-3 rounded-md text-sm border border-dashed border-border transition-colors',
            ...
            <div className="rounded-md border border-border p-3 space-y-2 bg-background/70">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="h-9 w-24 px-2 rounded-md text-xs bg-background border border-border outline-none"
                />
              </div>
              <button
                type="button"
                ...
                disabled={isRunning}
                className={cn(
                  'h-9 px-3 rounded-md text-sm border border-dashed border-border transition-colors inline-flex items-center gap-1.5',
                  'hover:bg-accent disabled:opacity-50 disabled:pointer-events-none',
                )}
              >
