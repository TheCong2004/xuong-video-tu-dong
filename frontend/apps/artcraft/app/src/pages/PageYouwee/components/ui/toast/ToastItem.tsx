...
export function ToastItem({ toast, onDismiss, onPause, onResume }: ToastItemProps) {
  ...
  return (
    <div
      className={cn(
        'pointer-events-auto rounded-md border border-border bg-card p-3 shadow-none',
    ...
    >
      <div className="flex items-start gap-3">
        <div className={cn('mt-0.5 rounded-md p-2', badgeClassName)}>{icon}</div>
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {hasPermissionError ? (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-2.5 py-2 text-xs">
          ...
          {hasPermissionError && pluginRun?.details ? (
            <details className="rounded-md border border-border bg-panel px-2.5 py-2 text-xs text-muted-foreground">
          ...
          {toast.action ? (
            <button
              ...
              className="rounded-md border border-border px-2.5 py-1 text-[11px] font-medium text-foreground transition-colors duration-150 hover:bg-muted"
            >
              {toast.action.label}
            </button>
          ) : null}
