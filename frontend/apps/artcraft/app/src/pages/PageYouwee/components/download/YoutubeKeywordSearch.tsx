...
function SearchResultGridItem({
  ...
  return (
    <button
      ...
      className={cn(
        'group relative flex flex-col text-left transition-all duration-300 rounded-md focus:outline-none',
    ...
    >
      ...
      <div className="relative w-full aspect-video rounded-md overflow-hidden bg-muted">
        ...
        {selected && !isAdded && (
          <div className="absolute inset-0 rounded-md ring-2 ring-inset ring-primary bg-primary/10 z-10 pointer-events-none transition-all duration-300" />
        )}
        ...
        {!isAdded && (
          ...
          >
            {selected ? (
              <div className="w-6 h-6 rounded-md bg-primary text-primary-foreground flex items-center justify-center shadow-md ring-2 ring-background">
        ...
        {isAdded && (
          <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
            ...
            <div className="relative flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-background/90 backdrop-blur-md border border-border shadow-sm text-foreground">
    ...
    <div className="flex flex-col h-full overflow-hidden relative">
          ...
          >
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-10 pr-10 h-12 rounded-md bg-background border-border/60 focus:bg-background text-base sm:text-sm shadow-sm"
              />
              {query ? (
                <button
                  type="button"
                  onClick={clearQuery}
                  disabled={disabled || isSearching}
                  className="absolute right-2.5 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:pointer-events-none disabled:opacity-50"
            ...
            <div className="flex gap-2">
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    disabled={disabled || isSearching}
                    className={cn(
                      'h-12 rounded-md border-border/60 bg-background px-4 shadow-sm',
                  ...
                  >
                    ...
                    {activeFilterCount > 0 && (
                      <span className="ml-2 inline-flex h-5 min-w-5 items-center justify-center rounded-md bg-primary px-1.5 text-xs font-semibold text-primary-foreground">
                        {activeFilterCount}
                      </span>
                    )}
                  </Button>
                ...
                <PopoverContent
                  align="end"
                  className="w-[min(420px,calc(100vw-2rem))] rounded-md p-0 shadow-xl"
                >
              ...
              <Button
                ...
                className="h-12 px-6 rounded-md font-medium shadow-sm transition-all"
              >
                {isSearching ? (
      ...
      <div className="flex-1 min-h-[300px] relative bg-muted/20">
        {error ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
            <div className="w-14 h-14 rounded-md bg-destructive/10 flex items-center justify-center mb-4">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
            ...
            <div className="mt-1 w-full max-w-sm rounded-md border border-primary/15 bg-primary/5 p-3 text-left shadow-sm">
              <div className="flex items-center gap-2 text-xs font-medium text-primary">
                <span className="relative flex h-2 w-2">
                  <span className="absolute inline-flex h-full w-full rounded-md bg-primary opacity-75 animate-ping" />
                  <span className="relative inline-flex h-2 w-2 rounded-md bg-primary" />
                </span>
              ...
              <div className="mt-3 space-y-2">
                {[0, 1, 2].map((index) => (
                  <div key={index} className="flex items-center gap-2">
                    <span className="h-1.5 w-1.5 rounded-md bg-primary/50" />
                    <span
                      className={cn(
                        'h-2 rounded-md bg-[linear-gradient(90deg,hsl(var(--muted)),hsl(var(--primary)/0.28),hsl(var(--muted)))] bg-[length:200%_100%] animate-shimmer',
                        index === 0 && 'w-11/12',
                        index === 1 && 'w-8/12',
                        index === 2 && 'w-10/12',
                      )}
      ...
      {hasResults && (
        <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-10 w-full max-w-fit px-4 pointer-events-none animate-in slide-in-from-bottom-4 fade-in duration-300">
          <div className="bg-background/80 backdrop-blur-2xl supports-[backdrop-filter]:bg-background/60 border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] rounded-md p-1.5 flex items-center gap-1 sm:gap-2 pointer-events-auto ring-1 ring-white/10 transition-all">
            {/* Selection Status */}
            ...
            <div className="flex items-center gap-1">
              <Button
                ...
                className="h-9 w-9 rounded-md text-muted-foreground hover:text-foreground hover:bg-foreground/5 transition-colors"
            ...
            <div className="flex items-center gap-1.5 pr-0.5">
              <Button
                ...
                className="h-9 px-4 rounded-md text-sm font-medium bg-secondary/60 hover:bg-secondary/80 transition-colors"
              >
                {isLoadingMore ? (
              ...
              <Button
                ...
                disabled={busy || selectedVideos.length === 0}
                className="h-9 px-5 rounded-md text-sm font-semibold shadow-md bg-primary hover:bg-primary/90 text-primary-foreground transition-all active:scale-95"
              >
                {isAdding ? (
                  <Loader2 className="w-4 h-4 sm:mr-1.5 animate-spin" />
                ) : (
