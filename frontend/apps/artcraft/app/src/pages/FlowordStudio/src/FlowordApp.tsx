  ...
  try {
    ...
    const pushArtifact = (
      ...
      const type = (a.artifact_type ??
        a.kind ??
        defaultType) as ArtifactRef["type"];
      artifacts.push({
        ...
        mimeType: String(
          a.mime_type ??
            (type === "video" || type === "rendered_video"
              ? "video/mp4"
              : "image/png"),
        ),
    ...
    ["script_artifact", "capcut_artifact"].forEach((key) => {
      pushArtifact(
        output[key],
        key.includes("script") ? "script" : "capcut_draft",
        key,
      );
    });
    ...
    if (
      ...
      output.final_video_path.trim().length > 0
    ) {
    ...
    return {
      artifacts,
      draftUrl:
        typeof output.draft_url === "string" ? output.draft_url : undefined,
      videoUrl,
    };
  } catch {
    return { artifacts: [] };
...
export const FlowordApp: React.FC<FlowordAppProps> = ({
  ...
  const [stepConfigs, setStepConfigs] =
    useState<StepConfig[]>(INITIAL_STEP_CONFIGS);
  ...
  const [currentStepMessage, setCurrentStepMessage] = useState<string>(
    "Ready to enqueue Rust Workflow Worker",
  );
  ...
  const [detailModalStepId, setDetailModalStepId] = useState<string | null>(
    null,
  );
  ...
  const [readiness, setReadiness] =
    useState<DetailedReadinessStatus>(DEFAULT_READINESS);
  ...
  const refreshJobs = useCallback(async () => {
    try {
      ...
      const hydratedRuns: WorkflowRun[] = workflows.map((wf) => {
        ...
        const percent = backendProgress(
          wf.business_status || wf.current_stage,
          wf.status,
        );
        ...
        const pageSnap =
          (wf.page_snapshot as Record<string, unknown> | null) ?? null;
        const pageId =
          wf.page_id ?? (pageSnap?.page_id as string | undefined) ?? "general";
        const pageName =
          (pageSnap?.page_name as string | undefined) ?? undefined;
        const inputPayload =
          (wf.input_payload as Partial<WorkflowInput> | null) ?? null;
  ...
  const refreshPages = useCallback(async () => {
    try {
      ...
      console.error(
        "[Floword] Failed to load ContentPages from database:",
        err,
      );
      setPages([]);
    }
  ...
  const startWorkflowPolling = useCallback(
    (jobId: string) => {
      ...
      pollTimerRef.current = setInterval(async () => {
        try {
          const res: GetFlowordWorkflowResponse =
            await getFlowordWorkflow(jobId);
          const outputs = parseWorkflowOutputs(res.stage_outputs);
          const percent = backendProgress(
            res.business_status || res.current_stage,
            res.status,
          );
          ...
          const pageSnap =
            (res.page_snapshot as Record<string, unknown> | null) ?? null;
          const pageId =
            res.page_id ??
            (pageSnap?.page_id as string | undefined) ??
            activePageId ??
            "general";
          const pageName =
            (pageSnap?.page_name as string | undefined) ?? undefined;
          const inputPayload =
            (res.input_payload as Partial<WorkflowInput> | null) ?? null;
          ...
          setAllRuns((prev) =>
            prev.map((r) => (r.id === jobId ? updatedRun : r)),
          );

          if (
            TERMINAL_STATUSES.has(res.status) ||
            TERMINAL_STAGES.has(res.current_stage)
          ) {
            if (pollTimerRef.current) clearInterval(pollTimerRef.current);
            setRunning(false);
  ...
  const handleRunWorkflow = async (inputToUse: WorkflowInput) => {
    const isGrokWorkflow =
      inputToUse.workflowMode === "grok_content_pipeline" ||
      inputToUse.workflowName?.toLowerCase().includes("grok");

    if (isGrokWorkflow) {
      const selectedPage = pages.find(
        (p) => p.id === (inputToUse.pageId || activePageId),
      );
      ...
      const effImagePrompt =
        inputToUse.imagePrompt ||
        inputToUse.prompt ||
        inputToUse.customPrompt ||
    ...
    setProgress(5);
    appendLog(
      `🚀 Khởi chạy ${inputToUse.workflowName || "Floword Production Pipeline"}...`,
    );
    ...
    try {
      const res = await enqueueFlowordWorkflow({
        ...
        workflow_name:
          inputToUse.workflowName ||
          (inputToUse.workflowMode === "grok_content_pipeline"
            ? "grok_content_pipeline"
            : "floword_video_pipeline"),
        workflow_mode:
          inputToUse.workflowMode ||
          (inputToUse.workflowName?.toLowerCase().includes("grok")
            ? "grok_content_pipeline"
            : undefined),
        ...
        image_prompt:
          inputToUse.imagePrompt ||
          (inputToUse.prompt !== DEFAULT_WORKFLOW_INPUT.prompt
            ? inputToUse.prompt
            : undefined),
        expand_9_16_prompt:
          inputToUse.expand916Prompt || inputToUse.expandPrompt,
        ...
        caption: inputToUse.caption?.trim()
          ? inputToUse.caption.trim()
          : undefined,
        hashtags: inputToUse.hashtags,
        description: inputToUse.description?.trim()
          ? inputToUse.description.trim()
          : undefined,
  ...
  const handleApprovePublish = async (runId: string) => {
    toast(
      "Chức năng Publishing Engine tự động đang chờ cấu hình profile mạng xã hội.",
      {
        icon: "⏳",
      },
    );
  ...
  const handleRejectPublish = async (runId: string) => {
    toast("Đã từ chối đăng video. Bạn có thể tinh chỉnh lại trong Studio.", {
      icon: "↩️",
    });
  };

  const handleCreatePage = async (
    req: CreateContentPageRequest,
  ): Promise<ContentPage> => {
    try {
  ...
  const handleUpdatePage = async (
    pageId: string,
    req: Partial<UpdateContentPageRequest> & { name?: string },
  ): Promise<ContentPage> => {
    try {
  ...
  return (
    <div className="floword-shell relative flex h-[calc(100vh-56px)] w-full overflow-hidden bg-[#0d1017]">
      ...
      <FlowordSidebar
        ...
        pendingPublishCount={
          allRuns.filter((r) => r.status === "completed" && !r.isPublished)
            .length
        }
          ...
          onRunWorkflow={
            activeView === "production" || activeView === "script_market"
              ...
              : running
                ...
                : () => handleRunWorkflow(workflowInput)
          }
      ...
      <ConfigureDrawer
        ...
        onChangeSteps={(newSteps) => {
          ...
          const researchEnabled = newSteps.find(
            (step) => step.module === "media_crawler",
          )?.enabled;
          if (researchEnabled !== undefined)
            setWorkflowInput((current) => ({ ...current, researchEnabled }));
        }}
