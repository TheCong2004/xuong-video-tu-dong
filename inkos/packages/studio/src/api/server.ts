...
export function createStudioServer(initialConfig: ProjectConfig, root: string, overrides: { readonly nodeImageGenerator?: NodeImageDeps } = {}) {
  ...
  // --- Floword Story Planning ---

  app.post("/api/v1/floword/story-plan", async (c) => {
    try {
      const body = await c.req.json();
      return c.json(buildFlowordStoryPlan(body));
    } catch (error) {
      return c.json(
        { error: { code: "STORY_INPUT_INVALID", message: error instanceof Error ? error.message : "Invalid story planning input" } },
        422,
      );
    }
  });
