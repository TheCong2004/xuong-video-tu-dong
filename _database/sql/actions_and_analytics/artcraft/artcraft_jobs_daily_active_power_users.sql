-- Daily active "power users" for ArtCraft since January 1st, 2026.
-- A "power user" is a user who created more than 10 ArtCraft inference jobs in the calendar month.
-- This query then counts how many such power users were active on each day.

SELECT
  DATE(j.created_at) AS day,
  COUNT(DISTINCT j.maybe_creator_user_token) AS daily_active_power_users
