...
const route = (fn) => async (req, res) => {
  try {
    ...
    res.status(status).json({ error: { code, message, details: {}, retryable: [409, 503, 504].includes(status) } });
  }
};
