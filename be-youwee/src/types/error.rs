...
impl BackendError {
  pub fn new(code: impl Into<String>, message: impl Into<String>) -> Self {
    ...
    Self { wire: BackendErrorWire { code, message: message.into(), params: None, source: None, retryable } }
  }
  ...
  pub fn to_wire_string(&self) -> String {
    match serde_json::to_string(&self.wire) {
      ...
      Err(_) => format!("{}{{\"code\":\"{}\",\"message\":\"{}\"}}", BACKEND_ERROR_PREFIX, code::BACKEND_UNKNOWN, escape_json_string(self.message())),
    }
  }
}
...
pub fn infer_error_code(message: &str) -> &'static str {
  ...
  if m.contains("this live event will begin") || m.contains("premieres in") || m.contains("premiere will begin") || m.contains("live event has not started") {
  ...
  if m.contains("aria2c not found") || (m.contains("aria2c") || m.contains("aria2")) && (m.contains("no such file") || m.contains("not recognized") || m.contains("not found")) {
  ...
  if m.contains("network error") || m.contains("connection") || m.contains("unable to download") || m.contains("failed to fetch") || m.contains("request error") {
  ...
  if m.contains("failed to read") || m.contains("failed to write") || m.contains("no such file") || m.contains("permission denied") || m.contains("failed to open") {
    return code::IO_OPERATION_FAILED;
  }
  if m.contains("query failed") || m.contains("database") || m.contains("failed to insert") || m.contains("failed to delete") || m.contains("failed to clear") {
...
pub fn default_retryable(code: &str) -> bool {
  matches!(code, code::NETWORK_TIMEOUT | code::NETWORK_REQUEST_FAILED | code::YT_RATE_LIMITED | code::PROCESS_START_FAILED | code::PROCESS_EXECUTION_FAILED | code::PROCESS_EXIT_NON_ZERO)
}
