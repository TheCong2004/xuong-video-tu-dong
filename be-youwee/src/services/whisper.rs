...
impl std::fmt::Display for WhisperError {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      WhisperError::FileTooLarge(size) => write!(f, "Audio file too large ({:.1} MB). Whisper API limit is 25 MB.", *size as f64 / 1_000_000.0),
...
fn build_subtitle_from_segments(segments: &[WhisperVerboseSegment], target_format: WhisperResponseFormat) -> String {
  match target_format {
    WhisperResponseFormat::Srt => segments.iter().enumerate().map(|(idx, segment)| format!("{}\n{} --> {}\n{}\n", idx + 1, format_srt_timestamp(segment.start), format_srt_timestamp(segment.end.max(segment.start + 0.1)), segment.text.trim())).collect::<Vec<_>>().join("\n"),
    WhisperResponseFormat::Vtt => {
      let cues = segments.iter().map(|segment| format!("{} --> {}\n{}\n", format_vtt_timestamp(segment.start), format_vtt_timestamp(segment.end.max(segment.start + 0.1)), segment.text.trim())).collect::<Vec<_>>().join("\n");
      format!("WEBVTT\n\n{}", cues)
    },
    _ => String::new(),
  }
...
fn convert_verbose_json_to_subtitle(response_text: &str, target_format: WhisperResponseFormat) -> Result<WhisperResult, WhisperError> {
  let verbose: WhisperVerboseResponse = serde_json::from_str(response_text).map_err(|e| WhisperError::ParseError(e.to_string()))?;
  ...
  let subtitle_text = if !segments.is_empty() {
    ...
    let fallback_segment = WhisperVerboseSegment { start: 0.0, end, text };
    build_subtitle_from_segments(&[fallback_segment], target_format)
  } else {
    return Err(WhisperError::ParseError("No text or segments in Whisper verbose_json response".to_string()));
  };

  Ok(WhisperResult { text: subtitle_text, duration_seconds: verbose.duration, language: verbose.language })
}
...
pub async fn transcribe_audio(api_key: &str, audio_path: &str, response_format: WhisperResponseFormat, language: Option<&str>, endpoint_url: Option<&str>, model: Option<&str>) -> Result<WhisperResult, WhisperError> {
  ...
  // Get file metadata
  let metadata = fs::metadata(path).await.map_err(|e| WhisperError::FileNotFound(format!("{}: {}", audio_path, e)))?;
  ...
  // Read file content
  let file_bytes = fs::read(path).await.map_err(|e| WhisperError::FileNotFound(format!("Failed to read file: {}", e)))?;

  // Get filename for the multipart form
  let filename = path.file_name().and_then(|n| n.to_str()).unwrap_or("audio.mp3").to_string();
  ...
  let file_part = Part::bytes(file_bytes).file_name(filename).mime_str(mime_type).map_err(|e| WhisperError::ParseError(format!("Invalid MIME type: {}", e)))?;
  ...
  let mut form = Form::new().part("file", file_part).text("model", model_name.to_string()).text("response_format", api_response_format.to_string());
  ...
  let response = client.post(api_url).header("Authorization", format!("Bearer {}", api_key)).multipart(form).send().await.map_err(|e| WhisperError::NetworkError(e.to_string()))?;
  ...
  match requested_format {
    WhisperResponseFormat::Text | WhisperResponseFormat::Srt | WhisperResponseFormat::Vtt => {
      ...
      Ok(WhisperResult { text: response_text, duration_seconds: None, language: None })
    },
    WhisperResponseFormat::Json => {
      // JSON format: {"text": "..."}
      let json: serde_json::Value = serde_json::from_str(&response_text).map_err(|e| WhisperError::ParseError(e.to_string()))?;

      let text = json.get("text").and_then(|t| t.as_str()).ok_or_else(|| WhisperError::ParseError("No text in response".to_string()))?;

      Ok(WhisperResult { text: text.to_string(), duration_seconds: None, language: None })
    },
    WhisperResponseFormat::VerboseJson => {
      ...
      Ok(WhisperResult { text: text.to_string(), duration_seconds: duration, language })
    },
  }
}
...
pub async fn extract_audio_for_whisper(input_path: &str, output_path: &str, ffmpeg_path: Option<&str>) -> Result<String, WhisperError> {
  ...
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| WhisperError::FfmpegError(format!("Failed to run FFmpeg: {}", e)))?;
  ...
  let metadata = fs::metadata(output_path).await.map_err(|e| WhisperError::FfmpegError(format!("Output file not created: {}", e)))?;
  ...
  if metadata.len() > MAX_FILE_SIZE {
    ...
    cmd2.hide_window();
    let output = cmd2.output().await.map_err(|e| WhisperError::FfmpegError(format!("Failed to run FFmpeg: {}", e)))?;
...
pub async fn get_audio_duration(audio_path: &str, ffprobe_path: Option<&str>) -> Result<f64, WhisperError> {
  use crate::utils::CommandExt;
  use tokio::process::Command;
  ...
  cmd.args(["-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", audio_path]);
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| WhisperError::FfmpegError(format!("Failed to run ffprobe: {}", e)))?;
  ...
  duration_str.parse::<f64>().map_err(|e| WhisperError::ParseError(format!("Invalid duration '{}': {}", duration_str, e)))
}
