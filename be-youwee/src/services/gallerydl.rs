...
pub async fn check_gallerydl_internal(app: &AppHandle) -> Result<GalleryDlStatus, String> {
  let Some(binary_path) = get_gallerydl_path(app) else {
    return Ok(GalleryDlStatus { installed: false, version: None, binary_path: None, is_system: true });
  };
  ...
  let output = cmd.output().await.map_err(|e| BackendError::from_message(format!("Failed to run gallery-dl: {}", e)).to_wire_string())?;

  if !output.status.success() {
    let stderr = String::from_utf8_lossy(&output.stderr);
    return Err(BackendError::from_message(format!("gallery-dl command failed: {}", stderr.trim())).to_wire_string());
  }

  Ok(GalleryDlStatus { installed: true, version: Some(String::from_utf8_lossy(&output.stdout).trim().to_string()), binary_path: Some(binary_path.to_string_lossy().to_string()), is_system: get_app_gallerydl_path(app).as_ref().map(|app_path| app_path != &binary_path).unwrap_or(true) })
}
