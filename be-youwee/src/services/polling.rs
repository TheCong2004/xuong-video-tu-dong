...
use crate::database;
use crate::services::{build_cookie_args, build_site_header_args, get_deno_path, run_ytdlp_with_stderr};
...
static POLLING_NETWORK_CONFIG: Mutex<PollingNetworkConfig> = Mutex::new(PollingNetworkConfig { cookie_mode: None, cookie_browser: None, cookie_browser_profile: None, cookie_file_path: None, cookie_skip_patterns: None, proxy_url: None });
...
fn get_network_config() -> PollingNetworkConfig {
  POLLING_NETWORK_CONFIG.lock().map(|g| g.clone()).unwrap_or_default()
}
        ...
        match check_channel_for_new_videos(&app, channel).await {
          Ok(new_count) => {
            if new_count > 0 {
              ...
              let _ = app.emit("channel-new-videos", NewVideosEvent { channel_id: channel.id.clone(), channel_name: channel.name.clone(), new_count, total_new });
              ...
              if channel.auto_download {
                let _ = app.emit("channel-auto-download", AutoDownloadEvent { channel_id: channel.id.clone(), channel_name: channel.name.clone(), quality: channel.download_quality.clone(), format: channel.download_format.clone(), video_codec: channel.download_video_codec.clone(), preferred_fps: channel.download_preferred_fps.clone(), audio_bitrate: channel.download_audio_bitrate.clone(), download_threads: channel.download_threads });
              }
            } else {
              // Still update last checked time
              let _ = database::update_channel_last_checked_db(channel.id.clone(), channel.last_video_id.clone());
            }
          },
          Err(e) => {
...
async fn check_channel_for_new_videos(app: &AppHandle, channel: &FollowedChannel) -> Result<usize, String> {
  let limit = channel.filter_max_videos.unwrap_or(20) as u32;
  let channel_urls = normalize_channel_content_urls(&channel.url, Some(&channel.youtube_content_type));
  ...
  for channel_url in channel_urls {
    ...
    let mut args = vec!["--dump-json".to_string(), "--no-warnings".to_string(), "--socket-timeout".to_string(), "30".to_string(), "--playlist-end".to_string(), limit.to_string()];
    ...
    let cookie_args = build_cookie_args(&channel_url, net.cookie_mode.as_deref(), net.cookie_browser.as_deref(), net.cookie_browser_profile.as_deref(), net.cookie_file_path.as_deref(), net.cookie_skip_patterns.as_deref());
  ...
  for line in output.lines() {
    ...
    if let Ok(json) = serde_json::from_str::<serde_json::Value>(line) {
      ...
      let video_url = json.get("url").or_else(|| json.get("webpage_url")).and_then(|v| v.as_str()).map(|s| s.to_string()).unwrap_or_else(|| build_fallback_video_url(&channel.url, &id));
      ...
      new_videos.push(ChannelVideo { id: uuid::Uuid::new_v4().to_string(), channel_id: channel.id.clone(), video_id: id, title, url: video_url, thumbnail, duration, upload_date, status: "new".to_string(), created_at: now.clone() });
    }
  }
...
fn send_notification(app: &AppHandle, channel_name: &str, new_count: usize) {
  ...
  let title = "Youwee";
  let body = if new_count == 1 { format!("{}: 1 new video", channel_name) } else { format!("{}: {} new videos", channel_name, new_count) };
