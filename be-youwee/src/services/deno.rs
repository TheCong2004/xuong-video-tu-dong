...
pub async fn check_deno_internal(app: &AppHandle) -> Result<DenoStatus, String> {
    ...
    if deno_path.exists() {
      ...
      if let Ok(output) = output {
        if output.status.success() {
          ...
          let version = stdout.lines().next().map(|l| l.trim_start_matches("deno ").split_whitespace().next().unwrap_or("").to_string()).unwrap_or_default();
          return Ok(DenoStatus { installed: true, version: Some(version), binary_path: Some(deno_path.to_string_lossy().to_string()), is_system: false });
        }
      }
    }
  }
  ...
  let deno_cmd = deno_path.as_ref().map(|path| path.to_string_lossy().to_string()).unwrap_or_else(|| "deno".to_string());
  ...
  match output {
    Ok(output) if output.status.success() => {
      ...
      Ok(DenoStatus { installed: true, version: Some(version), binary_path: deno_path.map(|path| path.to_string_lossy().to_string()), is_system: true })
    },
    _ => Ok(DenoStatus { installed: false, version: None, binary_path: None, is_system: false }),
  }
}
...
pub async fn check_deno_update_internal(app: &AppHandle) -> Result<DenoUpdateInfo, String> {
  ...
  if !current_status.installed {
    return Ok(DenoUpdateInfo { has_update: false, current_version: None, latest_version: None, release_url: None });
  }
  ...
  if current_status.is_system {
    return Ok(DenoUpdateInfo { has_update: false, current_version, latest_version: None, release_url: Some("System Deno - update via deno upgrade".to_string()) });
  }
  ...
  let response = client.get("https://api.github.com/repos/denoland/deno/releases/latest").send().await.map_err(|e| format!("Failed to fetch release info: {}", e))?;
  ...
  // Compare versions
  let has_update = if let Some(ref current) = current_version { latest_version != *current && !current.contains(&latest_version) } else { false };

  Ok(DenoUpdateInfo { has_update, current_version, latest_version: Some(latest_version), release_url: html_url })
}
