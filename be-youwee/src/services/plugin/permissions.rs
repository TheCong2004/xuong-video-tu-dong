...
use crate::types::{PluginConfigField, PluginConfigFieldInputType, PluginFilesystemPermission, PluginManifest, PluginPermissionApproval, PluginPermissionRequest};

use super::security_policy::validate_plugin_write_scope;

pub(super) fn resolve_plugin_entrypoint(plugin_dir: &Path, entrypoint: &str) -> Result<PathBuf, String> {
  ...
  let canonical_plugin_dir = std::fs::canonicalize(plugin_dir).map_err(|e| format!("Failed to resolve plugin directory {}: {}", plugin_dir.display(), e))?;
  let canonical_candidate = std::fs::canonicalize(&candidate).map_err(|e| format!("Failed to resolve plugin entrypoint {}: {}", candidate.display(), e))?;
  ...
  if !canonical_candidate.is_file() {
    return Err(format!("Plugin entrypoint {} is not a file", canonical_candidate.display()));
  }
  Ok(canonical_candidate)
}

pub(super) fn collect_missing_permissions(requested: &PluginPermissionRequest, approved: &PluginPermissionApproval) -> Vec<String> {
  ...
  for permission in &requested.tools {
    if !approved.tools.iter().any(|approved_permission| approved_permission == permission) {
      missing.push(permission.as_str().to_string());
    }
  }
  missing
...
fn collect_user_selected_permission_paths(fields: &[PluginConfigField], values: &BTreeMap<String, Value>) -> Vec<PathBuf> {
  fields.iter().filter(|field| matches!(field.input_type, PluginConfigFieldInputType::File | PluginConfigFieldInputType::Directory)).filter_map(|field| values.get(&field.key).and_then(Value::as_str)).map(str::trim).filter(|value| !value.is_empty()).map(PathBuf::from).collect()
}
...
pub(super) fn build_permission_path_scopes(plugin_dir: &Path, payload_file: &Path, manifest: &PluginManifest, resolved_config_values: &BTreeMap<String, Value>) -> Result<(Vec<PathBuf>, Vec<PathBuf>), String> {
  ...
  let user_selected_paths = collect_user_selected_permission_paths(&manifest.config_fields, resolved_config_values);
    ...
    match permission {
      ...
      PluginFilesystemPermission::UserSelectedRead => {
        for path in &user_selected_paths {
          if !path.exists() {
            return Err(format!("Failed to resolve configured path {}: path does not exist", path.display()));
          }
...
pub(super) fn push_allow_flag(args: &mut Vec<String>, flag_name: &str, values: &[PathBuf]) {
  ...
  args.push(format!("--{}={}", flag_name, values.iter().map(|path| path.to_string_lossy().to_string()).collect::<Vec<_>>().join(",")));
}
