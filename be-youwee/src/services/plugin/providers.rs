...
pub async fn get_runtime_provider_status_internal(app: &AppHandle, provider: PluginProvider) -> RuntimeProviderStatus {
  match provider {
    PluginProvider::Deno => {
      ...
      RuntimeProviderStatus { provider, available: resolved_path.is_some(), resolved_path, resolved_source, details: Some("Resolves app-managed Deno first, then system Deno.".to_string()) }
    },
    PluginProvider::Python => {
      let path = if let Some(path) = resolve_command_path("python3").await { Some(path) } else { resolve_command_path("python").await };
      RuntimeProviderStatus { provider, available: path.is_some(), resolved_path: path.map(|value| value.to_string_lossy().to_string()), resolved_source: Some("system".to_string()), details: None }
    },
  }
}
...
pub(super) async fn resolve_provider_command(app: &AppHandle, provider: &PluginProvider) -> Result<(String, Option<String>), String> {
  match provider {
    PluginProvider::Deno => {
      let path = crate::services::get_deno_path(app).await.ok_or_else(|| "Deno runtime is not available".to_string())?;
      let source = match app.path().app_data_dir() {
        Ok(app_data) if path.starts_with(app_data.join("bin")) => "app-managed",
        _ => "system",
      };
