...
use crate::types::{PackagedPluginBuildInfo, PackagedPluginChecksums, PackagedPluginSignature, PluginManifest, PluginPackageSource, PluginPackageSourceKind, PluginSignaturePayload};
...
fn extract_zip_to_temp(bytes: &[u8], label: &str) -> Result<PathBuf, String> {
  ...
  std::fs::create_dir_all(&temp_root).map_err(|e| format!("Failed to create temporary plugin extraction directory {}: {}", temp_root.display(), e))?;

  let cursor = Cursor::new(bytes.to_vec());
  let mut archive = ZipArchive::new(cursor).map_err(|e| format!("Failed to open plugin zip archive: {}", e))?;

  for index in 0..archive.len() {
    let mut file = archive.by_index(index).map_err(|e| format!("Failed to read zip entry {}: {}", index, e))?;
    let Some(safe_name) = file.enclosed_name().map(|path| path.to_path_buf()) else {
      continue;
    };
    let outpath = temp_root.join(safe_name);
    if file.name().ends_with('/') {
      std::fs::create_dir_all(&outpath).map_err(|e| format!("Failed to create extracted directory {}: {}", outpath.display(), e))?;
    } else {
      if let Some(parent) = outpath.parent() {
        std::fs::create_dir_all(parent).map_err(|e| format!("Failed to create extracted directory {}: {}", parent.display(), e))?;
      }
      let mut outfile = std::fs::File::create(&outpath).map_err(|e| format!("Failed to create extracted file {}: {}", outpath.display(), e))?;
      std::io::copy(&mut file, &mut outfile).map_err(|e| format!("Failed to extract zip entry to {}: {}", outpath.display(), e))?;
    }
  }
...
fn validate_ywp_extension(path: &Path) -> Result<(), String> {
  let is_ywp = path.extension().and_then(|value| value.to_str()).map(|value| value.eq_ignore_ascii_case("ywp")).unwrap_or(false);

  if is_ywp {
    Ok(())
  } else {
    Err(format!("Plugin package must use the .ywp extension: {}", path.display()))
  }
}

fn resolve_packaged_root(extracted_root: &Path) -> Result<PathBuf, String> {
  let has_layout = |root: &Path| root.join("manifest.json").is_file() && root.join("build.json").is_file() && root.join("checksums.json").is_file();

  if has_layout(extracted_root) {
    return Ok(extracted_root.to_path_buf());
  }

  let entries = std::fs::read_dir(extracted_root).map_err(|e| format!("Failed to read extracted package root {}: {}", extracted_root.display(), e))?.filter_map(|entry| entry.ok()).collect::<Vec<_>>();
...
fn validate_packaged_checksums(root: &Path, checksums: &PackagedPluginChecksums) -> Result<(), String> {
  if checksums.algorithm.to_lowercase() != "sha256" {
    return Err(format!("Unsupported checksums algorithm in .ywp package: {}", checksums.algorithm));
  }
  ...
  for relative in expected_files {
    ...
    let expected = checksums.files.get(&relative).ok_or_else(|| format!("Missing checksum entry for {}", relative))?;
    if &actual != expected {
      return Err(format!("Checksum mismatch in .ywp package for {}", relative));
    }
  }
...
fn validate_packaged_signature_payload(payload: &PluginSignaturePayload, manifest: &PluginManifest, build_info: &PackagedPluginBuildInfo, checksums_bytes: &[u8]) -> Result<(), String> {
...
fn validate_packaged_signature(root: &Path, manifest: &PluginManifest, build_info: &PackagedPluginBuildInfo, signature: &PackagedPluginSignature) -> Result<(), String> {
  if signature.version != 1 {
    return Err(format!("Unsupported plugin signature version: {}", signature.version));
  }
  ...
  let checksums_bytes = std::fs::read(&checksums_path).map_err(|e| format!("Failed to read {}: {}", checksums_path.display(), e))?;
  validate_packaged_signature_payload(&signature.payload, manifest, build_info, &checksums_bytes)?;

  let public_key_bytes = base64::engine::general_purpose::STANDARD.decode(signature.public_key.trim()).map_err(|e| format!("Invalid plugin signature public key: {}", e))?;
  let verifying_key_bytes: [u8; 32] = public_key_bytes.as_slice().try_into().map_err(|_| "Invalid plugin signature public key length.".to_string())?;
  let verifying_key = VerifyingKey::from_bytes(&verifying_key_bytes).map_err(|e| format!("Invalid plugin signature public key: {}", e))?;
  ...
  if signature.fingerprint != expected_fingerprint {
    return Err("Plugin signature fingerprint does not match the embedded public key.".to_string());
  }

  let payload_bytes = serde_json::to_vec(&signature.payload).map_err(|e| format!("Failed to serialize plugin signature payload: {}", e))?;
  let signature_bytes = base64::engine::general_purpose::STANDARD.decode(signature.signature.trim()).map_err(|e| format!("Invalid plugin signature bytes: {}", e))?;
  let signature_bytes: [u8; 64] = signature_bytes.try_into().map_err(|_| "Invalid plugin signature length.".to_string())?;
  let ed25519_signature = Signature::from_bytes(&signature_bytes);
  verifying_key.verify(&payload_bytes, &ed25519_signature).map_err(|_| "Plugin signature verification failed.".to_string())
}
...
fn validate_packaged_manifest_layout(root: &Path, manifest: &PluginManifest, build_info: &PackagedPluginBuildInfo) -> Result<(), String> {
  if build_info.package_format != "ywp" {
    return Err(format!("Unsupported plugin package format: {}", build_info.package_format));
  }
  if build_info.package_format_version != 1 {
    return Err(format!("Unsupported plugin package format version: {}", build_info.package_format_version));
  }
  if build_info.builder.tool != "youwee-sdk" {
    return Err(format!("Unsupported plugin package builder: {}", build_info.builder.tool));
  }
  ...
  if !entrypoint.is_file() {
    return Err(format!("Packaged plugin entrypoint is missing: {}", manifest.runtime.entrypoint));
  }

  if let Some(i18n) = manifest.i18n.as_ref() {
    let directory = i18n.directory.clone().unwrap_or_else(|| "locales".to_string());
    for locale in &i18n.supported_locales {
      let locale_path = root.join(&directory).join(format!("{}.json", locale));
      if !locale_path.is_file() {
        return Err(format!("Packaged plugin locale file is missing: {}", normalize_path_for_checksum(root, &locale_path)));
      }
    }
    if let Some(default_locale) = i18n.default_locale.as_ref() {
      ...
      if !default_locale_path.is_file() {
        return Err(format!("Packaged plugin default locale file is missing: {}", normalize_path_for_checksum(root, &default_locale_path)));
      }
    }
  }
...
fn prepared_from_ywp_file(path: &Path) -> Result<PreparedPackage, String> {
  ...
  let bytes = std::fs::read(path).map_err(|e| format!("Failed to read plugin package {}: {}", path.display(), e))?;
  prepared_from_ywp_bytes(&bytes, PluginPackageSourceKind::PackageYwp, path.to_string_lossy().to_string())
}

pub(super) fn inspect_ywp_file(path: &Path) -> Result<PreparedPackage, String> {
  validate_ywp_extension(path)?;
  let bytes = std::fs::read(path).map_err(|e| format!("Failed to read plugin package {}: {}", path.display(), e))?;
  inspect_ywp_bytes(&bytes, PluginPackageSourceKind::PackageYwp, path.to_string_lossy().to_string())
}

fn inspect_ywp_bytes(bytes: &[u8], kind: PluginPackageSourceKind, value: String) -> Result<PreparedPackage, String> {
  ...
  let (signature_status, signer_key_id, signer_fingerprint, signature_algorithm, signed_at) = match load_packaged_signature(&package_root) {
    Ok(signature) => match validate_packaged_signature(&package_root, &manifest, &build_info, &signature) {
      Ok(()) => (Some("signed".to_string()), Some(signature.key_id), Some(signature.fingerprint), Some(signature.algorithm), Some(signature.signed_at)),
      Err(error) => {
        warnings.push(error);
        (Some("invalid-signature".to_string()), Some(signature.key_id), Some(signature.fingerprint), Some(signature.algorithm), Some(signature.signed_at))
      },
    },
    Err(error) => {
      warnings.push(error);
      (Some(if signature_path.is_file() { "invalid-signature" } else { "missing-signature" }.to_string()), None, None, None, None)
    },
  };

  Ok(PreparedPackage { manifest, package_root, source: PluginPackageSource { kind, value, checksum: Some(package_checksum.clone()), package_format: Some(build_info.package_format.clone()), package_format_version: Some(build_info.package_format_version), builder_sdk_version: Some(build_info.builder.version.clone()), signature_status: signature_status.clone(), signer_key_id: signer_key_id.clone(), signer_fingerprint: signer_fingerprint.clone(), signature_algorithm: signature_algorithm.clone(), signed_at: signed_at.clone() }, warnings, package_format: Some(build_info.package_format), package_format_version: Some(build_info.package_format_version), builder_sdk_version: Some(build_info.builder.version), package_checksum: Some(package_checksum), signature_status, signer_key_id, signer_fingerprint, signature_algorithm, signed_at })
}

fn prepared_from_ywp_bytes(bytes: &[u8], kind: PluginPackageSourceKind, value: String) -> Result<PreparedPackage, String> {
  ...
  Ok(PreparedPackage {
    manifest,
    package_root,
    source: PluginPackageSource { kind, value, checksum: Some(package_checksum.clone()), package_format: Some(build_info.package_format.clone()), package_format_version: Some(build_info.package_format_version), builder_sdk_version: Some(build_info.builder.version.clone()), signature_status: Some("signed".to_string()), signer_key_id: Some(signature.key_id.clone()), signer_fingerprint: Some(signature.fingerprint.clone()), signature_algorithm: Some(signature.algorithm.clone()), signed_at: Some(signature.signed_at.clone()) },
    warnings: Vec::new(),
...
pub(super) async fn prepare_plugin_package(source: &InstallPluginPackageInput) -> Result<PreparedPackage, String> {
