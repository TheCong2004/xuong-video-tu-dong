...
pub(super) fn satisfies_version_range(version: &str, range: &str) -> Result<bool, String> {
  let clauses = range.split(|ch: char| ch.is_whitespace() || ch == ',').map(str::trim).filter(|part| !part.is_empty()).collect::<Vec<_>>();
  if clauses.is_empty() {
...
pub(super) fn collect_compatibility_issues(manifest: &PluginManifest) -> Result<Vec<String>, String> {
  ...
  if let Some(range) = compatibility.app_version.as_ref() {
    if !satisfies_version_range(env!("CARGO_PKG_VERSION"), range)? {
      issues.push(format!("Requires Youwee app version {} but current app version is {}", range, env!("CARGO_PKG_VERSION")));
    }
  }
  ...
  if let Some(range) = compatibility.sdk_version.as_ref() {
    ...
    if !satisfies_version_range(&sdk_version, range)? {
      issues.push(format!("Requires youwee-sdk version {} but bundled SDK version is {}", range, sdk_version));
    }
  }
...
pub(super) fn validate_install_compatibility(manifest: &PluginManifest) -> Result<(), String> {
  ...
  Err(format!("Plugin is not compatible with this Youwee build:\n- {}", issues.join("\n- ")))
}
