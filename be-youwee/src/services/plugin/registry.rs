...
use crate::types::{PluginPackageSource, PluginPermissionApproval, PluginProvider, PluginWorkflowStepConfig};
...
pub(super) fn read_registry(app: &AppHandle) -> Result<PluginRegistry, String> {
  ...
  let raw = std::fs::read_to_string(&path).map_err(|e| format!("Failed to read plugin registry {}: {}", path.display(), e))?;
  match serde_json::from_str(&raw) {
    Ok(registry) => Ok(registry),
    Err(original_error) => {
      let mut raw_json: Value = serde_json::from_str(&raw).map_err(|e| format!("Failed to parse plugin registry {}: {}", path.display(), e))?;

      if !normalize_legacy_registry_json(&mut raw_json) {
        return Err(format!("Failed to parse plugin registry {}: {}", path.display(), original_error));
      }

      let registry: PluginRegistry = serde_json::from_value(raw_json).map_err(|e| format!("Failed to parse normalized plugin registry {}: {}", path.display(), e))?;
...
fn normalize_legacy_registry_json(root: &mut Value) -> bool {
  ...
  if let Some(default_providers) = root_object.get_mut("defaultProviders").and_then(Value::as_object_mut) {
    for value in default_providers.values_mut() {
      changed |= normalize_provider_value(value);
    }
  }

  if let Some(installations) = root_object.get_mut("installations").and_then(Value::as_object_mut) {
    for installation in installations.values_mut() {
      ...
      if let Some(source_object) = installation_object.get_mut("source").and_then(Value::as_object_mut) {
...
pub(super) fn write_registry(app: &AppHandle, registry: &PluginRegistry) -> Result<(), String> {
  ...
  if let Some(parent) = path.parent() {
    std::fs::create_dir_all(parent).map_err(|e| format!("Failed to create plugin registry directory {}: {}", parent.display(), e))?;
  }
  let raw = serde_json::to_string_pretty(registry).map_err(|e| format!("Failed to serialize plugin registry: {}", e))?;
  std::fs::write(&path, raw).map_err(|e| format!("Failed to write plugin registry {}: {}", path.display(), e))
}
