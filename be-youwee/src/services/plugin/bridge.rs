...
use super::security_policy::{validate_command_path, validate_plugin_output_path, validate_plugin_write_scope, validate_tool_arg};

const MAX_BRIDGE_BODY_BYTES: usize = 32 * 1024 * 1024;
const STRIP_SUBPROCESS_ENV_KEYS: &[&str] = &["DYLD_FALLBACK_LIBRARY_PATH", "DYLD_LIBRARY_PATH", "LD_LIBRARY_PATH"];
...
pub(super) async fn start_plugin_bridge(token: String, policy: PluginBridgePolicy) -> Result<PluginBridgeServer, String> {
  let listener = TcpListener::bind("127.0.0.1:0").await.map_err(|e| format!("Failed to start plugin bridge: {e}"))?;
  let addr = listener.local_addr().map_err(|e| format!("Failed to resolve plugin bridge address: {e}"))?;
  ...
  Ok(PluginBridgeServer { url: format!("http://{}", addr), token, tools, shutdown: Some(shutdown_tx) })
}

async fn handle_bridge_connection(mut stream: TcpStream, token: &str, policy: Arc<PluginBridgePolicy>, state: Arc<Mutex<PluginBridgeRunState>>) -> Result<(), String> {
  let mut buffer = Vec::new();
  let mut headers_end = None;
  loop {
    let mut chunk = [0_u8; 4096];
    let read = stream.read(&mut chunk).await.map_err(|e| format!("Failed to read plugin bridge request: {e}"))?;
    if read == 0 {
      return Ok(());
    }
    buffer.extend_from_slice(&chunk[..read]);
    if buffer.len() > MAX_BRIDGE_BODY_BYTES {
      write_json_response::<serde_json::Value>(&mut stream, 413, None, Some("Request too large")).await?;
      return Ok(());
    }
    ...
    if let Some(end) = headers_end {
      ...
      if buffer.len() >= total {
        ...
        let status = handle_bridge_request(token, policy, state, &buffer[..end], body).await.unwrap_or_else(|error| (400, None, Some(error)));
        write_json_response::<serde_json::Value>(&mut stream, status.0, status.1, status.2.as_deref()).await?;
        return Ok(());
      }
    }
  }
...
async fn handle_bridge_request(token: &str, policy: Arc<PluginBridgePolicy>, state: Arc<Mutex<PluginBridgeRunState>>, headers: &[u8], body: Vec<u8>) -> Result<(u16, Option<serde_json::Value>, Option<String>), String> {
  ...
  if method != "POST" {
    return Ok((405, None, Some("Plugin bridge only accepts POST requests.".to_string())));
  }
  ...
  match path {
    ...
    "/fs/readDir" => {
      ...
      Ok((200, Some(serde_json::to_value(entries).unwrap_or_default()), None))
    },
    "/fs/readText" => {
      let request: FsPathRequest = parse_body(&body)?;
      let path = PathBuf::from(request.path);
      validate_read_path(&policy, &path)?;
      let content = tokio::fs::read_to_string(&path).await.map_err(|e| format!("Failed to read {}: {e}", path.display()))?;
    ...
    "/fs/writeText" => {
      ...
      tokio::fs::write(&path, request.content).await.map_err(|e| format!("Failed to write {}: {e}", path.display()))?;
    ...
    "/fs/writeBase64" => {
      ...
      validate_write_path(&policy, &path)?;
      let created_by_plugin = !path.exists();
      let content = base64::engine::general_purpose::STANDARD.decode(request.content).map_err(|e| format!("Invalid base64 file content: {e}"))?;
    ...
    "/fs/removeFile" => {
      ...
      tokio::fs::remove_file(&path).await.map_err(|e| format!("Failed to remove {}: {e}", path.display()))?;
      Ok((200, Some(serde_json::Value::Null), None))
    },
    "/fs/ensureDir" => {
      let request: FsPathRequest = parse_body(&body)?;
      ...
      tokio::fs::create_dir_all(&path).await.map_err(|e| format!("Failed to create {}: {e}", path.display()))?;
      Ok((200, Some(serde_json::Value::Null), None))
    },
    "/fs/tempDir" => {
      let request: FsTempDirRequest = parse_body(&body)?;
      let prefix = request.prefix.unwrap_or_else(|| "youwee-plugin-".to_string());
      let path = plugin_temp_dir_path(&prefix);
      validate_write_scope_path(&policy, &path)?;
      tokio::fs::create_dir_all(&path).await.map_err(|e| format!("Failed to create plugin temp directory: {e}"))?;
    ...
    "/tool/run" => {
      ...
      Ok((200, Some(serde_json::to_value(response).unwrap_or_default()), None))
    },
    "/youtube/searchVideos" => {
      if !policy.network_allowed {
        return Ok((403, None, Some("YouTube search requires approved plugin network permission.".to_string())));
      }
      let request: YoutubeSearchVideosRequest = parse_body(&body)?;
      let response: YoutubeSearchResponse = search_youtube_videos_internal(request.query, request.limit, request.filters, request.continuation).await?;
      Ok((200, Some(serde_json::to_value(response).unwrap_or_default()), None))
    },
    _ => Ok((404, None, Some("Unknown plugin bridge operation.".to_string()))),
  }
}

async fn read_directory_entries(path: &Path) -> Result<Vec<FsDirectoryEntry>, String> {
  let metadata = tokio::fs::metadata(path).await.map_err(|e| format!("Failed to inspect directory {}: {e}", path.display()))?;
  if !metadata.is_dir() {
    return Err(format!("Plugin read path is not a directory: {}", path.display()));
  }

  let mut reader = tokio::fs::read_dir(path).await.map_err(|e| format!("Failed to read directory {}: {e}", path.display()))?;
  let mut entries = Vec::new();
  while let Some(entry) = reader.next_entry().await.map_err(|e| format!("Failed to read directory entry in {}: {e}", path.display()))? {
    let entry_path = entry.path();
    let name = entry.file_name().to_string_lossy().to_string();
    let file_type = entry.file_type().await.map_err(|e| format!("Failed to inspect {}: {e}", entry_path.display()))?;
    ...
    entries.push(FsDirectoryEntry { name, path: entry_path.to_string_lossy().to_string(), kind });
  }
...
async fn run_tool(policy: Arc<PluginBridgePolicy>, state: Arc<Mutex<PluginBridgeRunState>>, request: ToolRunRequest) -> Result<ToolRunResponse, String> {
  let command_path = match request.tool.as_str() {
    "ffmpeg" => policy.ffmpeg_path.as_ref(),
    "ytdlp" => policy.ytdlp_path.as_ref(),
    _ => None,
  }
  .ok_or_else(|| format!("Plugin tool is not approved or unavailable: {}", request.tool))?;
  validate_command_path(command_path)?;
  let cwd = request.cwd.as_ref().filter(|value| !value.trim().is_empty()).map(PathBuf::from).unwrap_or_else(|| policy.plugin_dir.clone());
  validate_working_directory_path(&policy, &cwd)?;
  let output_candidates = validate_tool_args_with_policy(&policy, &request.tool, &request.args, &cwd)?;
  let created_by_plugin_candidates = output_candidates.iter().filter(|path| !path.exists()).cloned().collect::<Vec<_>>();

  let mut command = Command::new(command_path);
  command.args(&request.args).stdout(std::process::Stdio::piped()).stderr(std::process::Stdio::piped());
  command.hide_window();
  ...
  let output = tokio::time::timeout(Duration::from_secs(60 * 30), command.output()).await.map_err(|_| format!("Plugin tool timed out: {}", request.tool))?.map_err(|e| format!("Failed to run plugin tool {}: {e}", request.tool))?;
  ...
  Ok(ToolRunResponse { code: output.status.code(), signal: None, stdout: String::from_utf8_lossy(&output.stdout).to_string(), stderr: String::from_utf8_lossy(&output.stderr).to_string() })
}

fn validate_tool_args_with_policy(policy: &PluginBridgePolicy, tool_name: &str, args: &[String], cwd: &Path) -> Result<Vec<PathBuf>, String> {
...
fn validate_read_path(policy: &PluginBridgePolicy, path: &Path) -> Result<(), String> {
  if !path_within_scopes(path, &policy.allow_read_scopes) {
    return Err(format!("Plugin read path is outside approved scopes: {}", path.display()));
  }
  Ok(())
}

fn validate_working_directory_path(policy: &PluginBridgePolicy, path: &Path) -> Result<(), String> {
  if !path_within_scopes(path, &policy.allow_read_scopes) && !path_within_scopes(path, &policy.allow_write_scopes) {
    return Err(format!("Plugin tool working directory is outside approved scopes: {}", path.display()));
  }
  Ok(())
}
...
fn validate_write_path(policy: &PluginBridgePolicy, path: &Path) -> Result<(), String> {
  ...
  if !path_within_scopes(path, &policy.allow_write_scopes) {
    return Err(format!("Plugin write path is outside approved scopes: {}", path.display()));
  }
  Ok(())
}
...
fn validate_write_scope_path(policy: &PluginBridgePolicy, path: &Path) -> Result<(), String> {
  ...
  if !path_within_scopes(path, &policy.allow_write_scopes) {
    return Err(format!("Plugin write directory is outside approved scopes: {}", path.display()));
  }
  Ok(())
}

async fn validate_remove_file_path(policy: &PluginBridgePolicy, state: &Arc<Mutex<PluginBridgeRunState>>, path: &Path) -> Result<(), String> {
  validate_write_path(policy, path)?;
  let metadata = tokio::fs::symlink_metadata(path).await.map_err(|e| format!("Failed to inspect {} before removing it: {e}", path.display()))?;
  let file_type = metadata.file_type();
  if file_type.is_symlink() {
    return Err(format!("Plugin cleanup cannot remove symlinks: {}", path.display()));
  }
  if !file_type.is_file() {
    return Err(format!("Plugin cleanup can only remove files, not directories: {}", path.display()));
  }
  if !is_removable_plugin_file(state, path).await {
    return Err("This plugin can only delete files it created during this run or files inside its Youwee-managed temp directory.".to_string());
  }
  Ok(())
}
...
async fn is_removable_plugin_file(state: &Arc<Mutex<PluginBridgeRunState>>, path: &Path) -> bool {
  ...
  state.generated_files.contains(&normalized) || state.temp_dirs.iter().any(|dir| normalized == *dir || normalized.starts_with(&format!("{dir}/")))
}

fn path_within_scopes(path: &Path, scopes: &[PathBuf]) -> bool {
  let normalized = normalize_existing_or_parent(path);
  scopes.iter().map(|scope| normalize_existing_or_parent(scope)).any(|scope| normalized == scope || normalized.starts_with(&format!("{scope}/")))
}
...
fn normalize_existing_or_parent(path: &Path) -> String {
  let resolved = if path.exists() {
    ...
    std::fs::canonicalize(parent).map(|parent| parent.join(path.file_name().unwrap_or_default())).unwrap_or_else(|_| path.to_path_buf())
  } else {
    path.to_path_buf()
  };
...
fn is_local_path_candidate(value: &str) -> bool {
  ...
  path.is_absolute() || trimmed.contains('/') || trimmed.contains('\\') || path.extension().is_some()
}
...
fn has_valid_auth(headers: &str, token: &str) -> bool {
  headers.lines().any(|line| line.split_once(':').map(|(key, value)| key.eq_ignore_ascii_case("authorization") && value.trim() == format!("Bearer {token}")).unwrap_or(false))
}
  ...
  for line in text.lines() {
    if let Some((key, value)) = line.split_once(':') {
      if key.eq_ignore_ascii_case("content-length") {
        let parsed = value.trim().parse::<usize>().map_err(|_| "Invalid Content-Length.".to_string())?;
...
mod tests {
  ...
  fn test_dir(name: &str) -> PathBuf {
    let path = std::env::temp_dir().join(format!("youwee-plugin-bridge-test-{name}-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&path).expect("create test dir");
    path
  }

  fn test_policy(root: &Path) -> PluginBridgePolicy {
    PluginBridgePolicy { allow_read_scopes: vec![root.to_path_buf()], allow_write_scopes: vec![root.to_path_buf()], plugin_dir: root.to_path_buf(), ffmpeg_path: None, ytdlp_path: None, network_allowed: false }
  }
  ...
  async fn youtube_search_requires_network_permission() {
    ...
    let result = handle_bridge_request("token", policy, state, b"POST /youtube/searchVideos HTTP/1.1\r\nauthorization: Bearer token", body).await.expect("youtube search rejection");

    std::fs::remove_dir_all(&root).ok();
    assert_eq!(result.0, 403);
    assert_eq!(result.2.as_deref(), Some("YouTube search requires approved plugin network permission."));
  }
  ...
  async fn read_base64_returns_binary_file_content() {
    ...
    let result = handle_bridge_request("token", policy, state, b"POST /fs/readBase64 HTTP/1.1\r\nauthorization: Bearer token", body).await.expect("read base64");
  ...
  async fn read_dir_returns_directory_entries() {
    ...
    let result = handle_bridge_request("token", policy, state, b"POST /fs/readDir HTTP/1.1\r\nauthorization: Bearer token", body).await.expect("read directory");
  ...
  async fn write_base64_writes_binary_file_and_marks_generated() {
    ...
    let result = handle_bridge_request("token", policy, state.clone(), b"POST /fs/writeBase64 HTTP/1.1\r\nauthorization: Bearer token", body).await.expect("write base64");
    let remove_result = validate_remove_file_path(&test_policy(&root), &state, &file).await;

    assert_eq!(result.0, 200);
    assert_eq!(std::fs::read(&file).expect("read written file"), [0, 1, 2, 250, 255]);
...
async fn write_json_response<T: Serialize>(stream: &mut TcpStream, status: u16, result: Option<T>, error: Option<&str>) -> Result<(), String> {
  let response = BridgeResponse { ok: error.is_none() && status < 400, result, error: error.map(str::to_string) };
  let body = serde_json::to_vec(&response).map_err(|e| format!("Failed to serialize plugin bridge response: {e}"))?;
  ...
  stream.write_all(header.as_bytes()).await.map_err(|e| format!("Failed to write plugin bridge response: {e}"))?;
  stream.write_all(&body).await.map_err(|e| format!("Failed to write plugin bridge response: {e}"))?;
  Ok(())
}
