...
use crate::types::{PluginConfigField, PluginConfigFieldInputType, PluginInstallation, PluginManifest, PluginPackageSource, PluginProvider, PluginRuntimeLanguage, PluginSummary};

use super::manifest::config_value_matches_field_type;
use super::registry::PluginRegistry;

pub(super) fn manifest_summary(manifest: PluginManifest, installation: PluginInstallation, warnings: Vec<String>, readme_content: Option<String>) -> PluginSummary {
  PluginSummary { manifest, installation, warnings, readme_content }
}

fn append_locale_before_extension(path: &Path, locale: &str) -> PathBuf {
  let file_name = path.file_name().and_then(|value| value.to_str()).unwrap_or("README.md");
...
fn build_readme_locale_preferences(app_locale: Option<&str>, fallback_locale: Option<&str>) -> Vec<String> {
...
fn resolve_readme_path(root: &Path, manifest: &PluginManifest, preferred_locales: &[String]) -> Option<PathBuf> {
  let manifest_readme = manifest.readme.as_deref().map(str::trim).filter(|value| !value.is_empty()).map(PathBuf::from);

  let candidate = manifest_readme.unwrap_or_else(|| PathBuf::from("README.md"));
  let base_path = if candidate.is_absolute() { candidate } else { root.join(candidate) };
...
pub(super) fn load_plugin_readme_content(root: &Path, manifest: &PluginManifest, app_locale: Option<&str>, fallback_locale: Option<&str>) -> Option<String> {
...
pub(super) fn validate_runtime_config_value(field: &PluginConfigField, value: &Value) -> Result<(), String> {
  if !config_value_matches_field_type(field, value) {
    return Err(format!("Invalid value type for plugin config field {}", field.key));
  }
  ...
  match field.input_type {
    PluginConfigFieldInputType::Select => {
      ...
      if !field.options.iter().any(|option| option.value == selected) {
        return Err(format!("Plugin config field {} must use one of the declared options", field.key));
      }
    },
    PluginConfigFieldInputType::MultiSelect => {
      ...
      for entry in values {
        ...
        if !field.options.iter().any(|option| option.value == selected) {
          return Err(format!("Plugin config field {} contains unsupported option {}", field.key, selected));
        }
      }
    },
    PluginConfigFieldInputType::Number => {
      let Some(number) = value.as_f64() else {
        return Err(format!("Plugin config field {} must be a number", field.key));
      };
      if let Some(min) = field.min {
        if number < min {
          return Err(format!("Plugin config field {} must be greater than or equal to {}", field.key, min));
        }
      }
      if let Some(max) = field.max {
        if number > max {
          return Err(format!("Plugin config field {} must be less than or equal to {}", field.key, max));
        }
      }
    },
    _ => {},
...
pub(super) fn resolve_effective_plugin_config_values(manifest: &PluginManifest, stored: &BTreeMap<String, Value>) -> Result<BTreeMap<String, Value>, String> {
  let mut resolved = BTreeMap::new();

  for field in &manifest.config_fields {
    let value = stored.get(&field.key).cloned().or_else(|| field.default_value.clone());
    match value {
      Some(value) => {
      ...
      None if field.required => {
        return Err(format!("Plugin config field {} ({}) is required but not configured", field.key, field.label));
      },
      None => {},
    }
  }
...
pub(super) fn build_installation_from_registry(registry: &PluginRegistry, manifest: &PluginManifest, source: PluginPackageSource, installed_path: String) -> PluginInstallation {
  ...
  let config_values = manifest
    ...
    .filter_map(|field| {
      let value = entry.and_then(|record| record.config_values.get(&field.key)).cloned();
  ...
  let config_value_status = manifest
    ...
    .iter()
    .map(|field| {
      let is_set = entry.and_then(|value| value.config_values.get(&field.key)).or_else(|| field.default_value.as_ref()).is_some();
  ...
  PluginInstallation {
    ...
    approved_permissions: entry.map(|value| value.approved_permissions.clone()).unwrap_or_default(),
    selected_provider: entry.and_then(|value| value.selected_provider.clone()).or_else(|| manifest.runtime.preferred_provider.clone()),
    timeout_sec_override: entry.and_then(|value| value.timeout_sec_override),
    installed_path,
    source: entry.and_then(|value| value.source.clone()).unwrap_or(source),
