...
pub(super) fn combine_plugin_event_details(message: Option<&String>, stdout: Option<&String>, stderr: Option<&String>) -> Option<String> {
...
pub(super) fn build_plugin_completion_details(result: &PluginExecutionResult) -> Option<String> {
  ...
  if let Some(mutation) = result.mutations.as_ref() {
    ...
    let extra_files: Vec<&String> = mutation.extra_files.iter().filter(|path| !path.trim().is_empty() && mutation.active_filepath.as_ref().map(|active| active != *path).unwrap_or(true)).collect();
...
pub(super) fn emit_plugin_runtime_output(app: &AppHandle, plugin_id: &str, plugin_name: &str, run_id: Option<&str>, stream: &str, bytes: &[u8], log_url: Option<&str>, media_title: Option<&str>, filename: Option<&str>) {
  if bytes.is_empty() {
    return;
  }
  ...
  let log_type = if stream == "stdout" {
    ...
    match trimmed.trim_start().strip_prefix('[').and_then(|value| value.split_once(']').map(|(level, _)| level.to_ascii_lowercase())) {
  ...
  app.emit("plugin-execution-output", PluginExecutionOutputEvent { plugin_id: plugin_id.to_string(), run_id: run_id.map(|value| value.to_string()), plugin_name: Some(plugin_name.to_string()), stream: stream.to_string(), chunk: trimmed.to_string(), media_title: media_title.map(str::to_string), filename: filename.map(str::to_string), media_url: log_url.map(str::to_string) }).ok();
}

pub(super) fn should_persist_plugin_runtime_output(stream: &str, trimmed: &str, log_type: &str) -> bool {
  if stream == "stdout" {
...
pub(super) async fn capture_process_stream<R>(app: AppHandle, stream_name: &str, plugin_id: String, plugin_name: String, run_id: Option<String>, mut reader: R, log_url: Option<String>, media_title: Option<String>, filename: Option<String>) -> Vec<u8>
where
  ...
  loop {
    match reader.read(&mut buffer).await {
      ...
      Ok(size) => {
        ...
        emit_plugin_runtime_output(&app, &plugin_id, &plugin_name, run_id.as_deref(), stream_name, &buffer[..size], log_url.as_deref(), media_title.as_deref(), filename.as_deref());
      },
      Err(_) => break,
    }
  }
...
pub(super) fn parse_plugin_result(stdout: &str) -> Option<PluginScriptOutput> {
  ...
  trimmed.lines().rev().map(str::trim).find(|line| !line.is_empty()).and_then(|line| serde_json::from_str::<PluginScriptOutput>(line).ok())
}

pub(super) fn classify_plugin_runtime_error(stderr: &str) -> Option<PluginRuntimeError> {
  let line = stderr.lines().map(str::trim).find(|line| line.starts_with("Requires "))?;

  let (kind, resource, resource_label, user_message) = if let Some(resource) = extract_quoted_resource(line, "Requires env access to ") {
    let resource_label = friendly_env_resource_label(&resource);
    let user_message = if resource.starts_with("YOUWEE_AI_") { format!("Plugin AI helpers are disabled for security. The plugin tried to read Youwee's {} directly.", resource_label.as_deref().unwrap_or("AI setting")) } else { format!("This plugin tried to read {}, but Youwee did not allow that access.", resource_label.as_deref().unwrap_or("an environment setting")) };
    ...
    ("read".to_string(), Some(resource.clone()), Some(resource.clone()), format!("This plugin tried to read {resource}, but that path is outside its approved read permissions."))
  } else if let Some(resource) = extract_prefixed_resource(line, "Requires write access to ") {
    ("write".to_string(), Some(resource.clone()), Some(resource.clone()), format!("This plugin tried to write to {resource}, but that path is outside its approved write permissions."))
  } else if let Some(resource) = extract_prefixed_resource(line, "Requires run access to ") {
    ("run".to_string(), Some(resource.clone()), Some(resource.clone()), format!("This plugin tried to run {resource}, but that command or tool is not approved."))
  } else if line.starts_with("Requires net access") {
    ("net".to_string(), None, None, "This plugin tried to access the network, but network permission is not approved.".to_string())
  } else {
    return None;
  };

  Some(PluginRuntimeError { kind, resource, resource_label, user_message, technical_details: format!("Deno runtime permission error:\n{line}") })
}
...
fn extract_prefixed_resource(line: &str, prefix: &str) -> Option<String> {
  ...
  let resource = rest.split_once(". Run again").map(|(value, _)| value).or_else(|| rest.split_once(", run again").map(|(value, _)| value)).unwrap_or(rest).trim().trim_matches('"').to_string();
...
#[cfg(not(unix))]
pub(super) fn plugin_exit_reason(status: &std::process::ExitStatus) -> String {
  status.code().map(|code| format!("code {}", code)).unwrap_or_else(|| "terminated".to_string())
}

pub(super) async fn capture_process_stream_err<R>(app: AppHandle, stream_name: &str, plugin_id: String, plugin_name: String, run_id: Option<String>, reader: R, log_url: Option<String>, media_title: Option<String>, filename: Option<String>) -> Vec<u8>
where
  R: AsyncRead + Unpin + Send + 'static,
{
  capture_process_stream(app, stream_name, plugin_id, plugin_name, run_id, reader, log_url, media_title, filename).await
}
