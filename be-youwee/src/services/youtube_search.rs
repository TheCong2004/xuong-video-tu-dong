use crate::types::{BackendError, YoutubeSearchDurationFilter, YoutubeSearchFeatureFilter, YoutubeSearchFilters, YoutubeSearchResponse, YoutubeSearchSortFilter, YoutubeSearchUploadDateFilter, YoutubeSearchVideo};
...
fn clamp_search_limit(limit: Option<u32>) -> usize {
  limit.unwrap_or(DEFAULT_SEARCH_LIMIT).clamp(1, MAX_SEARCH_LIMIT) as usize
}
...
fn run_text(value: Option<&Value>) -> Option<String> {
  ...
  let text = value.get("runs").and_then(|runs| runs.as_array()).map(|runs| runs.iter().filter_map(|run| run.get("text").and_then(|v| v.as_str())).collect::<String>()).unwrap_or_default();
  let trimmed = text.trim();
  if trimmed.is_empty() {
    None
  } else {
...
fn best_thumbnail(renderer: &Value, video_id: &str) -> String {
  let matching_thumbnail = renderer.get("thumbnail").and_then(|thumbnail| thumbnail.get("thumbnails")).and_then(|thumbnails| thumbnails.as_array()).and_then(|thumbnails| thumbnails.iter().rev().find_map(|thumbnail| thumbnail.get("url").and_then(|url| url.as_str()).map(normalize_thumbnail_url).filter(|url| thumbnail_matches_video(url, video_id))));
...
fn parse_video_renderer(renderer: &Value) -> Option<YoutubeSearchVideo> {
  ...
  Some(YoutubeSearchVideo { id: id.to_string(), url: format!("https://www.youtube.com/watch?v={id}"), title, thumbnail: Some(best_thumbnail(renderer, id)), duration: run_text(renderer.get("lengthText")), channel: run_text(renderer.get("ownerText")).or_else(|| run_text(renderer.get("longBylineText"))).or_else(|| run_text(renderer.get("shortBylineText"))), view_count_text: run_text(renderer.get("viewCountText")).or_else(|| run_text(renderer.get("shortViewCountText"))), published_time_text: run_text(renderer.get("publishedTimeText")) })
}

fn collect_search_parts(value: &Value, videos: &mut Vec<YoutubeSearchVideo>, continuation: &mut Option<String>) {
  match value {
    Value::Object(map) => {
      ...
      if continuation.is_none() {
        if let Some(token) = map.get("continuationItemRenderer").and_then(|renderer| renderer.get("continuationEndpoint")).and_then(|endpoint| endpoint.get("continuationCommand")).and_then(|command| command.get("token")).and_then(|token| token.as_str()).filter(|token| !token.trim().is_empty()) {
...
fn parse_youtube_search_response(json: &Value) -> YoutubeSearchResponse {
  ...
  YoutubeSearchResponse { videos, continuation }
}

async fn fetch_search_page(client: &reqwest::Client, query: &str, filters: Option<&YoutubeSearchFilters>, continuation: Option<&str>) -> Result<YoutubeSearchResponse, String> {
  let mut body = json!({
      "context": {
          "client": {
  ...
  let response = client.post(YOUTUBE_SEARCH_API_URL).header(reqwest::header::CONTENT_TYPE, "application/json").json(&body).send().await.map_err(|e| {
  ...
  if status != StatusCode::OK {
    return Err(BackendError::from_message(format!("YouTube search returned HTTP {status}")).to_wire_string());
  }

  let json = response.json::<Value>().await.map_err(|e| BackendError::from_message(format!("Failed to parse YouTube search response: {e}")).to_wire_string())?;

  Ok(parse_youtube_search_response(&json))
}

pub async fn search_youtube_videos_internal(query: String, limit: Option<u32>, filters: Option<YoutubeSearchFilters>, continuation: Option<String>) -> Result<YoutubeSearchResponse, String> {
  let query = query.trim().to_string();
  let initial_continuation = continuation.as_deref().map(str::trim).filter(|token| !token.is_empty()).map(str::to_string);
  ...
  let client = reqwest::Client::builder().timeout(std::time::Duration::from_secs(15)).build().map_err(|e| BackendError::from_message(format!("Failed to create YouTube search client: {e}")).to_wire_string())?;

  let mut videos = Vec::new();
  let mut next_continuation = initial_continuation;

  loop {
    let page = fetch_search_page(&client, &query, filters.as_ref(), next_continuation.as_deref()).await?;
    for video in page.videos {
      if !videos.iter().any(|existing: &YoutubeSearchVideo| existing.id == video.id) {
        videos.push(video);
      }
      if videos.len() >= limit {
        break;
  ...
  Ok(YoutubeSearchResponse { videos, continuation: next_continuation })
}
...
mod tests {
  ...
  fn builds_combined_filter_params() {
    let params = build_search_params(Some(&YoutubeSearchFilters { upload_date: Some(YoutubeSearchUploadDateFilter::ThisWeek), duration: Some(YoutubeSearchDurationFilter::Long), sort: Some(YoutubeSearchSortFilter::ViewCount), features: vec![YoutubeSearchFeatureFilter::Hd, YoutubeSearchFeatureFilter::FourK, YoutubeSearchFeatureFilter::Hdr, YoutubeSearchFeatureFilter::Hd] }));
  ...
  fn parses_video_renderer_and_continuation() {
    ...
    assert_eq!(response.videos[0].url, "https://www.youtube.com/watch?v=abc123");
    assert_eq!(response.videos[0].thumbnail.as_deref(), Some("https://i.ytimg.com/vi/abc123/hqdefault.jpg"));
  ...
  fn serializes_response_with_sdk_camel_case_fields() {
    let response = YoutubeSearchResponse { videos: vec![YoutubeSearchVideo { id: "abc123".to_string(), url: "https://www.youtube.com/watch?v=abc123".to_string(), title: "Test Video".to_string(), thumbnail: Some("https://i.ytimg.com/vi/abc123/hqdefault.jpg".to_string()), duration: Some("3:21".to_string()), channel: Some("Channel".to_string()), view_count_text: Some("1,234 views".to_string()), published_time_text: Some("1 day ago".to_string()) }], continuation: Some("next-token".to_string()) };
