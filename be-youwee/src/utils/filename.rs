...
pub fn calc_trim_filenames_bytes(output_path: &str) -> u32 {
  ...
  let available = WINDOWS_MAX_PATH.saturating_sub(path_len).saturating_sub(RESERVED_SUFFIX_BYTES);
...
pub fn add_safe_filename_args(args: &mut Vec<String>, output_path: Option<&str>) {
  ...
  let trim = output_path.map(calc_trim_filenames_bytes).unwrap_or(DEFAULT_TRIM_FILENAMES);
...
mod tests {
  ...
  fn calc_trim_filenames_uses_byte_length_for_non_ascii_paths() {
    ...
    let expected = (WINDOWS_MAX_PATH.saturating_sub(path.as_bytes().len()).saturating_sub(RESERVED_SUFFIX_BYTES) as u32).clamp(MIN_TRIM_FILENAMES, MAX_TRIM_FILENAMES);
