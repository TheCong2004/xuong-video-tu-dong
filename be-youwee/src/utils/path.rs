...
pub fn sanitize_output_path(path: &str) -> Result<String, String> {
  ...
  if !path.exists() {
    std::fs::create_dir_all(path).map_err(|e| format!("Failed to create output directory: {}. Please select a different folder.", e))?;
  }

  // Canonicalize to resolve any symlinks and normalize the path
  let canonical = path.canonicalize().map_err(|e| format!("Invalid output path: {}", e))?;
  ...
  canonical.to_str().ok_or_else(|| "Invalid output path: contains invalid UTF-8".to_string()).map(|s| s.to_string())
}
...
pub fn find_system_binary(binary_name: &str, fallback_dirs: &[PathBuf]) -> Option<PathBuf> {
  system_binary_candidates(binary_name, fallback_dirs).into_iter().find(|path| path.exists())
}
...
pub fn unix_system_binary_dirs() -> Vec<PathBuf> {
  ...
  #[cfg(not(windows))]
  {
    ...
    dirs.extend([PathBuf::from("/opt/homebrew/bin"), PathBuf::from("/usr/local/bin"), PathBuf::from("/usr/bin")]);

    dirs
  }
}

fn push_path_var_candidates(candidates: &mut Vec<PathBuf>, path_var: std::ffi::OsString, binary_name: &str) {
  for dir in std::env::split_paths(&path_var) {
    candidates.push(dir.join(binary_name));
  }
}

fn binary_candidates_from_path_vars(binary_name: &str, path_vars: Vec<std::ffi::OsString>, fallback_dirs: &[PathBuf]) -> Vec<PathBuf> {
...
fn windows_registry_path_values() -> Vec<String> {
  [("HKCU\\Environment", "Path"), ("HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment", "Path")].into_iter().filter_map(|(key, value)| query_windows_registry_value(key, value)).map(|path| expand_windows_env_vars(&path)).collect()
}
  ...
  F: Fn(&str) -> Option<std::ffi::OsString>,
{
  ...
  if let Some(local_app_data) = lookup("LOCALAPPDATA") {
    ...
    dirs.push(local_app_data.join("Microsoft").join("WinGet").join("Links"));
    dirs.extend(windows_python_script_dirs(&local_app_data.join("Programs").join("Python")));
  }
  ...
  if let Some(app_data) = lookup("APPDATA") {
    dirs.extend(windows_python_script_dirs(&PathBuf::from(app_data).join("Python")));
  }
...
mod tests {
  ...
  fn unique_paths_preserves_first_occurrence() {
    let paths = unique_paths(vec![PathBuf::from("/first/bin/tool"), PathBuf::from("/second/bin/tool"), PathBuf::from("/first/bin/tool")]);

    assert_eq!(paths, vec![PathBuf::from("/first/bin/tool"), PathBuf::from("/second/bin/tool")]);
  }

  #[test]
  fn path_entries_are_prioritized_before_fallback_dirs() {
    let path_var = std::env::join_paths([PathBuf::from("/path/bin"), PathBuf::from("/other/bin")]).expect("test paths should join");

    let candidates = binary_candidates_from_path_vars("tool", vec![path_var], &[PathBuf::from("/fallback/bin")]);

    assert_eq!(candidates, vec![PathBuf::from("/path/bin/tool"), PathBuf::from("/other/bin/tool"), PathBuf::from("/fallback/bin/tool"),]);
  }
...
"#;
    ...
    assert_eq!(parse_windows_reg_query_value(output, "Path").as_deref(), Some(r"%USERPROFILE%\scoop\shims;C:\Tools"));
  }

  #[cfg(windows)]
  #[test]
  fn expands_windows_env_vars() {
    let expanded = expand_windows_env_vars_with(r"%USERPROFILE%\scoop\shims;%UNKNOWN%\bin", |name| match name {
  ...
  fn windows_common_dirs_include_package_manager_and_python_paths() {
    let nonce = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).expect("system time should be after epoch").as_nanos();
    ...
    std::fs::create_dir_all(local_app_data.join("Programs").join("Python").join("Python313").join("Scripts")).expect("local Python Scripts directory should be created");
    std::fs::create_dir_all(app_data.join("Python").join("Python312").join("Scripts")).expect("AppData Python Scripts directory should be created");
    ...
    assert!(dirs.contains(&local_app_data.join("Microsoft").join("WinGet").join("Links")));
    assert!(dirs.contains(&program_data.join("chocolatey").join("bin")));
    assert!(dirs.contains(&local_app_data.join("Programs").join("Python").join("Python313").join("Scripts")));
