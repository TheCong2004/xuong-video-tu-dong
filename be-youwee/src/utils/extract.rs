  ...
  for entry in archive.entries().map_err(|e| format!("Failed to read tar: {}", e))? {
    ...
    if let Some(name) = path.file_name() {
      ...
      if name_str == target_binary || name_str == "ffprobe" {
        ...
        entry.unpack(&dest_path).map_err(|e| format!("Failed to extract {}: {}", name_str, e))?;
      }
    }
  }
...
pub fn extract_zip_sync(data: Vec<u8>, dest_dir: PathBuf, target_binary: String) -> Result<(), String> {
  ...
  for i in 0..archive.len() {
    ...
    let file_name = Path::new(&name).file_name().ok_or_else(|| "Invalid file name".to_string())?;
    ...
    if is_target || file_name_str == "ffprobe" || file_name_str == "ffprobe.exe" {
      ...
      let mut outfile = std::fs::File::create(&dest_path).map_err(|e| format!("Failed to create file: {}", e))?;
...
pub async fn extract_tar_gz(data: &[u8], dest_dir: &Path, target_binary: &str) -> Result<(), String> {
  ...
  tokio::task::spawn_blocking(move || extract_tar_gz_sync(data, dest_dir, target_binary)).await.map_err(|e| format!("Task join error: {}", e))?
}
...
pub async fn extract_tar_xz(data: &[u8], dest_dir: &Path, target_binary: &str) -> Result<(), String> {
  ...
  tokio::task::spawn_blocking(move || extract_tar_xz_sync(data, dest_dir, target_binary)).await.map_err(|e| format!("Task join error: {}", e))?
}
...
pub async fn extract_zip(data: &[u8], dest_dir: &Path, target_binary: &str) -> Result<(), String> {
  ...
  tokio::task::spawn_blocking(move || extract_zip_sync(data, dest_dir, target_binary)).await.map_err(|e| format!("Task join error: {}", e))?
}
...
pub async fn extract_deno_zip(data: &[u8], dest_dir: &Path, target_binary: &str) -> Result<(), String> {
  ...
  tokio::task::spawn_blocking(move || extract_deno_zip_sync(data, dest_dir, target_binary)).await.map_err(|e| format!("Task join error: {}", e))?
}
