...
fn firefox_profile_folder_name(path: &str) -> Option<String> {
  path.replace('\\', "/").rsplit('/').find(|segment| !segment.is_empty()).map(str::to_string)
}
...
pub fn firefox_profiles_from_ini(content: &str) -> Vec<FirefoxProfile> {
  ...
  for raw_line in content.lines() {
    ...
    if line.starts_with('[') && line.ends_with(']') {
      ...
      current_profile = if line.starts_with("[Profile") {
        let entry = FirefoxProfileEntry { index: next_index, ..Default::default() };
        next_index += 1;
        Some(entry)
      } else {
        None
  ...
  entries.sort_by_key(|entry| {
    let is_install_default = entry.path.as_ref().is_some_and(|path| install_default_paths.iter().any(|default_path| default_path == path));
  ...
  entries
    ...
    .filter_map(|entry| {
      ...
      let folder_name = entry.path.as_deref().and_then(|path| firefox_profile_cookie_value(path, entry.is_relative))?;
      Some(FirefoxProfile { folder_name, display_name })
    })
    .collect()
}
...
pub fn resolve_firefox_profile_from_ini(content: &str, selected_profile: &str) -> Option<String> {
  ...
  firefox_profiles_from_ini(content).into_iter().find(|profile| profile.folder_name == selected_profile || profile.display_name == selected_profile).map(|profile| profile.folder_name)
}
...
pub fn firefox_profiles_ini_path() -> Option<PathBuf> {
  #[cfg(target_os = "macos")]
  {
    ...
    return Some(PathBuf::from(home).join("Library").join("Application Support").join("Firefox").join("profiles.ini"));
  }

  #[cfg(target_os = "windows")]
  {
    let app_data = std::env::var("APPDATA").ok()?;
    return Some(PathBuf::from(app_data).join("Mozilla").join("Firefox").join("profiles.ini"));
  }

  #[cfg(target_os = "linux")]
  {
    let home = std::env::var("HOME").ok()?;
    return Some(PathBuf::from(home).join(".mozilla").join("firefox").join("profiles.ini"));
  }
...
pub fn resolve_firefox_profile_for_cookies(selected_profile: &str) -> String {
  ...
  std::fs::read_to_string(profiles_ini).ok().and_then(|content| resolve_firefox_profile_from_ini(&content, selected_profile)).unwrap_or_else(|| selected_profile.to_string())
}

#[cfg(test)]
mod tests {
  use super::{firefox_profile_cookie_value, firefox_profile_folder_name, firefox_profiles_from_ini, resolve_firefox_profile_from_ini};

  #[test]
  fn firefox_relative_profile_uses_the_real_directory_name() {
    assert_eq!(firefox_profile_folder_name("Profiles/i879pxds.default-release"), Some("i879pxds.default-release".to_string()));
    assert_eq!(firefox_profile_folder_name(r"Profiles\i879pxds.default-release"), Some("i879pxds.default-release".to_string()));
  }

  #[test]
  fn firefox_absolute_profile_path_is_preserved_for_ytdlp() {
    assert_eq!(firefox_profile_cookie_value(r"C:\Users\Me\AppData\Roaming\Mozilla\Firefox\Profiles\external.default", Some(false),), Some(r"C:\Users\Me\AppData\Roaming\Mozilla\Firefox\Profiles\external.default".to_string()));
  }
...
"#,
    ...
    assert_eq!(profiles[0].folder_name, r"C:\Firefox\Profiles\external.default");
  }
...
"#;
    ...
    assert_eq!(resolve_firefox_profile_from_ini(content, "default-release"), Some("i879pxds.default-release".to_string()));
    assert_eq!(resolve_firefox_profile_from_ini(content, "i879pxds.default-release"), Some("i879pxds.default-release".to_string()));
  }
}
