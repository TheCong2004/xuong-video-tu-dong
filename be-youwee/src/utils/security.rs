...
pub fn normalize_channel_content_urls(url: &str, youtube_content_type: Option<&str>) -> Vec<String> {
  let lower = url.to_lowercase();
  if !lower.contains("youtube.com/") {
  ...
  match youtube_content_type.unwrap_or("videos") {
    "shorts" => normalize_youtube_channel_tab_url(url, "shorts").map(|url| vec![url]).unwrap_or_else(|| vec![normalize_url(url)]),
    "streams" => normalize_youtube_channel_tab_url(url, "streams").map(|url| vec![url]).unwrap_or_else(|| vec![normalize_url(url)]),
    "videos_shorts" => {
    ...
    _ => normalize_youtube_channel_tab_url(url, "videos").map(|url| vec![url]).unwrap_or_else(|| vec![normalize_url(url)]),
  }
}
...
fn normalize_youtube_channel_root_tab_url(url: &str, tab: &str) -> Option<String> {
  ...
  let segments: Vec<&str> = parsed.path_segments().map(|segments| segments.filter(|segment| !segment.is_empty()).collect()).unwrap_or_default();
...
fn normalize_youtube_channel_tab_url(url: &str, tab: &str) -> Option<String> {
  ...
  let channel_segments: Option<&[&str]> = match segments.as_slice() {
    [handle] if handle.starts_with('@') && handle.len() > 1 => Some(&segments[0..1]),
    [handle, current_tab] if handle.starts_with('@') && handle.len() > 1 && is_youtube_channel_content_tab(current_tab) => Some(&segments[0..1]),
    ["channel", channel_id] if !channel_id.is_empty() => Some(&segments[0..2]),
    ["channel", channel_id, current_tab] if !channel_id.is_empty() && is_youtube_channel_content_tab(current_tab) => Some(&segments[0..2]),
    ["c", channel_name] if !channel_name.is_empty() => Some(&segments[0..2]),
    ["c", channel_name, current_tab] if !channel_name.is_empty() && is_youtube_channel_content_tab(current_tab) => Some(&segments[0..2]),
    ["user", user_name] if !user_name.is_empty() => Some(&segments[0..2]),
    ["user", user_name, current_tab] if !user_name.is_empty() && is_youtube_channel_content_tab(current_tab) => Some(&segments[0..2]),
    _ => None,
  };
...
fn normalize_douyin(url: &str) -> Option<String> {
  ...
  let modal_id = query.split('&').filter_map(|pair| pair.split_once('=')).find(|(key, _)| *key == "modal_id").map(|(_, value)| value)?;
...
pub fn parse_ffmpeg_command_args(command: &str) -> Result<Vec<String>, String> {
  ...
  let program = tokens.first().ok_or_else(|| "FFmpeg command cannot be empty".to_string())?;
...
fn is_ffmpeg_program(program: &str) -> bool {
  ...
  std::path::Path::new(normalized).file_name().and_then(|name| name.to_str()).map(|name| name.eq_ignore_ascii_case("ffmpeg") || name.eq_ignore_ascii_case("ffmpeg.exe")).unwrap_or(false)
}
  ...
  while let Some(c) = chars.next() {
    match quote {
      ...
      None => match c {
        ...
        ';' | '|' | '&' | '<' | '>' => {
          return Err(format!("Shell control syntax is not allowed in ffmpeg command: {}", c));
        },
        '`' => {
...
mod tests {
  ...
  fn douyin_user_profile_modal() {
    let input = "https://www.douyin.com/user/self?modal_id=7587779409656007994&showTab=recommend";
    assert_eq!(normalize_url(input), "https://www.douyin.com/video/7587779409656007994");
  }
  ...
  fn douyin_user_profile_with_extra_params() {
    ...
    assert_eq!(normalize_url(input), "https://www.douyin.com/video/7338347129624218919");
  }

  #[test]
  fn douyin_jingxuan_modal() {
    let input = "https://www.douyin.com/jingxuan?modal_id=7612685550327205158";
    assert_eq!(normalize_url(input), "https://www.douyin.com/video/7612685550327205158");
  }
  ...
  fn douyin_jingxuan_search_modal() {
    ...
    assert_eq!(normalize_url(input), "https://www.douyin.com/video/7602194190675655906");
  }

  #[test]
  fn douyin_jingxuan_music_modal() {
    let input = "https://www.douyin.com/jingxuan/music?modal_id=7607040283255016755";
    assert_eq!(normalize_url(input), "https://www.douyin.com/video/7607040283255016755");
  }
  ...
  fn youtube_handle_root_normalizes_to_videos_tab() {
    ...
    assert_eq!(normalize_url(input), "https://www.youtube.com/@channelname/videos");
  }
  ...
  fn youtube_channel_id_root_normalizes_to_videos_tab() {
    ...
    assert_eq!(normalize_url(input), "https://www.youtube.com/channel/UCabc123/videos");
  }

  #[test]
  fn youtube_named_channel_roots_normalize_to_videos_tab() {
    assert_eq!(normalize_url("https://www.youtube.com/c/channelname"), "https://www.youtube.com/c/channelname/videos");
    assert_eq!(normalize_url("https://www.youtube.com/user/channelname"), "https://www.youtube.com/user/channelname/videos");
  }

  #[test]
  fn youtube_channel_tabs_are_unchanged() {
    for input in ["https://www.youtube.com/@channelname/videos", "https://www.youtube.com/@channelname/shorts", "https://www.youtube.com/@channelname/streams", "https://www.youtube.com/@channelname/playlists"] {
      assert_eq!(normalize_url(input), input);
    }
  }
  ...
  fn youtube_channel_content_type_short_normalizes_to_shorts_tab() {
    let input = "https://www.youtube.com/@channelname";
    assert_eq!(normalize_channel_content_urls(input, Some("shorts")), vec!["https://www.youtube.com/@channelname/shorts".to_string()]);
  }

  #[test]
  fn youtube_channel_content_type_streams_normalizes_to_streams_tab() {
    let input = "https://www.youtube.com/@channelname/videos";
    assert_eq!(normalize_channel_content_urls(input, Some("streams")), vec!["https://www.youtube.com/@channelname/streams".to_string()]);
  }

  #[test]
  fn youtube_channel_content_type_videos_shorts_returns_both_tabs() {
    let input = "https://www.youtube.com/channel/UCabc123";
    assert_eq!(normalize_channel_content_urls(input, Some("videos_shorts")), vec!["https://www.youtube.com/channel/UCabc123/videos".to_string(), "https://www.youtube.com/channel/UCabc123/shorts".to_string(),]);
  }

  #[test]
  fn channel_content_type_does_not_touch_non_youtube_urls() {
    let input = "https://space.bilibili.com/12345";
    assert_eq!(normalize_channel_content_urls(input, Some("shorts")), vec![input.to_string()]);
  }
  ...
  fn parse_ffmpeg_command_accepts_quoted_paths_and_filters() {
    let args = parse_ffmpeg_command_args(r#"ffmpeg -y -i "/tmp/Input File.mp4" -filter_complex "[0:v]scale=1280:-1[v];[0:a]anull[a]" -map "[v]" "/tmp/output file.mp4""#).expect("valid ffmpeg command should parse");
  ...
  fn parse_ffmpeg_command_accepts_json_escaped_quoted_paths() {
    let args = parse_ffmpeg_command_args(r#"ffmpeg -y -hwaccel auto -i \"/Users/locnguyen/yt/GIẢI CỨU THỎ BẢY MÀU.mp4\" -vn -c:a libmp3lame -q:a 2 -progress pipe:2 \"/Users/locnguyen/yt/GIẢI CỨU THỎ BẢY MÀU_audio.mp3\""#).expect("AI JSON-escaped quotes should still preserve paths with spaces");

    assert_eq!(args, vec!["-y", "-hwaccel", "auto", "-i", "/Users/locnguyen/yt/GIẢI CỨU THỎ BẢY MÀU.mp4", "-vn", "-c:a", "libmp3lame", "-q:a", "2", "-progress", "pipe:2", "/Users/locnguyen/yt/GIẢI CỨU THỎ BẢY MÀU_audio.mp3",]);
  }

  #[test]
  fn parse_ffmpeg_command_accepts_json_escaped_slashes() {
    let args = parse_ffmpeg_command_args(r#"ffmpeg -y -i "\/Users\/locnguyen\/yt\/ruindkid - Bad Pitch For You.mp4" -filter:a "atempo=2.0" "\/Users\/locnguyen\/yt\/ruindkid - Bad Pitch For You_x2.mp3""#).expect("AI JSON-escaped slashes should be normalized before execution");

    assert_eq!(args, vec!["-y", "-i", "/Users/locnguyen/yt/ruindkid - Bad Pitch For You.mp4", "-filter:a", "atempo=2.0", "/Users/locnguyen/yt/ruindkid - Bad Pitch For You_x2.mp3",]);
  }

  #[test]
  fn parse_ffmpeg_command_preserves_quoted_windows_paths() {
    let args = parse_ffmpeg_command_args(r#"ffmpeg -y -i "C:\Users\locnguyen\Videos\Input File.mp4" "C:\Users\locnguyen\Videos\Output File.mp3""#).expect("quoted Windows paths should preserve backslashes");

    assert_eq!(args, vec!["-y", "-i", r#"C:\Users\locnguyen\Videos\Input File.mp4"#, r#"C:\Users\locnguyen\Videos\Output File.mp3"#,]);
  }

  #[test]
  fn parse_ffmpeg_command_preserves_unquoted_windows_paths() {
    let args = parse_ffmpeg_command_args(r#"ffmpeg -y -i C:\Users\locnguyen\Videos\input.mp4 C:\Users\locnguyen\Videos\output.mp3"#).expect("unquoted Windows paths should preserve backslashes");

    assert_eq!(args, vec!["-y", "-i", r#"C:\Users\locnguyen\Videos\input.mp4"#, r#"C:\Users\locnguyen\Videos\output.mp3"#,]);
  }

  #[test]
  fn parse_ffmpeg_command_keeps_unix_escaped_spaces() {
    let args = parse_ffmpeg_command_args(r#"ffmpeg -y -i /tmp/Input\ File.mp4 /tmp/Output\ File.mp3"#).expect("escaped spaces should still be accepted for Unix-like paths");

    assert_eq!(args, vec!["-y", "-i", "/tmp/Input File.mp4", "/tmp/Output File.mp3"]);
  }

  #[test]
  fn parse_ffmpeg_command_allows_full_ffmpeg_binary_path() {
    let args = parse_ffmpeg_command_args(r#"/usr/local/bin/ffmpeg -i input.mp4 output.mp4"#).expect("ffmpeg basename should be accepted");
  ...
  fn parse_ffmpeg_command_rejects_shell_injection_operators() {
    let err = parse_ffmpeg_command_args("ffmpeg -i input.mp4 output.mp4 && rm -rf /").expect_err("shell operator should be rejected");
  ...
  fn parse_ffmpeg_command_rejects_non_ffmpeg_program() {
    let err = parse_ffmpeg_command_args(r#"bash -c "rm -rf /""#).expect_err("non-ffmpeg command should be rejected");
  ...
  fn parse_ffmpeg_command_rejects_command_substitution() {
    let err = parse_ffmpeg_command_args("ffmpeg -i $(touch /tmp/pwned) output.mp4").expect_err("command substitution should be rejected");
  ...
  fn validate_ffmpeg_args_rejects_shell_operator_arg() {
    let err = validate_ffmpeg_args(&["-i".to_string(), "input.mp4".to_string(), "&&".to_string()]).expect_err("operator arg should be rejected");
