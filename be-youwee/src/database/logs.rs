...
pub fn add_log_internal(log_type: &str, message: &str, details: Option<&str>, url: Option<&str>) -> Result<LogEntry, String> {
  let conn = get_db()?;
  ...
  Ok(LogEntry { id, timestamp, log_type: log_type.to_string(), message: message.to_string(), details: details.map(|s| s.to_string()), url: url.map(|s| s.to_string()) })
}

/// Get logs from database with optional filters
pub fn get_logs_from_db(filter: Option<String>, search: Option<String>, limit: Option<i64>) -> Result<Vec<LogEntry>, String> {
  let conn = get_db()?;
  ...
  let mut query = String::from("SELECT id, timestamp, log_type, message, details, url FROM logs WHERE 1=1");
  ...
  fn parse_row(row: &rusqlite::Row) -> rusqlite::Result<LogEntry> {
    Ok(LogEntry { id: row.get(0)?, timestamp: row.get(1)?, log_type: row.get(2)?, message: row.get(3)?, details: row.get(4)?, url: row.get(5)? })
  }
  ...
  let logs: Vec<LogEntry> = match (filter_active, search_active) {
    (true, true) => {
      ...
      stmt.query_map(params![f, s, limit], parse_row).map_err(|e| format!("Query failed: {}", e))?.filter_map(|r| r.ok()).collect()
    },
    (true, false) => {
      let f = filter.as_ref().unwrap();
      stmt.query_map(params![f, limit], parse_row).map_err(|e| format!("Query failed: {}", e))?.filter_map(|r| r.ok()).collect()
    },
    (false, true) => {
      let s = format!("%{}%", search.as_ref().unwrap());
      stmt.query_map(params![s, limit], parse_row).map_err(|e| format!("Query failed: {}", e))?.filter_map(|r| r.ok()).collect()
    },
    (false, false) => stmt.query_map(params![limit], parse_row).map_err(|e| format!("Query failed: {}", e))?.filter_map(|r| r.ok()).collect(),
  };

  Ok(logs)
}

pub fn get_plugin_logs_from_db(plugin_id: String, limit: Option<i64>, offset: Option<i64>) -> Result<PluginLogsPage, String> {
  let conn = get_db()?;
  let limit = limit.unwrap_or(100).min(500);
  ...
  let logs = stmt.query_map(params![legacy_pattern, runtime_pattern, limit, offset], |row| Ok(LogEntry { id: row.get(0)?, timestamp: row.get(1)?, log_type: row.get(2)?, message: row.get(3)?, details: row.get(4)?, url: row.get(5)? })).map_err(|e| format!("Plugin log query failed: {}", e))?.filter_map(|row| row.ok()).collect();

  Ok(PluginLogsPage { items: logs, total, has_more: offset + limit < total })
}
...
pub fn clear_plugin_logs_from_db(plugin_id: String) -> Result<(), String> {
  ...
  conn.execute("DELETE FROM logs WHERE details LIKE ?1 OR details LIKE ?2", params![legacy_pattern, runtime_pattern]).map_err(|e| format!("Failed to clear plugin logs: {}", e))?;
...
pub fn clear_logs_from_db() -> Result<(), String> {
  let conn = get_db()?;
  conn.execute("DELETE FROM logs", []).map_err(|e| format!("Failed to clear logs: {}", e))?;
  Ok(())
}
