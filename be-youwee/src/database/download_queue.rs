...
pub fn load_download_queue_from_db(queue_kind: String) -> Result<Option<String>, String> {
  ...
  let conn = get_db()?;
  let result = conn.query_row("SELECT items_json FROM download_queues WHERE queue_kind = ?1", params![queue_kind], |row| row.get::<_, String>(0));
...
pub fn clear_download_queue_from_db(queue_kind: String) -> Result<(), String> {
  ...
  let conn = get_db()?;
  conn.execute("DELETE FROM download_queues WHERE queue_kind = ?1", params![queue_kind]).map_err(|e| format!("Failed to clear download queue: {}", e))?;
