use crate::services::{check_deno_internal, check_deno_update_internal, check_ffmpeg_internal, check_ffmpeg_update_internal, check_gallerydl_internal, get_all_ytdlp_versions, get_channel_api_url, get_deno_download_url, get_ffmpeg_download_info, get_ffmpeg_path, get_ffmpeg_source, get_latest_ffmpeg_release_info, get_ytdlp_channel, get_ytdlp_channel_download_url, get_ytdlp_download_info, get_ytdlp_source, get_ytdlp_version_internal, parse_ffmpeg_version, set_ffmpeg_source, set_ytdlp_channel, set_ytdlp_source, system_ffmpeg_upgrade_message, system_ytdlp_upgrade_message, verify_sha256, write_app_ffmpeg_release_version, DenoUpdateInfo, FfmpegUpdateInfo};
use crate::types::{BackendError, DenoStatus, DependencySource, FfmpegStatus, GalleryDlStatus, YtdlpAllVersions, YtdlpChannel, YtdlpChannelUpdateInfo, YtdlpVersionInfo};
use crate::utils::{extract_deno_zip, extract_tar_gz, extract_tar_xz, extract_zip, firefox_profiles_from_ini, CommandExt};
...
pub async fn check_ytdlp_update() -> Result<String, String> {
  let client = reqwest::Client::builder().user_agent("Youwee/0.4.0").timeout(std::time::Duration::from_secs(10)).build().map_err(|e| format!("Failed to create HTTP client: {}", e))?;

  let response = client.get("https://api.github.com/repos/yt-dlp/yt-dlp/releases/latest").send().await.map_err(|e| {
    if e.is_timeout() {
...
pub async fn update_ytdlp(app: AppHandle) -> Result<String, String> {
  ...
  tokio::fs::create_dir_all(&bin_dir).await.map_err(|e| format!("Failed to create bin directory: {}", e))?;

  let binary_path = bin_dir.join(filename);

  let client = reqwest::Client::builder().user_agent("Youwee/0.4.0").timeout(std::time::Duration::from_secs(300)).build().map_err(|e| format!("Failed to create HTTP client: {}", e))?;
  ...
  let checksums_response = client.get(checksums_url).send().await.map_err(|e| format!("Failed to download checksums: {}", e))?;

  if !checksums_response.status().is_success() {
    return Err(format!("Failed to download checksums: HTTP {}", checksums_response.status()));
  }

  let checksums_text = checksums_response.text().await.map_err(|e| format!("Failed to read checksums: {}", e))?;
  ...
  // Download binary
  let response = client.get(download_url).send().await.map_err(|e| format!("Failed to download yt-dlp: {}", e))?;
  ...
  tokio::fs::write(&temp_path, &bytes).await.map_err(|e| format!("Failed to write binary: {}", e))?;

  #[cfg(unix)]
  {
    use std::os::unix::fs::PermissionsExt;
    let mut perms = tokio::fs::metadata(&temp_path).await.map_err(|e| format!("Failed to get file metadata: {}", e))?.permissions();
    perms.set_mode(0o755);
    tokio::fs::set_permissions(&temp_path, perms).await.map_err(|e| format!("Failed to set permissions: {}", e))?;
  }

  tokio::fs::rename(&temp_path, &binary_path).await.map_err(|e| format!("Failed to rename binary: {}", e))?;

  // Get version
  let mut cmd = Command::new(&binary_path);
  cmd.args(["--version"]);
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| format!("Failed to verify update: {}", e))?;
...
pub async fn check_ytdlp_channel_update(app: AppHandle, channel: String) -> Result<YtdlpChannelUpdateInfo, String> {
  ...
  let api_url = get_channel_api_url(&channel_enum).ok_or("Cannot check updates for bundled channel")?;
  ...
  let client = reqwest::Client::builder().user_agent("Youwee/0.6.0").timeout(std::time::Duration::from_secs(10)).build().map_err(|e| format!("Failed to create HTTP client: {}", e))?;
  ...
  let update_available = current_version.as_ref().map(|cv| normalize_version(cv) != normalize_version(&latest_version)).unwrap_or(true); // If not installed, update is available

  Ok(YtdlpChannelUpdateInfo { channel: channel_enum.as_str().to_string(), current_version, latest_version, update_available })
}
...
pub async fn download_ytdlp_channel(app: AppHandle, channel: String) -> Result<String, String> {
  ...
  let (download_url, checksum_filename) = get_ytdlp_channel_download_url(&channel_enum).ok_or("Cannot download bundled channel")?;
  ...
  let client = reqwest::Client::builder().user_agent("Youwee/0.6.0").timeout(std::time::Duration::from_secs(300)).build().map_err(|e| format!("Failed to create HTTP client: {}", e))?;
  ...
  cmd.args(["--version"]);
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| format!("Failed to verify installation: {}", e))?;
...
pub async fn download_ffmpeg(app: AppHandle) -> Result<String, String> {
  ...
  // Emit: Starting
  let _ = app.emit("ffmpeg-download-progress", DownloadProgress { stage: "checksum".to_string(), percent: 0, downloaded: 0, total: 0 });
  ...
  let client = reqwest::Client::builder().user_agent("Youwee/0.6.0").build().map_err(|e| format!("Failed to create HTTP client: {}", e))?;

  // Download checksum file
  let checksum_response = client.get(info.checksum_url).send().await.map_err(|e| format!("Failed to download checksum: {}", e))?;

  if !checksum_response.status().is_success() {
    return Err(format!("Failed to download checksum: HTTP {}", checksum_response.status()));
  }

  let checksum_text = checksum_response.text().await.map_err(|e| format!("Failed to read checksum: {}", e))?;
  ...
  let _ = app.emit("ffmpeg-download-progress", DownloadProgress { stage: "downloading".to_string(), percent: 0, downloaded: 0, total: 0 });

  // Download FFmpeg archive with progress
  let response = client.get(info.url).send().await.map_err(|e| format!("Failed to download FFmpeg: {}", e))?;
  ...
  let mut file = tokio::fs::File::create(&temp_path).await.map_err(|e| format!("Failed to create temp file: {}", e))?;
  ...
  while let Some(chunk) = stream.next().await {
    ...
    if percent >= last_percent + 5 || percent == 100 {
      ...
      let _ = app.emit("ffmpeg-download-progress", DownloadProgress { stage: "downloading".to_string(), percent, downloaded, total: total_size });
    }
  }
  ...
  let bytes = tokio::fs::read(&temp_path).await.map_err(|e| format!("Failed to read downloaded file: {}", e))?;

  // Emit: Verifying checksum
  let _ = app.emit("ffmpeg-download-progress", DownloadProgress { stage: "verifying".to_string(), percent: 100, downloaded, total: total_size });
  ...
  // Emit: Extracting
  let _ = app.emit("ffmpeg-download-progress", DownloadProgress { stage: "extracting".to_string(), percent: 100, downloaded, total: total_size });
  ...
  {
    use std::os::unix::fs::PermissionsExt;
    let mut perms = tokio::fs::metadata(&ffmpeg_path).await.map_err(|e| format!("Failed to get file metadata: {}", e))?.permissions();
    perms.set_mode(0o755);
    tokio::fs::set_permissions(&ffmpeg_path, perms).await.map_err(|e| format!("Failed to set permissions: {}", e))?;
  }

  // Emit: Complete
  let _ = app.emit("ffmpeg-download-progress", DownloadProgress { stage: "complete".to_string(), percent: 100, downloaded, total: total_size });

  let mut cmd = Command::new(&ffmpeg_path);
  cmd.args(["-version"]).stdout(Stdio::piped()).stderr(Stdio::piped());
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| format!("Failed to verify FFmpeg installation: {}", e))?;
...
pub async fn download_deno(app: AppHandle) -> Result<String, String> {
  ...
  // Emit: Starting
  let _ = app.emit("deno-download-progress", DownloadProgress { stage: "downloading".to_string(), percent: 0, downloaded: 0, total: 0 });
  ...
  let response = client.get(download_url).send().await.map_err(|e| format!("Failed to download Deno: {}", e))?;
  ...
  while let Some(chunk) = stream.next().await {
    ...
    if percent >= last_percent + 5 || percent == 100 {
      last_percent = percent;
      let _ = app.emit("deno-download-progress", DownloadProgress { stage: "downloading".to_string(), percent, downloaded, total: total_size });
    }
  }
  ...
  // Emit: Extracting
  let _ = app.emit("deno-download-progress", DownloadProgress { stage: "extracting".to_string(), percent: 100, downloaded, total: total_size });
  ...
  {
    use std::os::unix::fs::PermissionsExt;
    let mut perms = tokio::fs::metadata(&deno_path).await.map_err(|e| format!("Failed to get file metadata: {}", e))?.permissions();
    perms.set_mode(0o755);
    tokio::fs::set_permissions(&deno_path, perms).await.map_err(|e| format!("Failed to set permissions: {}", e))?;
  }

  // Emit: Complete
  let _ = app.emit("deno-download-progress", DownloadProgress { stage: "complete".to_string(), percent: 100, downloaded, total: total_size });

  let mut cmd = Command::new(&deno_path);
  cmd.args(["--version"]).stdout(Stdio::piped()).stderr(Stdio::piped());
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| format!("Failed to verify Deno installation: {}", e))?;

  // Parse version from "deno 2.1.2 (...)" format
  let version_output = String::from_utf8_lossy(&output.stdout);
  let version = version_output.lines().next().map(|l| l.trim_start_matches("deno ").split_whitespace().next().unwrap_or("").to_string()).unwrap_or_default();
...
pub async fn detect_installed_browsers() -> Result<Vec<DetectedBrowser>, String> {
  ...
  #[cfg(target_os = "macos")]
  {
    let browser_checks = [("Google Chrome", "chrome", "/Applications/Google Chrome.app"), ("Firefox", "firefox", "/Applications/Firefox.app"), ("Safari", "safari", "/Applications/Safari.app"), ("Microsoft Edge", "edge", "/Applications/Microsoft Edge.app"), ("Brave", "brave", "/Applications/Brave Browser.app"), ("Opera", "opera", "/Applications/Opera.app"), ("Vivaldi", "vivaldi", "/Applications/Vivaldi.app")];

    for (name, browser_type, path) in browser_checks {
      if std::path::Path::new(path).exists() {
        browsers.push(DetectedBrowser { name: name.to_string(), browser_type: browser_type.to_string() });
      }
    }
  }
  ...
  #[cfg(target_os = "windows")]
  {
    ...
    let browser_checks: [(&str, &str, Vec<String>); 6] = [
      ("Google Chrome", "chrome", vec![format!("{}\\Google\\Chrome\\Application\\chrome.exe", local_app_data), format!("{}\\Google\\Chrome\\Application\\chrome.exe", program_files), format!("{}\\Google\\Chrome\\Application\\chrome.exe", program_files_x86)]),
      ("Firefox", "firefox", vec![format!("{}\\Mozilla Firefox\\firefox.exe", program_files), format!("{}\\Mozilla Firefox\\firefox.exe", program_files_x86)]),
      ("Microsoft Edge", "edge", vec![format!("{}\\Microsoft\\Edge\\Application\\msedge.exe", program_files), format!("{}\\Microsoft\\Edge\\Application\\msedge.exe", program_files_x86)]),
      ("Brave", "brave", vec![format!("{}\\BraveSoftware\\Brave-Browser\\Application\\brave.exe", local_app_data), format!("{}\\BraveSoftware\\Brave-Browser\\Application\\brave.exe", program_files)]),
      ("Opera", "opera", vec![format!("{}\\Programs\\Opera\\opera.exe", local_app_data), format!("{}\\Opera\\opera.exe", program_files)]),
      ("Vivaldi", "vivaldi", vec![format!("{}\\Vivaldi\\Application\\vivaldi.exe", local_app_data)]),
    ];
  ...
  #[cfg(target_os = "linux")]
  {
    let browser_checks = [("Google Chrome", "chrome", vec!["google-chrome", "google-chrome-stable"]), ("Firefox", "firefox", vec!["firefox"]), ("Brave", "brave", vec!["brave-browser", "brave"]), ("Opera", "opera", vec!["opera"]), ("Vivaldi", "vivaldi", vec!["vivaldi", "vivaldi-stable"])];

    for (name, browser_type, commands) in browser_checks {
      for cmd_name in commands {
        let mut cmd = Command::new("which");
        cmd.arg(cmd_name).stdout(Stdio::null()).stderr(Stdio::null());
        cmd.hide_window();
...
pub async fn get_browser_profiles(browser: String) -> Result<Vec<BrowserProfile>, String> {
  ...
  fn get_firefox_profiles(content: &str) -> Vec<BrowserProfile> {
    firefox_profiles_from_ini(content).into_iter().map(|profile| BrowserProfile { folder_name: profile.folder_name, display_name: profile.display_name }).collect()
  }
  ...
  #[cfg(target_os = "macos")]
  {
    ...
    if let Ok(entries) = std::fs::read_dir(&profile_dir) {
      let exclude_dirs = ["Crashpad", "GrShaderCache", "ShaderCache", "BrowserMetrics", "Crowd Deny", "FileTypePolicies", "MEIPreload", "SafetyTips", "SSLErrorAssistant", "Subresource Filter", "WidevineCdm", "extensions", "hyphen-data", "pnacl", "ZxcvbnData", "component_crx_cache", "CertificateRevocation", "OriginTrials", "System Profile", "Guest Profile"];
      ...
      for entry in entries.flatten() {
        ...
        if path.is_dir() {
          ...
          if prefs_path.exists() && !exclude_dirs.contains(&folder_name.as_str()) {
            let display_name = get_chromium_profile_name(&prefs_path).unwrap_or_else(|| folder_name.clone());
            profiles.push(BrowserProfile { folder_name, display_name });
          }
        }
      }
    }
