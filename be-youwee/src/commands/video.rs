...
use crate::services::{build_cookie_args, build_proxy_args, build_site_header_args, get_deno_path, parse_ytdlp_error, run_ytdlp_json_with_cookies, run_ytdlp_with_stderr, run_ytdlp_with_stderr_and_cookies};
use crate::types::{BackendError, FormatOption, PlaylistVideoEntry, SubtitleInfo, VideoInfo, VideoInfoResponse};
...
fn default_transcript_languages(url: &str) -> Vec<String> {
  ...
  if lowered.contains("douyin.com") || lowered.contains("iesdouyin.com") || lowered.contains("bilibili.com") || lowered.contains("b23.tv") {
    return vec!["zh-Hans".to_string(), "zh-CN".to_string(), "zh".to_string(), "en".to_string()];
  }
...
#[tauri::command]
pub async fn download_subtitle_content(app: AppHandle, url: String, lang: String, is_auto: bool, format: String, cookie_mode: Option<String>, cookie_browser: Option<String>, cookie_browser_profile: Option<String>, cookie_file_path: Option<String>, cookie_skip_patterns: Option<Vec<String>>, proxy_url: Option<String>) -> Result<String, String> {
  validate_url(&url)?;
  let temp_dir = tempfile::tempdir().map_err(|error| format!("Failed to create subtitle temporary directory: {error}"))?;
  let output_template = temp_dir.path().join("subtitle.%(ext)s");
  let output_template = output_template.to_string_lossy().to_string();
  let subtitle_flag = if is_auto { "--write-auto-subs" } else { "--write-subs" };
  let requested_format = if format.trim().is_empty() { "srt" } else { format.as_str() };
  let args = ["--skip-download", subtitle_flag, "--sub-langs", lang.as_str(), "--sub-format", requested_format, "--convert-subs", requested_format, "--output", output_template.as_str(), "--", url.as_str()];
  let output = run_ytdlp_with_stderr_and_cookies(&app, &args, cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref(), proxy_url.as_deref()).await?;
  if !output.success {
    return Err(parse_ytdlp_error(&output.stderr).map(|error| error.to_wire_string()).unwrap_or_else(|| output.stderr.trim().to_string()));
  }

  let subtitle_path = std::fs::read_dir(temp_dir.path()).map_err(|error| format!("Failed to inspect downloaded subtitle: {error}"))?.filter_map(Result::ok).map(|entry| entry.path()).find(|path| path.is_file()).ok_or_else(|| "yt-dlp did not produce a subtitle file".to_string())?;
  std::fs::read_to_string(&subtitle_path).map_err(|error| format!("Failed to read downloaded subtitle: {error}"))
}

fn parse_basic_video_info_output(output: &str) -> Result<(String, Option<String>, Option<f64>), String> {
  let line = output.lines().map(str::trim).find(|line| !line.is_empty()).ok_or_else(|| "Failed to fetch video information".to_string())?;
  ...
  let thumbnail = if thumbnail.is_empty() || thumbnail == "NA" { None } else { Some(thumbnail.to_string()) };

  let duration = if duration.is_empty() || duration == "NA" { None } else { duration.parse::<f64>().ok() };

  Ok((title.to_string(), thumbnail, duration))
}

fn json_string(value: &serde_json::Value, keys: &[&str]) -> Option<String> {
  keys.iter().find_map(|key| value.get(*key).and_then(|v| v.as_str())).map(str::trim).filter(|value| !value.is_empty()).map(ToString::to_string)
}
...
fn playlist_entry_from_json(json: &serde_json::Value, fallback_playlist_title: Option<&str>) -> Option<PlaylistVideoEntry> {
  ...
  let video_url = json_string(json, &["url", "webpage_url"]).unwrap_or_else(|| format!("https://www.youtube.com/watch?v={}", id));

  let thumbnail = json.get("thumbnail").or_else(|| json.get("thumbnails").and_then(|t| t.as_array()).and_then(|arr| arr.first())).and_then(|v| if v.is_string() { v.as_str().map(|s| s.to_string()) } else { v.get("url").and_then(|u| u.as_str()).map(|s| s.to_string()) });

  let duration = json.get("duration").and_then(|v| v.as_f64());
  let channel = json_string(json, &["channel", "uploader"]);
  let upload_date = json_string(json, &["upload_date"]);
  let playlist_title = playlist_title_from_entry_json(json).or_else(|| fallback_playlist_title.map(ToString::to_string));

  Some(PlaylistVideoEntry { id, title, url: video_url, thumbnail, duration, channel, upload_date, playlist_title })
}

fn parse_playlist_entries_output(output: &str, fallback_playlist_title: Option<&str>) -> Vec<PlaylistVideoEntry> {
  ...
  if let Ok(json) = serde_json::from_str::<serde_json::Value>(trimmed) {
    if let Some(entries) = json.get("entries").and_then(|value| value.as_array()) {
      let playlist_title = playlist_title_from_json(&json).or(fallback_playlist_title.filter(|title| !title.trim().is_empty()).map(ToString::to_string));

      return entries.iter().filter_map(|entry| playlist_entry_from_json(entry, playlist_title.as_deref())).collect();
    }
  }

  let mut entries = Vec::new();
  let mut playlist_title = fallback_playlist_title.filter(|title| !title.trim().is_empty()).map(ToString::to_string);
...
#[tauri::command]
pub async fn get_video_transcript(app: AppHandle, url: String, languages: Option<Vec<String>>, cookie_mode: Option<String>, cookie_browser: Option<String>, cookie_browser_profile: Option<String>, cookie_file_path: Option<String>, cookie_skip_patterns: Option<Vec<String>>, proxy_url: Option<String>) -> Result<String, String> {
  ...
  for (idx, lang) in lang_list.iter().enumerate() {
    ...
    let mut subtitle_args: Vec<String> = vec!["--skip-download".to_string(), "--no-playlist".to_string(), "--ignore-no-formats-error".to_string(), "--write-auto-subs".to_string(), "--write-subs".to_string(), "--sub-langs".to_string(), lang.clone(), "--convert-subs".to_string(), "vtt".to_string(), "-o".to_string(), temp_path_str.clone(), "--no-warnings".to_string(), "--no-check-certificates".to_string(), "--no-cache-dir".to_string(), "--socket-timeout".to_string(), "30".to_string()];
    ...
    let subtitle_result = timeout(Duration::from_secs(45), run_ytdlp_with_stderr_and_cookies(&app, &subtitle_args_ref, cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref(), proxy_url.as_deref())).await;
  ...
  let info_result = timeout(
    ...
    run_ytdlp_json_with_cookies(&app, &info_args.iter().map(|s| *s).collect::<Vec<_>>(), cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref(), proxy_url.as_deref()),
  )
  .await;
  ...
  if !rate_limited {
    let metadata_args = vec!["--dump-json", "--no-download", "--no-playlist", "--no-warnings", "--no-cache-dir", "--socket-timeout", "30", "--", &url_for_info];
    let metadata_result = timeout(Duration::from_secs(45), run_ytdlp_json_with_cookies(&app, &metadata_args.iter().copied().collect::<Vec<_>>(), cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref(), proxy_url.as_deref())).await;
  ...
  let error_msg = if rate_limited {
    ...
    BackendError::from_message("No transcript available. This video has no subtitles, auto-generated captions, or meaningful description to summarize.")
  };
...
fn is_description_content_relevant(title: &str, description: &str) -> bool {
  ...
  let promo_indicators = ["subscribe", "đăng ký", "follow me", "theo dõi", "business inquiries", "liên hệ công việc", "copyright", "bản quyền", "patreon", "paypal", "donate", "merch", "merchandise", "social media", "mạng xã hội"];

  let promo_count = promo_indicators.iter().filter(|&ind| desc_lower.contains(ind)).count();
  ...
  let text_without_links: String = description.lines().filter(|line| !line.contains("http") && !line.contains("www.")).collect::<Vec<_>>().join("\n");
  ...
  let title_words: Vec<&str> = title_lower.split_whitespace().filter(|w| w.len() > 3).collect();

  if !title_words.is_empty() {
    let matching_words = title_words.iter().filter(|&w| desc_lower.contains(w)).count();
...
fn build_metadata_fallback_from_info_json(json: &serde_json::Value) -> Option<String> {
  let title = json.get("title").and_then(|v| v.as_str()).unwrap_or("").trim();
  let description = json.get("description").and_then(|v| v.as_str()).unwrap_or("").trim();
  ...
  let uploader = json.get("uploader").and_then(|v| v.as_str()).or_else(|| json.get("channel").and_then(|v| v.as_str())).unwrap_or("").trim();
  ...
  Some(format!("[Video Metadata - No subtitles available]\n{}", lines.join("\n")))
}
...
fn parse_subtitle_file(content: &str) -> String {
  ...
  for line in content.lines() {
    ...
    let clean_line = regex::Regex::new(r"<[^>]+>").map(|re| re.replace_all(line, "").to_string()).unwrap_or_else(|_| line.to_string());
...
#[tauri::command]
pub async fn get_video_basic_info(app: AppHandle, url: String, cookie_mode: Option<String>, cookie_browser: Option<String>, cookie_browser_profile: Option<String>, cookie_file_path: Option<String>, cookie_skip_patterns: Option<Vec<String>>, proxy_url: Option<String>) -> Result<VideoInfoResponse, String> {
  validate_url(&url).map_err(|e| BackendError::from_message(e).to_wire_string())?;
  let url = normalize_url(&url);

  let mut args = vec!["--skip-download".to_string(), "--no-warnings".to_string(), "--no-simulate".to_string(), "--no-playlist".to_string(), "--ignore-no-formats-error".to_string(), "--socket-timeout".to_string(), "15".to_string()];
  ...
  let output = match timeout(Duration::from_secs(45), run_ytdlp_with_stderr_and_cookies(&app, &args_ref, cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref(), proxy_url.as_deref())).await {
    Ok(result) => result?,
    Err(_) => {
      let error = BackendError::from_message("Timed out fetching video info. Please try again or check your cookie/proxy settings.");
  ...
  let (title, thumbnail, duration) = parse_basic_video_info_output(&output.stdout).map_err(|e| BackendError::from_message(e).to_wire_string())?;

  let info = VideoInfo { id: String::new(), title, thumbnail, duration, channel: None, uploader: None, upload_date: None, view_count: None, description: None, is_playlist: false, playlist_count: None, extractor: None, extractor_key: None, is_live: None, was_live: None, live_status: None };

  add_log_internal("info", &format!("Fetched basic video info - title: '{}'", info.title), None, Some(&url)).ok();

  Ok(VideoInfoResponse { info, formats: Vec::new() })
}

#[tauri::command]
pub async fn get_video_info(app: AppHandle, url: String, cookie_mode: Option<String>, cookie_browser: Option<String>, cookie_browser_profile: Option<String>, cookie_file_path: Option<String>, cookie_skip_patterns: Option<Vec<String>>, proxy_url: Option<String>) -> Result<VideoInfoResponse, String> {
  validate_url(&url).map_err(|e| BackendError::from_message(e).to_wire_string())?;
  let url = normalize_url(&url);

  let mut args = vec!["--dump-json".to_string(), "--no-download".to_string(), "--no-playlist".to_string(), "--ignore-no-formats-error".to_string(), "--no-warnings".to_string(), "--socket-timeout".to_string(), "15".to_string()];
  ...
  extra_args.extend(build_cookie_args(&url, cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref()));
  ...
  let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
  let output = match timeout(Duration::from_secs(45), run_ytdlp_with_stderr(&app, &args_ref)).await {
    Ok(result) => result?,
    Err(_) => {
      let error = BackendError::from_message("Timed out fetching video info. Please try again or check your cookie/proxy settings.");
      add_log_internal("error", error.message(), None, Some(&url)).ok();
  ...
  let playlist_count = if is_playlist { json.get("playlist_count").and_then(|v| v.as_u64()).map(|v| v as u32) } else { None };
  ...
  let info = VideoInfo {
    ...
    description: json.get("description").and_then(|v| v.as_str()).map(|s| if s.len() > 200 { format!("{}...", &s[..200]) } else { s.to_string() }),
    is_playlist,
    playlist_count,
  ...
  let formats = if let Some(formats_arr) = json.get("formats").and_then(|v| v.as_array()) {
    formats_arr
      ...
      .filter_map(|f| {
        ...
        Some(FormatOption { format_id: format_id.to_string(), ext: ext.to_string(), resolution: f.get("resolution").and_then(|v| v.as_str()).map(|s| s.to_string()), width: f.get("width").and_then(|v| v.as_u64()).map(|v| v as u32), height: f.get("height").and_then(|v| v.as_u64()).map(|v| v as u32), vcodec: f.get("vcodec").and_then(|v| v.as_str()).map(|s| s.to_string()), acodec: f.get("acodec").and_then(|v| v.as_str()).map(|s| s.to_string()), filesize: f.get("filesize").and_then(|v| v.as_u64()), filesize_approx: f.get("filesize_approx").and_then(|v| v.as_u64()), tbr: f.get("tbr").and_then(|v| v.as_f64()), format_note: f.get("format_note").and_then(|v| v.as_str()).map(|s| s.to_string()), fps: f.get("fps").and_then(|v| v.as_f64()), quality: f.get("quality").and_then(|v| v.as_f64()) })
      })
      .collect()
  } else {
    Vec::new()
...
#[tauri::command]
pub async fn get_playlist_entries(app: AppHandle, url: String, limit: Option<u32>, cookie_mode: Option<String>, cookie_browser: Option<String>, cookie_browser_profile: Option<String>, cookie_file_path: Option<String>, cookie_skip_patterns: Option<Vec<String>>, proxy_url: Option<String>) -> Result<Vec<PlaylistVideoEntry>, String> {
  validate_url(&url).map_err(|e| BackendError::from_message(e).to_wire_string())?;
  let url = normalize_url(&url);

  let mut args = vec!["--flat-playlist".to_string(), "--dump-single-json".to_string(), "--no-warnings".to_string(), "--socket-timeout".to_string(), "30".to_string()];
  ...
  // Add cookie args
  let cookie_args = build_cookie_args(&url, cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref());
...
#[tauri::command]
pub async fn get_available_subtitles(app: AppHandle, url: String, cookie_mode: Option<String>, cookie_browser: Option<String>, cookie_browser_profile: Option<String>, cookie_file_path: Option<String>, cookie_skip_patterns: Option<Vec<String>>, proxy_url: Option<String>) -> Result<Vec<SubtitleInfo>, String> {
  validate_url(&url).map_err(|e| BackendError::from_message(e).to_wire_string())?;
  let url = normalize_url(&url);

  let mut args = vec!["--list-subs".to_string(), "--skip-download".to_string(), "--no-warnings".to_string()];
  ...
  let output = run_ytdlp_json_with_cookies(&app, &args_ref, cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref(), proxy_url.as_deref()).await;

  let mut subtitles: Vec<SubtitleInfo> = Vec::new();

  let lang_names: std::collections::HashMap<&str, &str> = [("en", "English"), ("vi", "Vietnamese"), ("ja", "Japanese"), ("ko", "Korean"), ("zh", "Chinese"), ("zh-Hans", "Chinese (Simplified)"), ("zh-Hant", "Chinese (Traditional)"), ("th", "Thai"), ("id", "Indonesian"), ("ms", "Malay"), ("fr", "French"), ("de", "German"), ("es", "Spanish"), ("pt", "Portuguese"), ("ru", "Russian"), ("ar", "Arabic"), ("hi", "Hindi"), ("it", "Italian"), ("nl", "Dutch"), ("pl", "Polish"), ("tr", "Turkish"), ("uk", "Ukrainian")].iter().cloned().collect();
  ...
  if let Ok(text) = output {
    ...
    for line in text.lines() {
      ...
      if let Some(lang_code) = parts.first() {
        ...
        let name = lang_names.get(lang.as_str()).map(|s| s.to_string()).unwrap_or_else(|| lang.clone());

        subtitles.push(SubtitleInfo { lang, name, is_auto: is_auto_section });
      }
    }
  }

  if subtitles.is_empty() {
    subtitles = vec![SubtitleInfo { lang: "en".to_string(), name: "English".to_string(), is_auto: false }, SubtitleInfo { lang: "vi".to_string(), name: "Vietnamese".to_string(), is_auto: false }, SubtitleInfo { lang: "ja".to_string(), name: "Japanese".to_string(), is_auto: false }, SubtitleInfo { lang: "es".to_string(), name: "Spanish".to_string(), is_auto: false }, SubtitleInfo { lang: "ko".to_string(), name: "Korean".to_string(), is_auto: false }, SubtitleInfo { lang: "zh".to_string(), name: "Chinese".to_string(), is_auto: false }];
  }
...
mod tests {
  ...
  fn parse_basic_video_info_output_reads_printed_fields() {
    let output = concat!("Кто попадет в команду ChatGPT: учёный или программист? (Ответ тебя удивит)", "|||https://i.ytimg.com/vi/ePPilLkDn0s/maxresdefault.jpg|||3623\n");

    let (title, thumbnail, duration) = parse_basic_video_info_output(output).unwrap();

    assert_eq!(title, "Кто попадет в команду ChatGPT: учёный или программист? (Ответ тебя удивит)");
    assert_eq!(thumbnail.as_deref(), Some("https://i.ytimg.com/vi/ePPilLkDn0s/maxresdefault.jpg"));
    assert_eq!(duration, Some(3623.0));
  }

  #[test]
  fn parse_basic_video_info_output_ignores_missing_optional_fields() {
    let (title, thumbnail, duration) = parse_basic_video_info_output("Video title|||NA|||NA").unwrap();
  ...
  fn parse_playlist_entries_output_keeps_line_json_entry_playlist_title() {
    let output = concat!(r#"{"id":"abc123","title":"First video","url":"https://youtu.be/abc123","playlist_title":"Line Playlist"}"#, "\n", r#"{"id":"def456","title":"Second video","url":"https://youtu.be/def456"}"#, "\n");
    ...
    assert_eq!(entries[1].playlist_title.as_deref(), Some("Fallback Playlist"));
  }
}
