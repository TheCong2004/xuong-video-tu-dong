...
use crate::database::{add_history_internal, assign_history_collections_in_db, delete_history_from_db, ensure_collection_for_download_in_db};
...
fn unique_path(path: PathBuf) -> PathBuf {
  ...
  let stem = path.file_stem().map(|value| value.to_string_lossy().to_string()).unwrap_or_else(|| "segment".to_string());
...
#[tauri::command]
pub async fn split_media_segments(app: AppHandle, request: MediaSplitRequest) -> Result<MediaSplitResult, String> {
  let ffmpeg_path = get_ffmpeg_path(&app).await.ok_or_else(|| "FFmpeg is required to split media files.".to_string())?;
  ...
  let parent_dir = input_path.parent().ok_or_else(|| "Cannot determine media folder".to_string())?;
  ...
  tokio::fs::create_dir_all(&output_dir).await.map_err(|e| format!("Failed to create segment folder: {}", e))?;

  let extension = input_path.extension().map(|value| value.to_string_lossy().to_string()).filter(|value| !value.trim().is_empty()).unwrap_or_else(|| request.format.clone().unwrap_or_else(|| "mp4".to_string()).trim_start_matches('.').to_string());

  let collection_id = if request.auto_collection.unwrap_or(false) { Some(ensure_collection_for_download_in_db(&request.parent_title, Some("#f43f7f".to_string())).map(|collection| collection.id)?) } else { None };
  ...
  for (index, segment) in request.segments.iter().enumerate() {
    ...
    cmd.args(["-hide_banner", "-nostdin", "-y", "-ss", &start, "-to", &end, "-i"]).arg(&input_path).args(["-map", "0", "-c", "copy", "-avoid_negative_ts", "make_zero"]).arg(&output_path).stdout(Stdio::piped()).stderr(Stdio::piped());
    cmd.hide_window();
    ...
    if !output.status.success() {
      ...
      return Err(format!("FFmpeg failed while splitting {}: {}", segment_name, stderr.lines().last().unwrap_or("unknown error")));
    }

    let filesize = tokio::fs::metadata(&output_path).await.ok().map(|m| m.len());
    let filepath = output_path.to_string_lossy().to_string();
    let duration = (segment.end_seconds - segment.start_seconds).round().max(0.0) as u64;
    let time_range = format!("{start}-{end}");
    let history_id = match add_history_internal(request.source_url.clone(), segment_name.clone(), request.thumbnail.clone(), filepath.clone(), filesize, Some(duration), request.quality.clone(), Some(extension.clone()), request.source.clone(), Some(time_range.clone())) {
    ...
    if let Some(collection_id) = &collection_id {
      if let Err(error) = assign_history_collections_in_db(history_id.clone(), vec![collection_id.clone()]) {
        rollback_split_outputs(&created_output_paths, &created_history_ids).await;
        return Err(error);
      }
    }

    results.push(MediaSplitSegmentResult { history_id, title: segment_name, filepath, filesize, duration, time_range });
  }

  if request.delete_original.unwrap_or(false) {
    let metadata = tokio::fs::symlink_metadata(&input_path).await.map_err(|e| format!("Failed to inspect original file before deleting: {}", e))?;
    if metadata.is_dir() {
      return Err("Refusing to delete original media because it is a directory".to_string());
    }
    tokio::fs::remove_file(&input_path).await.map_err(|e| format!("Failed to delete original media file: {}", e))?;
  }

  Ok(MediaSplitResult { output_dir: output_dir.to_string_lossy().to_string(), segments: results })
}
