...
use crate::services::{add_safe_filename_args, build_cookie_args, build_site_header_args, get_deno_path, get_ffmpeg_path, get_ytdlp_path, get_ytdlp_source, run_ytdlp_with_stderr_and_cookies, search_youtube_videos_internal, system_ytdlp_not_found_message};
...
fn parse_input_lines(text: &str) -> Vec<String> {
  text.lines().map(|line| line.trim()).filter(|line| !line.is_empty() && !line.starts_with('#')).map(|line| line.to_string()).collect()
}

fn detect_platform(source: &ExportSource, extractor: Option<&str>, url: Option<&str>) -> String {
  let haystack = format!("{} {}", extractor.unwrap_or_default().to_lowercase(), url.unwrap_or_default().to_lowercase());
  ...
  if haystack.contains("youtube") || haystack.contains("youtu.be") {
    ...
    match source {
      ...
      ExportSource::YoutubePlaylist | ExportSource::YoutubeChannel | ExportSource::YoutubeKeyword => "youtube".to_string(),
...
fn string_field(json: &serde_json::Value, keys: &[&str]) -> Option<String> {
  keys.iter().find_map(|key| json.get(*key).and_then(|value| value.as_str())).filter(|value| !value.is_empty()).map(|value| value.to_string())
}

fn i64_field(json: &serde_json::Value, keys: &[&str]) -> Option<i64> {
  keys.iter().find_map(|key| json.get(*key).and_then(|value| value.as_i64()))
}

fn f64_field(json: &serde_json::Value, keys: &[&str]) -> Option<f64> {
  keys.iter().find_map(|key| json.get(*key).and_then(|value| value.as_f64()))
}

fn tags_field(json: &serde_json::Value) -> Option<Vec<String>> {
  json.get("tags").and_then(|value| value.as_array()).map(|tags| tags.iter().filter_map(|tag| tag.as_str()).filter(|tag| !tag.is_empty()).map(|tag| tag.to_string()).collect::<Vec<_>>()).filter(|tags| !tags.is_empty())
}

fn thumbnail_field(json: &serde_json::Value) -> Option<String> {
  string_field(json, &["thumbnail"]).or_else(|| json.get("thumbnails").and_then(|value| value.as_array()).and_then(|thumbnails| thumbnails.iter().rev().find_map(|item| item.get("url"))).and_then(|value| value.as_str()).map(|value| value.to_string())).map(|url| url.replace("http://", "https://"))
}
...
fn parse_view_count_text(value: Option<&str>) -> Option<i64> {
  ...
  let numeric = value.chars().skip_while(|ch| !ch.is_ascii_digit()).take_while(|ch| ch.is_ascii_digit() || *ch == '.' || *ch == ',').collect::<String>();
  ...
  if (multiplier - 1f64).abs() < f64::EPSILON {
    let digits = numeric.chars().filter(|ch| ch.is_ascii_digit()).collect::<String>();
    return digits.parse::<i64>().ok();
  }

  let normalized = numeric.replace(',', ".");
  normalized.parse::<f64>().ok().map(|count| (count * multiplier).round() as i64)
}

fn row_from_youtube_search_video(video: YoutubeSearchVideo) -> ExportRow {
  let metadata_text = [video.view_count_text.clone(), video.published_time_text.clone()].into_iter().flatten().filter(|value| !value.is_empty()).collect::<Vec<_>>().join(" • ");

  ExportRow { id: video.id, title: Some(video.title), url: Some(video.url), platform: Some("youtube".to_string()), uploader: video.channel, thumbnail: video.thumbnail, duration_seconds: duration_text_to_seconds(video.duration.as_deref()), upload_date: video.published_time_text, timestamp: None, view_count: parse_view_count_text(video.view_count_text.as_deref()), like_count: None, comment_count: None, share_count: None, description: if metadata_text.is_empty() { None } else { Some(metadata_text) }, tags: None, playlist_index: None, extractor: Some("youtube_search".to_string()) }
}
...
fn row_from_json(source: &ExportSource, json: &serde_json::Value, _source_url: &str) -> Option<ExportRow> {
  let id = string_field(json, &["id", "display_id"])?;
  let extractor = string_field(json, &["extractor_key", "extractor"]);
  let webpage_url = string_field(json, &["webpage_url", "original_url"]).or_else(|| string_field(json, &["url"]).and_then(|url| if url.starts_with("http://") || url.starts_with("https://") { Some(url) } else { fallback_url(&id) })).or_else(|| fallback_url(&id));
  let uploader = string_field(json, &["channel", "uploader", "creator", "artist"]);

  Some(ExportRow { id, title: string_field(json, &["title", "fulltitle"]), url: webpage_url.clone(), platform: Some(detect_platform(source, extractor.as_deref(), webpage_url.as_deref())), uploader, thumbnail: thumbnail_field(json), duration_seconds: f64_field(json, &["duration"]), upload_date: string_field(json, &["upload_date", "release_date"]), timestamp: i64_field(json, &["timestamp", "release_timestamp", "modified_timestamp"]), view_count: i64_field(json, &["view_count", "play_count"]), like_count: i64_field(json, &["like_count", "repost_count"]), comment_count: i64_field(json, &["comment_count"]), share_count: i64_field(json, &["share_count"]), description: string_field(json, &["description"]), tags: tags_field(json), playlist_index: i64_field(json, &["playlist_index"]), extractor })
}

fn parse_ytdlp_rows(source: &ExportSource, output: &str, source_url: &str) -> (Vec<ExportRow>, Option<String>) {
  ...
  for line in output.lines() {
    ...
    if let Ok(json) = serde_json::from_str::<serde_json::Value>(line) {
      if title.is_none() {
        title = string_field(&json, &["playlist_title", "playlist", "channel", "uploader"]);
      }
...
async fn run_export_ytdlp(app: &AppHandle, source: &ExportSource, url: &str, limit: Option<u32>, detail_mode: bool, input: &ExtractDataRowsInput) -> Result<(Vec<ExportRow>, Option<String>), String> {
  ...
  let mut args = vec!["--dump-json".to_string(), "--no-warnings".to_string(), "--socket-timeout".to_string(), "30".to_string()];
  ...
  let output = run_ytdlp_with_stderr_and_cookies(app, &arg_refs, input.cookie_mode.as_deref(), input.cookie_browser.as_deref(), input.cookie_browser_profile.as_deref(), input.cookie_file_path.as_deref(), input.cookie_skip_patterns.as_deref(), input.proxy_url.as_deref()).await?;

  if !output.success && output.stdout.trim().is_empty() {
    let detail = output.stderr.lines().rev().find(|line| !line.trim().is_empty()).unwrap_or("yt-dlp exited with error");
...
async fn run_export_youtube_keyword(query: &str, limit: Option<u32>) -> Result<Vec<ExportRow>, String> {
  let response = search_youtube_videos_internal(query.to_string(), limit, None, None).await?;
  Ok(response.videos.into_iter().map(row_from_youtube_search_video).collect())
}

#[tauri::command]
pub async fn extract_data_rows(app: AppHandle, input: ExtractDataRowsInput) -> Result<ExtractDataRowsOutput, String> {
  ...
  for line in lines {
    ...
    let export_result = if matches!(effective_source, ExportSource::YoutubeKeyword) { run_export_youtube_keyword(&line, input.limit).await.map(|rows| (rows, Some(line.clone()))) } else { run_export_ytdlp(&app, &effective_source, &line, input.limit, input.detail_mode, &input).await };
  ...
  Ok(ExtractDataRowsOutput { source: export_source_key(&input.source).to_string(), title, rows: all_rows, warnings })
}

fn sqlite_identifier(value: &str) -> String {
  let mut result = value.chars().map(|ch| if ch.is_ascii_alphanumeric() { ch.to_ascii_lowercase() } else { '_' }).collect::<String>();
...
fn json_export_value_to_string(value: Option<&serde_json::Value>) -> String {
  match value {
    ...
    Some(serde_json::Value::Array(values)) => values.iter().map(|value| json_export_value_to_string(Some(value))).filter(|value| !value.is_empty()).collect::<Vec<_>>().join(", "),
    Some(value) => value.to_string(),
  }
}

#[tauri::command]
pub fn export_data_rows_sqlite(file_path: String, columns: Vec<String>, rows: Vec<serde_json::Map<String, serde_json::Value>>) -> Result<(), String> {
  if columns.is_empty() {
    return Err(BackendError::from_message("No export fields selected").to_wire_string());
  }

  if Path::new(&file_path).exists() {
    std::fs::remove_file(&file_path).map_err(|error| BackendError::from_message(error.to_string()).to_wire_string())?;
  }

  let conn = Connection::open(&file_path).map_err(|error| BackendError::from_message(error.to_string()).to_wire_string())?;

  let sql_columns = columns.iter().map(|column| sqlite_identifier(column)).collect::<Vec<_>>();
  let create_columns = sql_columns.iter().map(|column| format!("{} TEXT", sqlite_quote_identifier(column))).collect::<Vec<_>>().join(", ");
  conn.execute(&format!("CREATE TABLE export_rows ({})", create_columns), []).map_err(|error| BackendError::from_message(error.to_string()).to_wire_string())?;

  let column_list = sql_columns.iter().map(|column| sqlite_quote_identifier(column)).collect::<Vec<_>>().join(", ");
  let placeholders = (0..columns.len()).map(|_| "?").collect::<Vec<_>>().join(", ");
  let insert_sql = format!("INSERT INTO export_rows ({}) VALUES ({})", column_list, placeholders);

  let tx = conn.unchecked_transaction().map_err(|error| BackendError::from_message(error.to_string()).to_wire_string())?;
  {
    let mut statement = tx.prepare(&insert_sql).map_err(|error| BackendError::from_message(error.to_string()).to_wire_string())?;

    for row in rows {
      let values = columns.iter().map(|column| json_export_value_to_string(row.get(column))).collect::<Vec<_>>();
      statement.execute(params_from_iter(values)).map_err(|error| BackendError::from_message(error.to_string()).to_wire_string())?;
    }
  }
  tx.commit().map_err(|error| BackendError::from_message(error.to_string()).to_wire_string())?;

  Ok(())
}

/// Split comments from info.json into separate files
fn split_info_json_and_comments(output_dir: &str, title: &str, write_info_json: bool, write_comments: bool) -> Result<(), String> {
  ...
  let content = std::fs::read_to_string(&info_json_path).map_err(|e| format!("Failed to read info.json: {}", e))?;

  let mut json: serde_json::Value = serde_json::from_str(&content).map_err(|e| format!("Failed to parse info.json: {}", e))?;
  ...
  if write_comments {
    if let Some(ref comments_data) = comments {
      ...
      let comments_str = serde_json::to_string_pretty(&comments_json).map_err(|e| format!("Failed to serialize comments: {}", e))?;
      std::fs::write(&comments_path, comments_str).map_err(|e| format!("Failed to write comments.json: {}", e))?;
    }
  }
  ...
  if write_info_json && write_comments {
    ...
    let clean_info = serde_json::to_string_pretty(&json).map_err(|e| format!("Failed to serialize info.json: {}", e))?;
    std::fs::write(&info_json_path, clean_info).map_err(|e| format!("Failed to write clean info.json: {}", e))?;
...
pub async fn fetch_metadata(
  ...
  app.emit("metadata-progress", MetadataProgress { id: id.clone(), status: "fetching".to_string(), title: None, thumbnail: None, error_message: None, error_code: None, error_params: None }).ok();
  ...
  if let Some((binary_path, is_bundled)) = get_ytdlp_path(&app).await {
    ...
    let mut process = cmd.spawn().map_err(|e| BackendError::from_message(format!("Failed to start yt-dlp: {}", e)).to_wire_string())?;

    let stdout = process.stdout.take().ok_or_else(|| BackendError::from_message("Failed to capture stdout").to_wire_string())?;
    let stderr = process.stderr.take().ok_or_else(|| BackendError::from_message("Failed to capture stderr").to_wire_string())?;
    ...
    let status = process.wait().await.map_err(|e| BackendError::from_message(format!("Process error: {}", e)).to_wire_string())?;
    ...
    if status.success() {
      ...
      if write_comments || (write_info_json && write_comments) {
        if let Err(e) = split_info_json_and_comments(&sanitized_path, &title, write_info_json, write_comments) {
          add_log_internal("stderr", &format!("Post-process warning: {}", e), None, Some(&url)).ok();
        }
      }
      ...
      app.emit("metadata-progress", MetadataProgress { id: id.clone(), status: "finished".to_string(), title: video_title, thumbnail: video_thumbnail, error_message: None, error_code: None, error_params: None }).ok();
      Ok(())
    } else {
      ...
      app.emit("metadata-progress", MetadataProgress { id: id.clone(), status: "error".to_string(), title: video_title, thumbnail: video_thumbnail, error_message: Some(err_msg.clone()), error_code: Some(backend_err.code().to_string()), error_params: backend_err.params().cloned() }).ok();
      Err(backend_err.to_wire_string())
    }
  } else {
    let err_msg = if get_ytdlp_source(&app).await == DependencySource::System { system_ytdlp_not_found_message() } else { "yt-dlp not found".to_string() };
    add_log_internal("error", &err_msg, None, Some(&url)).ok();
