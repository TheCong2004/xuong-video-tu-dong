...
use crate::database::{add_history_internal, add_history_with_summary, assign_history_collections_in_db, assign_history_tags_in_db, clear_history_from_db, create_collection_in_db, delete_collection_from_db, delete_history_from_db, find_duplicate_downloads_in_history_db, get_collections_from_db, get_history_count_from_db, get_history_entries_by_ids_from_db, get_history_from_db, get_tags_from_db, remove_history_from_collection_in_db, remove_history_tag_from_db, rename_collection_in_db, update_history_filepath_and_title, update_history_filepath_and_title_by_id, update_history_summary};
use crate::types::{DownloadDuplicateIdentity, DownloadDuplicateMatch, HistoryAdvancedFilters, HistoryCollection, HistoryEntry, HistorySort, HistoryTag};

#[tauri::command]
pub fn add_history(url: String, title: String, thumbnail: Option<String>, filepath: String, filesize: Option<u64>, duration: Option<u64>, quality: Option<String>, format: Option<String>, source: Option<String>) -> Result<String, String> {
...
#[tauri::command]
pub fn add_summary_only_history(url: String, title: String, thumbnail: Option<String>, duration: Option<u64>, source: Option<String>, summary: String) -> Result<String, String> {
  add_history_with_summary(url, title, thumbnail, duration, source, summary)
}

#[tauri::command]
pub fn get_history(limit: Option<i64>, offset: Option<i64>, source: Option<String>, search: Option<String>, filters: Option<HistoryAdvancedFilters>, sort: Option<HistorySort>) -> Result<Vec<HistoryEntry>, String> {
...
#[tauri::command]
pub fn find_duplicate_downloads(identities: Vec<DownloadDuplicateIdentity>) -> Result<Vec<DownloadDuplicateMatch>, String> {
...
pub fn delete_history(id: String, delete_file: Option<bool>) -> Result<(), String> {
  if delete_file.unwrap_or(false) {
    let entry = get_history_entries_by_ids_from_db(vec![id.clone()])?.into_iter().next().ok_or_else(|| "History entry not found".to_string())?;
...
fn delete_history_media_file(filepath: &str) -> Result<(), String> {
  ...
  let metadata = std::fs::symlink_metadata(path).map_err(|e| format!("Failed to inspect media file before deleting: {}", e))?;
  if metadata.is_dir() {
...
#[tauri::command]
pub fn get_history_count(source: Option<String>, search: Option<String>, filters: Option<HistoryAdvancedFilters>) -> Result<i64, String> {
...
#[tauri::command]
pub fn assign_history_collections(history_id: String, collection_ids: Vec<String>) -> Result<(), String> {
...
#[tauri::command]
pub fn remove_history_from_collection(history_id: String, collection_id: String) -> Result<(), String> {
...
#[tauri::command]
pub fn rename_downloaded_file(filepath: String, new_name: String, history_id: Option<String>) -> Result<RenameDownloadedFileResult, String> {
  ...
  if new_path == old_path {
    return Ok(RenameDownloadedFileResult { new_filepath: filepath, new_title });
  }
  ...
  let new_filepath = new_path.to_str().ok_or_else(|| "New file path contains invalid UTF-8".to_string())?.to_string();

  let update_result = if let Some(id) = history_id { update_history_filepath_and_title_by_id(id, new_filepath.clone(), new_title.clone()) } else { update_history_filepath_and_title(filepath.clone(), new_filepath.clone(), new_title.clone()) };
  ...
  Ok(RenameDownloadedFileResult { new_filepath, new_title })
}
...
pub async fn open_file_location(filepath: String) -> Result<(), String> {
  ...
  #[cfg(target_os = "macos")]
  {
    if is_dir {
      ...
      tokio::process::Command::new("open").arg(&filepath).spawn().map_err(|e| format!("Failed to open folder: {}", e))?;
    } else {
      // Reveal file in Finder
      tokio::process::Command::new("open").arg("-R").arg(&filepath).spawn().map_err(|e| format!("Failed to open location: {}", e))?;
    }
  }
  ...
  #[cfg(target_os = "windows")]
  {
    if is_dir {
      ...
      tokio::process::Command::new("explorer").arg(&filepath).spawn().map_err(|e| format!("Failed to open folder: {}", e))?;
    } else {
      // Select file in Explorer
      tokio::process::Command::new("explorer").arg("/select,").arg(&filepath).spawn().map_err(|e| format!("Failed to open location: {}", e))?;
    }
  }
  ...
  #[cfg(target_os = "linux")]
  {
    if is_dir {
      ...
      tokio::process::Command::new("xdg-open").arg(&filepath).spawn().map_err(|e| format!("Failed to open folder: {}", e))?;
    } else {
      // Open parent directory (Linux doesn't have native file select)
      let dir = path.parent().unwrap_or(path);
      tokio::process::Command::new("xdg-open").arg(dir).spawn().map_err(|e| format!("Failed to open location: {}", e))?;
    }
  }
...
pub async fn open_macos_privacy_settings() -> Result<(), String> {
  #[cfg(target_os = "macos")]
  {
    ...
    tokio::process::Command::new("open").arg("x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles").spawn().map_err(|e| format!("Failed to open Privacy Settings: {}", e))?;
    return Ok(());
  }
...
mod tests {
  ...
  fn build_renamed_path_keeps_extension() {
    ...
    let (new_path, new_title) = build_renamed_path(&old, "new video").expect("build renamed path");
  ...
  fn build_renamed_path_fails_for_missing_file() {
    let missing = std::env::temp_dir().join(format!("youwee-missing-{}.mp4", uuid::Uuid::new_v4()));
  ...
  fn delete_history_media_file_ignores_missing_file() {
    ...
    delete_history_media_file(missing.to_str().expect("utf8 path")).expect("missing media file should not fail");
  }
  ...
  fn delete_history_media_file_rejects_directory() {
    ...
    let error = delete_history_media_file(dir.to_str().expect("utf8 path")).expect_err("reject dir");
  ...
  fn ensure_test_history_table() {
    ...
    conn.execute("DELETE FROM history", []).expect("clear history table");
  }
  ...
  fn rename_downloaded_file_updates_history_by_id() {
    ...
    {
      let conn = get_db().expect("get db");
      conn.execute("INSERT INTO history (id, url, title, filepath, downloaded_at) VALUES (?1, ?2, ?3, ?4, ?5)", params![history_id, "https://example.com/v", "video", old_path, 0_i64]).expect("insert history row");
    }

    let result = rename_downloaded_file(old.to_string_lossy().to_string(), "renamed".to_string(), Some(history_id.clone())).expect("rename should succeed");

    let (db_title, db_filepath): (String, String) = {
      let conn = get_db().expect("get db");
      conn.query_row("SELECT title, filepath FROM history WHERE id = ?1", params![history_id], |row| Ok((row.get(0)?, row.get(1)?))).expect("query updated history row")
    };
    ...
    let _ = fs::remove_dir_all(Path::new(&result.new_filepath).parent().unwrap_or_else(|| Path::new("/")));
  }
  ...
  fn rename_downloaded_file_updates_history_by_filepath_fallback() {
    ...
    {
      let conn = get_db().expect("get db");
      conn.execute("INSERT INTO history (id, url, title, filepath, downloaded_at) VALUES (?1, ?2, ?3, ?4, ?5)", params![history_id, "https://example.com/m", "movie", old_path, 0_i64]).expect("insert history row");
    }

    let result = rename_downloaded_file(old.to_string_lossy().to_string(), "movie-new".to_string(), None).expect("rename should succeed");
