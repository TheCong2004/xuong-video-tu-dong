...
use crate::services::{extract_audio_for_whisper, get_ffmpeg_path, run_ytdlp_with_stderr_and_cookies, transcribe_audio, WhisperError, WhisperResponseFormat};
use std::path::Path;
use tauri::AppHandle;
use uuid::Uuid;
...
pub async fn transcribe_video_with_whisper(
  ...
  let audio_path = if ["mp4", "mkv", "webm", "avi", "mov", "flv"].contains(&extension.as_str()) {
    ...
    extract_audio_for_whisper(&video_path, &audio_output_str, ffmpeg_path_str.as_deref()).await.map_err(|e| e.to_string())?;
  ...
  let result = transcribe_audio(&openai_api_key, &audio_path, format, language.as_deref(), whisper_endpoint_url.as_deref(), whisper_model.as_deref()).await.map_err(|e| e.to_string())?;
...
pub async fn transcribe_url_with_whisper(
  ...
  let _result = run_ytdlp_with_stderr_and_cookies(&app, &download_args, cookie_mode.as_deref(), cookie_browser.as_deref(), cookie_browser_profile.as_deref(), cookie_file_path.as_deref(), cookie_skip_patterns.as_deref(), proxy_url.as_deref()).await.map_err(|e| format!("Failed to download audio: {}", e))?;
  ...
  let metadata = std::fs::metadata(&audio_file).map_err(|e| format!("Failed to get file metadata: {}", e))?;
  ...
  if metadata.len() > max_size {
    ...
    extract_audio_for_whisper(&audio_file, &compressed_str, ffmpeg_path_str.as_deref()).await.map_err(|e| e.to_string())?;
  ...
  let whisper_result = transcribe_audio(&openai_api_key, &audio_file, format, language.as_deref(), whisper_endpoint_url.as_deref(), whisper_model.as_deref()).await.map_err(|e| e.to_string())?;

  // Clean up temp files
  std::fs::remove_dir_all(&temp_dir).ok();

  add_log_internal("success", &format!("Whisper transcription complete ({} chars)", whisper_result.text.len()), None, Some(&url)).ok();
...
pub async fn generate_subtitles_with_whisper(
  ...
  // Transcribe with Whisper
  let subtitle_content = transcribe_video_with_whisper(app, video_path.clone(), format.to_string(), openai_api_key, language, whisper_endpoint_url, whisper_model).await?;
  ...
  // Save subtitle file
  std::fs::write(&output_path, &subtitle_content).map_err(|e| format!("Failed to save subtitle file: {}", e))?;

  add_log_internal("success", &format!("Saved subtitles to: {}", output_str), None, None).ok();
