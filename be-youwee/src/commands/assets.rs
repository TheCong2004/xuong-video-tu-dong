...
pub fn allow_asset_file(app: AppHandle, path: String) -> Result<(), String> {
  ...
  if normalized.is_dir() {
    scopes.allow_directory(&normalized, true).map_err(|e| format!("Failed to allow asset directory {}: {}", normalized.display(), e))?;
  } else {
    scopes.allow_file(&normalized).map_err(|e| format!("Failed to allow asset file {}: {}", normalized.display(), e))?;
  }
...
pub fn sync_asset_scope_paths(app: AppHandle, paths: Vec<String>) -> Result<usize, String> {
  ...
  for directory in &directories {
    scopes.allow_directory(directory, true).map_err(|e| format!("Failed to allow asset directory {}: {}", directory.display(), e))?;
  }
...
mod tests {
  ...
  fn collect_scope_directories_dedupes_parent_directories() {
    let directories = collect_scope_directories(&[String::from(r"\\?\D:\Music\track.mp3"), String::from(r"D:\Music\track.mp3"), String::from(r"D:\Music"), String::from(r"C:\Users\86153\Downloads\song.mp3")]);

    assert_eq!(directories, vec![PathBuf::from(r"C:\Users\86153\Downloads"), PathBuf::from(r"D:\Music"),]);
  }
}
