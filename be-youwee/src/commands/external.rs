...
mod tests {
  ...
  fn deep_link_extraction_stays_string_based() {
    let argv = vec!["youwee".to_string(), "youwee://download?v=1&url=https%3A%2F%2Fexample.com%2Fvideo".to_string()];

    let links = extract_external_links_from_argv(&argv);

    assert_eq!(links, vec!["youwee://download?v=1&url=https%3A%2F%2Fexample.com%2Fvideo"]);
  }

  #[test]
  fn summary_deep_link_extraction_uses_same_pending_queue() {
    let argv = vec!["youwee".to_string(), "youwee://summary?v=1&url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3Dabc123".to_string()];

    let links = extract_external_links_from_argv(&argv);

    assert_eq!(links, vec!["youwee://summary?v=1&url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3Dabc123"]);
  }

  #[test]
  fn pending_external_links_are_deduped_and_drained() {
    let _ = take_pending_external_links();
    let link = "youwee://download?v=1&url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3Dabc123".to_string();

    enqueue_external_links(vec![link.clone(), link.clone()]);

    assert_eq!(take_pending_external_links(), vec![link]);
    assert!(take_pending_external_links().is_empty());
  }
}
