...
#[tauri::command]
pub async fn execute_ffmpeg_command(app: AppHandle, job_id: String, command_args: Vec<String>, input_path: String, output_path: String) -> Result<(), String> {
    ...
    while let Ok(Some(line)) = reader.next_line().await {
      ...
      if line.starts_with("frame=") {
        if let Some(val) = line.strip_prefix("frame=") {
          current_frame = val.trim().split_whitespace().next().and_then(|s| s.parse().ok()).unwrap_or(current_frame);
        }
      } else if line.starts_with("fps=") {
        if let Some(val) = line.strip_prefix("fps=") {
          current_fps = val.trim().split_whitespace().next().and_then(|s| s.parse().ok()).unwrap_or(current_fps);
        }
        ...
        if let Some(val) = line.strip_prefix("out_time=") {
          ...
          current_time = if let Some(dot_pos) = time_str.find('.') { time_str[..dot_pos].to_string() } else { time_str.to_string() };
        }
        ...
        println!("[FFMPEG PROGRESS] time_secs={}, duration={}, percent={:.1}%", current_time_secs, total_duration_secs, percent);

        let progress = ProcessingProgress { job_id: job_id_clone.clone(), percent, frame: current_frame, total_frames, fps: current_fps, speed: current_speed.clone(), time: current_time.clone(), size: current_size.clone() };
...
pub async fn get_processing_history(_app: AppHandle, limit: i32) -> Result<Vec<ProcessingJob>, String> {
  ...
  let jobs = stmt.query_map(params![limit], |row| Ok(ProcessingJob { id: row.get(0)?, input_path: row.get(1)?, output_path: row.get(2)?, task_type: row.get(3)?, user_prompt: row.get(4)?, ffmpeg_command: row.get(5)?, status: row.get(6)?, progress: row.get(7)?, error_message: row.get(8)?, created_at: row.get(9)?, completed_at: row.get(10)? })).map_err(|e| format!("Query failed: {}", e))?.filter_map(|r| r.ok()).collect();
...
pub async fn delete_processing_job(_app: AppHandle, id: String) -> Result<(), String> {
  ...
  conn.execute("DELETE FROM processing_jobs WHERE id = ?1", params![id]).map_err(|e| format!("Failed to delete job: {}", e))?;
...
pub async fn clear_processing_history(_app: AppHandle) -> Result<u64, String> {
  ...
  let deleted = conn.execute("DELETE FROM processing_jobs", []).map_err(|e| format!("Failed to clear history: {}", e))?;

  Ok(deleted as u64)
}

#[tauri::command]
pub async fn save_processing_job(_app: AppHandle, id: String, input_path: String, output_path: Option<String>, task_type: String, user_prompt: Option<String>, ffmpeg_command: String) -> Result<(), String> {
  let conn = get_db()?;
...
#[tauri::command]
pub async fn update_processing_job(_app: AppHandle, id: String, status: String, progress: f64, error_message: Option<String>) -> Result<(), String> {
  let conn = get_db()?;

  if status == "completed" || status == "failed" || status == "cancelled" {
    let completed_at = chrono::Utc::now().to_rfc3339();
    conn.execute("UPDATE processing_jobs SET status = ?1, progress = ?2, error_message = ?3, completed_at = ?4 WHERE id = ?5", params![status, progress, error_message, completed_at, id]).map_err(|e| format!("Failed to update job: {}", e))?;
  } else {
    conn.execute("UPDATE processing_jobs SET status = ?1, progress = ?2, error_message = ?3 WHERE id = ?4", params![status, progress, error_message, id]).map_err(|e| format!("Failed to update job: {}", e))?;
  }
...
pub async fn get_processing_presets(_app: AppHandle) -> Result<Vec<ProcessingPreset>, String> {
  ...
  let presets = stmt.query_map([], |row| Ok(ProcessingPreset { id: row.get(0)?, name: row.get(1)?, description: row.get(2)?, task_type: row.get(3)?, prompt_template: row.get(4)?, icon: row.get(5)?, created_at: row.get(6)? })).map_err(|e| format!("Query failed: {}", e))?.filter_map(|r| r.ok()).collect();

  Ok(presets)
}

#[tauri::command]
pub async fn save_processing_preset(_app: AppHandle, name: String, description: Option<String>, prompt_template: String, task_type: String) -> Result<(), String> {
  let conn = get_db()?;
...
pub async fn delete_processing_preset(_app: AppHandle, id: String) -> Result<(), String> {
  ...
  conn.execute("DELETE FROM processing_presets WHERE id = ?1", params![id]).map_err(|e| format!("Failed to delete preset: {}", e))?;
