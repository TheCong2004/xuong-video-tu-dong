...
pub async fn get_video_metadata(app: AppHandle, path: String) -> Result<VideoMetadata, String> {
  let ffprobe_path = get_ffprobe_path(&app).await.ok_or("FFprobe not found. Please install FFmpeg.")?;
  ...
  let json: serde_json::Value = serde_json::from_str(&json_str).map_err(|e| format!("Failed to parse ffprobe output: {}", e))?;
  ...
  let duration = format.get("duration").and_then(|d| d.as_str()).and_then(|d| d.parse::<f64>().ok()).unwrap_or(0.0);
  ...
  let audio_codec = audio_stream.and_then(|a| a.get("codec_name")).and_then(|c| c.as_str()).unwrap_or("none").to_string();

  let bitrate = format.get("bit_rate").and_then(|b| b.as_str()).and_then(|b| b.parse::<i64>().ok()).unwrap_or(0) / 1000;

  let file_size = format.get("size").and_then(|s| s.as_str()).and_then(|s| s.parse::<i64>().ok()).unwrap_or(0);

  let format_name = format.get("format_name").and_then(|f| f.as_str()).unwrap_or("unknown").split(',').next().unwrap_or("unknown").to_string();

  Ok(VideoMetadata { path, filename, duration, width, height, fps, video_codec, audio_codec, bitrate, file_size, format: format_name, has_audio: audio_stream.is_some() })
}

/// Detect shot changes using FFmpeg scene detection filter.
#[tauri::command]
pub async fn detect_shot_changes(app: AppHandle, path: String, threshold: Option<f64>, min_interval_ms: Option<i64>) -> Result<ShotDetectionResult, String> {
  let input_path = Path::new(&path);
  if !input_path.exists() {
    return Err(format!("Video not found: {}", path));
  }

  let ffmpeg_path = get_ffmpeg_path(&app).await.ok_or("FFmpeg not found. Please install FFmpeg from Settings > Dependencies.")?;
  ...
  cmd.args(["-hide_banner", "-i", &path, "-vf", &scene_filter, "-an", "-f", "null", "-"]);
  cmd.hide_window();

  let output = cmd.output().await.map_err(|e| format!("Failed to run FFmpeg shot detection: {}", e))?;
  ...
  let stderr = String::from_utf8_lossy(&output.stderr);
  let re = regex::Regex::new(r"pts_time:([0-9]+(?:\.[0-9]+)?)").map_err(|e| format!("Failed to build regex: {}", e))?;
  ...
  Ok(ShotDetectionResult { shot_times_ms: filtered, threshold: threshold_value, min_interval_ms: min_interval })
}
