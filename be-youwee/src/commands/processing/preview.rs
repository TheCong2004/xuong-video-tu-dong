...
fn needs_preview_transcode(codec: &str, container_format: &str) -> bool {
  ...
  let has_supported_container = format_lower.contains("mp4") || format_lower.contains("mov") || format_lower.contains("m4v") || format_lower.contains("m4a") || format_lower.contains("3gp");
...
#[tauri::command]
pub async fn generate_video_preview(app: AppHandle, input_path: String, video_codec: String, container_format: String) -> Result<String, String> {
  ...
  cmd.args(["-y", "-i", &input_path, "-vf", "scale=-2:720", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "28", "-r", "30", "-an", "-movflags", "+faststart", preview_path.to_str().unwrap()]).stdout(Stdio::piped()).stderr(Stdio::piped());
  cmd.hide_window();
...
#[tauri::command]
pub async fn check_preview_exists(app: AppHandle, input_path: String) -> Result<Option<String>, String> {
  let app_data_dir = app.path().app_data_dir().map_err(|_| "Failed to get app data directory")?;
  let preview_dir = app_data_dir.join("previews");
...
#[tauri::command]
pub async fn generate_video_thumbnail(app: AppHandle, input_path: String) -> Result<String, String> {
  let ffmpeg_path = get_ffmpeg_path(&app).await.ok_or_else(|| {
  ...
  let mut cmd = Command::new(&ffmpeg_path);
  cmd.args(["-y", "-ss", "1", "-i", &input_path, "-frames:v", "1", "-vf", "scale=-2:720", "-q:v", "2", thumb_path.to_str().unwrap()]).stdout(Stdio::piped()).stderr(Stdio::piped());
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| format!("Failed to run FFmpeg: {}", e))?;
...
pub async fn generate_audio_preview(app: AppHandle, input_path: String) -> Result<String, String> {
  ...
  let mut cmd = Command::new(&ffmpeg_path);
  cmd.args(["-y", "-i", &input_path, "-vn", "-c:a", "pcm_s16le", "-ar", "44100", "-ac", "1", audio_path.to_str().unwrap()]).stdout(Stdio::piped()).stderr(Stdio::piped());
  cmd.hide_window();
  let output = cmd.output().await.map_err(|e| format!("Failed to run FFmpeg: {}", e))?;
