...
pub async fn get_processing_attachment_info(path: String) -> Result<ProcessingAttachment, String> {
  ...
  let filename = file_path.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_else(|| "unknown".to_string());
  ...
  Ok(ProcessingAttachment { path, filename, kind, width, height, size: metadata.len(), format })
}
...
pub async fn get_image_metadata(path: String) -> Result<ImageInfo, String> {
  ...
  let file_size = std::fs::metadata(&path).map_err(|e| format!("Failed to read file metadata: {}", e))?.len();

  let filename = file_path.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_else(|| "unknown".to_string());

  let reader = image::ImageReader::open(&path).map_err(|e| format!("Failed to open image: {}", e))?.with_guessed_format().map_err(|e| format!("Failed to detect image format: {}", e))?;

  let format = reader.format().map(|f| format!("{:?}", f).to_lowercase()).unwrap_or_else(|| file_path.extension().map(|e| e.to_string_lossy().to_lowercase()).unwrap_or_else(|| "unknown".to_string()));

  let (width, height) = reader.into_dimensions().map_err(|e| format!("Failed to read image dimensions: {}", e))?;

  Ok(ImageInfo { path, filename, width, height, size: file_size, format })
}
