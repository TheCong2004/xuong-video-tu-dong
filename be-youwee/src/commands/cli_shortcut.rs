...
fn command_parent_is_in_path(command_path: &Path) -> bool {
  ...
  let parent_compare = if cfg!(windows) { parent_display.to_lowercase() } else { parent_display };
...
fn command_parent_path(command_path: &Path) -> String {
  command_path.parent().map(|p| p.display().to_string()).unwrap_or_else(|| command_path.display().to_string())
}

fn path_not_in_path_note(command_path: &Path) -> String {
  let parent = command_path.parent().map(|p| p.display().to_string()).unwrap_or_else(|| command_path.display().to_string());
  format!("{} exists, but {} is not in PATH.", command_path.display(), parent)
}
...
fn ensure_command_parent_is_in_path(command_path: &Path) -> Result<(), String> {
  ...
  let parent = command_path.parent().map(|p| p.display().to_string()).unwrap_or_else(|| command_path.display().to_string());
  Err(format!("Shortcut was created at {}, but {} is not in PATH. Add that folder to PATH to use the command from any directory.", command_path.display(), parent))
}

#[cfg(target_os = "windows")]
fn windows_apps_shim_path() -> Option<PathBuf> {
  std::env::var_os("LOCALAPPDATA").map(PathBuf::from).map(|dir| dir.join("Microsoft").join("WindowsApps").join("youwee.cmd"))
}
...
pub fn get_cli_shortcut_status() -> CliShortcutStatus {
  ...
  #[cfg(target_os = "macos")]
  {
    ...
    let note = if primary.exists() && !command_parent_is_in_path(&primary) { Some((path_not_in_path_note(&primary), command_parent_path(&primary))) } else { fallback.as_ref().filter(|p| p.exists() && !command_parent_is_in_path(p)).map(|p| (path_not_in_path_note(p), command_parent_path(p))) };
    return CliShortcutStatus { platform, installed, target_path, exe_path: exe_path_str, can_auto_install: true, note: note.as_ref().map(|(text, _)| text.clone()), note_key: note.as_ref().map(|_| "path_not_in_path".to_string()), note_path: note.map(|(_, path)| path) };
  }

  #[cfg(target_os = "windows")]
  {
    let shim = windows_apps_shim_path();
    let installed = shim.as_ref().map(|path| path.exists() && command_parent_is_in_path(path)).unwrap_or(false) || path_contains_exe_dir(exe_path.as_deref());
    let note = shim.as_ref().filter(|path| path.exists() && !command_parent_is_in_path(path)).map(|path| (path_not_in_path_note(path), command_parent_path(path)));
    return CliShortcutStatus { platform, installed, target_path: shim.or_else(|| exe_path.as_ref().and_then(|p| p.parent()).map(Path::to_path_buf)).map(|p| p.display().to_string()), exe_path: exe_path_str, can_auto_install: true, note: note.as_ref().map(|(text, _)| text.clone()), note_key: note.as_ref().map(|_| "path_not_in_path".to_string()), note_path: note.map(|(_, path)| path) };
  }
  ...
  #[cfg(target_os = "linux")]
  {
    ...
    let target_path = if system_installed { Some(system.display().to_string()) } else { local.as_ref().map(|p| p.display().to_string()) };
    return CliShortcutStatus {
      ...
      } else if let Some(path) = local.as_ref().filter(|p| p.exists() && !command_parent_is_in_path(p)) {
      ...
      } else if local.as_ref().map(|p| p.exists() && !command_parent_is_in_path(p)).unwrap_or(false) {
        Some("path_not_in_path".to_string())
      } else {
        None
      },
      note_path: if system_installed { Some("/usr/bin/youwee".to_string()) } else { local.as_ref().filter(|p| p.exists() && !command_parent_is_in_path(p)).map(|p| command_parent_path(p)) },
    };
  }

  #[allow(unreachable_code)]
  CliShortcutStatus { platform, installed: false, target_path: None, exe_path: exe_path_str, can_auto_install: false, note: Some("Unsupported platform.".to_string()), note_key: Some("unsupported".to_string()), note_path: None }
}
...
fn path_contains_exe_dir(exe: Option<&std::path::Path>) -> bool {
  ...
  std::env::var("PATH").map(|path| path.split(';').any(|entry| entry.trim().to_lowercase() == dir_norm)).unwrap_or(false)
}
...
fn create_symlink(exe: &Path, link: &Path) -> Result<(), String> {
  ...
  if let Ok(metadata) = std::fs::symlink_metadata(link) {
    if !metadata.file_type().is_symlink() {
      return Err(format!("{} already exists and is not a Youwee symlink. Remove it manually before reinstalling.", link.display()));
    }

    let existing_target = std::fs::read_link(link).map_err(|e| format!("Failed to inspect existing {}: {}", link.display(), e))?;
    if existing_target == exe {
      return Ok(());
    }

    std::fs::remove_file(link).map_err(|e| format!("Failed to replace existing symlink {}: {}", link.display(), e))?;
  }
...
fn install_macos_symlink(exe: &Path) -> Result<String, String> {
  ...
  let fallback = user_local_bin().ok_or_else(|| "Could not resolve HOME directory.".to_string())?.join("youwee");
...
fn install_linux_symlink(exe: &Path) -> Result<String, String> {
  ...
  let link = user_local_bin().ok_or_else(|| "Could not resolve HOME directory.".to_string())?.join("youwee");
...
fn install_windows_path(exe: &Path) -> Result<String, String> {
  if path_contains_exe_dir(Some(exe)) {
    return exe.parent().map(|dir| dir.display().to_string()).ok_or_else(|| "Could not resolve install directory.".to_string());
  }

  let shim = windows_apps_shim_path().ok_or_else(|| "Could not resolve the per-user WindowsApps command directory.".to_string())?;
  ...
  std::fs::write(&shim, content).map_err(|e| format!("Failed to write {}: {}", shim.display(), e))?;
